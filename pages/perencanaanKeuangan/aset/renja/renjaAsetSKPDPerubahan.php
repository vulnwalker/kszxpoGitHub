...
class renjaAsetSKPDPerubahanObj  extends DaftarObj2{
	var $Prefix = 'renjaAsetSKPDPerubahan';
	var $elCurrPage="HalDefault";
	var $SHOW_CEK = TRUE;
	var $TblName = 'view_renja'; //bonus
	var $TblName_Hapus = 'renjaAsetSKPDPerubahan';
	var $MaxFlush = 10;
	...
	var $pagePerHal ='';
	var $fileNameExcel='renjaAsetSKPDPerubahan.xls';
	var $namaModulCetak='DAFTAR renjaAsetSKPDPerubahan';
	var $Cetak_Judul = 'DAFTAR renjaAsetSKPDPerubahan';
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'renjaAsetSKPDPerubahanForm';
	...
	function set_selector_other($tipe){
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){
		...
		case 'lihatrenjaAsetSKPDPerubahan':{
			$fm = $this->setFormEdit();
			$cek = $fm['cek'];
			$err = $fm['err'];
			$content = $fm['content'];
		...
		case 'newTab':{
			 ...
			 foreach ($_REQUEST as $key => $value) {
				...
				$ID_PLAFON = $grabID['id'];
				$content = array('idrenjaAsetSKPDPerubahan' => $ID_PLAFON, "status" => $statunya );
			break;
		    }
			...
			case 'editTab':{
			 ...
			 $urutArray = explode(" ",$renjaAsetSKPDPerubahan_cb[0]);
			 $urut = $renjaAsetSKPDPerubahan_cb[0];
			 $query = "select * from view_renja where id_anggaran = '$renjaAsetSKPDPerubahan_cb[0]' ";
			 $getViewrenjaAsetSKPDPerubahan = mysql_fetch_array(mysql_query($query));
			 foreach ($getViewrenjaAsetSKPDPerubahan as $key => $value) {
				  $$key = $value;
			 }
			 $IDEDIT = $id_anggaran;
			 ...
			 $content = array('ID_PLAFON' => $ID_PLAFON,  'ID_EDIT' => $renjaAsetSKPDPerubahan_cb[0] , 'query ' =>"select * from view_renja where jenis_anggaran = '$this->jenisAnggaran' and tahun = '$this->tahun' and c1 = '$c1' and c = '$c' and d='$d' and e='00' and e1='000' and bk='0' and ck = '0' and dk = '0' and p ='0' and q = '0' and no_urut = '$nomor' ");
			...
			case 'jenisChanged':{
				...
				if($jenisKegiatan == 'baru'){
					...
					$plus = "<input type='text' name ='plus' id ='plus' style='width:200px; text-align:right;'  onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='document.getElementById(`keyPP`).textContent = `Rp. ` + popupProgramrenjaAsetSKPDPerubahan.formatCurrency(this.value);' > ";
					$minus = "<input type='text' name ='minus' id ='minus' style='width:200px; text-align:right;'  onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='document.getElementById(`keyMM`).textContent = `Rp. ` + popupProgramrenjaAsetSKPDPerubahan.formatCurrency(this.value);'> ";
				}
		...
		case 'SaveValid':{
	    	foreach ($_REQUEST as $key => $value) {
			 ...
			 $getSKPD = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$renjaAsetSKPDPerubahan_idplh'"));
			 ...
			 $query = VulnWalkerUpdate("tabel_anggaran",$data," id_anggaran = '$renjaAsetSKPDPerubahan_idplh'");
			 mysql_query($query);
	 ...
	 function setPage_OtherScript(){
	  ...
	  return
	    ...
	    <script type='text/javascript' src='js/perencanaanKeuangan/aset/renja/renjaAsetSKPDPerubahan.js' language='JavaScript' ></script>
	    <script type='text/javascript' src='js/perencanaan_v3/renja/renjaAsetSKPDPerubahan_ins.js' language='JavaScript' ></script>
	...
	function setFormBaru(){
  	...
  	function setFormEdit(){
		...
		if($cnt['cnt'] > 0) $err = "renjaAsetSKPDPerubahan Tidak Bisa Diubah ! Sudah Digunakan Di Ref Barang.";
		if($err == ''){
	...
	function Info(){
	 ...
	 $angkaPlafon = number_format($getAngkaPlafon['plafon'],2,',','.');
	 $codeAndNameUrusan = "select c1, concat(c1, '. ', nm_skpd) from ref_skpd where c='00' and d='00' and e='00' and e1='000' ";
	 $urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=renjaAsetSKPDPerubahan.comboChanged(); disabled','-- URUSAN --');

	 $codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
	 $bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=renjaAsetSKPDPerubahan.comboChanged(); disabled','-- BIDANG --');

	 $codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
	 $skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=renjaAsetSKPDPerubahan.comboChanged(); disabled' ,'-- SKPD --');
	...
	$fmBIDANG = cekPOST('fmBIDANG');
	 ...
	 $arr = array(
			...
			array('nama_renjaAsetSKPDPerubahan','NAMA renjaAsetSKPDPerubahan'),
			array('alamat','ALAMAT'),
	 ...
	 //data order ------------------------------
	 $arrOrder = array(
			     	array('1','NAMA renjaAsetSKPDPerubahan'),
					array('2','ALAMAT'),
	...
	"<div class='FilterBar' style='margin-top:10px;'><table style='width:100%'>".
			CmbUrusanBidangSkpd('renjaAsetSKPDPerubahan').
"</table></div>";
...
$renjaAsetSKPDPerubahan = new renjaAsetSKPDPerubahanObj();

$arrayResult = tahapPerencanaanV3($renjaAsetSKPDPerubahan->modul,'PERUBAHAN');
...
$renjaAsetSKPDPerubahan->jenisForm = $jenisForm;
$renjaAsetSKPDPerubahan->nomorUrut = $nomorUrut;
$renjaAsetSKPDPerubahan->tahun = $tahun;
$renjaAsetSKPDPerubahan->jenisAnggaran = $jenisAnggaran;
$renjaAsetSKPDPerubahan->idTahap = $idTahap;
$renjaAsetSKPDPerubahan->username = $_COOKIE['coID'];
$renjaAsetSKPDPerubahan->wajibValidasi = $arrayResult['wajib_validasi'];
$renjaAsetSKPDPerubahan->settingAnggaran = $arrayResult['settingAnggaran'];
