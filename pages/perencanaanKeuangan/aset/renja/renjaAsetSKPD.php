...
class renjaAsetSKPDObj  extends DaftarObj2{
	var $Prefix = 'renjaAsetSKPD';
	var $elCurrPage="HalDefault";
	var $SHOW_CEK = TRUE;
	var $TblName = 'view_renja'; //bonus
	var $TblName_Hapus = 'renjaAsetSKPD';
	var $MaxFlush = 10;
	...
	var $pagePerHal ='';
	var $fileNameExcel='renjaAsetSKPD.xls';
	var $namaModulCetak='DAFTAR renjaAsetSKPD';
	var $Cetak_Judul = 'DAFTAR renjaAsetSKPD';
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'renjaAsetSKPDForm';
	...
	var $currentTahap = "";
	var $settingAnggaran = "";
	var $reportURLAset = "pages.php?Pg=renjaAsetSKPD&tipe=Laporan";
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){
		...
		case 'postReport':{
			...
			mysql_query($query);
			$content= array('urusan'=>$fmSKPDUrusan,'bidang'=>$fmSKPDBidang,'skpd'=>$fmSKPDskpd);
		break;
		}
    ...
    $arrayTandaTanganAset = explode(';', $getJenisReportAset['tanda_tangan']);
		...
		case 'newTab':{
			 ...
			 foreach ($_REQUEST as $key => $value) {
				...
				$ID_PLAFON = $grabID['id'];
				$content = array('idrenjaAsetSKPD' => $ID_PLAFON, "status" => $statunya );
			break;
		    }
			...
			case 'editTab':{
			 ...
			 $urutArray = explode(" ",$renjaAsetSKPD_cb[0]);
			 $urut = $renjaAsetSKPD_cb[0];
			 $query = "select * from view_renja where id_anggaran = '$renjaAsetSKPD_cb[0]' ";
			 $getViewrenjaAsetSKPD = mysql_fetch_array(mysql_query($query));
			 foreach ($getViewrenjaAsetSKPD as $key => $value) {
				  $$key = $value;
			 }
			 $IDEDIT = $id_anggaran;
			 ...
			 $content = array('ID_PLAFON' => $ID_PLAFON,  'ID_EDIT' => $renjaAsetSKPD_cb[0] , 'query ' =>"select * from view_renja where jenis_anggaran = '$this->jenisAnggaran' and tahun = '$this->tahun' and c1 = '$c1' and c = '$c' and d='$d' and e='00' and e1='000' and bk='0' and ck = '0' and dk = '0'  and p ='0' and q = '0' and no_urut = '$nomor' ");
			...
			case 'jenisChanged':{
				...
				if($jenisKegiatan == 'baru'){
					...
					$plus = "<input type='text' name ='plus' id ='plus' style='width:200px; text-align:right;'  onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='document.getElementById(`keyPP`).textContent = `Rp. ` + popupProgramrenjaAsetSKPD.formatCurrency(this.value);' > ";
					$minus = "<input type='text' name ='minus' id ='minus' style='width:200px; text-align:right;'  onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='document.getElementById(`keyMM`).textContent = `Rp. ` + popupProgramrenjaAsetSKPD.formatCurrency(this.value);'> ";
				}
		...
		case 'SaveValid':{
	    	foreach ($_REQUEST as $key => $value) {
			 ...
			 $getSKPD = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$renjaAsetSKPD_idplh'"));
			 ...
			 $query = VulnWalkerUpdate("tabel_anggaran",$data," id_anggaran = '$renjaAsetSKPD_idplh'");
			 mysql_query($query);
	 ...
	 function setPage_OtherScript(){
	  ...
	  return
	    ...
	    <script type='text/javascript' src='js/perencanaanKeuangan/aset/renja/renjaAsetSKPD.js' language='JavaScript' ></script>
	    <script type='text/javascript' src='js/perencanaanKeuangan/aset/renja/renjaAsetSKPD_ins.js' language='JavaScript' ></script>
	...
	function setFormBaru(){
  	...
  	function setFormEdit(){
		...
		if($cnt['cnt'] > 0) $err = "renjaAsetSKPD Tidak Bisa Diubah ! Sudah Digunakan Di Ref Barang.";
		if($err == ''){
	...
	function Info(){
	 ...
	 $angkaPlafon = number_format($getAngkaPlafon['plafon'],2,',','.');
	 $codeAndNameUrusan = "select c1, concat(c1, '. ', nm_skpd) from ref_skpd where c='00' and d='00' and e='00' and e1='000' ";
	 $urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=renjaAsetSKPD.comboChanged(); disabled','-- URUSAN --');

	 $codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
	 $bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=renjaAsetSKPD.comboChanged(); disabled','-- BIDANG --');

	 $codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
	 $skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=renjaAsetSKPD.comboChanged(); disabled' ,'-- SKPD --');
	...
	$fmBIDANG = cekPOST('fmBIDANG');
	 ...
	 $arr = array(
			...
			array('nama_renjaAsetSKPD','NAMA renjaAsetSKPD'),
			array('alamat','ALAMAT'),
	...
	"<div class='FilterBar' style='margin-top:10px;'><table style='width:100%'>".
			CmbUrusanBidangSkpd('renjaAsetSKPD').
"</table></div>";
...
$renjaAsetSKPD = new renjaAsetSKPDObj();

$arrayResult = tahapPerencanaanV3($renjaAsetSKPD->modul,"MURNI");
...
$renjaAsetSKPD->jenisForm = $jenisForm;
$renjaAsetSKPD->nomorUrut = $nomorUrut;
$renjaAsetSKPD->tahun = $tahun;
$renjaAsetSKPD->jenisAnggaran = $jenisAnggaran;
$renjaAsetSKPD->idTahap = $idTahap;
$renjaAsetSKPD->username = $_COOKIE['coID'];
$renjaAsetSKPD->wajibValidasi = $arrayResult['wajib_validasi'];
$renjaAsetSKPD->settingAnggaran = $arrayResult['settingAnggaran'];
