...
class renjaPerubahan_v3Obj  extends DaftarObj2{	
	var $Prefix = 'renjaPerubahan_v3';
	var $elCurrPage="HalDefault";
	var $SHOW_CEK = TRUE;
	var $TblName = 'view_renja'; //bonus
	var $TblName_Hapus = 'renjaPerubahan_v3';
	var $MaxFlush = 10;
	...
	var $pagePerHal ='';
	var $fileNameExcel='renjaPerubahan_v3.xls';
	var $namaModulCetak='DAFTAR renjaPerubahan_v3';
	var $Cetak_Judul = 'DAFTAR renjaPerubahan_v3';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'renjaPerubahan_v3Form';
	  ...
	  switch($tipe){	
		...
		case 'newTab':{
			 ...
			 foreach ($_REQUEST as $key => $value) { 
				...
				$ID_PLAFON = $grabID['id'];
				$content = array('idrenjaPerubahan_v3' => $ID_PLAFON, "status" => $statunya );
			break;
		    }
			...
			case 'editTab':{
			 ...
			 $urutArray = explode(" ",$renjaPerubahan_v3_cb[0]);
			 $urut = $renjaPerubahan_v3_cb[0];
			 $query = "select * from view_renja where id_anggaran = '$renjaPerubahan_v3_cb[0]' ";
			 $getViewrenjaPerubahan_v3 = mysql_fetch_array(mysql_query($query)); 
			 foreach ($getViewrenjaPerubahan_v3 as $key => $value) { 
				  $$key = $value; 
			 }
			 $IDEDIT = $id_anggaran;
			 ...
			 $content = array('ID_PLAFON' => $ID_PLAFON,  'ID_EDIT' => $renjaPerubahan_v3_cb[0] , 'query ' =>"select * from view_renja where jenis_anggaran = '$this->jenisAnggaran' and tahun = '$this->tahun' and c1 = '$c1' and c = '$c' and d='$d' and e='00' and e1='000' and bk='0' and ck = '0'  and p ='0' and q = '0' and no_urut = '$nomor' ");
			...
			case 'jenisChanged':{
				...
				if($jenisKegiatan == 'baru'){
					...
					$plus = "<input type='text' name ='plus' id ='plus' style='width:200px; text-align:right;'  onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='document.getElementById(`keyPP`).textContent = `Rp. ` + popupProgramrenjaPerubahan_v3.formatCurrency(this.value);' > ";
					$minus = "<input type='text' name ='minus' id ='minus' style='width:200px; text-align:right;'  onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='document.getElementById(`keyMM`).textContent = `Rp. ` + popupProgramrenjaPerubahan_v3.formatCurrency(this.value);'> ";
				}
		...
		case 'SaveValid':{
	    	foreach ($_REQUEST as $key => $value) { 
			 ...
			 $getSKPD = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$renjaPerubahan_v3_idplh'"));
			 ...
			 $query = VulnWalkerUpdate("tabel_anggaran",$data," id_anggaran = '$renjaPerubahan_v3_idplh'");
			 mysql_query($query);
   ...
   function setPage_OtherScript(){
	  ...
	  /* if($this->jenisForm == "KOREKSI"){
		...
		return 	
			...
			<script type='text/javascript' src='js/perencanaan_v3/renja/renjaPerubahan.js' language='JavaScript' ></script>
			<script type='text/javascript' src='js/perencanaan_v3/renja/renjaPerubahan_ins.js' language='JavaScript' ></script>
	...
	function setFormBaru(){
  	...
  	function setFormEdit(){
		...
		if($cnt['cnt'] > 0) $err = "renjaPerubahan_v3 Tidak Bisa Diubah ! Sudah Digunakan Di Ref Barang.";
		if($err == ''){
	...
	function Info(){	
	 ...
	 $angkaPlafon = number_format($getAngkaPlafon['plafon'],2,',','.');
	 $codeAndNameUrusan = "select c1, concat(c1, '. ', nm_skpd) from ref_skpd where c='00' and d='00' and e='00' and e1='000' ";
	 $urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=renjaPerubahan_v3.comboChanged(); disabled','-- URUSAN --');
	
	 $codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
	 $bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=renjaPerubahan_v3.comboChanged(); disabled','-- BIDANG --');
	
	 $codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
	 $skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=renjaPerubahan_v3.comboChanged(); disabled' ,'-- SKPD --');
	 ...
	 $NomorColSpan = $Mode==1? 2: 1;
	  ...
	  if($this->jenisForm == "PENYUSUNAN"){
		  ...
		  "<thead>
		   ...
		   <th class='th01' width='100'>UNIT KERJA PELAKSANA</th>
		   $tergantung
		   </tr>
		   </thead>";
	  }elseif($this->jenisForm == "KOREKSI"){
	...
	function setKolomData($no, $isi, $Mode, $TampilCheckBox){
	 ...
	 if($this->jenisForm == "PENYUSUNAN"){
		 ...
		 if($q!='0' && $e1 !='000')$unitPelaksana = $getUnitPelaksana['nm_skpd'];
		  $Koloms[] = array('align="left"', $unitPelaksana );
		 if ($status_validasi =='1') {
			 	$validnya = "valid.png";
			 }else{
	...
	$fmBIDANG = cekPOST('fmBIDANG');
	 ...
	 $arr = array(
			...
			array('nama_renjaPerubahan_v3','NAMA renjaPerubahan_v3'),		
			array('alamat','ALAMAT'),	
	 ...
	 //data order ------------------------------
	 $arrOrder = array(
			     	array('1','NAMA renjaPerubahan_v3'),		
					array('2','ALAMAT'),	
	...
	"<div class='FilterBar' style='margin-top:10px;'><table style='width:100%'>".
			CmbUrusanBidangSkpd('renjaPerubahan_v3').
"</table></div>";
		...
		if($this->settingAnggaran != "MURNI"){
			...
			while($rows = mysql_fetch_array($grabAllData)){
				foreach ($rows as $key => $value) { 
				 ...
				 if(mysql_num_rows(mysql_query("select * from view_renja where c1 ='$c1' and c= '$c' and d='$d' and e='$e' and e1='$e1' and bk='$bk' and ck='$ck' and p ='$p' and q='0' and id_tahap = '$this->idTahap'")) == 0){
				 		$data = array(
									    'c1' => $c1,
										'c' => $c,
										'd' => $d,
				 ...
				 if(mysql_num_rows(mysql_query("select * from view_renja where c1 ='$c1' and c= '$c' and d='$d' and e='$e' and e1='$e1' and bk='$bk' and ck='$ck' and p ='$p' and q='$q' and id_tahap = '$this->idTahap'")) == 0){
				 		$data = array(
									    'c1' => $c1,
										'c' => $c,
										'd' => $d,
						...
						$query = VulnWalkerInsert("tabel_anggaran", $data);
						mysql_query($query);
						$getIdBarusan = mysql_fetch_array(mysql_query("select * from view_renja where c1 ='$c1' and c= '$c' and d='$d' and e='$e' and e1='$e1' and bk='$bk' and ck='$ck' and p ='$p' and q='$q' and id_tahap = '$this->idTahap'"));
						...
						while($Skpd= mysql_fetch_array($getSkpd)){
							...
							if(mysql_num_rows(mysql_query("select * from view_renja where c1='$c1' and c='$c' and d='$d' and q != '0' and tahun = '$this->tahun' and jenis_anggaran = '$this->jenisAnggaran' and no_urut = '$this->nomorUrut'")) == 0 ){
								...
								while($Program= mysql_fetch_array($getProgram)){
									...
									if(mysql_num_rows(mysql_query("select * from view_renja where c1='$c1' and c='$c' and d='$d' and bk = '$bk' and ck = '$ck' and p ='$p' and q != '0' and tahun = '$this->tahun' and jenis_anggaran = '$this->jenisAnggaran' and no_urut = '$this->nomorUrut'")) == 0 ){
										$arrKondisi[] = "id_anggaran !='$id_anggaran'";
									}
								}
							}
...
$renjaPerubahan_v3 = new renjaPerubahan_v3Obj();

$arrayResult = tahapPerencanaanV3($renjaPerubahan_v3->modul,'PERUBAHAN');
...
$renjaPerubahan_v3->jenisForm = $jenisForm;
$renjaPerubahan_v3->nomorUrut = $nomorUrut;
$renjaPerubahan_v3->tahun = $tahun;
$renjaPerubahan_v3->jenisAnggaran = $jenisAnggaran;
$renjaPerubahan_v3->idTahap = $idTahap;
$renjaPerubahan_v3->username = $_COOKIE['coID'];
$renjaPerubahan_v3->wajibValidasi = $arrayResult['wajib_validasi'];
$renjaPerubahan_v3->settingAnggaran = $arrayResult['settingAnggaran'];
