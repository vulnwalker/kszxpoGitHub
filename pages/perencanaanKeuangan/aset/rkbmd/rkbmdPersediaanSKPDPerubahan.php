...
class rkbmdPersediaanSKPDPerubahanObj  extends DaftarObj2{
	var $Prefix = 'rkbmdPersediaanSKPDPerubahan';
	...
	var $FormName = 'rkbmdPersediaanSKPDPerubahanForm';
	var $kode_skpd = '';
	var $modul = "RKBMD";
	var $jenisForm = "";
	var $tahun = "";
	...
	function setPage_HeaderOther(){
	  ...
	  <A href=\"pages.php?Pg=rkbmdPersediaanSKPDPerubahan\" title='RKBMD PERSEDIAAN' style='color : blue;' > PERSEDIAAN </a>
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  case 'excel':{
			foreach ($_REQUEST as $key => $value) {
			 ...
			 if($rkbmdPersediaanSKPDPerubahanSkpdfmUrusan =='0'){
			 	$err = "Pilih Urusan";
			 }elseif($rkbmdPersediaanSKPDPerubahanSkpdfmSKPD =='00'){
			 	$err = "Pilih Bidang";
			 }elseif($rkbmdPersediaanSKPDPerubahanSkpdfmUNIT =='00'){
			 	$err = "Pilih SKPD";
			 }else{
	...
	case 'formBaru':{
		...
		case 'newTab':{
			 ...
			 $cekKeberadaanMangkluk =  mysql_num_rows(mysql_query("select * from view_renja where jenis_anggaran = '$this->jenisAnggaran' and tahun = '$this->tahun' and c1 = '$rkbmdPersediaanSKPDPerubahanSkpdfmUrusan' and c = '$rkbmdPersediaanSKPDPerubahanSkpdfmSKPD' and d='$rkbmdPersediaanSKPDPerubahanSkpdfmUNIT' and e = '$rkbmdPersediaanSKPDPerubahanSkpdfmSUBUNIT' and e1='$rkbmdPersediaanSKPDPerubahanSkpdfmSEKSI'  and q!='0' and no_urut ='$nomorUrutSebelumnya' "));
			 $getDatarenja = mysql_fetch_array(mysql_query("select * from view_renja where jenis_anggaran = '$this->jenisAnggaran' and tahun = '$this->tahun' and c1 = '$rkbmdPersediaanSKPDPerubahanSkpdfmUrusan' and c = '$rkbmdPersediaanSKPDPerubahanSkpdfmSKPD' and d='$rkbmdPersediaanSKPDPerubahanSkpdfmUNIT' and e = '$rkbmdPersediaanSKPDPerubahanSkpdfmSUBUNIT' and e1='$rkbmdPersediaanSKPDPerubahanSkpdfmSEKSI' and q!='0' and no_urut = '$nomorUrutSebelumnya'"));
			 $lastID = $getDatarenja['id_anggaran'];
			 	if($cekKeberadaanMangkluk != 0){
					if($getDatarenja['jenis_form_modul']  == 'PENYUSUNAN' ){
						$getJumlahRenjaValidasi = mysql_num_rows(mysql_query("select * from view_renja where jenis_anggaran = '$this->jenisAnggaran' and tahun = '$this->tahun' and c1 = '$rkbmdPersediaanSKPDPerubahanSkpdfmUrusan' and c = '$rkbmdPersediaanSKPDPerubahanSkpdfmSKPD' and d='$rkbmdPersediaanSKPDPerubahanSkpdfmUNIT' and e = '$rkbmdPersediaanSKPDPerubahanSkpdfmSUBUNIT' and e1='$rkbmdPersediaanSKPDPerubahanSkpdfmSEKSI' and q!='0' $this->sqlValidasi and no_urut = '$nomorUrutSebelumnya'"));
						if($getJumlahRenjaValidasi == 0){
							$err = "Renja Belum Di Validasi";
						}
					}

					$getParentrkbmdPersediaanSKPDPerubahan = mysql_fetch_array(mysql_query("select * from view_renja where id_anggaran = '$lastID'"));
					$parentC1 = $getParentrkbmdPersediaanSKPDPerubahan['c1'];
					$parentC = $getParentrkbmdPersediaanSKPDPerubahan['c'];
					$parentD = $getParentrkbmdPersediaanSKPDPerubahan['d'];
					$parentE = $getParentrkbmdPersediaanSKPDPerubahan['e'];
					$parentE1= $getParentrkbmdPersediaanSKPDPerubahan['e1'];
					...
					if($cmbJenisrkbmdPersediaanSKPDPerubahan == 'PERSEDIAAN'){
						$kemana = 'ins_v3';
					}else{
		...
		case 'editTab':{
			 $id = $_REQUEST['rkbmdPersediaanSKPDPerubahan_cb'];
			foreach ($_REQUEST as $key => $value) {
				  $$key = $value;
			 }
					$nomorUrutSebelumnya = $this->nomorUrut - 1;
					$getParentrkbmdPersediaanSKPDPerubahan = mysql_fetch_array(mysql_query("select * from view_rkbmd where id_anggaran = '$id[0]'"));
					foreach ($getParentrkbmdPersediaanSKPDPerubahan as $key => $value) {
				 		 $$key = $value;
					 }
					$parentC1 = $getParentrkbmdPersediaanSKPDPerubahan['c1'];
					$parentC = $getParentrkbmdPersediaanSKPDPerubahan['c'];
		...
		case 'SaveValid':{
	    	foreach ($_REQUEST as $key => $value) {
			 ...
			 $getSKPD = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$rkbmdPersediaanSKPDPerubahan_idplh'"));
			 ...
			 $query = VulnWalkerUpdate("tabel_anggaran",$data," id_anggaran = '$rkbmdPersediaanSKPDPerubahan_idplh'");
			 mysql_query($query);
	   ...
	   case 'remove':{
		...
		case 'koreksi':{
			...
			$getrkbmdPersediaanSKPDPerubahannya = mysql_fetch_array(mysql_query($queryRows));
			foreach ($getrkbmdPersediaanSKPDPerubahannya as $key => $value) {
				  $$key = $value;
			}
			 ...
			 $dataSesuai = array(           'jenis_anggaran' => $this->jenisAnggaran,
			  ...
			  if($cekKoreksi > 0 ){
			   	 $getID = mysql_fetch_array(mysql_query("select * from tabel_anggaran where c1 = '$c1' and c='$c' and d = '$d' and e='$e' and e1='$e1' and bk='$bk' and ck='$ck' and dk='$dk' and p = '$p' and q='$q'  and f1='$f1' and f2='$f2' and f='$f' and g='$g' and h='$h' and i='$i' and j='$j' and id_jenis_pemeliharaan ='$id_jenis_pemeliharaan' and tahun = '$this->tahun' and jenis_anggaran = '$this->jenisAnggaran' and id_tahap = '$this->idTahap' "));
						...
						}elseif($angkaKoreksi <= $kebutuhanRiil || (empty($jumlahOptimal) && empty($kebutuhanMax) && $getrkbmdPersediaanSKPDPerubahannya['volume_barang'] >= $angkaKoreksi )  ){
							mysql_query("update tabel_anggaran set volume_barang = '$angkaKoreksi' , cara_pemenuhan = '$caraPemenuhan' where id_anggaran='$idnya'");
						}elseif($getrkbmdPersediaanSKPDPerubahannya['volume_barang'] < $angkaKoreksi){
						...
						if($angkaKoreksi <= $kebutuhanRiil || (empty($jumlahOptimal) && empty($kebutuhanMax) ) && $getrkbmdPersediaanSKPDPerubahannya['volume_barang'] >= $angkaKoreksi ){
	 ...
	 function setPage_OtherScript(){
	   ...
	   return
	     ...
	     <script type='text/javascript' src='js/skpd.js' language='JavaScript' ></script>".
	      "<script type='text/javascript' src='js/perencanaanKeuangan/aset/rkbmd/rkbmdPersediaanSKPDPerubahan.js' language='JavaScript' ></script>".
	     $scriptload;
	 }
...
/*		$nomorUrutSebelumnya = $this-> -1;*/
	 ...
	 if($this->jenisForm=="KOREKSI PENGGUNA"){
					...
					if($f == '00' && $q =='0')$TampilCheckBox = "";
						...
						// $Koloms.= "<td class='$cssclass' align='center'>$no</td>";
						 ...
						 $aksi  = "<img src='images/administrator/images/valid.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkbmdPersediaanSKPDPerubahan.sesuai('$id_anggaran');></img>&nbsp  &nbsp <img src='images/administrator/images/invalid.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkbmdPersediaanSKPDPerubahan.koreksi('$id_anggaran');></img>";
	...
	function Laporan($dt){
	 ...
	 $c1 = $dt['rkbmdPersediaanSKPDPerubahanSkpdfmUrusan'];
	 $c = $dt['rkbmdPersediaanSKPDPerubahanSkpdfmSKPD'];
	 $d = $dt['rkbmdPersediaanSKPDPerubahanSkpdfmUNIT'];
	 $e = $dt['rkbmdPersediaanSKPDPerubahanSkpdfmSUBUNIT'];
	 $e1 = $dt['rkbmdPersediaanSKPDPerubahanSkpdfmSEKSI'];
	...
	$TampilOpt =
			...
			<td width=\"100%\" valign=\"top\">" .
				WilSKPD_ajxVW("rkbmdPersediaanSKPDPerubahanSkpd") .
			"</td>
			<td >" .
			"</td></tr>
			<tr><td>
			<input type='hidden' name='cmbJenisrkbmdPersediaanSKPDPerubahan' id='cmbJenisrkbmdPersediaanSKPDPerubahan' value='PERSEDIAAN'>
			</td></tr>
			</table>";
	...
	function getDaftarOpsi($Mode=1){
		global $Main, $HTTP_COOKIE_VARS;
		$UID  = $_COOKIE['coID'];
		$c1   = $_REQUEST['rkbmdPersediaanSKPDPerubahanSkpdfmUrusan'];
		$c    = $_REQUEST['rkbmdPersediaanSKPDPerubahanSkpdfmSKPD'];
		$d    = $_REQUEST['rkbmdPersediaanSKPDPerubahanSkpdfmUNIT'];
		$e    = $_REQUEST['rkbmdPersediaanSKPDPerubahanSkpdfmSUBUNIT'];
		$e1   = $_REQUEST['rkbmdPersediaanSKPDPerubahanSkpdfmSEKSI'];
		...
		$fmKODE = $_REQUEST['fmKODE'];
		$fmBARANG = $_REQUEST['fmBARANG'];
		$cmbJenisrkbmdPersediaanSKPDPerubahan = $_REQUEST['cmbJenisrkbmdPersediaanSKPDPerubahan'];
		$arrKondisi = array();
...
$rkbmdPersediaanSKPDPerubahan = new rkbmdPersediaanSKPDPerubahanObj();
...
$rkbmdPersediaanSKPDPerubahan->jenisForm = $jenisForm;
$rkbmdPersediaanSKPDPerubahan->nomorUrut = $nomorUrut;
$rkbmdPersediaanSKPDPerubahan->urutTerakhir = $nomorUrut;
$rkbmdPersediaanSKPDPerubahan->tahun = $tahun;
$rkbmdPersediaanSKPDPerubahan->jenisAnggaran = $jenisAnggaran;
$rkbmdPersediaanSKPDPerubahan->idTahap = $idTahap;
$rkbmdPersediaanSKPDPerubahan->username = $_COOKIE['coID'];
$rkbmdPersediaanSKPDPerubahan->wajibValidasi = $arrayResult['wajib_validasi'];
if($rkbmdPersediaanSKPDPerubahan->wajibValidasi == TRUE){
	$rkbmdPersediaanSKPDPerubahan->sqlValidasi = " and status_validasi ='1' ";
}else{
	$rkbmdPersediaanSKPDPerubahan->sqlValidasi = " ";
}

$rkbmdPersediaanSKPDPerubahan->provinsi = $arrayResult['provinsi'];
$rkbmdPersediaanSKPDPerubahan->kota = $arrayResult['kota'];
$rkbmdPersediaanSKPDPerubahan->pengelolaBarang = $arrayResult['pengelolaBarang'];
$rkbmdPersediaanSKPDPerubahan->pejabatPengelolaBarang = $arrayResult['pejabat'];
$rkbmdPersediaanSKPDPerubahan->pengurusPengelolaBarang = $arrayResult['pengurus'];
$rkbmdPersediaanSKPDPerubahan->nipPengelola = $arrayResult['nipPengelola'];
$rkbmdPersediaanSKPDPerubahan->nipPengurus = $arrayResult['nipPengurus'];
$rkbmdPersediaanSKPDPerubahan->nipPejabat = $arrayResult['nipPejabat'];
