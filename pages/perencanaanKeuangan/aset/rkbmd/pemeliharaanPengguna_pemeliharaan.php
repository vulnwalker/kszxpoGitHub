<?php
class pemeliharaanPengguna_pemeliharaanObj  extends DaftarObj2{	
	var $Prefix = 'pemeliharaanPengguna_pemeliharaan';
	var $elCurrPage="HalDefault";
	var $SHOW_CEK = TRUE;
	var $TblName = 'pemeliharaanPengguna_pemeliharaan'; //bonus
	...
	var $FormName = 'pemeliharaanPengguna_pemeliharaanForm';
	var $noModul=14; 
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){	
		...
		case 'CekAda':{
			...
			mysql_query("delete from pemeliharaanPengguna_pemeliharaan where user='$username'");

			$getAll = mysql_query("select * from view_rkbmd where tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and jenis_form_modul ='PENYUSUNAN' and id_tahap='$this->idTahap' and j !='000' and id_jenis_pemeliharaan !='0' and c1='$c1' and c='$c' and d='$d' and e='$e' and e1 ='$e1' and bk='$bk' and ck='$ck' and p='$p' and q='$q' and status_validasi !='1'");
				...
				$tergantung = 'ada';
						...
						while($rows = mysql_fetch_array($getAll)){
							...
							if($id_jenis_pemeliharaan != '0'){
								mysql_query(VulnWalkerInsert('pemeliharaanPengguna_pemeliharaan',$data));
							}	  
						}
		...
		case 'finish':{		
			$username = $_COOKIE['coID'];		
			$execute = mysql_query("select * from pemeliharaanPengguna_pemeliharaan where user='$username'");
			$get = mysql_fetch_array($execute);
			foreach ($get as $key => $value) { 
				  $$key = $value; 
			}
			if(mysql_num_rows(mysql_query("select * from pemeliharaanPengguna_pemeliharaan where user='$username'")) == 0){
				$err = "Data Kosong";
			    ...
			    mysql_query("delete from tabel_anggaran where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and bk='$bk' and ck='$ck' and p='$p' and q='$q' and tahun='$this->tahun' and id_tahap ='$this->idTahap' and ((id_jenis_pemeliharaan != '0' and f !='00') or uraian_pemeliharaan = 'RKBMD PEMELIHARAAN') and status_validasi !='1' ");
				...
				$cekKegiatan = mysql_num_rows(mysql_query("select * from view_rkbmd where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and bk = '$bk' and ck = '$ck' and p = '$p' and q= '$q' and  f='00' and id_tahap='$this->idTahap' and uraian_pemeliharaan = 'RKBMD PEMELIHARAAN'"));												
				if($cekKegiatan < 1){
					$data = array('jenis_anggaran' => $this->jenisAnggaran,
								  'tahun' => $this->tahun,
								  'c1' => $c1,
				...
				$execute2 = mysql_query("select * from pemeliharaanPengguna_pemeliharaan where user='$username'");
				while($rows = mysql_fetch_array($execute2)){
					...
					$data = array('jenis_anggaran' => $this->jenisAnggaran,
						...
						mysql_query("delete from pemeliharaanPengguna_pemeliharaan where id = '$id'");
				}	
			}
		...
		case 'newRowPemeliharaan':{
			...
			$query = (VulnWalkerInsert('temp_pemeliharaanPengguna_pemeliharaan',$data));
			mysql_query($query);
			$username = $_COOKIE['coID'];
			$getIdAkhir = mysql_fetch_array(mysql_query("select max(id) as idAkhir from temp_pemeliharaanPengguna_pemeliharaan where user = '$username'"));
		...
		case 'Simpan':{
			...
			$concat = $f.".".$g.".".$h.".".$i.".".$j;
			$username = $_COOKIE['coID'];			
			$cekSame = mysql_num_rows(mysql_query("select * from pemeliharaanPengguna_pemeliharaan where concat(f,'.',g,'.',h,'.',i,'.',j) = '$concat'  and user = '$username'"));
	...
	$cmbKegiatan = cmbQuery('q', $q, $codeAndNameKegiatan,' disabled','-- KEGIATAN --');  
			...
			$getMaxIDHUbla = mysql_fetch_array(mysql_query("select max(id) from temp_pemeliharaanPengguna_pemeliharaan where user = '$this->username'"));
		...
		case 'subSubDelete':{				
			$id = $_REQUEST['id'];
			mysql_query("delete from temp_pemeliharaanPengguna_pemeliharaan where id='$id'");				
		break;
		}
		case 'subHapus':{				
			$id = $_REQUEST['id'];
			$get = mysql_fetch_array(mysql_query("select * from pemeliharaanPengguna_pemeliharaan where id ='$id'"));
			foreach ($get as $key => $value) { 
			  $$key = $value; 
			}
			$concat = $f.".".$g.".".$h.".".$i.".".$j;
			$username = $_COOKIE['coID'];	 
			$execute = mysql_query("select * from pemeliharaanPengguna_pemeliharaan where concat(f,'.',g,'.',h,'.',i,'.',j) = '$concat'  and user = '$username'");	
		...
		case 'subSubSave':{				
			...
			$data = array(
				...
				$query=VulnWalkerUpdate('temp_pemeliharaanPengguna_pemeliharaan',$data,"id = '$id'");
				
				$concat = $c1.".".$c.".".$d.".".$e.".".$e1.".".$bk.".".$ck.".".$dk.".".$p.".".$q.".".$f.".".$g.".".$h.".".$i.".".$j;
				$cekSame = mysql_num_rows(mysql_query("select * from temp_pemeliharaanPengguna_pemeliharaan where concat(c1,'.',c,'.',d,'.',e,'.',e1,'.',bk,'.',ck,'.',dk,'.',p,'.',q,'.',f,'.',g,'.',h,'.',i,'.',j) = '$concat' and id_jenis_pemeliharaan ='$idJenisPemeliharaan' and id !='$id'"));
				...
				$content = "select * from temp_pemeliharaanPengguna_pemeliharaan where concat(c1,'.',c,'.',d,'.',e,'.',e1,'.',bk,'.',ck,'.',dk,'.',p,'.',q,'.',f,'.',g,'.',h,'.',i,'.',j) = '$concat' and id_jenis_pemeliharaan ='$idJenisPemeliharaan' and id !='$id'";
		...
		case 'subSubCancel':{	
		    $username = $_COOKIE['coID'];			
			mysql_query("delete from temp_pemeliharaanPengguna_pemeliharaan where user = '$username' and (id_jenis_pemeliharaan = '' OR id_jenis_pemeliharaan = '0' )");				
		break;
		}
		case 'clear':{	
			 $username = $_COOKIE['coID'];
		  	 mysql_query("delete from temp_pemeliharaanPengguna_pemeliharaan where user='$username'");
			 mysql_query("delete from pemeliharaanPengguna_pemeliharaan where user='$username'");
		break;
		}
		...
		case 'subCancel':{	
			...
			$username = $_COOKIE['coID'];	
			mysql_query("delete from temp_pemeliharaanPengguna_pemeliharaan where user = '$username' and concat(f,'.',g,'.',h,'.',i,'.',j) = '$kodeBarang'");				
		break;
		}
		...
		case 'moveBack':{	
			...
			$concat = $kodeBarang;
			$username =$_COOKIE['coID'];
			$execute = mysql_query("select * from temp_pemeliharaanPengguna_pemeliharaan where user = '$username' and (id_jenis_pemeliharaan != '0' or  id_jenis_pemeliharaan !='')");
			if(mysql_num_rows($execute) == 0){
				$err = "Data kosong !";
			}else{	
			mysql_query("delete from pemeliharaanPengguna_pemeliharaan where user = '$username' and concat(f,'.',g,'.',h,'.',i,'.',j) = '$kodeBarang'");
					while($rows = mysql_fetch_array($execute)){
					 foreach ($rows as $key => $value) { 
				  		$$key = $value; 
					 }
					$data = array(
					 ...
					 $query = VulnWalkerInsert('pemeliharaanPengguna_pemeliharaan',$data);
					 mysql_query($query);
					 mysql_query("delete from temp_pemeliharaanPengguna_pemeliharaan where id = '$id'");
					 $codeAndNameKegiatan = "select q, concat(q,'. ', nama) from ref_program where bk = '$bk' and ck = '$ck'  and p = '$p' and q ='$q' ";
					 $cmbKegiatan = cmbQuery('q', $q, $codeAndNameKegiatan,' disabled','-- KEGIATAN --');  
		...
		case 'moveList':{	
			...
			$concat = $f.".".$g.".".$h.".".$i.".".$j;
			$execute = mysql_query("select * from pemeliharaanPengguna_pemeliharaan where user = '$username' and concat(f,'.',g,'.',h,'.',i,'.',j) = '$concat'");
			mysql_query("delete from temp_pemeliharaanPengguna_pemeliharaan where user = '$username'");
			while($rows = mysql_fetch_array($execute)){
				foreach ($rows as $key => $value) { 
			  		$$key = $value; 
		...
		case 'subSimpan':{	
			...
			$username = $_COOKIE['coID'];
			$execute = mysql_query("select * from temp_pemeliharaanPengguna_pemeliharaan where user = '$username' and id_jenis_pemeliharaan != '0' and  id_jenis_pemeliharaan !=''");
			if(mysql_num_rows($execute) == 0){
				$err = "Data kosong !";
			}else{
					while($rows = mysql_fetch_array($execute)){
   ...
   function setPage_OtherScript(){
		...
		return
			...
			"<script type='text/javascript' src='js/perencanaan_v3/rkbmd/pemeliharaanPengguna_pemeliharaan.js' language='JavaScript' ></script>".
	...
	$hitungData = mysql_num_rows(mysql_query("select * from pemeliharaanPengguna_pemeliharaan where concat(f,'.',g,'.',h,'.',i,'.',j) = '$concat2' and user = '$username'"));
	if($hitungData > 1){
		$getMin = mysql_fetch_array(mysql_query("select min(concat(c1,'.',c,'.',d,'.',e,'.',e1,'.',bk,'.',ck,'.',p,'.',q,'.',f,'.',g,'.',h,'.',i,'.',j,'.',id_jenis_pemeliharaan)) as min from pemeliharaanPengguna_pemeliharaan where concat(f,'.',g,'.',h,'.',i,'.',j) = '$concat2' and user = '$username'"));
		$min = $getMin['min'];
		$realConcat = $c1.".".$c.".".$d.".".$e.".".$e1.".".$bk.".".$ck.".".$p.".".$q.".".$f.".".$g.".".$h.".".$i.".".$j.".".$id_jenis_pemeliharaan; 
		if($realConcat == $min){
			$getTotal = mysql_fetch_array(mysql_query("select sum(volume_barang) as total from pemeliharaanPengguna_pemeliharaan where concat(f,'.',g,'.',h,'.',i,'.',j) = '$concat2' and user='$username'"));
			...
			$aksi  = "<img src='images/administrator/images/invalid.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=pemeliharaanPengguna_pemeliharaan.subHapus('$id');></img>&nbsp &nbsp <img src='images/administrator/images/edit_f2.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=pemeliharaanPengguna_pemeliharaan.moveList('$id');></img> ";
		}else{
	...
	function tabelPemeliharaan(){
		...
		$qry = "select * from temp_pemeliharaanPengguna_pemeliharaan where id_jenis_pemeliharaan != '' and user = '$username'";$cek.=$qry;
		...
		while($dt = mysql_fetch_array($aqry)){
			...
			if($dt['id_jenis_pemeliharaan'] == '' || $dt['id_jenis_pemeliharaan'] == '0'){
				$action = "<img id='action$id' src='datepicker/save.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=pemeliharaanPengguna_pemeliharaan.subSubSave('$id');>";
				...
				$jumlah = "<input type='text' id='jumlah$id' name='jumlah$id' onkeypress='return event.charCode >= 48 && event.charCode <= 57' style='text-align:right;width:40px;'>";
				$keterangan2 = "<input type = 'text' id='keterangan$id' name='keterangan$id' style='width:100%;'>";
			}else{
				foreach ($dt as $key => $value) { 
				  $$key = $value; 
				}
				$action = "<img id='action$id' src='images/administrator/images/invalid.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=pemeliharaanPengguna_pemeliharaan.subSubDel('$id');>";
				...
				$jumlah = "<input type='text' id='jumlah$id' name='jumlah$id' value='$jumlah' onkeypress='return event.charCode >= 48 && event.charCode <= 57' readonly style='text-align:right; width:40px;'> ";
				$keterangan2 = "<input type = 'text' id='keterangan$id' name='keterangan$id' value='$keterangan' readonly style='width:100%;'>";
			}
			$datanya.="
			
				<tr class='row0'>
					<td class='GarisDaftar' align='right'><a onclick=pemeliharaanPengguna_pemeliharaan.subSubEdit($id) style='cursor:pointer;'>$no</a></td>
			...
			genFilterBar(
				array("
					<table class='koptable' style='min-width:900px;' border='1'>
						<tr>
							<th class='th01'>NO</th>
							<th class='th01' width='60px;'>JUMLAH  </th>
							<th class='th01' width='400px;'>URAIAN PEMELIHARAAN</th>
							<th class='th01'  width='100px;'>JENIS PEMELIHARAAN</th>
							<th class='th01'>KETERANGAN  </th>
							<th class='th01'>
								<span id='atasbutton'>
								<a href='javascript:pemeliharaanPengguna_pemeliharaan.$tujuan' id='linkAtasButton' /><img id='gambarAtasButton' src='$gambar' style='width:20px;height:20px;' /></a>
								</span>
							</th>
						</tr>
						$datanya
	...
	$e   = $ambilSKPD['e'];
	$e1  = $ambilSKPD['e1'];
	$cekMayoritas = mysql_fetch_array(mysql_query("select * from pemeliharaanPengguna_pemeliharaan where user = '$this->username'"));
	$e = $cekMayoritas['e'];
	$e1 = $cekMayoritas['e1'];
	...
	$cmbKegiatan = cmbQuery('q', $q, $codeAndNameKegiatan,'onchange=pemeliharaanPengguna_pemeliharaan.CekAda(); ','-- KEGIATAN --'); 
	$username = $_COOKIE['coID'];
	$matiButton = "";
	$syntax = mysql_query("select * from pemeliharaanPengguna_pemeliharaan where user = '$username'");
	...
	$vOrder=
					...
					$this->isiform(
						array(
							...
							array( 
								...
								<input type='button' value='Cari' id='findProgram' onclick ='pemeliharaanPengguna_pemeliharaan.CariProgram($ID_RENJA)'  title='Cari Program dan Kegiatan' $matiButton >"				
							),
							array( 
								'label'=>'KEGIATAN',
								'label-width'=>'200px;',
			...
			genFilterBar(
						...
						array(
							...
							array(
									...
									'value'=>"<input type='text' name='kodeBarang' onkeyup='cariBarang.pilBar2(this.value)' id='kodeBarang' placeholder='KODE BARANG' style='width:150px;' value='".$dt['kodebarangnya']."' readonly /> 
										...
										<button type='button' id='findBarang' onclick=pemeliharaanPengguna_pemeliharaan.CariBarang('$filterSKPD'); > Cari </button>
									",
								),
							array(
				...
				genFilterBar(
					array(
					"
					<input type='hidden' id='unit' name='unit' value = '$e.'>
						<input type='hidden' id='subunit' name='subunit' value = '$e1.'>
					...
					<td class='border:none'> 
						<a class='toolbar' id='btsave' href='javascript:pemeliharaanPengguna_pemeliharaan.$tujuan'> 
					...
					<tbody><tr valign='middle' align='center'> 
					<td class='border:none'> 
						<a class='toolbar' id='btcancel' href='javascript:pemeliharaanPengguna_pemeliharaan.closeTab()'> 
					...
					<tbody><tr valign='middle' align='center'> 
					<td class='border:none'> 
						<a class='toolbar' id='finish' href='javascript:pemeliharaanPengguna_pemeliharaan.finish()'> 
	...
	function getDaftarOpsi($Mode=1){
		...
		$Asc1 = $fmDESC1 ==''? '': 'desc';		
		$arrOrders = array();
		$arrOrders[] = " c1,c,d,e,e1,bk,ck,p,q,f,g,h,i,j,id_jenis_pemeliharaan $Asc1 " ;
...
$pemeliharaanPengguna_pemeliharaan = new pemeliharaanPengguna_pemeliharaanObj();

$arrayResult = tahapPerencanaanV3($pemeliharaanPengguna_pemeliharaan->modul,"MURNI");
...
$pemeliharaanPengguna_pemeliharaan->jenisForm = $jenisForm;
$pemeliharaanPengguna_pemeliharaan->nomorUrut = $nomorUrut;
$pemeliharaanPengguna_pemeliharaan->urutTerakhir = $nomorUrut;
$pemeliharaanPengguna_pemeliharaan->tahun = $tahun;
$pemeliharaanPengguna_pemeliharaan->jenisAnggaran = $jenisAnggaran;
$pemeliharaanPengguna_pemeliharaan->idTahap = $idTahap;
$pemeliharaanPengguna_pemeliharaan->username = $_COOKIE['coID'];
$pemeliharaanPengguna_pemeliharaan->wajibValidasi = $Main->wajibValidasi;
if($pemeliharaanPengguna_pemeliharaan->wajibValidasi == TRUE){
	$pemeliharaanPengguna_pemeliharaan->sqlValidasi = " and status_validasi ='1' ";
}else{
	$pemeliharaanPengguna_pemeliharaan->sqlValidasi = " ";
}
