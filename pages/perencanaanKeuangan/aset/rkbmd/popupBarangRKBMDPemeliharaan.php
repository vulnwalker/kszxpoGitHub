...
class popupBarangRKBMDPemeliharaanObj  extends DaftarObj2{	
	var $Prefix = 'popupBarangRKBMDPemeliharaan';
	...
	var $FormName = 'popupBarangRKBMDPemeliharaanForm'; 	
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){
		...
		$kode_barang=$get['f'].'.'.$get['g'].'.'.$get['h'].'.'.$get['i'].'.'.$get['j'];
		 ...
		 $getKebutuhanOptimal = mysql_fetch_array(mysql_query("select sum(jml_barang) as kebutuhanOptimal from buku_induk where concat(c1,'.',c,'.',d,'.',e,'.',e1,'.',f,'.',g,'.',h,'.',i,'.',j) = '$concat' and status_barang = '1' and (kondisi = '1' or kondisi = '2' or kondisi = '3') "));	
		 ...
		 $getKondisiKurangBaik = mysql_fetch_array(mysql_query("select sum(jml_barang) as kurangBaik from buku_induk where concat(c1,'.',c,'.',d,'.',e,'.',e1,'.',f,'.',g,'.',h,'.',i,'.',j) = '$concat' and status_barang = '1' and kondisi = '2'" ));		
		 $kurangBaik = $getKondisiKurangBaik['kurangBaik'];
		 $getKondisiRusakBerat = mysql_fetch_array(mysql_query("select sum(jml_barang) as rusakBerat from buku_induk where concat(c1,'.',c,'.',d,'.',e,'.',e1,'.',f,'.',g,'.',h,'.',i,'.',j) = '$concat' and status_barang = '1' and kondisi = '3'" ));		
...
/*			'' => array( 
	...
	function setKolomData($no, $isi, $Mode, $TampilCheckBox){
	 ...
	 $concat = $isi['c1'].".".$isi['c'].".".$isi['d'].".".$isi['e'].".".$isi['e1'].".".$isi['f'].'.'.$isi['g'].'.'.$isi['h'].'.'.$isi['i'].'.'.$isi['j'];
	 	...
	 	if($isi['j'] == '000'){
 		 ...
 		 $Koloms[] = array('align="left" width="200"',"<a style='cursor:pointer;' onclick = popupBarangRKBMDPemeliharaan.windowSave('$kode_barang')>". $isi['nm_barang']. "</a>");
	...
	$TampilOpt = 
			...
			<td>".
			cmbQuery1("cmbJenis",$cmbJenis,"select f as valueCmbJenis, nm_barang from ref_barang where f1 = '$cmbAkun' and f2 = '$cmbKelompok' and f != '00' and f != '06' and f!='07' and f!='08'  and g ='00' and h ='00' and i='00' and j = '000'","onChange=\"$this->Prefix.refreshList(true)\"",'Pilih','').
			"</td>
			</tr><tr>
	...
	function getDaftarOpsi($Mode=1){
		...
		/*switch($fmORDER1){
			...
			$Order =  $Order ==''? $OrderDefault : ' group by f1,f2,f,g,h,i,j
 Order By '.$Order;
...
$popupBarangRKBMDPemeliharaan = new popupBarangRKBMDPemeliharaanObj();
