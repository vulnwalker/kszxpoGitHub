...
class rkbmdPengadaanSKPKDPerubahanObj  extends DaftarObj2{
	var $Prefix = 'rkbmdPengadaanSKPKDPerubahan';
	...
	var $FormName = 'rkbmdPengadaanSKPKDPerubahanForm';
	var $kode_skpd = '';
	var $modul = "RKBMD";
	var $jenisForm = "";
	var $tahun = "";
	...
	function setPage_HeaderOther(){
	  ...
	  <A href=\"pages.php?Pg=rkbmdPengadaanSKPKDPerubahan\" title='RKBMD'  style='color : blue;' > PENGADAAN </a> |
	  <A href=\"pages.php?Pg=rkbmdPemeliharaanSKPKDPerubahan\" title='RKBMD PENGELOLA' > PEMELIHARAAN </a> |
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  case 'excel':{
			foreach ($_REQUEST as $key => $value) {
			 ...
			 /*if($rkbmdPengadaanSKPKDPerubahanSkpdfmUrusan =='0'){
			 	$err = "Pilih Urusan";
			 }elseif($rkbmdPengadaanSKPKDPerubahanSkpdfmSKPD =='00'){
			 	$err = "Pilih Bidang";
			 }elseif($rkbmdPengadaanSKPKDPerubahanSkpdfmUNIT =='00'){
			 	$err = "Pilih SKPD";
			 }elseif($rkbmdPengadaanSKPKDPerubahanSkpdfmSUBUNIT =='00'){
			 	$err = "Pilih Unit";
			 }elseif($rkbmdPengadaanSKPKDPerubahanSkpdfmSEKSI =='000'){
			 	$err = "Pilih Sub Unit";
			 }else{*/
	...
	case 'formBaru':{
		...
		case 'newTab':{
			 ...
			 $cekKeberadaanMangkluk =  mysql_num_rows(mysql_query("select * from view_renja where jenis_anggaran = '$this->jenisAnggaran' and tahun = '$this->tahun' and c1 = '$rkbmdPengadaanSKPKDPerubahanSkpdfmUrusan' and c = '$rkbmdPengadaanSKPKDPerubahanSkpdfmSKPD' and d='$rkbmdPengadaanSKPKDPerubahanSkpdfmUNIT' and e = '$rkbmdPengadaanSKPKDPerubahanSkpdfmSUBUNIT' and e1='$rkbmdPengadaanSKPKDPerubahanSkpdfmSEKSI'  and q!='0' and no_urut ='$nomorUrutSebelumnya' "));
			 $getDatarenja = mysql_fetch_array(mysql_query("select * from view_renja where jenis_anggaran = '$this->jenisAnggaran' and tahun = '$this->tahun' and c1 = '$rkbmdPengadaanSKPKDPerubahanSkpdfmUrusan' and c = '$rkbmdPengadaanSKPKDPerubahanSkpdfmSKPD' and d='$rkbmdPengadaanSKPKDPerubahanSkpdfmUNIT' and e = '$rkbmdPengadaanSKPKDPerubahanSkpdfmSUBUNIT' and e1='$rkbmdPengadaanSKPKDPerubahanSkpdfmSEKSI' and q!='0' and no_urut = '$nomorUrutSebelumnya'"));
			 $lastID = $getDatarenja['id_anggaran'];
			 	if($cekKeberadaanMangkluk != 0){
					if($getDatarenja['jenis_form_modul']  == 'PENYUSUNAN' ){
						$getJumlahRenjaValidasi = mysql_num_rows(mysql_query("select * from view_renja where jenis_anggaran = '$this->jenisAnggaran' and tahun = '$this->tahun' and c1 = '$rkbmdPengadaanSKPKDPerubahanSkpdfmUrusan' and c = '$rkbmdPengadaanSKPKDPerubahanSkpdfmSKPD' and d='$rkbmdPengadaanSKPKDPerubahanSkpdfmUNIT' and e = '$rkbmdPengadaanSKPKDPerubahanSkpdfmSUBUNIT' and e1='$rkbmdPengadaanSKPKDPerubahanSkpdfmSEKSI' and q!='0' $this->sqlValidasi and no_urut = '$nomorUrutSebelumnya'"));
						if($getJumlahRenjaValidasi == 0){
							$err = "Renja Belum Di Validasi";
						}
					}

					$getParentrkbmdPengadaanSKPKDPerubahan = mysql_fetch_array(mysql_query("select * from view_renja where id_anggaran = '$lastID'"));
					$parentC1 = $getParentrkbmdPengadaanSKPKDPerubahan['c1'];
					$parentC = $getParentrkbmdPengadaanSKPKDPerubahan['c'];
					$parentD = $getParentrkbmdPengadaanSKPKDPerubahan['d'];
					$parentE = $getParentrkbmdPengadaanSKPKDPerubahan['e'];
					$parentE1= $getParentrkbmdPengadaanSKPKDPerubahan['e1'];
					...
					if($cmbJenisrkbmdPengadaanSKPKDPerubahan == 'PENGADAAN'){
						$kemana = 'ins_v3';
					}else{
		...
		case 'editTab':{
			 $id = $_REQUEST['rkbmdPengadaanSKPKDPerubahan_cb'];
			foreach ($_REQUEST as $key => $value) {
				  $$key = $value;
			 }
					$nomorUrutSebelumnya = $this->nomorUrut - 1;
					$getParentrkbmdPengadaanSKPKDPerubahan = mysql_fetch_array(mysql_query("select * from view_rkbmd where id_anggaran = '$id[0]'"));
					foreach ($getParentrkbmdPengadaanSKPKDPerubahan as $key => $value) {
				 		 $$key = $value;
					 }
					$parentC1 = $getParentrkbmdPengadaanSKPKDPerubahan['c1'];
					$parentC = $getParentrkbmdPengadaanSKPKDPerubahan['c'];
		...
		case 'SaveValid':{
	    	foreach ($_REQUEST as $key => $value) {
			 ...
			 $getSKPD = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$rkbmdPengadaanSKPKDPerubahan_idplh'"));
			 ...
			 $query = VulnWalkerUpdate("tabel_anggaran",$data," id_anggaran = '$rkbmdPengadaanSKPKDPerubahan_idplh'");
			 mysql_query($query);
	   ...
	   case 'remove':{
		...
		case 'koreksi':{
			...
			$getrkbmdPengadaanSKPKDPerubahannya = mysql_fetch_array(mysql_query($queryRows));
			foreach ($getrkbmdPengadaanSKPKDPerubahannya as $key => $value) {
				  $$key = $value;
			}
			 ...
			 $dataSesuai = array(           'jenis_anggaran' => $this->jenisAnggaran,
			  ...
			  if($cekKoreksi > 0 ){
			   	 $getID = mysql_fetch_array(mysql_query("select * from tabel_anggaran where c1 = '$c1' and c='$c' and d = '$d' and e='$e' and e1='$e1' and bk='$bk' and ck='$ck' and dk='$dk' and p = '$p' and q='$q'  and f1='$f1' and f2='$f2' and f='$f' and g='$g' and h='$h' and i='$i' and j='$j' and id_jenis_pemeliharaan ='$id_jenis_pemeliharaan' and tahun = '$this->tahun' and jenis_anggaran = '$this->jenisAnggaran' and id_tahap = '$this->idTahap' "));
						...
						if($angkaKoreksi <= $kebutuhanRiil || (empty($jumlahOptimal) && empty($kebutuhanMax) && $getrkbmdPengadaanSKPKDPerubahannya['volume_barang'] >= $angkaKoreksi )  ){
							mysql_query("update tabel_anggaran set volume_barang = '$angkaKoreksi' , cara_pemenuhan = '$caraPemenuhan' where id_anggaran='$idnya'");
						}elseif($getrkbmdPengadaanSKPKDPerubahannya['volume_barang'] < $angkaKoreksi){
	 ...
	 function setPage_OtherScript(){
	   ...
	   return
	     ...
	     <script type='text/javascript' src='js/skpd.js' language='JavaScript' ></script>".
	      "<script type='text/javascript' src='js/perencanaanKeuangan/aset/rkbmd/rkbmdPengadaanSKPKDPerubahan.js' language='JavaScript' ></script>".
	     $scriptload;
	 }
...
/*		$nomorUrutSebelumnya = $this-> -1;*/
	   ...
	   elseif($this->jenisForm=="KOREKSI PENGGUNA"){
					...
					if($f == '00' && $q =='0')$TampilCheckBox = "";
						...
						$Koloms.= "<td class='$cssclass' align='left'></td>";
						 ...
						 $aksi  = "<img src='images/administrator/images/valid.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkbmdPengadaanSKPKDPerubahan.sesuai('$id_anggaran');></img>&nbsp  &nbsp <img src='images/administrator/images/invalid.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkbmdPengadaanSKPKDPerubahan.koreksi('$id_anggaran');></img>";
	...
	function Laporan($dt){
	 ...
	 $c1 = $dt['rkbmdPengadaanSKPKDPerubahanSkpdfmUrusan'];
	 $c = $dt['rkbmdPengadaanSKPKDPerubahanSkpdfmSKPD'];
	 $d = $dt['rkbmdPengadaanSKPKDPerubahanSkpdfmUNIT'];
	 $e = $dt['rkbmdPengadaanSKPKDPerubahanSkpdfmSUBUNIT'];
	 $e1 = $dt['rkbmdPengadaanSKPKDPerubahanSkpdfmSEKSI'];
	...
	$TampilOpt =
			...
			<td width=\"100%\" valign=\"top\">" .
				WilSKPD_ajxVW("rkbmdPengadaanSKPKDPerubahanSkpd") .
			"</td>
			<td >" .
			"</td></tr>
			<tr><td>
			<input type='hidden' name='cmbJenisrkbmdPengadaanSKPKDPerubahan' id='cmbJenisrkbmdPengadaanSKPKDPerubahan' value='PENGADAAN'>
			</td></tr>
			</table>";
	...
	function getDaftarOpsi($Mode=1){
		global $Main, $HTTP_COOKIE_VARS;
		$UID  = $_COOKIE['coID'];
		$c1   = $_REQUEST['rkbmdPengadaanSKPKDPerubahanSkpdfmUrusan'];
		$c    = $_REQUEST['rkbmdPengadaanSKPKDPerubahanSkpdfmSKPD'];
		$d    = $_REQUEST['rkbmdPengadaanSKPKDPerubahanSkpdfmUNIT'];
		$e    = $_REQUEST['rkbmdPengadaanSKPKDPerubahanSkpdfmSUBUNIT'];
		$e1   = $_REQUEST['rkbmdPengadaanSKPKDPerubahanSkpdfmSEKSI'];
		if(isset($e1)){
		...
		$fmKODE = $_REQUEST['fmKODE'];
		$fmBARANG = $_REQUEST['fmBARANG'];
		$cmbJenisrkbmdPengadaanSKPKDPerubahan = $_REQUEST['cmbJenisrkbmdPengadaanSKPKDPerubahan'];
		$arrKondisi = array();
...
$rkbmdPengadaanSKPKDPerubahan = new rkbmdPengadaanSKPKDPerubahanObj();
...
$rkbmdPengadaanSKPKDPerubahan->jenisForm = $jenisForm;
$rkbmdPengadaanSKPKDPerubahan->nomorUrut = $nomorUrut;
$rkbmdPengadaanSKPKDPerubahan->urutTerakhir = $nomorUrut;
$rkbmdPengadaanSKPKDPerubahan->tahun = $tahun;
$rkbmdPengadaanSKPKDPerubahan->jenisAnggaran = $jenisAnggaran;
$rkbmdPengadaanSKPKDPerubahan->idTahap = $idTahap;
$rkbmdPengadaanSKPKDPerubahan->username = $_COOKIE['coID'];
$rkbmdPengadaanSKPKDPerubahan->wajibValidasi = $arrayResult['wajib_validasi'];
if($rkbmdPengadaanSKPKDPerubahan->wajibValidasi == TRUE){
	$rkbmdPengadaanSKPKDPerubahan->sqlValidasi = " and status_validasi ='1' ";
}else{
	$rkbmdPengadaanSKPKDPerubahan->sqlValidasi = " ";
}

$rkbmdPengadaanSKPKDPerubahan->provinsi = $arrayResult['provinsi'];
$rkbmdPengadaanSKPKDPerubahan->kota = $arrayResult['kota'];
$rkbmdPengadaanSKPKDPerubahan->pengelolaBarang = $arrayResult['pengelolaBarang'];
$rkbmdPengadaanSKPKDPerubahan->pejabatPengelolaBarang = $arrayResult['pejabat'];
$rkbmdPengadaanSKPKDPerubahan->pengurusPengelolaBarang = $arrayResult['pengurus'];
$rkbmdPengadaanSKPKDPerubahan->nipPengelola = $arrayResult['nipPengelola'];
$rkbmdPengadaanSKPKDPerubahan->nipPengurus = $arrayResult['nipPengurus'];
$rkbmdPengadaanSKPKDPerubahan->nipPejabat = $arrayResult['nipPejabat'];
