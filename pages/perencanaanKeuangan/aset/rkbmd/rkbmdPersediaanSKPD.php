...
class rkbmdPersediaanSKPDObj  extends DaftarObj2{
	var $Prefix = 'rkbmdPersediaanSKPD';
	...
	var $FormName = 'rkbmdPersediaanSKPDForm';
	var $kode_skpd = '';
	var $modul = "RKBMD";
	var $jenisForm = "";
	var $tahun = "";
	...
	var $reportURL4 = "pages.php?Pg=rkbmdPersediaanSKPD&tipe=Persediaan4";
	var $reportURL5 = "pages.php?Pg=rkbmdPersediaanSKPD&tipe=Persediaan5";
	var $reportURL6 = "pages.php?Pg=rkbmdPersediaanSKPD&tipe=Persediaan6";
	...
	function setPage_HeaderOther(){
	  ...
	  <A href=\"pages.php?Pg=rkbmdPersediaanSKPD\" title='RKBMD PERSEDIAAN'  style='color : blue;'> PERSEDIAAN </a> |
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  case 'excel':{
			foreach ($_REQUEST as $key => $value) {
			 ...
			 if($rkbmdPersediaanSKPDSkpdfmUrusan =='0'){
			 	$err = "Pilih Urusan";
			 }elseif($rkbmdPersediaanSKPDSkpdfmSKPD =='00'){
			 	$err = "Pilih Bidang";
			 }elseif($rkbmdPersediaanSKPDSkpdfmUNIT =='00'){
			 	$err = "Pilih SKPD";
			 }else{
	...
	case 'formBaru':{
		...
		case 'newTab':{
			 ...
			 $cekKeberadaanMangkluk =  mysql_num_rows(mysql_query("select * from view_renja where jenis_anggaran = '$this->jenisAnggaran' and tahun = '$this->tahun' and c1 = '$rkbmdPersediaanSKPDSkpdfmUrusan' and c = '$rkbmdPersediaanSKPDSkpdfmSKPD' and d='$rkbmdPersediaanSKPDSkpdfmUNIT' and e = '$rkbmdPersediaanSKPDSkpdfmSUBUNIT' and e1='$rkbmdPersediaanSKPDSkpdfmSEKSI'  and q!='0' and no_urut ='$nomorUrutSebelumnya' "));
			 $getDatarenja = mysql_fetch_array(mysql_query("select * from view_renja where jenis_anggaran = '$this->jenisAnggaran' and tahun = '$this->tahun' and c1 = '$rkbmdPersediaanSKPDSkpdfmUrusan' and c = '$rkbmdPersediaanSKPDSkpdfmSKPD' and d='$rkbmdPersediaanSKPDSkpdfmUNIT' and e = '$rkbmdPersediaanSKPDSkpdfmSUBUNIT' and e1='$rkbmdPersediaanSKPDSkpdfmSEKSI' and q!='0' and no_urut = '$nomorUrutSebelumnya'"));
			 $lastID = $getDatarenja['id_anggaran'];
			 	if($cekKeberadaanMangkluk != 0){
					if($getDatarenja['jenis_form_modul']  == 'PENYUSUNAN' ){
						$getJumlahRenjaValidasi = mysql_num_rows(mysql_query("select * from view_renja where jenis_anggaran = '$this->jenisAnggaran' and tahun = '$this->tahun' and c1 = '$rkbmdPersediaanSKPDSkpdfmUrusan' and c = '$rkbmdPersediaanSKPDSkpdfmSKPD' and d='$rkbmdPersediaanSKPDSkpdfmUNIT' and e = '$rkbmdPersediaanSKPDSkpdfmSUBUNIT' and e1='$rkbmdPersediaanSKPDSkpdfmSEKSI' and q!='0' $this->sqlValidasi and no_urut = '$nomorUrutSebelumnya'"));
						if($getJumlahRenjaValidasi == 0){
							$err = "Renja Belum Di Validasi";
						}
					}

					$getParentrkbmdPersediaanSKPD = mysql_fetch_array(mysql_query("select * from view_renja where id_anggaran = '$lastID'"));
					$parentC1 = $getParentrkbmdPersediaanSKPD['c1'];
					$parentC = $getParentrkbmdPersediaanSKPD['c'];
					$parentD = $getParentrkbmdPersediaanSKPD['d'];
					$parentE = $getParentrkbmdPersediaanSKPD['e'];
					$parentE1= $getParentrkbmdPersediaanSKPD['e1'];
					...
					if($cmbJenisrkbmdPersediaanSKPD == 'PERSEDIAAN'){
						$kemana = 'ins_v3';
					}else{
		...
		case 'editTab':{
			 $id = $_REQUEST['rkbmdPersediaanSKPD_cb'];
			foreach ($_REQUEST as $key => $value) {
				  $$key = $value;
			 }
					$nomorUrutSebelumnya = $this->nomorUrut - 1;
					$getParentrkbmdPersediaanSKPD = mysql_fetch_array(mysql_query("select * from view_rkbmd where id_anggaran = '$id[0]'"));
					foreach ($getParentrkbmdPersediaanSKPD as $key => $value) {
				 		 $$key = $value;
					 }
					$parentC1 = $getParentrkbmdPersediaanSKPD['c1'];
					$parentC = $getParentrkbmdPersediaanSKPD['c'];
		...
		case 'SaveValid':{
	    	foreach ($_REQUEST as $key => $value) {
			 ...
			 $getSKPD = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$rkbmdPersediaanSKPD_idplh'"));
			 ...
			 $query = VulnWalkerUpdate("tabel_anggaran",$data," id_anggaran = '$rkbmdPersediaanSKPD_idplh'");
			 mysql_query($query);
	   ...
	   case 'remove':{
		...
		case 'koreksi':{
			...
			$getrkbmdPersediaanSKPDnya = mysql_fetch_array(mysql_query($queryRows));
			foreach ($getrkbmdPersediaanSKPDnya as $key => $value) {
				  $$key = $value;
			}
			 ...
			 $dataSesuai = array(           'jenis_anggaran' => $this->jenisAnggaran,
			  ...
			  if($cekKoreksi > 0 ){
			   	 $getID = mysql_fetch_array(mysql_query("select * from tabel_anggaran where c1 = '$c1' and c='$c' and d = '$d' and e='$e' and e1='$e1' and bk='$bk' and ck='$ck' and dk='$dk' and p = '$p' and q='$q'  and f1='$f1' and f2='$f2' and f='$f' and g='$g' and h='$h' and i='$i' and j='$j' and id_jenis_pemeliharaan ='$id_jenis_pemeliharaan' and tahun = '$this->tahun' and jenis_anggaran = '$this->jenisAnggaran' and id_tahap = '$this->idTahap' "));
						...
						}elseif($angkaKoreksi <= $kebutuhanRiil || (empty($jumlahOptimal) && empty($kebutuhanMax) && $getrkbmdPersediaanSKPDnya['volume_barang'] >= $angkaKoreksi )  ){
							mysql_query("update tabel_anggaran set volume_barang = '$angkaKoreksi' , cara_pemenuhan = '$caraPemenuhan' where id_anggaran='$idnya'");
						}elseif($getrkbmdPersediaanSKPDnya['volume_barang'] < $angkaKoreksi){
						...
						if($angkaKoreksi <= $kebutuhanRiil || (empty($jumlahOptimal) && empty($kebutuhanMax) ) && $getrkbmdPersediaanSKPDnya['volume_barang'] >= $angkaKoreksi ){
	 ...
	 function setPage_OtherScript(){
	   ...
	   return
	     ...
	     <script type='text/javascript' src='js/skpd.js' language='JavaScript' ></script>".
	      "<script type='text/javascript' src='js/perencanaanKeuangan/aset/rkbmd/rkbmdPersediaanSKPD.js' language='JavaScript' ></script>".
	     $scriptload;
	 }
...
/*		$nomorUrutSebelumnya = $this-> -1;*/
	 ...
	 if($this->jenisForm=="KOREKSI PENGGUNA"){
					...
					if($f == '00' && $q =='0')$TampilCheckBox = "";
						...
						// $Koloms.= "<td class='$cssclass' align='center'>$no</td>";
						 ...
						 $aksi  = "<img src='images/administrator/images/valid.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkbmdPersediaanSKPD.sesuai('$id_anggaran');></img>&nbsp  &nbsp <img src='images/administrator/images/invalid.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkbmdPersediaanSKPD.koreksi('$id_anggaran');></img>";
	...
	function Laporan($dt){
	 ...
	 $c1 = $dt['rkbmdPersediaanSKPDSkpdfmUrusan'];
	 $c = $dt['rkbmdPersediaanSKPDSkpdfmSKPD'];
	 $d = $dt['rkbmdPersediaanSKPDSkpdfmUNIT'];
	 $e = $dt['rkbmdPersediaanSKPDSkpdfmSUBUNIT'];
	 $e1 = $dt['rkbmdPersediaanSKPDSkpdfmSEKSI'];
	...
	$TampilOpt =
			...
			<td width=\"100%\" valign=\"top\">" .
				WilSKPD_ajxVW("rkbmdPersediaanSKPDSkpd") .
			"</td>
			<td >" .
			"</td></tr>
			<tr><td>
			<input type='hidden' name='cmbJenisrkbmdPersediaanSKPD' id='cmbJenisrkbmdPersediaanSKPD' value='PERSEDIAAN'>
			</td></tr>
			</table>";
	...
	function getDaftarOpsi($Mode=1){
		global $Main, $HTTP_COOKIE_VARS;
		$UID  = $_COOKIE['coID'];
		$c1   = $_REQUEST['rkbmdPersediaanSKPDSkpdfmUrusan'];
		$c    = $_REQUEST['rkbmdPersediaanSKPDSkpdfmSKPD'];
		$d    = $_REQUEST['rkbmdPersediaanSKPDSkpdfmUNIT'];
		$e    = $_REQUEST['rkbmdPersediaanSKPDSkpdfmSUBUNIT'];
		$e1   = $_REQUEST['rkbmdPersediaanSKPDSkpdfmSEKSI'];
		...
		$fmKODE = $_REQUEST['fmKODE'];
		$fmBARANG = $_REQUEST['fmBARANG'];
		$cmbJenisrkbmdPersediaanSKPD = $_REQUEST['cmbJenisrkbmdPersediaanSKPD'];
		$arrKondisi = array();
...
$rkbmdPersediaanSKPD = new rkbmdPersediaanSKPDObj();
...
$rkbmdPersediaanSKPD->jenisForm = $jenisForm;
$rkbmdPersediaanSKPD->nomorUrut = $nomorUrut;
$rkbmdPersediaanSKPD->urutTerakhir = $nomorUrut;
$rkbmdPersediaanSKPD->tahun = $tahun;
$rkbmdPersediaanSKPD->jenisAnggaran = $jenisAnggaran;
$rkbmdPersediaanSKPD->idTahap = $idTahap;
$rkbmdPersediaanSKPD->username = $_COOKIE['coID'];
$rkbmdPersediaanSKPD->wajibValidasi = $arrayResult['wajib_validasi'];
if($rkbmdPersediaanSKPD->wajibValidasi == TRUE){
	$rkbmdPersediaanSKPD->sqlValidasi = " and status_validasi ='1' ";
}else{
	$rkbmdPersediaanSKPD->sqlValidasi = " ";
}

$rkbmdPersediaanSKPD->provinsi = $arrayResult['provinsi'];
$rkbmdPersediaanSKPD->kota = $arrayResult['kota'];
$rkbmdPersediaanSKPD->pengelolaBarang = $arrayResult['pengelolaBarang'];
$rkbmdPersediaanSKPD->pejabatPengelolaBarang = $arrayResult['pejabat'];
$rkbmdPersediaanSKPD->pengurusPengelolaBarang = $arrayResult['pengurus'];
$rkbmdPersediaanSKPD->nipPengelola = $arrayResult['nipPengelola'];
$rkbmdPersediaanSKPD->nipPengurus = $arrayResult['nipPengurus'];
$rkbmdPersediaanSKPD->nipPejabat = $arrayResult['nipPejabat'];
