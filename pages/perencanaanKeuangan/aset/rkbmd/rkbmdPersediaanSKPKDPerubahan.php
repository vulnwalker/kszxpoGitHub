...
class rkbmdPersediaanSKPKDPerubahanObj  extends DaftarObj2{
	var $Prefix = 'rkbmdPersediaanSKPKDPerubahan';
	...
	var $FormName = 'rkbmdPersediaanSKPKDPerubahanForm';
	var $kode_skpd = '';
	var $modul = "RKBMD";
	var $jenisForm = "";
	var $tahun = "";
	...
	function setPage_HeaderOther(){
	  ...
	  <A href=\"pages.php?Pg=rkbmdPersediaanSKPKDPerubahan\" title='RKBMD PERSEDIAAN' style='color : blue;' > PERSEDIAAN </a>
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  case 'excel':{
			foreach ($_REQUEST as $key => $value) {
			 ...
			 /*if($rkbmdPersediaanSKPKDPerubahanSkpdfmUrusan =='0'){
			 	$err = "Pilih Urusan";
			 }elseif($rkbmdPersediaanSKPKDPerubahanSkpdfmSKPD =='00'){
			 	$err = "Pilih Bidang";
			 }elseif($rkbmdPersediaanSKPKDPerubahanSkpdfmUNIT =='00'){
			 	$err = "Pilih SKPD";
			 }elseif($rkbmdPersediaanSKPKDPerubahanSkpdfmSUBUNIT =='00'){
			 	$err = "Pilih Unit";
			 }elseif($rkbmdPersediaanSKPKDPerubahanSkpdfmSEKSI =='000'){
			 	$err = "Pilih Sub Unit";
			 }else{*/
	...
	case 'formBaru':{
		...
		case 'newTab':{
			 ...
			 $cekKeberadaanMangkluk =  mysql_num_rows(mysql_query("select * from view_renja where jenis_anggaran = '$this->jenisAnggaran' and tahun = '$this->tahun' and c1 = '$rkbmdPersediaanSKPKDPerubahanSkpdfmUrusan' and c = '$rkbmdPersediaanSKPKDPerubahanSkpdfmSKPD' and d='$rkbmdPersediaanSKPKDPerubahanSkpdfmUNIT' and e = '$rkbmdPersediaanSKPKDPerubahanSkpdfmSUBUNIT' and e1='$rkbmdPersediaanSKPKDPerubahanSkpdfmSEKSI'  and q!='0' and no_urut ='$nomorUrutSebelumnya' "));
			 $getDatarenja = mysql_fetch_array(mysql_query("select * from view_renja where jenis_anggaran = '$this->jenisAnggaran' and tahun = '$this->tahun' and c1 = '$rkbmdPersediaanSKPKDPerubahanSkpdfmUrusan' and c = '$rkbmdPersediaanSKPKDPerubahanSkpdfmSKPD' and d='$rkbmdPersediaanSKPKDPerubahanSkpdfmUNIT' and e = '$rkbmdPersediaanSKPKDPerubahanSkpdfmSUBUNIT' and e1='$rkbmdPersediaanSKPKDPerubahanSkpdfmSEKSI' and q!='0' and no_urut = '$nomorUrutSebelumnya'"));
			 $lastID = $getDatarenja['id_anggaran'];
			 	if($cekKeberadaanMangkluk != 0){
					if($getDatarenja['jenis_form_modul']  == 'PENYUSUNAN' ){
						$getJumlahRenjaValidasi = mysql_num_rows(mysql_query("select * from view_renja where jenis_anggaran = '$this->jenisAnggaran' and tahun = '$this->tahun' and c1 = '$rkbmdPersediaanSKPKDPerubahanSkpdfmUrusan' and c = '$rkbmdPersediaanSKPKDPerubahanSkpdfmSKPD' and d='$rkbmdPersediaanSKPKDPerubahanSkpdfmUNIT' and e = '$rkbmdPersediaanSKPKDPerubahanSkpdfmSUBUNIT' and e1='$rkbmdPersediaanSKPKDPerubahanSkpdfmSEKSI' and q!='0' $this->sqlValidasi and no_urut = '$nomorUrutSebelumnya'"));
						if($getJumlahRenjaValidasi == 0){
							$err = "Renja Belum Di Validasi";
						}
					}

					$getParentrkbmdPersediaanSKPKDPerubahan = mysql_fetch_array(mysql_query("select * from view_renja where id_anggaran = '$lastID'"));
					$parentC1 = $getParentrkbmdPersediaanSKPKDPerubahan['c1'];
					$parentC = $getParentrkbmdPersediaanSKPKDPerubahan['c'];
					$parentD = $getParentrkbmdPersediaanSKPKDPerubahan['d'];
					$parentE = $getParentrkbmdPersediaanSKPKDPerubahan['e'];
					$parentE1= $getParentrkbmdPersediaanSKPKDPerubahan['e1'];
					...
					if($cmbJenisrkbmdPersediaanSKPKDPerubahan == 'PERSEDIAAN'){
						$kemana = 'ins_v3';
					}else{
		...
		case 'editTab':{
			 $id = $_REQUEST['rkbmdPersediaanSKPKDPerubahan_cb'];
			foreach ($_REQUEST as $key => $value) {
				  $$key = $value;
			 }
					$nomorUrutSebelumnya = $this->nomorUrut - 1;
					$getParentrkbmdPersediaanSKPKDPerubahan = mysql_fetch_array(mysql_query("select * from view_rkbmd where id_anggaran = '$id[0]'"));
					foreach ($getParentrkbmdPersediaanSKPKDPerubahan as $key => $value) {
				 		 $$key = $value;
					 }
					$parentC1 = $getParentrkbmdPersediaanSKPKDPerubahan['c1'];
					$parentC = $getParentrkbmdPersediaanSKPKDPerubahan['c'];
		...
		case 'SaveValid':{
	    	foreach ($_REQUEST as $key => $value) {
			 ...
			 $getSKPD = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$rkbmdPersediaanSKPKDPerubahan_idplh'"));
			 ...
			 $query = VulnWalkerUpdate("tabel_anggaran",$data," id_anggaran = '$rkbmdPersediaanSKPKDPerubahan_idplh'");
			 mysql_query($query);
	   ...
	   case 'remove':{
		...
		case 'koreksi':{
			...
			$getrkbmdPersediaanSKPKDPerubahannya = mysql_fetch_array(mysql_query($queryRows));
			foreach ($getrkbmdPersediaanSKPKDPerubahannya as $key => $value) {
				  $$key = $value;
			}
			 ...
			 $dataSesuai = array(           'jenis_anggaran' => $this->jenisAnggaran,
			  ...
			  if($cekKoreksi > 0 ){
			   	 $getID = mysql_fetch_array(mysql_query("select * from tabel_anggaran where c1 = '$c1' and c='$c' and d = '$d' and e='$e' and e1='$e1' and bk='$bk' and ck='$ck' and dk='$dk' and p = '$p' and q='$q'  and f1='$f1' and f2='$f2' and f='$f' and g='$g' and h='$h' and i='$i' and j='$j' and id_jenis_pemeliharaan ='$id_jenis_pemeliharaan' and tahun = '$this->tahun' and jenis_anggaran = '$this->jenisAnggaran' and id_tahap = '$this->idTahap' "));
						...
						if($angkaKoreksi <= $kebutuhanRiil || (empty($jumlahOptimal) && empty($kebutuhanMax) && $getrkbmdPersediaanSKPKDPerubahannya['volume_barang'] >= $angkaKoreksi )  ){
							mysql_query("update tabel_anggaran set volume_barang = '$angkaKoreksi' , cara_pemenuhan = '$caraPemenuhan' where id_anggaran='$idnya'");
						}elseif($getrkbmdPersediaanSKPKDPerubahannya['volume_barang'] < $angkaKoreksi){
	 ...
	 function setPage_OtherScript(){
	   ...
	   return
	     ...
	     <script type='text/javascript' src='js/skpd.js' language='JavaScript' ></script>".
	      "<script type='text/javascript' src='js/perencanaanKeuangan/aset/rkbmd/rkbmdPersediaanSKPKDPerubahan.js' language='JavaScript' ></script>".
	     $scriptload;
	 }
...
/*		$nomorUrutSebelumnya = $this-> -1;*/
	   ...
	   elseif($this->jenisForm=="KOREKSI PENGGUNA"){
					...
					if($f == '00' && $q =='0')$TampilCheckBox = "";
						...
						$Koloms.= "<td class='$cssclass' align='left'></td>";
						 ...
						 $aksi  = "<img src='images/administrator/images/valid.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkbmdPersediaanSKPKDPerubahan.sesuai('$id_anggaran');></img>&nbsp  &nbsp <img src='images/administrator/images/invalid.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkbmdPersediaanSKPKDPerubahan.koreksi('$id_anggaran');></img>";
	...
	function Laporan($dt){
	 ...
	 $c1 = $dt['rkbmdPersediaanSKPKDPerubahanSkpdfmUrusan'];
	 $c = $dt['rkbmdPersediaanSKPKDPerubahanSkpdfmSKPD'];
	 $d = $dt['rkbmdPersediaanSKPKDPerubahanSkpdfmUNIT'];
	 $e = $dt['rkbmdPersediaanSKPKDPerubahanSkpdfmSUBUNIT'];
	 $e1 = $dt['rkbmdPersediaanSKPKDPerubahanSkpdfmSEKSI'];
	...
	$TampilOpt =
			...
			<td width=\"100%\" valign=\"top\">" .
				WilSKPD_ajxVW("rkbmdPersediaanSKPKDPerubahanSkpd") .
			"</td>
			<td >" .
			"</td></tr>
			<tr><td>
			<input type='hidden' name='cmbJenisrkbmdPersediaanSKPKDPerubahan' id='cmbJenisrkbmdPersediaanSKPKDPerubahan' value='PERSEDIAAN'>
			</td></tr>
			</table>";
	...
	function getDaftarOpsi($Mode=1){
		global $Main, $HTTP_COOKIE_VARS;
		$UID  = $_COOKIE['coID'];
		$c1   = $_REQUEST['rkbmdPersediaanSKPKDPerubahanSkpdfmUrusan'];
		$c    = $_REQUEST['rkbmdPersediaanSKPKDPerubahanSkpdfmSKPD'];
		$d    = $_REQUEST['rkbmdPersediaanSKPKDPerubahanSkpdfmUNIT'];
		$e    = $_REQUEST['rkbmdPersediaanSKPKDPerubahanSkpdfmSUBUNIT'];
		$e1   = $_REQUEST['rkbmdPersediaanSKPKDPerubahanSkpdfmSEKSI'];
		if(isset($e1)){
		...
		$fmKODE = $_REQUEST['fmKODE'];
		$fmBARANG = $_REQUEST['fmBARANG'];
		$cmbJenisrkbmdPersediaanSKPKDPerubahan = $_REQUEST['cmbJenisrkbmdPersediaanSKPKDPerubahan'];
		$arrKondisi = array();
...
$rkbmdPersediaanSKPKDPerubahan = new rkbmdPersediaanSKPKDPerubahanObj();
...
$rkbmdPersediaanSKPKDPerubahan->jenisForm = $jenisForm;
$rkbmdPersediaanSKPKDPerubahan->nomorUrut = $nomorUrut;
$rkbmdPersediaanSKPKDPerubahan->urutTerakhir = $nomorUrut;
$rkbmdPersediaanSKPKDPerubahan->tahun = $tahun;
$rkbmdPersediaanSKPKDPerubahan->jenisAnggaran = $jenisAnggaran;
$rkbmdPersediaanSKPKDPerubahan->idTahap = $idTahap;
$rkbmdPersediaanSKPKDPerubahan->username = $_COOKIE['coID'];
$rkbmdPersediaanSKPKDPerubahan->wajibValidasi = $arrayResult['wajib_validasi'];
if($rkbmdPersediaanSKPKDPerubahan->wajibValidasi == TRUE){
	$rkbmdPersediaanSKPKDPerubahan->sqlValidasi = " and status_validasi ='1' ";
}else{
	$rkbmdPersediaanSKPKDPerubahan->sqlValidasi = " ";
}

$rkbmdPersediaanSKPKDPerubahan->provinsi = $arrayResult['provinsi'];
$rkbmdPersediaanSKPKDPerubahan->kota = $arrayResult['kota'];
$rkbmdPersediaanSKPKDPerubahan->pengelolaBarang = $arrayResult['pengelolaBarang'];
$rkbmdPersediaanSKPKDPerubahan->pejabatPengelolaBarang = $arrayResult['pejabat'];
$rkbmdPersediaanSKPKDPerubahan->pengurusPengelolaBarang = $arrayResult['pengurus'];
$rkbmdPersediaanSKPKDPerubahan->nipPengelola = $arrayResult['nipPengelola'];
$rkbmdPersediaanSKPKDPerubahan->nipPengurus = $arrayResult['nipPengurus'];
$rkbmdPersediaanSKPKDPerubahan->nipPejabat = $arrayResult['nipPejabat'];
