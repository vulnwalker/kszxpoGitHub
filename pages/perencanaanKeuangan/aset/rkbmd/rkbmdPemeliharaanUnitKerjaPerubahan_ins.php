<?php
class rkbmdPemeliharaanUnitKerjaPerubahan_insObj  extends DaftarObj2{
	var $Prefix = 'rkbmdPemeliharaanUnitKerjaPerubahan_ins';
	...
	var $FormName = 'rkbmdPemeliharaanUnitKerjaPerubahan_insForm';
	var $noModul=14;
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){
		...
		case 'Simpan':{
			...
			$concat = $f.".".$g.".".$h.".".$i.".".$j;
			$username = $_COOKIE['coID'];
			$cekSame = mysql_num_rows(mysql_query("select * from rkbmd_pemeliharaan where concat(f,'.',g,'.',h,'.',i,'.',j) = '$concat'  and user = '$username'"));
	...
	$cmbKegiatan = cmbQuery('q', $q, $codeAndNameKegiatan,' disabled','-- KEGIATAN --');
		...
		case 'subHapus':{
			...
			$concat = $f.".".$g.".".$h.".".$i.".".$j;
			$username = $_COOKIE['coID'];
			$execute = mysql_query("select * from rkbmd_pemeliharaan where concat(f,'.',g,'.',h,'.',i,'.',j) = '$concat'  and user = '$username'");
		...
		case 'moveBack':{
			...
			mysql_query("delete from rkbmd_pemeliharaan where user = '$username' and concat(f,'.',g,'.',h,'.',i,'.',j) = '$kodeBarang'");
					while($rows = mysql_fetch_array($execute)){
					 foreach ($rows as $key => $value) {
				  		$$key = $value;
					 }
		...
		case 'moveList':{
			...
			$concat = $f.".".$g.".".$h.".".$i.".".$j;
			$execute = mysql_query("select * from rkbmd_pemeliharaan where user = '$username' and concat(f,'.',g,'.',h,'.',i,'.',j) = '$concat'");
			mysql_query("delete from temp_rkbmd_pemeliharaan where user = '$username'");
			while($rows = mysql_fetch_array($execute)){
				foreach ($rows as $key => $value) {
			  		$$key = $value;
	 ...
	 function setPage_OtherScript(){
	  ...
	  return
	    ...
	    "<script type='text/javascript' src='js/perencanaanKeuangan/aset/rkbmd/rkbmdPemeliharaanUnitKerjaPerubahan_ins.js' language='JavaScript' ></script>".
	    "<script type='text/javascript' src='js/perencanaanKeuangan/aset/rkbmd/popupProgramRKBMDUnitKerja.js' language='JavaScript' ></script>
	    <script type='text/javascript' src='js//perencanaanKeuangan/aset/rkbmd/popupBarangRKBMDPemeliharaan.js' language='JavaScript' ></script>
	...
	$hitungData = mysql_num_rows(mysql_query("select * from rkbmd_pemeliharaan where concat(f,'.',g,'.',h,'.',i,'.',j) = '$concat2' and user = '$username'"));
	if($hitungData > 1){
		$getMin = mysql_fetch_array(mysql_query("select min(concat(c1,'.',c,'.',d,'.',e,'.',e1,'.',bk,'.',ck,'.',dk,'.',p,'.',q,'.',f,'.',g,'.',h,'.',i,'.',j,'.',id_jenis_pemeliharaan)) as min from rkbmd_pemeliharaan where concat(f,'.',g,'.',h,'.',i,'.',j) = '$concat2' and user = '$username'"));
		$min = $getMin['min'];
		$realConcat = $c1.".".$c.".".$d.".".$e.".".$e1.".".$bk.".".$ck.".".$dk.".".$p.".".$q.".".$f.".".$g.".".$h.".".$i.".".$j.".".$id_jenis_pemeliharaan;
		if($realConcat == $min){
			$getTotal = mysql_fetch_array(mysql_query("select sum(volume_barang) as total from rkbmd_pemeliharaan where concat(f,'.',g,'.',h,'.',i,'.',j) = '$concat2' and user='$username'"));
			...
			$aksi  = "<img src='images/administrator/images/invalid.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkbmdPemeliharaanUnitKerjaPerubahan_ins.subHapus('$id');></img>&nbsp &nbsp <img src='images/administrator/images/edit_f2.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkbmdPemeliharaanUnitKerjaPerubahan_ins.moveList('$id');></img> ";
		}else{
	...
	function tabelPemeliharaan(){
		...
		while($dt = mysql_fetch_array($aqry)){
			...
			if($dt['id_jenis_pemeliharaan'] == '' || $dt['id_jenis_pemeliharaan'] == '0'){
				$action = "<img id='action$id' src='datepicker/save.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkbmdPemeliharaanUnitKerjaPerubahan_ins.subSubSave('$id');>";
				...
				$action = "<img id='action$id' src='images/administrator/images/invalid.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkbmdPemeliharaanUnitKerjaPerubahan_ins.subSubDel('$id');>";
			...
			$datanya.="
				...
				<tr class='row0'>
					<td class='GarisDaftar' align='right'><a onclick=rkbmdPemeliharaanUnitKerjaPerubahan_ins.subSubEdit($id) style='cursor:pointer;'>$no</a></td>
				...
				array("
					<table class='koptable' style='min-width:900px;' border='1'>
						<tr>
							...
							<th class='th01'>
								<span id='atasbutton'>
								<a href='javascript:rkbmdPemeliharaanUnitKerjaPerubahan_ins.$tujuan' id='linkAtasButton' /><img id='gambarAtasButton' src='$gambar' style='width:20px;height:20px;' /></a>
								</span>
							</th>
						</tr>
						$datanya
	...
	$e   = $ambilSKPD['e'];
	$e1  = $ambilSKPD['e1'];
	$cekMayoritas = mysql_fetch_array(mysql_query("select * from rkbmd_pemeliharaan where user = '$this->username'"));
	$e = $cekMayoritas['e'];
	$e1 = $cekMayoritas['e1'];
	...
	$cmbKegiatan = cmbQuery('q', $q, $codeAndNameKegiatan,'onchange=rkbmdPemeliharaanUnitKerjaPerubahan_ins.CekAda(); ','-- KEGIATAN --');
	$username = $_COOKIE['coID'];
	$matiButton = "";
					...
					$this->isiform(
						array(
							...
							array(
								...
								<input type='button' value='Cari' id='findProgram' onclick ='rkbmdPemeliharaanUnitKerjaPerubahan_ins.CariProgram($ID_RENJA)'  title='Cari Program dan Kegiatan' $matiButton >"
							),
							array(
								'label'=>'KEGIATAN',
								'label-width'=>'200px;',
			...
			genFilterBar(
						...
						array(
							...
							array(
									...
									'value'=>"<input type='text' name='kodeBarang' onkeyup='cariBarang.pilBar2(this.value)' id='kodeBarang' placeholder='KODE BARANG' style='width:150px;' value='".$dt['kodebarangnya']."' readonly />
										...
										<button type='button' id='findBarang' onclick=rkbmdPemeliharaanUnitKerjaPerubahan_ins.CariBarang('$filterSKPD'); > Cari </button>
									",
								),
							array(
				...
				genFilterBar(
					...
					<td class='border:none'>
						<a class='toolbar' id='btsave' href='javascript:rkbmdPemeliharaanUnitKerjaPerubahan_ins.$tujuan'>
					...
					<tbody><tr valign='middle' align='center'>
					<td class='border:none'>
						<a class='toolbar' id='btcancel' href='javascript:rkbmdPemeliharaanUnitKerjaPerubahan_ins.closeTab()'>
					...
					<tbody><tr valign='middle' align='center'>
					<td class='border:none'>
						<a class='toolbar' id='finish' href='javascript:rkbmdPemeliharaanUnitKerjaPerubahan_ins.finish()'>
...
$rkbmdPemeliharaanUnitKerjaPerubahan_ins = new rkbmdPemeliharaanUnitKerjaPerubahan_insObj();

$arrayResult = tahapPerencanaanV3($rkbmdPemeliharaanUnitKerjaPerubahan_ins->modul,"PERUBAHAN");
...
$rkbmdPemeliharaanUnitKerjaPerubahan_ins->jenisForm = $jenisForm;
$rkbmdPemeliharaanUnitKerjaPerubahan_ins->nomorUrut = $nomorUrut;
$rkbmdPemeliharaanUnitKerjaPerubahan_ins->urutTerakhir = $nomorUrut;
$rkbmdPemeliharaanUnitKerjaPerubahan_ins->tahun = $tahun;
$rkbmdPemeliharaanUnitKerjaPerubahan_ins->jenisAnggaran = $jenisAnggaran;
$rkbmdPemeliharaanUnitKerjaPerubahan_ins->idTahap = $idTahap;
$rkbmdPemeliharaanUnitKerjaPerubahan_ins->username = $_COOKIE['coID'];
$rkbmdPemeliharaanUnitKerjaPerubahan_ins->wajibValidasi = $Main->wajibValidasi;
if($rkbmdPemeliharaanUnitKerjaPerubahan_ins->wajibValidasi == TRUE){
	$rkbmdPemeliharaanUnitKerjaPerubahan_ins->sqlValidasi = " and status_validasi ='1' ";
}else{
	$rkbmdPemeliharaanUnitKerjaPerubahan_ins->sqlValidasi = " ";
}
