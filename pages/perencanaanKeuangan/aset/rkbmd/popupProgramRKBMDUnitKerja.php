...
class popupProgramRKBMDUnitKerjaObj  extends DaftarObj2{
	var $Prefix = 'popupProgramRKBMDUnitKerja';
	...
	var $FormName = 'popupProgramRKBMDUnitKerjaForm';
	var $noModul=14;
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){
		...
		case 'getdata':{
				...
				$getView = mysql_query("select * from view_renja where c1 = '$c1' and c='$c'  and d = '$d' and e ='$e' and e1 ='$e1' and bk ='$abk' and ck = '$ack' and ck = '$adk' and p='$ap' and tahun = '$tahun' and jenis_anggaran = '$jenis_anggaran' and id_tahap = '$id_tahap'");
				$arrayID = array();
				while($rows = mysql_fetch_array($getView)){
					if($rows['jenis_form_modul'] == "PENYUSUNAN" && $this->wajibValidasi == TRUE && $rows['status_validasi'] != "1"){
						...
						if($rows['q'] == '0'){
							...
							$concat = $rows['bk'].".".$rows['ck'].".".$rows['dk'].".".$rows['p'].".".$rows['q'];
							$arrayID[]  = "concat(bk,'.',ck,'.',dk,'.',p,'.',q) = '$concat' " ;
						}
				...
				$getSecondView = mysql_query("select * from view_renja where c1 = '$c1' and c='$c'  and d = '$d' and e ='00' and e1 ='000' and bk ='$abk' and ck = '$ack' and dk = '$adk' and p='$ap' and tahun = '$tahun' and jenis_anggaran = '$jenis_anggaran' and id_tahap = '$id_tahap'");
				$arrayID2 = array();
				while($rows2 = mysql_fetch_array($getSecondView)){
					if($rows2['jenis_form_modul'] == "PENYUSUNAN" && $this->wajibValidasi == TRUE && $rows2['status_validasi'] != "1"){
						...
						if($rows2['q'] == '0'){
							...
							$concat2 = $rows2['bk'].".".$rows2['ck'].".".$rows2['dk'].".".$rows2['p'].".".$rows2['q'];
							$arrayID2[]  = "concat(bk,'.',ck,'.',dk,'.',p,'.',q) = '$concat2' " ;
						}
				...
				$content = array("bk" => $abk, "ck" => $ack , "dk" => $adk, "p" => $ap, "cmbKegiatan" => $cmbKegiatan, "program" => $program, "sql" => $codeAndNameKegiatan);
		break;
		}
	...
	function setKolomData($no, $isi, $Mode, $TampilCheckBox){
	 ...
	 $Koloms[] = array('align="left"',"<a style='cursor:pointer;' onclick = popupProgramRKBMDUnitKerja.windowSave('$tak'); >" .$isi['nama']."</a>" );
	 return $Koloms;
	}
	...
	function getDaftarOpsi($Mode=1){
		...
		$arrayID = array();
		while($rows = mysql_fetch_array($getView)){
			$concat = $rows['bk'].".".$rows['ck'].".".$rows['dk'].".".$rows['p'];
			$abk = $rows['bk'];
			$ack = $rows['ck'];
			$adk = $rows['dk'];
			$ap = $rows['p'];
			$arrayKondisiKegiatan = array();
			$getKegiatan = mysql_query("select * from tabel_anggaran  where bk='$abk' and ck='$ack' and dk='$adk'  and p='$ap' and q !='0' and c1 = '$c1' and c= '$c' and d = '$d' and e ='$e' and e1 ='$e1' and tahun = '$tahun' and jenis_anggaran = '$jenis_anggaran' and id_tahap = '$id_tahap' ");
			while($root = mysql_fetch_array($getKegiatan)){
				if($root['jenis_form_modul'] == "PENYUSUNAN" && $this->wajibValidasi == TRUE && $root['status_validasi'] !='1'){
					...
					$concat3 = $root['bk'].".".$root['ck'].".".$root['dk'].".".$root['p'].".".$root['q'];
					$arrayKondisiKegiatan[]  = "concat(bk,'.',ck,'.',dk,'.',p,'.',q) = '$concat3' " ;
			...
			if($_REQUEST['kategori'] == "PENGADAAN"){
					$codeAndNameKegiatan = "select q, concat(q,'. ', nama) from ref_program where bk = '$abk' and ck = '$ack' and dk = '$adk' and p = '$ap' and q !='0' and kategori !='2' and $kondisiKegiatan";
			}elseif($_REQUEST['kategori'] == "PEMELIHARAAN"){
					$codeAndNameKegiatan = "select q, concat(q,'. ', nama) from ref_program where bk = '$abk' and ck = '$ack' and dk = '$adk' and p = '$ap' and q !='0' and kategori !='1' and $kondisiKegiatan";
			}
			$got = mysql_query($codeAndNameKegiatan);
			if(mysql_num_rows($got) > 0){
				$arrayID[]  = "concat(bk,'.',ck,'.',dk,'.',p) = '$concat' " ;
			}
		...
		$arrayID2 = array();
		while($rows2 = mysql_fetch_array($getSecondView)){
			$concat2 = $rows2['bk'].".".$rows2['ck'].".".$rows2['dk'].".".$rows2['p'];
			$abk2 = $rows2['bk'];
			$ack2 = $rows2['ck'];
			$adk2 = $rows2['dk'];
			$ap2 = $rows2['p'];
			$arrayKondisiKegiatan2 = array();
			$getKegiatan2 = mysql_query("select * from tabel_anggaran  where bk='$abk2' and ck='$ack2' and dk='$adk2'  and p='$ap2' and q !='0' and c1 = '$c1' and c= '$c' and d = '$d' and e ='00' and e1 ='000' and tahun = '$tahun' and jenis_anggaran = '$jenis_anggaran' and id_tahap = '$id_tahap' ");
			while($root2 = mysql_fetch_array($getKegiatan2)){
				if($root2['jenis_form_modul'] == "PENYUSUNAN" && $this->wajibValidasi == TRUE && $root2['status_validasi'] !='1'){
					...
					$concat32 = $root2['bk'].".".$root2['ck'].".".$root2['dk'].".".$root2['p'].".".$root2['q'];
					$arrayKondisiKegiatan2[]  = "concat(bk,'.',ck,'.',dk,'.',p,'.',q) = '$concat32' " ;
			...
			if($_REQUEST['kategori'] == "PENGADAAN"){
					$codeAndNameKegiatan2 = "select q, concat(q,'. ', nama) from ref_program where bk = '$abk2' and ck = '$ack2' and dk = '$adk2' and p = '$ap2' and q !='0' and kategori !='2' and $kondisiKegiatan2";
			}elseif($_REQUEST['kategori'] == "PEMELIHARAAN"){
					$codeAndNameKegiatan2 = "select q, concat(q,'. ', nama) from ref_program where bk = '$abk2' and ck = '$ack2' and dk = '$adk2' and p = '$ap2' and q !='0' and kategori !='1' and $kondisiKegiatan2";
			}
			$got2 = mysql_query($codeAndNameKegiatan2);
			if(mysql_num_rows($got2) > 0){
				$arrayID2[]  = "concat(bk,'.',ck,'.',dk,'.',p) = '$concat2' " ;
			}
...
$popupProgramRKBMDUnitKerja = new popupProgramRKBMDUnitKerjaObj();
$popupProgramRKBMDUnitKerja->wajibValidasi = $Main->wajibValidasi;
?>
