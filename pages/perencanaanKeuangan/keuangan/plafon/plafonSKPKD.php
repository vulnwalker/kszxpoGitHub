...
class plafonSKPKDObj  extends DaftarObj2{	
	var $Prefix = 'plafonSKPKD';
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='plafonSKPKD.xls';
	var $namaModulCetak='PLAFON';
	var $Cetak_Judul = 'PLAFON';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'plafonSKPKDForm';
	var $modul = "PLAFON";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){	
		...
		case 'formEdit':{
			...
			$getData = mysql_fetch_array(mysql_query("select * from view_plafon where id_anggaran ='$plafonSKPKD_cb[0]'"));
	    ...
	    case 'SaveValid':{
	    	foreach ($_REQUEST as $key => $value) { 
			 ...
			 $query = VulnWalkerUpdate("tabel_anggaran",$data," id_anggaran = '$plafonSKPKD_idplh'");
			 mysql_query($query);
		break;
	    }
   ...
   if($this->jenisForm == "KOREKSI"){
		...
		return 	
			...
			<script type='text/javascript' src='js/perencanaanKeuangan/keuangan/plafon/plafonSKPKD.js' language='JavaScript' ></script> 
...
if($_COOKIE['cofmSKPD']!='00'){
	...
	function setKolomHeader($Mode=1, $Checkbox=''){
	 ...
	 if ($this->jenisForm == "PENYUSUNAN"){
	 	if($this->wajibValidasi == TRUE){
			$tergantungJenisForm = "		<th class='th01' width='100' >VALIDASI</th>
			";
		}
	...
	function setKolomData($no, $isi, $Mode, $TampilCheckBox){
	 ...
	 //TAHAP PENYUSUNAN
			 ...
			 $aksi  = "<img src='images/administrator/images/valid.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=plafonSKPKD.sesuai('$id_anggaran');></img>&nbsp  &nbsp <img src='images/administrator/images/invalid.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=plafonSKPKD.koreksi('$id_anggaran');></img>&nbsp &nbsp <img src='images/administrator/images/edit_f2.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=plafonSKPKD.catatan('$id_anggaran');></img> ";
			 if ($d == '00') {
			 $aksi = "";
			 }
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	$TampilOpt = 
			...
			<tr>
			".CmbUrusanBidangSkpd('plafonSKPKD')."
			</tr>
			</table>".
			"</div>"
	...
	function Hapus($ids){ 
		...
		for($i = 0; $i<count($ids); $i++)	{
			$getData = mysql_fetch_array(mysql_query("select * from view_plafon where id_anggaran ='$ids[$i]' "));
			if($getData['status_validasi'] == '1'){
				
			}else{
				$qy = "DELETE FROM $this->TblName_Hapus WHERE id_anggaran='".$ids[$i]."' ";
				$qry = mysql_query($qy);
			}
...
$plafonSKPKD = new plafonSKPKDObj();
$arrayResult = tahapPerencanaanV3("PLAFON","MURNI");
...
$plafonSKPKD->jenisForm = $jenisForm;
$plafonSKPKD->nomorUrut = $nomorUrut;
$plafonSKPKD->tahun = $tahun;
$plafonSKPKD->jenisAnggaran = $jenisAnggaran;
$plafonSKPKD->idTahap = $idTahap;
$plafonSKPKD->username = $_COOKIE['coID'];
$plafonSKPKD->wajibValidasi = $arrayResult['wajib_validasi'];
$plafonSKPKD->settingAnggaran = $arrayResult['settingAnggaran'];


/*$arrayResult = VulnWalkerTahap_v2($plafonSKPKD->modul);
$jenisForm = $arrayResult['jenisForm'];
$nomorUrut = $arrayResult['nomorUrut'];
$tahun = $arrayResult['tahun'];
...
$plafonSKPKD->wajibValidasi = $Main->wajibValidasi;


if(empty($plafonSKPKD->tahun)){
	...
	$plafonSKPKD->tahun = "select max(id_anggaran) as max from view_plafon where nama_modul = 'PLAFON'";
	$plafonSKPKD->tahun  = $get2['tahun'];
	$plafonSKPKD->jenisAnggaran = $get2['jenis_anggaran'];
	$plafonSKPKD->urutTerakhir = $get2['no_urut'];
	$plafonSKPKD->jenisFormTerakhir = $get2['jenis_form_modul'];
	$plafonSKPKD->urutSebelumnya = $plafonSKPKD->urutTerakhir - 1;
	...
	$plafonSKPKD->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$plafonSKPKD->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$plafonSKPKD->noUrutTerakhirPlafon = $namaTahap['no_urut'];
	...
	$plafonSKPKD->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];
	
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$plafonSKPKD->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$plafonSKPKD->idTahap'"));
	$plafonSKPKD->currentTahap = $getCurrenttahap['nama_tahap'];
	
	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$plafonSKPKD->idTahap'"));
	$plafonSKPKD->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$plafonSKPKD->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$plafonSKPKD->noUrutTerakhirPlafon = $namaTahap['no_urut'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
