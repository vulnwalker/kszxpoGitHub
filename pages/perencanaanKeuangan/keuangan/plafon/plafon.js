var plafon_v2 = new DaftarObj2({
	prefix : 'plafon_v2',
	url : 'pages.php?Pg=plafon_v2', 
	formName : 'plafon_v2Form',
	plafon_v2_form : '0',//default js plafon_v2
	loading: function(){
		//alert('loading');
		this.topBarRender();
		this.filterRender();
...
/*				me.sumHalRender();*/
	...
	Baru: function(){	
		...
		if (err =='' ){		
			...
			if(me.plafon_v2_form==0){//baru dari plafon_v2
	...
	koreksi:function(id){
		$("#span"+id).html("<input type='text' name='"+id+"' id='"+id+"' onkeyup='return plafon_v2.submitKoreksi(event,id)'  onkeypress='return event.charCode >= 48 && event.charCode <= 57'  style='text-align: right;'>  <span id='bantu"+id+"' ></span>");
	},
		...
		$.ajax({
			...
			url: this.url+'&tipe=simpan',
		  	success: function(data) {		
				...
				//document.getElementById(cover).innerHTML = resp.content;
				if(resp.err==''){
					if(me.plafon_v2_form==0){
						me.Close();
						me.AfterSimpan();						
					}else{
						me.Close();
						barang.refreshComboplafon_v2();
					}
