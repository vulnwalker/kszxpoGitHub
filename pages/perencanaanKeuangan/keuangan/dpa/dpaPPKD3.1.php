...
class dpaPPKD31_v2Obj  extends DaftarObj2{	
	var $Prefix = 'dpaPPKD31_v2';
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='dpaPPKD31_v2.xls';
	var $namaModulCetak='DPA';
	var $Cetak_Judul = 'DPA';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'dpaPPKD31_v2Form';
	var $modul = "DPA";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	...
	function genRowSum($ColStyle, $Mode, $Total){
		...
		$getData = mysql_fetch_array(mysql_query("select sum(jumlah_harga) from tabel_anggaran where (rincian_perhitungan !='' or f !='00' ) and id_tahap='$idTahap' and tahun ='$this->tahun' and jenis_anggaran = '$this->jenisAnggaran' and jenis_rka = '3.1' and nama_modul='DPA-PPKD' $kondisiSKPD $kondisiRekening"));
	...
	$getJumlahBarang = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$dpaPPKD31_v2_idplh'"));
	...
	$sisaPaguIndikatif = $paguIndikatif - $getPaguYangTelahTerpakai['paguYangTerpakai'];
	 ...
	 if(mysql_num_rows(mysql_query("select * from view_dpa_ppkd_3_1 where c1='0' and f = '00' and k = '$k' and l ='$l' and m='$m' and n='$n' and o='$o'  and id_tahap='$this->idTahap' ")) > 0){
 	 ...
 	 if(empty($cmbJenisDPAForm) ){
		...
		$query = VulnWalkerUpdate('tabel_anggaran',$data,"id_anggaran = '$dpaPPKD31_v2_idplh'");
		mysql_query($query);
	 }
	  ...
	  switch($tipe){	
		...
		case 'Report':{	
			foreach ($_REQUEST as $key => $value) { 
				...
				if(mysql_num_rows(mysql_query("select * from skpd_report_dpa_ppkd_31 where username= '$this->username'")) == 0){
					$data = array(
					...
					$query = VulnWalkerInsert('skpd_report_dpa_ppkd_31',$data);
					mysql_query($query);
				}else{
					$data = array(
								  'username' => $this->username,
					...
					$query = VulnWalkerUpdate('skpd_report_dpa_ppkd_31',$data,"username = '$this->username'");
					mysql_query($query);
				}
		...
		case 'sync':{	
			if($this->jenisForm == 'READ'){
			//grabing
			$getMaxIdTahapDPA= mysql_fetch_array(mysql_query("select max(id_tahap) from view_rka_ppkd_3_1 where tahun='$this->tahun' and jenis_anggaran = '$this->jenisAnggaran'"));
			$idTahapTerakhir = $getMaxIdTahapDPA['max(id_tahap)'];
		    $kondisiFilter = " and id_tahap = '$idTahapTerakhir' ";
			$getJenisFormRka = mysql_fetch_array(mysql_query("selecrt * from view_rka_ppkd_3_1 where id_tahap = '$idTahapTerakhir'"));
			if($getJenisFormRka['jenis_form_modul'] == "PENYUSUNAN" && $this->wajibValidasi == TRUE ){
				$kondisiFilter = $kondisiFilter." and status_validasi ='1' ";
			}
			
			$getAllChildern = mysql_query("select * from view_rka_ppkd_3_1 where id_tahap = '$idTahapTerakhir' and rincian_perhitungan !='' $kondisiFilter ");
			...
			while($rows = mysql_fetch_array($getAllChildern)){
				...
				$queryCekPekerjaan = "select * from view_dpa_ppkd_3_1 where c1='0' and o1='$o1' and f = '00' and rincian_perhitungan = ''  and id_tahap='$this->idTahap' ";
				...
				if(mysql_num_rows(mysql_query("select * from view_dpa_ppkd_3_1 where k='$k' and l='$l' and m='$m' and n='$n' and o='$o' and c1 = '0' and id_tahap='$this->idTahap' ")) == 0){
					$arrayRekening = array(
												 'c1' => '0',
												 'c' => '00',
												 'd' => '00',
					...
					if(mysql_num_rows(mysql_query("select * from view_dpa_ppkd_3_1 where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and k='$k' and l='$l' and m='$m' and n='$n' and o='$o' and o1='$o1' and rincian_perhitungan = '$rincian_perhitungan' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and id_tahap='$this->idTahap'")) == 0){
						$data = array(	'c1' => $c1,
										'c' => $c,
										'd' => $d,
										'e' => $e,
		...
		case 'checkAlokasi':{
			...
			if($this->jenisForm != "READ"){
				...
				$jenisAlokasi = "";
				if(mysql_num_rows(mysql_query("select * from tabel_spd where tahun ='$this->tahun' and anggaran ='$this->jenisAnggaran' and c1 ='$c1' and c='$c' and d ='$d'  and jenis_rka = '3.1' and jenis_dpa = 'DPA-PPKD'")) != 0){
					$getAlokasi = mysql_fetch_array(mysql_query("select * from tabel_spd where tahun ='$this->tahun' and anggaran ='$this->jenisAnggaran' and c1 ='$c1' and c='$c' and d ='$d' and jenis_rka = '3.1' and jenis_dpa = 'DPA-PPKD' "));
			...
			if($this->jenisForm != "READ"){
				...
				if($err == ""){
					...
					if(mysql_num_rows(mysql_query("select * from tabel_spd where tahun ='$this->tahun' and anggaran ='$this->jenisAnggaran' and c1 ='$c1' and c='$c' and d ='$d'  and jenis_rka = '3.1' and jenis_dpa = 'DPA-PPKD'")) == 0){
						...
						$grepId = mysql_fetch_array(mysql_query("select * from tabel_spd where tahun ='$this->tahun' and anggaran ='$this->jenisAnggaran' and c1 ='$c1' and c='$c' and d ='$d'  and jenis_rka = '3.1' and jenis_dpa = 'DPA-PPKD'"));
						$id = $grepId['id'];
						$data = array(
										'jenis_alokasi_kas' => $jenisAlokasi,
										"alokasi_jan" => $jan,
	...
	<A href=\"pages.php?Pg=dpa-ppkd-3.1_v2\" title='DPA-PPKD MURNI'  style='color:blue;' > DPA-PPKD 3.1 </a> |
   ...
   function setPage_OtherScript(){
		...
		return 	
			...
			<script type='text/javascript' src='js/perencanaan_v2/dpa/dpaPPKD3.1.js' language='JavaScript' ></script> 
	 ...
	 $this->form_height = 400;
	  if ($this->form_fmST==0) {
		...
		foreach ($dt as $key => $value) { 
		 ...
		 $jenis_rka = $jenis_rka;
		 $cmbJenisDPA = cmbArray('cmbJenisDPAForm',$jenis_rka,$arrayJenisDPA,'-- JENIS DPA --','onchange=dpaPPKD31_v2.unlockFindRekening();');
	 	 if(empty($jenis_rka)){
	  ...
	  $this->form_fields = array(
			...
			'kode7' => array( 
						...
						'value'=> "<input type='text' id='kodeRekening' name='kodeRekening' style='width:120px;' readonly value = '$kodeRekening'> &nbsp&nbsp
						 ...
						 <button type='button' id='findRekening' onclick=dpaPPKD31_v2.findRekening('$jenis_rka'); $tergantungJenis> CARI </button> "
						 ),
			'kode8' => array( 
						'label'=>'HARGA SATUAN',
						'labelWidth'=>150, 
						'value'=> "<input type='text' name='hargaSatuan' id='hargaSatuan' value='$hargaSatuan' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='dpaPPKD31_v2.bantu();' > <span id='bantu' style='color:red;'> </span>"
						 ),	
	...
	function setKolomHeader($Mode=1, $Checkbox=''){
		...
		$headerTable =
		  "<thead>
		   <tr>
	  		 <th class='th01' width='10' rowspan='2'></th>
		...
		if(!empty($hiddenP)){
			...
			$getIDTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where no_urut ='$this->urutTerakhir' and tahun ='$this->tahun' and anggaran='$this->jenisAnggaran'"));			
			 ...
			 if($c1 == '0'){
				...
				$tanda = "";
			 	$getAllMemberOfThisRekening = mysql_query("select * from view_dpa_ppkd_3_1 where id_tahap = '$idTahap' and k='$k' and l ='$l' and m = '$m' and n ='$n' and o ='$o' and (rincian_perhitungan != '' or j!='000')");
				...
				for($nambur = 0 ; $nambur < sizeof($arrayMemberRek) ; $nambur++){
					if(mysql_num_rows(mysql_query("select * from tabel_spd where tahun = '$this->tahun' and anggaran = '$this->jenisAnggaran' and jenis_rka = '3.1' and jenis_dpa = 'DPA-PPKD' and concat(c1,'.',c,'.',d) = '$arrayMemberRek[$nambur]'")) == 0){
						$tanda = "color:red;";
					}
				}	 
			 ...
			 //batas
			 	if(strlen($k) > 1){
					...
					if($k == '0' && $n == '0'){
						...
						$jarak = '15px';
				 		$cekAdaParent = mysql_num_rows(mysql_query("select * from view_dpa_ppkd_3_1 where id_tahap = '$idTahap' and o1 = '$o1' and o1 !='0' and o1 !='' "));
				...
				$ilustrasi = "";
				$getSumJumlahBarang = mysql_fetch_array(mysql_query("select sum(volume_rek) as total from view_dpa_ppkd_3_1 where k='$k' and l='$l' and m='$m' and n='$n' and o='$o' $kondisiSKPD and id_tahap='$idTahap'"));
				$Koloms[] = array('align="left"',"" );
			 ...
			 $getSumSatuanRek = mysql_fetch_array(mysql_query("select sum(jumlah) as total from view_dpa_ppkd_3_1 where k='$k' and l='$l' and m='$m' and n='$n'  and o='$o' $kondisiSKPD and id_tahap='$idTahap'"));
			 ...
			 if($c1 == '0'){
			 if($k == '0' && $n =='0'){
			 	$getTotalJumalhHarga = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as total from view_dpa_ppkd_3_1 where o1='$o1' $kondisiSKPD $kondisiBelanja and id_tahap='$idTahap'"));
			 }else{
			 	$getTotalJumalhHarga = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as total from view_dpa_ppkd_3_1 where k='$k' and l='$l' and m='$m' and n='$n' and o='$o' $kondisiSKPD and id_tahap='$idTahap'"));	
			 }
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	foreach ($_COOKIE as $key => $value) { 
		...
		$urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=dpaPPKD31_v2.refreshList(true);','-- URUSAN --');
		
		$codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
		$bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=dpaPPKD31_v2.refreshList(true);','-- BIDANG --');
		
		$codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
		$skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=dpaPPKD31_v2.refreshList(true);','-- SKPD --');
		...
		$unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=dpaPPKD31_v2.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=dpaPPKD31_v2.refreshList(true);','-- SUB UNIT --');
...
function formAlokasi($dt){	
	...
	$arrayJenisPerhitungan = array(
	 ...
	 $getKodeRekening = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran ='$terpilih'"));
				...
				$n = $getKodeRekening['n'];
				$o = $getKodeRekening['o'];
	 $this->publicSum = "select sum(jumlah_harga) from tabel_anggaran where (rincian_perhitungan !='' or f !='00' ) and id_tahap='$this->idTahap' and tahun ='$this->tahun' and jenis_anggaran = '$this->jenisAnggaran' and jenis_rka ='3.1' and nama_modul = 'DPA-PPKD' and c1= '$cmbUrusan' and c ='$cmbBidang' and d='$cmbSKPD'  ";
	 $getSumJumlah = mysql_fetch_array(mysql_query($this->publicSum));
	 $jumlahHargaForm =$getSumJumlah['sum(jumlah_harga)'];
	 if(mysql_num_rows(mysql_query("select * from tabel_spd where tahun ='$this->tahun' and anggaran ='$this->jenisAnggaran' and c1 ='$cmbUrusan' and c='$cmbBidang' and d ='$cmbSKPD'  and jenis_rka ='3.1' and jenis_dpa = 'DPA-PPKD'")) != 0 ){
	 	$grabSPD = mysql_fetch_array(mysql_query("select * from tabel_spd where tahun ='$this->tahun' and anggaran ='$this->jenisAnggaran' and c1 ='$cmbUrusan' and c='$cmbBidang' and d ='$cmbSKPD'  and jenis_rka ='3.1' and jenis_dpa = 'DPA-PPKD'"));
...
$dpaPPKD31_v2 = new dpaPPKD31_v2Obj();

$arrayResult = VulnWalkerTahap_v2($dpaPPKD31_v2->modul);
...
$dpaPPKD31_v2->jenisForm = $jenisForm;
$dpaPPKD31_v2->nomorUrut = $nomorUrut;
$dpaPPKD31_v2->urutTerakhir = $nomorUrut;
$dpaPPKD31_v2->tahun = $tahun;
$dpaPPKD31_v2->jenisAnggaran = $jenisAnggaran;
$dpaPPKD31_v2->idTahap = $idTahap;
$dpaPPKD31_v2->username = $_COOKIE['coID'];
$dpaPPKD31_v2->wajibValidasi = $Main->wajibValidasi;
if($Main->wajibValidasi == TRUE){
	$dpaPPKD31_v2->sqlValidasi = " and status_validasi ='1' ";
}else{
	$dpaPPKD31_v2->sqlValidasi = " ";
}

if(empty($dpaPPKD31_v2->tahun)){
	...
	/*$dpaPPKD31_v2->tahun = "select max(id_anggaran) as max from view_dpa_ppkd_3_1 where nama_modul = 'dpaPPKD31_v2'";*/
	$dpaPPKD31_v2->tahun  = $get2['tahun'];
	$dpaPPKD31_v2->jenisAnggaran = $get2['jenis_anggaran'];
	$dpaPPKD31_v2->urutTerakhir = $get2['no_urut'];
	$dpaPPKD31_v2->jenisFormTerakhir = $get2['jenis_form_modul'];
	$dpaPPKD31_v2->urutSebelumnya = $dpaPPKD31_v2->urutTerakhir - 1;
	...
	$dpaPPKD31_v2->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$dpaPPKD31_v2->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];
	
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$dpaPPKD31_v2->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$dpaPPKD31_v2->idTahap'"));
	$dpaPPKD31_v2->currentTahap = $getCurrenttahap['nama_tahap'];
	
	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$dpaPPKD31_v2->idTahap'"));
	$dpaPPKD31_v2->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$dpaPPKD31_v2->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$dpaPPKD31_v2->provinsi = $setting['provinsi'];
$dpaPPKD31_v2->kota = $setting['kota'];
$dpaPPKD31_v2->pengelolaBarang = $setting['pengelolaBarang'];
$dpaPPKD31_v2->pejabatPengelolaBarang = $setting['pejabat'];
$dpaPPKD31_v2->pengurusPengelolaBarang = $setting['pengurus'];
$dpaPPKD31_v2->nipPengelola = $setting['nipPengelola'];
$dpaPPKD31_v2->nipPengurus = $setting['nipPengurus'];
$dpaPPKD31_v2->nipPejabat = $setting['nipPejabat'];
