...
class dpaSKPD21NewObj  extends DaftarObj2{
	var $Prefix = 'dpaSKPD21New';
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='dpaSKPD21New.xls';
	var $namaModulCetak='DPA';
	var $Cetak_Judul = 'DPA';
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'dpaSKPD21NewForm';
	var $modul = "DPA";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	...
	var $nipPejabat = "";
	var $nipPengurus ="";
  var $reportURL = "pages.php?Pg=dpaSKPD21&tipe=Laporan";
    ...
    break;
	 ...
	 function setPage_HeaderOther(){
 	 ...
 	 return
		 ...
		 <A href=\"pages.php?Pg=dpaSKPD21New\" title='TIDAK LANGSUNG' style='color:blue;' > TIDAK LANGSUNG </a> |
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	foreach ($_COOKIE as $key => $value) {
		...
		$urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=dpaSKPD21New.refreshList(true);','-- URUSAN --');

		$codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
		$bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=dpaSKPD21New.refreshList(true);','-- BIDANG --');

		$codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
		$skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=dpaSKPD21New.refreshList(true);','-- SKPD --');
		...
		$unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=dpaSKPD21New.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=dpaSKPD21New.refreshList(true);','-- SUB UNIT --');
...
function Laporan($dt){
   ...
   $this->form_width = 300;
   $this->form_height = 100;
   $this->form_caption = 'LAPORAN DPA-SKPD 2.1 MURNI';
   ...
   $d = $dt['cmbSKPD'];
     ...
     $arrayJenisLaporan = array(
                 array('dpaSKPD21', 'DPA-SKPD 2.1 MURNI'),
    ...
    $this->form_menubawah =
      ...
      "<input type='button' value='Batal' onclick ='".'dpaSKPD21'.".Close()' >";
...
function dpaSKPD21($xls =FALSE){
    ...
    $arrayTandaTangan = explode(';', $getJenisReport['tanda_tangan']);
	  ...
	  $arrKondisi[] = "jenis_anggaran = '$this->jenisAnggaran'";
		...
		$qry ="select * from view_dpa_2_1 where $Kondisi order by urut";
		$aqry = mysql_query($qry);
    ...
    echo 
      ...
      <script type='text/javascript' src='assets/js/bootstrap.min.js'></script>".
			...
			"<body >
				...
				";
		echo "
								<table class='cetak' style='width:100.1%; border-bottom: 1px solid; margin-left: -1px; margin-top: -0.8%;'>
								<thead>
									<tr>
		...
		if (!empty($k) && !empty($l) && !empty($m)  && !empty($n)  && !empty($o)  && empty($id_rincian_belanja) ){
				//Rekening
						...
						// $getDataViewDPA21 = mysql_fetch_array(mysql_query("select * from view_dpa_2_1 where id_tahap = '$this->idTahap' and bk='$bk' and ck='$ck' and dk='$dk' and p='$p' and q='$q' and k ='$k' and l='$l' and m='$m' and n='$n' and o='$o' $kondisiSKPD $kondisiRekening"));
                ...
                $rekeningLevel4 = "";
							...
							$naonkitu = "
											<tr valign='top'>
												...
												<td align='left' class='GarisCetak5' style='font-weight: bold; border: 1px solid #000; border-right: 1px solid; border-left: 1px solid;'></td>
												<td align='right' class='GarisCetak5' style='font-weight: bold; border: 1px solid #000; border-right: 1px solid; border-left: 1px solid;'></td>
												<td align='right' class='GarisCetak5' style='font-weight: bold; border: 1px solid #000; border-right: 1px solid; border-left: 1px solid;'>".$jumlahHarga."</td>
											</tr>
							";
...
$dpaSKPD21New = new dpaSKPD21NewObj();

$arrayResult = VulnWalkerTahap_v2($dpaSKPD21New->modul);
...
$dpaSKPD21New->jenisForm = $jenisForm;
$dpaSKPD21New->nomorUrut = $nomorUrut;
$dpaSKPD21New->urutTerakhir = $nomorUrut;
$dpaSKPD21New->tahun = $tahun;
$dpaSKPD21New->jenisAnggaran = $jenisAnggaran;
$dpaSKPD21New->idTahap = $idTahap;
$dpaSKPD21New->username = $_COOKIE['coID'];

if(empty($dpaSKPD21New->tahun)){
	...
	/*$dpaSKPD21New->tahun = "select max(id_anggaran) as max from view_dpa_2_1 where nama_modul = 'dpaSKPD21New'";*/
	$dpaSKPD21New->tahun  = $get2['tahun'];
	$dpaSKPD21New->jenisAnggaran = $get2['jenis_anggaran'];
	$dpaSKPD21New->urutTerakhir = $get2['no_urut'];
	$dpaSKPD21New->jenisFormTerakhir = $get2['jenis_form_modul'];
	$dpaSKPD21New->urutSebelumnya = $dpaSKPD21New->urutTerakhir - 1;
	...
	$dpaSKPD21New->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$dpaSKPD21New->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];

	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$dpaSKPD21New->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$dpaSKPD21New->idTahap'"));
	$dpaSKPD21New->currentTahap = $getCurrenttahap['nama_tahap'];

	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$dpaSKPD21New->idTahap'"));
	$dpaSKPD21New->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$dpaSKPD21New->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$dpaSKPD21New->provinsi = $setting['provinsi'];
$dpaSKPD21New->kota = $setting['kota'];
$dpaSKPD21New->pengelolaBarang = $setting['pengelolaBarang'];
$dpaSKPD21New->pejabatPengelolaBarang = $setting['pejabat'];
$dpaSKPD21New->pengurusPengelolaBarang = $setting['pengurus'];
$dpaSKPD21New->nipPengelola = $setting['nipPengelola'];
$dpaSKPD21New->nipPengurus = $setting['nipPengurus'];
$dpaSKPD21New->nipPejabat = $setting['nipPejabat'];
