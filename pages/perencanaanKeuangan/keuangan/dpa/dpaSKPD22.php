...
class dpaSKPD22Obj  extends DaftarObj2{	
	var $Prefix = 'dpaSKPD22';
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='dpaSKPD22.xls';
	var $namaModulCetak='DPA';
	var $Cetak_Judul = 'DPA';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'dpaSKPD22Form';
	var $modul = "DPA";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	...
	$getJumlahBarang = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$dpaSKPD22_idplh'"));
	...
	$sisaPaguIndikatif = $paguIndikatif - $getPaguYangTelahTerpakai['paguYangTerpakai'];
	 ...
	 if(mysql_num_rows(mysql_query("select * from view_dpa_2_2_1 where c1='0' and f1 = '0' and k = '$k' and l ='$l' and m='$m' and n='$n' and o='$o'  and id_tahap='$this->idTahap' ")) > 0){
 	 ...
 	 if(empty($cmbJenisDPAForm) ){
		...
		$query = VulnWalkerUpdate('tabel_anggaran',$data,"id_anggaran = '$dpaSKPD22_idplh'");
		mysql_query($query);
	 }
	...
	<A href=\"pages.php?Pg=dpaSKPD22\" title='DPA-SKPKD MURNI'  style='color:blue;'> DPA-SKPD 2.2 </a> |
   ...
   function setPage_OtherScript(){
		...
		return 	
			...
			<script type='text/javascript' src='js/perencanaanKeuangan/keuangan/dpa/dpaSKPD22.js' language='JavaScript' ></script>
	 ...
	 $this->form_height = 400;
	  if ($this->form_fmST==0) {
		...
		foreach ($dt as $key => $value) { 
		 ...
		 $jenis_rka = $jenis_rka;
		 $cmbJenisDPA = cmbArray('cmbJenisDPAForm',$jenis_rka,$arrayJenisDPA,'-- JENIS DPA --','onchange=dpaSKPD22.unlockFindRekening();');
	 	 if(empty($jenis_rka)){
	  ...
	  $this->form_fields = array(
			...
			'kode7' => array( 
						...
						'value'=> "<input type='text' id='kodeRekening' name='kodeRekening' style='width:120px;' readonly value = '$kodeRekening'> &nbsp&nbsp
						 ...
						 <button type='button' id='findRekening' onclick=dpaSKPD22.findRekening('$jenis_rka'); $tergantungJenis> CARI </button> "
						 ),
			'kode8' => array( 
						'label'=>'HARGA SATUAN',
						'labelWidth'=>150, 
						'value'=> "<input type='text' name='hargaSatuan' id='hargaSatuan' value='$hargaSatuan' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='dpaSKPD22.bantu();' > <span id='bantu' style='color:red;'> </span>"
						 ),	
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	if(!isset($selectedC1) ){
		...
		$urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=dpaSKPD22.refreshList(true);','-- URUSAN --');
		
		$codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
		$bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=dpaSKPD22.refreshList(true);','-- BIDANG --');
		
		$codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
		$skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=dpaSKPD22.refreshList(true);','-- SKPD --');
		...
		$unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=dpaSKPD22.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=dpaSKPD22.refreshList(true);','-- SUB UNIT --');
      ...
      <script type='text/javascript' src='assets/js/bootstrap.min.js'></script>".
		...
		echo "
								...
								<thead>
									...
									<tr>
									   <th class='th02' width='100'  rowspan='2' >KODE<br> Program/<br>Kegiatan</th>
									   <th class='th02' width='400'  rowspan='2' >URAIAN</th>
									   <th class='th02' width='100'  rowspan='2' >LOKASI<br> KEGIATAN</th>
									   <th class='th02' width='200'  rowspan='2' >TARGET KINERJA<br>(Kuantitatif)</th>
									   <th class='th02' width='100'  rowspan='2' >Sumber<br>Dana</th>
									   <th class='th02' colspan='4'  rowspan='1' width='500' >Triwulan</th>
									   <th class='th02' width='100'  rowspan='2' >Jumlah</th>
									   <th class='th02' width='100'  rowspan='2' >Tahun N</th>
...
$dpaSKPD22 = new dpaSKPD22Obj();

$arrayResult = VulnWalkerTahap_v2($dpaSKPD22->modul);
$jenisForm = $arrayResult['jenisForm'];
$nomorUrut = $arrayResult['nomorUrut'];
$tahun = $arrayResult['tahun'];
$jenisAnggaran = $arrayResult['jenisAnggaran'];
...
$dpaSKPD22->jenisForm = $jenisForm;
$dpaSKPD22->nomorUrut = $nomorUrut;
$dpaSKPD22->tahun = $tahun;
$dpaSKPD22->jenisAnggaran = $jenisAnggaran;
$dpaSKPD22->idTahap = $idTahap;
$dpaSKPD22->username = $_COOKIE['coID'];


$dpaSKPD22->wajibValidasi = $Main->wajibValidasi;
if($Main->wajibValidasi == TRUE){
	$dpaSKPD22->sqlValidasi = " and status_validasi ='1' ";
}else{
	$dpaSKPD22->sqlValidasi = " ";
}


if(empty($dpaSKPD22->tahun)){
	...
	/*$dpaSKPD22->tahun = "select max(id_anggaran) as max from view_dpa_2_2_1 where nama_modul = 'dpaSKPD22'";*/
	$dpaSKPD22->tahun  = $get2['tahun'];
	$dpaSKPD22->jenisAnggaran = $get2['jenis_anggaran'];
	$dpaSKPD22->urutTerakhir = $get2['no_urut'];
	$dpaSKPD22->jenisFormTerakhir = $get2['jenis_form_modul'];
	$dpaSKPD22->urutSebelumnya = $dpaSKPD22->urutTerakhir - 1;
	...
	$dpaSKPD22->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$dpaSKPD22->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];
	
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$dpaSKPD22->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$dpaSKPD22->idTahap'"));
	$dpaSKPD22->currentTahap = $getCurrenttahap['nama_tahap'];
	
	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$dpaSKPD22->idTahap'"));
	$dpaSKPD22->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$dpaSKPD22->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$dpaSKPD22->provinsi = $setting['provinsi'];
$dpaSKPD22->kota = $setting['kota'];
$dpaSKPD22->pengelolaBarang = $setting['pengelolaBarang'];
$dpaSKPD22->pejabatPengelolaBarang = $setting['pejabat'];
$dpaSKPD22->pengurusPengelolaBarang = $setting['pengurus'];
$dpaSKPD22->nipPengelola = $setting['nipPengelola'];
$dpaSKPD22->nipPengurus = $setting['nipPengurus'];
$dpaSKPD22->nipPejabat = $setting['nipPejabat'];
