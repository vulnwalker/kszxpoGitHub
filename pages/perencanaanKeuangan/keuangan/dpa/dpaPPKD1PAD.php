...
class dpaPPKD1PADObj  extends DaftarObj2{
	var $Prefix = 'dpaPPKD1PAD';
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='dpaPPKD1PAD.xls';
	var $namaModulCetak='DPA';
	var $Cetak_Judul = 'DPA';
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'dpaPPKD1PADForm';
	var $modul = "DPA";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	...
	$getJumlahBarang = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$dpaPPKD1PAD_idplh'"));
	...
	$sisaPaguIndikatif = $paguIndikatif - $getPaguYangTelahTerpakai['paguYangTerpakai'];
	 ...
	 if(mysql_num_rows(mysql_query("select * from view_dpa_ppkd_1 where c1='0' and f = '00' and k = '$k' and l ='$l' and m='$m' and n='$n' and o='$o'  and id_tahap='$this->idTahap' ")) > 0){
 	 ...
 	 if(empty($cmbJenisDPAForm) ){
		...
		$query = VulnWalkerUpdate('tabel_anggaran',$data,"id_anggaran = '$dpaPPKD1PAD_idplh'");
		mysql_query($query);
	 }
	...
	function set_selector_other($tipe){
	 ...
	 function setPage_HeaderOther(){
		 	...
		 	<tr><td class=\"menudottedline\" width=\"40%\" height=\"20\" style='text-align:right'><B>
		 	<A href=\"pages.php?Pg=dpaPPKD1PAD\" title='PENDAPATAN ASLI DAERAH' style='color:blue;' > PENDAPATAN ASLI DAERAH </a> |
		 	<A href=\"pages.php?Pg=dpaPPKD1DP\" title='DANA PERIMBANGAN' > DANA PERIMBANGAN </a> |
		 	<A href=\"pages.php?Pg=dpaPPKD1LP\" title='LAIN LAIN PENDAPATAN' > LAIN LAIN PENDAPATAN </a>
   ...
   function setPage_OtherScript(){
		...
		return
			...
			<script type='text/javascript' src='js/perencanaanKeuangan/keuangan/dpa/dpaPPKD1PAD.js' language='JavaScript' ></script>
	 ...
	 $this->form_height = 400;
	  if ($this->form_fmST==0) {
		...
		foreach ($dt as $key => $value) {
		 ...
		 $jenis_rka = $jenis_rka;
		 $cmbJenisDPA = cmbArray('cmbJenisDPAForm',$jenis_rka,$arrayJenisDPA,'-- JENIS DPA --','onchange=dpaPPKD1PAD.unlockFindRekening();');
	 	 if(empty($jenis_rka)){
	  ...
	  $this->form_fields = array(
			...
			'kode7' => array(
						...
						'value'=> "<input type='text' id='kodeRekening' name='kodeRekening' style='width:120px;' readonly value = '$kodeRekening'> &nbsp&nbsp
						 ...
						 <button type='button' id='findRekening' onclick=dpaPPKD1PAD.findRekening('$jenis_rka'); $tergantungJenis> CARI </button> "
						 ),
			'kode8' => array(
						'label'=>'HARGA SATUAN',
						'labelWidth'=>150,
						'value'=> "<input type='text' name='hargaSatuan' id='hargaSatuan' value='$hargaSatuan' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='dpaPPKD1PAD.bantu();' > <span id='bantu' style='color:red;'> </span>"
						 ),
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	foreach ($_COOKIE as $key => $value) {
		...
		$urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=dpaPPKD1PAD.refreshList(true);','-- URUSAN --');

		$codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
		$bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=dpaPPKD1PAD.refreshList(true);','-- BIDANG --');

		$codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
		$skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=dpaPPKD1PAD.refreshList(true);','-- SKPD --');
		...
		$unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=dpaPPKD1PAD.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=dpaPPKD1PAD.refreshList(true);','-- SUB UNIT --');
...
$dpaPPKD1PAD = new dpaPPKD1PADObj();

$arrayResult = VulnWalkerTahap_v2($dpaPPKD1PAD->modul);
...
$dpaPPKD1PAD->jenisForm = $jenisForm;
$dpaPPKD1PAD->nomorUrut = $nomorUrut;
$dpaPPKD1PAD->urutTerakhir = $nomorUrut;
$dpaPPKD1PAD->tahun = $tahun;
$dpaPPKD1PAD->jenisAnggaran = $jenisAnggaran;
$dpaPPKD1PAD->idTahap = $idTahap;
$dpaPPKD1PAD->username = $_COOKIE['coID'];

if(empty($dpaPPKD1PAD->tahun)){
	...
	/*$dpaPPKD1PAD->tahun = "select max(id_anggaran) as max from view_dpa_ppkd_1 where nama_modul = 'dpaPPKD1PAD'";*/
	$dpaPPKD1PAD->tahun  = $get2['tahun'];
	$dpaPPKD1PAD->jenisAnggaran = $get2['jenis_anggaran'];
	$dpaPPKD1PAD->urutTerakhir = $get2['no_urut'];
	$dpaPPKD1PAD->idTahap = $get2['id_tahap'];
	$dpaPPKD1PAD->jenisFormTerakhir = $get2['jenis_form_modul'];
	$dpaPPKD1PAD->urutSebelumnya = $dpaPPKD1PAD->urutTerakhir - 1;
	...
	$dpaPPKD1PAD->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$dpaPPKD1PAD->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];

	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$dpaPPKD1PAD->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$dpaPPKD1PAD->idTahap'"));
	$dpaPPKD1PAD->currentTahap = $getCurrenttahap['nama_tahap'];

	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$dpaPPKD1PAD->idTahap'"));
	$dpaPPKD1PAD->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$dpaPPKD1PAD->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$dpaPPKD1PAD->provinsi = $setting['provinsi'];
$dpaPPKD1PAD->kota = $setting['kota'];
$dpaPPKD1PAD->pengelolaBarang = $setting['pengelolaBarang'];
$dpaPPKD1PAD->pejabatPengelolaBarang = $setting['pejabat'];
$dpaPPKD1PAD->pengurusPengelolaBarang = $setting['pengurus'];
$dpaPPKD1PAD->nipPengelola = $setting['nipPengelola'];
$dpaPPKD1PAD->nipPengurus = $setting['nipPengurus'];
$dpaPPKD1PAD->nipPejabat = $setting['nipPejabat'];
