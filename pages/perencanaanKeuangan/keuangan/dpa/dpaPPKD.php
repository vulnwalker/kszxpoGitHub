...
class dpaPPKDObj  extends DaftarObj2{	
	var $Prefix = 'dpaPPKD';
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='dpaPPKD.xls';
	var $namaModulCetak='DPA';
	var $Cetak_Judul = 'DPA';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'dpaPPKDForm';
	var $modul = "DPA";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	...
	$getJumlahBarang = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$dpaPPKD_idplh'"));
	...
	$sisaPaguIndikatif = $paguIndikatif - $getPaguYangTelahTerpakai['paguYangTerpakai'];
	 ...
	 if(mysql_num_rows(mysql_query("select * from view_dpa_ppkd where c1='0' and f1 = '0' and k = '$k' and l ='$l' and m='$m' and n='$n' and o='$o'  and id_tahap='$this->idTahap' ")) > 0){
 	 ...
 	 if(empty($cmbJenisDPAForm) ){
		...
		$query = VulnWalkerUpdate('tabel_anggaran',$data,"id_anggaran = '$dpaPPKD_idplh'");
		mysql_query($query);
	 }
	...
	<A href=\"pages.php?Pg=dpaPPKD1\" title='DPA-PPKD MURNI'  > DPA-PPKD 1 </a> |
	...
	<A href=\"pages.php?Pg=dpaPPKD\" title='DPA-PPKD MURNI'  style='color:blue;'  > DPA-PPKD </a> 
   ...
   function setPage_OtherScript(){
		...
		return 	
			...
			<script type='text/javascript' src='js/perencanaanKeuangan/keuangan/dpa/dpaPPKD.js' language='JavaScript' ></script> 
	 ...
	 $this->form_height = 400;
	  if ($this->form_fmST==0) {
		...
		foreach ($dt as $key => $value) { 
		 ...
		 $jenis_rka = $jenis_rka;
		 $cmbJenisDPA = cmbArray('cmbJenisDPAForm',$jenis_rka,$arrayJenisDPA,'-- JENIS DPA --','onchange=dpaPPKD.unlockFindRekening();');
	 	 if(empty($jenis_rka)){
	  ...
	  $this->form_fields = array(
			...
			'kode7' => array( 
						...
						'value'=> "<input type='text' id='kodeRekening' name='kodeRekening' style='width:120px;' readonly value = '$kodeRekening'> &nbsp&nbsp
						 ...
						 <button type='button' id='findRekening' onclick=dpaPPKD.findRekening('$jenis_rka'); $tergantungJenis> CARI </button> "
						 ),
			'kode8' => array( 
						'label'=>'HARGA SATUAN',
						'labelWidth'=>150, 
						'value'=> "<input type='text' name='hargaSatuan' id='hargaSatuan' value='$hargaSatuan' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='dpaPPKD.bantu();' > <span id='bantu' style='color:red;'> </span>"
						 ),	
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	foreach ($_COOKIE as $key => $value) { 
		...
		$urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=dpaPPKD.refreshList(true);','-- URUSAN --');
		
		$codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
		$bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=dpaPPKD.refreshList(true);','-- BIDANG --');
		
		$codeAndNamePPKD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
		$skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNamePPKD,'onchange=dpaPPKD.refreshList(true);','-- SKPD --');
		...
		$unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=dpaPPKD.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=dpaPPKD.refreshList(true);','-- SUB UNIT --');
...
$dpaPPKD = new dpaPPKDObj();

$arrayResult = VulnWalkerTahap_v2($dpaPPKD->modul);
$jenisForm = $arrayResult['jenisForm'];
$nomorUrut = $arrayResult['nomorUrut'];
$tahun = $arrayResult['tahun'];
$jenisAnggaran = $arrayResult['jenisAnggaran'];
...
$dpaPPKD->jenisForm = $jenisForm;
$dpaPPKD->nomorUrut = $nomorUrut;
$dpaPPKD->urutTerakhir = $nomorUrut;
$dpaPPKD->tahun = $tahun;
$dpaPPKD->jenisAnggaran = $jenisAnggaran;
$dpaPPKD->idTahap = $idTahap;

$dpaPPKD->username = $_COOKIE['coID'];


$dpaPPKD->wajibValidasi = $Main->wajibValidasi;
if($Main->wajibValidasi == TRUE){
	$dpaPPKD->sqlValidasi = " and status_validasi ='1' ";
}else{
	$dpaPPKD->sqlValidasi = " ";
}

if(empty($dpaPPKD->tahun)){
	...
	/*$dpaPPKD->tahun = "select max(id_anggaran) as max from view_dpa_ppkd where nama_modul = 'dpaPPKD'";*/
	$dpaPPKD->tahun  = $get2['tahun'];
	$dpaPPKD->jenisAnggaran = $get2['jenis_anggaran'];
	$dpaPPKD->urutTerakhir = $get2['no_urut'];
	$dpaPPKD->jenisFormTerakhir = $get2['jenis_form_modul'];
	$dpaPPKD->urutSebelumnya = $dpaPPKD->urutTerakhir - 1;
	...
	$dpaPPKD->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$dpaPPKD->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];
	
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$dpaPPKD->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$dpaPPKD->idTahap'"));
	$dpaPPKD->currentTahap = $getCurrenttahap['nama_tahap'];
	
	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$dpaPPKD->idTahap'"));
	$dpaPPKD->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$dpaPPKD->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$dpaPPKD->provinsi = $setting['provinsi'];
$dpaPPKD->kota = $setting['kota'];
$dpaPPKD->pengelolaBarang = $setting['pengelolaBarang'];
$dpaPPKD->pejabatPengelolaBarang = $setting['pejabat'];
$dpaPPKD->pengurusPengelolaBarang = $setting['pengurus'];
$dpaPPKD->nipPengelola = $setting['nipPengelola'];
$dpaPPKD->nipPengurus = $setting['nipPengurus'];
$dpaPPKD->nipPejabat = $setting['nipPejabat'];
