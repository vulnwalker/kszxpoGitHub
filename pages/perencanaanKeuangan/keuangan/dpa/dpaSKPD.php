...
class dpaSKPDObj  extends DaftarObj2{
	var $Prefix = 'dpaSKPD';
	var $elCurrPage="HalDefault";
	var $SHOW_CEK = TRUE;
	var $TblName = 'tabel_dpa'; //bonus
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='dpaSKPD.xls';
	var $namaModulCetak='RKA';
	var $Cetak_Judul = 'RKA';
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'dpaSKPDForm';
	var $modul = "DPA-SKPD";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	...
	function genRowSum($ColStyle, $Mode, $Total){
		...
		if(!empty($this->jenisForm)){
			...
			$getIdTahapRKATerakhir = mysql_fetch_array(mysql_query("select max(id_tahap) as max from tabel_dpa where tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and jenis_rka !=''  and (rincian_perhitungan !='' or j !='000' ) and nama_modul = 'DPA-SKPD' "));
		 	$idTahap = $getIdTahapRKATerakhir['max'];
		}

		$getData = mysql_fetch_array(mysql_query("select sum(jumlah_harga) from tabel_dpa where  (rincian_perhitungan !='' or j !='000' ) and id_tahap='$idTahap' and tahun ='$this->tahun' and nama_modul='DPA-SKPD' and jenis_anggaran = '$this->jenisAnggaran' $kondisiSKPD $kondisiRekening"));
	...
	$getJumlahBarang = mysql_fetch_array(mysql_query("select * from tabel_dpa where id = '$dpaSKPD_idplh'"));
	...
	$getPaguYangTelahTerpakai = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as paguYangTerpakai from view_dpa where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and bk='$bk' and ck='$ck' and p='$p' and q='$q' and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'  and no_urut = '$this->nomorUrut' and id!='$dpaSKPD_idplh' "));
	$sisaPaguIndikatif = $paguIndikatif - $getPaguYangTelahTerpakai['paguYangTerpakai'];

	 if(mysql_num_rows(mysql_query("select * from view_dpa where c1='0' and f1 = '0' and k = '$k' and l ='$l' and m='$m' and n='$n' and o='$o'  and id_tahap='$this->idTahap' ")) > 0){
						...
						$queryRekening = VulnWalkerInsert('tabel_dpa',$arrayRekening);
 	 ...
 	 if(empty($cmbJenisRKAForm) ){
		...
		$query = VulnWalkerUpdate('tabel_dpa',$data,"id = '$dpaSKPD_idplh'");
		mysql_query($query);
	 }
	...
	function set_selector_other($tipe){
	 ...
	 <A href=\"pages.php?Pg=dpaSKPD1\" title='DPA-SKPKD MURNI'    > DPA-SKPD 1 </a> |
	 <A href=\"pages.php?Pg=dpaSKPD\" title='DPA-SKPKD MURNI' style='color:blue;'> DPA-SKPD </a> |
	 ...
	 function setPage_OtherScript(){
	  ...
	  return
	    ...
	    <script type='text/javascript' src='js/perencanaanKeuangan/keuangan/dpa/dpaSKPD.js' language='JavaScript' ></script>
	 ...
	 $this->form_height = 400;
	  if ($this->form_fmST==0) {
		...
		foreach ($dt as $key => $value) {
		 ...
		 $jenis_rka = $jenis_rka;
		 $cmbJenisRKA = cmbArray('cmbJenisRKAForm',$jenis_rka,$arrayJenisRKA,'-- JENIS RKA --','onchange=dpaSKPD.unlockFindRekening();');
	 	 if(empty($jenis_rka)){
	  ...
	  $this->form_fields = array(
			...
			'kode7' => array(
						...
						'value'=> "<input type='text' id='kodeRekening' name='kodeRekening' style='width:120px;' readonly value = '$kodeRekening'> &nbsp&nbsp
						 ...
						 <button type='button' id='findRekening' onclick=dpaSKPD.findRekening('$jenis_rka'); $tergantungJenis> CARI </button> "
						 ),
			'kode8' => array(
						'label'=>'HARGA SATUAN',
						'labelWidth'=>150,
						'value'=> "<input type='text' name='hargaSatuan' id='hargaSatuan' value='$hargaSatuan' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='dpaSKPD.bantu();' > <span id='bantu' style='color:red;'> </span>"
						 ),
	...
	if(!empty($this->idTahap)){
				...
				if($this->jenisForm == "PENYUSUNAN" && $this->wajibValidasi == TRUE){
				 ...
				 $getIdTahapTerakhir = mysql_fetch_array(mysql_query("select max(id_tahap) from tabel_dpa where tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and nama_modul = 'DPA-SKPD'"));
   ...
   //
	 // return $Koloms;
					 ...
					 if(mysql_num_rows(mysql_query("select * from ref_rekening where k='$k' and l ='$l' and m='$m' and n='$n' and o='$o'")) == 0){
						 $uraianList = "<span style='color:red;cursor:pointer' class='uraianList' id='spanEditRekening$id' onclick=$this->Prefix.editRekening($id);>Belanja xxx</span>";
						 $kode = "<span style='color:red;'>x.x.x.xx.xxx</span>";
					 }else{
						 $kode = $k.".".$l.".".$m.".".$n.".".$o;
						 // $uraianList = $getNamaRekening['nm_rekening'];
						 $uraianList = "<span style='cursor:pointer' class='uraianList'  id='spanEditRekening$id' onclick=$this->Prefix.editRekening($id);>".$getNamaRekening['nm_rekening']."</span>";
						 $uraianList2 = "<span style='cursor:pointer' class='uraianList'  id='spanEditRekening$id' onclick=$this->Prefix.editRekening($id);>".$getNamaRekening2['nm_rekening']."</span>";
						 $uraianList3 = "<span style='cursor:pointer' class='uraianList'  id='spanEditRekening$id' onclick=$this->Prefix.editRekening($id);>".$getNamaRekening3['nm_rekening']."</span>";
						 $uraianList4 = "<span style='cursor:pointer' class='uraianList'  id='spanEditRekening$id' onclick=$this->Prefix.editRekening($id);>".$getNamaRekening4['nm_rekening']."</span>";
						 $uraianList5 = "<span style='cursor:pointer' class='uraianList'  id='spanEditRekening$id' onclick=$this->Prefix.editRekening($id);>".$getNamaRekening5['nm_rekening']."</span>";
					 }
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	foreach ($_COOKIE as $key => $value) {
		...
		$urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=dpaSKPD.refreshList(true);','-- URUSAN --');

		$codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
		$bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=dpaSKPD.refreshList(true);','-- BIDANG --');

		$codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
		$skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=dpaSKPD.refreshList(true);','-- SKPD --');
		...
		$unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=dpaSKPD.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=dpaSKPD.refreshList(true);','-- SUB UNIT --');
	...
	function Laporan($xls =FALSE){
		...
		$Kondisi= join(' and ',$arrKondisi);
		$qry ="select * from view_dpa where $Kondisi  ";
		...
		";
		$getTotal = mysql_fetch_array(mysql_query("select sum(jumlah_harga) from tabel_dpa where 1=1 $kondisiSKPD $kondisiFilter  and nama_modul = 'DPA-SKPD' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' "));
		...
		while($daqry = mysql_fetch_array($aqry)){
			foreach ($daqry as $key => $value) {
				...
				$getSumJumlahHarga = mysql_fetch_array(mysql_query("select sum(jumlah_harga) from tabel_dpa where k='$k' and l='$l' and m='$m' and n='$n' and o='$o' $kondisiSKPD $kondisiFilter and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and nama_modul = 'DPA-SKPD'"));
...
$dpaSKPD->jenisForm = $jenisForm;
$dpaSKPD->nomorUrut = $nomorUrut;
$dpaSKPD->tahun = $tahun;
$dpaSKPD->jenisAnggaran = $jenisAnggaran;
$dpaSKPD->idTahap = $idTahap;

$dpaSKPD->username = $_COOKIE['coID'];

$dpaSKPD->wajibValidasi = $Main->wajibValidasi;
if($Main->wajibValidasi == TRUE){
	$dpaSKPD->sqlValidasi = " and status_validasi ='1' ";
}else{
	$dpaSKPD->sqlValidasi = " ";
}

if(empty($dpaSKPD->tahun)){

	$get1 = mysql_fetch_array(mysql_query("select max(id)  from view_dpa "));
	$maxAnggaran = $get1['max(id)'];
	$get2 = mysql_fetch_array(mysql_query("select * from view_dpa where id = '$maxAnggaran'"));
	/*$dpaSKPD->tahun = "select max(id) as max from view_dpa where nama_modul = 'dpaSKPD'";*/
	$dpaSKPD->tahun  = $get2['tahun'];
	$dpaSKPD->jenisAnggaran = $get2['jenis_anggaran'];
	$dpaSKPD->idTahap = $get2['id_tahap'];
	$dpaSKPD->urutTerakhir = $get2['no_urut'];
	$dpaSKPD->jenisFormTerakhir = $get2['jenis_form_modul'];
	$dpaSKPD->urutSebelumnya = $dpaSKPD->urutTerakhir - 1;
	...
	$dpaSKPD->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$dpaSKPD->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];

	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$dpaSKPD->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$dpaSKPD->idTahap'"));
	$dpaSKPD->currentTahap = $getCurrenttahap['nama_tahap'];

	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$dpaSKPD->idTahap'"));
	$dpaSKPD->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$dpaSKPD->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$dpaSKPD->provinsi = $setting['provinsi'];
$dpaSKPD->kota = $setting['kota'];
$dpaSKPD->pengelolaBarang = $setting['pengelolaBarang'];
$dpaSKPD->pejabatPengelolaBarang = $setting['pejabat'];
$dpaSKPD->pengurusPengelolaBarang = $setting['pengurus'];
$dpaSKPD->nipPengelola = $setting['nipPengelola'];
$dpaSKPD->nipPengurus = $setting['nipPengurus'];
$dpaSKPD->nipPejabat = $setting['nipPejabat'];


if($dpaSKPD->jenisForm != "PENYUSUNAN"){
$get1 = mysql_fetch_array(mysql_query("select max(id)  from tabel_dpa  "));
	$maxAnggaran = $get1['max(id)'];
	$get2 = mysql_fetch_array(mysql_query("select * from tabel_dpa where id = '$maxAnggaran'"));
	$dpaSKPD->tahun  = $get2['tahun'];
	$dpaSKPD->jenisAnggaran = $get2['jenis_anggaran'];
	$dpaSKPD->urutTerakhir = $get2['no_urut'];
	$dpaSKPD->jenisFormTerakhir = $get2['jenis_form_modul'];
	...
	$dpaSKPD->currentTahap = $arrayHasil['currentTahap'];
			$dpaSKPD->jenisForm = "";
			$dpaSKPD->jenisFormTerakhir = "PENYUSUNAN";
			$dpaSKPD->idTahap = $get2['id_tahap'];
			$dpaSKPD->nomorUrut = $get2['no_urut'];
			$dpaSKPD->urutTerakhir = $get2['no_urut'];
}
