...
class dpaSKPD32NewObj  extends DaftarObj2{
	var $Prefix = 'dpaSKPD32New';
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='dpaSKPD32New.xls';
	var $namaModulCetak='DPA';
	var $Cetak_Judul = 'DPA';
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'dpaSKPD32NewForm';
	var $modul = "DPA";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
   ...
   // if( $err=='' && $kategori =='' ) $err= 'Kategori Belum Di Pilih !!';
    ...
    break;
	 ...
	 function setPage_HeaderOther(){
 	   ...
 	   <A href=\"pages.php?Pg=dpaSKPD32New\" title='PENGELUARAN' style='color:blue;' > PENGELUARAN </a>
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	foreach ($_COOKIE as $key => $value) {
		...
		$urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=dpaSKPD32New.refreshList(true);','-- URUSAN --');

		$codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
		$bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=dpaSKPD32New.refreshList(true);','-- BIDANG --');

		$codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
		$skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=dpaSKPD32New.refreshList(true);','-- SKPD --');
		...
		$unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=dpaSKPD32New.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=dpaSKPD32New.refreshList(true);','-- SUB UNIT --');
...
function Laporan($dt){
   ...
   $dari = $_REQUEST['dari'];
     ...
     $arrayJenisLaporan = array(
                 array('dpaSKPD32', 'DPA-SKPD 3 MURNI'),
    ...
    $this->form_menubawah =
      ...
      "<input type='button' value='Batal' onclick ='dpaSKPD32.Close()' >";
		...
		while($daqry = mysql_fetch_array($aqry)){
			...
			if (!empty($k) && !empty($l) && !empty($m)  && !empty($n)  && !empty($o)  && empty($id_rincian_belanja) ){
				//Rekening
						...
						$getDataViewDPA221 = mysql_fetch_array(mysql_query("select * from view_dpa_3_2 where id_tahap = '$this->idTahap' and bk='$bk' and ck='$ck' and dk='$dk' and p='$p' and q='$q' and k ='$k' and l='$l' and m='$m' and n='$n' and o='$o' $kondisiSKPD $kondisiRekening"));
...
$dpaSKPD32New = new dpaSKPD32NewObj();

$arrayResult = VulnWalkerTahap_v2($dpaSKPD32New->modul);
...
$dpaSKPD32New->jenisForm = $jenisForm;
$dpaSKPD32New->nomorUrut = $nomorUrut;
$dpaSKPD32New->urutTerakhir = $nomorUrut;
$dpaSKPD32New->tahun = $tahun;
$dpaSKPD32New->jenisAnggaran = $jenisAnggaran;
$dpaSKPD32New->idTahap = $idTahap;
$dpaSKPD32New->username = $_COOKIE['coID'];

if(empty($dpaSKPD32New->tahun)){
	...
	/*$dpaSKPD32New->tahun = "select max(id_anggaran) as max from view_dpa_3_2 where nama_modul = 'dpaSKPD32New'";*/
	$dpaSKPD32New->tahun  = $get2['tahun'];
	$dpaSKPD32New->jenisAnggaran = $get2['jenis_anggaran'];
	$dpaSKPD32New->urutTerakhir = $get2['no_urut'];
	$dpaSKPD32New->jenisFormTerakhir = $get2['jenis_form_modul'];
	$dpaSKPD32New->urutSebelumnya = $dpaSKPD32New->urutTerakhir - 1;
	...
	$dpaSKPD32New->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$dpaSKPD32New->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];

	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$dpaSKPD32New->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$dpaSKPD32New->idTahap'"));
	$dpaSKPD32New->currentTahap = $getCurrenttahap['nama_tahap'];

	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$dpaSKPD32New->idTahap'"));
	$dpaSKPD32New->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$dpaSKPD32New->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$dpaSKPD32New->provinsi = $setting['provinsi'];
$dpaSKPD32New->kota = $setting['kota'];
$dpaSKPD32New->pengelolaBarang = $setting['pengelolaBarang'];
$dpaSKPD32New->pejabatPengelolaBarang = $setting['pejabat'];
$dpaSKPD32New->pengurusPengelolaBarang = $setting['pengurus'];
$dpaSKPD32New->nipPengelola = $setting['nipPengelola'];
$dpaSKPD32New->nipPengurus = $setting['nipPengurus'];
$dpaSKPD32New->nipPejabat = $setting['nipPejabat'];
