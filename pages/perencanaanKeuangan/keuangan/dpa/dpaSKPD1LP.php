...
class dpaSKPD1LPObj  extends DaftarObj2{
	var $Prefix = 'dpaSKPD1LP';
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='dpaSKPD1LP.xls';
	var $namaModulCetak='DPA';
	var $Cetak_Judul = 'DPA';
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'dpaSKPD1LPForm';
	var $modul = "DPA";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	...
	$getJumlahBarang = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$dpaSKPD1LP_idplh'"));
	...
	$sisaPaguIndikatif = $paguIndikatif - $getPaguYangTelahTerpakai['paguYangTerpakai'];
	 ...
	 if(mysql_num_rows(mysql_query("select * from view_dpa_1 where c1='0' and f = '00' and k = '$k' and l ='$l' and m='$m' and n='$n' and o='$o'  and id_tahap='$this->idTahap' ")) > 0){
 	 ...
 	 if(empty($cmbJenisDPAForm) ){
		...
		$query = VulnWalkerUpdate('tabel_anggaran',$data,"id_anggaran = '$dpaSKPD1LP_idplh'");
		mysql_query($query);
	 }
	...
	function set_selector_other($tipe){
	 ...
	 function setPage_HeaderOther(){
		 	...
		 	<A href=\"pages.php?Pg=dpaSKPD1LP\" title='PENDAPATAN'  style='color:blue;'> PENDAPATAN </a> |
		 	...
		 	<tr><td class=\"menudottedline\" width=\"40%\" height=\"20\" style='text-align:right'><B>
		 	<A href=\"pages.php?Pg=dpaSKPD1PAD\" title='PENDAPATAN ASLI DAERAH' > PENDAPATAN ASLI DAERAH </a> |
		 	<A href=\"pages.php?Pg=dpaSKPD1DP\" title='DANA PERIMBANGAN'  > DANA PERIMBANGAN </a> |
		 	<A href=\"pages.php?Pg=dpaSKPD1LP\" title='LAIN LAIN PENDAPATAN' style='color:blue;'  > LAIN LAIN PENDAPATAN </a>
   ...
   function setPage_OtherScript(){
		...
		return
			...
			<script type='text/javascript' src='js/perencanaanKeuangan/keuangan/dpa/dpaSKPD1LP.js' language='JavaScript' ></script>
	 ...
	 $this->form_height = 400;
	  if ($this->form_fmST==0) {
		...
		foreach ($dt as $key => $value) {
		 ...
		 $jenis_rka = $jenis_rka;
		 $cmbJenisDPA = cmbArray('cmbJenisDPAForm',$jenis_rka,$arrayJenisDPA,'-- JENIS DPA --','onchange=dpaSKPD1LP.unlockFindRekening();');
	 	 if(empty($jenis_rka)){
	  ...
	  $this->form_fields = array(
			...
			'kode7' => array(
						...
						'value'=> "<input type='text' id='kodeRekening' name='kodeRekening' style='width:120px;' readonly value = '$kodeRekening'> &nbsp&nbsp
						 ...
						 <button type='button' id='findRekening' onclick=dpaSKPD1LP.findRekening('$jenis_rka'); $tergantungJenis> CARI </button> "
						 ),
			'kode8' => array(
						'label'=>'HARGA SATUAN',
						'labelWidth'=>150,
						'value'=> "<input type='text' name='hargaSatuan' id='hargaSatuan' value='$hargaSatuan' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='dpaSKPD1LP.bantu();' > <span id='bantu' style='color:red;'> </span>"
						 ),
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	foreach ($_COOKIE as $key => $value) {
		...
		$urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=dpaSKPD1LP.refreshList(true);','-- URUSAN --');

		$codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
		$bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=dpaSKPD1LP.refreshList(true);','-- BIDANG --');

		$codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
		$skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=dpaSKPD1LP.refreshList(true);','-- SKPD --');
		...
		$unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=dpaSKPD1LP.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=dpaSKPD1LP.refreshList(true);','-- SUB UNIT --');
	...
	function getDaftarOpsi($Mode=1){
	  ...
	  if(isset($cmbUrusan) && $cmbUrusan== ''){
	    ...
	    while($subRincian = mysql_fetch_array($getAllSubRincian)){
	          if(mysql_num_rows(mysql_query("select * from view_dpa_1 where id_tahap = '$this->idTahap' $this->kondisiRekening and id_anggaran='".$subRincian['id_anggaran']."' and (j!='000' or rincian_perhitungan !='' ) $kondisiSKPD $kondisiRekening")) == 0){
	              $blackListSubRincian[] = "id_anggaran != '".$subRincian['id_anggaran']."'";
	              $arrKondisi[] = "id_anggaran !='".$subRincian['id_anggaran']."'";
	              // $this->injectQuery = "select * from view_dpa_1 where id_tahap = '$this->idTahap' and id_anggaran='".$subRincian['id_anggaran']."' and (j!='000' or rincian_perhitungan !='' ) $kondisiSKPD $kondisiRekening";
	          }
	    }
...
$dpaSKPD1LP = new dpaSKPD1LPObj();

$arrayResult = VulnWalkerTahap_v2($dpaSKPD1LP->modul);
...
$dpaSKPD1LP->jenisForm = $jenisForm;
$dpaSKPD1LP->nomorUrut = $nomorUrut;
$dpaSKPD1LP->urutTerakhir = $nomorUrut;
$dpaSKPD1LP->tahun = $tahun;
$dpaSKPD1LP->jenisAnggaran = $jenisAnggaran;
$dpaSKPD1LP->idTahap = $idTahap;
$dpaSKPD1LP->username = $_COOKIE['coID'];

if(empty($dpaSKPD1LP->tahun)){
	...
	/*$dpaSKPD1LP->tahun = "select max(id_anggaran) as max from view_dpa_1 where nama_modul = 'dpaSKPD1LP'";*/
	$dpaSKPD1LP->tahun  = $get2['tahun'];
	$dpaSKPD1LP->jenisAnggaran = $get2['jenis_anggaran'];
	$dpaSKPD1LP->urutTerakhir = $get2['no_urut'];
	$dpaSKPD1LP->idTahap = $get2['id_tahap'];
	$dpaSKPD1LP->jenisFormTerakhir = $get2['jenis_form_modul'];
	$dpaSKPD1LP->urutSebelumnya = $dpaSKPD1LP->urutTerakhir - 1;
	...
	$dpaSKPD1LP->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$dpaSKPD1LP->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];

	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$dpaSKPD1LP->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$dpaSKPD1LP->idTahap'"));
	$dpaSKPD1LP->currentTahap = $getCurrenttahap['nama_tahap'];

	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$dpaSKPD1LP->idTahap'"));
	$dpaSKPD1LP->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$dpaSKPD1LP->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$dpaSKPD1LP->provinsi = $setting['provinsi'];
$dpaSKPD1LP->kota = $setting['kota'];
$dpaSKPD1LP->pengelolaBarang = $setting['pengelolaBarang'];
$dpaSKPD1LP->pejabatPengelolaBarang = $setting['pejabat'];
$dpaSKPD1LP->pengurusPengelolaBarang = $setting['pengurus'];
$dpaSKPD1LP->nipPengelola = $setting['nipPengelola'];
$dpaSKPD1LP->nipPengurus = $setting['nipPengurus'];
$dpaSKPD1LP->nipPejabat = $setting['nipPejabat'];
