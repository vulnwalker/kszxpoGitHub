...
class dpaPPKD21Obj  extends DaftarObj2{
	var $Prefix = 'dpaPPKD21';
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='dpaPPKD21.xls';
	var $namaModulCetak='DPA';
	var $Cetak_Judul = 'DPA';
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'dpaPPKD21Form';
	var $modul = "DPA";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	...
	function setTitle(){
		return 'DPA-PPKD 2.1 '.$this->jenisAnggaran.' TAHUN '.$this->tahun;
	}
	...
	$getJumlahBarang = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$dpaPPKD21_idplh'"));
	...
	$getPaguYangTelahTerpakai = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as paguYangTerpakai from view_dpa_ppkd_2_1 where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and bk='$bk' and ck='$ck' and p='$p' and q='$q' and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'  and no_urut = '$this->nomorUrut' and id_anggaran!='$dpaPPKD21_idplh' "));
	$sisaPaguIndikatif = $paguIndikatif - $getPaguYangTelahTerpakai['paguYangTerpakai'];

	 if(mysql_num_rows(mysql_query("select * from view_dpa_ppkd_2_1 where c1='0' and f = '00' and k = '$k' and l ='$l' and m='$m' and n='$n' and o='$o'  and id_tahap='$this->idTahap' ")) > 0){
 	 ...
 	 if(empty($cmbJenisDPAForm) ){
		...
		$query = VulnWalkerUpdate('tabel_anggaran',$data,"id_anggaran = '$dpaPPKD21_idplh'");
		mysql_query($query);
	 }
	...
	function set_selector_other($tipe){
	   ...
	   case 'formAlokasi':{
		...
		case 'checkAlokasi':{
			...
			if($this->jenisForm != "READ"){
				...
				$o = $getKodeRekening['o'];
				$jenisAlokasi = "";
				if(mysql_num_rows(mysql_query("select * from tabel_spd where tahun ='$this->tahun' and anggaran ='$this->jenisAnggaran' and c1 ='$c1' and c='$c' and d ='$d' and k='$k' and l='$l' and m='$m' and n ='$n' and o ='$o'  and jenis_rka = '2.1' and jenis_dpa = 'DPA-PPKD'")) != 0){
					$getAlokasi = mysql_fetch_array(mysql_query("select * from tabel_spd where tahun ='$this->tahun' and anggaran ='$this->jenisAnggaran' and c1 ='$c1' and c='$c' and d ='$d' and k='$k' and l='$l' and m='$m' and n ='$n' and o ='$o' and jenis_rka = '2.1' and jenis_dpa = 'DPA-PPKD' "));
			...
			if($this->jenisForm != "READ"){
				...
				if($err == ""){
					...
					if(mysql_num_rows(mysql_query("select * from tabel_spd where tahun ='$this->tahun' and anggaran ='$this->jenisAnggaran' and c1 ='$c1' and c='$c' and d ='$d' and k='$k' and l='$l' and m='$m' and n ='$n' and o ='$o'  and jenis_rka = '2.1' and jenis_dpa = 'DPA-PPKD'")) == 0){
						...
						$grepId = mysql_fetch_array(mysql_query("select * from tabel_spd where tahun ='$this->tahun' and anggaran ='$this->jenisAnggaran' and c1 ='$c1' and c='$c' and d ='$d' and k='$k' and l='$l' and m='$m' and n ='$n' and o ='$o'  and jenis_rka = '2.1' and jenis_dpa = 'DPA-PPKD'"));
						$id = $grepId['id'];
						$data = array(
										'jenis_alokasi_kas' => $jenisAlokasi,
										"alokasi_jan" => $jan,
	 ...
	 function setPage_HeaderOther(){
 	 ...
 	 <A href=\"pages.php?Pg=dpaPPKD21\" title='DPA-PPKD MURNI'  style='color:blue;' > BELANJA TIDAK LANGSUNG </a> |
 	 <A href=\"pages.php?Pg=dpaPPKD1PAD\" title='DPA-PPKD MURNI'  > PENDAPATAN </a> |

 	 <A href=\"pages.php?Pg=dpaPPKD31\" title='DPA-PPKD MURNI'  > PEMBIAYAAN </a> |
 	 <A href=\"pages.php?Pg=dpaPPKD\" title='DPA-PPKD MURNI'  > REKAP </a>
   ...
   function setPage_OtherScript(){
		...
		return
			...
			<script type='text/javascript' src='js/perencanaanKeuangan/keuangan/dpa/dpaPPKD21.js' language='JavaScript' ></script>
	 ...
	 $this->form_height = 400;
	  if ($this->form_fmST==0) {
		...
		foreach ($dt as $key => $value) {
		 ...
		 $jenis_rka = $jenis_rka;
		 $cmbJenisDPA = cmbArray('cmbJenisDPAForm',$jenis_rka,$arrayJenisDPA,'-- JENIS DPA --','onchange=dpaPPKD21.unlockFindRekening();');
	 	 if(empty($jenis_rka)){
	  ...
	  $this->form_fields = array(
			...
			'kode7' => array(
						...
						'value'=> "<input type='text' id='kodeRekening' name='kodeRekening' style='width:120px;' readonly value = '$kodeRekening'> &nbsp&nbsp
						 ...
						 <button type='button' id='findRekening' onclick=dpaPPKD21.findRekening('$jenis_rka'); $tergantungJenis> CARI </button> "
						 ),
			'kode8' => array(
						'label'=>'HARGA SATUAN',
						'labelWidth'=>150,
						'value'=> "<input type='text' name='hargaSatuan' id='hargaSatuan' value='$hargaSatuan' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='dpaPPKD21.bantu();' > <span id='bantu' style='color:red;'> </span>"
						 ),
	...
	function setKolomData($no, $isi, $Mode, $TampilCheckBox){
	 ...
	 foreach ($_REQUEST as $key => $value) {
	  ...
	  if( !empty($k) && !empty($l) && !empty($m)  && !empty($n)  && !empty($o)  && empty($id_rincian_belanja) ){
	       ...
	       $getSumJumlahHarga = mysql_fetch_array(mysql_query("select sum(jumlah_harga) from view_dpa_ppkd_2_1 where id_tahap = '$this->idTahap' and bk='$bk' and ck='$ck' and dk='$dk' and p='$p' and q='$q' and k ='$k' and l='$l' and m='$m' and n='$n' and o='$o' and id_rincian_belanja='$id_rincian_belanja' $kondisiSKPD $kondisiRekening"));
	       $jumlahHarga = number_format($getSumJumlahHarga['sum(jumlah_harga)'] ,2,',','.');
	       $TampilCheckBox = "";
	  }elseif( !empty($k) && !empty($l) && !empty($m)  && !empty($n)  && !empty($o)  && !empty($id_rincian_belanja)  && !empty($rincian_perhitungan) ){
	       //SubRincianBelanja
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	foreach ($_COOKIE as $key => $value) {
		...
		$urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=dpaPPKD21.refreshList(true);','-- URUSAN --');

		$codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
		$bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=dpaPPKD21.refreshList(true);','-- BIDANG --');

		$codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
		$skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=dpaPPKD21.refreshList(true);','-- SKPD --');
		...
		$unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=dpaPPKD21.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=dpaPPKD21.refreshList(true);','-- SUB UNIT --');
	...
	function getDaftarOpsi($Mode=1){
	  ...
	  if(isset($cmbUrusan) && $cmbUrusan== ''){
	    ...
	    $getAllSubRincian = mysql_query("select * from view_dpa_ppkd_2_1 where id_tahap = '$this->idTahap' and (j!='000' or rincian_perhitungan !='' )");
	    while($subRincian = mysql_fetch_array($getAllSubRincian)){
	          if(mysql_num_rows(mysql_query("select * from view_dpa_ppkd_2_1 where id_tahap = '$this->idTahap' and id_anggaran='".$subRincian['id_anggaran']."' and (j!='000' or rincian_perhitungan !='' ) $kondisiSKPD $kondisiRekening")) == 0){
	              $blackListSubRincian[] = "id_anggaran != '".$subRincian['id_anggaran']."'";
	              $arrKondisi[] = "id_anggaran !='".$subRincian['id_anggaran']."'";
	              // $this->injectQuery = "select * from view_dpa_ppkd_2_1 where id_tahap = '$this->idTahap' and id_anggaran='".$subRincian['id_anggaran']."' and (j!='000' or rincian_perhitungan !='' ) $kondisiSKPD $kondisiRekening";
	          }
	    }
	    ...
	    $getAllRincian =  mysql_query("select * from view_dpa_ppkd_2_1 where id_tahap ='$this->idTahap'  and k !='0' and k!=''  and l !='0' and l!=''  and m !='0' and m!=''  and n !='0' and n !='' and id_rincian_belanja != '' and id_rincian_belanja !='0' and j='000' and rincian_perhitungan = '' ");
	    while($rincianBelanja = mysql_fetch_array($getAllRincian)){
	        if(mysql_num_rows(mysql_query("select  * from view_dpa_ppkd_2_1 where id_tahap = '$this->idTahap' and bk = '".$rincianBelanja['bk']."' and ck = '".$rincianBelanja['ck']."' and dk = '".$rincianBelanja['dk']."' and p = '".$rincianBelanja['p']."' and q = '".$rincianBelanja['q']."' and k = '".$rincianBelanja['k']."' and l = '".$rincianBelanja['l']."'  and m = '".$rincianBelanja['m']."'  and n = '".$rincianBelanja['n']."' and o = '".$rincianBelanja['o']."' and id_rincian_belanja = '".$rincianBelanja['id_rincian_belanja']."' and (j!='000' or rincian_perhitungan !='') $kondisiBlackListSubRincian ")) == 0){
...
function Laporan($xls =FALSE){
		...
		$getIDTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where no_urut ='$this->urutTerakhir' and tahun ='$this->tahun' and anggaran='$this->jenisAnggaran'"));
				...
				$getAllParent = mysql_query("select * from view_dpa_ppkd_2_1 where id_tahap ='$idTahap'  and o1 !='0' and k='0' and n='0' and rincian_perhitungan=''  ");
				while($rows = mysql_fetch_array($getAllParent)){
					foreach ($rows as $key => $value) {
				 	 $$key = $value;
					}
					$cekPekerjaan = mysql_num_rows(mysql_query("select * from view_dpa_ppkd_2_1 where o1 = '$o1' and id_tahap ='$idTahap' $kondisiRekening $kondisiSKPD and (rincian_perhitungan !='' or rincian_perhitungan!='') "));
					if($cekPekerjaan == 0){
						$arrKondisi[] = "o1 !='$o1'";
					}else{
						...
						while($row2s = mysql_fetch_array($getAllRekening)){
							...
							$cekRekening = mysql_num_rows(mysql_query("select * from view_dpa_ppkd_2_1 where k ='$k' and l='$l' and m='$m' and n='$n' and o='$o' and id_tahap ='$idTahap' $kondisiRekening $kondisiSKPD and (rincian_perhitungan !='' or rincian_perhitungan!='') "));
							if($cekRekening == 0){
								$concat = $k.".".$l.".".$m.".".$n.".".$o;
								...
								while($row3s = mysql_fetch_array($getAllProgram)){
									...
									$cekProgram = mysql_num_rows(mysql_query("select * from view_dpa_ppkd_2_1 where bk ='$bk' and ck='$ck' and p='$p' and id_tahap ='$idTahap' $kondisiRekening $kondisiSKPD and (rincian_perhitungan !='' or rincian_perhitungan!='') "));
									if($cekProgram == 0){
										$concat = $bk.".".$ck.".".$p;
										...
										while($row4s = mysql_fetch_array($getAllKegiatan)){
											...
											$cekKegiatan = mysql_num_rows(mysql_query("select * from view_dpa_ppkd_2_1 where bk ='$bk' and ck='$ck' and p='$p' and q='$q' and id_tahap ='$idTahap' $kondisiRekening $kondisiSKPD and (rincian_perhitungan !='' or rincian_perhitungan!='') "));
											if($cekKegiatan == 0){
												$concat = $bk.".".$ck.".".$p;
												$arrKondisi[] = "concat(bk,'.',ck,'.',p)  !='$concat'";
											}else{
				...
				$grabNonMapingRekening= mysql_query("select * from view_dpa_ppkd_2_1 where id_tahap ='$idTahap' and (LENGTH(k) > 1) and rincian_perhitungan=''");
				while($got = mysql_fetch_array($grabNonMapingRekening)){
					if(mysql_num_rows(mysql_query("select * from view_dpa_ppkd_2_1 where id_tahap ='$idTahap' and k ='".$got['k']."' and l ='".$got['l']."' and m ='".$got['m']."' and n ='".$got['n']."' and o ='".$got['o']."' and rincian_perhitungan!='' $kondisiRekening $kondisiSKPD ")) != 0){
						$arrKondisi[] = "id_tahap = '$idTahap' or id_anggaran ='".$got['id_anggaran']."'";
					}

				}

				$grabNonHostedRekening= mysql_query("select * from view_dpa_ppkd_2_1 where id_tahap ='$idTahap' and (LENGTH(k) = 1 and k !='0') and rincian_perhitungan=''");
		...
		$getSumAlokasi = mysql_fetch_array(mysql_query("select  sum(alokasi_jan), sum(alokasi_feb) , sum(alokasi_mar) , sum(alokasi_apr) , sum(alokasi_mei) , sum(alokasi_jun) , sum(alokasi_jul) , sum(alokasi_agu) , sum(alokasi_sep) , sum(alokasi_okt) , sum(alokasi_nop), sum(alokasi_des) from tabel_spd where anggaran='$this->jenisAnggaran' and tahun ='$this->tahun' $kondisiSKPD and jenis_rka='2.1' and jenis_dpa = 'DPA-PPKD'"));
...
function formAlokasi($dt){
	...
	$arrayJenisPerhitungan = array(
	 ...
	 if(mysql_num_rows(mysql_query("select * from tabel_spd where tahun ='$this->tahun' and anggaran ='$this->jenisAnggaran' and c1 ='$cmbUrusan' and c='$cmbBidang' and d ='$cmbSKPD' and k='$k' and l='$l' and m='$m' and n ='$n' and o ='$o'   and jenis_rka ='2.1' and jenis_dpa = 'DPA-PPKD'")) != 0 ){
	 	$grabSPD = mysql_fetch_array(mysql_query("select * from tabel_spd where tahun ='$this->tahun' and anggaran ='$this->jenisAnggaran' and c1 ='$cmbUrusan' and c='$cmbBidang' and d ='$cmbSKPD' and k='$k' and l='$l' and m='$m' and n ='$n' and o ='$o'   and jenis_rka ='2.1' and jenis_dpa = 'DPA-PPKD'"));
...
$dpaPPKD21 = new dpaPPKD21Obj();

$arrayResult = VulnWalkerTahap_v2($dpaPPKD21->modul);
...
$dpaPPKD21->jenisForm = $jenisForm;
$dpaPPKD21->nomorUrut = $nomorUrut;
$dpaPPKD21->urutTerakhir = $nomorUrut;
$dpaPPKD21->tahun = $tahun;
$dpaPPKD21->jenisAnggaran = $jenisAnggaran;
$dpaPPKD21->idTahap = $idTahap;
$dpaPPKD21->username = $_COOKIE['coID'];

if(empty($dpaPPKD21->tahun)){
	...
	/*$dpaPPKD21->tahun = "select max(id_anggaran) as max from view_dpa_ppkd_2_1 where nama_modul = 'dpaPPKD21'";*/
	$dpaPPKD21->tahun  = $get2['tahun'];
	$dpaPPKD21->jenisAnggaran = $get2['jenis_anggaran'];
	$dpaPPKD21->urutTerakhir = $get2['no_urut'];
	$dpaPPKD21->jenisFormTerakhir = $get2['jenis_form_modul'];
	$dpaPPKD21->urutSebelumnya = $dpaPPKD21->urutTerakhir - 1;
	...
	$dpaPPKD21->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$dpaPPKD21->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];

	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$dpaPPKD21->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$dpaPPKD21->idTahap'"));
	$dpaPPKD21->currentTahap = $getCurrenttahap['nama_tahap'];

	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$dpaPPKD21->idTahap'"));
	$dpaPPKD21->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$dpaPPKD21->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$dpaPPKD21->provinsi = $setting['provinsi'];
$dpaPPKD21->kota = $setting['kota'];
$dpaPPKD21->pengelolaBarang = $setting['pengelolaBarang'];
$dpaPPKD21->pejabatPengelolaBarang = $setting['pejabat'];
$dpaPPKD21->pengurusPengelolaBarang = $setting['pengurus'];
$dpaPPKD21->nipPengelola = $setting['nipPengelola'];
$dpaPPKD21->nipPengurus = $setting['nipPengurus'];
$dpaPPKD21->nipPejabat = $setting['nipPejabat'];
