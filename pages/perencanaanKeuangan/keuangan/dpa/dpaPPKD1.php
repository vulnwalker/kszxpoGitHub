...
class dpaPPKD1Obj  extends DaftarObj2{	
	var $Prefix = 'dpaPPKD1';
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='dpaPPKD1.xls';
	var $namaModulCetak='DPA';
	var $Cetak_Judul = 'DPA';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'dpaPPKD1Form';
	var $modul = "DPA";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	...
	function genRowSum($ColStyle, $Mode, $Total){
		...
		if(!empty($this->jenisForm)){
			...
			$getIdTahapDPATerakhir = mysql_fetch_array(mysql_query("select max(id_tahap) as max from tabel_anggaran where tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and jenis_rka !='' and o1 !='0' and (rincian_perhitungan !='' or f !='00' ) and nama_modul = 'DPA-PPKD' "));
		 	$idTahap = $getIdTahapDPATerakhir['max'];
			$number = 5;
		}

		
		$getData = mysql_fetch_array(mysql_query("select sum(jumlah_harga) from tabel_anggaran where (rincian_perhitungan !='' or f !='00' ) and id_tahap='$idTahap' and tahun ='$this->tahun' and jenis_anggaran = '$this->jenisAnggaran' and jenis_rka = '1' and nama_modul='DPA-PPKD' $kondisiSKPD $kondisiRekening"));
	...
	$getJumlahBarang = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$dpaPPKD1_idplh'"));
	...
	$sisaPaguIndikatif = $paguIndikatif - $getPaguYangTelahTerpakai['paguYangTerpakai'];
	 ...
	 if(mysql_num_rows(mysql_query("select * from view_dpa_ppkd_1 where c1='0' and f = '00' and k = '$k' and l ='$l' and m='$m' and n='$n' and o='$o'  and id_tahap='$this->idTahap' ")) > 0){
 	 ...
 	 if(empty($cmbJenisDPAForm) ){
		...
		$query = VulnWalkerUpdate('tabel_anggaran',$data,"id_anggaran = '$dpaPPKD1_idplh'");
		mysql_query($query);
	 }
	  ...
	  switch($tipe){	
		...
		case 'Report':{	
			foreach ($_REQUEST as $key => $value) { 
				...
				if(mysql_num_rows(mysql_query("select * from skpd_report_dpa_ppkd_1 where username= '$this->username'")) == 0){
					$data = array(
		...
		case 'sync':{	
			...
			$getJenisFormRka = mysql_fetch_array(mysql_query("selecrt * from view_rka_ppkd_1 where id_tahap = '$idTahapTerakhir'"));
			if($getJenisFormRka['jenis_form_modul'] == "PENYUSUNAN" && $this->wajibValidasi == TRUE ){
				$kondisiFilter = $kondisiFilter." and status_validasi ='1' ";
			}
			
			$getAllChildern = mysql_query("select * from view_rka_ppkd_1 where id_tahap = '$idTahapTerakhir' and rincian_perhitungan !='' $kondisiFilter ");
			...
			while($rows = mysql_fetch_array($getAllChildern)){
				...
				if(mysql_num_rows(mysql_query("select * from view_dpa_ppkd_1 where k='$k' and l='$l' and m='$m' and n='$n' and o='$o' and c1 = '0' and id_tahap='$this->idTahap' ")) == 0){
					$arrayRekening = array(
												 'c1' => '0',
												 'c' => '00',
												 'd' => '00',
					...
					if(mysql_num_rows(mysql_query("select * from view_dpa_ppkd_1 where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and k='$k' and l='$l' and m='$m' and n='$n' and o='$o' and o1='$o1' and rincian_perhitungan = '$rincian_perhitungan' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and id_tahap='$this->idTahap'")) == 0){
						$data = array(	'c1' => $c1,
										'c' => $c,
										'd' => $d,
										'e' => $e,
 ...
 case 'formAlokasi':{
	...
	<A href=\"pages.php?Pg=dpaPPKD1\" title='DPA-PPKD MURNI' style='color:blue;' > DPA-PPKD 1 </a> |
	<A href=\"pages.php?Pg=dpaPPKD21\" title='DPA-PPKD MURNI'  > DPA-PPKD 2.1 </a> |
	<A href=\"pages.php?Pg=dpaPPKD31\" title='DPA-PPKD MURNI'  > DPA-PPKD 3.1 </a> |
	<A href=\"pages.php?Pg=dpaPPKD32\" title='DPA-PPKD MURNI'  > DPA-PPKD 3.2 </a> |
	<A href=\"pages.php?Pg=dpaPPKD\" title='DPA-PPKD MURNI'  > DPA-PPKD </a> 
   ...
   function setPage_OtherScript(){
		...
		return 	
			...
			<script type='text/javascript' src='js/perencanaanKeuangan/keuangan/dpa/dpaPPKD1.js' language='JavaScript' ></script> 
	 ...
	 $this->form_height = 400;
	  if ($this->form_fmST==0) {
		...
		foreach ($dt as $key => $value) { 
		 ...
		 $jenis_rka = $jenis_rka;
		 $cmbJenisDPA = cmbArray('cmbJenisDPAForm',$jenis_rka,$arrayJenisDPA,'-- JENIS DPA --','onchange=dpaPPKD1.unlockFindRekening();');
	 	 if(empty($jenis_rka)){
	  ...
	  $this->form_fields = array(
			...
			'kode7' => array( 
						...
						'value'=> "<input type='text' id='kodeRekening' name='kodeRekening' style='width:120px;' readonly value = '$kodeRekening'> &nbsp&nbsp
						 ...
						 <button type='button' id='findRekening' onclick=dpaPPKD1.findRekening('$jenis_rka'); $tergantungJenis> CARI </button> "
						 ),
			'kode8' => array( 
						'label'=>'HARGA SATUAN',
						'labelWidth'=>150, 
						'value'=> "<input type='text' name='hargaSatuan' id='hargaSatuan' value='$hargaSatuan' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='dpaPPKD1.bantu();' > <span id='bantu' style='color:red;'> </span>"
						 ),	
		...
		if(!empty($hiddenP)){
			...
			$kondisiSKPD = "and c1='$cmbUrusan'";
			 ...
			 if($c1 == '0'){
				...
				$tanda = "";
			 	$getAllMemberOfThisRekening = mysql_query("select * from view_dpa_ppkd_1 where id_tahap = '$idTahap' and k='$k' and l ='$l' and m = '$m' and n ='$n' and o ='$o' and (rincian_perhitungan != '' or j!='000')");
				...
				for($nambur = 0 ; $nambur < sizeof($arrayMemberRek) ; $nambur++){
					if(mysql_num_rows(mysql_query("select * from tabel_spd where tahun = '$this->tahun' and anggaran = '$this->jenisAnggaran' and jenis_rka = '1' and jenis_dpa = 'DPA-PPKD' and concat(c1,'.',c,'.',d) = '$arrayMemberRek[$nambur]' and k='$k' and l='$l' and m='$m' and n ='$n' and o ='$o'")) == 0){
						$tanda = "color:red;";
					}
				}	 
			 ...
			 //batas
			 	if(strlen($k) > 1){
					...
					if($k == '0' && $n == '0'){
						...
						$jarak = '15px';
				 		$cekAdaParent = mysql_num_rows(mysql_query("select * from view_dpa_ppkd_1 where id_tahap = '$idTahap' and o1 = '$o1' and o1 !='0' and o1 !='' "));
				...
				$ilustrasi = "";
				$getSumJumlahBarang = mysql_fetch_array(mysql_query("select sum(volume_rek) as total from view_dpa_ppkd_1 where k='$k' and l='$l' and m='$m' and n='$n' and o='$o' $kondisiSKPD and id_tahap='$idTahap'"));
				$Koloms[] = array('align="left"',"" );
			 ...
			 $getSumSatuanRek = mysql_fetch_array(mysql_query("select sum(jumlah) as total from view_dpa_ppkd_1 where k='$k' and l='$l' and m='$m' and n='$n'  and o='$o' $kondisiSKPD and id_tahap='$idTahap'"));
			 ...
			 if($c1 == '0'){
			 if($k == '0' && $n =='0'){
			 	$getTotalJumalhHarga = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as total from view_dpa_ppkd_1 where o1='$o1' $kondisiSKPD $kondisiBelanja and id_tahap='$idTahap'"));
			 }else{
			 	$getTotalJumalhHarga = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as total from view_dpa_ppkd_1 where k='$k' and l='$l' and m='$m' and n='$n' and o='$o' $kondisiSKPD and id_tahap='$idTahap'"));	
			 }
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	foreach ($_COOKIE as $key => $value) { 
		...
		$urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=dpaPPKD1.refreshList(true);','-- URUSAN --');
		
		$codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
		$bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=dpaPPKD1.refreshList(true);','-- BIDANG --');
		
		$codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
		$skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=dpaPPKD1.refreshList(true);','-- SKPD --');
		...
		$unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=dpaPPKD1.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=dpaPPKD1.refreshList(true);','-- SUB UNIT --');
...
$dpaPPKD1 = new dpaPPKD1Obj();

$arrayResult = VulnWalkerTahap_v2($dpaPPKD1->modul);
$jenisForm = $arrayResult['jenisForm'];
$nomorUrut = $arrayResult['nomorUrut'];
$tahun = $arrayResult['tahun'];
$jenisAnggaran = $arrayResult['jenisAnggaran'];
...
$dpaPPKD1->jenisForm = $jenisForm;
$dpaPPKD1->nomorUrut = $nomorUrut;
$dpaPPKD1->urutTerakhir = $nomorUrut;
$dpaPPKD1->tahun = $tahun;
$dpaPPKD1->jenisAnggaran = $jenisAnggaran;
$dpaPPKD1->idTahap = $idTahap;

$dpaPPKD1->username = $_COOKIE['coID'];

$dpaPPKD1->wajibValidasi = $Main->wajibValidasi;
if($Main->wajibValidasi == TRUE){
	$dpaPPKD1->sqlValidasi = " and status_validasi ='1' ";
}else{
	$dpaPPKD1->sqlValidasi = " ";
}

if(empty($dpaPPKD1->tahun)){
	...
	/*$dpaPPKD1->tahun = "select max(id_anggaran) as max from view_dpa_ppkd_1 where nama_modul = 'dpaPPKD1'";*/
	$dpaPPKD1->tahun  = $get2['tahun'];
	$dpaPPKD1->jenisAnggaran = $get2['jenis_anggaran'];
	$dpaPPKD1->urutTerakhir = $get2['no_urut'];
	$dpaPPKD1->jenisFormTerakhir = $get2['jenis_form_modul'];
	$dpaPPKD1->urutSebelumnya = $dpaPPKD1->urutTerakhir - 1;
	...
	$dpaPPKD1->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$dpaPPKD1->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];
	
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$dpaPPKD1->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$dpaPPKD1->idTahap'"));
	$dpaPPKD1->currentTahap = $getCurrenttahap['nama_tahap'];
	
	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$dpaPPKD1->idTahap'"));
	$dpaPPKD1->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$dpaPPKD1->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$dpaPPKD1->provinsi = $setting['provinsi'];
$dpaPPKD1->kota = $setting['kota'];
$dpaPPKD1->pengelolaBarang = $setting['pengelolaBarang'];
$dpaPPKD1->pejabatPengelolaBarang = $setting['pejabat'];
$dpaPPKD1->pengurusPengelolaBarang = $setting['pengurus'];
$dpaPPKD1->nipPengelola = $setting['nipPengelola'];
$dpaPPKD1->nipPengurus = $setting['nipPengurus'];
$dpaPPKD1->nipPejabat = $setting['nipPejabat'];
