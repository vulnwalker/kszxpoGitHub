...
class dpaPPKD1DPObj  extends DaftarObj2{
	var $Prefix = 'dpaPPKD1DP';
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='dpaPPKD1DP.xls';
	var $namaModulCetak='DPA';
	var $Cetak_Judul = 'DPA';
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'dpaPPKD1DPForm';
	var $modul = "DPA";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	...
	$getJumlahBarang = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$dpaPPKD1DP_idplh'"));
	...
	$sisaPaguIndikatif = $paguIndikatif - $getPaguYangTelahTerpakai['paguYangTerpakai'];
	 ...
	 if(mysql_num_rows(mysql_query("select * from view_dpa_ppkd_1 where c1='0' and f = '00' and k = '$k' and l ='$l' and m='$m' and n='$n' and o='$o'  and id_tahap='$this->idTahap' ")) > 0){
 	 ...
 	 if(empty($cmbJenisDPAForm) ){
		...
		$query = VulnWalkerUpdate('tabel_anggaran',$data,"id_anggaran = '$dpaPPKD1DP_idplh'");
		mysql_query($query);
	 }
	...
	function set_selector_other($tipe){
	 ...
	 function setPage_HeaderOther(){
		 	...
		 	<A href=\"pages.php?Pg=dpaPPKD1DP\" title='DANA PERIMBANGAN' style='color:blue;'  > DANA PERIMBANGAN </a> |
   ...
   function setPage_OtherScript(){
		...
		return
			...
			<script type='text/javascript' src='js/perencanaanKeuangan/keuangan/dpa/dpaPPKD1DP.js' language='JavaScript' ></script>
	 ...
	 $this->form_height = 400;
	  if ($this->form_fmST==0) {
		...
		foreach ($dt as $key => $value) {
		 ...
		 $jenis_rka = $jenis_rka;
		 $cmbJenisDPA = cmbArray('cmbJenisDPAForm',$jenis_rka,$arrayJenisDPA,'-- JENIS DPA --','onchange=dpaPPKD1DP.unlockFindRekening();');
	 	 if(empty($jenis_rka)){
	  ...
	  $this->form_fields = array(
			...
			'kode7' => array(
						...
						'value'=> "<input type='text' id='kodeRekening' name='kodeRekening' style='width:120px;' readonly value = '$kodeRekening'> &nbsp&nbsp
						 ...
						 <button type='button' id='findRekening' onclick=dpaPPKD1DP.findRekening('$jenis_rka'); $tergantungJenis> CARI </button> "
						 ),
			'kode8' => array(
						'label'=>'HARGA SATUAN',
						'labelWidth'=>150,
						'value'=> "<input type='text' name='hargaSatuan' id='hargaSatuan' value='$hargaSatuan' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='dpaPPKD1DP.bantu();' > <span id='bantu' style='color:red;'> </span>"
						 ),
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	foreach ($_COOKIE as $key => $value) {
		...
		$urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=dpaPPKD1DP.refreshList(true);','-- URUSAN --');

		$codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
		$bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=dpaPPKD1DP.refreshList(true);','-- BIDANG --');

		$codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
		$skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=dpaPPKD1DP.refreshList(true);','-- SKPD --');
		...
		$unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=dpaPPKD1DP.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=dpaPPKD1DP.refreshList(true);','-- SUB UNIT --');
...
$dpaPPKD1DP = new dpaPPKD1DPObj();

$arrayResult = VulnWalkerTahap_v2($dpaPPKD1DP->modul);
...
$dpaPPKD1DP->jenisForm = $jenisForm;
$dpaPPKD1DP->nomorUrut = $nomorUrut;
$dpaPPKD1DP->urutTerakhir = $nomorUrut;
$dpaPPKD1DP->tahun = $tahun;
$dpaPPKD1DP->jenisAnggaran = $jenisAnggaran;
$dpaPPKD1DP->idTahap = $idTahap;
$dpaPPKD1DP->username = $_COOKIE['coID'];

if(empty($dpaPPKD1DP->tahun)){
	...
	/*$dpaPPKD1DP->tahun = "select max(id_anggaran) as max from view_dpa_ppkd_1 where nama_modul = 'dpaPPKD1DP'";*/
	$dpaPPKD1DP->tahun  = $get2['tahun'];
	$dpaPPKD1DP->jenisAnggaran = $get2['jenis_anggaran'];
	$dpaPPKD1DP->urutTerakhir = $get2['no_urut'];
	$dpaPPKD1DP->idTahap = $get2['id_tahap'];
	$dpaPPKD1DP->jenisFormTerakhir = $get2['jenis_form_modul'];
	$dpaPPKD1DP->urutSebelumnya = $dpaPPKD1DP->urutTerakhir - 1;
	...
	$dpaPPKD1DP->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$dpaPPKD1DP->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];

	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$dpaPPKD1DP->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$dpaPPKD1DP->idTahap'"));
	$dpaPPKD1DP->currentTahap = $getCurrenttahap['nama_tahap'];

	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$dpaPPKD1DP->idTahap'"));
	$dpaPPKD1DP->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$dpaPPKD1DP->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$dpaPPKD1DP->provinsi = $setting['provinsi'];
$dpaPPKD1DP->kota = $setting['kota'];
$dpaPPKD1DP->pengelolaBarang = $setting['pengelolaBarang'];
$dpaPPKD1DP->pejabatPengelolaBarang = $setting['pejabat'];
$dpaPPKD1DP->pengurusPengelolaBarang = $setting['pengurus'];
$dpaPPKD1DP->nipPengelola = $setting['nipPengelola'];
$dpaPPKD1DP->nipPengurus = $setting['nipPengurus'];
$dpaPPKD1DP->nipPejabat = $setting['nipPejabat'];
