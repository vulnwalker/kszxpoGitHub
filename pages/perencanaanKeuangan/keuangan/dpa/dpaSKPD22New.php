...
class dpaSKPD22NewObj  extends DaftarObj2{	
	var $Prefix = 'dpaSKPD22New';
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='dpaSKPD22New.xls';
	var $namaModulCetak='DPA';
	var $Cetak_Judul = 'DPA';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'dpaSKPD22NewForm';
	var $modul = "DPA";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	...
	var $nipPejabat = "";
	var $nipPengurus ="";
	var $reportURL = "pages.php?Pg=dpaSKPD22&tipe=Laporan";
		...
		$ContentTotalHal=''; $ContentTotal='';
			...
			$Kiri2 = $TotalColSpan1 > 0 ? "<td class='$ColStyle' colspan='7' align='center'><b>Total</td>": '';
				...
				"<tr>
					...
					<td class='GarisDaftar' align='right'><b><div  id='{$this->Prefix}_cont_sum'>".number_format($this->totalJumlahPlus,2,',','.')."</div></td>
				</tr>" ;
	...
	$getJumlahBarang = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$dpaSKPD22New_idplh'"));
	...
	$sisaPaguIndikatif = $paguIndikatif - $getPaguYangTelahTerpakai['paguYangTerpakai'];
	 ...
	 if(mysql_num_rows(mysql_query("select * from view_dpa_2_2_1 where c1='0' and f1 = '0' and k = '$k' and l ='$l' and m='$m' and n='$n' and o='$o'  and id_tahap='$this->idTahap' ")) > 0){
 	 ...
 	 if(empty($cmbJenisDPAForm) ){
		...
		$query = VulnWalkerUpdate('tabel_anggaran',$data,"id_anggaran = '$dpaSKPD22New_idplh'");
		mysql_query($query);
	 }
	...
	<A href=\"pages.php?Pg=dpaSKPD22New1\" title='DPA MURNI'  > DPA-SKPD 2.2.1 </a> |
	<A href=\"pages.php?Pg=dpaSKPD22New\" title='DPA-SKPKD MURNI'  style='color:blue;'> DPA-SKPD 2.2 </a> |
   ...
   function setPage_OtherScript(){
		...
		return 	
			...
			<script type='text/javascript' src='js/perencanaanKeuangan/keuangan/dpa/dpaSKPD22New.js' language='JavaScript' ></script> 
	 ...
	 $this->form_height = 400;
	  if ($this->form_fmST==0) {
		...
		foreach ($dt as $key => $value) { 
		 ...
		 $jenis_rka = $jenis_rka;
		 $cmbJenisDPA = cmbArray('cmbJenisDPAForm',$jenis_rka,$arrayJenisDPA,'-- JENIS DPA --','onchange=dpaSKPD22New.unlockFindRekening();');
	 	 if(empty($jenis_rka)){
	  ...
	  $this->form_fields = array(
			...
			'kode7' => array( 
						...
						'value'=> "<input type='text' id='kodeRekening' name='kodeRekening' style='width:120px;' readonly value = '$kodeRekening'> &nbsp&nbsp
						 ...
						 <button type='button' id='findRekening' onclick=dpaSKPD22New.findRekening('$jenis_rka'); $tergantungJenis> CARI </button> "
						 ),
			'kode8' => array( 
						'label'=>'HARGA SATUAN',
						'labelWidth'=>150, 
						'value'=> "<input type='text' name='hargaSatuan' id='hargaSatuan' value='$hargaSatuan' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='dpaSKPD22New.bantu();' > <span id='bantu' style='color:red;'> </span>"
						 ),	
	...
	function setKolomData($no, $isi, $Mode, $TampilCheckBox){
	 ...
	 if($q == '0'){
		...
		while($got = mysql_fetch_array($getAllKegiatan)){
			...
			$gotE1 = $got['e1'];
			$gotQ= $got['q'];
			$getIdAwalRenja = mysql_fetch_array(mysql_query("select min(id_anggaran) as idAwalRenja from view_renja where c1='$gotC1' and c='$gotC' and d='$gotD' and bk='".$got['bk']."' and ck='".$got['ck']."' and p='".$got['p']."' and q='$gotQ' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'  "));
			$idAwalRenja = $getIdAwalRenja['idAwalRenja'];
			$getDetailRenja = mysql_fetch_array(mysql_query("select * from detail_renja where id_anggaran ='$idAwalRenja'"));
			$sumTahunPlus = $sumTahunPlus + $getDetailRenja['plus'];
		}
		...
		$getRenja = mysql_fetch_array(mysql_query("select * from view_renja where id_anggaran ='$idAwalRenja'"));
		$Koloms[] = array('align="left"',  $getRenja['sumber_dana'] );
		$tahunPlus = number_format($getDetailRenja['plus'] ,2,',','.');
		$this->totalJumlahPlus += $getDetailRenja['plus'];
	 }
	 if($q != '0'){
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	if(!isset($selectedC1) ){
		...
		$urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=dpaSKPD22New.refreshList(true);','-- URUSAN --');
		
		$codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
		$bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=dpaSKPD22New.refreshList(true);','-- BIDANG --');
		
		$codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
		$skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=dpaSKPD22New.refreshList(true);','-- SKPD --');
		...
		$unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=dpaSKPD22New.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=dpaSKPD22New.refreshList(true);','-- SUB UNIT --');
...
function Laporan($dt){
   ...
   $this->form_width = 300;
   $this->form_height = 100;
   $this->form_caption = 'LAPORAN DPA-SKPD 2.2 MURNI';
   ...
   $d = $dt['cmbSKPD'];
     ...
     $arrayJenisLaporan = array(
                 array('dpaSKPD22', 'DPA-SKPD 2.2 MURNI'),
    ...
    $this->form_menubawah =
      ...
      "<input type='button' value='Batal' onclick ='".'dpaSKPD22'.".Close()' >";
    ...
    $arrayTandaTangan = explode(';', $getJenisReport['tanda_tangan']);
      ...
      <script type='text/javascript' src='assets/js/bootstrap.min.js'></script>".
								...
								<tr>
									...
									<th class='th02' rowspan='1' style='text-align: center;'>
										<span style='font-size:18px;font-weight:bold;'>
											DOKUMEN PELAKSANAAN ANGGARAN<br> SATUAN KERJA PERANGKAT DAERAH
										</span>
									</th>
									<th class='th02' rowspan='2' style='text-align: center; width: 20%; border-left: 1px solid; border-bottom: 1px solid;'>
										<span style='font-size: 14px; font-weight: bold;'>
											Formulir<br> DPA - SKPD 2.2
										</span>
									</th>
								</tr>
								<tr style='border-bottom: 1px solid;'>
									<th class='th02'  style='text-align: center; border-top: 1px solid; padding: 4px;'>
		...
		echo "
								...
								<thead>
									<tr>
										<th colspan='11' style='text-align: center; border-top: 1px solid; padding: 3px;' >
											Rekapitulasi Anggaran Belanja Langsung Berdasarkan Program dan Kegiatan
										</th>
									</tr>
									<tr>
									   <th class='th02' width='100'  rowspan='2' colspan='2' >KODE</th>
									   <th class='th02' width='400'  rowspan='2' >URAIAN</th>
									   <th class='th02' width='100'  rowspan='2' >LOKASI<br> KEGIATAN</th>
									   <th class='th02' width='200'  rowspan='2' style='width: 10%;'>TARGET KINERJA<br>(KUANTITATIF)</th>
									   <th class='th02' width='100'  rowspan='2' >SUMBER<br>DANA</th>
									   <th class='th02' colspan='3'  rowspan='1' width='500' >TRIWULAN</th>
									   <th class='th02' width='100'  rowspan='2' >JUMLAH</th>
									   <th class='th01' width='100'  rowspan='3' >TAHUN N + 1</th>
									 
									   </tr>
									   <tr>
									   <th class='th02' rowspan='1'>I</th>
									   <th class='th02' rowspan='1'>II</th>
									   <th class='th02' rowspan='1'>III</th>
									   </tr>

									   <tr>
									   	<th class='th01'>PROGRAM</th>
									   	<th class='th01'>KEGIATAN</th>
									   	<th class='th01'>3</th>
									   	<th class='th01'>4</th>
									   	<th class='th01'>5</th>
									   	<th class='th01'>6</th>
	 ...
	 if($q == '0'){
		...
		$namaKegiatan =  $getNamaKegiatan['nama'];
		echo "<td align='left' class='GarisCetak' style='padding-left: 10px;'>"."<span>".$namaKegiatan."</span>"."</td>";	
		$getIdAwalRenja = mysql_fetch_array(mysql_query("select min(id_anggaran) as idAwalRenja from view_renja where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and bk='".$daqry['bk']."' and ck='".$daqry['ck']."' and p='$p' and q='$q' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'  "));
		$idAwalRenja = $getIdAwalRenja['idAwalRenja'];
		$getDetailRenja = mysql_fetch_array(mysql_query("select * from detail_renja where id_anggaran ='$idAwalRenja'"));
		...
		$tahunPlus = number_format($getDetailRenja['plus'] ,2,',','.');
		 $this->totalTahunPlusReport += $getDetailRenja['plus'];
	 }
	 if($q != '0'){
		 ...
		 echo "<td align='right' class='GarisCetak' >".$tahunPlus."</td>";
	 }else{
	 ...
	 echo "</tr>";
		...
		$ContentTotalHal=''; $ContentTotal='';
			...
			$Kiri2 = $TotalColSpan1 > 0 ? "<td class='$ColStyle' colspan='6' align='center'><b>Total</td>": '';
				$ContentTotal = 
				"<tr>
					<td align='right' colspan='6' class='GarisCetak'><b>JUMLAH</b></td>
					...
					<td class='GarisCetak' align='right'><b><div  id='{$this->Prefix}_cont_sum'>".number_format($this->totalTahunPlusReport,2,',','.')."</div></td>
				</tr>" ;
...
$dpaSKPD22New = new dpaSKPD22NewObj();

$arrayResult = VulnWalkerTahap_v2($dpaSKPD22New->modul);
$jenisForm = $arrayResult['jenisForm'];
$nomorUrut = $arrayResult['nomorUrut'];
$tahun = $arrayResult['tahun'];
$jenisAnggaran = $arrayResult['jenisAnggaran'];
...
$dpaSKPD22New->jenisForm = $jenisForm;
$dpaSKPD22New->nomorUrut = $nomorUrut;
$dpaSKPD22New->tahun = $tahun;
$dpaSKPD22New->jenisAnggaran = $jenisAnggaran;
$dpaSKPD22New->idTahap = $idTahap;
$dpaSKPD22New->username = $_COOKIE['coID'];


$dpaSKPD22New->wajibValidasi = $Main->wajibValidasi;
if($Main->wajibValidasi == TRUE){
	$dpaSKPD22New->sqlValidasi = " and status_validasi ='1' ";
}else{
	$dpaSKPD22New->sqlValidasi = " ";
}


if(empty($dpaSKPD22New->tahun)){
	...
	/*$dpaSKPD22New->tahun = "select max(id_anggaran) as max from view_dpa_2_2_1 where nama_modul = 'dpaSKPD22New'";*/
	$dpaSKPD22New->tahun  = $get2['tahun'];
	$dpaSKPD22New->jenisAnggaran = $get2['jenis_anggaran'];
	$dpaSKPD22New->urutTerakhir = $get2['no_urut'];
	$dpaSKPD22New->jenisFormTerakhir = $get2['jenis_form_modul'];
	$dpaSKPD22New->urutSebelumnya = $dpaSKPD22New->urutTerakhir - 1;
	...
	$dpaSKPD22New->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$dpaSKPD22New->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];
	
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$dpaSKPD22New->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$dpaSKPD22New->idTahap'"));
	$dpaSKPD22New->currentTahap = $getCurrenttahap['nama_tahap'];
	
	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$dpaSKPD22New->idTahap'"));
	$dpaSKPD22New->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$dpaSKPD22New->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$dpaSKPD22New->provinsi = $setting['provinsi'];
$dpaSKPD22New->kota = $setting['kota'];
$dpaSKPD22New->pengelolaBarang = $setting['pengelolaBarang'];
$dpaSKPD22New->pejabatPengelolaBarang = $setting['pejabat'];
$dpaSKPD22New->pengurusPengelolaBarang = $setting['pengurus'];
$dpaSKPD22New->nipPengelola = $setting['nipPengelola'];
$dpaSKPD22New->nipPengurus = $setting['nipPengurus'];
$dpaSKPD22New->nipPejabat = $setting['nipPejabat'];
