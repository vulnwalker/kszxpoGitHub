...
class dpaSKPD1DPObj  extends DaftarObj2{
	var $Prefix = 'dpaSKPD1DP';
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='dpaSKPD1DP.xls';
	var $namaModulCetak='DPA';
	var $Cetak_Judul = 'DPA';
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'dpaSKPD1DPForm';
	var $modul = "DPA";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	...
	$getJumlahBarang = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$dpaSKPD1DP_idplh'"));
	...
	$sisaPaguIndikatif = $paguIndikatif - $getPaguYangTelahTerpakai['paguYangTerpakai'];
	 ...
	 if(mysql_num_rows(mysql_query("select * from view_dpa_1 where c1='0' and f = '00' and k = '$k' and l ='$l' and m='$m' and n='$n' and o='$o'  and id_tahap='$this->idTahap' ")) > 0){
 	 ...
 	 if(empty($cmbJenisDPAForm) ){
		...
		$query = VulnWalkerUpdate('tabel_anggaran',$data,"id_anggaran = '$dpaSKPD1DP_idplh'");
		mysql_query($query);
	 }
	...
	function set_selector_other($tipe){
	 ...
	 function setPage_HeaderOther(){
		 	...
		 	<A href=\"pages.php?Pg=dpaSKPD1DP\" title='PENDAPATAN'  style='color:blue;'> PENDAPATAN </a> |
		 	...
		 	<A href=\"pages.php?Pg=dpaSKPD1DP\" title='DANA PERIMBANGAN' style='color:blue;'  > DANA PERIMBANGAN </a> |
   ...
   function setPage_OtherScript(){
		...
		return
			...
			<script type='text/javascript' src='js/perencanaanKeuangan/keuangan/dpa/dpaSKPD1DP.js' language='JavaScript' ></script>
	 ...
	 $this->form_height = 400;
	  if ($this->form_fmST==0) {
		...
		foreach ($dt as $key => $value) {
		 ...
		 $jenis_rka = $jenis_rka;
		 $cmbJenisDPA = cmbArray('cmbJenisDPAForm',$jenis_rka,$arrayJenisDPA,'-- JENIS DPA --','onchange=dpaSKPD1DP.unlockFindRekening();');
	 	 if(empty($jenis_rka)){
	  ...
	  $this->form_fields = array(
			...
			'kode7' => array(
						...
						'value'=> "<input type='text' id='kodeRekening' name='kodeRekening' style='width:120px;' readonly value = '$kodeRekening'> &nbsp&nbsp
						 ...
						 <button type='button' id='findRekening' onclick=dpaSKPD1DP.findRekening('$jenis_rka'); $tergantungJenis> CARI </button> "
						 ),
			'kode8' => array(
						'label'=>'HARGA SATUAN',
						'labelWidth'=>150,
						'value'=> "<input type='text' name='hargaSatuan' id='hargaSatuan' value='$hargaSatuan' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='dpaSKPD1DP.bantu();' > <span id='bantu' style='color:red;'> </span>"
						 ),
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	foreach ($_COOKIE as $key => $value) {
		...
		$urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=dpaSKPD1DP.refreshList(true);','-- URUSAN --');

		$codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
		$bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=dpaSKPD1DP.refreshList(true);','-- BIDANG --');

		$codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
		$skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=dpaSKPD1DP.refreshList(true);','-- SKPD --');
		...
		$unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=dpaSKPD1DP.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=dpaSKPD1DP.refreshList(true);','-- SUB UNIT --');
...
$dpaSKPD1DP = new dpaSKPD1DPObj();

$arrayResult = VulnWalkerTahap_v2($dpaSKPD1DP->modul);
...
$dpaSKPD1DP->jenisForm = $jenisForm;
$dpaSKPD1DP->nomorUrut = $nomorUrut;
$dpaSKPD1DP->urutTerakhir = $nomorUrut;
$dpaSKPD1DP->tahun = $tahun;
$dpaSKPD1DP->jenisAnggaran = $jenisAnggaran;
$dpaSKPD1DP->idTahap = $idTahap;
$dpaSKPD1DP->username = $_COOKIE['coID'];

if(empty($dpaSKPD1DP->tahun)){
	...
	/*$dpaSKPD1DP->tahun = "select max(id_anggaran) as max from view_dpa_1 where nama_modul = 'dpaSKPD1DP'";*/
	$dpaSKPD1DP->tahun  = $get2['tahun'];
	$dpaSKPD1DP->jenisAnggaran = $get2['jenis_anggaran'];
	$dpaSKPD1DP->urutTerakhir = $get2['no_urut'];
	$dpaSKPD1DP->idTahap = $get2['id_tahap'];
	$dpaSKPD1DP->jenisFormTerakhir = $get2['jenis_form_modul'];
	$dpaSKPD1DP->urutSebelumnya = $dpaSKPD1DP->urutTerakhir - 1;
	...
	$dpaSKPD1DP->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$dpaSKPD1DP->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];

	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$dpaSKPD1DP->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$dpaSKPD1DP->idTahap'"));
	$dpaSKPD1DP->currentTahap = $getCurrenttahap['nama_tahap'];

	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$dpaSKPD1DP->idTahap'"));
	$dpaSKPD1DP->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$dpaSKPD1DP->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$dpaSKPD1DP->provinsi = $setting['provinsi'];
$dpaSKPD1DP->kota = $setting['kota'];
$dpaSKPD1DP->pengelolaBarang = $setting['pengelolaBarang'];
$dpaSKPD1DP->pejabatPengelolaBarang = $setting['pejabat'];
$dpaSKPD1DP->pengurusPengelolaBarang = $setting['pengurus'];
$dpaSKPD1DP->nipPengelola = $setting['nipPengelola'];
$dpaSKPD1DP->nipPengurus = $setting['nipPengurus'];
$dpaSKPD1DP->nipPejabat = $setting['nipPejabat'];
