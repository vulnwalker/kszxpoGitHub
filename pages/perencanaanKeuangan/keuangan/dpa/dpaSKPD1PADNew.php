...
class dpaSKPD1PADNewObj  extends DaftarObj2{
	var $Prefix = 'dpaSKPD1PADNew';
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='dpaSKPD1PADNew.xls';
	var $namaModulCetak='DPA';
	var $Cetak_Judul = 'DPA';
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'dpaSKPD1PADNewForm';
	var $modul = "DPA";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
   ...
   // if( $err=='' && $kategori =='' ) $err= 'Kategori Belum Di Pilih !!';
    ...
    break;
	 ...
	 function setPage_HeaderOther(){
		 	...
		 	<A href=\"pages.php?Pg=dpaSKPD1PADNew\" title='PENDAPATAN'  style='color:blue;'> PENDAPATAN </a> |
		 	...
		 	<tr><td class=\"menudottedline\" width=\"40%\" height=\"20\" style='text-align:right'><B>
		 	<A href=\"pages.php?Pg=dpaSKPD1PADNew\" title='PENDAPATAN ASLI DAERAH' style='color:blue;' > PENDAPATAN ASLI DAERAH </a> |
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	foreach ($_COOKIE as $key => $value) {
		...
		$urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=dpaSKPD1PADNew.refreshList(true);','-- URUSAN --');

		$codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
		$bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=dpaSKPD1PADNew.refreshList(true);','-- BIDANG --');

		$codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
		$skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=dpaSKPD1PADNew.refreshList(true);','-- SKPD --');
		...
		$unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=dpaSKPD1PADNew.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=dpaSKPD1PADNew.refreshList(true);','-- SUB UNIT --');
...
function Laporan($dt){
   ...
   $this->form_width = 300;
   $this->form_height = 100;
   $this->form_caption = 'LAPORAN DPA-SKPD 1 MURNI';
   ...
   $dari = $_REQUEST['dari'];
     ...
     $arrayJenisLaporan = array(
                 array('dpaSKPD1PAD', 'DPA-SKPD 1 MURNI'),
...
function dpaSKPD1PAD($xls =FALSE){
		global $Main;
    ...
    $arrayTandaTangan = explode(';', $getJenisReport['tanda_tangan']);
	  ...
	  $arrKondisi[] = "jenis_anggaran = '$this->jenisAnggaran'";
		...
		$qry ="select * from view_dpa_1 where $Kondisi order by urut";
		$aqry = mysql_query($qry);
    ...
    $css = $xls	? "<style>.nfmt5 {mso-number-format:'\@';}</style>":"<link rel=\"stylesheet\" href=\"css/template_css.css\" type=\"text/css\" />";
      ...
      <script type='text/javascript' src='assets/js/bootstrap.min.js'></script>".
		...
		echo "
								...
								<table class='cetak' style='width:100%; border-bottom: 1px solid; margin-left: 0px; margin-top: -0.8%;'>
								<thead>
									<tr>
		...
		while($daqry = mysql_fetch_array($aqry)){
			...
			if (!empty($k) && !empty($l) && !empty($m)  && !empty($n)  && !empty($o)  && empty($id_rincian_belanja) ){
				//Rekening
						...
						$getDataViewDPA221 = mysql_fetch_array(mysql_query("select * from view_dpa_1 where id_tahap = '$this->idTahap' and bk='$bk' and ck='$ck' and dk='$dk' and p='$p' and q='$q' and k ='$k' and l='$l' and m='$m' and n='$n' and o='$o' $kondisiSKPD $kondisiRekening"));
...
$dpaSKPD1PADNew = new dpaSKPD1PADNewObj();

$arrayResult = VulnWalkerTahap_v2($dpaSKPD1PADNew->modul);
...
$dpaSKPD1PADNew->jenisForm = $jenisForm;
$dpaSKPD1PADNew->nomorUrut = $nomorUrut;
$dpaSKPD1PADNew->urutTerakhir = $nomorUrut;
$dpaSKPD1PADNew->tahun = $tahun;
$dpaSKPD1PADNew->jenisAnggaran = $jenisAnggaran;
$dpaSKPD1PADNew->idTahap = $idTahap;
$dpaSKPD1PADNew->username = $_COOKIE['coID'];

if(empty($dpaSKPD1PADNew->tahun)){
	...
	/*$dpaSKPD1PADNew->tahun = "select max(id_anggaran) as max from view_dpa_1 where nama_modul = 'dpaSKPD1PADNew'";*/
	$dpaSKPD1PADNew->tahun  = $get2['tahun'];
	$dpaSKPD1PADNew->jenisAnggaran = $get2['jenis_anggaran'];
	$dpaSKPD1PADNew->urutTerakhir = $get2['no_urut'];
	$dpaSKPD1PADNew->idTahap = $get2['id_tahap'];
	$dpaSKPD1PADNew->jenisFormTerakhir = $get2['jenis_form_modul'];
	$dpaSKPD1PADNew->urutSebelumnya = $dpaSKPD1PADNew->urutTerakhir - 1;
	...
	$dpaSKPD1PADNew->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$dpaSKPD1PADNew->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];

	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$dpaSKPD1PADNew->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$dpaSKPD1PADNew->idTahap'"));
	$dpaSKPD1PADNew->currentTahap = $getCurrenttahap['nama_tahap'];

	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$dpaSKPD1PADNew->idTahap'"));
	$dpaSKPD1PADNew->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$dpaSKPD1PADNew->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$dpaSKPD1PADNew->provinsi = $setting['provinsi'];
$dpaSKPD1PADNew->kota = $setting['kota'];
$dpaSKPD1PADNew->pengelolaBarang = $setting['pengelolaBarang'];
$dpaSKPD1PADNew->pejabatPengelolaBarang = $setting['pejabat'];
$dpaSKPD1PADNew->pengurusPengelolaBarang = $setting['pengurus'];
$dpaSKPD1PADNew->nipPengelola = $setting['nipPengelola'];
$dpaSKPD1PADNew->nipPengurus = $setting['nipPengurus'];
$dpaSKPD1PADNew->nipPejabat = $setting['nipPejabat'];
