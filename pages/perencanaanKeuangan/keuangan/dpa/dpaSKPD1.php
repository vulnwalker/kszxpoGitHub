...
class dpaSKPD1Obj  extends DaftarObj2{	
	var $Prefix = 'dpaSKPD1';
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='dpaSKPD1.xls';
	var $namaModulCetak='DPA-SKPD';
	var $Cetak_Judul = 'DPA-SKPD';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'dpaSKPD1Form';
	var $modul = "DPA";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	...
	function genRowSum($ColStyle, $Mode, $Total){
		...
		$getData = mysql_fetch_array(mysql_query("select sum(jumlah_harga) from tabel_anggaran where  (rincian_perhitungan !='' or f !='00' ) and id_tahap='$idTahap' and tahun ='$this->tahun' and jenis_anggaran = '$this->jenisAnggaran' and jenis_rka = '1' and nama_modul='DPA-SKPD' $kondisiSKPD $kondisiRekening  and nama_modul = 'DPA-SKPD'"));
	...
	$getJumlahBarang = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$dpaSKPD1_idplh'"));
	 ...
	 if(mysql_num_rows(mysql_query("select * from view_dpa_1 where c1='0' and f = '00' and k = '$k' and l ='$l' and m='$m' and n='$n' and o='$o'  and id_tahap='$this->idTahap' ")) > 0){
 	 ...
 	 if(empty($cmbJenisDPAForm) ){
	 	...
	 	$data = array(
						...
						'jenis_dpa' => $cmbJenisDPAForm,
						'jumlah_harga' => $total
							
					   );
		$query = VulnWalkerUpdate('tabel_anggaran',$data,"id_anggaran = '$dpaSKPD1_idplh'");
		mysql_query($query);
	 }
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){	
		...
		case 'sync':{	
			...
			$getJenisFormRka = mysql_fetch_array(mysql_query("selecrt * from view_rka_1 where id_tahap = '$idTahapTerakhir'"));
			if($getJenisFormRka['jenis_form_modul'] == "PENYUSUNAN" && $this->wajibValidasi == TRUE ){
				$kondisiFilter = $kondisiFilter." and status_validasi ='1' ";
			}
			
			$getAllChildern = mysql_query("select * from view_rka_1 where id_tahap = '$idTahapTerakhir' and rincian_perhitungan !='' $kondisiFilter ");
		
			while($rows = mysql_fetch_array($getAllChildern)){
				foreach ($rows as $key => $value) { 
			 	 $$key = $value; 
				}
				$queryCekPekerjaan = "select * from view_dpa_1 where c1='0' and o1='$o1' and f = '00' and rincian_perhitungan = ''  and id_tahap='$this->idTahap' ";
				...
				if(mysql_num_rows(mysql_query("select * from view_dpa_1 where k='$k' and l='$l' and m='$m' and n='$n' and o='$o' and c1 = '0' and id_tahap='$this->idTahap' ")) == 0){
					$arrayRekening = array(
												 'c1' => '0',
												 'c' => '00',
												 'd' => '00',
				...
				if(mysql_num_rows(mysql_query("select * from view_dpa_1 where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and k='$k' and l='$l' and m='$m' and n='$n' and o='$o' and o1='$o1' and rincian_perhitungan = '$rincian_perhitungan' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and id_tahap='$this->idTahap'")) == 0){
						$data = array(	'c1' => $c1,
										'c' => $c,
										'd' => $d,
										'e' => $e,
		...
		case 'clearTemp':{
				...
				mysql_query("delete from temp_dpa_1_v2 where user ='$username'");	
				foreach ($_REQUEST as $key => $value) { 
			 	 $$key = $value; 
				}
	...
	<A href=\"pages.php?Pg=dpaSKPD221\" title='DPA MURNI'  > DPA-SKPD 2.2.1 </a> |
	<A href=\"pages.php?Pg=dpaSKPD22\" title='DPA-SKPKD MURNI'  > DPA-SKPD 2.2 </a> |
	<A href=\"pages.php?Pg=dpaSKPD21\" title='DPA-SKPKD MURNI'  > DPA-SKPD 2.1 </a> |
	<A href=\"pages.php?Pg=dpaSKPD1\" title='DPA-SKPKD MURNI'   style='color:blue;' > DPA-SKPD 1 </a> |
   ...
   function setPage_OtherScript(){
		...
		return 	
			...
			<script type='text/javascript' src='js/perencanaanKeuangan/keuangan/dpa/dpaSKPD1.js' language='JavaScript' ></script> 
	 ...
	 $this->form_height = 400;
	  if ($this->form_fmST==0) {
		...
		foreach ($dt as $key => $value) { 
		 ...
		 $jenis_dpa = $jenis_dpa;
		 $cmbJenisDPA = cmbArray('cmbJenisDPAForm',$jenis_dpa,$arrayJenisDPA,'-- JENIS DPA --','onchange=dpaSKPD1.unlockFindRekening();');
	 	 if(empty($jenis_dpa)){
	  ...
	  $this->form_fields = array(
			...
			'kode7' => array( 
						...
						'value'=> "<input type='text' id='kodeRekening' name='kodeRekening' style='width:120px;' readonly value = '$kodeRekening'> &nbsp&nbsp
						 ...
						 <button type='button' id='findRekening' onclick=dpaSKPD1.findRekening('$jenis_dpa'); $tergantungJenis> CARI </button> "
						 ),
			'kode8' => array( 
						'label'=>'HARGA SATUAN',
						'labelWidth'=>150, 
						'value'=> "<input type='text' name='hargaSatuan' id='hargaSatuan' value='$hargaSatuan' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='dpaSKPD1.bantu();' > <span id='bantu' style='color:red;'> </span>"
						 ),	
		...
		if(!empty($hiddenP)){
			...
			$getIDTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where no_urut ='$this->urutTerakhir' and tahun ='$this->tahun' and anggaran='$this->jenisAnggaran'"));			
			 ...
			 if($c1 == '0'){
				...
				$tanda = "";
			 	$getAllMemberOfThisRekening = mysql_query("select * from view_dpa_1 where id_tahap = '$idTahap' and k='$k' and l ='$l' and m = '$m' and n ='$n' and o ='$o' and (rincian_perhitungan != '' or j!='000')");
				...
				for($nambur = 0 ; $nambur < sizeof($arrayMemberRek) ; $nambur++){
					if(mysql_num_rows(mysql_query("select * from tabel_spd where tahun = '$this->tahun' and anggaran = '$this->jenisAnggaran' and jenis_rka = '1' and jenis_dpa = 'DPA-SKPD' and concat(c1,'.',c,'.',d) = '$arrayMemberRek[$nambur]'  and k='$k' and l ='$l' and m = '$m' and n ='$n' and o ='$o'")) == 0){
						$tanda = "color:red;";
					}
				}	 
			 ...
			 //batas
			 	if(strlen($k) > 1){
					...
					if($k == '0' && $n == '0'){
						...
						$jarak = '15px';
				 		$cekAdaParent = mysql_num_rows(mysql_query("select * from view_dpa_1 where id_tahap = '$idTahap' and o1 = '$o1' and o1 !='0' and o1 !='' "));
				...
				$ilustrasi = "";
				$getSumJumlahBarang = mysql_fetch_array(mysql_query("select sum(volume_rek) as total from view_dpa_1 where k='$k' and l='$l' and m='$m' and n='$n' and o='$o' $kondisiSKPD and id_tahap='$idTahap'"));
				$Koloms[] = array('align="left"',"" );
			 ...
			 $getSumSatuanRek = mysql_fetch_array(mysql_query("select sum(jumlah) as total from view_dpa_1 where k='$k' and l='$l' and m='$m' and n='$n'  and o='$o' $kondisiSKPD and id_tahap='$idTahap'"));
			 ...
			 if($c1 == '0'){
			 if($k == '0' && $n =='0'){
			 	$getTotalJumalhHarga = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as total from view_dpa_1 where o1='$o1' $kondisiSKPD $kondisiBelanja and id_tahap='$idTahap'"));
			 }else{
			 	$getTotalJumalhHarga = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as total from view_dpa_1 where k='$k' and l='$l' and m='$m' and n='$n' and o='$o' $kondisiSKPD and id_tahap='$idTahap'"));	
			 }
	...
	function Validasi($dt){	
	 ...
	 $this->form_caption = 'VALIDASI DPA 1';
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	foreach ($_COOKIE as $key => $value) { 
		...
		$urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=dpaSKPD1.refreshList(true);','-- URUSAN --');
		
		$codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
		$bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=dpaSKPD1.refreshList(true);','-- BIDANG --');
		
		$codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
		$skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=dpaSKPD1.refreshList(true);','-- SKPD --');
		...
		$unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=dpaSKPD1.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=dpaSKPD1.refreshList(true);','-- SUB UNIT --');
...
$dpaSKPD1 = new dpaSKPD1Obj();

$arrayResult = VulnWalkerTahap_v2($dpaSKPD1->modul);
$jenisForm = $arrayResult['jenisForm'];
$nomorUrut = $arrayResult['nomorUrut'];
$tahun = $arrayResult['tahun'];
$jenisAnggaran = $arrayResult['jenisAnggaran'];
...
$dpaSKPD1->jenisForm = $jenisForm;
$dpaSKPD1->nomorUrut = $nomorUrut;
$dpaSKPD1->urutTerakhir = $nomorUrut;
$dpaSKPD1->tahun = $tahun;
$dpaSKPD1->jenisAnggaran = $jenisAnggaran;
$dpaSKPD1->idTahap = $idTahap;
$dpaSKPD1->username = $_COOKIE['coID'];

$dpaSKPD1->wajibValidasi = $Main->wajibValidasi;
if($Main->wajibValidasi == TRUE){
	$dpaSKPD1->sqlValidasi = " and status_validasi ='1' ";
}else{
	$dpaSKPD1->sqlValidasi = " ";
}



if(empty($dpaSKPD1->tahun)){
	...
	/*$dpaSKPD1->tahun = "select max(id_anggaran) as max from view_dpa_1 where nama_modul = 'dpaSKPD1'";*/
	$dpaSKPD1->tahun  = $get2['tahun'];
	$dpaSKPD1->jenisAnggaran = $get2['jenis_anggaran'];
	$dpaSKPD1->urutTerakhir = $get2['no_urut'];
	$dpaSKPD1->jenisFormTerakhir = $get2['jenis_form_modul'];
	$dpaSKPD1->urutSebelumnya = $dpaSKPD1->urutTerakhir - 1;
	...
	$dpaSKPD1->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$dpaSKPD1->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];
	
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$dpaSKPD1->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$dpaSKPD1->idTahap'"));
	$dpaSKPD1->currentTahap = $getCurrenttahap['nama_tahap'];
	
	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$dpaSKPD1->idTahap'"));
	$dpaSKPD1->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$dpaSKPD1->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$dpaSKPD1->provinsi = $setting['provinsi'];
$dpaSKPD1->kota = $setting['kota'];
$dpaSKPD1->pengelolaBarang = $setting['pengelolaBarang'];
$dpaSKPD1->pejabatPengelolaBarang = $setting['pejabat'];
$dpaSKPD1->pengurusPengelolaBarang = $setting['pengurus'];
$dpaSKPD1->nipPengelola = $setting['nipPengelola'];
$dpaSKPD1->nipPengurus = $setting['nipPengurus'];
$dpaSKPD1->nipPejabat = $setting['nipPejabat'];
