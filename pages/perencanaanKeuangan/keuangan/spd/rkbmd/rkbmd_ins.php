<?php
class rkbmd_ins_v2Obj  extends DaftarObj2{	
	var $Prefix = 'rkbmd_ins_v2';
	var $elCurrPage="HalDefault";
	var $SHOW_CEK = TRUE;
	var $TblName = 'temp_rkbmd_pengadaan_v2'; //bonus
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){	
		...
		case 'subDelete':{				
			...
			mysql_query("delete from temp_rkbmd_pengadaan_v2 where id='$id'");	
			$getRow = mysql_num_rows(mysql_query("select * from temp_rkbmd_pengadaan_v2 where user = '$username'"));	
			if($getRow > 0){
				$content = "ada";
			}else{
				$content = "";
		...
		case 'subEdit':{				
			...
			if($jumlah > $jumlahKebutuhanRiil && $jumlahKebutuhanRiil != '0' && !empty($jumlahKebutuhanOptimal) && !empty($jumlahKebutuhanMaksimal)){
				...
				mysql_query(VulnWalkerUpdate('temp_rkbmd_pengadaan_v2',$data,"id = '$id'"));
			}
			
			$content = VulnWalkerUpdate('temp_rkbmd_pengadaan_v2',$data,"id = '$id'");
		...
		case 'finish':{				
			$username = $_COOKIE['coID'];
			$execute = mysql_query("select * from temp_rkbmd_pengadaan_v2 where user='$username'");	
			...
			mysql_query("delete from tabel_anggaran where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and bk='$bk' and ck='$ck' and p='$p' and q='$q' and tahun='$this->tahun' and id_tahap ='$this->idTahap' and ((id_jenis_pemeliharaan = '0' and f !='00') or uraian_pemeliharaan = 'RKBMD PENGADAAN') and status_validasi !='1' ");
			...
			$execute2 = mysql_query("select * from temp_rkbmd_pengadaan_v2 where user='$username'");
		...
		case 'Simpan':{
			...
			$username = $_COOKIE['coID'];
			$cekAda = mysql_num_rows(mysql_query("select * from temp_rkbmd_pengadaan_v2 where  f='$f' and g='$g' and h='$h' and i='$i' and j='$j' and user='$username'"));
			if($cekAda == 1){
			...
			if($err == ''){
				...
				$query = VulnWalkerInsert("temp_rkbmd_pengadaan_v2",$data);
	...
	$cmbKegiatan = cmbQuery('q', $q, $codeAndNameKegiatan,' disabled','-- KEGIATAN --'); 
		...
		case 'subShowEdit':{
			...
			$get = mysql_fetch_array(mysql_query("select * from temp_rkbmd_pengadaan_v2 where id = '$id'"));
			foreach ($get as $key => $value) { 
			  $$key = $value; 
			}
		    $concat2 = $f.".".$g.".".$h.".".$i.".".$j;
   ...
   function setPage_OtherScript(){
		...
		return
			...
			"<script type='text/javascript' src='js/perencanaan_v2/rkbmd/rkbmd_ins.js' language='JavaScript' ></script>".
			...
			<script type='text/javascript' src='js/perencanaan_v2/rkbmd/popupBarang.js' language='JavaScript' ></script>
	...
	$aksi  = "<img src='images/administrator/images/invalid.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkbmd_ins_v2.hapus('$id');></img>&nbsp &nbsp <img src='images/administrator/images/edit_f2.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkbmd_ins_v2.edit('$id');></img> ";
	...
	$cekMayoritas = mysql_fetch_array(mysql_query("select * from temp_rkbmd_pengadaan_v2 where user = '$this->username'"));
	$e = $cekMayoritas['e'];
	$e1 = $cekMayoritas['e1'];
	...
	$cmbKegiatan = cmbQuery('q', $selecteQ, $codeAndNameKegiatan,' onchange=rkbmd_ins_v2.CekAda(); ','-- KEGIATAN --');  
	
	$cekGordon = mysql_num_rows(mysql_query("select * from temp_rkbmd_pengadaan_v2 where user = '$username'"));
	if($cekGordon > 0){
		$cokot = mysql_fetch_array(mysql_query("select * from temp_rkbmd_pengadaan_v2 where user = '$username'"));
		$selectedBK = $cokot['bk'];
		$selectedCK = $cokot['ck'];
		$selectedP = $cokot['p'];
		$selecteQ = $cokot['q'];
					...
					$this->isiform(
						array(
							...
							array( 
								...
								<input type='button' value='Cari' id='findProgram' onclick ='rkbmd_ins_v2.CariProgram($ID_RENJA)'  title='Cari Program dan Kegiatan' $disabledCariProgram >"				
							),
							array( 
								'label'=>'KEGIATAN',
								'label-width'=>'200px;',
			...
			genFilterBar(
						...
						array(
							...
							array(
									...
									'value'=>"<input type='text' name='kodeBarang' onkeyup='cariBarang.pilBar2(this.value)' id='kodeBarang' placeholder='KODE BARANG' style='width:150px;' value='".$dt['kodebarangnya']."' readonly /> 
										...
										<input type='button' id='findBarang' value='CARI' onclick='rkbmd_ins_v2.CariBarang();'/>
									",
								),
							array(
									'label'=>'JUMLAH',
				...
				genFilterBar(
					...
					<td class='border:none'> 
						<a class='toolbar' id='btsave' href='javascript:rkbmd_ins_v2.$tujuan'> 
					...
					<tbody><tr valign='middle' align='center'> 
					<td class='border:none'> 
						<a class='toolbar' id='btcancel' href='javascript:rkbmd_ins_v2.closeTab()'> 
					...
					<tbody><tr valign='middle' align='center'> 
					<td class='border:none'> 
						<a class='toolbar' id='finish' href='javascript:rkbmd_ins_v2.finish()'> 
...
$rkbmd_ins_v2 = new rkbmd_ins_v2Obj();

$arrayResult = VulnWalkerTahap_v2($rkbmd_ins_v2->modul);
$jenisForm = $arrayResult['jenisForm'];
$nomorUrut = $arrayResult['nomorUrut'];
$tahun = $arrayResult['tahun'];
$jenisAnggaran = $arrayResult['jenisAnggaran'];
$idTahap = $arrayResult['idTahap'];
$rkbmd_ins_v2->username = $_COOKIE['coID'];
$rkbmd_ins_v2->jenisForm = $jenisForm;
$rkbmd_ins_v2->nomorUrut = $nomorUrut;
$rkbmd_ins_v2->tahun = $tahun;
$rkbmd_ins_v2->jenisAnggaran = $jenisAnggaran;
$rkbmd_ins_v2->idTahap = $idTahap;
