...
class drafAnggaranKasObj  extends DaftarObj2{
	var $Prefix = 'drafAnggaranKas';
	...
	var $PageTitle = 'DRAF ANGGARAN KAS';
	var $PageIcon = 'images/masterData_01.gif';
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='drafAnggaranKas.xls';
	var $namaModulCetak='DPA';
	var $Cetak_Judul = 'DPA';
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'drafAnggaranKasForm';
	var $modul = "SPD";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	...
	function setTitle(){
		return 'DRAF ANGGARAN KAS '.$this->jenisAnggaran.' TAHUN '.$this->tahun;
	}
	function genRowSum($ColStyle, $Mode, $Total){
		...
		"<tr>
			...
			<td class='GarisDaftar' align='right'><b><div  id='{$this->Prefix}_cont_sum'>".number_format($this->totalTriwulan1,2,',','.')."</div></td>
			<td class='GarisDaftar' align='right'><b><div  id='{$this->Prefix}_cont_sum'>".number_format($this->totalTriwulan2,2,',','.')."</div></td>
			<td class='GarisDaftar' align='right'><b><div  id='{$this->Prefix}_cont_sum'>".number_format($this->totalTriwulan3,2,',','.')."</div></td>
			<td class='GarisDaftar' align='right'><b><div  id='{$this->Prefix}_cont_sum'>".number_format($this->totalTriwulan4,2,',','.')."</div></td>
			$tergantung
		</tr>" ;
	...
	function set_selector_other($tipe){
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){
		...
		case 'Remove':{
			$id = $_REQUEST['drafAnggaranKas_cb'];
			foreach ($_REQUEST as $key => $value) {
				  $$key = $value;
			}
	 ...
	 function setPage_HeaderOther(){
		 ...
		 <A href=\"pages.php?Pg=drafAnggaranKas\" title='DRAF' style='color:blue;' > DRAF </a> |
		 <A href=\"pages.php?Pg=listSpd\" title='SPD' > SPD </a>
   ...
   function setPage_OtherScript(){
		...
		return
			...
			<script type='text/javascript' src='js/perencanaanKeuangan/keuangan/spd/drafAnggaranKas.js' language='JavaScript' ></script>
	 ...
	 $this->form_height = 400;
	  if ($this->form_fmST==0) {
		...
		foreach ($dt as $key => $value) {
		 ...
		 $jenis_rka = $jenis_rka;
		 $cmbJenisDPA = cmbArray('cmbJenisDPAForm',$jenis_rka,$arrayJenisDPA,'-- JENIS DPA --','onchange=drafAnggaranKas.unlockFindRekening();');
	 	 if(empty($jenis_rka)){
	  ...
	  $this->form_fields = array(
			...
			'kode7' => array(
						...
						'value'=> "<input type='text' id='kodeRekening' name='kodeRekening' style='width:120px;' readonly value = '$kodeRekening'> &nbsp&nbsp
						 ...
						 <button type='button' id='findRekening' onclick=drafAnggaranKas.findRekening('$jenis_rka'); $tergantungJenis> CARI </button> "
						 ),
			'kode8' => array(
						'label'=>'HARGA SATUAN',
						'labelWidth'=>150,
						'value'=> "<input type='text' name='hargaSatuan' id='hargaSatuan' value='$hargaSatuan' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='drafAnggaranKas.bantu();' > <span id='bantu' style='color:red;'> </span>"
						 ),
...
/*		 $arrayResult = VulnWalkerTahap_v2('RKA');
		...
		$headerTable =
		  "<thead>
		   ...
		   <th class='th01' width='100'   >PROGRAM / KEGIATAN </th>
		   <th class='th01' width='100'   >KODE REKENING </th>
		   <th class='th01' width='400'   >URAIAN</th>
		   <th class='th01' width='120'   >ANGGARAN</th>
		   <th class='th01' width='120'  >TRIWULAN I</th>
		   <th class='th01' width='120'  >TRIWULAN II</th>
		   <th class='th01' width='120'  >TRIWULAN III</th>
		   <th class='th01' width='120'  >TRIWULAN IV</th>
	...
	//$marginStatus = "20px;";
	  ...
	  //	 $marginKode = "10px;";
	     ...
	     $getSumJumlahHarga = mysql_fetch_array(mysql_query("select sum(jumlah_harga) from tabel_dpa where id_tahap = '$idTahapDpa' and bk='$bk' and ck='$ck' and dk='$dk' and p='$p' and q='$q' and k ='$k' and l='$l' and m='$m' and n='$n' and o='$o' $kondisiSKPD $kondisiRekening"));
	     ...
	     $alokasiTriwulan1 = $alokasiJanuari + $alokasiFebruari + $alokasiMaret;
	     $alokasiTriwulan2 = $alokasiApril + $alokasiMei + $alokasiJuni;
	     $alokasiTriwulan3 = $alokasiJuli + $alokasiAgustus + $alokasiSeptember;
	     $alokasiTriwulan4 = $alokasiOktober + $alokasiNopember + $alokasiDesember;
	     $this->totalTriwulan1 += $alokasiJanuari + $alokasiFebruari + $alokasiMaret;
	     $this->totalTriwulan2 += $alokasiApril + $alokasiMei + $alokasiJuni;
	     $this->totalTriwulan3 += $alokasiJuli + $alokasiAgustus + $alokasiSeptember;
	     $this->totalTriwulan4 += $alokasiOktober + $alokasiNopember + $alokasiDesember;


	$Koloms[] = array(' align="left"', $TampilCheckBox );
	$Koloms[] = array(' align="left"', "<span style='color:$warnaMapping;margin-left:$marginKode;'>".genNumber($bk).".".genNumber($ck).".".genNumber($dk).".".genNumber($p).".".genNumber($q)."</span>" );
	$Koloms[] = array(' align="left"', "<span style='color:$warnaMapping;margin-left:$marginKode;'>$kode</span>" );

	$Koloms[] = array(' align="left"', "<span style='font-weight:$boldStatus;margin-left:$marginStatus'>$uraianList</span> " );
	$Koloms[] = array(' align="right"',  "<span style='font-weight:$boldStatus;'>$jumlahHarga </span>" );


	$queryWhileTW1 = mysql_query("select * from t_nomor_spd_det where refid_anggaran_kas ='".$id."'");
	while($getDetailSPDTW1 = mysql_fetch_array($queryWhileTW1)){
	    $getNomorSPDTW1 = mysql_fetch_array(mysql_query("select * from t_nomor_spd where Id = '".$getDetailSPDTW1['refid_nomor_spd']."' "));
	    if($getNomorSPDTW1['type_spd'] == "TRIWULAN 1"){
	        $nomorSPDTW1 = $getNomorSPDTW1['nomor_spd'];
	    }
	}

	$queryWhileTW2 = mysql_query("select * from t_nomor_spd_det where refid_anggaran_kas ='".$id."'");
	while($getDetailSPDTW2 = mysql_fetch_array($queryWhileTW2)){
	    $getNomorSPDTW2 = mysql_fetch_array(mysql_query("select * from t_nomor_spd where Id = '".$getDetailSPDTW2['refid_nomor_spd']."' "));
	    if($getNomorSPDTW2['type_spd'] == "TRIWULAN 2"){
	        $nomorSPDTW2 = $getNomorSPDTW2['nomor_spd'];
	    }
	}

	$queryWhileTW3 = mysql_query("select * from t_nomor_spd_det where refid_anggaran_kas ='".$id."'");
	while($getDetailSPDTW3 = mysql_fetch_array($queryWhileTW3)){
	    $getNomorSPDTW3 = mysql_fetch_array(mysql_query("select * from t_nomor_spd where Id = '".$getDetailSPDTW3['refid_nomor_spd']."' "));
	    if($getNomorSPDTW3['type_spd'] == "TRIWULAN 3"){
	        $nomorSPDTW3 = $getNomorSPDTW3['nomor_spd'];
	    }
	}

	$queryWhileTW4 = mysql_query("select * from t_nomor_spd_det where refid_anggaran_kas ='".$id."'");
	while($getDetailSPDTW4 = mysql_fetch_array($queryWhileTW4)){
	    $getNomorSPDTW4 = mysql_fetch_array(mysql_query("select * from t_nomor_spd where Id = '".$getDetailSPDTW4['refid_nomor_spd']."' "));
	    if($getNomorSPDTW4['type_spd'] == "TRIWULAN 4"){
	        $nomorSPDTW4 = $getNomorSPDTW4['nomor_spd'];
	    }
	}

	$Koloms[] = array(' align="right"', "<div style='float:left;color:red;'>".$nomorSPDTW1."</div>".number_format($alokasiTriwulan1,2,',','.') );
	$Koloms[] = array(' align="right"', "<div style='float:left;color:red;'>".$nomorSPDTW2."</div>".number_format($alokasiTriwulan2 ,2,',','.') );
	$Koloms[] = array(' align="right"', "<div style='float:left;color:red;'>".$nomorSPDTW3."</div>".number_format($alokasiTriwulan3 ,2,',','.') );
	$Koloms[] = array(' align="right"', "<div style='float:left;color:red;'>".$nomorSPDTW4."</div>". number_format($alokasiTriwulan4 ,2,',','.') );
	...
	foreach ($_COOKIE as $key => $value) {
	  ...
	  $urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=drafAnggaranKas.refreshList(true);','-- URUSAN --');

	  $codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
	  $bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=drafAnggaranKas.refreshList(true);','-- BIDANG --');

	  $codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
	  $skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=drafAnggaranKas.refreshList(true);','-- SKPD --');
	  ...
	  $unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=drafAnggaranKas.refreshList(true);','-- UNIT --');
	  ...
	  $subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=drafAnggaranKas.refreshList(true);','-- SUB UNIT --');
	...
	$program = "<input type='hidden' id='bk' name='bk' value='$selectedBK'> <input type='hidden' id='ck' name='ck' value='$selectedCK'> <input type='hidden' id='dk' name='dk' value='$selectedDK'> <input type='hidden' id='hiddenP' name='hiddenP' value='$selectedP'>".cmbArray('p',$_REQUEST['p'],$arrayP,'-- PROGRAM --','onchange=drafAnggaranKas.programChanged();');
	...
	$kegiatan = cmbArray('q',$_REQUEST['q'],$arrayQ,'-- KEGIATAN --','onchange=drafAnggaranKas.refreshList(true);');
	...
	$cmbBelanja = cmbArray('cmbBelanja',$_REQUEST['cmbBelanja'],$arrayBelanja,'-- JENIS BELANJA--','onchange=drafAnggaranKas.refreshList(true);');
...
$drafAnggaranKas = new drafAnggaranKasObj();

$arrayResult = VulnWalkerTahap_v2($drafAnggaranKas->modul);
$jenisForm = $arrayResult['jenisForm'];
$nomorUrut = $arrayResult['nomorUrut'];
$tahun = $arrayResult['tahun'];
$jenisAnggaran = $arrayResult['jenisAnggaran'];
...
$drafAnggaranKas->jenisForm = $jenisForm;
$drafAnggaranKas->nomorUrut = $nomorUrut;
$drafAnggaranKas->tahun = $tahun;
$drafAnggaranKas->jenisAnggaran = $jenisAnggaran;
$drafAnggaranKas->idTahap = $idTahap;
$drafAnggaranKas->username = $_COOKIE['coID'];

$drafAnggaranKas->wajibValidasi = $Main->wajibValidasi;
if($Main->wajibValidasi == TRUE){
	$drafAnggaranKas->sqlValidasi = " and status_validasi ='1' ";
}else{
	$drafAnggaranKas->sqlValidasi = " ";
}

if(empty($drafAnggaranKas->tahun)){
	...
	$drafAnggaranKas->tahun  = $get2['tahun'];
	$drafAnggaranKas->jenisAnggaran = $get2['jenis_anggaran'];
	$drafAnggaranKas->urutTerakhir = $get2['no_urut'];
	$drafAnggaranKas->jenisFormTerakhir = $get2['jenis_form_modul'];
	$drafAnggaranKas->urutSebelumnya = $spd_v2->urutTerakhir - 1;


	$idtahapTerakhir = $get2['id_tahap'];
	$drafAnggaranKas->idTahap = $idtahapTerakhir;
	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$idtahapTerakhir'"));
	$drafAnggaranKas->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$drafAnggaranKas->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];

	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$drafAnggaranKas->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$spd_v2->idTahap'"));
	$drafAnggaranKas->currentTahap = $getCurrenttahap['nama_tahap'];

	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$spd_v2->idTahap'"));
	$drafAnggaranKas->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$drafAnggaranKas->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$drafAnggaranKas->provinsi = $setting['provinsi'];
$drafAnggaranKas->kota = $setting['kota'];
$drafAnggaranKas->pengelolaBarang = $setting['pengelolaBarang'];
$drafAnggaranKas->pejabatPengelolaBarang = $setting['pejabat'];
$drafAnggaranKas->pengurusPengelolaBarang = $setting['pengurus'];
$drafAnggaranKas->nipPengelola = $setting['nipPengelola'];
$drafAnggaranKas->nipPengurus = $setting['nipPengurus'];
$drafAnggaranKas->nipPejabat = $setting['nipPejabat'];
