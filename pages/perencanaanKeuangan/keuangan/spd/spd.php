...
class spdObj  extends DaftarObj2{	
	var $Prefix = 'spd';
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='spd.xls';
	var $namaModulCetak='RKA';
	var $Cetak_Judul = 'RKA';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'spdForm';
	var $modul = "SPD";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	...
	$getJumlahBarang = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$spd_idplh'"));
	...
	$sisaPaguIndikatif = $paguIndikatif - $getPaguYangTelahTerpakai['paguYangTerpakai'];
	 ...
	 if(mysql_num_rows(mysql_query("select * from tabel_spd where c1='0' and f = '00' and k = '$k' and l ='$l' and m='$m' and n='$n' and o='$o'  and id_tahap='$this->idTahap' ")) > 0){
 	 ...
 	 if(empty($cmbJenisRKAForm) ){
		...
		$query = VulnWalkerUpdate('tabel_anggaran',$data,"id_anggaran = '$spd_idplh'");
		mysql_query($query);
	 }
	...
	<A href=\"pages.php?Pg=spd\" title='INPUT' style='color:blue;' > INPUT </a> |
	<A href=\"pages.php?Pg=spdList\" title='LIST'  > LIST </a> 
	&nbsp&nbsp&nbsp	
	</td></tr>
	</table>";
	}
   function setPage_OtherScript(){
		...
		//if($this->jenisForm == "PENYUSUNAN")
		$read = "<script type='text/javascript' src='js/perencanaanKeuangan/keuangan/spd/spd.js' language='JavaScript' ></script> ";
		return 	
			"
	 ...
	 $this->form_height = 400;
	  if ($this->form_fmST==0) {
		...
		foreach ($dt as $key => $value) { 
		 ...
		 $jenis_rka = $jenis_rka;
		 $cmbJenisRKA = cmbArray('cmbJenisRKAForm',$jenis_rka,$arrayJenisRKA,'-- JENIS RKA --','onchange=spd.unlockFindRekening();');
	 	 if(empty($jenis_rka)){
	  ...
	  $this->form_fields = array(
			...
			'kode7' => array( 
						...
						'value'=> "<input type='text' id='kodeRekening' name='kodeRekening' style='width:120px;' readonly value = '$kodeRekening'> &nbsp&nbsp
						 ...
						 <button type='button' id='findRekening' onclick=spd.findRekening('$jenis_rka'); $tergantungJenis> CARI </button> "
						 ),
			'kode8' => array( 
						'label'=>'HARGA SATUAN',
						'labelWidth'=>150, 
						'value'=> "<input type='text' name='hargaSatuan' id='hargaSatuan' value='$hargaSatuan' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='spd.bantu();' > <span id='bantu' style='color:red;'> </span>"
						 ),	
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	foreach ($_COOKIE as $key => $value) { 
		...
		$urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=spd.refreshList(true);','-- URUSAN --');
		
		$codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
		$bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=spd.refreshList(true);','-- BIDANG --');
		
		$codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
		$skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=spd.refreshList(true);','-- SKPD --');
		...
		$unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=spd.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=spd.refreshList(true);','-- SUB UNIT --');
	...
	$program = "<input type='hidden' id='bk' name='bk' value='$selectedBK'> <input type='hidden' id='ck' name='ck' value='$selectedCK'> <input type='hidden' id='hiddenP' name='hiddenP' value='$selectedP'>".cmbArray('p',$_REQUEST['p'],$arrayP,'-- PROGRAM --','onchange=spd.programChanged();');
	...
	$kegiatan = cmbArray('q',$_REQUEST['q'],$arrayQ,'-- KEGIATAN --','onchange=spd.refreshList(true);');
	...
	//$cmbBelanja = cmbArray('cmbBelanja',$_REQUEST['cmbBelanja'],$arrayBelanja,'-- JENIS BELANJA--','onchange=spd.refreshList(true);');
	$cmbDPA = cmbArray('cmbDPA',$_REQUEST['cmbDPA'],$arrayDPA,'-- JENIS DPA --','onchange=spd.refreshList(true);');
...
$arrayResult = VulnWalkerTahap_v2($spd->modul);
...
$spd->jenisForm = $jenisForm;
$spd->nomorUrut = $nomorUrut;
$spd->tahun = $tahun;
$spd->jenisAnggaran = $jenisAnggaran;
$spd->idTahap = $idTahap;
$spd->username = $_COOKIE['coID'];

$spd->wajibValidasi = $Main->wajibValidasi;
if($Main->wajibValidasi == TRUE){
	$spd->sqlValidasi = " and status_validasi ='1' ";
}else{
	$spd->sqlValidasi = " ";
}
...
if(empty($spd->tahun)){
	...
	/*$spd->tahun = "select max(id_anggaran) as max from tabel_spd where nama_modul = 'spd'";*/
	$spd->tahun  = $get2['tahun'];
	$spd->jenisAnggaran = $get2['jenis_anggaran'];
	$spd->urutTerakhir = $get2['no_urut'];
	$spd->jenisFormTerakhir = $get2['jenis_form_modul'];
	$spd->urutSebelumnya = $spd->urutTerakhir - 1;
	
	
	$idtahapTerakhir = $get2['id_tahap'];
	$spd->idTahap = $idtahapTerakhir;
	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$idtahapTerakhir'"));
	$spd->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$spd->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];
	
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$spd->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$spd->idTahap'"));
	$spd->currentTahap = $getCurrenttahap['nama_tahap'];
	
	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$spd->idTahap'"));
	$spd->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$spd->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$spd->provinsi = $setting['provinsi'];
$spd->kota = $setting['kota'];
$spd->pengelolaBarang = $setting['pengelolaBarang'];
$spd->pejabatPengelolaBarang = $setting['pejabat'];
$spd->pengurusPengelolaBarang = $setting['pengurus'];
$spd->nipPengelola = $setting['nipPengelola'];
$spd->nipPengurus = $setting['nipPengurus'];
$spd->nipPejabat = $setting['nipPejabat'];
