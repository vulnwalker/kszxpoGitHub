...
class r_apbd5Obj  extends DaftarObj2{	
	var $Prefix = 'r_apbd5';
	var $elCurrPage="HalDefault";
	var $SHOW_CEK = TRUE;
	var $TblName = 'view_r_apbd5'; //bonus
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='r_apbd5.xls';
	var $namaModulCetak='RKA';
	var $Cetak_Judul = 'RKA';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'r_apbd5Form';
	var $modul = "RKA-SKPD";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	...
	$getJumlahBarang = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$r_apbd5_idplh'"));
	...
	$getPaguYangTelahTerpakai = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as paguYangTerpakai from view_r_apbd5_1 where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and bk='$bk' and ck='$ck' and p='$p' and q='$q' and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'  and no_urut = '$this->nomorUrut' and id_anggaran!='$r_apbd5_idplh' "));
	$sisaPaguIndikatif = $paguIndikatif - $getPaguYangTelahTerpakai['paguYangTerpakai'];
    
	 if(mysql_num_rows(mysql_query("select * from view_r_apbd5_1 where c1='0' and f1 = '0' and k = '$k' and l ='$l' and m='$m' and n='$n' and o='$o'  and id_tahap='$this->idTahap' ")) > 0){
 	 ...
 	 if(empty($cmbJenisRKAForm) ){
		...
		$query = VulnWalkerUpdate('tabel_anggaran',$data,"id_anggaran = '$r_apbd5_idplh'");
		mysql_query($query);
	 }
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){	
		...
		case 'sesuai':{
			...
			$getr_apbd5nya = mysql_fetch_array(mysql_query($queryRows));
			foreach ($getr_apbd5nya as $key => $value) { 
				  $$key = $value; 
			} 
			$cmbUrusanForm = $c1;
			$cmbBidangForm = $c;
		...
		case 'koreksi':{
			...
			foreach ($getr_apbd5nya as $key => $value) { 
			 ...
			 $getSisaPagu = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as paguYangTerpakai  from view_r_apbd5_1 where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and bk='$bk' and p='$p' and q='$q' and id_tahap='$this->idTahap' and concat(c1,'.',c,'.',d,'.',e,'.',e1,'.',bk,'.',ck,'.',p,'.',q,'.',f1,'.',f2,'.',f,'.',g,'.',h,'.',i,'.',j,'.',id_jenis_pemeliharaan,'.',k,'.',l,'.',m,'.',n,'.',o) != '$filterPagu'  and o1 !='0' and rincian_perhitungan !=''"));
			 $sisaPagu = $paguIndikatif - $getSisaPagu['paguYangTerpakai'];
			 $content .= "Sisa PAGU : ".$sisaPagu;
			 $content .= "getSIsaPAGU nya : "."select sum(jumlah_harga) as paguYangTerpakai  from view_r_apbd5_1 where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and bk='$bk' and p='$p' and q='$q' and id_tahap='$this->idTahap' and concat(c1,'.',c,'.',d,'.',e,'.',e1,'.',bk,'.',ck,'.',p,'.',q,'.',f1,'.',f2,'.',f,'.',g,'.',h,'.',i,'.',j,'.',id_jenis_pemeliharaan,'.',k,'.',l,'.',m,'.',n,'.',o) != '$filterPagu' ";
			 if($hasilKali > $sisaPagu){
			 	$err = "Tidak dapat melebihi Pagu Indikatif";
			 }
			 if($err == ""){
				 	if(mysql_num_rows(mysql_query("select * from view_r_apbd5_1 where o1='0' and rincian_perhitungan ='' and c1='0' and k='$k' and l='$l' and m='$m' and n='$n' and o='$o' and id_tahap='$this->idTahap'  ")) > 0){
					 ...
					 if(mysql_num_rows(mysql_query("select * from view_r_apbd5_1 where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and k = '$k' and l ='$l' and m='$m' and n='$n' and o='$o'  and o1='$o1' and f1='0' and rincian_perhitungan ='' and id_tahap='$this->idTahap' ")) > 0){
		...
		case 'editTab':{
			...
			$getAll = mysql_query("select * from view_r_apbd5_1 where concat(k,'.',l,'.',m,'.',n,'.',o) ='$kodeRek' and id_tahap='$this->idTahap' and rincian_perhitungan !=''   and c1='$cmbUrusan' and c='$cmbBidang' and d='$cmbSKPD' and e='$cmbUnit' and e1='$cmbSubUnit' and bk='$bk' and ck='$ck' and p='$hiddenP' and q='$q' order by o1, rincian_perhitungan");
			mysql_query("delete from temp_rka_221 where user='$username'");
	...
	<A href=\"pages.php?Pg=r-apbd\" title='R-APRBD'  > LAMPIRAN II </a> |
	<A href=\"pages.php?Pg=r-apbd5\" title='R-APRBD' style='color : blue;' > LAMPIRAN IV </a> |
   ...
   function setPage_OtherScript(){
		...
		return 	
			...
			<script type='text/javascript' src='js/perencanaan/r-apbd/r-apbd5.js' language='JavaScript' ></script> 
	 ...
	 $this->form_height = 400;
	  if ($this->form_fmST==0) {
		...
		foreach ($dt as $key => $value) { 
		 ...
		 $jenis_rka = $jenis_rka;
		 $cmbJenisRKA = cmbArray('cmbJenisRKAForm',$jenis_rka,$arrayJenisRKA,'-- JENIS RKA --','onchange=r_apbd5.unlockFindRekening();');
	 	 if(empty($jenis_rka)){
	  ...
	  $this->form_fields = array(
			...
			'kode7' => array( 
						...
						'value'=> "<input type='text' id='kodeRekening' name='kodeRekening' style='width:120px;' readonly value = '$kodeRekening'> &nbsp&nbsp
						 ...
						 <button type='button' id='findRekening' onclick=r_apbd5.findRekening('$jenis_rka'); $tergantungJenis> CARI </button> "
						 ),
			'kode8' => array( 
						'label'=>'HARGA SATUAN',
						'labelWidth'=>150, 
						'value'=> "<input type='text' name='hargaSatuan' id='hargaSatuan' value='$hargaSatuan' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='r_apbd5.bantu();' > <span id='bantu' style='color:red;'> </span>"
						 ),	
	...
	function setKolomData($no, $isi, $Mode, $TampilCheckBox){
	 ...
	 if($q !='0'){
		...
		$getBelanjaPegawai = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as belanjaPegawai from tabel_anggaran where nama_modul = 'RKA-SKPD' and c1='$c1' and c='$c' and d='$d' $kondisiFilter and bk='$bk' and ck='$ck' and p='$p'  and k='5' and l='2' and m='1' "));
	   	$getBelanjaBarangJasa = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as belanjaBarangJasa from tabel_anggaran where nama_modul = 'RKA-SKPD' and c1='$c1' and c='$c' and d='$d' $kondisiFilter and bk='$bk' and ck='$ck' and p='$p'  and k='5' and l='2' and m='2' "));
	 	$getBelanjaModal = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as belanjaModal from tabel_anggaran where nama_modul = 'RKA-SKPD' c1='$c1' and c='$c' and d='$d' $kondisiFilter and bk='$bk' and ck='$ck' and p='$p' and k='5' and l='2' and m='3' "));
		...
		$getBelanjaPegawai = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as belanjaPegawai from tabel_anggaran where nama_modul = 'RKA-SKPD' and c1='$c1' and c='$c' and d='$d' $kondisiFilter   and k='5' and l='2' and m='1' "));
	 	...
	 	$getBelanjaModal = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as belanjaModal from tabel_anggaran where nama_modul = 'RKA-SKPD' c1='$c1' and c='$c' and d='$d' $kondisiFilter  and k='5' and l='2' and m='3' "));
		$Koloms[] = array('align="right"',"<span style='font-weight:bold;'>" . number_format($getBelanjaPegawai['belanjaPegawai'] ,2,',','.') );
		$Koloms[] = array('align="right"',"<span style='font-weight:bold;'>" . number_format($getBelanjaBarangJasa['belanjaBarangJasa'] ,2,',','.') );
		$Koloms[] = array('align="right"',"<span style='font-weight:bold;'>" . number_format($getBelanjaModal['belanjaModal'] ,2,',','.') );
		$jumlahRekening = $getBelanjaPegawai['belanjaPegawai'] + $getBelanjaBarangJasa['belanjaBarangJasa'] + $getBelanjaModal['belanjaModal'];
		...
		$getBelanjaPegawai = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as belanjaPegawai from tabel_anggaran where nama_modul = 'RKA-SKPD' and c1='$c1' and c='$c'  $kondisiFilter   and k='5' and l='2' and m='1' "));
	 	...
	 	$getBelanjaModal = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as belanjaModal from tabel_anggaran where nama_modul = 'RKA-SKPD' c1='$c1' and c='$c'  $kondisiFilter  and k='5' and l='2' and m='3' "));
		$Koloms[] = array('align="right"',"<span style='font-weight:bold;'>" . number_format($getBelanjaPegawai['belanjaPegawai'] ,2,',','.') );
		$Koloms[] = array('align="right"',"<span style='font-weight:bold;'>" . number_format($getBelanjaBarangJasa['belanjaBarangJasa'] ,2,',','.') );
		$Koloms[] = array('align="right"',"<span style='font-weight:bold;'>" . number_format($getBelanjaModal['belanjaModal'] ,2,',','.') );
		$jumlahRekening = $getBelanjaPegawai['belanjaPegawai'] + $getBelanjaBarangJasa['belanjaBarangJasa'] + $getBelanjaModal['belanjaModal'];
		...
		$getBelanjaPegawai = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as belanjaPegawai from tabel_anggaran where nama_modul = 'RKA-SKPD' and c1='$c1'  $kondisiFilter   and k='5' and l='2' and m='1' "));
	 	...
	 	$getBelanjaModal = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as belanjaModal from tabel_anggaran where nama_modul = 'RKA-SKPD' c1='$c1'   $kondisiFilter  and k='5' and l='2' and m='3' "));
		$Koloms[] = array('align="right"',"<span style='font-weight:bold;'>" . number_format($getBelanjaPegawai['belanjaPegawai'] ,2,',','.') );
		$Koloms[] = array('align="right"',"<span style='font-weight:bold;'>" . number_format($getBelanjaBarangJasa['belanjaBarangJasa'] ,2,',','.') );
		$Koloms[] = array('align="right"',"<span style='font-weight:bold;'>" . number_format($getBelanjaModal['belanjaModal'] ,2,',','.') );
		$jumlahRekening = $getBelanjaPegawai['belanjaPegawai'] + $getBelanjaBarangJasa['belanjaBarangJasa'] + $getBelanjaModal['belanjaModal'];
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	if($_COOKIE['VulnWalkerUrusan'] != '0' || $_COOKIE['VulnWalkerUrusan'] != '00' ){
		...
		$urusan = cmbQuery('cmbUrusan',$VulnWalkerUrusan,$codeAndNameUrusan,'onchange=r_apbd5.refreshList(true);','-- URUSAN --');
		...
		$bidang = cmbQuery('cmbBidang',$VulnWalkerBidang,$codeAndNameBidang,'onchange=r_apbd5.refreshList(true);','-- BIDANG --');
		...
		$skpd= cmbQuery('cmbSKPD',$VulnWalkerSKPD,$codeAndNameSKPD,'onchange=r_apbd5.refreshList(true);','-- SKPD --');
		...
		$unit = cmbQuery('cmbUnit',$VulnWalkerUnit,$codeAndNameUnit,'onchange=r_apbd5.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$VulnWalkerSubUnit,$codeAndNameSubUnit,'onchange=r_apbd5.refreshList(true);','-- SUB UNIT --');*/
		...
		$urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=r_apbd5.refreshList(true);','-- URUSAN --');
		
		$codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
		$bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=r_apbd5.refreshList(true);','-- BIDANG --');
		
		$codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
		$skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=r_apbd5.refreshList(true);','-- SKPD --');
...
/*		$codeAndNameUnit = "select e, concat(e, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d='$selectedD' and e!='00' and e1='000' ";
		$unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=r_apbd5.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=r_apbd5.refreshList(true);','-- SUB UNIT --');*/
	}
	...
	if($this->jenisForm == "KOREKSI" || $this->jenisForm == "PENYUSUNAN" || $this->jenisForm == "VALIDASI"){
		...
		$getPaguYangTerpakai =  mysql_fetch_array(mysql_query("select sum(jumlah_harga) as paguYangTerpakai from view_r_apbd5_1 where c1= '$selectedC1' and c='$selectedC' and d='$selectedD' and e='$selectedE' and e1='$selectedE1' and bk='$selectedBK' and ck='$selectedCK' and p='$selectedP' and q='$selectedQ' and id_tahap = '$this->idTahap'  "));
	...
	function getDaftarOpsi($Mode=1){
		...
		foreach ($_REQUEST as $key => $value) { 
			...
			while($rows = mysql_fetch_array($getAllFromRka221)){
				...
				if(mysql_fetch_array(mysql_query("select * from view_r_apbd5 where c1 = '$c1' and c='00' and d='00' and e='00' and e1='000' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' ")) > 0){
				}else{
					$data = array(
									'c1' => $c1,
									'c' => '00',
				...
				if(mysql_fetch_array(mysql_query("select * from view_r_apbd5 where c1 = '$c1' and c='$c' and d='00' and e='00' and e1='000' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' ")) > 0){
				}else{
					$data = array(
									'c1' => $c1,
									'c' => $c,
				...
				if(mysql_fetch_array(mysql_query("select * from view_r_apbd5 where c1 = '$c1' and c='$c' and d='$d' and e='00' and e1='000' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' ")) > 0){
				}else{
					$data = array(
									'c1' => $c1,
									'c' => $c,
				...
				if(mysql_fetch_array(mysql_query("select * from view_r_apbd5 where  c1='$c1' and c='$c' and d='$d' and e='00' and e1='000' and bk='$bk' and ck='$ck' and p='$p' and q='0' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' ")) > 0){
				}else{
					$data = array(
									'c1' => $c1,
									'c' => $c,
				...
				if(mysql_fetch_array(mysql_query("select * from view_r_apbd5 where c1='$c1' and c='$c' and d='$d' and e='00' and e1='000' and bk='$bk' and ck='$ck' and p='$p' and q='$q' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' ")) > 0){
				}else{
					$data = array(
									'c1' => $c1,
									'c' => $c,
		...
		/*$getAllProgram = mysql_query("select * from view_r_apbd5 where q != '0'");
		$arrayProgram = array();
		while($rows = mysql_fetch_array($getAllProgram)){
			foreach ($rows as $key => $value) { 
				  $$key = $value; 
			}
			if(mysql_num_rows(mysql_query("select * from view_r_apbd5_1 where  o1 !='0' and (rincian_perhitungan !='' or f1 !='0' ) and bk='$bk' and ck='$ck' and p='$p' and q='$q'  $kondisiFilter $kondisiSKPD")) == 0){
				$arrKondisi[] = "id_anggaran !='$id_anggaran'";	
				if(mysql_num_rows(mysql_query("select * from view_r_apbd5_1 where  o1 !='0' and (rincian_perhitungan !='' or f1 !='0' ) and bk='$bk' and ck='$ck' and p='$p'   $kondisiFilter $kondisiSKPD")) == 0){
					$concat = $bk.".".$ck.".".$p;
					$arrKondisi[] = "concat(bk,'.',ck,'.',p) !='$concat'";
				}
			}else{
					$getIDProgram =  mysql_fetch_array(mysql_query("select * from view_r_apbd5 where bk='$bk' and ck='$ck' and p='$p' and  q = '0'"));
...
$r_apbd5 = new r_apbd5Obj();

$arrayResult = VulnWalkerTahap($r_apbd5->modul);
$jenisForm = $arrayResult['jenisForm'];
$nomorUrut = $arrayResult['nomorUrut'];
$tahun = $arrayResult['tahun'];
$jenisAnggaran = $arrayResult['jenisAnggaran'];
...
$r_apbd5->jenisForm = $jenisForm;
$r_apbd5->nomorUrut = $nomorUrut;
$r_apbd5->tahun = $tahun;
$r_apbd5->jenisAnggaran = $jenisAnggaran;
$r_apbd5->idTahap = $idTahap;


if(empty($r_apbd5->tahun)){
    
	$get1 = mysql_fetch_array(mysql_query("select max(id_anggaran)  from view_r_apbd5_1 "));
	$maxAnggaran = $get1['max(id_anggaran)'];
	$get2 = mysql_fetch_array(mysql_query("select * from view_r_apbd5_1 where id_anggaran = '$maxAnggaran'"));
	/*$r_apbd5->tahun = "select max(id_anggaran) as max from view_r_apbd5_1 where nama_modul = 'r_apbd5'";*/
	$r_apbd5->tahun  = $get2['tahun'];
	$r_apbd5->jenisAnggaran = $get2['jenis_anggaran'];
	$r_apbd5->urutTerakhir = $get2['no_urut'];
	$r_apbd5->jenisFormTerakhir = $get2['jenis_form_modul'];
	$r_apbd5->urutSebelumnya = $r_apbd5->urutTerakhir - 1;
	...
	$r_apbd5->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$r_apbd5->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];
	
	$arrayHasil =  VulnWalkerLASTTahap();
	$r_apbd5->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$r_apbd5->idTahap'"));
	$r_apbd5->currentTahap = $getCurrenttahap['nama_tahap'];
	
	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$r_apbd5->idTahap'"));
	$r_apbd5->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$r_apbd5->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
