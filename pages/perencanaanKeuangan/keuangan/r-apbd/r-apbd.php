...
class r_apbd_v2Obj  extends DaftarObj2{	
	var $Prefix = 'r_apbd_v2';
	var $elCurrPage="HalDefault";
	var $SHOW_CEK = TRUE;
	var $TblName = 'view_r_apbd'; //bonus
	var $TblName_Hapus = 'tabel_anggaran';
	var $MaxFlush = 10;
	var $TblStyle = array( 'koptable', 'cet
	ak','cetak'); //berdasar mode
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='r_apbd_v2.xls';
	var $namaModulCetak='r_apbd_v2';
	var $Cetak_Judul = 'r_apbd_v2';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'r_apbd_v2Form';
	var $modul = "R-APBD";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	  ...
	  switch($tipe){	
		...
		if($this->jenisForm == "READ"){
			...
			while($rows = mysql_fetch_array($getAllSkpdFromRka)){
				...
				if(mysql_num_rows(mysql_query("select * from view_r_apbd where c1 = '$c1' and c='00' and d='00' and e='00' and e1='000' and j = '000'  and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and id_tahap = '$this->idTahap'")) == 0){
				...
				$dataBidang = array(
					...
					if(mysql_num_rows(mysql_query("select * from view_r_apbd where c1 = '$c1' and c='$c' and d='00' and e='00' and e1='000' and j = '000'  and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and id_tahap = '$this->idTahap'")) == 0){
				...
				$dataSKPD = array(
					...
					if(mysql_num_rows(mysql_query("select * from view_r_apbd where c1 = '$c1' and c='$c' and d='$d' and e='00' and e1='000' and j = '000'  and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and id_tahap = '$this->idTahap'")) == 0){
	    ...
	    case 'SaveValid':{
	    	foreach ($_REQUEST as $key => $value) { 
			 ...
			 $getSKPD = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$r_apbd_v2_idplh'"));
			 ...
			 $query = VulnWalkerUpdate("tabel_anggaran",$data," id_anggaran = '$r_apbd_v2_idplh'");
			 mysql_query($query);
	...
	<A href=\"pages.php?Pg=r-apbd_v2\" title='R-APRBD' style='color : blue;' > LAMPIRAN II </a> |
	<A href=\"pages.php?Pg=r-apbd4_v2\" title='R-APRBD' > LAMPIRAN IV </a> |
   ...
   function setPage_OtherScript(){
		$scriptload = 
					...
					//if($this->jenisForm == "READ")
					$read ="<script type='text/javascript' src='js/perencanaan_v2/r-apbd/r-apbd.js' language='JavaScript' ></script> ";
		return 	
			"
...
function Laporan($xls =FALSE){
	 ...
	 $getIdTahapTerakhir = mysql_fetch_array(mysql_query("select max(id_tahap) from tabel_anggaran where tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and nama_modul = 'RKA-SKPD' and id_tahap < $this->idTahap"));
	  ...
	  $getPembiayaan = mysql_query("select * from view_r_apbd $Kondisi2");
...
$r_apbd_v2 = new r_apbd_v2Obj();

$arrayResult = VulnWalkerTahap_v2($r_apbd_v2->modul);
...
$r_apbd_v2->jenisForm = $jenisForm;
$r_apbd_v2->nomorUrut = $nomorUrut;
$r_apbd_v2->tahun = $tahun;
$r_apbd_v2->jenisAnggaran = $jenisAnggaran;
$r_apbd_v2->idTahap = $idTahap;
$r_apbd_v2->username = $_COOKIE['coID'];


$r_apbd_v2->wajibValidasi = $Main->wajibValidasi;
if($Main->wajibValidasi == TRUE){
	$r_apbd_v2->sqlValidasi = " and status_validasi ='1' ";
}else{
	$r_apbd_v2->sqlValidasi = " ";
}



if(empty($r_apbd_v2->tahun)){
	...
	/*$r_apbd_v2->tahun = "select max(id_anggaran) as max from view_r_apbd where nama_modul = 'apbd'";*/
	$r_apbd_v2->tahun  = $get2['tahun'];
	$r_apbd_v2->jenisAnggaran = $get2['jenis_anggaran'];
	$r_apbd_v2->urutTerakhir = $get2['no_urut'];
	$r_apbd_v2->jenisFormTerakhir = $get2['jenis_form_modul'];
	$r_apbd_v2->urutSebelumnya = $r_apbd_v2->urutTerakhir - 1;
	
	
	$idtahapTerakhir = $get2['id_tahap'];
	$r_apbd_v2->idTahap = $idtahapTerakhir;
	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$idtahapTerakhir'"));
	$r_apbd_v2->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$r_apbd_v2->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$r_apbd_v2->noUrutTerakhirapbd = $namaTahap['no_urut'];
	...
	$r_apbd_v2->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];
	
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$r_apbd_v2->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$r_apbd_v2->idTahap'"));
	$r_apbd_v2->currentTahap = $getCurrenttahap['nama_tahap'];
	
	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$r_apbd_v2->idTahap'"));
	$r_apbd_v2->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$r_apbd_v2->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$r_apbd_v2->noUrutTerakhirapbd = $namaTahap['no_urut'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
...
$r_apbd_v2->provinsi = $setting['provinsi'];
$r_apbd_v2->kota = $setting['kota'];
$r_apbd_v2->pengelolaBarang = $setting['pengelolaBarang'];
$r_apbd_v2->pejabatPengelolaBarang = $setting['pejabat'];
$r_apbd_v2->pengurusPengelolaBarang = $setting['pengurus'];
$r_apbd_v2->nipPengelola = $setting['nipPengelola'];
$r_apbd_v2->nipPengurus = $setting['nipPengurus'];
$r_apbd_v2->nipPejabat = $setting['nipPejabat'];
