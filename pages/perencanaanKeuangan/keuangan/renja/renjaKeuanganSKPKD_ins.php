<?php
class renjaKeuanganSKPKD_insObj  extends DaftarObj2{
	var $Prefix = 'renjaKeuanganSKPKD_ins';
	...
	var $FormName = 'renjaKeuanganSKPKD_insForm';
	var $noModul=14;
	...
	function setTitle(){
		...
		return 'RENCANA KERJA SKPD TAHUN '.$this->tahun ;
	}
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){
		...
		case 'saveNewPlafon':{
			...
			if(empty($belanjaLangsung)){
				$err = "Isi Plafon Belanja Langsung";
			}elseif(empty($belanjaTidakLangsung)){
				$err = "Isi Plafon Belanja Tidak Langsung";
			}else{
				$data = array(
								'tahun' => $this->tahun,
								...
								'c' => $c,
								'd' => $d,
								'belanja_langsung' => $belanjaLangsung,
								'belanja_tidak_langsung' => $belanjaTidakLangsung,
							);
				$query = VulnWalkerInsert('plafon_anggaran', $data);
				if(mysql_num_rows(mysql_query("select * from plafon_anggaran where tahun='$this->tahun' and jenis_anggaran='$this->jenisAnggaran' and c1='$c1' and c='$c' and d='$d'")) == 0){
					mysql_query($query);
					$getIdPlafon = mysql_fetch_array(mysql_query("select max(id) from plafon_anggaran where tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and c1='$c1' and c='$c' and d='$d'"));
					$content = array('idPlafon' => $getIdPlafon['max(id)']);
				}else{
		...
		case 'saveEditPlafon':{
			...
			if(empty($belanjaLangsung)){
				...
				$data = array(
					...
					$query = VulnWalkerUpdate('plafon_anggaran', $data,"id = '$id'");
					mysql_query($query);
					$content = array('idPlafon' => $id);
		...
		case 'editPlafon':{
			$fm = $this->editPlafon($_REQUEST['idPlafon']);
			$cek = $fm['cek'];
			$err = $fm['err'];
			$content = $fm['content'];
		break;
		...
		case 'jenisChanged':{
				...
				if($jenisKegiatan == 'baru'){
					...
					$plus = "<input type='text' name ='plus' id ='plus' style='width:200px; text-align:right;'  onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='document.getElementById(`keyPP`).textContent = `Rp. ` + renjaKeuanganSKPKD_ins.refreshPagu(this.value);' > ";
					$minus = "<input type='text' name ='minus' id ='minus' style='width:200px; text-align:right;'  onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='document.getElementById(`keyMM`).textContent = `Rp. ` + renjaKeuanganSKPKD_ins.refreshPagu(this.value);'> ";
				}
		...
		// if(!empty($cmbUnitForm) && empty($cmbSubUnitForm))$err = "Pilih Unit Pelaksana";
			if($err == ''){
						if($tahunJamak =='on'){
							$tahunJamak = '1';
						}else{
					...
					if(empty($cmbUnitForm)){
						...
						$angkaSekarang = $plus + $minus + $paguIndikatif;
								...
								$cekKegiatan = "select * from tabel_anggaran  where jenis_anggaran = '$this->jenisAnggaran' and tahun = '$this->tahun' and c1= '$cmbUrusanForm' and c='$cmbBidangForm' and d='$cmbSKPDForm' and e = '$cmbUnitForm' and e1 = '$cmbSubUnitForm' and bk ='$bk' and ck ='$ck' and dk ='$dk' and p = '$p' and q='$cmbKegiatan' and id_tahap = '$this->idTahap'";
   ...
   function setPage_OtherScript(){
		...
		return
			...
			"<script type='text/javascript' src='js/perencanaanKeuangan/keuangan/renja/renjaKeuanganSKPKD_ins.js' language='JavaScript' ></script>".
			"<script type='text/javascript' src='js/perencanaanKeuangan/aset/renja/popupProgramAset.js' language='JavaScript' ></script>
			".
	...
	$Syntax_ambil_yang_udah_ada_plafon = "select * from skpd_renja_v3 where id='$ID_PLAFON'";
	$ambilSKPD = mysql_fetch_array(mysql_query($Syntax_ambil_yang_udah_ada_plafon));
	$c1 = $ambilSKPD['c1'];
	$c  = $ambilSKPD['c'];
	$d  = $ambilSKPD['d'];
	$tanggalSekarang = str_replace("-","",date("Y-m-d"));
	$jenisTransaksi = $this->jenisAnggaran;

	$getPlafonAnggaran = mysql_fetch_array(mysql_query("select * from plafon_anggaran where c1='$c1' and c='$c' and d='$d'"));

	$plafon = $getPlafonAnggaran['belanja_langsung'];
	$idPlafon = $getPlafonAnggaran['id'];
	if($plafon == ''){
		$disablePlafon = "";
	}else{
	...
	if($ID_RENJA != '' && $ID_RENJA > 0){
		...
		$getNamaProgram = mysql_fetch_array(mysql_query("select * from ref_program where bk = '$selectedBK' and ck ='$selectedCK' and dk ='$selectedDK' and p='$selectedProgram' and q='0'"));
		$program = $getNamaProgram ['nama'];
		$getDetail = mysql_fetch_array(mysql_query("select * from detail_renja where id_anggaran = '$ID_RENJA'"));
		$dataOutput = $getDetail['output'];
		...
		$codeAndNameKegiatan = "select q, concat(q, '. ', nama) as vnama from ref_program where bk='$selectedBK' and ck = '$selectedCK' and dk = '$selectedDK' and p='$selectedProgram' ";//and q='$selectedKegiatan'
		...
		if($getDetail['jenis_kegiatan'] == 'lanjutan'){
			...
			$readOnlyJenisKegiatanPlus = "style='width:200px; text-align:right;' onkeypress='return event.charCode >= 48 &amp;&amp; event.charCode <= 57' onkeyup='document.getElementById(`keyPP`).textContent = `Rp. ` + renjaKeuanganSKPKD_ins.refreshPagu(this.value);'";
			$readOnlyJenisKegiatanMin = "style='width:200px; text-align:right;' onkeypress='return event.charCode >= 48 &amp;&amp; event.charCode <= 57' onkeyup='document.getElementById(`keyMM`).textContent = `Rp. ` + renjaKeuanganSKPKD_ins.refreshPagu(this.value);'";
...
$comboBoxJenisKegiatan = cmbArray('jenisKegiatan',$jenisKegiatan,$arrayComboJenisKegiatan,'-- JENIS KEGIATAN --','onchange = renjaKeuanganSKPKD_ins.jenisChanged();');
...
$comboBoxJenisKegiatan = cmbArray('jenisKegiatan',$selectedJenisKegiatan,$arrayComboJenisKegiatan,'','onchange = renjaKeuanganSKPKD_ins.jenisChanged();');
...
$waktuPelaksanaan = $mulai."&nbsp&nbsp s/d &nbsp&nbsp ".$selesai." &nbsp&nbsp ".$tahunJamak;
					...
					$this->isiform(
						array(
							...
							array(
								...
								'labelWidth'=>250,
								'value'=>"<input type='text' name='plafon' id='plafon' style='text-align:right; width :200px;' value='$plafon' readonly onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='document.getElementById(`keyPlafon`).textContent = `Rp. ` + popupProgramRenja.formatCurrency(this.value);'></div>&nbsp<input type='button' value='Baru' onclick=$this->Prefix.newPlafon(); >&nbsp <input type='button' value='Edit' id='editPlafon' onclick=$this->Prefix.editPlafon($idPlafon); >  <strong id='keyPlafon' style='color:red; width: 200px;'>Rp. ".number_format($plafon,2,',','.')."</strong>  "
							 ),
							 array(
							 ...
							 array(
								'label'=>'PROGRAM',
								'labelWidth'=>250,
								'value'=>"<input type='hidden' name='bk' id='bk' value='$selectedBK'><input type='hidden' name='ck' id='ck' value='$selectedCK'>
								<input type='hidden' name='dk' id='dk' value='$selectedDK'><input type='hidden' name='p' id='p' value='$selectedP'><input type='text' name='program' value='".$program."' style='width:600px;' id='program' readonly>&nbsp
								<input type='button' value='Cari' id='findProgram' onclick ='renjaKeuanganSKPKD_ins.CariProgramKegiatan()'  title='Cari Program dan Kegiatan' $disabledCariProgram >"
							  ),
							...
							array(
								...
								'labelWidth'=>250,
								'value'=>"<input type='text' name='paguIndikatif' id='paguIndikatif' value='$paguIndikatif' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='document.getElementById(`keyPagu`).textContent = `Rp. ` + renjaKeuanganSKPKD_ins.refreshPagu(this.value);'> <strong id='keyPagu' style='color:red; width: 200px;'>Rp.</strong> <input type='hidden' name='sisaPlafonDariDB' id='sisaPlafonDariDB' value='$sisaDariDB' > ",
								 ),
							 array(
							...
							array(
								'label'=>'OUTPUT',
								'labelWidth'=>150,
								'value'=>"<textarea name='output' id='output' style='width:400px;'>$dataOutput</textarea>"
							),
						)
					).
	...
	function rincianpenerimaanDET(){
		...
		$idplh = addslashes($_REQUEST['renjaKeuanganSKPKD_ins_idplh']);
		$ID_RENJA = $_REQUEST['ID_RENJA'];
	 ...
	 //items ----------------------
	  $this->form_fields = array(
			...
			'5' => array(
						...
						'labelWidth'=>200,
						'value'=> "<input type='text' name='belanjaLangsung' id='belanjaLangsung' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='document.getElementById(`belanjaLangsungSpan`).textContent = `Rp. ` + renjaKeuanganSKPKD_ins.formatCurrency(this.value);' > <span id='belanjaLangsungSpan'></span>" ,
						 ),
			'6' => array(
						'label'=>'BELANJA TIDAK LANGSUNG',
						'labelWidth'=>200,
						'value'=> "<input type='text' name='belanjaTidakLangsung' id='belanjaTidakLangsung' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='document.getElementById(`belanjaTidakLangsungSpan`).textContent = `Rp. ` + renjaKeuanganSKPKD_ins.formatCurrency(this.value);' > <span id='belanjaTidakLangsungSpan'></span>" ,
						 ),
		...
		$this->form_menubawah =
			...
			<input type='button' value='Simpan' onclick ='".$this->Prefix.".saveNewPlafon()' >&nbsp".
	...
	function editPlafon($id){
	 ...
	 $getPlafon = mysql_fetch_array(mysql_query("select * from plafon_anggaran where id='$id'"));
	 ...
	 //items ----------------------
	  $this->form_fields = array(
			...
			'5' => array(
						'label'=>'BELANJA LANGSUNG',
						'labelWidth'=>200,
						'value'=> "<input type='text' name='belanjaLangsung' value='".$getPlafon['belanja_langsung']."' id='belanjaLangsung' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='document.getElementById(`belanjaLangsungSpan`).textContent = `Rp. ` + renjaKeuanganSKPKD_ins.formatCurrency(this.value);' > <span id='belanjaLangsungSpan'>Rp. ".number_format($getPlafon['belanja_langsung'],2,',','.')."</span>" ,
						 ),
			'6' => array(
						'label'=>'BELANJA TIDAK LANGSUNG',
						'labelWidth'=>200,
						'value'=> "<input type='text' name='belanjaTidakLangsung' id='belanjaTidakLangsung' value='".$getPlafon['belanja_tidak_langsung']."' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='document.getElementById(`belanjaTidakLangsungSpan`).textContent = `Rp. ` + renjaKeuanganSKPKD_ins.formatCurrency(this.value);' > <span id='belanjaTidakLangsungSpan'>Rp. ".number_format($getPlafon['belanja_tidak_langsung'],2,',','.')."</span>" ,
						 ),
		...
		$this->form_menubawah =
			...
			<input type='button' value='Simpan' onclick ='".$this->Prefix.".saveEditPlafon($id)' >&nbsp".
			"<input type='button' value='Batal' onclick ='".$this->Prefix.".Close()' >";
...
$renjaKeuanganSKPKD_ins = new renjaKeuanganSKPKD_insObj();

$arrayResult = tahapPerencanaanV3($renjaKeuanganSKPKD_ins->modul,"MURNI");
...
$renjaKeuanganSKPKD_ins->jenisForm = $jenisForm;
$renjaKeuanganSKPKD_ins->nomorUrut = $nomorUrut;
$renjaKeuanganSKPKD_ins->tahun = $tahun;
$renjaKeuanganSKPKD_ins->jenisAnggaran = $jenisAnggaran;
$renjaKeuanganSKPKD_ins->idTahap = $idTahap;
$renjaKeuanganSKPKD_ins->username = $_COOKIE['coID'];
$renjaKeuanganSKPKD_ins->wajibValidasi = $Main->wajibValidasi;
