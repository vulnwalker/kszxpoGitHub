...
class rkaPPKD1KoreksiObj  extends DaftarObj2{	
	var $Prefix = 'rkaPPKD1Koreksi';
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='rkaPPKD1Koreksi.xls';
	var $namaModulCetak='RKA';
	var $Cetak_Judul = 'RKA';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'rkaPPKD1KoreksiForm';
	var $modul = "RKA-PPKD";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	...
	$getJumlahBarang = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$rkaPPKD1Koreksi_idplh'"));
	...
	$sisaPaguIndikatif = $paguIndikatif - $getPaguYangTelahTerpakai['paguYangTerpakai'];
	 ...
	 if(mysql_num_rows(mysql_query("select * from view_rka_ppkd_1 where c1='0' and f = '00' and k = '$k' and l ='$l' and m='$m' and n='$n' and o='$o'  and id_tahap='$this->idTahap' ")) > 0){
 	 ...
 	 if(empty($cmbJenisRKAForm) ){
		...
		$query = VulnWalkerUpdate('tabel_anggaran',$data,"id_anggaran = '$rkaPPKD1Koreksi_idplh'");
		mysql_query($query);
	 }
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){	
		...
		case 'Gruping':{	
			...
			if(!isset($rkaPPKD1Koreksi_cb)){
				$err = "Pilih Data";
			}
			$dt = implode(',',$rkaPPKD1Koreksi_cb);
		...
		case 'sesuai':{
			...
			$getrkaPPKD1Koreksinya = mysql_fetch_array(mysql_query($queryRows));
			foreach ($getrkaPPKD1Koreksinya as $key => $value) { 
				  $$key = $value; 
			} 
			$cmbUrusanForm = $c1;
			$cmbBidangForm = $c;
		...
		case 'editTab':{
			$id = $_REQUEST['rkaPPKD1Koreksi_cb'];
			foreach ($_REQUEST as $key => $value) { 
				  $$key = $value; 
			} 
	...
	<A href=\"pages.php?Pg=rkaPPKD1Koreksi\" title='RKA-PPKD MURNI' style='color:blue;' > RKA-PPKD 1 </a> |
   ...
   function setPage_OtherScript(){
		...
		return 	
			...
			<script type='text/javascript' src='js/perencanaanKeuangan/keuangan/rka/rkaPPKD1Koreksi.js' language='JavaScript' ></script> 
	 ...
	 $this->form_height = 400;
	  if ($this->form_fmST==0) {
		...
		foreach ($dt as $key => $value) { 
		 ...
		 $jenis_rka = $jenis_rka;
		 $cmbJenisRKA = cmbArray('cmbJenisRKAForm',$jenis_rka,$arrayJenisRKA,'-- JENIS RKA --','onchange=rkaPPKD1Koreksi.unlockFindRekening();');
	 	 if(empty($jenis_rka)){
	  ...
	  $this->form_fields = array(
			...
			'kode7' => array( 
						...
						'value'=> "<input type='text' id='kodeRekening' name='kodeRekening' style='width:120px;' readonly value = '$kodeRekening'> &nbsp&nbsp
						 ...
						 <button type='button' id='findRekening' onclick=rkaPPKD1Koreksi.findRekening('$jenis_rka'); $tergantungJenis> CARI </button> "
						 ),
			'kode8' => array( 
						'label'=>'HARGA SATUAN',
						'labelWidth'=>150, 
						'value'=> "<input type='text' name='hargaSatuan' id='hargaSatuan' value='$hargaSatuan' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='rkaPPKD1Koreksi.bantu();' > <span id='bantu' style='color:red;'> </span>"
						 ),	
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	foreach ($_COOKIE as $key => $value) { 
		...
		$urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=rkaPPKD1Koreksi.refreshList(true);','-- URUSAN --');
		
		$codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
		$bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=rkaPPKD1Koreksi.refreshList(true);','-- BIDANG --');
		
		$codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
		$skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=rkaPPKD1Koreksi.refreshList(true);','-- SKPD --');
		...
		$unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=rkaPPKD1Koreksi.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=rkaPPKD1Koreksi.refreshList(true);','-- SUB UNIT --');
...
$rkaPPKD1Koreksi = new rkaPPKD1KoreksiObj();
...
$rkaPPKD1Koreksi->jenisForm = $jenisForm;
$rkaPPKD1Koreksi->nomorUrut = $nomorUrut;
$rkaPPKD1Koreksi->urutTerakhir = $nomorUrut;
$rkaPPKD1Koreksi->tahun = $tahun;
$rkaPPKD1Koreksi->jenisAnggaran = $jenisAnggaran;
$rkaPPKD1Koreksi->idTahap = $idTahap;
$getNamaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$idTahap'"));
$rkaPPKD1Koreksi->namaTahap = $getNamaTahap['nama_tahap'];

$rkaPPKD1Koreksi->username = $_COOKIE['coID'];

$rkaPPKD1Koreksi->wajibValidasi = $Main->wajibValidasi;
if($Main->wajibValidasi == TRUE){
	$rkaPPKD1Koreksi->sqlValidasi = " and status_validasi ='1' ";
}else{
	$rkaPPKD1Koreksi->sqlValidasi = " ";
}

if(empty($rkaPPKD1Koreksi->tahun)){
	...
	/*$rkaPPKD1Koreksi->tahun = "select max(id_anggaran) as max from view_rka_ppkd_1 where nama_modul = 'rkaPPKD1Koreksi'";*/
	$rkaPPKD1Koreksi->tahun  = $get2['tahun'];
	$rkaPPKD1Koreksi->jenisAnggaran = $get2['jenis_anggaran'];
	$rkaPPKD1Koreksi->urutTerakhir = $get2['no_urut'];
	$rkaPPKD1Koreksi->jenisFormTerakhir = $get2['jenis_form_modul'];
	$rkaPPKD1Koreksi->urutSebelumnya = $rkaPPKD1Koreksi->urutTerakhir - 1;
	...
	$rkaPPKD1Koreksi->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$rkaPPKD1Koreksi->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];
	
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$rkaPPKD1Koreksi->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$rkaPPKD1Koreksi->idTahap'"));
	$rkaPPKD1Koreksi->currentTahap = $getCurrenttahap['nama_tahap'];
	
	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$rkaPPKD1Koreksi->idTahap'"));
	$rkaPPKD1Koreksi->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$rkaPPKD1Koreksi->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$rkaPPKD1Koreksi->provinsi = $setting['provinsi'];
$rkaPPKD1Koreksi->kota = $setting['kota'];
$rkaPPKD1Koreksi->pengelolaBarang = $setting['pengelolaBarang'];
$rkaPPKD1Koreksi->pejabatPengelolaBarang = $setting['pejabat'];
$rkaPPKD1Koreksi->pengurusPengelolaBarang = $setting['pengurus'];
$rkaPPKD1Koreksi->nipPengelola = $setting['nipPengelola'];
$rkaPPKD1Koreksi->nipPengurus = $setting['nipPengurus'];
$rkaPPKD1Koreksi->nipPejabat = $setting['nipPejabat'];

if($rkaPPKD1Koreksi->jenisForm != "KOREKSI"){
$get1 = mysql_fetch_array(mysql_query("select max(id_anggaran)  from view_rka_ppkd_1 where jenis_form_modul = 'KOREKSI' "));
	...
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$rkaPPKD1Koreksi->currentTahap = $arrayHasil['currentTahap'];
			$rkaPPKD1Koreksi->jenisForm = "";
			$rkaPPKD1Koreksi->jenisFormTerakhir = "KOREKSI";
			$rkaPPKD1Koreksi->idTahap = $get2['id_tahap'];
			$rkaPPKD1Koreksi->nomorUrut = $get2['no_urut'];
			$rkaPPKD1Koreksi->urutTerakhir = $get2['no_urut'];
