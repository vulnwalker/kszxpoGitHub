...
class rkaPPKD1LPObj  extends DaftarObj2{
	var $Prefix = 'rkaPPKD1LP';
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='rkaPPKD1LP.xls';
	var $namaModulCetak='RKA';
	var $Cetak_Judul = 'RKA';
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'rkaPPKD1LPForm';
	var $modul = "RKA-PPKD";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	...
	//get data -----------------
	 ...
	 $ilplh = $_REQUEST[$this->Prefix.'_ilplh'];
	...
	$getJumlahBarang = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$rkaPPKD1LP_ilplh'"));
	...
	$getPaguYangTelahTerpakai = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as paguYangTerpakai from view_rka_ppkd_1 where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1'  and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'  and no_urut = '$this->nomorUrut' and id_anggaran!='$rkaPPKD1LP_ilplh' "));
	$sisaPaguIndikatif = $paguIndikatif - $getPaguYangTelahTerpakai['paguYangTerpakai'];
	 ...
	 if(mysql_num_rows(mysql_query("select * from view_rka_ppkd_1 where c1='0' and f = '00' and k = '$k' and l ='$l' and m='$m' and n='$n' and o='$o'  and id_tahap='$this->idTahap' ")) > 0){
 	 ...
 	 if(empty($cmbJenisRKAForm) ){
		...
		$query = VulnWalkerUpdate('tabel_anggaran',$data,"id_anggaran = '$rkaPPKD1LP_ilplh'");
		mysql_query($query);
	 }
	...
	function set_selector_other($tipe){
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){
		...
		case 'Gruping':{
			...
			if(!isset($rkaPPKD1LP_cb)){
				$err = "Pilih Data";
			}
			$dt = implode(',',$rkaPPKD1LP_cb);
		...
		case 'clearTemp':{
				$username =$_COOKIE['coID'];
				mysql_query("delete from temp_rekening_rka_pendapatan_lp_ppkd where username ='$username'");
				mysql_query("delete from temp_rincian_pendapatan_lp_ppkd where username ='$username'");
				mysql_query("delete from temp_sub_rincian_pendapatan_lp_ppkd where username ='$username'");
				mysql_query("delete from temp_remove_rka_1_lp_ppkd where username ='$username'");
				foreach ($_REQUEST as $key => $value) {
			 	 $$key = $value;
				}
				...
				while ($rows = mysql_fetch_array($getALlData)) {
						...
						if(mysql_num_rows(mysql_query("select * from temp_rekening_rka_pendapatan_lp_ppkd where username = '$this->username' and k='$k'  and l ='$l' and m='$m' and n='$n' and o='$o'")) == 0){
								$dataRekening = array(
																				'k'=> $k,
																				'l'=> $l,
																				'm'=> $m,
								...
								$queryRekening = VulnWalkerInsert('temp_rekening_rka_pendapatan_lp_ppkd',$dataRekening);
								mysql_query($queryRekening);
						}
						...
						$getIdRekeningBelanja = mysql_fetch_array(mysql_query("select * from temp_rekening_rka_pendapatan_lp_ppkd where k ='$k' and l='$l' and m='$m' and n='$n' and o='$o' and username = '$this->username'"));
						if(mysql_num_rows(mysql_query("select * from temp_rincian_pendapatan_lp_ppkd where username = '$this->username' and id_rekening_belanja = '".$getIdRekeningBelanja['id']."' and uraian = '".$getUraianRincianBelanja['nama_rincian_belanja']."'")) == 0){
								...
								$queryRincianBelanja = VulnWalkerInsert('temp_rincian_pendapatan_lp_ppkd',$dataRincianBelanja);
						...
						$getIdRincianBelanja = mysql_fetch_array(mysql_query("select * from temp_rincian_pendapatan_lp_ppkd where username = '$this->username' and uraian = '".$getUplineRincianBelanja['nama_rincian_belanja']."'"));
						...
						$querySubRincian = VulnWalkerInsert("temp_sub_rincian_pendapatan_lp_ppkd",$dataSubRincian);
		...
		case 'sesuai':{
			...
			$getrkaPPKD1LPnya = mysql_fetch_array(mysql_query($queryRows));
			foreach ($getrkaPPKD1LPnya as $key => $value) {
				  $$key = $value;
			}
			$cmbUrusanForm = $c1;
			$cmbBidangForm = $c;
	    ...
	    case 'Validasi':{
				...
				$ilplh = $_REQUEST['id_anggaran'];
				$this->form_ilplh = $_REQUEST['id_anggaran'];


					$qry = "SELECT * FROM tabel_anggaran WHERE id_anggaran = '$ilplh' ";$cek=$qry;
 	...
 	<A href=\"pages.php?Pg=rkaPPKD1DP\" title='DANA PERIMBANGAN' > DANA PERIMBANGAN </a> |
 	<A href=\"pages.php?Pg=rkaPPKD1LP\" title='LAIN LAIN PENDAPATAN' style='color:blue;' > LAIN LAIN PENDAPATAN </a>
   ...
   function setPage_OtherScript(){
		...
		return
			...
			<script type='text/javascript' src='js/perencanaanKeuangan/keuangan/rka/rkaPPKD1LP.js' language='JavaScript' ></script>
	...
	function setFormBaru(){
  	...
  	function setFormEdit(){
		$cek ='';
		$cbid = $_REQUEST[$this->Prefix.'_cb'];
		$this->form_ilplh = $cbid[0];
		$kode = explode(' ',$this->form_ilplh);
		$this->form_fmST = 1;
		if($err == ''){
			$aqry = "SELECT * FROM  tabel_anggaran WHERE id_anggaran='".$this->form_ilplh."' "; $cek.=$aqry;
			$dt = mysql_fetch_array(mysql_query($aqry));
			$fm = $this->setForm($dt);
		}
	 ...
	 $this->form_height = 400;
	  if ($this->form_fmST==0) {
		...
		foreach ($dt as $key => $value) {
		 ...
		 $jenis_rka = $jenis_rka;
		 $cmbJenisRKA = cmbArray('cmbJenisRKAForm',$jenis_rka,$arrayJenisRKA,'-- JENIS RKA --','onchange=rkaPPKD1LP.unlockFindRekening();');
	 	 if(empty($jenis_rka)){
	  ...
	  $this->form_fields = array(
			...
			'kode7' => array(
						...
						'value'=> "<input type='text' id='kodeRekening' name='kodeRekening' style='width:120px;' readonly value = '$kodeRekening'> &nbsp&nbsp
						 ...
						 <button type='button' id='findRekening' onclick=rkaPPKD1LP.findRekening('$jenis_rka'); $tergantungJenis> CARI </button> "
						 ),
			'kode8' => array(
						'label'=>'HARGA SATUAN',
						'labelWidth'=>150,
						'value'=> "<input type='text' name='hargaSatuan' id='hargaSatuan' value='$hargaSatuan' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='rkaPPKD1LP.bantu();' > <span id='bantu' style='color:red;'> </span>"
						 ),
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	foreach ($_COOKIE as $key => $value) {
		...
		$urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=rkaPPKD1LP.refreshList(true);','-- URUSAN --');

		$codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
		$bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=rkaPPKD1LP.refreshList(true);','-- BIDANG --');

		$codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
		$skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=rkaPPKD1LP.refreshList(true);','-- SKPD --');
		...
		$unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=rkaPPKD1LP.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=rkaPPKD1LP.refreshList(true);','-- SUB UNIT --');
	...
	function pageShow(){
		...
		"<html>".
			...
			"<table id='KerangkaHal' class='menubar' cellspacing='0' celllpding='0' border='0'  height='100%' >".
				"<tr height='34'><td>".
					$this->setPage_Header().
					"<div id='header' ></div>".
				"</td></tr>".
...
$rkaPPKD1LP->jenisForm = $jenisForm;
$rkaPPKD1LP->nomorUrut = $nomorUrut;
$rkaPPKD1LP->urutTerakhir = $nomorUrut;
$rkaPPKD1LP->tahun = $tahun;
$rkaPPKD1LP->jenisAnggaran = $jenisAnggaran;
$rkaPPKD1LP->idTahap = $idTahap;
$rkaPPKD1LP->username = $_COOKIE['coID'];

$rkaPPKD1LP->wajibValidasi = $Main->wajibValidasi;
if($Main->wajibValidasi == TRUE){
	$rkaPPKD1LP->sqlValidasi = " and status_validasi ='1' ";
}else{
	$rkaPPKD1LP->sqlValidasi = " ";
}

if(empty($rkaPPKD1LP->tahun)){
	...
	$rkaPPKD1LP->tahun  = $get2['tahun'];
	$rkaPPKD1LP->jenisAnggaran = $get2['jenis_anggaran'];
	$rkaPPKD1LP->urutTerakhir = $get2['no_urut'];
	$rkaPPKD1LP->jenisFormTerakhir = $get2['jenis_form_modul'];
	$rkaPPKD1LP->urutSebelumnya = $rkaPPKD1LP->urutTerakhir - 1;
	...
	$rkaPPKD1LP->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$rkaPPKD1LP->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];

	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$rkaPPKD1LP->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$rkaPPKD1LP->idTahap'"));
	$rkaPPKD1LP->currentTahap = $getCurrenttahap['nama_tahap'];

	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$rkaPPKD1LP->idTahap'"));
	$rkaPPKD1LP->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$rkaPPKD1LP->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$rkaPPKD1LP->provinsi = $setting['provinsi'];
$rkaPPKD1LP->kota = $setting['kota'];
$rkaPPKD1LP->pengelolaBarang = $setting['pengelolaBarang'];
$rkaPPKD1LP->pejabatPengelolaBarang = $setting['pejabat'];
$rkaPPKD1LP->pengurusPengelolaBarang = $setting['pengurus'];
$rkaPPKD1LP->nipPengelola = $setting['nipPengelola'];
$rkaPPKD1LP->nipPengurus = $setting['nipPengurus'];
$rkaPPKD1LP->nipPejabat = $setting['nipPejabat'];


if($rkaPPKD1LP->jenisForm != "PENYUSUNAN"){
$get1 = mysql_fetch_array(mysql_query("select max(id_anggaran)  from view_rka_ppkd_1 where jenis_form_modul = 'PENYUSUNAN' "));
	...
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$rkaPPKD1LP->currentTahap = $arrayHasil['currentTahap'];
			$rkaPPKD1LP->jenisForm = "";
			$rkaPPKD1LP->jenisFormTerakhir = "PENYUSUNAN";
			$rkaPPKD1LP->idTahap = $get2['id_tahap'];
			$rkaPPKD1LP->tahun = $get2['tahun'];
			$rkaPPKD1LP->nomorUrut = $get2['no_urut'];
			$rkaPPKD1LP->urutTerakhir = $get2['no_urut'];
}
