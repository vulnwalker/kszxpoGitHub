...
class rkaSKPD22KoreksiObj  extends DaftarObj2{	
	var $Prefix = 'rkaSKPD22Koreksi';
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='rkaSKPD22Koreksi.xls';
	var $namaModulCetak='RKA';
	var $Cetak_Judul = 'RKA';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'rkaSKPD22KoreksiForm';
	var $modul = "RKA-SKPD";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	...
	$getJumlahBarang = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$rkaSKPD22Koreksi_idplh'"));
	...
	$sisaPaguIndikatif = $paguIndikatif - $getPaguYangTelahTerpakai['paguYangTerpakai'];
	 ...
	 if(mysql_num_rows(mysql_query("select * from view_rka_2_2_1 where c1='0' and j = '000' and k = '$k' and l ='$l' and m='$m' and n='$n' and o='$o'  and id_tahap='$this->idTahap' ")) > 0){
 	 ...
 	 if(empty($cmbJenisRKAForm) ){
		...
		$query = VulnWalkerUpdate('tabel_anggaran',$data,"id_anggaran = '$rkaSKPD22Koreksi_idplh'");
		mysql_query($query);
	 }
	...
	<A href=\"pages.php?Pg=rkaSKPD22Koreksi\" title='RKA-SKPKD MURNI' style='color:blue;' > RKA-SKPD 2.2 </a> |
   ...
   function setPage_OtherScript(){
		...
		return 	
			...
			<script type='text/javascript' src='js/perencanaanKeuangan/keuangan/rka/rkaSKPD22Koreksi.js' language='JavaScript' ></script> 
	 ...
	 $this->form_height = 400;
	  if ($this->form_fmST==0) {
		...
		foreach ($dt as $key => $value) { 
		 ...
		 $jenis_rka = $jenis_rka;
		 $cmbJenisRKA = cmbArray('cmbJenisRKAForm',$jenis_rka,$arrayJenisRKA,'-- JENIS RKA --','onchange=rkaSKPD22Koreksi.unlockFindRekening();');
	 	 if(empty($jenis_rka)){
	  ...
	  $this->form_fields = array(
			...
			'kode7' => array( 
						...
						'value'=> "<input type='text' id='kodeRekening' name='kodeRekening' style='width:120px;' readonly value = '$kodeRekening'> &nbsp&nbsp
						 ...
						 <button type='button' id='findRekening' onclick=rkaSKPD22Koreksi.findRekening('$jenis_rka'); $tergantungJenis> CARI </button> "
						 ),
			'kode8' => array( 
						'label'=>'HARGA SATUAN',
						'labelWidth'=>150, 
						'value'=> "<input type='text' name='hargaSatuan' id='hargaSatuan' value='$hargaSatuan' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='rkaSKPD22Koreksi.bantu();' > <span id='bantu' style='color:red;'> </span>"
						 ),	
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	if(!isset($selectedC1) ){
		...
		$urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=rkaSKPD22Koreksi.refreshList(true);','-- URUSAN --');
		
		$codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
		$bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=rkaSKPD22Koreksi.refreshList(true);','-- BIDANG --');
		
		$codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
		$skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=rkaSKPD22Koreksi.refreshList(true);','-- SKPD --');
		...
		$unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=rkaSKPD22Koreksi.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=rkaSKPD22Koreksi.refreshList(true);','-- SUB UNIT --');
...
$rkaSKPD22Koreksi = new rkaSKPD22KoreksiObj();
...
$rkaSKPD22Koreksi->jenisForm = $jenisForm;
$rkaSKPD22Koreksi->nomorUrut = $nomorUrut;
$rkaSKPD22Koreksi->tahun = $tahun;
$rkaSKPD22Koreksi->jenisAnggaran = $jenisAnggaran;
$rkaSKPD22Koreksi->idTahap = $idTahap;
$rkaSKPD22Koreksi->username = $_COOKIE['coID'];
$getNamaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$idTahap'"));
$rkaSKPD22Koreksi->namaTahap = $getNamaTahap['nama_tahap'];

$rkaSKPD22Koreksi->wajibValidasi = $Main->wajibValidasi;
if($Main->wajibValidasi == TRUE){
	$rkaSKPD22Koreksi->sqlValidasi = " and status_validasi ='1' ";
}else{
	$rkaSKPD22Koreksi->sqlValidasi = " ";
}


if(empty($rkaSKPD22Koreksi->tahun)){
	...
	/*$rkaSKPD22Koreksi->tahun = "select max(id_anggaran) as max from view_rka_2_2_1 where nama_modul = 'rkaSKPD22Koreksi'";*/
	$rkaSKPD22Koreksi->tahun  = $get2['tahun'];
	$rkaSKPD22Koreksi->jenisAnggaran = $get2['jenis_anggaran'];
	$rkaSKPD22Koreksi->urutTerakhir = $get2['no_urut'];
	$rkaSKPD22Koreksi->jenisFormTerakhir = $get2['jenis_form_modul'];
	$rkaSKPD22Koreksi->urutSebelumnya = $rkaSKPD22Koreksi->urutTerakhir - 1;
	...
	$rkaSKPD22Koreksi->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$rkaSKPD22Koreksi->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];
	
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$rkaSKPD22Koreksi->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$rkaSKPD22Koreksi->idTahap'"));
	$rkaSKPD22Koreksi->currentTahap = $getCurrenttahap['nama_tahap'];
	
	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$rkaSKPD22Koreksi->idTahap'"));
	$rkaSKPD22Koreksi->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$rkaSKPD22Koreksi->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$rkaSKPD22Koreksi->provinsi = $setting['provinsi'];
$rkaSKPD22Koreksi->kota = $setting['kota'];
$rkaSKPD22Koreksi->pengelolaBarang = $setting['pengelolaBarang'];
$rkaSKPD22Koreksi->pejabatPengelolaBarang = $setting['pejabat'];
$rkaSKPD22Koreksi->pengurusPengelolaBarang = $setting['pengurus'];
$rkaSKPD22Koreksi->nipPengelola = $setting['nipPengelola'];
$rkaSKPD22Koreksi->nipPengurus = $setting['nipPengurus'];
$rkaSKPD22Koreksi->nipPejabat = $setting['nipPejabat'];

if($rkaSKPD22Koreksi->jenisForm != "KOREKSI"){
	...
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$rkaSKPD22Koreksi->currentTahap = $arrayHasil['currentTahap'];
			$rkaSKPD22Koreksi->jenisForm = "";
			$rkaSKPD22Koreksi->jenisFormTerakhir = "KOREKSI";
			$rkaSKPD22Koreksi->idTahap = $get2['id_tahap'];
			$rkaSKPD22Koreksi->nomorUrut = $get2['no_urut'];
			$rkaSKPD22Koreksi->urutTerakhir = $get2['no_urut'];
