...
class rkaSKPD221NewObj  extends DaftarObj2{
	var $Prefix = 'rkaSKPD221New';
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='rkaSKPD221New.xls';
	var $namaModulCetak='RKA';
	var $Cetak_Judul = 'RKA';
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'rkaSKPD221NewForm';
	var $modul = "RKA-SKPD";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	...
	var $nipPejabat = "";
	var $nipPengurus ="";
	var $reportURL = "pages.php?Pg=rkaSKPD221&tipe=Laporan";
   ...
   // if( $err=='' && $kategori =='' ) $err= 'Kategori Belum Di Pilih !!';
    ...
    case 'pilihPangkat':{
      ...
      break;
		...
		case 'editSubRincian':{
				...
				$eventKeyup = 'document.getElementById("formatjumlah'.$id.'").innerHTML = rkaSKPD221New.formatCurrency(this.value); ';
		...
		// 			mysql_query($query);
		// 		}
		// 		$content = array('urusan' => $cmbUrusan, 'bidang' => $cmbBidang, 'skpd' => $cmbSKPD, 'namaPemda' => $namaPemda[option_value], 'cetakjang' => $cetakjang, 'e' => $cmbUnit, 'e1' => $cmbSubUnit, 'bk' => $bk, 'ck' => $ck, 'dk' => $dk, 'p' => $hiddenP, 'q' => $q );
		// 	}
		// break;
		// }
		case 'Report':{
    ...
    break;
		...
		case 'Gruping':{
			...
			if(!isset($rkaSKPD221New_cb)){
				$err = "Pilih Data";
			}
			$dt = implode(',',$rkaSKPD221New_cb);
		...
		case 'sesuai':{
			...
			$getrkaSKPD221Newnya = mysql_fetch_array(mysql_query($queryRows));
			foreach ($getrkaSKPD221Newnya as $key => $value) {
				  $$key = $value;
			}
			$cmbUrusanForm = $c1;
			$cmbBidangForm = $c;
	    ...
	    case 'SaveValid':{
	    	foreach ($_REQUEST as $key => $value) {
			 ...
			 $query = VulnWalkerUpdate("tabel_anggaran",$data," id_anggaran = '$rkaSKPD221New_idplh'");
			 mysql_query($query);
	...
	<A href=\"pages.php?Pg=rkaSKPD221New\" title='BELANJA' style='color:blue;' > BELANJA </a> |
	...
	<tr><td class=\"menudottedline\" width=\"40%\" height=\"20\" style='text-align:right'><B>
	<A href=\"pages.php?Pg=rkaSKPD221New\" title='BELANJA LANGSUNG' style='color:blue;' > LANGSUNG </a> |
	<A href=\"pages.php?Pg=rkaSKPD21\" title='BELANJA TIDAK LANGSUNG' > TIDAK LANGSUNG </a>

	&nbsp&nbsp&nbsp
	</td></tr>
	<tr><td class=\"menudottedline\" width=\"40%\" height=\"20\" style='text-align:right'><B>
	<A href=\"pages.php?Pg=rkaSKPD221New\" title='INPUT' style='color:blue;' > INPUT </a> |
...
/*		 $arrayResult = VulnWalkerTahap_v2('RKA');
	...
	// if($this->jenisForm == 'PENYUSUNAN'){
			...
			//  $Koloms[] = array(' align="left"', "<span style='color:$warnaMapping;margin-left:$marginKode;' id='actionSpan$id_anggaran'>$kode</span>" );
			 ...
			 $Koloms[] = array(' align="right"',  "<span style='font-weight:$boldStatus;' id='actionSpan$id_anggaran'>$jumlahHarga $this->lockedEditor</span>" );
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	foreach ($_COOKIE as $key => $value) {
		...
		$urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=rkaSKPD221New.refreshList(true);','-- URUSAN --');

		$codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
		$bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=rkaSKPD221New.refreshList(true);','-- BIDANG --');

		$codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
		$skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=rkaSKPD221New.refreshList(true);','-- SKPD --');
		...
		$unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=rkaSKPD221New.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=rkaSKPD221New.refreshList(true);','-- SUB UNIT --');
	...
	$program = "<input type='hidden' id='bk' name='bk' value='$selectedBK'> <input type='hidden' id='ck' name='ck' value='$selectedCK'> <input type='hidden' id='dk' name='dk' value='$selectedDK'> <input type='hidden' id='hiddenP' name='hiddenP' value='$selectedP'>".cmbArray('p',$_REQUEST['p'],$arrayP,'-- PROGRAM --','onchange=rkaSKPD221New.programChanged();');
	...
	$kegiatan = cmbArray('q',$_REQUEST['q'],$arrayQ,'-- KEGIATAN --','onchange=rkaSKPD221New.refreshList(true);');
	...
	$cmbBelanja = cmbArray('cmbBelanja',$_REQUEST['cmbBelanja'],$arrayBelanja,'-- JENIS BELANJA--','onchange=rkaSKPD221New.refreshList(true);');
	...
	function getDaftarOpsi($Mode=1){
	   ...
	   if(isset($cmbUrusan) && $cmbUrusan== ''){
		...
		$this->total = $getTotalHarga['sum(jumlah_harga)'];
   ...
   $this->form_width = 300;
   $this->form_height = 100;
   $this->form_caption = 'LAPORAN RKA-SKPD 2.2.1 MURNI';
   ...
   $q = $dt['q'];
     ...
     $arrayJenisLaporan = array(
                 array('RKASKPD221', 'RKA-SKPD 2.2.1 MURNI'),
    ...
    $this->form_menubawah =
      ...
      "<input type='button' value='Batal' onclick ='".'rkaSKPD221'.".Close()' >";
...
function RKASKPD221($xls =FALSE){
		...
		if ($getJenisUkuran == 'L') {
			...
			$cetak = "<link rel='stylesheet' type='text/css' href='css/pageNumber.css'>";
			$width = "33cm";
			$height = "21.5cm";
		}else{
		...
		$getNamaKegiatan = mysql_fetch_array(mysql_query("select * from ref_program where bk='$bk' and ck='$ck' and dk='$dk' and p='$hiddenP' and q='0'"));
		$program = $getNamaKegiatan['nama'];
    ...
    $kota = mysql_fetch_array(mysql_query("SELECT * from t_pengaturan"));
  		...
  		<script type='text/javascript' src='assets/js/bootstrap.min.js'></script>".
			...
			"<body >
				<div style='width:$this->Cetak_WIDTH_Landscape;'>
					<table class=\"rangkacetak\" style='width: $width; font-family: sans-serif; height: $height; margin-left: 1cm; border: 1px solid #000; border-bottom-color: transparent; display: block;'>
						<tr>
				...
				<table class='subjudulcetak' style='width: 100.1%; font-family: sans-serif; margin-bottom: -1%; margin-top: -1%; border: 1px solid; margin-left: -1px;'>
					<tr>
					...
					<tr>
						<td width='11%' valign='top' ><span style='margin-left: 5%;'>Program</span></td>
						$pisah
					...
					<tr>
						<td width='11%' valign='top' ><span style='margin-left: 5%;'>Kegiatan</span></td>
						$pisah
						<td width='10%' valign='top'> ".$cmbUrusan.".".$cmbBidang.".".$cmbSKPD.".".genNumber($hublaBK).genNumber($hublaCK).genNumber($hublaP).".".genNumber($hublaQ)." </td>
						<td valign='top'>$kegiatan</td>
					</tr>

					<tr style='border: 1px solid black;'>
						<td width='11%' valign='top' ><span style='margin-left: 5%;'>Lokasi Kegiatan</span></td>
						$pisah
						<td valign='top' colspan='2'>".$lokasiKegiatan."</td>
					</tr>

					<tr>
						<td width='11%' valign='top' ><span style='margin-left: 5%;'>JUMLAH TAHUN n - 1</span></td>
						$pisah
						<td valign='top' colspan='2'> Rp. ".number_format($getDetailRenja['min'],2,',','.')."</td>
					</tr>

					<tr>
						<td width='11%' valign='top' ><span style='margin-left: 5%;'>JUMLAH TAHUN n</span></td>
						$pisah
				...
				<table table width='100%' class='cetak' border='1' style='width:100.1%; margin-left: -1px; margin-bottom: -0.7%;'>
					<tr>
						<th class='th01' width = '200' style='font-size: 12px;'>INDIKATOR</th>
						<th class='th01' style='font-size: 12px;'>TOLAK UKUR KINERJA</th>
						<th class='th01' style='font-size: 12px;'>TARGET KINERJA</th>
					</tr>
					<tr>
						<td  valign='top' class = 'GarisCetak' style='font-weight: bold; padding-left: 1%; border: 1px solid #000;'>CAPAIAN PROGRAM</td>
						<td  valign='top' class = 'GarisCetak' style='border: 1px solid #000;'>".$isiQueryDetailRenja['capaian_program_tuk']."</td>
						<td  valign='top' class = 'GarisCetak' style='border: 1px solid #000;'>".$isiQueryDetailRenja['capaian_program_tk']."</td>
					</tr>
					<tr>
						<td  valign='top' class = 'GarisCetak' style='font-weight: bold; padding-left: 1%; border: 1px solid #000;'>MASUKAN</td>
						<td  valign='top' class = 'GarisCetak' style='border: 1px solid #000;'>".$isiQueryDetailRenja['masuk_tuk']."</td>
						<td  valign='top' class = 'GarisCetak' style='border: 1px solid #000;'>".$isiQueryDetailRenja['masuk_tk']."</td>
					</tr>
					<tr>
						<td  valign='top' class = 'GarisCetak' style='font-weight: bold; padding-left: 1%; border: 1px solid #000;'>KELUARAN</td>
						<td  valign='top' class = 'GarisCetak' style='border: 1px solid #000;'>".$isiQueryDetailRenja['keluaran_tuk']."</td>
						<td  valign='top' class = 'GarisCetak' style='border: 1px solid #000;'>".$isiQueryDetailRenja['keluaran_tk']."</td>
					</tr>
					<tr>
						<td valign='top' class = 'GarisCetak' style='font-weight: bold; padding-left: 1%; border: 1px solid #000;'>HASIL</td>
						<td  valign='top' class = 'GarisCetak' style='border: 1px solid #000;'>".$isiQueryDetailRenja['hasil_tuk']."</td>
						<td  valign='top' class = 'GarisCetak' style='border: 1px solid #000;'>".$isiQueryDetailRenja['hasil_tk']."</td>
					</tr>
				...
				<table style='margin-left: 1%; width: 17%;'>
									<tr>
										<td valign='top' style='font-weight: bold; font-size: 12px;'>KELOMPOK SASARAN KEGIATAN</td>
										<td width='1%' valign='top'> : </td>
										<td valign='top' colspan='2'>".$isiQueryDetailRenja['kelompok_sasaran_kegiatan']."</td>
									</tr>
								</table>
				";
		...
		echo "
								...
								<table class='cetak' border='1' style='margin:2 0 0 0;width:100.1%; margin-left: -1px;'>
								<thead>
									<tr>
										<th style='text-align: center; border: 1px solid black; padding: 1.5px;' colspan='10'>
										<span style='font-size:14px;font-weight:bold;text-decoration: '>
									...
									<tr>
										...
										<th class='th01' rowspan='2' >JUMLAH<br> (Rp)</th>
										
									</tr>
									<tr>
										<th class='th01' style='width: 4%;'>VOLUME</th>
              ...
              if (in_array($concatRekeningLevel4, $arrayRekening4)) {
				...
				$naonkitu = "
								<tr valign='top'>
									...
									<td align='right' class='GarisCetak5' >".$volumeRekening."</td>
									<td align='left' class='GarisCetak5'>$satuan_rek</td>
									<td align='right' class='GarisCetak' >".$hargaSatuan."</td>
									<td align='right' class='GarisCetak' >".$jumlahHarga."</td>
								</tr>
				";
			}
    ...
    $arrayRekening4[] = $k.".".$l.".".$m.".".$n;
		...
		echo "
			<tr valign='top'>
				<td align='right' colspan='5' class='GarisCetak' style='border: 1px solid #000;'><b>Jumlah</b></td>
				<td align='right' class='GarisCetak' style='border: 1px solid #000;'><b>".$total."</b></td>
			</tr>
		";
		...
		echo 			
						"<table style='width:100.1%; margin-left: -1px; border: 1px solid #000;'>
							<tr>
						...
						<table class='cetak' border='1' style='width:100.1%; margin-left: -1px; margin-top: -0.1%;'>
								<thead>
									...
									<tr>
										<th class='th02' style='width: 2%;'>No.</th>
										<th class='th02' >NAMA</th>
										<th class='th02' >NIP</th>
										<th class='th02' >JABATAN</th>
										<th class='th02' style='width: 10%;'>TANDA TANGAN</th>
									</tr>
									</thead>
									<tbody>
											".$isiTimAnggaran."
	 ...
	 //items ----------------------
	  $this->form_fields = array(
			...
			array( 		'label' => '',
						'labelWidth' => 1,
						'pemisah' => ' ',
						'value'=>" VOLUME &nbsp &nbsp<input type='text' value='$totalResult' placeholder='JUMLAH' id='jumlah4' readonly >&nbsp SATUAN VOL &nbsp <input type='text' name ='satuanVolume' id ='satuanVolume' value='$satuanVolume' readonly> &nbsp &nbsp <span id='detailVolumeRincian'>",
						 ),
			 array( 		'label' => '',
 						'labelWidth' => 1,
 						'pemisah' => ' ',
...
$rkaSKPD221New = new rkaSKPD221NewObj();
...
$rkaSKPD221New->jenisForm = $jenisForm;
$rkaSKPD221New->nomorUrut = $nomorUrut;
$rkaSKPD221New->urutTerakhir = $nomorUrut;
$rkaSKPD221New->tahun = $tahun;
$rkaSKPD221New->jenisAnggaran = $jenisAnggaran;
$rkaSKPD221New->idTahap = $idTahap;

$rkaSKPD221New->username = $_COOKIE['coID'];

$rkaSKPD221New->wajibValidasi = $Main->wajibValidasi;
if($Main->wajibValidasi == TRUE){
	$rkaSKPD221New->sqlValidasi = " and status_validasi ='1' ";
}else{
	$rkaSKPD221New->sqlValidasi = " ";
}


if(empty($rkaSKPD221New->tahun)){
	...
	/*$rkaSKPD221New->tahun = "select max(id_anggaran) as max from view_rka_2_2_1 where nama_modul = 'rkaSKPD221New'";*/
	$rkaSKPD221New->tahun  = $get2['tahun'];
	$rkaSKPD221New->jenisAnggaran = $get2['jenis_anggaran'];
	$rkaSKPD221New->urutTerakhir = $get2['no_urut'];
	$rkaSKPD221New->jenisFormTerakhir = $get2['jenis_form_modul'];
	$rkaSKPD221New->urutSebelumnya = $rkaSKPD221New->urutTerakhir - 1;
	...
	$rkaSKPD221New->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$rkaSKPD221New->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];

	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$rkaSKPD221New->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$rkaSKPD221New->idTahap'"));
	$rkaSKPD221New->currentTahap = $getCurrenttahap['nama_tahap'];

	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$rkaSKPD221New->idTahap'"));
	$rkaSKPD221New->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$rkaSKPD221New->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$rkaSKPD221New->provinsi = $setting['provinsi'];
$rkaSKPD221New->kota = $setting['kota'];
$rkaSKPD221New->pengelolaBarang = $setting['pengelolaBarang'];
$rkaSKPD221New->pejabatPengelolaBarang = $setting['pejabat'];
$rkaSKPD221New->pengurusPengelolaBarang = $setting['pengurus'];
$rkaSKPD221New->nipPengelola = $setting['nipPengelola'];
$rkaSKPD221New->nipPengurus = $setting['nipPengurus'];
$rkaSKPD221New->nipPejabat = $setting['nipPejabat'];


if($rkaSKPD221New->jenisForm != "PENYUSUNAN"){
$get1 = mysql_fetch_array(mysql_query("select max(id_anggaran)  from view_rka_2_2_1 where jenis_form_modul = 'PENYUSUNAN' "));
	...
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$rkaSKPD221New->currentTahap = $arrayHasil['currentTahap'];
			$rkaSKPD221New->jenisForm = "";
			$rkaSKPD221New->jenisFormTerakhir = "PENYUSUNAN";
			$rkaSKPD221New->idTahap = $get2['id_tahap'];
			$rkaSKPD221New->nomorUrut = $get2['no_urut'];
			$rkaSKPD221New->urutTerakhir = $get2['no_urut'];
			$rkaSKPD221New->idTahap = $idtahapTerakhir;
			$rkaSKPD221New->tahun = $get2['tahun'];
}
