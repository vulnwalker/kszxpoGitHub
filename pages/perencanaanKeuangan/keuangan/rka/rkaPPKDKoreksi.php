...
class rkaPPKDKoreksiObj  extends DaftarObj2{	
	var $Prefix = 'rkaPPKDKoreksi';
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='rkaPPKDKoreksi.xls';
	var $namaModulCetak='RKA';
	var $Cetak_Judul = 'RKA';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'rkaPPKDKoreksiForm';
	var $modul = "RKA-PPKD";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	...
	$getJumlahBarang = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$rkaPPKDKoreksi_idplh'"));
	...
	$sisaPaguIndikatif = $paguIndikatif - $getPaguYangTelahTerpakai['paguYangTerpakai'];
	 ...
	 if(mysql_num_rows(mysql_query("select * from view_rka_ppkd where c1='0' and f1 = '0' and k = '$k' and l ='$l' and m='$m' and n='$n' and o='$o'  and id_tahap='$this->idTahap' ")) > 0){
 	 ...
 	 if(empty($cmbJenisRKAForm) ){
		...
		$query = VulnWalkerUpdate('tabel_anggaran',$data,"id_anggaran = '$rkaPPKDKoreksi_idplh'");
		mysql_query($query);
	 }
	...
	<A href=\"pages.php?Pg=rkaPPKDKoreksi\" title='RKA-PPKD MURNI' style='color:blue;' > RKA-PPKD </a> 
   ...
   function setPage_OtherScript(){
		...
		return 	
			...
			<script type='text/javascript' src='js/perencanaanKeuangan/keuangan/rka/rkaPPKDKoreksi.js' language='JavaScript' ></script> 
	 ...
	 $this->form_height = 400;
	  if ($this->form_fmST==0) {
		...
		foreach ($dt as $key => $value) { 
		 ...
		 $jenis_rka = $jenis_rka;
		 $cmbJenisRKA = cmbArray('cmbJenisRKAForm',$jenis_rka,$arrayJenisRKA,'-- JENIS RKA --','onchange=rkaPPKDKoreksi.unlockFindRekening();');
	 	 if(empty($jenis_rka)){
	  ...
	  $this->form_fields = array(
			...
			'kode7' => array( 
						...
						'value'=> "<input type='text' id='kodeRekening' name='kodeRekening' style='width:120px;' readonly value = '$kodeRekening'> &nbsp&nbsp
						 ...
						 <button type='button' id='findRekening' onclick=rkaPPKDKoreksi.findRekening('$jenis_rka'); $tergantungJenis> CARI </button> "
						 ),
			'kode8' => array( 
						'label'=>'HARGA SATUAN',
						'labelWidth'=>150, 
						'value'=> "<input type='text' name='hargaSatuan' id='hargaSatuan' value='$hargaSatuan' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='rkaPPKDKoreksi.bantu();' > <span id='bantu' style='color:red;'> </span>"
						 ),	
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	foreach ($_COOKIE as $key => $value) { 
		...
		$urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=rkaPPKDKoreksi.refreshList(true);','-- URUSAN --');
		
		$codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
		$bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=rkaPPKDKoreksi.refreshList(true);','-- BIDANG --');
		
		$codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
		$skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=rkaPPKDKoreksi.refreshList(true);','-- SKPD --');
		...
		$unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=rkaPPKDKoreksi.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=rkaPPKDKoreksi.refreshList(true);','-- SUB UNIT --');
...
$rkaPPKDKoreksi = new rkaPPKDKoreksiObj();
...
$rkaPPKDKoreksi->jenisForm = $jenisForm;
$rkaPPKDKoreksi->nomorUrut = $nomorUrut;
$rkaPPKDKoreksi->tahun = $tahun;
$rkaPPKDKoreksi->jenisAnggaran = $jenisAnggaran;
$rkaPPKDKoreksi->idTahap = $idTahap;
$rkaPPKDKoreksi->username = $_COOKIE['coID'];
$getNamaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '".$idTahap."'"));
			$rkaPPKDKoreksi->namaTahap = $getNamaTahap['nama_tahap'];


if(empty($rkaPPKDKoreksi->tahun)){
	...
	/*$rkaPPKDKoreksi->tahun = "select max(id_anggaran) as max from view_rka_ppkd where nama_modul = 'rkaPPKDKoreksi'";*/
	$rkaPPKDKoreksi->tahun  = $get2['tahun'];
	$rkaPPKDKoreksi->jenisAnggaran = $get2['jenis_anggaran'];
	$rkaPPKDKoreksi->urutTerakhir = $get2['no_urut'];
	$rkaPPKDKoreksi->jenisFormTerakhir = $get2['jenis_form_modul'];
	$rkaPPKDKoreksi->urutSebelumnya = $rkaPPKDKoreksi->urutTerakhir - 1;
	...
	$rkaPPKDKoreksi->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$rkaPPKDKoreksi->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];
	
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$rkaPPKDKoreksi->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$rkaPPKDKoreksi->idTahap'"));
	$rkaPPKDKoreksi->currentTahap = $getCurrenttahap['nama_tahap'];
	
	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$rkaPPKDKoreksi->idTahap'"));
	$rkaPPKDKoreksi->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$rkaPPKDKoreksi->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$rkaPPKDKoreksi->provinsi = $setting['provinsi'];
$rkaPPKDKoreksi->kota = $setting['kota'];
$rkaPPKDKoreksi->pengelolaBarang = $setting['pengelolaBarang'];
$rkaPPKDKoreksi->pejabatPengelolaBarang = $setting['pejabat'];
$rkaPPKDKoreksi->pengurusPengelolaBarang = $setting['pengurus'];
$rkaPPKDKoreksi->nipPengelola = $setting['nipPengelola'];
$rkaPPKDKoreksi->nipPengurus = $setting['nipPengurus'];
$rkaPPKDKoreksi->nipPejabat = $setting['nipPejabat'];


if($rkaPPKDKoreksi->jenisForm != "KOREKSI"){
$get1 = mysql_fetch_array(mysql_query("select max(id_anggaran)  from view_rka_ppkd_1 where jenis_form_modul = 'KOREKSI' "));
	...
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$rkaPPKDKoreksi->currentTahap = $arrayHasil['currentTahap'];
			$rkaPPKDKoreksi->jenisForm = "";
			$rkaPPKDKoreksi->jenisFormTerakhir = "KOREKSI";
			$rkaPPKDKoreksi->idTahap = $get2['id_tahap'];
			$rkaPPKDKoreksi->nomorUrut = $get2['no_urut'];
			$rkaPPKDKoreksi->urutTerakhir = $get2['no_urut'];
