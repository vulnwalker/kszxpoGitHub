...
class rkaSKPD1LPObj  extends DaftarObj2{
	var $Prefix = 'rkaSKPD1LP';
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='rkaSKPD1LP.xls';
	var $namaModulCetak='RKA';
	var $Cetak_Judul = 'RKA';
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'rkaSKPD1LPForm';
	var $modul = "RKA-SKPD";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	...
	function setTitle(){
		return 'RKA-SKPD 1 LP '.$this->jenisAnggaran.' TAHUN '.$this->tahun;
	}
	...
	$getJumlahBarang = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$rkaSKPD1LP_idplh'"));
	...
	$getPaguYangTelahTerpakai = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as paguYangTerpakai from view_rka_1 where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1'  and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'  and no_urut = '$this->nomorUrut' and id_anggaran!='$rkaSKPD1LP_idplh' "));
	$sisaPaguIndikatif = $paguIndikatif - $getPaguYangTelahTerpakai['paguYangTerpakai'];
	 ...
	 if(mysql_num_rows(mysql_query("select * from view_rka_1 where c1='0' and f = '00' and k = '$k' and l ='$l' and m='$m' and n='$n' and o='$o'  and id_tahap='$this->idTahap' ")) > 0){
 	 ...
 	 if(empty($cmbJenisRKAForm) ){
		...
		$query = VulnWalkerUpdate('tabel_anggaran',$data,"id_anggaran = '$rkaSKPD1LP_idplh'");
		mysql_query($query);
	 }
	...
	function set_selector_other($tipe){
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){
			...
			case 'editSubRincian':{
					...
					$eventKeyup = 'document.getElementById("formatjumlah'.$id.'").innerHTML = rkaSKPD1LP.formatCurrency(this.value); ';
		...
		case 'Gruping':{
			...
			if(!isset($rkaSKPD1LP_cb)){
				$err = "Pilih Data";
			}
			$dt = implode(',',$rkaSKPD1LP_cb);
		...
		case 'clearTemp':{
				$username =$_COOKIE['coID'];
				mysql_query("delete from temp_rekening_rka_pendapatan_lp where username ='$username'");
				mysql_query("delete from temp_rincian_pendapatan_lp where username ='$username'");
				mysql_query("delete from temp_sub_rincian_pendapatan_lp where username ='$username'");
				mysql_query("delete from temp_remove_rka_1_lp where username ='$username'");
				foreach ($_REQUEST as $key => $value) {
			 	 $$key = $value;
				}
				...
				while ($rows = mysql_fetch_array($getALlData)) {
						...
						if(mysql_num_rows(mysql_query("select * from temp_rekening_rka_pendapatan_lp where username = '$this->username' and k='$k'  and l ='$l' and m='$m' and n='$n' and o='$o'")) == 0){
								$dataRekening = array(
																				'k'=> $k,
																				'l'=> $l,
																				'm'=> $m,
								...
								$queryRekening = VulnWalkerInsert('temp_rekening_rka_pendapatan_lp',$dataRekening);
								mysql_query($queryRekening);
						}
						...
						$getIdRekeningBelanja = mysql_fetch_array(mysql_query("select * from temp_rekening_rka_pendapatan_lp where k ='$k' and l='$l' and m='$m' and n='$n' and o='$o' and username = '$this->username'"));
						if(mysql_num_rows(mysql_query("select * from temp_rincian_pendapatan_lp where username = '$this->username' and id_rekening_belanja = '".$getIdRekeningBelanja['id']."' and uraian = '".$getUraianRincianBelanja['nama_rincian_belanja']."'")) == 0){
								...
								$queryRincianBelanja = VulnWalkerInsert('temp_rincian_pendapatan_lp',$dataRincianBelanja);
						...
						$getIdRincianBelanja = mysql_fetch_array(mysql_query("select * from temp_rincian_pendapatan_lp where username = '$this->username' and uraian = '".$getUplineRincianBelanja['nama_rincian_belanja']."'"));
						...
						$querySubRincian = VulnWalkerInsert("temp_sub_rincian_pendapatan_lp",$dataSubRincian);
		...
		case 'sesuai':{
			...
			$getrkaSKPD1LPnya = mysql_fetch_array(mysql_query($queryRows));
			foreach ($getrkaSKPD1LPnya as $key => $value) {
				  $$key = $value;
			}
			$cmbUrusanForm = $c1;
			$cmbBidangForm = $c;
	 ...
	 return
	 "<table width=\"100%\" class=\"menubar\" celllpding=\"0\" cellspacing=\"0\" border=\"0\" style='margin:0 0 0 0'>
	 <tr><td class=\"menudottedline\" width=\"40%\" height=\"20\" style='text-align:right'><B>
	 <A href=\"pages.php?Pg=rkaSKPD221\" title='BELANJA'  > BELANJA </a> |
	 <A href=\"pages.php?Pg=rkaSKPD1LP\" title='PENDAPATAN'  style='color:blue;'> PENDAPATAN </a> |
	 ...
	 <A href=\"pages.php?Pg=rkaSKPD1LP\" title='LAIN LAIN PENDAPATAN' style='color:blue;' > LAIN LAIN PENDAPATAN </a>
   ...
   function setPage_OtherScript(){
		...
		return
			...
			<script type='text/javascript' src='js/perencanaanKeuangan/keuangan/rka/rkaSKPD1LP.js' language='JavaScript' ></script>
	 ...
	 $this->form_height = 400;
	  if ($this->form_fmST==0) {
		...
		foreach ($dt as $key => $value) {
		 ...
		 $namaRekening = $getNamaRekening['nm_rekening'];
		 $arrayJenisRKA = array(
						array("1 LP","RKA-SKPD 1 LP"),
						array("1 LP","RKA-SKPD 1 LP")

						);
		 $jenis_rka = $jenis_rka;
		 $cmbJenisRKA = cmbArray('cmbJenisRKAForm',$jenis_rka,$arrayJenisRKA,'-- JENIS RKA --','onchange=rkaSKPD1LP.unlockFindRekening();');
	 	 if(empty($jenis_rka)){
	  ...
	  $this->form_fields = array(
			...
			'kode7' => array(
						...
						'value'=> "<input type='text' id='kodeRekening' name='kodeRekening' style='width:120px;' readonly value = '$kodeRekening'> &nbsp&nbsp
						 ...
						 <button type='button' id='findRekening' onclick=rkaSKPD1LP.findRekening('$jenis_rka'); $tergantungJenis> CARI </button> "
						 ),
			'kode8' => array(
						'label'=>'HARGA SATUAN',
						'labelWidth'=>150,
						'value'=> "<input type='text' name='hargaSatuan' id='hargaSatuan' value='$hargaSatuan' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='rkaSKPD1LP.bantu();' > <span id='bantu' style='color:red;'> </span>"
						 ),
	...
	function Validasi($dt){
	 ...
	 $this->form_caption = 'VALIDASI RKA-SKPD 1 LP';
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	foreach ($_COOKIE as $key => $value) {
		...
		$urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=rkaSKPD1LP.refreshList(true);','-- URUSAN --');

		$codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
		$bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=rkaSKPD1LP.refreshList(true);','-- BIDANG --');

		$codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
		$skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=rkaSKPD1LP.refreshList(true);','-- SKPD --');
		...
		$unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=rkaSKPD1LP.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=rkaSKPD1LP.refreshList(true);','-- SUB UNIT --');
...
$rkaSKPD1LP->jenisForm = $jenisForm;
$rkaSKPD1LP->nomorUrut = $nomorUrut;
$rkaSKPD1LP->urutTerakhir = $nomorUrut;
$rkaSKPD1LP->tahun = $tahun;
$rkaSKPD1LP->jenisAnggaran = $jenisAnggaran;
$rkaSKPD1LP->idTahap = $idTahap;
$rkaSKPD1LP->username = $_COOKIE['coID'];

$rkaSKPD1LP->wajibValidasi = $Main->wajibValidasi;
if($Main->wajibValidasi == TRUE){
	$rkaSKPD1LP->sqlValidasi = " and status_validasi ='1' ";
}else{
	$rkaSKPD1LP->sqlValidasi = " ";
}

if(empty($rkaSKPD1LP->tahun)){
	...
	$rkaSKPD1LP->tahun  = $get2['tahun'];
	$rkaSKPD1LP->jenisAnggaran = $get2['jenis_anggaran'];
	$rkaSKPD1LP->urutTerakhir = $get2['no_urut'];
	$rkaSKPD1LP->jenisFormTerakhir = $get2['jenis_form_modul'];
	$rkaSKPD1LP->urutSebelumnya = $rkaSKPD1LP->urutTerakhir - 1;
	...
	$rkaSKPD1LP->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$rkaSKPD1LP->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];

	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$rkaSKPD1LP->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$rkaSKPD1LP->idTahap'"));
	$rkaSKPD1LP->currentTahap = $getCurrenttahap['nama_tahap'];

	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$rkaSKPD1LP->idTahap'"));
	$rkaSKPD1LP->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$rkaSKPD1LP->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$rkaSKPD1LP->provinsi = $setting['provinsi'];
$rkaSKPD1LP->kota = $setting['kota'];
$rkaSKPD1LP->pengelolaBarang = $setting['pengelolaBarang'];
$rkaSKPD1LP->pejabatPengelolaBarang = $setting['pejabat'];
$rkaSKPD1LP->pengurusPengelolaBarang = $setting['pengurus'];
$rkaSKPD1LP->nipPengelola = $setting['nipPengelola'];
$rkaSKPD1LP->nipPengurus = $setting['nipPengurus'];
$rkaSKPD1LP->nipPejabat = $setting['nipPejabat'];


if($rkaSKPD1LP->jenisForm != "PENYUSUNAN"){
$get1 = mysql_fetch_array(mysql_query("select max(id_anggaran)  from view_rka_1 where jenis_form_modul = 'PENYUSUNAN' "));
	...
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$rkaSKPD1LP->currentTahap = $arrayHasil['currentTahap'];
			$rkaSKPD1LP->jenisForm = "";
			$rkaSKPD1LP->jenisFormTerakhir = "PENYUSUNAN";
			$rkaSKPD1LP->idTahap = $get2['id_tahap'];
			$rkaSKPD1LP->tahun = $get2['tahun'];
			$rkaSKPD1LP->nomorUrut = $get2['no_urut'];
			$rkaSKPD1LP->urutTerakhir = $get2['no_urut'];
}
