...
class rkaPPKD_v2_koreksiObj  extends DaftarObj2{	
	var $Prefix = 'rkaPPKD_v2_koreksi';
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='rkaPPKD_v2_koreksi.xls';
	var $namaModulCetak='RKA';
	var $Cetak_Judul = 'RKA';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'rkaPPKD_v2_koreksiForm';
	var $modul = "RKA-PPKD";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	...
	function setTitle(){
		return 'RKA-PPKD '.$this->jenisAnggaran.' TAHUN '.$this->tahun."&nbsp ".  strtoupper($this->namaTahap);
	}
	...
	function genRowSum($ColStyle, $Mode, $Total){
		...
		if(!empty($this->jenisForm)){
			...
			$getIdTahapRKATerakhir = mysql_fetch_array(mysql_query("select max(id_tahap) as max from tabel_anggaran where tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and jenis_rka !=''  and (rincian_perhitungan !='' or j !='000' ) and nama_modul = 'RKA-PPKD' "));
		 	$idTahap = $getIdTahapRKATerakhir['max'];
		}

		$getData = mysql_fetch_array(mysql_query("select sum(jumlah_harga) from tabel_anggaran where  (rincian_perhitungan !='' or j !='000' ) and id_tahap='$idTahap' and tahun ='$this->tahun' and nama_modul='RKA-PPKD' and jenis_anggaran = '$this->jenisAnggaran' $kondisiSKPD $kondisiRekening"));
	...
	$getJumlahBarang = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$rkaPPKD_v2_koreksi_idplh'"));
	...
	$getPaguYangTelahTerpakai = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as paguYangTerpakai from view_rka_ppkd where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and bk='$bk' and ck='$ck' and p='$p' and q='$q' and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'  and no_urut = '$this->nomorUrut' and id_anggaran!='$rkaPPKD_v2_koreksi_idplh' "));
	$sisaPaguIndikatif = $paguIndikatif - $getPaguYangTelahTerpakai['paguYangTerpakai'];
    
	 if(mysql_num_rows(mysql_query("select * from view_rka_ppkd where c1='0' and f1 = '0' and k = '$k' and l ='$l' and m='$m' and n='$n' and o='$o'  and id_tahap='$this->idTahap' ")) > 0){
 	 ...
 	 if(empty($cmbJenisRKAForm) ){
		...
		$query = VulnWalkerUpdate('tabel_anggaran',$data,"id_anggaran = '$rkaPPKD_v2_koreksi_idplh'");
		mysql_query($query);
	 }
	...
	<A href=\"pages.php?Pg=rka-ppkd_v2_koreksi\" title='RKA-PPKD MURNI'  style='color:blue;' > RKA-PPKD </a> 
   ...
   function setPage_OtherScript(){
		...
		return 	
			...
			<script type='text/javascript' src='js/perencanaan_v2/rka/rkaPPKD_koreksi.js' language='JavaScript' ></script> 
	 ...
	 $this->form_height = 400;
	  if ($this->form_fmST==0) {
		...
		foreach ($dt as $key => $value) { 
		 ...
		 $jenis_rka = $jenis_rka;
		 $cmbJenisRKA = cmbArray('cmbJenisRKAForm',$jenis_rka,$arrayJenisRKA,'-- JENIS RKA --','onchange=rkaPPKD_v2_koreksi.unlockFindRekening();');
	 	 if(empty($jenis_rka)){
	  ...
	  $this->form_fields = array(
			...
			'kode7' => array( 
						...
						'value'=> "<input type='text' id='kodeRekening' name='kodeRekening' style='width:120px;' readonly value = '$kodeRekening'> &nbsp&nbsp
						 ...
						 <button type='button' id='findRekening' onclick=rkaPPKD_v2_koreksi.findRekening('$jenis_rka'); $tergantungJenis> CARI </button> "
						 ),
			'kode8' => array( 
						'label'=>'HARGA SATUAN',
						'labelWidth'=>150, 
						'value'=> "<input type='text' name='hargaSatuan' id='hargaSatuan' value='$hargaSatuan' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='rkaPPKD_v2_koreksi.bantu();' > <span id='bantu' style='color:red;'> </span>"
						 ),	
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	foreach ($_COOKIE as $key => $value) { 
		...
		$urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=rkaPPKD_v2_koreksi.refreshList(true);','-- URUSAN --');
		
		$codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
		$bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=rkaPPKD_v2_koreksi.refreshList(true);','-- BIDANG --');
		
		$codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
		$skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=rkaPPKD_v2_koreksi.refreshList(true);','-- SKPD --');
		...
		$unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=rkaPPKD_v2_koreksi.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=rkaPPKD_v2_koreksi.refreshList(true);','-- SUB UNIT --');
	...
	if($this->jenisForm == "KOREKSI" || $this->jenisForm == "PENYUSUNAN" || $this->jenisForm == "VALIDASI"){
		...
		$getPaguYangTerpakai =  mysql_fetch_array(mysql_query("select sum(jumlah_harga) as paguYangTerpakai from view_rka_ppkd where c1= '$selectedC1' and c='$selectedC' and d='$selectedD' and e='$selectedE' and e1='$selectedE1' and bk='$selectedBK' and ck='$selectedCK' and p='$selectedP' and q='$selectedQ' and id_tahap = '$this->idTahap'  "));
	...
	function getDaftarOpsi($Mode=1){
	  ...
	  if(isset($cmbUrusan) && $cmbUrusan== ''){
		...
		if(!empty($this->idTahap)){
			...
			$getAllRekeningFromRka = mysql_query("select * from tabel_anggaran where jenis_rka !='' and jenis_rka !='RKA-PPKD'  and c1 ='0' and f1='0' and k !='0' and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and nama_modul ='RKA-PPKD' ");	
			while($rows = mysql_fetch_array($getAllRekeningFromRka)){
				...
				if(strlen($k) > 1){
					...
					$data = array(
					  ...
					  if(mysql_num_rows(mysql_query("select * from view_rka_ppkd where k='$k' and l='$l' and m='$m' and n='$n' and o ='$o'")) == 0){
		 ...
		 $grabAll = mysql_query("select * from view_rka_ppkd where tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'");
...
$rkaPPKD_v2_koreksi = new rkaPPKD_v2_koreksiObj();
...
$rkaPPKD_v2_koreksi->jenisForm = $jenisForm;
$rkaPPKD_v2_koreksi->nomorUrut = $nomorUrut;
$rkaPPKD_v2_koreksi->tahun = $tahun;
$rkaPPKD_v2_koreksi->jenisAnggaran = $jenisAnggaran;
$rkaPPKD_v2_koreksi->idTahap = $idTahap;
$rkaPPKD_v2_koreksi->username = $_COOKIE['coID'];
$getNamaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '".$idTahap."'"));
			$rkaPPKD_v2_koreksi->namaTahap = $getNamaTahap['nama_tahap'];


if(empty($rkaPPKD_v2_koreksi->tahun)){
    
	$get1 = mysql_fetch_array(mysql_query("select max(id_anggaran)  from view_rka_ppkd "));
	$maxAnggaran = $get1['max(id_anggaran)'];
	$get2 = mysql_fetch_array(mysql_query("select * from view_rka_ppkd where id_anggaran = '$maxAnggaran'"));
	/*$rkaPPKD_v2_koreksi->tahun = "select max(id_anggaran) as max from view_rka_ppkd where nama_modul = 'rkaPPKD_v2_koreksi'";*/
	$rkaPPKD_v2_koreksi->tahun  = $get2['tahun'];
	$rkaPPKD_v2_koreksi->jenisAnggaran = $get2['jenis_anggaran'];
	$rkaPPKD_v2_koreksi->urutTerakhir = $get2['no_urut'];
	$rkaPPKD_v2_koreksi->jenisFormTerakhir = $get2['jenis_form_modul'];
	$rkaPPKD_v2_koreksi->urutSebelumnya = $rkaPPKD_v2_koreksi->urutTerakhir - 1;
	...
	$rkaPPKD_v2_koreksi->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$rkaPPKD_v2_koreksi->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];
	
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$rkaPPKD_v2_koreksi->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$rkaPPKD_v2_koreksi->idTahap'"));
	$rkaPPKD_v2_koreksi->currentTahap = $getCurrenttahap['nama_tahap'];
	
	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$rkaPPKD_v2_koreksi->idTahap'"));
	$rkaPPKD_v2_koreksi->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$rkaPPKD_v2_koreksi->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$rkaPPKD_v2_koreksi->provinsi = $setting['provinsi'];
$rkaPPKD_v2_koreksi->kota = $setting['kota'];
$rkaPPKD_v2_koreksi->pengelolaBarang = $setting['pengelolaBarang'];
$rkaPPKD_v2_koreksi->pejabatPengelolaBarang = $setting['pejabat'];
$rkaPPKD_v2_koreksi->pengurusPengelolaBarang = $setting['pengurus'];
$rkaPPKD_v2_koreksi->nipPengelola = $setting['nipPengelola'];
$rkaPPKD_v2_koreksi->nipPengurus = $setting['nipPengurus'];
$rkaPPKD_v2_koreksi->nipPejabat = $setting['nipPejabat'];


if($rkaPPKD_v2_koreksi->jenisForm != "KOREKSI"){
$get1 = mysql_fetch_array(mysql_query("select max(id_anggaran)  from view_rka_ppkd_1 where jenis_form_modul = 'KOREKSI' "));
	...
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$rkaPPKD_v2_koreksi->currentTahap = $arrayHasil['currentTahap'];
			$rkaPPKD_v2_koreksi->jenisForm = "";
			$rkaPPKD_v2_koreksi->jenisFormTerakhir = "KOREKSI";
			$rkaPPKD_v2_koreksi->idTahap = $get2['id_tahap'];
			$rkaPPKD_v2_koreksi->nomorUrut = $get2['no_urut'];
			$rkaPPKD_v2_koreksi->urutTerakhir = $get2['no_urut'];
