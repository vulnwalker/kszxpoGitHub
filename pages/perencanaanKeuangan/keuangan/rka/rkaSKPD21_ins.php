<?php
class rkaSKPD21_insObj  extends DaftarObj2{
	var $Prefix = 'rkaSKPD21_ins';
	...
	var $FormName = 'rkaSKPD21_insForm';
	var $noModul=14;
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){
		...
		case 'finish':{
			...
			$getRekeningKosong = mysql_query("select * from temp_rekening_rka_belanja_tidak_langsung where username = '$this->username'");
			while($dataRekening = mysql_fetch_array($getRekeningKosong)){
					$dataRincianBelanjaKosong = mysql_query("select * from temp_rincian_belanja_tidak_langsung where id_rekening_belanja = '".$dataRekening['id']."'");
					...
					while ($dataRincianBelanja = mysql_fetch_array($dataRincianBelanjaKosong)) {
							if(mysql_num_rows(mysql_query("select * from temp_sub_rincian_belanja_tidak_langsung where username = '$this->username' and id_rincian_belanja ='".$dataRincianBelanja['id']."'")) == 0){
			...
			if(empty($err)){
				if(mysql_num_rows(mysql_query("select * from temp_sub_rincian_belanja_tidak_langsung where username = '$this->username' and id_anggaran !=''")) == 0){
						$execute = mysql_query("select * from temp_sub_rincian_belanja_tidak_langsung  where username ='$username' ");
   ...
   function setPage_OtherScript(){
		...
		return
			...
			"<script type='text/javascript' src='js/perencanaanKeuangan/keuangan/rka/rkaSKPD21_ins.js' language='JavaScript' ></script>".
	...
	$vOrder=
			...
			,'','','').
				...
				genFilterBar(
					...
					<tbody><tr valign='middle' align='center'>
					<td class='border:none'>
						<a class='toolbar' id='btsave' href='javascript:rkaSKPD21_ins.finish()'>
					...
					<tbody><tr valign='middle' align='center'>
					<td class='border:none'>
						<a class='toolbar' id='btcancel' href='javascript:rkaSKPD21_ins.closeTab()'>
...
$rkaSKPD21_ins = new rkaSKPD21_insObj();
...
$rkaSKPD21_ins->jenisForm = $jenisForm;
$rkaSKPD21_ins->nomorUrut = $nomorUrut;
$rkaSKPD21_ins->tahun = $tahun;
$rkaSKPD21_ins->jenisAnggaran = $jenisAnggaran;
$rkaSKPD21_ins->idTahap = $idTahap;
$rkaSKPD21_ins->username = $_COOKIE['coID'];
