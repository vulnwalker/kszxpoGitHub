...
class rkaPPKD32KoreksiObj  extends DaftarObj2{	
	var $Prefix = 'rkaPPKD32Koreksi';
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='rkaPPKD32Koreksi.xls';
	var $namaModulCetak='RKA';
	var $Cetak_Judul = 'RKA';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'rkaPPKD32KoreksiForm';
	var $modul = "RKA-PPKD";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	...
	$getJumlahBarang = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$rkaPPKD32Koreksi_idplh'"));
	...
	$sisaPaguIndikatif = $paguIndikatif - $getPaguYangTelahTerpakai['paguYangTerpakai'];
	 ...
	 if(mysql_num_rows(mysql_query("select * from view_rka_ppkd_3_2 where c1='0' and f = '00' and k = '$k' and l ='$l' and m='$m' and n='$n' and o='$o'  and id_tahap='$this->idTahap' ")) > 0){
 	 ...
 	 if(empty($cmbJenisRKAForm) ){
		...
		$query = VulnWalkerUpdate('tabel_anggaran',$data,"id_anggaran = '$rkaPPKD32Koreksi_idplh'");
		mysql_query($query);
	 }
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){
		...
		case 'Gruping':{	
			...
			if(!isset($rkaPPKD32Koreksi_cb)){
				$err = "Pilih Data";
			}
			$dt = implode(',',$rkaPPKD32Koreksi_cb);
		...
		case 'sesuai':{
			...
			$getrkaPPKD32Koreksinya = mysql_fetch_array(mysql_query($queryRows));
			foreach ($getrkaPPKD32Koreksinya as $key => $value) { 
				  $$key = $value; 
			} 
			$cmbUrusanForm = $c1;
			$cmbBidangForm = $c;
		...
		case 'editTab':{
			$id = $_REQUEST['rkaPPKD32Koreksi_cb'];
			foreach ($_REQUEST as $key => $value) { 
				  $$key = $value; 
			} 
	...
	<A href=\"pages.php?Pg=rkaPPKD1Koreksi\" title='RKA-PPKD MURNI'  > RKA-PPKD 1 </a> |
	<A href=\"pages.php?Pg=rkaPPKD21Koreksi\" title='RKA-PPKD MURNI'  > RKA-PPKD 2.1 </a> |
	<A href=\"pages.php?Pg=rkaPPKD31Koreksi\" title='RKA-PPKD MURNI'   > RKA-PPKD 3.1 </a> |
	<A href=\"pages.php?Pg=rkaPPKD32Koreksi\" title='RKA-PPKD MURNI' style='color:blue;'> RKA-PPKD 3.2 </a> |
	<A href=\"pages.php?Pg=rkaPPKDKoreksi\" title='RKA-PPKD MURNI'  > RKA-PPKD </a> 
   ...
   function setPage_OtherScript(){
		...
		return 	
			...
			<script type='text/javascript' src='js/perencanaanKeuangan/keuangan/rka/rkaPPKD32Koreksi.js' language='JavaScript' ></script> 
	 ...
	 $this->form_height = 400;
	  if ($this->form_fmST==0) {
		...
		foreach ($dt as $key => $value) { 
		 ...
		 $jenis_rka = $jenis_rka;
		 $cmbJenisRKA = cmbArray('cmbJenisRKAForm',$jenis_rka,$arrayJenisRKA,'-- JENIS RKA --','onchange=rkaPPKD32Koreksi.unlockFindRekening();');
	 	 if(empty($jenis_rka)){
	  ...
	  $this->form_fields = array(
			...
			'kode7' => array( 
						...
						'value'=> "<input type='text' id='kodeRekening' name='kodeRekening' style='width:120px;' readonly value = '$kodeRekening'> &nbsp&nbsp
						 ...
						 <button type='button' id='findRekening' onclick=rkaPPKD32Koreksi.findRekening('$jenis_rka'); $tergantungJenis> CARI </button> "
						 ),
			'kode8' => array( 
						'label'=>'HARGA SATUAN',
						'labelWidth'=>150, 
						'value'=> "<input type='text' name='hargaSatuan' id='hargaSatuan' value='$hargaSatuan' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='rkaPPKD32Koreksi.bantu();' > <span id='bantu' style='color:red;'> </span>"
						 ),	
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	foreach ($_COOKIE as $key => $value) { 
		...
		$urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=rkaPPKD32Koreksi.refreshList(true);','-- URUSAN --');
		
		$codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
		$bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=rkaPPKD32Koreksi.refreshList(true);','-- BIDANG --');
		
		$codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
		$skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=rkaPPKD32Koreksi.refreshList(true);','-- SKPD --');
		...
		$unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=rkaPPKD32Koreksi.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=rkaPPKD32Koreksi.refreshList(true);','-- SUB UNIT --');
...
$rkaPPKD32Koreksi = new rkaPPKD32KoreksiObj();
...
$rkaPPKD32Koreksi->jenisForm = $jenisForm;
$rkaPPKD32Koreksi->nomorUrut = $nomorUrut;
$rkaPPKD32Koreksi->urutTerakhir = $nomorUrut;
$rkaPPKD32Koreksi->tahun = $tahun;
$rkaPPKD32Koreksi->jenisAnggaran = $jenisAnggaran;
$rkaPPKD32Koreksi->idTahap = $idTahap;
$getNamaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$idTahap'"));
$rkaPPKD32Koreksi->namaTahap = $getNamaTahap['nama_tahap'];

$rkaPPKD32Koreksi->wajibValidasi = $Main->wajibValidasi;
if($Main->wajibValidasi == TRUE){
	$rkaPPKD32Koreksi->sqlValidasi = " and status_validasi ='1' ";
}else{
	$rkaPPKD32Koreksi->sqlValidasi = " ";
}

if(empty($rkaPPKD32Koreksi->tahun)){
	...
	/*$rkaPPKD32Koreksi->tahun = "select max(id_anggaran) as max from view_rka_ppkd_3_2 where nama_modul = 'rkaPPKD32Koreksi'";*/
	$rkaPPKD32Koreksi->tahun  = $get2['tahun'];
	$rkaPPKD32Koreksi->jenisAnggaran = $get2['jenis_anggaran'];
	$rkaPPKD32Koreksi->urutTerakhir = $get2['no_urut'];
	$rkaPPKD32Koreksi->jenisFormTerakhir = $get2['jenis_form_modul'];
	$rkaPPKD32Koreksi->urutSebelumnya = $rkaPPKD32Koreksi->urutTerakhir - 1;
	...
	$rkaPPKD32Koreksi->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$rkaPPKD32Koreksi->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];
	
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$rkaPPKD32Koreksi->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$rkaPPKD32Koreksi->idTahap'"));
	$rkaPPKD32Koreksi->currentTahap = $getCurrenttahap['nama_tahap'];
	
	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$rkaPPKD32Koreksi->idTahap'"));
	$rkaPPKD32Koreksi->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$rkaPPKD32Koreksi->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$rkaPPKD32Koreksi->provinsi = $setting['provinsi'];
$rkaPPKD32Koreksi->kota = $setting['kota'];
$rkaPPKD32Koreksi->pengelolaBarang = $setting['pengelolaBarang'];
$rkaPPKD32Koreksi->pejabatPengelolaBarang = $setting['pejabat'];
$rkaPPKD32Koreksi->pengurusPengelolaBarang = $setting['pengurus'];
$rkaPPKD32Koreksi->nipPengelola = $setting['nipPengelola'];
$rkaPPKD32Koreksi->nipPengurus = $setting['nipPengurus'];
$rkaPPKD32Koreksi->nipPejabat = $setting['nipPejabat'];
$rkaPPKD32Koreksi->username = $_COOKIE['coID'];

if($rkaPPKD32Koreksi->jenisForm != "KOREKSI"){
$get1 = mysql_fetch_array(mysql_query("select max(id_anggaran)  from view_rka_ppkd_3_2 where jenis_form_modul = 'KOREKSI' "));
	...
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$rkaPPKD32Koreksi->currentTahap = $arrayHasil['currentTahap'];
			$rkaPPKD32Koreksi->jenisForm = "";
			$rkaPPKD32Koreksi->jenisFormTerakhir = "KOREKSI";
			$rkaPPKD32Koreksi->idTahap = $get2['id_tahap'];
			$rkaPPKD32Koreksi->nomorUrut = $get2['no_urut'];
			$rkaPPKD32Koreksi->urutTerakhir = $get2['no_urut'];
