...
class rkaSKPDKoreksiObj  extends DaftarObj2{	
	var $Prefix = 'rkaSKPDKoreksi';
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='rkaSKPDKoreksi.xls';
	var $namaModulCetak='RKA';
	var $Cetak_Judul = 'RKA';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'rkaSKPDKoreksiForm';
	var $modul = "RKA-SKPD";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	...
	$getJumlahBarang = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$rkaSKPDKoreksi_idplh'"));
	...
	$sisaPaguIndikatif = $paguIndikatif - $getPaguYangTelahTerpakai['paguYangTerpakai'];
	 ...
	 if(mysql_num_rows(mysql_query("select * from view_rka where c1='0' and f1 = '0' and k = '$k' and l ='$l' and m='$m' and n='$n' and o='$o'  and id_tahap='$this->idTahap' ")) > 0){
 	 ...
 	 if(empty($cmbJenisRKAForm) ){
		...
		$query = VulnWalkerUpdate('tabel_anggaran',$data,"id_anggaran = '$rkaSKPDKoreksi_idplh'");
		mysql_query($query);
	 }
   ...
   function setPage_HeaderOther(){
	...
	<A href=\"pages.php?Pg=rkaSKPDKoreksi\" title='RKA-SKPKD MURNI' style='color:blue;'  > RKA-SKPD  </a> 
   ...
   function setPage_OtherScript(){
		...
		return 	
			...
			<script type='text/javascript' src='js/perencanaanKeuangan/keuangan/rka/rkaSKPDKoreksi.js' language='JavaScript' ></script> 
	 ...
	 $this->form_height = 400;
	  if ($this->form_fmST==0) {
		...
		foreach ($dt as $key => $value) { 
		 ...
		 $jenis_rka = $jenis_rka;
		 $cmbJenisRKA = cmbArray('cmbJenisRKAForm',$jenis_rka,$arrayJenisRKA,'-- JENIS RKA --','onchange=rkaSKPDKoreksi.unlockFindRekening();');
	 	 if(empty($jenis_rka)){
	  ...
	  $this->form_fields = array(
			...
			'kode7' => array( 
						...
						'value'=> "<input type='text' id='kodeRekening' name='kodeRekening' style='width:120px;' readonly value = '$kodeRekening'> &nbsp&nbsp
						 ...
						 <button type='button' id='findRekening' onclick=rkaSKPDKoreksi.findRekening('$jenis_rka'); $tergantungJenis> CARI </button> "
						 ),
			'kode8' => array( 
						'label'=>'HARGA SATUAN',
						'labelWidth'=>150, 
						'value'=> "<input type='text' name='hargaSatuan' id='hargaSatuan' value='$hargaSatuan' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='rkaSKPDKoreksi.bantu();' > <span id='bantu' style='color:red;'> </span>"
						 ),	
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	foreach ($_COOKIE as $key => $value) { 
		...
		$urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=rkaSKPDKoreksi.refreshList(true);','-- URUSAN --');
		
		$codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
		$bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=rkaSKPDKoreksi.refreshList(true);','-- BIDANG --');
		
		$codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
		$skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=rkaSKPDKoreksi.refreshList(true);','-- SKPD --');
		...
		$unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=rkaSKPDKoreksi.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=rkaSKPDKoreksi.refreshList(true);','-- SUB UNIT --');
...
$rkaSKPDKoreksi = new rkaSKPDKoreksiObj();
...
$rkaSKPDKoreksi->jenisForm = $jenisForm;
$rkaSKPDKoreksi->nomorUrut = $nomorUrut;
$rkaSKPDKoreksi->tahun = $tahun;
$rkaSKPDKoreksi->jenisAnggaran = $jenisAnggaran;
$rkaSKPDKoreksi->idTahap = $idTahap;
$getNamaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$idTahap'"));
$rkaSKPDKoreksi->namaTahap = $getNamaTahap['nama_tahap'];

$rkaSKPDKoreksi->username = $_COOKIE['coID'];

$rkaSKPDKoreksi->wajibValidasi = $Main->wajibValidasi;
if($Main->wajibValidasi == TRUE){
	$rkaSKPDKoreksi->sqlValidasi = " and status_validasi ='1' ";
}else{
	$rkaSKPDKoreksi->sqlValidasi = " ";
}

if(empty($rkaSKPDKoreksi->tahun)){
	...
	/*$rkaSKPDKoreksi->tahun = "select max(id_anggaran) as max from view_rka where nama_modul = 'rkaSKPDKoreksi'";*/
	$rkaSKPDKoreksi->tahun  = $get2['tahun'];
	$rkaSKPDKoreksi->jenisAnggaran = $get2['jenis_anggaran'];
	$rkaSKPDKoreksi->urutTerakhir = $get2['no_urut'];
	$rkaSKPDKoreksi->jenisFormTerakhir = $get2['jenis_form_modul'];
	$rkaSKPDKoreksi->urutSebelumnya = $rkaSKPDKoreksi->urutTerakhir - 1;
	...
	$rkaSKPDKoreksi->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$rkaSKPDKoreksi->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];
	
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$rkaSKPDKoreksi->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$rkaSKPDKoreksi->idTahap'"));
	$rkaSKPDKoreksi->currentTahap = $getCurrenttahap['nama_tahap'];
	
	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$rkaSKPDKoreksi->idTahap'"));
	$rkaSKPDKoreksi->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$rkaSKPDKoreksi->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$rkaSKPDKoreksi->provinsi = $setting['provinsi'];
$rkaSKPDKoreksi->kota = $setting['kota'];
$rkaSKPDKoreksi->pengelolaBarang = $setting['pengelolaBarang'];
$rkaSKPDKoreksi->pejabatPengelolaBarang = $setting['pejabat'];
$rkaSKPDKoreksi->pengurusPengelolaBarang = $setting['pengurus'];
$rkaSKPDKoreksi->nipPengelola = $setting['nipPengelola'];
$rkaSKPDKoreksi->nipPengurus = $setting['nipPengurus'];
$rkaSKPDKoreksi->nipPejabat = $setting['nipPejabat'];


if($rkaSKPDKoreksi->jenisForm != "KOREKSI"){
$get1 = mysql_fetch_array(mysql_query("select max(id_anggaran)  from view_rka_2_2_1 where jenis_form_modul = 'KOREKSI' "));
	...
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$rkaSKPDKoreksi->currentTahap = $arrayHasil['currentTahap'];
			$rkaSKPDKoreksi->jenisForm = "";
			$rkaSKPDKoreksi->jenisFormTerakhir = "KOREKSI";
			$rkaSKPDKoreksi->idTahap = $get2['id_tahap'];
			$rkaSKPDKoreksi->nomorUrut = $get2['no_urut'];
			$rkaSKPDKoreksi->urutTerakhir = $get2['no_urut'];
