...
class rkaSKPD1PADNewObj  extends DaftarObj2{
	var $Prefix = 'rkaSKPD1PADNew';
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='rkaSKPD1PADNew.xls';
	var $namaModulCetak='RKA';
	var $Cetak_Judul = 'RKA';
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'rkaSKPD1PADNewForm';
	var $modul = "RKA-SKPD";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
   ...
   // if( $err=='' && $kategori =='' ) $err= 'Kategori Belum Di Pilih !!';
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
      ...
      break;
			...
			case 'editSubRincian':{
					...
					$eventKeyup = 'document.getElementById("formatjumlah'.$id.'").innerHTML = rkaSKPD1PADNew.formatCurrency(this.value); ';
		...
		case 'Gruping':{
			...
			if(!isset($rkaSKPD1PADNew_cb)){
				$err = "Pilih Data";
			}
			$dt = implode(',',$rkaSKPD1PADNew_cb);
    ...
    break;
		...
		case 'sesuai':{
			...
			$getrkaSKPD1PADNewnya = mysql_fetch_array(mysql_query($queryRows));
			foreach ($getrkaSKPD1PADNewnya as $key => $value) {
				  $$key = $value;
			}
			$cmbUrusanForm = $c1;
			$cmbBidangForm = $c;
	    ...
	    case 'SaveValid':{
	    	foreach ($_REQUEST as $key => $value) {
			 ...
			 $query = VulnWalkerUpdate("tabel_anggaran",$data," id_anggaran = '$rkaSKPD1PADNew_idplh'");
			 mysql_query($query);
	 ...
	 <A href=\"pages.php?Pg=rkaSKPD1PADNew\" title='PENDAPATAN'  style='color:blue;'> PENDAPATAN </a> |
	 ...
	 <tr><td class=\"menudottedline\" width=\"40%\" height=\"20\" style='text-align:right'><B>
	 <A href=\"pages.php?Pg=rkaSKPD1PADNew\" title='PENDAPATAN ASLI DAERAH' style='color:blue;' > PENDAPATAN ASLI DAERAH </a> |
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	foreach ($_COOKIE as $key => $value) {
		...
		$urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=rkaSKPD1PADNew.refreshList(true);','-- URUSAN --');

		$codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
		$bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=rkaSKPD1PADNew.refreshList(true);','-- BIDANG --');

		$codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
		$skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=rkaSKPD1PADNew.refreshList(true);','-- SKPD --');
		...
		$unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=rkaSKPD1PADNew.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=rkaSKPD1PADNew.refreshList(true);','-- SUB UNIT --');
...
function Laporan($dt){
   ...
   $this->form_width = 300;
   $this->form_height = 100;
   $this->form_caption = 'LAPORAN RKA-SKPD 1 MURNI';
   ...
   $dari = $_REQUEST['dari'];
     ...
     $arrayJenisLaporan = array(
                 array('RKASKPD1PAD', 'RKA-SKPD 1 MURNI'),
...
function RKASKPD1PAD($xls =FALSE){
		global $Main;
...
$rkaSKPD1PADNew = new rkaSKPD1PADNewObj();
...
$rkaSKPD1PADNew->jenisForm = $jenisForm;
$rkaSKPD1PADNew->nomorUrut = $nomorUrut;
$rkaSKPD1PADNew->urutTerakhir = $nomorUrut;
$rkaSKPD1PADNew->tahun = $tahun;
$rkaSKPD1PADNew->jenisAnggaran = $jenisAnggaran;
$rkaSKPD1PADNew->idTahap = $idTahap;
$rkaSKPD1PADNew->username = $_COOKIE['coID'];

$rkaSKPD1PADNew->wajibValidasi = $Main->wajibValidasi;
if($Main->wajibValidasi == TRUE){
	$rkaSKPD1PADNew->sqlValidasi = " and status_validasi ='1' ";
}else{
	$rkaSKPD1PADNew->sqlValidasi = " ";
}

if(empty($rkaSKPD1PADNew->tahun)){
	...
	$rkaSKPD1PADNew->tahun  = $get2['tahun'];
	$rkaSKPD1PADNew->jenisAnggaran = $get2['jenis_anggaran'];
	$rkaSKPD1PADNew->urutTerakhir = $get2['no_urut'];
	$rkaSKPD1PADNew->jenisFormTerakhir = $get2['jenis_form_modul'];
	$rkaSKPD1PADNew->urutSebelumnya = $rkaSKPD1PADNew->urutTerakhir - 1;
	...
	$rkaSKPD1PADNew->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$rkaSKPD1PADNew->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];

	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$rkaSKPD1PADNew->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$rkaSKPD1PADNew->idTahap'"));
	$rkaSKPD1PADNew->currentTahap = $getCurrenttahap['nama_tahap'];

	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$rkaSKPD1PADNew->idTahap'"));
	$rkaSKPD1PADNew->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$rkaSKPD1PADNew->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$rkaSKPD1PADNew->provinsi = $setting['provinsi'];
$rkaSKPD1PADNew->kota = $setting['kota'];
$rkaSKPD1PADNew->pengelolaBarang = $setting['pengelolaBarang'];
$rkaSKPD1PADNew->pejabatPengelolaBarang = $setting['pejabat'];
$rkaSKPD1PADNew->pengurusPengelolaBarang = $setting['pengurus'];
$rkaSKPD1PADNew->nipPengelola = $setting['nipPengelola'];
$rkaSKPD1PADNew->nipPengurus = $setting['nipPengurus'];
$rkaSKPD1PADNew->nipPejabat = $setting['nipPejabat'];


if($rkaSKPD1PADNew->jenisForm != "PENYUSUNAN"){
$get1 = mysql_fetch_array(mysql_query("select max(id_anggaran)  from view_rka_1 where jenis_form_modul = 'PENYUSUNAN' "));
	...
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$rkaSKPD1PADNew->currentTahap = $arrayHasil['currentTahap'];
			$rkaSKPD1PADNew->jenisForm = "";
			$rkaSKPD1PADNew->jenisFormTerakhir = "PENYUSUNAN";
			$rkaSKPD1PADNew->idTahap = $get2['id_tahap'];
			$rkaSKPD1PADNew->tahun = $get2['tahun'];
			$rkaSKPD1PADNew->nomorUrut = $get2['no_urut'];
			$rkaSKPD1PADNew->urutTerakhir = $get2['no_urut'];
}
