...
class rkaSKPD1KoreksiObj  extends DaftarObj2{	
	var $Prefix = 'rkaSKPD1Koreksi';
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='rkaSKPD1Koreksi.xls';
	var $namaModulCetak='RKA';
	var $Cetak_Judul = 'RKA';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'rkaSKPD1KoreksiForm';
	var $modul = "RKA-SKPD";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	...
	$getJumlahBarang = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$rkaSKPD1Koreksi_idplh'"));
	...
	$sisaPaguIndikatif = $paguIndikatif - $getPaguYangTelahTerpakai['paguYangTerpakai'];
	 ...
	 if(mysql_num_rows(mysql_query("select * from view_rka_1 where c1='0' and f = '00' and k = '$k' and l ='$l' and m='$m' and n='$n' and o='$o'  and id_tahap='$this->idTahap' ")) > 0){
 	 ...
 	 if(empty($cmbJenisRKAForm) ){
		...
		$query = VulnWalkerUpdate('tabel_anggaran',$data,"id_anggaran = '$rkaSKPD1Koreksi_idplh'");
		mysql_query($query);
	 }
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){	
		...
		case 'Gruping':{	
			...
			if(!isset($rkaSKPD1Koreksi_cb)){
				$err = "Pilih Data";
			}
			$dt = implode(',',$rkaSKPD1Koreksi_cb);
		...
		case 'sesuai':{
			...
			$getrkaSKPD1Koreksinya = mysql_fetch_array(mysql_query($queryRows));
			foreach ($getrkaSKPD1Koreksinya as $key => $value) { 
				  $$key = $value; 
			} 
			$cmbUrusanForm = $c1;
			$cmbBidangForm = $c;
		...
		case 'editTab':{
			$id = $_REQUEST['rkaSKPD1Koreksi_cb'];
			foreach ($_REQUEST as $key => $value) { 
				  $$key = $value; 
			} 
	...
	<A href=\"pages.php?Pg=rkaSKPD1Koreksi\" title='RKA-SKPKD MURNI' style='color:blue;' > RKA-SKPD 1 </a> |
   ...
   function setPage_OtherScript(){
		...
		return 	
			...
			<script type='text/javascript' src='js/perencanaanKeuangan/keuangan/rka/rkaSKPD1Koreksi.js' language='JavaScript' ></script> 
	 ...
	 $this->form_height = 400;
	  if ($this->form_fmST==0) {
		...
		foreach ($dt as $key => $value) { 
		 ...
		 $jenis_rka = $jenis_rka;
		 $cmbJenisRKA = cmbArray('cmbJenisRKAForm',$jenis_rka,$arrayJenisRKA,'-- JENIS RKA --','onchange=rkaSKPD1Koreksi.unlockFindRekening();');
	 	 if(empty($jenis_rka)){
	  ...
	  $this->form_fields = array(
			...
			'kode7' => array( 
						...
						'value'=> "<input type='text' id='kodeRekening' name='kodeRekening' style='width:120px;' readonly value = '$kodeRekening'> &nbsp&nbsp
						 ...
						 <button type='button' id='findRekening' onclick=rkaSKPD1Koreksi.findRekening('$jenis_rka'); $tergantungJenis> CARI </button> "
						 ),
			'kode8' => array( 
						'label'=>'HARGA SATUAN',
						'labelWidth'=>150, 
						'value'=> "<input type='text' name='hargaSatuan' id='hargaSatuan' value='$hargaSatuan' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='rkaSKPD1Koreksi.bantu();' > <span id='bantu' style='color:red;'> </span>"
						 ),	
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	foreach ($_COOKIE as $key => $value) { 
		...
		$urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=rkaSKPD1Koreksi.refreshList(true);','-- URUSAN --');
		
		$codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
		$bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=rkaSKPD1Koreksi.refreshList(true);','-- BIDANG --');
		
		$codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
		$skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=rkaSKPD1Koreksi.refreshList(true);','-- SKPD --');
		...
		$unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=rkaSKPD1Koreksi.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=rkaSKPD1Koreksi.refreshList(true);','-- SUB UNIT --');
...
$rkaSKPD1Koreksi = new rkaSKPD1KoreksiObj();
...
$rkaSKPD1Koreksi->jenisForm = $jenisForm;
$rkaSKPD1Koreksi->nomorUrut = $nomorUrut;
$rkaSKPD1Koreksi->urutTerakhir = $nomorUrut;
$rkaSKPD1Koreksi->tahun = $tahun;
$rkaSKPD1Koreksi->jenisAnggaran = $jenisAnggaran;
$rkaSKPD1Koreksi->idTahap = $idTahap;
$rkaSKPD1Koreksi->username = $_COOKIE['coID'];
$getNamaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$idTahap'"));
$rkaSKPD1Koreksi->namaTahap = $getNamaTahap['nama_tahap'];

$rkaSKPD1Koreksi->wajibValidasi = $Main->wajibValidasi;
if($Main->wajibValidasi == TRUE){
	$rkaSKPD1Koreksi->sqlValidasi = " and status_validasi ='1' ";
}else{
	$rkaSKPD1Koreksi->sqlValidasi = " ";
}

if(empty($rkaSKPD1Koreksi->tahun)){
	...
	/*$rkaSKPD1Koreksi->tahun = "select max(id_anggaran) as max from view_rka_1 where nama_modul = 'rkaSKPD1Koreksi'";*/
	$rkaSKPD1Koreksi->tahun  = $get2['tahun'];
	$rkaSKPD1Koreksi->jenisAnggaran = $get2['jenis_anggaran'];
	$rkaSKPD1Koreksi->urutTerakhir = $get2['no_urut'];
	$rkaSKPD1Koreksi->jenisFormTerakhir = $get2['jenis_form_modul'];
	$rkaSKPD1Koreksi->urutSebelumnya = $rkaSKPD1Koreksi->urutTerakhir - 1;
	...
	$rkaSKPD1Koreksi->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$rkaSKPD1Koreksi->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];
	
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$rkaSKPD1Koreksi->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$rkaSKPD1Koreksi->idTahap'"));
	$rkaSKPD1Koreksi->currentTahap = $getCurrenttahap['nama_tahap'];
	
	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$rkaSKPD1Koreksi->idTahap'"));
	$rkaSKPD1Koreksi->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$rkaSKPD1Koreksi->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$rkaSKPD1Koreksi->provinsi = $setting['provinsi'];
$rkaSKPD1Koreksi->kota = $setting['kota'];
$rkaSKPD1Koreksi->pengelolaBarang = $setting['pengelolaBarang'];
$rkaSKPD1Koreksi->pejabatPengelolaBarang = $setting['pejabat'];
$rkaSKPD1Koreksi->pengurusPengelolaBarang = $setting['pengurus'];
$rkaSKPD1Koreksi->nipPengelola = $setting['nipPengelola'];
$rkaSKPD1Koreksi->nipPengurus = $setting['nipPengurus'];
$rkaSKPD1Koreksi->nipPejabat = $setting['nipPejabat'];


if($rkaSKPD1Koreksi->jenisForm != "KOREKSI"){
$get1 = mysql_fetch_array(mysql_query("select max(id_anggaran)  from view_rka_1 where jenis_form_modul = 'KOREKSI' "));
	...
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$rkaSKPD1Koreksi->currentTahap = $arrayHasil['currentTahap'];
			$rkaSKPD1Koreksi->jenisForm = "";
			$rkaSKPD1Koreksi->jenisFormTerakhir = "KOREKSI";
			$rkaSKPD1Koreksi->idTahap = $get2['id_tahap'];
			$rkaSKPD1Koreksi->nomorUrut = $get2['no_urut'];
			$rkaSKPD1Koreksi->urutTerakhir = $get2['no_urut'];
