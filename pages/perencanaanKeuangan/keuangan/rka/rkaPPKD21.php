...
class rkaPPKD21Obj  extends DaftarObj2{
	var $Prefix = 'rkaPPKD21';
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='rkaPPKD21.xls';
	var $namaModulCetak='RKA';
	var $Cetak_Judul = 'RKA';
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'rkaPPKD21Form';
	var $modul = "RKA-PPKD";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	...
	function genRowSum($ColStyle, $Mode, $Total){
		foreach ($_REQUEST as $key => $value) {
		 ...
		 if($cmbSubUnit != ''){
			$kondisiPPKD = "and c1='$cmbUrusan' and c='$cmbBidang' and d='$cmbSKPD' and e='$cmbUnit' and e1='$cmbSubUnit'";
		}elseif($cmbUnit != ''){
			$kondisiPPKD = "and c1='$cmbUrusan' and c='$cmbBidang' and d='$cmbSKPD' and e='$cmbUnit' ";
		}elseif($cmbSKPD != ''){
			$kondisiPPKD = "and c1='$cmbUrusan' and c='$cmbBidang' and d='$cmbSKPD'  ";
		}elseif($cmbBidang != ''){
			$kondisiPPKD = "and c1='$cmbUrusan' and c='$cmbBidang'  ";
		}elseif($cmbUrusan != ''){
			$kondisiPPKD = "and c1='$cmbUrusan'";
		}
	...
	$getPaguYangTelahTerpakai = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as paguYangTerpakai from view_rka_ppkd_2_1 where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1'  and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'  and no_urut = '$this->nomorUrut' and id_anggaran!='$rkaPPKD21_idplh' "));
	...
	function set_selector_other($tipe){
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){
		...
		case 'Gruping':{
			...
			if(!isset($rkaPPKD21_cb)){
				$err = "Pilih Data";
			}
			$dt = implode(',',$rkaPPKD21_cb);
		...
		case 'clearTemp':{
				$username =$_COOKIE['coID'];
				mysql_query("delete from temp_rekening_rka_belanja_tidak_langsung_ppkd where username ='$username'");
				mysql_query("delete from temp_rincian_belanja_tidak_langsung_ppkd where username ='$username'");
				mysql_query("delete from temp_sub_rincian_belanja_tidak_langsung_ppkd where username ='$username'");
				mysql_query("delete from temp_remove_rka_21_ppkd where username ='$username'");
				foreach ($_REQUEST as $key => $value) {
			 	 $$key = $value;
				}
				$getALlData = mysql_query("select * from view_rka_ppkd_2_1 where id_tahap = '$this->idTahap' and c1 ='$cmbUrusan' and c ='$cmbBidang' and d='$cmbSKPD' and bk ='0' and ck='0' and dk='0' and p='0' and q='0' and rincian_perhitungan !='' and id_rincian_belanja !=''");
				while ($rows = mysql_fetch_array($getALlData)) {
						foreach ($rows as $key => $value) {
								$$key = $value;
						}
						$getMyIdRekening = mysql_fetch_array(mysql_query("select * from view_rka_ppkd_2_1 where id_tahap = '$this->idTahap' and bk ='0' and ck='0' and dk='0' and p='0' and q='0' and k='$k'  and l ='$l' and m='$m' and n='$n' and o='$o' and id_rincian_belanja ='' and rincian_perhitungan = '' "));
						$getDataRekening = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '".$getMyIdRekening['id_anggaran']."'"));
						if(mysql_num_rows(mysql_query("select * from temp_rekening_rka_belanja_tidak_langsung_ppkd where username = '$this->username' and k='$k'  and l ='$l' and m='$m' and n='$n' and o='$o'")) == 0){
								$dataRekening = array(
																				'k'=> $k,
																				'l'=> $l,
																				'm'=> $m,
								...
								$queryRekening = VulnWalkerInsert('temp_rekening_rka_belanja_tidak_langsung_ppkd',$dataRekening);
								mysql_query($queryRekening);
						}

						$getMyIdRincianBelanja = mysql_fetch_array(mysql_query("select * from view_rka_ppkd_2_1 where id_tahap = '$this->idTahap' and c1='$c1' and c='$c' and d='$d' and bk ='0' and ck='0' and dk='0' and p='0' and q='0' and k='$k'  and l ='$l' and m='$m' and n='$n' and o='$o' and id_rincian_belanja ='$id_rincian_belanja' and rincian_perhitungan = '' "));
						...
						$getIdRekeningBelanja = mysql_fetch_array(mysql_query("select * from temp_rekening_rka_belanja_tidak_langsung_ppkd where k ='$k' and l='$l' and m='$m' and n='$n' and o='$o' and username = '$this->username'"));
						if(mysql_num_rows(mysql_query("select * from temp_rincian_belanja_tidak_langsung_ppkd where username = '$this->username' and id_rekening_belanja = '".$getIdRekeningBelanja['id']."' and uraian = '".$getUraianRincianBelanja['nama_rincian_belanja']."'")) == 0){
								...
								$queryRincianBelanja = VulnWalkerInsert('temp_rincian_belanja_tidak_langsung_ppkd',$dataRincianBelanja);
						...
						$getIdRincianBelanja = mysql_fetch_array(mysql_query("select * from temp_rincian_belanja_tidak_langsung_ppkd where username = '$this->username' and uraian = '".$getUplineRincianBelanja['nama_rincian_belanja']."'"));
						...
						$querySubRincian = VulnWalkerInsert("temp_sub_rincian_belanja_tidak_langsung_ppkd",$dataSubRincian);
		...
		case 'sesuai':{
			...
			$cekPPKD =  mysql_num_rows(mysql_query("select * from tabel_anggaran where c1 = '$cmbUrusanForm' and c='$cmbBidangForm' and d = '$d' and e='00' and e1='000'  and p = '00' and q='00'   and tahun = '$this->tahun' and jenis_anggaran = '$this->jenisAnggaran' and id_tahap = '$this->idTahap' "));
					if($cekPPKD > 0 ){
	 ...
	 function setPage_HeaderOther(){
		...
		<A href=\"pages.php?Pg=rkaPPKD21\" title='RKA-PPKD MURNI'  style='color:blue;' > BELANJA TIDAK LANGSUNG </a> |
		<A href=\"pages.php?Pg=rkaPPKD1PAD\" title='RKA-PPKD MURNI'  > PENDAPATAN </a> |

		<A href=\"pages.php?Pg=rkaPPKD31\" title='RKA-PPKD MURNI'  > PEMBIAYAAN </a> |
		<A href=\"pages.php?Pg=rkaPPKD\" title='RKA-PPKD MURNI'  > RKA-PPKD </a>
	   ...
	   function setPage_OtherScript(){
			...
			return
				...
				<script type='text/javascript' src='js/perencanaanKeuangan/keuangan/rka/rkaPPKD21.js' language='JavaScript' ></script>
	 ...
	 $this->form_height = 400;
	  if ($this->form_fmST==0) {
		...
		foreach ($dt as $key => $value) {
		 ...
		 $skpd = "<input type ='hidden' name='d' id='d' value = '$d' > <input type ='text'  value = '$namaPPKD' style='width:400px;' readonly>";
		 ...
		 $jenis_rka = $jenis_rka;
		 $cmbJenisRKA = cmbArray('cmbJenisRKAForm',$jenis_rka,$arrayJenisRKA,'-- JENIS RKA --','onchange=rkaPPKD21.unlockFindRekening();');
	 	 if(empty($jenis_rka)){
	  ...
	  $this->form_fields = array(
			...
			'kode7' => array(
						...
						'value'=> "<input type='text' id='kodeRekening' name='kodeRekening' style='width:120px;' readonly value = '$kodeRekening'> &nbsp&nbsp
						 ...
						 <button type='button' id='findRekening' onclick=rkaPPKD21.findRekening('$jenis_rka'); $tergantungJenis> CARI </button> "
						 ),
			'kode8' => array(
						'label'=>'HARGA SATUAN',
						'labelWidth'=>150,
						'value'=> "<input type='text' name='hargaSatuan' id='hargaSatuan' value='$hargaSatuan' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='rkaPPKD21.bantu();' > <span id='bantu' style='color:red;'> </span>"
						 ),
	...
	function setKolomData($no, $isi, $Mode, $TampilCheckBox){
	 ...
	 foreach ($_REQUEST as $key => $value) {
		...
		if( !empty($k) && !empty($l) && !empty($m)  && !empty($n)  && !empty($o)  && empty($id_rincian_belanja) ){
				 ...
				 $getSumJumlahHarga = mysql_fetch_array(mysql_query("select sum(jumlah_harga) from view_rka_ppkd_2_1 where id_tahap = '$this->idTahap' and bk='$bk' and ck='$ck' and dk='$dk' and p='$p' and q='$q' and k ='$k' and l='$l' and m='$m' and n='$n' and o='$o' $kondisiPPKD $kondisiRekening"));
				 ...
				 $getSumJumlahHarga = mysql_fetch_array(mysql_query("select sum(jumlah_harga) from view_rka_ppkd_2_1 where id_tahap = '$this->idTahap' and bk='$bk' and ck='$ck' and dk='$dk' and p='$p' and q='$q' and k ='$k' and l='$l' and m='$m' and n='$n' and o='$o' and id_rincian_belanja='$id_rincian_belanja' $kondisiPPKD $kondisiRekening"));
				 $jumlahHarga = number_format($getSumJumlahHarga['sum(jumlah_harga)'] ,2,',','.');
				 $TampilCheckBox = "";
		}elseif( !empty($k) && !empty($l) && !empty($m)  && !empty($n)  && !empty($o)  && !empty($id_rincian_belanja)  && !empty($rincian_perhitungan) ){
				 //SubRincianBelanja
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	foreach ($_COOKIE as $key => $value) {
		...
		$urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=rkaPPKD21.refreshList(true);','-- URUSAN --');

		$codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
		$bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=rkaPPKD21.refreshList(true);','-- BIDANG --');

		$codeAndNamePPKD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
		$skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNamePPKD,'onchange=rkaPPKD21.refreshList(true);','-- SKPD --');
		...
		$unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=rkaPPKD21.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=rkaPPKD21.refreshList(true);','-- SUB UNIT --');
	...
	function getDaftarOpsi($Mode=1){
	  ...
	  if(isset($cmbUrusan) && $cmbUrusan== ''){
			...
			$getAllSubRincian = mysql_query("select * from view_rka_ppkd_2_1 where id_tahap = '$this->idTahap' and (j!='000' or rincian_perhitungan !='' )");
			while($subRincian = mysql_fetch_array($getAllSubRincian)){
						if(mysql_num_rows(mysql_query("select * from view_rka_ppkd_2_1 where id_tahap = '$this->idTahap' and id_anggaran='".$subRincian['id_anggaran']."' and (j!='000' or rincian_perhitungan !='' ) $kondisiPPKD $kondisiRekening")) == 0){
								$blackListSubRincian[] = "id_anggaran != '".$subRincian['id_anggaran']."'";
								$arrKondisi[] = "id_anggaran !='".$subRincian['id_anggaran']."'";
								// $this->injectQuery = "select * from view_rka_ppkd_2_1 where id_tahap = '$this->idTahap' and id_anggaran='".$subRincian['id_anggaran']."' and (j!='000' or rincian_perhitungan !='' ) $kondisiPPKD $kondisiRekening";
						}
			}
			...
			$getAllRincian =  mysql_query("select * from view_rka_ppkd_2_1 where id_tahap ='$this->idTahap'  and k !='0' and k!=''  and l !='0' and l!=''  and m !='0' and m!=''  and n !='0' and n !='' and id_rincian_belanja != '' and id_rincian_belanja !='0' and j='000' and rincian_perhitungan = '' ");
			while($rincianBelanja = mysql_fetch_array($getAllRincian)){
					if(mysql_num_rows(mysql_query("select  * from view_rka_ppkd_2_1 where id_tahap = '$this->idTahap' and bk = '".$rincianBelanja['bk']."' and ck = '".$rincianBelanja['ck']."' and dk = '".$rincianBelanja['dk']."' and p = '".$rincianBelanja['p']."' and q = '".$rincianBelanja['q']."' and k = '".$rincianBelanja['k']."' and l = '".$rincianBelanja['l']."'  and m = '".$rincianBelanja['m']."'  and n = '".$rincianBelanja['n']."' and o = '".$rincianBelanja['o']."' and id_rincian_belanja = '".$rincianBelanja['id_rincian_belanja']."' and (j!='000' or rincian_perhitungan !='') $kondisiBlackListSubRincian ")) == 0){
			...
			$getAllRekening =  mysql_query("select * from view_rka_ppkd_2_1 where id_tahap ='$this->idTahap'  and k !='0' and k!=''  and l !='0' and l!=''  and m !='0' and m!=''  and n !='0' and n !='' and id_rincian_belanja ='' and j='000' and rincian_perhitungan = '' ");
			while($rekeningBelanja = mysql_fetch_array($getAllRekening)){
					if(mysql_num_rows(mysql_query("select  * from view_rka_ppkd_2_1 where id_tahap = '$this->idTahap' and bk = '".$rekeningBelanja['bk']."' and ck = '".$rekeningBelanja['ck']."' and dk = '".$rekeningBelanja['dk']."' and p = '".$rekeningBelanja['p']."' and q = '".$rekeningBelanja['q']."' and k = '".$rekeningBelanja['k']."' and l = '".$rekeningBelanja['l']."'  and m = '".$rekeningBelanja['m']."'  and n = '".$rekeningBelanja['n']."' and o = '".$rekeningBelanja['o']."' and (j!='000' or rincian_perhitungan !='') $kondisiBlackListSubRincian ")) == 0){
...
function Laporan($xls =FALSE){
		...
		$arrKondisi = array();
		$grabPPKD = mysql_fetch_array(mysql_query("select * from skpd_report_rka_21 where username='$this->username'"));
		...
		if($this->jenisForm == 'PENYUSUNAN'){
			...
			while($rows = mysql_fetch_array($getAllParent)){
				...
				$cekPekerjaan = mysql_num_rows(mysql_query("select * from view_rka_ppkd_2_1 where o1 = '$o1' and id_tahap ='$this->idTahap' $kondisiRekening $kondisiPPKD and (rincian_perhitungan !='' or rincian_perhitungan!='') "));
				if($cekPekerjaan == 0){
					$arrKondisi[] = "o1 !='$o1'";
				}else{
					...
					while($row2s = mysql_fetch_array($getAllRekening)){
						...
						$cekRekening = mysql_num_rows(mysql_query("select * from view_rka_ppkd_2_1 where k ='$k' and l='$l' and m='$m' and n='$n' and o='$o' and id_tahap ='$this->idTahap' $kondisiRekening $kondisiPPKD and (rincian_perhitungan !='' or rincian_perhitungan!='') "));
						if($cekRekening == 0){
							$concat = $k.".".$l.".".$m.".".$n.".".$o;
				...
				while($got = mysql_fetch_array($grabNonMapingRekening)){
					if(mysql_num_rows(mysql_query("select * from view_rka_ppkd_2_1 where id_tahap ='$this->idTahap' and k ='".$got['k']."' and l ='".$got['l']."' and m ='".$got['m']."' and n ='".$got['n']."' and o ='".$got['o']."' and rincian_perhitungan!=''  $kondisiPPKD ")) != 0){
			...
			while($rows = mysql_fetch_array($getAllParent)){
				...
				$cekPekerjaan = mysql_num_rows(mysql_query("select * from view_rka_ppkd_2_1 where o1 = '$o1' and id_tahap = '$idTahap' $kondisiRekening $kondisiPPKD $blackList and (rincian_perhitungan !='' or rincian_perhitungan!='') "));
				if($cekPekerjaan == 0){
					$arrKondisi[] = "o1 !='$o1'";
				}else{
					...
					while($row2s = mysql_fetch_array($getAllRekening)){
						...
						$cekRekening = mysql_num_rows(mysql_query("select * from view_rka_ppkd_2_1 where k ='$k' and l='$l' and m='$m' and n='$n' and o='$o' and id_tahap = '$idTahap' $kondisiRekening $kondisiPPKD $blackList and (rincian_perhitungan !='' or rincian_perhitungan!='') "));
						if($cekRekening == 0){
							$concat = $k.".".$l.".".$m.".".$n.".".$o;
							$arrKondisi[] = "concat(k,'.',l,'.',m,'.',n,'.',o)  !='$concat'";
						}else{
							...
							while($row3s = mysql_fetch_array($getAllProgram)){
								...
								$cekProgram = mysql_num_rows(mysql_query("select * from view_rka_ppkd_2_1 where bk ='$bk' and ck='$ck' and p='$p' and id_tahap = '$idTahap' $kondisiRekening $kondisiPPKD $blackList and (rincian_perhitungan !='' or rincian_perhitungan!='') "));
								if($cekProgram == 0){
									$concat = $bk.".".$ck.".".$p;
									...
									while($row4s = mysql_fetch_array($getAllKegiatan)){
										...
										$cekKegiatan = mysql_num_rows(mysql_query("select * from view_rka_ppkd_2_1 where bk ='$bk' and ck='$ck' and p='$p' and q='$q' and id_tahap = '$idTahap' $kondisiRekening $kondisiPPKD  $blackList and (rincian_perhitungan !='' or rincian_perhitungan!='') "));
										if($cekKegiatan == 0){
											$concat = $bk.".".$ck.".".$p;
											$arrKondisi[] = "concat(bk,'.',ck,'.',p)  !='$concat'";
										}else{
				...
				while($got = mysql_fetch_array($grabNonMapingRekening)){
					if(mysql_num_rows(mysql_query("select * from view_rka_ppkd_2_1 where id_tahap = '$idTahap' and k ='".$got['k']."' and l ='".$got['l']."' and m ='".$got['m']."' and n ='".$got['n']."' and o ='".$got['o']."' and rincian_perhitungan!='' $kondisiRekening $kondisiPPKD $blackList")) != 0){
			...
			if($this->jenisFormTerakhir == "KOREKSI"){
				...
				while($rows = mysql_fetch_array($getAllParent)){
					...
					$cekPekerjaan = mysql_num_rows(mysql_query("select * from view_rka_ppkd_2_1 where o1 = '$o1' and id_tahap ='$idTahap' $kondisiRekening $kondisiPPKD and (rincian_perhitungan !='' or rincian_perhitungan!='') "));
					if($cekPekerjaan == 0){
						$arrKondisi[] = "o1 !='$o1'";
					}else{
						$arrKondisi[] = " id_tahap='$idTahap'  or id_anggaran = '$id_anggaran' ";
						...
						while($row2s = mysql_fetch_array($getAllRekening)){
							...
							$cekRekening = mysql_num_rows(mysql_query("select * from view_rka_ppkd_2_1 where k ='$k' and l='$l' and m='$m' and n='$n' and o='$o' and id_tahap ='$idTahap' $kondisiRekening $kondisiPPKD and (rincian_perhitungan !='' or rincian_perhitungan!='') "));
							if($cekRekening == 0){
								$concat = $k.".".$l.".".$m.".".$n.".".$o;
								$arrKondisi[] = "concat(k,'.',l,'.',m,'.',n,'.',o)  !='$concat'";
							}else{
								...
								while($row3s = mysql_fetch_array($getAllProgram)){
									...
									$cekProgram = mysql_num_rows(mysql_query("select * from view_rka_ppkd_2_1 where bk ='$bk' and ck='$ck' and p='$p' and id_tahap ='$idTahap' $kondisiRekening $kondisiPPKD and (rincian_perhitungan !='' or rincian_perhitungan!='') "));
									if($cekProgram == 0){
										$concat = $bk.".".$ck.".".$p;
										$arrKondisi[] = "concat(bk,'.',ck,'.',p)  !='$concat'";
									}else{
										...
										while($row4s = mysql_fetch_array($getAllKegiatan)){
											...
											$cekKegiatan = mysql_num_rows(mysql_query("select * from view_rka_ppkd_2_1 where bk ='$bk' and ck='$ck' and p='$p' and q='$q' and id_tahap ='$idTahap' $kondisiRekening $kondisiPPKD and (rincian_perhitungan !='' or rincian_perhitungan!='') "));
											if($cekKegiatan == 0){
												$concat = $bk.".".$ck.".".$p;
												$arrKondisi[] = "concat(bk,'.',ck,'.',p)  !='$concat'";
											}else{
				...
				$grabNonMapingRekening= mysql_query("select * from view_rka_ppkd_2_1 where id_tahap ='$idTahap' and (LENGTH(k) > 1) and rincian_perhitungan=''");
				while($got = mysql_fetch_array($grabNonMapingRekening)){
					if(mysql_num_rows(mysql_query("select * from view_rka_ppkd_2_1 where id_tahap ='$idTahap' and k ='".$got['k']."' and l ='".$got['l']."' and m ='".$got['m']."' and n ='".$got['n']."' and o ='".$got['o']."' and rincian_perhitungan!='' $kondisiRekening $kondisiPPKD ")) != 0){
						$arrKondisi[] = "id_tahap = '$idTahap' or id_anggaran ='".$got['id_anggaran']."'";
					}
		...
		echo
			...
			"<body >
				...
				<table width=\"100%\" border=\"0\" class='subjudulcetak'>
					...
					<tr>
						<td width='10%' valign='top'>PPKD</td>
						<td width='1%' valign='top'> : </td>
		...
		while($daqry = mysql_fetch_array($aqry)){
			...
			if($k == '0' && $n =='0' ){
				...
				$getSumJumlahHarga = mysql_fetch_array(mysql_query("select sum(jumlah_harga) from view_rka_ppkd_2_1 where  o1 ='$o1' $kondisiPPKD $kondisiFilter  "));
				...
				$getSumJumlahHarga = mysql_fetch_array(mysql_query("select sum(jumlah_harga) from view_rka_ppkd_2_1 where  k = '$k' and l='$l' and m='$m' and n='$n' and o='$o'  $kondisiPPKD $kondisiFilter"));
...
$rkaPPKD21->jenisForm = $jenisForm;
$rkaPPKD21->nomorUrut = $nomorUrut;
$rkaPPKD21->urutTerakhir = $nomorUrut;
$rkaPPKD21->tahun = $tahun;
$rkaPPKD21->jenisAnggaran = $jenisAnggaran;
$rkaPPKD21->idTahap = $idTahap;
$rkaPPKD21->username = $_COOKIE['coID'];

$rkaPPKD21->wajibValidasi = $Main->wajibValidasi;
if($Main->wajibValidasi == TRUE){
	$rkaPPKD21->sqlValidasi = " and status_validasi ='1' ";
}else{
	$rkaPPKD21->sqlValidasi = " ";
}
...
if(empty($rkaPPKD21->tahun)){
	...
	$rkaPPKD21->urutTerakhir = $get2['no_urut'];
	$rkaPPKD21->jenisFormTerakhir = $get2['jenis_form_modul'];
	$rkaPPKD21->urutSebelumnya = $rkaPPKD21->urutTerakhir - 1;
	...
	$rkaPPKD21->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];
...
$rkaPPKD21->provinsi = $setting['provinsi'];
$rkaPPKD21->kota = $setting['kota'];
$rkaPPKD21->pengelolaBarang = $setting['pengelolaBarang'];
$rkaPPKD21->pejabatPengelolaBarang = $setting['pejabat'];
$rkaPPKD21->pengurusPengelolaBarang = $setting['pengurus'];
$rkaPPKD21->nipPengelola = $setting['nipPengelola'];
$rkaPPKD21->nipPengurus = $setting['nipPengurus'];
$rkaPPKD21->nipPejabat = $setting['nipPejabat'];


if($rkaPPKD21->jenisForm != "PENYUSUNAN"){
$get1 = mysql_fetch_array(mysql_query("select max(id_anggaran)  from view_rka_ppkd_2_1 where jenis_form_modul = 'PENYUSUNAN' "));
	...
	$rkaPPKD21->currentTahap = $arrayHasil['currentTahap'];
			$rkaPPKD21->jenisForm = "";
			$rkaPPKD21->jenisFormTerakhir = "PENYUSUNAN";
			$rkaPPKD21->idTahap = $get2['id_tahap'];
			$rkaPPKD21->tahun = $get2['tahun'];
			$rkaPPKD21->nomorUrut = $get2['no_urut'];
			$rkaPPKD21->urutTerakhir = $get2['no_urut'];
}
