...
class rkaPPKD32Obj  extends DaftarObj2{
	...
	$getPaguYangTelahTerpakai = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as paguYangTerpakai from view_rka_ppkd_3_2 where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1'  and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'  and no_urut = '$this->nomorUrut' and id_anggaran!='$rkaPPKD32_idplh' "));
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){
		...
		case 'SaveEditJob':{
			...
			$getUrut = mysql_fetch_array(mysql_query("select * from temp_rka_232 where o1='$o1'"));
			$urut = $getUrut['urut'];
		...
		case 'Gruping':{
			...
			if(!isset($rkaPPKD32_cb)){
				$err = "Pilih Data";
			}
			$dt = implode(',',$rkaPPKD32_cb);
		...
		case 'SaveJob':{
			...
			$getCurrentInsert = mysql_fetch_array(mysql_query("select max(id) from ref_pekerjaan"));
			$cmbPekerjaan = cmbQuery('pekerjaan', $getCurrentInsert['max(id)'], $codeAndNamePekerjaan,"  ",'-- PEKERJAAN --');
			$getMaxUrut = mysql_fetch_array(mysql_query("select max(urut) from temp_rka_232 where user ='$username'"));
		...
		case 'Report':{
			...
			if(empty($cmbUrusan)){
				...
				if(mysql_num_rows(mysql_query("select * from skpd_report_rka_32 where username= '$this->username'")) == 0){
					$data = array(
					...
					$query = VulnWalkerInsert('skpd_report_rka_32',$data);
					mysql_query($query);
				}else{
					$data = array(
								  'username' => $this->username,
					...
					$query = VulnWalkerUpdate('skpd_report_rka_32',$data,"username = '$this->username'");
					mysql_query($query);
				}
		...
		case 'clearTemp':{
				$username =$_COOKIE['coID'];
				mysql_query("delete from temp_rekening_rka_pembiayaan_pengeluaran_ppkd where username ='$username'");
				mysql_query("delete from temp_rincian_pembiayaan_pengeluaran_ppkd where username ='$username'");
				mysql_query("delete from temp_sub_rincian_pembiayaan_pengeluaran_ppkd where username ='$username'");
				mysql_query("delete from temp_remove_rka_32_ppkd where username ='$username'");
				foreach ($_REQUEST as $key => $value) {
			 	 $$key = $value;
				}
				$getALlData = mysql_query("select * from view_rka_ppkd_3_2 where id_tahap = '$this->idTahap' and c1 ='$cmbUrusan' and c ='$cmbBidang' and d='$cmbSKPD' and bk ='0' and ck='0' and dk='0' and p='0' and q='0' and rincian_perhitungan !='' and id_rincian_belanja !=''");
				while ($rows = mysql_fetch_array($getALlData)) {
						foreach ($rows as $key => $value) {
								$$key = $value;
						}
						$getMyIdRekening = mysql_fetch_array(mysql_query("select * from view_rka_ppkd_3_2 where id_tahap = '$this->idTahap' and bk ='0' and ck='0' and dk='0' and p='0' and q='0' and k='$k'  and l ='$l' and m='$m' and n='$n' and o='$o' and id_rincian_belanja ='' and rincian_perhitungan = '' "));
						$getDataRekening = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '".$getMyIdRekening['id_anggaran']."'"));
						if(mysql_num_rows(mysql_query("select * from temp_rekening_rka_pembiayaan_pengeluaran_ppkd where username = '$this->username' and k='$k'  and l ='$l' and m='$m' and n='$n' and o='$o'")) == 0){
								$dataRekening = array(
																				'k'=> $k,
																				'l'=> $l,
																				'm'=> $m,
								...
								$queryRekening = VulnWalkerInsert('temp_rekening_rka_pembiayaan_pengeluaran_ppkd',$dataRekening);
								mysql_query($queryRekening);
						}

						$getMyIdRincianBelanja = mysql_fetch_array(mysql_query("select * from view_rka_ppkd_3_2 where id_tahap = '$this->idTahap' and c1='$c1' and c='$c' and d='$d' and bk ='0' and ck='0' and dk='0' and p='0' and q='0' and k='$k'  and l ='$l' and m='$m' and n='$n' and o='$o' and id_rincian_belanja ='$id_rincian_belanja' and rincian_perhitungan = '' "));
						...
						$getIdRekeningBelanja = mysql_fetch_array(mysql_query("select * from temp_rekening_rka_pembiayaan_pengeluaran_ppkd where k ='$k' and l='$l' and m='$m' and n='$n' and o='$o' and username = '$this->username'"));
						if(mysql_num_rows(mysql_query("select * from temp_rincian_pembiayaan_pengeluaran_ppkd where username = '$this->username' and id_rekening_belanja = '".$getIdRekeningBelanja['id']."' and uraian = '".$getUraianRincianBelanja['nama_rincian_belanja']."'")) == 0){
								...
								$queryRincianBelanja = VulnWalkerInsert('temp_rincian_pembiayaan_pengeluaran_ppkd',$dataRincianBelanja);
						...
						$getIdRincianBelanja = mysql_fetch_array(mysql_query("select * from temp_rincian_pembiayaan_pengeluaran_ppkd where username = '$this->username' and uraian = '".$getUplineRincianBelanja['nama_rincian_belanja']."'"));
						...
						$querySubRincian = VulnWalkerInsert("temp_sub_rincian_pembiayaan_pengeluaran_ppkd",$dataSubRincian);
						mysql_query($querySubRincian);

				}
				$cek = "select * from view_rka_ppkd_3_2 where id_tahap = '$this->idTahap' and c1 ='$cmbUrusan' and c ='$cmbBidang' and d='$cmbSKPD' and bk ='0' and ck='0' and dk='0' and p='0' and q='0' and rincian_perhitungan !='' and id_rincian_belanja !=''";
		...
		case 'editTab':{
			...
			mysql_query("delete from temp_rka_32 where user='$username'");
			mysql_query("delete from temp_rincian_volume_32 where user='$username'");
		    mysql_query("delete from temp_alokasi_rka_32_v2 where user='$username'");
			$noUrutPekerjaan = 0;
			$angkaO2 = 0;
			$lastO1 = 0;
			while($rows = mysql_fetch_array($getAll)){
				...
				$getMaxID = mysql_fetch_array(mysql_query("select  max(id) as gblk from temp_rka_32 where user ='$username'"));
				$maxID = $getMaxID['gblk'];
				$lastO1 = $o1;

				$getLastO1 = mysql_fetch_array(mysql_query("select o1 from temp_rka_32 where id='$maxID' "));
				...
				$query = VulnWalkerInsert('temp_rka_32',$data);
				mysql_query($query);
				$angkaO2 = $angkaO2 + 1;
   ...
   function setPage_HeaderOther(){
	...
	<A href=\"pages.php?Pg=rkaPPKD1PAD\" title='PENDAPATAN' > PENDAPATAN </a> |
	<A href=\"pages.php?Pg=rkaPPKD32\" title='RKA-SKPKD MURNI' style='color:blue;'  > PEMBIAYAAN </a> |
	...
	<tr><td class=\"menudottedline\" width=\"40%\" height=\"20\" style='text-align:right'><B>
	<A href=\"pages.php?Pg=rkaPPKD31\" title='pengeluaran'   > PENERIMAAN </a> |
	<A href=\"pages.php?Pg=rkaPPKD32\" title='PENGELUARAN' style='color:blue;' > PENGELUARAN </a>
   ...
   function setPage_OtherScript(){
		...
		return
			...
			<script type='text/javascript' src='js/perencanaanKeuangan/keuangan/rka/rkaPPKD32.js' language='JavaScript' ></script>
	...
	function setKolomData($no, $isi, $Mode, $TampilCheckBox){
	 ...
	 foreach ($_REQUEST as $key => $value) {
		...
		if( !empty($k) && !empty($l) && !empty($m)  && !empty($n)  && !empty($o)  && empty($id_rincian_belanja) ){
				 ...
				 $getSumJumlahHarga = mysql_fetch_array(mysql_query("select sum(jumlah_harga) from view_rka_ppkd_3_2 where id_tahap = '$this->idTahap' and bk='$bk' and ck='$ck' and dk='$dk' and p='$p' and q='$q' and k ='$k' and l='$l' and m='$m' and n='$n' and o='$o' $kondisiSKPD $kondisiRekening"));
				 ...
				 $getSumJumlahHarga = mysql_fetch_array(mysql_query("select sum(jumlah_harga) from view_rka_ppkd_3_2 where id_tahap = '$this->idTahap' and bk='$bk' and ck='$ck' and dk='$dk' and p='$p' and q='$q' and k ='$k' and l='$l' and m='$m' and n='$n' and o='$o' and id_rincian_belanja='$id_rincian_belanja' $kondisiSKPD $kondisiRekening"));
				 $jumlahHarga = number_format($getSumJumlahHarga['sum(jumlah_harga)'] ,2,',','.');
				 $TampilCheckBox = "";
		}elseif( !empty($k) && !empty($l) && !empty($m)  && !empty($n)  && !empty($o)  && !empty($id_rincian_belanja)  && !empty($rincian_perhitungan) ){
				 //SubRincianBelanja
	...
	function Validasi($dt){
	 ...
	 $this->form_caption = 'VALIDASI RKA-PPKD 3.2';
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	function getDaftarOpsi($Mode=1){
	  ...
	  if(isset($cmbUrusan) && $cmbUrusan== ''){
			...
			$getAllSubRincian = mysql_query("select * from view_rka_ppkd_3_2 where id_tahap = '$this->idTahap' and (j!='000' or rincian_perhitungan !='' )");
			while($subRincian = mysql_fetch_array($getAllSubRincian)){
						if(mysql_num_rows(mysql_query("select * from view_rka_ppkd_3_2 where id_tahap = '$this->idTahap' and id_anggaran='".$subRincian['id_anggaran']."' and (j!='000' or rincian_perhitungan !='' ) $kondisiSKPD $kondisiRekening")) == 0){
								$blackListSubRincian[] = "id_anggaran != '".$subRincian['id_anggaran']."'";
								$arrKondisi[] = "id_anggaran !='".$subRincian['id_anggaran']."'";
								// $this->injectQuery = "select * from view_rka_ppkd_3_2 where id_tahap = '$this->idTahap' and id_anggaran='".$subRincian['id_anggaran']."' and (j!='000' or rincian_perhitungan !='' ) $kondisiSKPD $kondisiRekening";
						}
			}
			...
			$getAllRincian =  mysql_query("select * from view_rka_ppkd_3_2 where id_tahap ='$this->idTahap'  and k !='0' and k!=''  and l !='0' and l!=''  and m !='0' and m!=''  and n !='0' and n !='' and id_rincian_belanja != '' and id_rincian_belanja !='0' and j='000' and rincian_perhitungan = '' ");
			while($rincianBelanja = mysql_fetch_array($getAllRincian)){
					if(mysql_num_rows(mysql_query("select  * from view_rka_ppkd_3_2 where id_tahap = '$this->idTahap' and bk = '".$rincianBelanja['bk']."' and ck = '".$rincianBelanja['ck']."' and dk = '".$rincianBelanja['dk']."' and p = '".$rincianBelanja['p']."' and q = '".$rincianBelanja['q']."' and k = '".$rincianBelanja['k']."' and l = '".$rincianBelanja['l']."'  and m = '".$rincianBelanja['m']."'  and n = '".$rincianBelanja['n']."' and o = '".$rincianBelanja['o']."' and id_rincian_belanja = '".$rincianBelanja['id_rincian_belanja']."' and (j!='000' or rincian_perhitungan !='') $kondisiBlackListSubRincian ")) == 0){
			...
			$getAllRekening =  mysql_query("select * from view_rka_ppkd_3_2 where id_tahap ='$this->idTahap'  and k !='0' and k!=''  and l !='0' and l!=''  and m !='0' and m!=''  and n !='0' and n !='' and id_rincian_belanja ='' and j='000' and rincian_perhitungan = '' ");
			while($rekeningBelanja = mysql_fetch_array($getAllRekening)){
					if(mysql_num_rows(mysql_query("select  * from view_rka_ppkd_3_2 where id_tahap = '$this->idTahap' and bk = '".$rekeningBelanja['bk']."' and ck = '".$rekeningBelanja['ck']."' and dk = '".$rekeningBelanja['dk']."' and p = '".$rekeningBelanja['p']."' and q = '".$rekeningBelanja['q']."' and k = '".$rekeningBelanja['k']."' and l = '".$rekeningBelanja['l']."'  and m = '".$rekeningBelanja['m']."'  and n = '".$rekeningBelanja['n']."' and o = '".$rekeningBelanja['o']."' and (j!='000' or rincian_perhitungan !='') $kondisiBlackListSubRincian ")) == 0){
...
function Laporan($xls =FALSE){
		...
		$arrKondisi = array();
		$grabSKPD = mysql_fetch_array(mysql_query("select * from skpd_report_rka_32 where username='$this->username'"));
	 ...
	 //items ----------------------
	  $this->form_fields = array(
			'1' => array(
						...
						<input type='text' value='Rp. ".number_format($jumlahHargaForm,2,',','.')."' readonly style='width:320px;'> ",
						 ),
			'2' => array(
						'label'=>'JENIS ALOKASI',
						'labelWidth'=>150,
	 ...
	 //items ----------------------
	  $this->form_fields = array(
			'1' => array(
						'label'=>'JUMLAH HARGA',
						'labelWidth'=>150,
						'value'=>"<input type='hidden' name='jumlahHargaForm' id ='jumlahHargaForm'  value='$jumlahHargaForm'> <input type='text' value='Rp. ".number_format($jumlahHargaForm,2,',','.')."' readonly style='width:320px;'>",
						 ),
			'2' => array(
						'label'=>'JENIS ALOKASI',
						'labelWidth'=>150,
	 ...
	 //items ----------------------
	  $this->form_fields = array(
			'namaPekerjaan' => array(
						...
						'labelWidth'=>130,
						'value'=>"<input type='text' name='namaPekerjaan' id='namaPekerjaan' style='width:320px;'>",
						 ),
	 ...
	 //items ----------------------
	  $this->form_fields = array(
			'namaPekerjaan' => array(
						'label'=>'NAMA PEKERJAAN',
						'labelWidth'=>130,
						'value'=>"<input type='text' name='namaPekerjaan' id='namaPekerjaan' value='$namaPekerjaan' style='width:320px;'>",
						 ),
...
$rkaPPKD32->urutTerakhir = $nomorUrut;
$rkaPPKD32->tahun = $tahun;
$rkaPPKD32->jenisAnggaran = $jenisAnggaran;
$rkaPPKD32->idTahap = $idTahap;
$rkaPPKD32->username = $_COOKIE['coID'];

$rkaPPKD32->wajibValidasi = $Main->wajibValidasi;
if($Main->wajibValidasi == TRUE){
	$rkaPPKD32->sqlValidasi = " and status_validasi ='1' ";
}else{
	$rkaPPKD32->sqlValidasi = " ";
}
...
$rkaPPKD32->provinsi = $setting['provinsi'];
$rkaPPKD32->kota = $setting['kota'];
$rkaPPKD32->pengelolaBarang = $setting['pengelolaBarang'];
$rkaPPKD32->pejabatPengelolaBarang = $setting['pejabat'];
$rkaPPKD32->pengurusPengelolaBarang = $setting['pengurus'];
$rkaPPKD32->nipPengelola = $setting['nipPengelola'];
$rkaPPKD32->nipPengurus = $setting['nipPengurus'];
$rkaPPKD32->nipPejabat = $setting['nipPejabat'];


if($rkaPPKD32->jenisForm != "PENYUSUNAN"){
$get1 = mysql_fetch_array(mysql_query("select max(id_anggaran)  from view_rka_ppkd_3_2 where jenis_form_modul = 'PENYUSUNAN' "));
	...
	$rkaPPKD32->currentTahap = $arrayHasil['currentTahap'];
			$rkaPPKD32->jenisForm = "";
			$rkaPPKD32->jenisFormTerakhir = "PENYUSUNAN";
			$rkaPPKD32->idTahap = $get2['id_tahap'];
			$rkaPPKD32->tahun = $get2['tahun'];
			$rkaPPKD32->nomorUrut = $get2['no_urut'];
			$rkaPPKD32->urutTerakhir = $get2['no_urut'];
}
