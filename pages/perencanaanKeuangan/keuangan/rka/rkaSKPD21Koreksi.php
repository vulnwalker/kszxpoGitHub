...
class rkaSKPD21KoreksiObj  extends DaftarObj2{	
	var $Prefix = 'rkaSKPD21Koreksi';
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='rkaSKPD21Koreksi.xls';
	var $namaModulCetak='RKA';
	var $Cetak_Judul = 'RKA';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'rkaSKPD21KoreksiForm';
	var $modul = "RKA-SKPD";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	...
	$getJumlahBarang = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$rkaSKPD21Koreksi_idplh'"));
	...
	$getPaguYangTelahTerpakai = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as paguYangTerpakai from view_rka_2_1 where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1'  and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'  and no_urut = '$this->nomorUrut' and id_anggaran!='$rkaSKPD21Koreksi_idplh' "));
	$sisaPaguIndikatif = $paguIndikatif - $getPaguYangTelahTerpakai['paguYangTerpakai'];
	 ...
	 if(mysql_num_rows(mysql_query("select * from view_rka_2_1 where c1='0' and f = '00' and k = '$k' and l ='$l' and m='$m' and n='$n' and o='$o'  and id_tahap='$this->idTahap' ")) > 0){
 	 ...
 	 if(empty($cmbJenisRKAForm) ){
		...
		$query = VulnWalkerUpdate('tabel_anggaran',$data,"id_anggaran = '$rkaSKPD21Koreksi_idplh'");
		mysql_query($query);
	 }
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){	
		...
		case 'Gruping':{	
			...
			if(!isset($rkaSKPD21Koreksi_cb)){
				$err = "Pilih Data";
			}
			$dt = implode(',',$rkaSKPD21Koreksi_cb);
		...
		case 'sesuai':{
			...
			$getrkaSKPD21Koreksinya = mysql_fetch_array(mysql_query($queryRows));
			foreach ($getrkaSKPD21Koreksinya as $key => $value) { 
				  $$key = $value; 
			} 
			$cmbUrusanForm = $c1;
			$cmbBidangForm = $c;
		...
		case 'editTab':{
			$id = $_REQUEST['rkaSKPD21Koreksi_cb'];
			foreach ($_REQUEST as $key => $value) { 
				  $$key = $value; 
			} 
	...
	<A href=\"pages.php?Pg=rkaSKPD221Koreksi\" title='RKA-SKPD MURNI'  > RKA-SKPD 2.2.1 </a> |
	<A href=\"pages.php?Pg=rkaSKPD22Koreksi\" title='RKA-SKPKD MURNI'  > RKA-SKPD 2.2 </a> |
	<A href=\"pages.php?Pg=rkaSKPD21Koreksi\" title='RKA-SKPKD MURNI' style='color:blue;' > RKA-SKPD 2.1 </a> |
   ...
   function setPage_OtherScript(){
		...
		return 	
			...
			<script type='text/javascript' src='js/perencanaanKeuangan/keuangan/rka/rkaSKPD21Koreksi.js' language='JavaScript' ></script> 
	 ...
	 $this->form_height = 400;
	  if ($this->form_fmST==0) {
		...
		foreach ($dt as $key => $value) { 
		 ...
		 $jenis_rka = $jenis_rka;
		 $cmbJenisRKA = cmbArray('cmbJenisRKAForm',$jenis_rka,$arrayJenisRKA,'-- JENIS RKA --','onchange=rkaSKPD21Koreksi.unlockFindRekening();');
	 	 if(empty($jenis_rka)){
	  ...
	  $this->form_fields = array(
			...
			'kode7' => array( 
						...
						'value'=> "<input type='text' id='kodeRekening' name='kodeRekening' style='width:120px;' readonly value = '$kodeRekening'> &nbsp&nbsp
						 ...
						 <button type='button' id='findRekening' onclick=rkaSKPD21Koreksi.findRekening('$jenis_rka'); $tergantungJenis> CARI </button> "
						 ),
			'kode8' => array( 
						'label'=>'HARGA SATUAN',
						'labelWidth'=>150, 
						'value'=> "<input type='text' name='hargaSatuan' id='hargaSatuan' value='$hargaSatuan' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='rkaSKPD21Koreksi.bantu();' > <span id='bantu' style='color:red;'> </span>"
						 ),	
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	foreach ($_COOKIE as $key => $value) { 
		...
		$urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=rkaSKPD21Koreksi.refreshList(true);','-- URUSAN --');
		
		$codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
		$bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=rkaSKPD21Koreksi.refreshList(true);','-- BIDANG --');
		
		$codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
		$skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=rkaSKPD21Koreksi.refreshList(true);','-- SKPD --');
		...
		$unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=rkaSKPD21Koreksi.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=rkaSKPD21Koreksi.refreshList(true);','-- SUB UNIT --');
...
$rkaSKPD21Koreksi = new rkaSKPD21KoreksiObj();
...
$rkaSKPD21Koreksi->jenisForm = $jenisForm;
$rkaSKPD21Koreksi->nomorUrut = $nomorUrut;
$rkaSKPD21Koreksi->urutTerakhir = $nomorUrut;
$rkaSKPD21Koreksi->tahun = $tahun;
$rkaSKPD21Koreksi->jenisAnggaran = $jenisAnggaran;
$rkaSKPD21Koreksi->idTahap = $idTahap;
$rkaSKPD21Koreksi->username = $_COOKIE['coID'];
$getNamaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$idTahap'"));
$rkaSKPD21Koreksi->namaTahap = $getNamaTahap['nama_tahap'];

$rkaSKPD21Koreksi->wajibValidasi = $Main->wajibValidasi;
if($Main->wajibValidasi == TRUE){
	$rkaSKPD21Koreksi->sqlValidasi = " and status_validasi ='1' ";
}else{
	$rkaSKPD21Koreksi->sqlValidasi = " ";
}

if(empty($rkaSKPD21Koreksi->tahun)){
	...
	$rkaSKPD21Koreksi->tahun  = $get2['tahun'];
	$rkaSKPD21Koreksi->jenisAnggaran = $get2['jenis_anggaran'];
	$rkaSKPD21Koreksi->urutTerakhir = $get2['no_urut'];
	$rkaSKPD21Koreksi->nomorUrut = $get2['no_urut'];
	$rkaSKPD21Koreksi->jenisFormTerakhir = $get2['jenis_form_modul'];
	$rkaSKPD21Koreksi->urutSebelumnya = $rkaSKPD21Koreksi->urutTerakhir - 1;
	...
	$rkaSKPD21Koreksi->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$rkaSKPD21Koreksi->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];
	
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$rkaSKPD21Koreksi->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$rkaSKPD21Koreksi->idTahap'"));
	$rkaSKPD21Koreksi->currentTahap = $getCurrenttahap['nama_tahap'];
	
	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$rkaSKPD21Koreksi->idTahap'"));
	$rkaSKPD21Koreksi->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$rkaSKPD21Koreksi->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$rkaSKPD21Koreksi->provinsi = $setting['provinsi'];
$rkaSKPD21Koreksi->kota = $setting['kota'];
$rkaSKPD21Koreksi->pengelolaBarang = $setting['pengelolaBarang'];
$rkaSKPD21Koreksi->pejabatPengelolaBarang = $setting['pejabat'];
$rkaSKPD21Koreksi->pengurusPengelolaBarang = $setting['pengurus'];
$rkaSKPD21Koreksi->nipPengelola = $setting['nipPengelola'];
$rkaSKPD21Koreksi->nipPengurus = $setting['nipPengurus'];
$rkaSKPD21Koreksi->nipPejabat = $setting['nipPejabat'];


if($rkaSKPD21Koreksi->jenisForm != "KOREKSI"){
$get1 = mysql_fetch_array(mysql_query("select max(id_anggaran)  from view_rka_2_1 where jenis_form_modul = 'KOREKSI' "));
	...
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$rkaSKPD21Koreksi->currentTahap = $arrayHasil['currentTahap'];
			$rkaSKPD21Koreksi->jenisForm = "";
			$rkaSKPD21Koreksi->jenisFormTerakhir = "KOREKSI";
			$rkaSKPD21Koreksi->idTahap = $get2['id_tahap'];
			$rkaSKPD21Koreksi->nomorUrut = $get2['no_urut'];
			$rkaSKPD21Koreksi->urutTerakhir = $get2['no_urut'];
