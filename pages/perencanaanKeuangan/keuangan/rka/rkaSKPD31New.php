...
class rkaSKPD31NewObj  extends DaftarObj2{
	var $Prefix = 'rkaSKPD31New';
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='rkaSKPD31New.xls';
	var $namaModulCetak='RKA';
	var $Cetak_Judul = 'RKA';
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'rkaSKPD31NewForm';
	var $modul = "RKA-SKPD";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
   ...
   // if( $err=='' && $kategori =='' ) $err= 'Kategori Belum Di Pilih !!';
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
      ...
      break;
			...
			case 'editSubRincian':{
					...
					$eventKeyup = 'document.getElementById("formatjumlah'.$id.'").innerHTML = rkaSKPD31New.formatCurrency(this.value); ';
		...
		case 'Gruping':{
			...
			if(!isset($rkaSKPD31New_cb)){
				$err = "Pilih Data";
			}
			$dt = implode(',',$rkaSKPD31New_cb);
    ...
    break;
		...
		case 'sesuai':{
			...
			$getrkaSKPD31Newnya = mysql_fetch_array(mysql_query($queryRows));
			foreach ($getrkaSKPD31Newnya as $key => $value) {
				  $$key = $value;
			}
			$cmbUrusanForm = $c1;
			$cmbBidangForm = $c;
	    ...
	    case 'SaveValid':{
	    	foreach ($_REQUEST as $key => $value) {
			 ...
			 $query = VulnWalkerUpdate("tabel_anggaran",$data," id_anggaran = '$rkaSKPD31New_idplh'");
			 mysql_query($query);
	 ...
	 <A href=\"pages.php?Pg=rkaSKPD31New\" title='PENDAPATAN'  style='color:blue;'> PENDAPATAN </a> |
	 ...
	 <tr><td class=\"menudottedline\" width=\"40%\" height=\"20\" style='text-align:right'><B>
	 <A href=\"pages.php?Pg=rkaSKPD31New\" title='PENDAPATAN ASLI DAERAH' style='color:blue;' > PENDAPATAN ASLI DAERAH </a> |
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	foreach ($_COOKIE as $key => $value) {
		...
		$urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=rkaSKPD31New.refreshList(true);','-- URUSAN --');

		$codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
		$bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=rkaSKPD31New.refreshList(true);','-- BIDANG --');

		$codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
		$skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=rkaSKPD31New.refreshList(true);','-- SKPD --');
		...
		$unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=rkaSKPD31New.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=rkaSKPD31New.refreshList(true);','-- SUB UNIT --');
...
function Laporan($dt){
   ...
   $dari = $_REQUEST['dari'];
     ...
     $arrayJenisLaporan = array(
                 array('RKASKPD31', 'RKA-SKPD 3 MURNI'),
    ...
    $this->form_menubawah =
      ...
      "<input type='button' value='Batal' onclick ='rkaSKPD31.Close()' >";
...
function RKASKPD31($xls =FALSE){
    ...
    $arrKondisi[] = "jenis_anggaran = '$this->jenisAnggaran'";
		...
		$qry ="select * from view_rka_3_1 where $Kondisi order by urut ";
...
$rkaSKPD31New = new rkaSKPD31NewObj();
...
$rkaSKPD31New->jenisForm = $jenisForm;
$rkaSKPD31New->nomorUrut = $nomorUrut;
$rkaSKPD31New->urutTerakhir = $nomorUrut;
$rkaSKPD31New->tahun = $tahun;
$rkaSKPD31New->jenisAnggaran = $jenisAnggaran;
$rkaSKPD31New->idTahap = $idTahap;
$rkaSKPD31New->username = $_COOKIE['coID'];

$rkaSKPD31New->wajibValidasi = $Main->wajibValidasi;
if($Main->wajibValidasi == TRUE){
	$rkaSKPD31New->sqlValidasi = " and status_validasi ='1' ";
}else{
	$rkaSKPD31New->sqlValidasi = " ";
}

if(empty($rkaSKPD31New->tahun)){
	...
	$rkaSKPD31New->tahun  = $get2['tahun'];
	$rkaSKPD31New->jenisAnggaran = $get2['jenis_anggaran'];
	$rkaSKPD31New->urutTerakhir = $get2['no_urut'];
	$rkaSKPD31New->jenisFormTerakhir = $get2['jenis_form_modul'];
	$rkaSKPD31New->urutSebelumnya = $rkaSKPD31New->urutTerakhir - 1;
	...
	$rkaSKPD31New->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$rkaSKPD31New->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];

	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$rkaSKPD31New->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$rkaSKPD31New->idTahap'"));
	$rkaSKPD31New->currentTahap = $getCurrenttahap['nama_tahap'];

	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$rkaSKPD31New->idTahap'"));
	$rkaSKPD31New->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$rkaSKPD31New->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$rkaSKPD31New->provinsi = $setting['provinsi'];
$rkaSKPD31New->kota = $setting['kota'];
$rkaSKPD31New->pengelolaBarang = $setting['pengelolaBarang'];
$rkaSKPD31New->pejabatPengelolaBarang = $setting['pejabat'];
$rkaSKPD31New->pengurusPengelolaBarang = $setting['pengurus'];
$rkaSKPD31New->nipPengelola = $setting['nipPengelola'];
$rkaSKPD31New->nipPengurus = $setting['nipPengurus'];
$rkaSKPD31New->nipPejabat = $setting['nipPejabat'];


if($rkaSKPD31New->jenisForm != "PENYUSUNAN"){
$get1 = mysql_fetch_array(mysql_query("select max(id_anggaran)  from view_rka_1 where jenis_form_modul = 'PENYUSUNAN' "));
	...
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$rkaSKPD31New->currentTahap = $arrayHasil['currentTahap'];
			$rkaSKPD31New->jenisForm = "";
			$rkaSKPD31New->jenisFormTerakhir = "PENYUSUNAN";
			$rkaSKPD31New->idTahap = $get2['id_tahap'];
			$rkaSKPD31New->tahun = $get2['tahun'];
			$rkaSKPD31New->nomorUrut = $get2['no_urut'];
			$rkaSKPD31New->urutTerakhir = $get2['no_urut'];
}
