...
class rkaPPKD1PADObj  extends DaftarObj2{
	var $Prefix = 'rkaPPKD1PAD';
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='rkaPPKD1PAD.xls';
	var $namaModulCetak='RKA';
	var $Cetak_Judul = 'RKA';
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'rkaPPKD1PADForm';
	var $modul = "RKA-PPKD";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	...
	$getJumlahBarang = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$rkaPPKD1PAD_idplh'"));
	...
	$getPaguYangTelahTerpakai = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as paguYangTerpakai from view_rka_ppkd_1 where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1'  and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'  and no_urut = '$this->nomorUrut' and id_anggaran!='$rkaPPKD1PAD_idplh' "));
	$sisaPaguIndikatif = $paguIndikatif - $getPaguYangTelahTerpakai['paguYangTerpakai'];
	 ...
	 if(mysql_num_rows(mysql_query("select * from view_rka_ppkd_1 where c1='0' and f = '00' and k = '$k' and l ='$l' and m='$m' and n='$n' and o='$o'  and id_tahap='$this->idTahap' ")) > 0){
 	 ...
 	 if(empty($cmbJenisRKAForm) ){
		...
		$query = VulnWalkerUpdate('tabel_anggaran',$data,"id_anggaran = '$rkaPPKD1PAD_idplh'");
		mysql_query($query);
	 }
	...
	function set_selector_other($tipe){
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){
		...
		case 'Gruping':{
			...
			if(!isset($rkaPPKD1PAD_cb)){
				$err = "Pilih Data";
			}
			$dt = implode(',',$rkaPPKD1PAD_cb);
		...
		case 'clearTemp':{
				$username =$_COOKIE['coID'];
				mysql_query("delete from temp_rekening_rka_pendapatan_pad_ppkd where username ='$username'");
				mysql_query("delete from temp_rincian_pendapatan_pad_ppkd where username ='$username'");
				mysql_query("delete from temp_sub_rincian_pendapatan_pad_ppkd where username ='$username'");
				mysql_query("delete from temp_remove_rka_1_pad_ppkd where username ='$username'");
				foreach ($_REQUEST as $key => $value) {
			 	 $$key = $value;
				}
				...
				while ($rows = mysql_fetch_array($getALlData)) {
						...
						if(mysql_num_rows(mysql_query("select * from temp_rekening_rka_pendapatan_pad_ppkd where username = '$this->username' and k='$k'  and l ='$l' and m='$m' and n='$n' and o='$o'")) == 0){
								$dataRekening = array(
																				'k'=> $k,
																				'l'=> $l,
																				'm'=> $m,
								...
								$queryRekening = VulnWalkerInsert('temp_rekening_rka_pendapatan_pad_ppkd',$dataRekening);
								mysql_query($queryRekening);
						}
						...
						$getIdRekeningBelanja = mysql_fetch_array(mysql_query("select * from temp_rekening_rka_pendapatan_pad_ppkd where k ='$k' and l='$l' and m='$m' and n='$n' and o='$o' and username = '$this->username'"));
						if(mysql_num_rows(mysql_query("select * from temp_rincian_pendapatan_pad_ppkd where username = '$this->username' and id_rekening_belanja = '".$getIdRekeningBelanja['id']."' and uraian = '".$getUraianRincianBelanja['nama_rincian_belanja']."'")) == 0){
								...
								$queryRincianBelanja = VulnWalkerInsert('temp_rincian_pendapatan_pad_ppkd',$dataRincianBelanja);
						...
						$getIdRincianBelanja = mysql_fetch_array(mysql_query("select * from temp_rincian_pendapatan_pad_ppkd where username = '$this->username' and uraian = '".$getUplineRincianBelanja['nama_rincian_belanja']."'"));
						...
						$querySubRincian = VulnWalkerInsert("temp_sub_rincian_pendapatan_pad_ppkd",$dataSubRincian);
		...
		case 'sesuai':{
			...
			$getrkaPPKD1PADnya = mysql_fetch_array(mysql_query($queryRows));
			foreach ($getrkaPPKD1PADnya as $key => $value) {
				  $$key = $value;
			}
			$cmbUrusanForm = $c1;
			$cmbBidangForm = $c;
 	...
 	<tr><td class=\"menudottedline\" width=\"40%\" height=\"20\" style='text-align:right'><B>
 	<A href=\"pages.php?Pg=rkaPPKD1PAD\" title='PENDAPATAN ASLI DAERAH' style='color:blue;' > PENDAPATAN ASLI DAERAH </a> |
   ...
   function setPage_OtherScript(){
		...
		return
			...
			<script type='text/javascript' src='js/perencanaanKeuangan/keuangan/rka/rkaPPKD1PAD.js' language='JavaScript' ></script>
	 ...
	 $this->form_height = 400;
	  if ($this->form_fmST==0) {
		...
		foreach ($dt as $key => $value) {
		 ...
		 $jenis_rka = $jenis_rka;
		 $cmbJenisRKA = cmbArray('cmbJenisRKAForm',$jenis_rka,$arrayJenisRKA,'-- JENIS RKA --','onchange=rkaPPKD1PAD.unlockFindRekening();');
	 	 if(empty($jenis_rka)){
	  ...
	  $this->form_fields = array(
			...
			'kode7' => array(
						...
						'value'=> "<input type='text' id='kodeRekening' name='kodeRekening' style='width:120px;' readonly value = '$kodeRekening'> &nbsp&nbsp
						 ...
						 <button type='button' id='findRekening' onclick=rkaPPKD1PAD.findRekening('$jenis_rka'); $tergantungJenis> CARI </button> "
						 ),
			'kode8' => array(
						'label'=>'HARGA SATUAN',
						'labelWidth'=>150,
						'value'=> "<input type='text' name='hargaSatuan' id='hargaSatuan' value='$hargaSatuan' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='rkaPPKD1PAD.bantu();' > <span id='bantu' style='color:red;'> </span>"
						 ),
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	foreach ($_COOKIE as $key => $value) {
		...
		$urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=rkaPPKD1PAD.refreshList(true);','-- URUSAN --');

		$codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
		$bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=rkaPPKD1PAD.refreshList(true);','-- BIDANG --');

		$codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
		$skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=rkaPPKD1PAD.refreshList(true);','-- SKPD --');
		...
		$unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=rkaPPKD1PAD.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=rkaPPKD1PAD.refreshList(true);','-- SUB UNIT --');
...
$rkaPPKD1PAD = new rkaPPKD1PADObj();
...
$rkaPPKD1PAD->jenisForm = $jenisForm;
$rkaPPKD1PAD->nomorUrut = $nomorUrut;
$rkaPPKD1PAD->urutTerakhir = $nomorUrut;
$rkaPPKD1PAD->tahun = $tahun;
$rkaPPKD1PAD->jenisAnggaran = $jenisAnggaran;
$rkaPPKD1PAD->idTahap = $idTahap;
$rkaPPKD1PAD->username = $_COOKIE['coID'];

$rkaPPKD1PAD->wajibValidasi = $Main->wajibValidasi;
if($Main->wajibValidasi == TRUE){
	$rkaPPKD1PAD->sqlValidasi = " and status_validasi ='1' ";
}else{
	$rkaPPKD1PAD->sqlValidasi = " ";
}

if(empty($rkaPPKD1PAD->tahun)){
	...
	$rkaPPKD1PAD->tahun  = $get2['tahun'];
	$rkaPPKD1PAD->jenisAnggaran = $get2['jenis_anggaran'];
	$rkaPPKD1PAD->urutTerakhir = $get2['no_urut'];
	$rkaPPKD1PAD->jenisFormTerakhir = $get2['jenis_form_modul'];
	$rkaPPKD1PAD->urutSebelumnya = $rkaPPKD1PAD->urutTerakhir - 1;
	...
	$rkaPPKD1PAD->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$rkaPPKD1PAD->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];

	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$rkaPPKD1PAD->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$rkaPPKD1PAD->idTahap'"));
	$rkaPPKD1PAD->currentTahap = $getCurrenttahap['nama_tahap'];

	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$rkaPPKD1PAD->idTahap'"));
	$rkaPPKD1PAD->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$rkaPPKD1PAD->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$rkaPPKD1PAD->provinsi = $setting['provinsi'];
$rkaPPKD1PAD->kota = $setting['kota'];
$rkaPPKD1PAD->pengelolaBarang = $setting['pengelolaBarang'];
$rkaPPKD1PAD->pejabatPengelolaBarang = $setting['pejabat'];
$rkaPPKD1PAD->pengurusPengelolaBarang = $setting['pengurus'];
$rkaPPKD1PAD->nipPengelola = $setting['nipPengelola'];
$rkaPPKD1PAD->nipPengurus = $setting['nipPengurus'];
$rkaPPKD1PAD->nipPejabat = $setting['nipPejabat'];


if($rkaPPKD1PAD->jenisForm != "PENYUSUNAN"){
$get1 = mysql_fetch_array(mysql_query("select max(id_anggaran)  from view_rka_ppkd_1 where jenis_form_modul = 'PENYUSUNAN' "));
	...
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$rkaPPKD1PAD->currentTahap = $arrayHasil['currentTahap'];
			$rkaPPKD1PAD->jenisForm = "";
			$rkaPPKD1PAD->jenisFormTerakhir = "PENYUSUNAN";
			$rkaPPKD1PAD->idTahap = $get2['id_tahap'];
			$rkaPPKD1PAD->tahun = $get2['tahun'];
			$rkaPPKD1PAD->nomorUrut = $get2['no_urut'];
			$rkaPPKD1PAD->urutTerakhir = $get2['no_urut'];
}
