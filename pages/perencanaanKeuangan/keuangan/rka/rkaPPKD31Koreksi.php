...
class rkaPPKD31KoreksiObj  extends DaftarObj2{	
	var $Prefix = 'rkaPPKD31Koreksi';
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='rkaPPKD31Koreksi.xls';
	var $namaModulCetak='RKA';
	var $Cetak_Judul = 'RKA';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'rkaPPKD31KoreksiForm';
	var $modul = "RKA-PPKD";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	...
	$getJumlahBarang = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$rkaPPKD31Koreksi_idplh'"));
	...
	$sisaPaguIndikatif = $paguIndikatif - $getPaguYangTelahTerpakai['paguYangTerpakai'];
	 ...
	 if(mysql_num_rows(mysql_query("select * from view_rka_ppkd_3_1 where c1='0' and f = '00' and k = '$k' and l ='$l' and m='$m' and n='$n' and o='$o'  and id_tahap='$this->idTahap' ")) > 0){
 	 ...
 	 if(empty($cmbJenisRKAForm) ){
		...
		$query = VulnWalkerUpdate('tabel_anggaran',$data,"id_anggaran = '$rkaPPKD31Koreksi_idplh'");
		mysql_query($query);
	 }
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){	
		...
		case 'Gruping':{	
			...
			if(!isset($rkaPPKD31Koreksi_cb)){
				$err = "Pilih Data";
			}
			$dt = implode(',',$rkaPPKD31Koreksi_cb);
		...
		case 'sesuai':{
			...
			$getrkaPPKD31Koreksinya = mysql_fetch_array(mysql_query($queryRows));
			foreach ($getrkaPPKD31Koreksinya as $key => $value) { 
				  $$key = $value; 
			} 
			$cmbUrusanForm = $c1;
			$cmbBidangForm = $c;
		...
		case 'editTab':{
			$id = $_REQUEST['rkaPPKD31Koreksi_cb'];
			foreach ($_REQUEST as $key => $value) { 
				  $$key = $value; 
			} 
	...
	<A href=\"pages.php?Pg=rkaPPKD31Koreksi\" title='RKA-PPKD MURNI'  style='color:blue;' > RKA-PPKD 3.1 </a> |
   ...
   function setPage_OtherScript(){
		...
		return 	
			...
			<script type='text/javascript' src='js/perencanaanKeuangan/keuangan/rka/rkaPPKD31Koreksi.js' language='JavaScript' ></script> 
	 ...
	 $this->form_height = 400;
	  if ($this->form_fmST==0) {
		...
		foreach ($dt as $key => $value) { 
		 ...
		 $jenis_rka = $jenis_rka;
		 $cmbJenisRKA = cmbArray('cmbJenisRKAForm',$jenis_rka,$arrayJenisRKA,'-- JENIS RKA --','onchange=rkaPPKD31Koreksi.unlockFindRekening();');
	 	 if(empty($jenis_rka)){
	  ...
	  $this->form_fields = array(
			...
			'kode7' => array( 
						...
						'value'=> "<input type='text' id='kodeRekening' name='kodeRekening' style='width:120px;' readonly value = '$kodeRekening'> &nbsp&nbsp
						 ...
						 <button type='button' id='findRekening' onclick=rkaPPKD31Koreksi.findRekening('$jenis_rka'); $tergantungJenis> CARI </button> "
						 ),
			'kode8' => array( 
						'label'=>'HARGA SATUAN',
						'labelWidth'=>150, 
						'value'=> "<input type='text' name='hargaSatuan' id='hargaSatuan' value='$hargaSatuan' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='rkaPPKD31Koreksi.bantu();' > <span id='bantu' style='color:red;'> </span>"
						 ),	
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	foreach ($_COOKIE as $key => $value) { 
		...
		$urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=rkaPPKD31Koreksi.refreshList(true);','-- URUSAN --');
		
		$codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
		$bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=rkaPPKD31Koreksi.refreshList(true);','-- BIDANG --');
		
		$codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
		$skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=rkaPPKD31Koreksi.refreshList(true);','-- SKPD --');
		...
		$unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=rkaPPKD31Koreksi.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=rkaPPKD31Koreksi.refreshList(true);','-- SUB UNIT --');
...
$rkaPPKD31Koreksi = new rkaPPKD31KoreksiObj();
...
$rkaPPKD31Koreksi->jenisForm = $jenisForm;
$rkaPPKD31Koreksi->nomorUrut = $nomorUrut;
$rkaPPKD31Koreksi->urutTerakhir = $nomorUrut;
$rkaPPKD31Koreksi->tahun = $tahun;
$rkaPPKD31Koreksi->jenisAnggaran = $jenisAnggaran;
$rkaPPKD31Koreksi->idTahap = $idTahap;
$rkaPPKD31Koreksi->username = $_COOKIE['coID'];
$getNamaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$idTahap'"));
$rkaPPKD31Koreksi->namaTahap = $getNamaTahap['nama_tahap'];


$rkaPPKD31Koreksi->wajibValidasi = $Main->wajibValidasi;
if($Main->wajibValidasi == TRUE){
	$rkaPPKD31Koreksi->sqlValidasi = " and status_validasi ='1' ";
}else{
	$rkaPPKD31Koreksi->sqlValidasi = " ";
}

if(empty($rkaPPKD31Koreksi->tahun)){
	...
	/*$rkaPPKD31Koreksi->tahun = "select max(id_anggaran) as max from view_rka_ppkd_3_1 where nama_modul = 'rkaPPKD31Koreksi'";*/
	$rkaPPKD31Koreksi->tahun  = $get2['tahun'];
	$rkaPPKD31Koreksi->jenisAnggaran = $get2['jenis_anggaran'];
	$rkaPPKD31Koreksi->urutTerakhir = $get2['no_urut'];
	$rkaPPKD31Koreksi->jenisFormTerakhir = $get2['jenis_form_modul'];
	$rkaPPKD31Koreksi->urutSebelumnya = $rkaPPKD31Koreksi->urutTerakhir - 1;
	...
	$rkaPPKD31Koreksi->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$rkaPPKD31Koreksi->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];
	
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$rkaPPKD31Koreksi->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$rkaPPKD31Koreksi->idTahap'"));
	$rkaPPKD31Koreksi->currentTahap = $getCurrenttahap['nama_tahap'];
	
	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$rkaPPKD31Koreksi->idTahap'"));
	$rkaPPKD31Koreksi->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$rkaPPKD31Koreksi->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$rkaPPKD31Koreksi->provinsi = $setting['provinsi'];
$rkaPPKD31Koreksi->kota = $setting['kota'];
$rkaPPKD31Koreksi->pengelolaBarang = $setting['pengelolaBarang'];
$rkaPPKD31Koreksi->pejabatPengelolaBarang = $setting['pejabat'];
$rkaPPKD31Koreksi->pengurusPengelolaBarang = $setting['pengurus'];
$rkaPPKD31Koreksi->nipPengelola = $setting['nipPengelola'];
$rkaPPKD31Koreksi->nipPengurus = $setting['nipPengurus'];
$rkaPPKD31Koreksi->nipPejabat = $setting['nipPejabat'];

if($rkaPPKD31Koreksi->jenisForm != "KOREKSI"){
$get1 = mysql_fetch_array(mysql_query("select max(id_anggaran)  from view_rka_ppkd_3_1 where jenis_form_modul = 'KOREKSI' "));
	...
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$rkaPPKD31Koreksi->currentTahap = $arrayHasil['currentTahap'];
			$rkaPPKD31Koreksi->jenisForm = "";
			$rkaPPKD31Koreksi->jenisFormTerakhir = "KOREKSI";
			$rkaPPKD31Koreksi->idTahap = $get2['id_tahap'];
			$rkaPPKD31Koreksi->nomorUrut = $get2['no_urut'];
			$rkaPPKD31Koreksi->urutTerakhir = $get2['no_urut'];
