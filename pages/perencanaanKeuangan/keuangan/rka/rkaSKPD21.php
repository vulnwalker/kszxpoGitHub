...
class rkaSKPD21Obj  extends DaftarObj2{
	...
	$getPaguYangTelahTerpakai = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as paguYangTerpakai from view_rka_2_1 where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1'  and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'  and no_urut = '$this->nomorUrut' and id_anggaran!='$rkaSKPD21_idplh' "));
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){
			...
			case 'editSubRincian':{
					...
					$eventKeyup = 'document.getElementById("formatjumlah'.$id.'").innerHTML = rkaSKPD21.formatCurrency(this.value); ';
			...
			case 'saveEditRekening':{
					...
					$getOldRekening = mysql_fetch_array(mysql_query("select * from view_rka_2_1 where id_anggaran	 = '$id'"));
					...
					if(mysql_num_rows(mysql_query("select * from view_rka_2_1 where c1 = '0' and concat(bk,'.',ck,'.',dk,'.',p,'.',q,'.',k,'.',l,'.',m,'.',n,'.',o) = '$concatKondisiNew'")) == 0){
							$arrayRekening = array(
												'c1' => '0',
												'c' => '00',
												'd' => '00',
		...
		case 'Gruping':{
			...
			if(!isset($rkaSKPD21_cb)){
				$err = "Pilih Data";
			}
			$dt = implode(',',$rkaSKPD21_cb);
   ...
   function setPage_HeaderOther(){
	...
	<A href=\"pages.php?Pg=rkaSKPD21\" title='BELANJA' style='color:blue;' > BELANJA </a> |
	...
	<A href=\"pages.php?Pg=rkaSKPD21\" title='TIDAK LANGSUNG' style='color:blue;' > TIDAK LANGSUNG </a> |
   ...
   function setPage_OtherScript(){
		...
		return
			...
			<script type='text/javascript' src='js/perencanaanKeuangan/keuangan/rka/rkaSKPD21.js' language='JavaScript' ></script>
	...
	function setKolomData($no, $isi, $Mode, $TampilCheckBox){
	 ...
	 foreach ($_REQUEST as $key => $value) {
		...
		if( !empty($k) && !empty($l) && !empty($m)  && !empty($n)  && !empty($o)  && empty($id_rincian_belanja) ){
				 ...
				 $getSumJumlahHarga = mysql_fetch_array(mysql_query("select sum(jumlah_harga) from view_rka_2_1 where id_tahap = '$this->idTahap' and k ='$k' and l='$l' and m='$m' and n='$n' and o='$o' $kondisiSKPD $kondisiRekening"));
				 $jumlahHarga = number_format($getSumJumlahHarga['sum(jumlah_harga)'] ,2,',','.');
				 $TampilCheckBox = "";
		}elseif(!empty($k) && !empty($l) && !empty($m)  && !empty($n)  && !empty($o)  && !empty($id_rincian_belanja)  && empty($rincian_perhitungan)){
				 //RincianBelanja
				 ...
				 $getSumJumlahHarga = mysql_fetch_array(mysql_query("select sum(jumlah_harga) from view_rka_2_1 where id_tahap = '$this->idTahap' and k ='$k' and l='$l' and m='$m' and n='$n' and o='$o' and id_rincian_belanja='$id_rincian_belanja' $kondisiSKPD $kondisiRekening"));
				 $jumlahHarga = number_format($getSumJumlahHarga['sum(jumlah_harga)'] ,2,',','.');
				 $TampilCheckBox = "";
		}elseif( !empty($k) && !empty($l) && !empty($m)  && !empty($n)  && !empty($o)  && !empty($id_rincian_belanja)  && !empty($rincian_perhitungan) ){
				 //SubRincianBelanja
...
function formAlokasi($dt){
	...
	function formRincianVolume($dt){
	 ...
	 $jumlahHargaForm = $dt;
	 $username =$_COOKIE['coID'];
	//  $getRincianVolume = mysql_fetch_array(mysql_query("select * from view_rka_2_1 where id_anggaran ='$dt'"));
...
$rkaSKPD21->urutTerakhir = $nomorUrut;
$rkaSKPD21->tahun = $tahun;
$rkaSKPD21->jenisAnggaran = $jenisAnggaran;
$rkaSKPD21->idTahap = $idTahap;
$rkaSKPD21->username = $_COOKIE['coID'];

$rkaSKPD21->wajibValidasi = $Main->wajibValidasi;
if($Main->wajibValidasi == TRUE){
	$rkaSKPD21->sqlValidasi = " and status_validasi ='1' ";
}else{
	$rkaSKPD21->sqlValidasi = " ";
}
...
$rkaSKPD21->provinsi = $setting['provinsi'];
$rkaSKPD21->kota = $setting['kota'];
$rkaSKPD21->pengelolaBarang = $setting['pengelolaBarang'];
$rkaSKPD21->pejabatPengelolaBarang = $setting['pejabat'];
$rkaSKPD21->pengurusPengelolaBarang = $setting['pengurus'];
$rkaSKPD21->nipPengelola = $setting['nipPengelola'];
$rkaSKPD21->nipPengurus = $setting['nipPengurus'];
$rkaSKPD21->nipPejabat = $setting['nipPejabat'];


if($rkaSKPD21->jenisForm != "PENYUSUNAN"){
$get1 = mysql_fetch_array(mysql_query("select max(id_anggaran)  from view_rka_2_1 where jenis_form_modul = 'PENYUSUNAN' "));
	...
	$rkaSKPD21->currentTahap = $arrayHasil['currentTahap'];
			$rkaSKPD21->jenisForm = "";
			$rkaSKPD21->jenisFormTerakhir = "PENYUSUNAN";
			$rkaSKPD21->idTahap = $get2['id_tahap'];
			$rkaSKPD21->tahun = $get2['tahun'];
			$rkaSKPD21->nomorUrut = $get2['no_urut'];
			$rkaSKPD21->urutTerakhir = $get2['no_urut'];
}
