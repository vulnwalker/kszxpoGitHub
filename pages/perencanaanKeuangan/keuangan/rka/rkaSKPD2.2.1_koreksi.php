...
class rkaSKPD221_v2_koreksiObj  extends DaftarObj2{	
	var $Prefix = 'rkaSKPD221_v2_koreksi';
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='rkaSKPD221_v2_koreksi.xls';
	var $namaModulCetak='RKA';
	var $Cetak_Judul = 'RKA';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'rkaSKPD221_v2_koreksiForm';
	var $modul = "RKA-SKPD";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	...
	$getJumlahBarang = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$rkaSKPD221_v2_koreksi_idplh'"));
	...
	$sisaPaguIndikatif = $paguIndikatif - $getPaguYangTelahTerpakai['paguYangTerpakai'];
	 ...
	 if(mysql_num_rows(mysql_query("select * from view_rka_2_2_1 where c1='0' and f = '00' and k = '$k' and l ='$l' and m='$m' and n='$n' and o='$o'  and id_tahap='$this->idTahap' ")) > 0){
 	 ...
 	 if(empty($cmbJenisRKAForm) ){
		...
		$query = VulnWalkerUpdate('tabel_anggaran',$data,"id_anggaran = '$rkaSKPD221_v2_koreksi_idplh'");
		mysql_query($query);
	 }
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){	
		...
		case 'Gruping':{	
			...
			if(!isset($rkaSKPD221_v2_koreksi_cb)){
				$err = "Pilih Data";
			}
			$dt = implode(',',$rkaSKPD221_v2_koreksi_cb);
		...
		case 'Remove':{
			$id = $_REQUEST['rkaSKPD221_v2_koreksi_cb'];
			foreach ($_REQUEST as $key => $value) { 
				  $$key = $value; 
			} 
		...
		case 'sesuai':{
			...
			$getrkaSKPD221_v2_koreksinya = mysql_fetch_array(mysql_query($queryRows));
			foreach ($getrkaSKPD221_v2_koreksinya as $key => $value) { 
				  $$key = $value; 
			} 
			$cmbUrusanForm = $c1;
			$cmbBidangForm = $c;
	...
	<A href=\"pages.php?Pg=rka-skpd-2.2.1_v2_koreksi\" title='RKA-SKPD MURNI' style='color:blue;' > RKA-SKPD 2.2.1 </a> |
   ...
   function setPage_OtherScript(){
		...
		return 	
			...
			<script type='text/javascript' src='js/perencanaan_v2/rka/rkaSKPD2.2.1_koreksi.js' language='JavaScript' ></script> 
	 ...
	 $this->form_height = 400;
	  if ($this->form_fmST==0) {
		...
		foreach ($dt as $key => $value) { 
		 ...
		 $jenis_rka = $jenis_rka;
		 $cmbJenisRKA = cmbArray('cmbJenisRKAForm',$jenis_rka,$arrayJenisRKA,'-- JENIS RKA --','onchange=rkaSKPD221_v2_koreksi.unlockFindRekening();');
	 	 if(empty($jenis_rka)){
	  ...
	  $this->form_fields = array(
			...
			'kode7' => array( 
						...
						'value'=> "<input type='text' id='kodeRekening' name='kodeRekening' style='width:120px;' readonly value = '$kodeRekening'> &nbsp&nbsp
						 ...
						 <button type='button' id='findRekening' onclick=rkaSKPD221_v2_koreksi.findRekening('$jenis_rka'); $tergantungJenis> CARI </button> "
						 ),
			'kode8' => array( 
						'label'=>'HARGA SATUAN',
						'labelWidth'=>150, 
						'value'=> "<input type='text' name='hargaSatuan' id='hargaSatuan' value='$hargaSatuan' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='rkaSKPD221_v2_koreksi.bantu();' > <span id='bantu' style='color:red;'> </span>"
						 ),	
...
/*		 $arrayResult = VulnWalkerTahap_v2('RKA');
	...
	function setKolomData($no, $isi, $Mode, $TampilCheckBox){
	 ...
	 //TAHAP PENYUSUNAN
			 ...
			 $aksi  = "<img src='images/administrator/images/valid.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkaSKPD221_v2_koreksi.sesuai('$id_anggaran');></img>&nbsp  &nbsp <img src='images/administrator/images/invalid.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkaSKPD221_v2_koreksi.koreksi('$id_anggaran');></img> ";
			 if ($rincian_perhitungan == '') {
			 $aksi = "";
			 }
			 if(empty($rincian_perhitungan)){
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	foreach ($_COOKIE as $key => $value) { 
		...
		$urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=rkaSKPD221_v2_koreksi.refreshList(true);','-- URUSAN --');
		
		$codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
		$bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=rkaSKPD221_v2_koreksi.refreshList(true);','-- BIDANG --');
		
		$codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
		$skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=rkaSKPD221_v2_koreksi.refreshList(true);','-- SKPD --');
		...
		$unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=rkaSKPD221_v2_koreksi.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=rkaSKPD221_v2_koreksi.refreshList(true);','-- SUB UNIT --');
	...
	$program = "<input type='hidden' id='bk' name='bk' value='$selectedBK'> <input type='hidden' id='ck' name='ck' value='$selectedCK'> <input type='hidden' id='hiddenP' name='hiddenP' value='$selectedP'>".cmbArray('p',$_REQUEST['p'],$arrayP,'-- PROGRAM --','onchange=rkaSKPD221_v2_koreksi.programChanged();');
	...
	$kegiatan = cmbArray('q',$_REQUEST['q'],$arrayQ,'-- KEGIATAN --','onchange=rkaSKPD221_v2_koreksi.refreshList(true);');
	...
	$cmbBelanja = cmbArray('cmbBelanja',$_REQUEST['cmbBelanja'],$arrayBelanja,'-- JENIS BELANJA--','onchange=rkaSKPD221_v2_koreksi.refreshList(true);');
...
$rkaSKPD221_v2_koreksi = new rkaSKPD221_v2_koreksiObj();
...
$rkaSKPD221_v2_koreksi->namaTahap = $getNamaTahap['nama_tahap'];

$rkaSKPD221_v2_koreksi->jenisForm = $jenisForm;
$rkaSKPD221_v2_koreksi->nomorUrut = $nomorUrut;
$rkaSKPD221_v2_koreksi->urutTerakhir = $nomorUrut;
$rkaSKPD221_v2_koreksi->tahun = $tahun;
$rkaSKPD221_v2_koreksi->jenisAnggaran = $jenisAnggaran;
$rkaSKPD221_v2_koreksi->idTahap = $idTahap;

$rkaSKPD221_v2_koreksi->username = $_COOKIE['coID'];

$rkaSKPD221_v2_koreksi->wajibValidasi = $Main->wajibValidasi;
if($Main->wajibValidasi == TRUE){
	$rkaSKPD221_v2_koreksi->sqlValidasi = " and status_validasi ='1' ";
}else{
	$rkaSKPD221_v2_koreksi->sqlValidasi = " ";
}


if(empty($rkaSKPD221_v2_koreksi->tahun)){
	...
	$rkaSKPD221_v2_koreksi->tahun  = $get2['tahun'];
	$rkaSKPD221_v2_koreksi->jenisAnggaran = $get2['jenis_anggaran'];
	$rkaSKPD221_v2_koreksi->urutTerakhir = $get2['no_urut'];
	$rkaSKPD221_v2_koreksi->jenisFormTerakhir = $get2['jenis_form_modul'];
	$rkaSKPD221_v2_koreksi->urutSebelumnya = $rkaSKPD221_v2_koreksi->urutTerakhir - 1;
	...
	$rkaSKPD221_v2_koreksi->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$rkaSKPD221_v2_koreksi->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];
	
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$rkaSKPD221_v2_koreksi->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$rkaSKPD221_v2_koreksi->idTahap'"));
	$rkaSKPD221_v2_koreksi->currentTahap = $getCurrenttahap['nama_tahap'];
	
	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$rkaSKPD221_v2_koreksi->idTahap'"));
	$rkaSKPD221_v2_koreksi->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$rkaSKPD221_v2_koreksi->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$rkaSKPD221_v2_koreksi->provinsi = $setting['provinsi'];
$rkaSKPD221_v2_koreksi->kota = $setting['kota'];
$rkaSKPD221_v2_koreksi->pengelolaBarang = $setting['pengelolaBarang'];
$rkaSKPD221_v2_koreksi->pejabatPengelolaBarang = $setting['pejabat'];
$rkaSKPD221_v2_koreksi->pengurusPengelolaBarang = $setting['pengurus'];
$rkaSKPD221_v2_koreksi->nipPengelola = $setting['nipPengelola'];
$rkaSKPD221_v2_koreksi->nipPengurus = $setting['nipPengurus'];
$rkaSKPD221_v2_koreksi->nipPejabat = $setting['nipPejabat'];


if($rkaSKPD221_v2_koreksi->jenisForm !="KOREKSI"){
	...
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$rkaSKPD221_v2_koreksi->currentTahap = $arrayHasil['currentTahap'];
			$rkaSKPD221_v2_koreksi->jenisForm = "";
			$rkaSKPD221_v2_koreksi->jenisFormTerakhir = "KOREKSI";
			$rkaSKPD221_v2_koreksi->idTahap = $get2['id_tahap'];
			$rkaSKPD221_v2_koreksi->nomorUrut = $get2['no_urut'];
			$rkaSKPD221_v2_koreksi->urutTerakhir = $get2['no_urut'];
