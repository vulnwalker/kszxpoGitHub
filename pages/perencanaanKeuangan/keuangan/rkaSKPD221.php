	  ...
	  switch($tipe){
		...
		case 'saveEditSubRincian':{
			...
			$dataUpdate = array(
													'jumlah1' => $jumlah,
													'jumlah2' => $kuantitas,
													'volume_rek' => $jumlah * $kuantitas,
													'satuan1' => $satuan1,
													'satuan2' => $satuan2,
													'satuan_total' => $satuan1."/".$satuan2,
													'jumlah' => $hargaSatuan,
													'jumlah_harga' => $jumlah * $kuantitas * $hargaSatuan,
												);
		...
		case 'editSubRincian':{
				...
				$inputanVolume = "<input style='width:25px;  text-align:right;' onkeyup=$this->Prefix.hitungVolume($id);  placeholder='JUMLAH'  type='text' name='jumlah$id' value='$jumlah1' id='jumlah$id' onkeypress='return event.charCode >= 48 && event.charCode <= 57' > x <input style='width:25px;  text-align:right;' onkeyup=$this->Prefix.hitungVolume($id);  placeholder='KUANTITAS'  type='text' name='kuantitas$id' value='$kuantitas' id='kuantitas$id' onkeypress='return event.charCode >= 48 && event.charCode <= 57' > = <input style='width:25px;  text-align:right;'  placeholder='VOLUME'  type='text' name='volume$id' value='$volume_rek' id='volume$id' onkeypress='return event.charCode >= 48 && event.charCode <= 57' readonly >";
				$inputanSatuan = "<input style='width:30px;  text-align:right;'   type='text' name='satuan1$id' value='$satuan1' id='satuan1$id'  > / <input style='width:30px;  text-align:right;'  type='text' name='satuan2$id' value='$satuan2' id='satuan2$id' >";
				$inputanSatuanHarga = "<input style='width:100%;  text-align:right;' onkeyup=$this->Prefix.hitungVolume($id);  placeholder='JUMLAH'  type='text' name='hargaSatuan$id' value='$jumlah' id='hargaSatuan$id' onkeypress='return event.charCode >= 48 && event.charCode <= 57' > ";
			 ...
			 if(!empty($p) && $q== '0'){
					...
					//	$kodeRekening = $this->injectQuery;
						...
						if(mysql_num_rows(mysql_query("select * from ref_rekening where k='$k' and l ='$l' and m='$m' and n='$n' and o='$o'")) == 0){
							$uraianList = "<span style='color:red;'>Belanja xxx</span>";
...
function formAlokasi($dt){
	...
	function syncDataRKBMD(){
	  ...
	  if(empty($err)){
			$getAllBarang = mysql_query("select * from view_rkbmd where tahun = '$this->tahun' and jenis_anggaran = '$this->jenisAnggaran' and no_urut = '$nomorUrutSebelumnya' and j!='000' $kondisiSKPD $exceptSKPD");
			$cek = "select * from view_rkbmd where tahun = '$this->tahun' and jenis_anggaran = '$this->jenisAnggaran' and no_urut = '$nomorUrutSebelumnya' and j!='000' $kondisiSKPD $exceptSKPD";
		}
	                ...
	                if(empty($k11)){
	                  $arrayRekening = array(
	                    ...
	                    //Data Barang
	                          ...
	                          mysql_query("update tabel_anggaran set volume_rek = volume_rek + $volume_barang  where id_anggaran = '".$getIdSubRincian['id_anggaran']."'");
	                    }
