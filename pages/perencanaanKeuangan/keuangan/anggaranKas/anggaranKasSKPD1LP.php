...
class anggaranKasSKPD1LPObj  extends DaftarObj2{
	var $Prefix = 'anggaranKasSKPD1LP';
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='anggaranKasSKPD1LP.xls';
	var $namaModulCetak='DPA';
	var $Cetak_Judul = 'DPA';
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'anggaranKasSKPD1LPForm';
	var $modul = "DPA";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	...
	function setTitle(){
		return 'ANGGARAN KAS SKPD LAIN LAIN PENDAPATAN '.$this->jenisAnggaran.' TAHUN '.$this->tahun;
	}
	...
	function set_selector_other($tipe){
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){
		...
		case 'Remove':{
			$id = $_REQUEST['anggaranKasSKPD1LP_cb'];
			foreach ($_REQUEST as $key => $value) {
				  $$key = $value;
			}
		...
		case 'checkAlokasi':{
			...
			$d = $cmbSKPD;
			$p = $hiddenP;
			$terpilih = $anggaranKasSKPD1LP_cb[0];
	 ...
	 function setPage_HeaderOther(){
		 	...
		 	<A href=\"pages.php?Pg=anggaranKasSKPD1LP\" title='PENDAPATAN'  style='color:blue;'> PENDAPATAN </a> |
		 	...
		 	<A href=\"pages.php?Pg=anggaranKasSKPD1LP\" title='LAIN LAIN PENDAPATAN' style='color:blue;'> LAIN LAIN PENDAPATAN </a>
   ...
   function setPage_OtherScript(){
		...
		return
			...
			<script type='text/javascript' src='js/perencanaanKeuangan/keuangan/anggaranKas/anggaranKasSKPD1LP.js' language='JavaScript' ></script>
	 ...
	 $this->form_height = 400;
	  if ($this->form_fmST==0) {
		...
		foreach ($dt as $key => $value) {
		 ...
		 $jenis_rka = $jenis_rka;
		 $cmbJenisDPA = cmbArray('cmbJenisDPAForm',$jenis_rka,$arrayJenisDPA,'-- JENIS DPA --','onchange=anggaranKasSKPD1LP.unlockFindRekening();');
	 	 if(empty($jenis_rka)){
	  ...
	  $this->form_fields = array(
			...
			'kode7' => array(
						...
						'value'=> "<input type='text' id='kodeRekening' name='kodeRekening' style='width:120px;' readonly value = '$kodeRekening'> &nbsp&nbsp
						 ...
						 <button type='button' id='findRekening' onclick=anggaranKasSKPD1LP.findRekening('$jenis_rka'); $tergantungJenis> CARI </button> "
						 ),
			'kode8' => array(
						'label'=>'HARGA SATUAN',
						'labelWidth'=>150,
						'value'=> "<input type='text' name='hargaSatuan' id='hargaSatuan' value='$hargaSatuan' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='anggaranKasSKPD1LP.bantu();' > <span id='bantu' style='color:red;'> </span>"
						 ),
...
/*		 $arrayResult = VulnWalkerTahap_v2('RKA');
	...
	foreach ($_COOKIE as $key => $value) {
	  ...
	  $urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=anggaranKasSKPD1LP.refreshList(true);','-- URUSAN --');

	  $codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
	  $bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=anggaranKasSKPD1LP.refreshList(true);','-- BIDANG --');

	  $codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
	  $skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=anggaranKasSKPD1LP.refreshList(true);','-- SKPD --');
...
$anggaranKasSKPD1LP = new anggaranKasSKPD1LPObj();


$arrayResult = VulnWalkerTahap_v2($anggaranKasSKPD1LP->modul);
$jenisForm = $arrayResult['jenisForm'];
$nomorUrut = $arrayResult['nomorUrut'];
$tahun = $arrayResult['tahun'];
$jenisAnggaran = $arrayResult['jenisAnggaran'];
...
$anggaranKasSKPD1LP->jenisForm = $jenisForm;
$anggaranKasSKPD1LP->nomorUrut = $nomorUrut;
$anggaranKasSKPD1LP->urutTerakhir = $nomorUrut;
$anggaranKasSKPD1LP->tahun = $tahun;
$anggaranKasSKPD1LP->jenisAnggaran = $jenisAnggaran;
$anggaranKasSKPD1LP->idTahap = $idTahap;

$anggaranKasSKPD1LP->username = $_COOKIE['coID'];


if(empty($anggaranKasSKPD1LP->tahun)){
	...
	/*$anggaranKasSKPD1LP->tahun = "select max(id_anggaran) as max from view_dpa_ppkd_1 where nama_modul = 'anggaranKasSKPD1LP'";*/
	$anggaranKasSKPD1LP->tahun  = $get2['tahun'];
	$anggaranKasSKPD1LP->jenisAnggaran = $get2['jenis_anggaran'];
	$anggaranKasSKPD1LP->urutTerakhir = $get2['no_urut'];
	$anggaranKasSKPD1LP->jenisFormTerakhir = $get2['jenis_form_modul'];
	$anggaranKasSKPD1LP->urutSebelumnya = $anggaranKasSKPD1LP->urutTerakhir - 1;
	...
	$anggaranKasSKPD1LP->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$anggaranKasSKPD1LP->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];

	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$anggaranKasSKPD1LP->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$anggaranKasSKPD1LP->idTahap'"));
	$anggaranKasSKPD1LP->currentTahap = $getCurrenttahap['nama_tahap'];

	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$anggaranKasSKPD1LP->idTahap'"));
	$anggaranKasSKPD1LP->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$anggaranKasSKPD1LP->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$anggaranKasSKPD1LP->provinsi = $setting['provinsi'];
$anggaranKasSKPD1LP->kota = $setting['kota'];
$anggaranKasSKPD1LP->pengelolaBarang = $setting['pengelolaBarang'];
$anggaranKasSKPD1LP->pejabatPengelolaBarang = $setting['pejabat'];
$anggaranKasSKPD1LP->pengurusPengelolaBarang = $setting['pengurus'];
$anggaranKasSKPD1LP->nipPengelola = $setting['nipPengelola'];
$anggaranKasSKPD1LP->nipPengurus = $setting['nipPengurus'];
$anggaranKasSKPD1LP->nipPejabat = $setting['nipPejabat'];
