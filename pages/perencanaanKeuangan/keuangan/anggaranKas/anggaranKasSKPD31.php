...
class anggaranKasSKPD31Obj  extends DaftarObj2{
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='anggaranKasSKPD31.xls';
	...
	function setTitle(){
		return 'ANGGARAN KAS SKPD PEMBIAYAAN PENERIMAAN '.$this->jenisAnggaran.' TAHUN '.$this->tahun;
	}
	...
	function set_selector_other($tipe){
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){
		...
		case 'sync':{
				if($this->jenisForm == 'READ'){
						...
						while($subRincian = mysql_fetch_array($getAllSubRincian)){
									$queryCekProgram = "select * from view_dpa_3_1 where c1='0' and j = '000' and rincian_perhitungan = '' and bk ='".$subRincian['bk']."' and ck ='".$subRincian['ck']."' and dk='".$subRincian['dk']."' and p ='".$subRincian['p']."' and q='0' and id_tahap='$this->idTahap' ";
									...
									$queryCekKegiatan= "select * from view_dpa_3_1 where c1='0' and j = '000' and rincian_perhitungan = '' and bk ='".$subRincian['bk']."' and ck ='".$subRincian['ck']."' and dk='".$subRincian['dk']."' and p ='".$subRincian['p']."' and q='".$subRincian['q']."' and id_tahap='$this->idTahap' ";
		...
		case 'Remove':{
			$id = $_REQUEST['anggaranKasSKPD31_cb'];
			foreach ($_REQUEST as $key => $value) {
				  $$key = $value;
			}
			...
			$getAll = mysql_query("select * from view_dpa_3_1 where concat(k,'.',l,'.',m,'.',n,'.',o) ='$kodeRek' and id_tahap='$this->idTahap' and (rincian_perhitungan !='' or f!='00' )   and c1='$cmbUrusan' and c='$cmbBidang' and d='$cmbSKPD' and e='$cmbUnit' and e1='$cmbSubUnit' and bk='$bk' and ck='$ck' and p='$hiddenP' and q='$q' order by o1, rincian_perhitungan");
		...
		case 'checkAlokasi':{
			...
			$d = $cmbSKPD;
			$p = $hiddenP;
			$terpilih = $anggaranKasSKPD31_cb[0];
			...
			if($this->jenisForm != "READ"){
				...
				$o = $getKodeRekening['o'];
				$jenisAlokasi = "";
				if(mysql_num_rows(mysql_query("select * from anggaran_kas where tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and c1 ='$c1' and c='$c' and d ='$d' and bk ='$bk' and ck ='$ck' and dk ='$dk' and p='$p' and q ='$q' and k='$k' and l='$l' and m='$m' and n ='$n' and o ='$o' and jenis_rka = '3.1' and jenis_dpa = 'DPA-SKPD'")) != 0){
					$getAlokasi = mysql_fetch_array(mysql_query("select * from anggaran_kas where tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and c1 ='$c1' and c='$c' and d ='$d' and bk ='$bk' and ck ='$ck' and dk ='$dk' and p='$p' and q ='$q' and k='$k' and l='$l' and m='$m' and n ='$n' and o ='$o' and jenis_rka = '3.1' and jenis_dpa = 'DPA-SKPD'"));
			...
			if($this->jenisForm != "READ"){
				...
				if($err == ""){
					...
					if(mysql_num_rows(mysql_query("select * from anggaran_kas where tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and c1 ='$c1' and c='$c' and d ='$d' and bk ='$bk' and ck ='$ck' and dk ='$dk' and p='$p' and q ='$q' and k='$k' and l='$l' and m='$m' and n ='$n' and o ='$o'  and jenis_rka ='3.1'  and jenis_dpa = 'DPA-SKPD'")) == 0){
						...
						$grepId = mysql_fetch_array(mysql_query("select * from anggaran_kas where tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and c1 ='$c1' and c='$c' and d ='$d' and bk ='$bk' and ck ='$ck' and dk ='$dk' and p='$p' and q ='$q' and k='$k' and l='$l' and m='$m' and n ='$n' and o ='$o'  and jenis_dpa = 'DPA-SKPD' and jenis_rka = '3.1'"));
						$id = $grepId['id'];
						$data = array(
										'jenis_alokasi_kas' => $jenisAlokasi,
										"alokasi_jan" => $jan,
	 ...
	 function setPage_HeaderOther(){
		 ...
		 <A href=\"pages.php?Pg=anggaranKasSKPD31\" title='PEMBIAYAAN'  style='color:blue;' > PEMBIAYAAN </a>

		 &nbsp&nbsp&nbsp
		 </td></tr>

		 <tr><td class=\"menudottedline\" width=\"40%\" height=\"20\" style='text-align:right'><B>
		 <A href=\"pages.php?Pg=anggaranKasSKPD31\" title='PENERIMAAN' style='color:blue;' > PENERIMAAN </a> |
		 <A href=\"pages.php?Pg=anggaranKasSKPD32\" title='PENGELUARAN'  > PENGELUARAN </a>
   ...
   function setPage_OtherScript(){
		...
		return
			...
			<script type='text/javascript' src='js/perencanaanKeuangan/keuangan/anggaranKas/anggaranKasSKPD31.js' language='JavaScript' ></script>
	...
	function Info(){
	 ...
	 $this->form_caption = 'INFO ALOKASI DPA-SKPD 3.1';
	 ...
	 $this->form_height = 400;
	  if ($this->form_fmST==0) {
		...
		foreach ($dt as $key => $value) {
		 ...
		 $jenis_rka = $jenis_rka;
		 $cmbJenisDPA = cmbArray('cmbJenisDPAForm',$jenis_rka,$arrayJenisDPA,'-- JENIS DPA --','onchange=anggaranKasSKPD31.unlockFindRekening();');
	 	 if(empty($jenis_rka)){
	  ...
	  $this->form_fields = array(
			...
			'kode7' => array(
						...
						'value'=> "<input type='text' id='kodeRekening' name='kodeRekening' style='width:120px;' readonly value = '$kodeRekening'> &nbsp&nbsp
						 ...
						 <button type='button' id='findRekening' onclick=anggaranKasSKPD31.findRekening('$jenis_rka'); $tergantungJenis> CARI </button> "
						 ),
			'kode8' => array(
						'label'=>'HARGA SATUAN',
						'labelWidth'=>150,
						'value'=> "<input type='text' name='hargaSatuan' id='hargaSatuan' value='$hargaSatuan' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='anggaranKasSKPD31.bantu();' > <span id='bantu' style='color:red;'> </span>"
						 ),
...
/*		 $arrayResult = VulnWalkerTahap_v2('RKA');
	...
	foreach ($_COOKIE as $key => $value) {
	  ...
	  $urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=anggaranKasSKPD31.refreshList(true);','-- URUSAN --');

	  $codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
	  $bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=anggaranKasSKPD31.refreshList(true);','-- BIDANG --');

	  $codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
	  $skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=anggaranKasSKPD31.refreshList(true);','-- SKPD --');
...
function Laporan($xls =FALSE){
		...
		$arrKondisi[] = "c1 = '$cmbUrusan'";
  		...
  		$arrKondisi[] = "c1 != '0'";
			...
			$getIDTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where no_urut ='$this->urutTerakhir' and tahun ='$this->tahun' and anggaran='$this->jenisAnggaran'"));
				...
				$getAllParent = mysql_query("select * from view_dpa_3_1 where id_tahap ='$idTahap'  and o1 !='0' and k='0' and n='0' and j='000'  ");
				while($rows = mysql_fetch_array($getAllParent)){
					foreach ($rows as $key => $value) {
				 	 $$key = $value;
					}
					$cekPekerjaan = mysql_num_rows(mysql_query("select * from view_dpa_3_1 where o1 = '$o1' and id_tahap ='$idTahap' $kondisiRekening $kondisiSKPD and (rincian_perhitungan !='' or j!='000') "));
					if($cekPekerjaan == 0){
						$arrKondisi[] = "o1 !='$o1'";
					}else{
						...
						while($row2s = mysql_fetch_array($getAllRekening)){
							...
							$cekRekening = mysql_num_rows(mysql_query("select * from view_dpa_3_1 where k ='$k' and l='$l' and m='$m' and n='$n' and o='$o' and id_tahap ='$idTahap' $kondisiRekening $kondisiSKPD and (rincian_perhitungan !='' or j!='000') "));
							if($cekRekening == 0){
								$concat = $k.".".$l.".".$m.".".$n.".".$o;
								...
								while($row3s = mysql_fetch_array($getAllProgram)){
									...
									$cekProgram = mysql_num_rows(mysql_query("select * from view_dpa_3_1 where bk ='$bk' and ck='$ck' and p='$p' and id_tahap ='$idTahap' $kondisiRekening $kondisiSKPD and (rincian_perhitungan !='' or j!='000') "));
									if($cekProgram == 0){
										$concat = $bk.".".$ck.".".$p;
										...
										while($row4s = mysql_fetch_array($getAllKegiatan)){
											...
											$cekKegiatan = mysql_num_rows(mysql_query("select * from view_dpa_3_1 where bk ='$bk' and ck='$ck' and p='$p' and q='$q' and id_tahap ='$idTahap' $kondisiRekening $kondisiSKPD and (rincian_perhitungan !='' or j!='000') "));
											if($cekKegiatan == 0){
												$concat = $bk.".".$ck.".".$p;
												$arrKondisi[] = "concat(bk,'.',ck,'.',p)  !='$concat'";
											}else{
				...
				$grabNonMapingRekening= mysql_query("select * from view_dpa_3_1 where id_tahap ='$idTahap' and (LENGTH(k) > 1) and j='000'");
				while($got = mysql_fetch_array($grabNonMapingRekening)){
					if(mysql_num_rows(mysql_query("select * from view_dpa_3_1 where id_tahap ='$idTahap' and k ='".$got['k']."' and l ='".$got['l']."' and m ='".$got['m']."' and n ='".$got['n']."' and o ='".$got['o']."' and ( rincian_perhitungan !='' or  j!='000' ) $kondisiRekening $kondisiSKPD ")) != 0){
						$arrKondisi[] = "id_tahap = '$idTahap' or id_anggaran ='".$got['id_anggaran']."'";
					}

				}

				$grabNonHostedRekening= mysql_query("select * from view_dpa_3_1 where id_tahap ='$idTahap' and (LENGTH(k) = 1 and k !='0') and j='000'");
		...
		while($daqry = mysql_fetch_array($aqry)){
			...
			$grabWihtCondition = mysql_query("select * from view_dpa_3_1 where $Kondisi group by c1,c,d,bk,ck,p,q,k,l,m,n,o,catatan,id_jenis_pemeliharaan");
				while($rows = mysql_fetch_array($grabWihtCondition)){
			 	...
			 	if(in_array($grubId, $this->publicGrupId)) {
					    $jumlah_harga = mysql_fetch_array(mysql_query("select sum(jumlah_harga) from view_dpa_3_1 where concat(c1,'.',c,'.',d,'.',bk,'.',ck,'.',p,'.',q,'.',f,'.',g,'.',h,'.',i,'.',j,'.',k,'.',l,'.',m,'.',n,'.',o,'.',catatan,'.',id_jenis_pemeliharaan) = '$grubId' and id_tahap = '$idTahap' "));
				 		$jumlah_harga= $jumlah_harga['sum(jumlah_harga)'];
						$volume_rek  = mysql_fetch_array(mysql_query("select sum(volume_rek) from view_dpa_3_1 where concat(c1,'.',c,'.',d,'.',bk,'.',ck,'.',p,'.',q,'.',f,'.',g,'.',h,'.',i,'.',j,'.',k,'.',l,'.',m,'.',n,'.',o,'.',catatan,'.',id_jenis_pemeliharaan) = '$grubId' and id_tahap = '$idTahap' "));
			...
			if(in_array($id_anggaran, $this->publicExcept)) {
				...
				";
				}elseif($j != '000' && mysql_num_rows(mysql_query("select * from view_dpa_3_1 where concat(c1,'.',c,'.',d,'.',bk,'.',ck,'.',p,'.',q,'.',f,'.',g,'.',h,'.',i,'.',j,'.',k,'.',l,'.',m,'.',n,'.',o,'.',catatan,'.',id_jenis_pemeliharaan) = '$grubId' and id_tahap = '$idTahap' ")) > 1 ){
					echo "";
				}else{
					echo "
								<tr valign='top'>
...
function formAlokasi($dt){
	...
	$arrayJenisPerhitungan = array(
	 ...
	 $this->publicSum = "select sum(jumlah_harga) from tabel_anggaran where (rincian_perhitungan !='' or f !='00' ) and id_tahap='$this->idTahap' and tahun ='$this->tahun' and jenis_anggaran = '$this->jenisAnggaran' and jenis_rka ='3.1' and nama_modul = 'DPA-SKPD' and c1= '$cmbUrusan' and c ='$cmbBidang' and d='$cmbSKPD' and bk = '$bk' and ck ='$ck' and dk ='$dk' and p = '$hiddenP' and q='$q' and k='$k' and l='$l' and m='$m' and n ='$n' and o ='$o' ";
	 $getSumJumlah = mysql_fetch_array(mysql_query($this->publicSum));
	 $jumlahHargaForm =$getSumJumlah['sum(jumlah_harga)'];
	 if(mysql_num_rows(mysql_query("select * from anggaran_kas where tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and c1 ='$cmbUrusan' and c='$cmbBidang' and d ='$cmbSKPD' and k='$k' and l='$l' and m='$m' and n ='$n' and o ='$o' and jenis_rka ='3.1' and jenis_dpa = 'DPA-SKPD'")) != 0 ){
	 	$grabSPD = mysql_fetch_array(mysql_query("select * from anggaran_kas where tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and c1 ='$cmbUrusan' and c='$cmbBidang' and d ='$cmbSKPD' and  k='$k' and l='$l' and m='$m' and n ='$n' and o ='$o' and jenis_rka ='3.1' and jenis_dpa = 'DPA-SKPD'"));
...
function formAlokasiTriwulan(){
	...
	$arrayJenisPerhitungan = array(
	 ...
	 if(mysql_num_rows(mysql_query("select * from anggaran_kas where tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and c1 ='$cmbUrusan' and c='$cmbBidang' and d ='$cmbSKPD' and bk ='$bk' and ck ='$ck' and dk ='$dk' and p='$hiddenP' and q ='$q' and k='$k' and l='$l' and m='$m' and n ='$n' and o ='$o' and jenis_rka ='3.1' and jenis_dpa = 'DPA-SKPD'")) != 0 ){
	 	$grabSPD = mysql_fetch_array(mysql_query("select * from anggaran_kas where tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and c1 ='$cmbUrusan' and c='$cmbBidang' and d ='$cmbSKPD' and bk ='$bk' and ck ='$ck' and dk ='$dk' and p='$hiddenP' and q ='$q' and k='$k' and l='$l' and m='$m' and n ='$n' and o ='$o' and jenis_rka ='3.1' and jenis_dpa = 'DPA-SKPD'"));
		foreach ($grabSPD as $key => $value) {
				  $$key = $value;
		}
		$jan = $alokasi_jan;
...
$anggaranKasSKPD31 = new anggaranKasSKPD31Obj();


$arrayResult = VulnWalkerTahap_v2($anggaranKasSKPD31->modul);
$jenisForm = $arrayResult['jenisForm'];
$nomorUrut = $arrayResult['nomorUrut'];
$tahun = $arrayResult['tahun'];
$jenisAnggaran = $arrayResult['jenisAnggaran'];
...
$anggaranKasSKPD31->jenisForm = $jenisForm;
$anggaranKasSKPD31->nomorUrut = $nomorUrut;
$anggaranKasSKPD31->urutTerakhir = $nomorUrut;
$anggaranKasSKPD31->tahun = $tahun;
$anggaranKasSKPD31->jenisAnggaran = $jenisAnggaran;
$anggaranKasSKPD31->idTahap = $idTahap;

$anggaranKasSKPD31->username = $_COOKIE['coID'];


if(empty($anggaranKasSKPD31->tahun)){
	...
	/*$anggaranKasSKPD31->tahun = "select max(id_anggaran) as max from view_dpa_3_1 where nama_modul = 'anggaranKasSKPD31'";*/
	$anggaranKasSKPD31->tahun  = $get2['tahun'];
	$anggaranKasSKPD31->jenisAnggaran = $get2['jenis_anggaran'];
	$anggaranKasSKPD31->urutTerakhir = $get2['no_urut'];
	$anggaranKasSKPD31->jenisFormTerakhir = $get2['jenis_form_modul'];
	$anggaranKasSKPD31->urutSebelumnya = $anggaranKasSKPD31->urutTerakhir - 1;
	...
	$anggaranKasSKPD31->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$anggaranKasSKPD31->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];

	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$anggaranKasSKPD31->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$anggaranKasSKPD31->idTahap'"));
	$anggaranKasSKPD31->currentTahap = $getCurrenttahap['nama_tahap'];

	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$anggaranKasSKPD31->idTahap'"));
	$anggaranKasSKPD31->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$anggaranKasSKPD31->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$anggaranKasSKPD31->provinsi = $setting['provinsi'];
$anggaranKasSKPD31->kota = $setting['kota'];
$anggaranKasSKPD31->pengelolaBarang = $setting['pengelolaBarang'];
$anggaranKasSKPD31->pejabatPengelolaBarang = $setting['pejabat'];
$anggaranKasSKPD31->pengurusPengelolaBarang = $setting['pengurus'];
$anggaranKasSKPD31->nipPengelola = $setting['nipPengelola'];
$anggaranKasSKPD31->nipPengurus = $setting['nipPengurus'];
$anggaranKasSKPD31->nipPejabat = $setting['nipPejabat'];
