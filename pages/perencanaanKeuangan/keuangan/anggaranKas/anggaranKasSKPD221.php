...
class anggaranKasSKPD221Obj  extends DaftarObj2{
	var $Prefix = 'anggaranKasSKPD221';
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='anggaranKasSKPD221.xls';
	var $namaModulCetak='DPA';
	var $Cetak_Judul = 'DPA';
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'anggaranKasSKPD221Form';
	var $modul = "DPA";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	...
	function setTitle(){
		return 'ANGGARAN KAS SKPD BELANJA LANGSUNG '.$this->jenisAnggaran.' TAHUN '.$this->tahun;
	}
	...
	function set_selector_other($tipe){
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){
		...
		case 'Remove':{
			$id = $_REQUEST['anggaranKasSKPD221_cb'];
			foreach ($_REQUEST as $key => $value) {
				  $$key = $value;
			}
		...
		case 'checkAlokasi':{
			...
			$d = $cmbSKPD;
			$p = $hiddenP;
			$terpilih = $anggaranKasSKPD221_cb[0];
...
// </td></tr>
		 ...
		 <tr><td class=\"menudottedline\" width=\"40%\" height=\"20\" style='text-align:right'><B>
		 <A href=\"pages.php?Pg=anggaranKasSKPD221\" title='BELANJA LANGSUNG' style='color:blue;' > LANGSUNG </a> |
		 <A href=\"pages.php?Pg=anggaranKasSKPD21\" title='BELANJA TIDAK LANGSUNG' > TIDAK LANGSUNG </a>
   ...
   function setPage_OtherScript(){
		...
		return
			...
			<script type='text/javascript' src='js/perencanaanKeuangan/keuangan/anggaranKas/anggaranKasSKPD221.js' language='JavaScript' ></script>
	 ...
	 $this->form_height = 400;
	  if ($this->form_fmST==0) {
		...
		foreach ($dt as $key => $value) {
		 ...
		 $jenis_rka = $jenis_rka;
		 $cmbJenisDPA = cmbArray('cmbJenisDPAForm',$jenis_rka,$arrayJenisDPA,'-- JENIS DPA --','onchange=anggaranKasSKPD221.unlockFindRekening();');
	 	 if(empty($jenis_rka)){
	  ...
	  $this->form_fields = array(
			...
			'kode7' => array(
						...
						'value'=> "<input type='text' id='kodeRekening' name='kodeRekening' style='width:120px;' readonly value = '$kodeRekening'> &nbsp&nbsp
						 ...
						 <button type='button' id='findRekening' onclick=anggaranKasSKPD221.findRekening('$jenis_rka'); $tergantungJenis> CARI </button> "
						 ),
			'kode8' => array(
						'label'=>'HARGA SATUAN',
						'labelWidth'=>150,
						'value'=> "<input type='text' name='hargaSatuan' id='hargaSatuan' value='$hargaSatuan' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='anggaranKasSKPD221.bantu();' > <span id='bantu' style='color:red;'> </span>"
						 ),
...
/*		 $arrayResult = VulnWalkerTahap_v2('RKA');
		...
		$headerTable =
		  "<thead>
		   ...
		   <th class='th01' width='100'>KODE REKENING / URAIAN </th>
	...
	if(!empty($p) && $q== '0'){
			 ...
			 $jumlahHarga = number_format($getSumJumlahHarga['sum(jumlah_harga)'] ,2,',','.');
			  $getDataAnggaranKas = mysql_fetch_array(mysql_query("select sum(alokasi_jan) as alokasi_jan, sum(alokasi_feb) as alokasi_feb, sum(alokasi_mar) as alokasi_mar, sum(alokasi_apr) as alokasi_apr, sum(alokasi_mei) as alokasi_mei, sum(alokasi_jun) as alokasi_jun, sum(alokasi_jul) as alokasi_jul, sum(alokasi_agu) as alokasi_agu, sum(alokasi_sep) as alokasi_sep, sum(alokasi_okt) as alokasi_okt, sum(alokasi_nop) as alokasi_nop, sum(alokasi_des) as alokasi_des from anggaran_kas where tahun = '$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' $kondisiSKPD and bk='$bk' and ck='$ck' and dk='$dk' and p='$p'   "));
		 ...
		 $getDataAnggaranKas = mysql_fetch_array(mysql_query("select sum(alokasi_jan) as alokasi_jan, sum(alokasi_feb) as alokasi_feb, sum(alokasi_mar) as alokasi_mar, sum(alokasi_apr) as alokasi_apr, sum(alokasi_mei) as alokasi_mei, sum(alokasi_jun) as alokasi_jun, sum(alokasi_jul) as alokasi_jul, sum(alokasi_agu) as alokasi_agu, sum(alokasi_sep) as alokasi_sep, sum(alokasi_okt) as alokasi_okt, sum(alokasi_nop) as alokasi_nop, sum(alokasi_des) as alokasi_des  from anggaran_kas where tahun = '$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' $kondisiSKPD and bk='$bk' and ck='$ck' and dk='$dk' and p='$p' and q='$q'  "));
		 $alokasiJanuari = $getDataAnggaranKas['alokasi_jan'];
		 $alokasiFebruari= $getDataAnggaranKas['alokasi_feb'];
		 $alokasiMaret = $getDataAnggaranKas['alokasi_mar'];
		 $alokasiApril = $getDataAnggaranKas['alokasi_apr'];
		 ...
		 $alokasiDesember = $getDataAnggaranKas['alokasi_des'];
		  $TampilCheckBox = "";
			 ...
			 $boldStatus = "bold";
			 $marginStatus = "20px;";
			 $kode = $k.".".$l.".".$m.".".$n.".".$o."<br><span style='margin-left:$marginStatus'>";
			 $marginKode = "10px;";
			 $getNamaRekening = mysql_fetch_array(mysql_query("select * from ref_rekening where k='$k' and l ='$l' and m='$m' and n='$n' and o='$o'"));
	...
	$Koloms[] = array(' align="left"', "<span style='color:$warnaMapping;margin-left:$marginStatus;'>$kode $uraianList</span> <div style='float:right'>".$isi['rincian_volume']."</div>" );
	...
	foreach ($_COOKIE as $key => $value) {
	  ...
	  $urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=anggaranKasSKPD221.refreshList(true);','-- URUSAN --');

	  $codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
	  $bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=anggaranKasSKPD221.refreshList(true);','-- BIDANG --');

	  $codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
	  $skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=anggaranKasSKPD221.refreshList(true);','-- SKPD --');
	  ...
	  $unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=anggaranKasSKPD221.refreshList(true);','-- UNIT --');
	  ...
	  $subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=anggaranKasSKPD221.refreshList(true);','-- SUB UNIT --');
	...
	$program = "<input type='hidden' id='bk' name='bk' value='$selectedBK'> <input type='hidden' id='ck' name='ck' value='$selectedCK'> <input type='hidden' id='dk' name='dk' value='$selectedDK'> <input type='hidden' id='hiddenP' name='hiddenP' value='$selectedP'>".cmbArray('p',$_REQUEST['p'],$arrayP,'-- PROGRAM --','onchange=anggaranKasSKPD221.programChanged();');
	...
	$kegiatan = cmbArray('q',$_REQUEST['q'],$arrayQ,'-- KEGIATAN --','onchange=anggaranKasSKPD221.refreshList(true);');
	...
	$cmbBelanja = cmbArray('cmbBelanja',$_REQUEST['cmbBelanja'],$arrayBelanja,'-- JENIS BELANJA--','onchange=anggaranKasSKPD221.refreshList(true);');
...
$anggaranKasSKPD221 = new anggaranKasSKPD221Obj();


$arrayResult = VulnWalkerTahap_v2($anggaranKasSKPD221->modul);
$jenisForm = $arrayResult['jenisForm'];
$nomorUrut = $arrayResult['nomorUrut'];
$tahun = $arrayResult['tahun'];
$jenisAnggaran = $arrayResult['jenisAnggaran'];
...
$anggaranKasSKPD221->jenisForm = $jenisForm;
$anggaranKasSKPD221->nomorUrut = $nomorUrut;
$anggaranKasSKPD221->urutTerakhir = $nomorUrut;
$anggaranKasSKPD221->tahun = $tahun;
$anggaranKasSKPD221->jenisAnggaran = $jenisAnggaran;
$anggaranKasSKPD221->idTahap = $idTahap;

$anggaranKasSKPD221->username = $_COOKIE['coID'];


if(empty($anggaranKasSKPD221->tahun)){

	$get1 = mysql_fetch_array(mysql_query("select max(id)  from view_dpa_2_2_1 "));
	$maxAnggaran = $get1['max(id)'];
	$get2 = mysql_fetch_array(mysql_query("select * from view_dpa_2_2_1 where id = '$maxAnggaran'"));
	/*$anggaranKasSKPD221->tahun = "select max(id_anggaran) as max from view_dpa_2_2_1 where nama_modul = 'anggaranKasSKPD221'";*/
	$anggaranKasSKPD221->tahun  = $get2['tahun'];
	$anggaranKasSKPD221->idTahap  = $get2['id_tahap'];
	$anggaranKasSKPD221->jenisAnggaran = $get2['jenis_anggaran'];
	$anggaranKasSKPD221->urutTerakhir = $get2['no_urut'];
	$anggaranKasSKPD221->jenisFormTerakhir = $get2['jenis_form_modul'];
	$anggaranKasSKPD221->urutSebelumnya = $anggaranKasSKPD221->urutTerakhir - 1;
	...
	$anggaranKasSKPD221->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$anggaranKasSKPD221->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];

	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$anggaranKasSKPD221->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$anggaranKasSKPD221->idTahap'"));
	$anggaranKasSKPD221->currentTahap = $getCurrenttahap['nama_tahap'];

	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$anggaranKasSKPD221->idTahap'"));
	$anggaranKasSKPD221->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$anggaranKasSKPD221->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$anggaranKasSKPD221->provinsi = $setting['provinsi'];
$anggaranKasSKPD221->kota = $setting['kota'];
$anggaranKasSKPD221->pengelolaBarang = $setting['pengelolaBarang'];
$anggaranKasSKPD221->pejabatPengelolaBarang = $setting['pejabat'];
$anggaranKasSKPD221->pengurusPengelolaBarang = $setting['pengurus'];
$anggaranKasSKPD221->nipPengelola = $setting['nipPengelola'];
$anggaranKasSKPD221->nipPengurus = $setting['nipPengurus'];
$anggaranKasSKPD221->nipPejabat = $setting['nipPejabat'];
