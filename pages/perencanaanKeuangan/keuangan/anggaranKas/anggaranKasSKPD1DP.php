...
class anggaranKasSKPD1DPObj  extends DaftarObj2{
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='anggaranKasSKPD1DP.xls';
	...
	function setTitle(){
		return 'ANGGARAN KAS SKPD DANA PERIMBANGAN '.$this->jenisAnggaran.' TAHUN '.$this->tahun;
	}
	...
	function set_selector_other($tipe){
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){
		...
		case 'Remove':{
			$id = $_REQUEST['anggaranKasSKPD1DP_cb'];
			foreach ($_REQUEST as $key => $value) {
				  $$key = $value;
			}
		...
		case 'checkAlokasi':{
			...
			$d = $cmbSKPD;
			$p = $hiddenP;
			$terpilih = $anggaranKasSKPD1DP_cb[0];
	 ...
	 function setPage_HeaderOther(){
		 	...
		 	<A href=\"pages.php?Pg=anggaranKasSKPD1DP\" title='PENDAPATAN'  style='color:blue;'> PENDAPATAN </a> |
		 	...
		 	<tr><td class=\"menudottedline\" width=\"40%\" height=\"20\" style='text-align:right'><B>
		 	<A href=\"pages.php?Pg=anggaranKasSKPD1PAD\" title='PENDAPATAN ASLI DAERAH'  > PENDAPATAN ASLI DAERAH </a> |
		 	<A href=\"pages.php?Pg=anggaranKasSKPD1DP\" title='DANA PERIMBANGAN' style='color:blue;' > DANA PERIMBANGAN </a> |
   ...
   function setPage_OtherScript(){
		...
		return
			...
			<script type='text/javascript' src='js/perencanaanKeuangan/keuangan/anggaranKas/anggaranKasSKPD1DP.js' language='JavaScript' ></script>
	 ...
	 $this->form_height = 400;
	  if ($this->form_fmST==0) {
		...
		foreach ($dt as $key => $value) {
		 ...
		 $jenis_rka = $jenis_rka;
		 $cmbJenisDPA = cmbArray('cmbJenisDPAForm',$jenis_rka,$arrayJenisDPA,'-- JENIS DPA --','onchange=anggaranKasSKPD1DP.unlockFindRekening();');
	 	 if(empty($jenis_rka)){
	  ...
	  $this->form_fields = array(
			...
			'kode7' => array(
						...
						'value'=> "<input type='text' id='kodeRekening' name='kodeRekening' style='width:120px;' readonly value = '$kodeRekening'> &nbsp&nbsp
						 ...
						 <button type='button' id='findRekening' onclick=anggaranKasSKPD1DP.findRekening('$jenis_rka'); $tergantungJenis> CARI </button> "
						 ),
			'kode8' => array(
						'label'=>'HARGA SATUAN',
						'labelWidth'=>150,
						'value'=> "<input type='text' name='hargaSatuan' id='hargaSatuan' value='$hargaSatuan' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='anggaranKasSKPD1DP.bantu();' > <span id='bantu' style='color:red;'> </span>"
						 ),
...
/*		 $arrayResult = VulnWalkerTahap_v2('RKA');
	...
	foreach ($_COOKIE as $key => $value) {
	  ...
	  $urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=anggaranKasSKPD1DP.refreshList(true);','-- URUSAN --');

	  $codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
	  $bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=anggaranKasSKPD1DP.refreshList(true);','-- BIDANG --');

	  $codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
	  $skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=anggaranKasSKPD1DP.refreshList(true);','-- SKPD --');
...
function Laporan($xls =FALSE){
		...
		echo
			...
			"<body >
				...
				<span style='font-size:16px;font-weight:bold;text-decoration: '>
					Indikator & Tolak Ukur Kinerja DANA PERIMBANGAN
				</span><br>
		...
		echo "
				<span style='font-size:16px;font-weight:bold;text-decoration: '>
					Rincian Dokumen Pelaksanaan Anggaran DANA PERIMBANGAN <br>
...
$anggaranKasSKPD1DP = new anggaranKasSKPD1DPObj();


$arrayResult = VulnWalkerTahap_v2($anggaranKasSKPD1DP->modul);
$jenisForm = $arrayResult['jenisForm'];
$nomorUrut = $arrayResult['nomorUrut'];
$tahun = $arrayResult['tahun'];
$jenisAnggaran = $arrayResult['jenisAnggaran'];
...
$anggaranKasSKPD1DP->jenisForm = $jenisForm;
$anggaranKasSKPD1DP->nomorUrut = $nomorUrut;
$anggaranKasSKPD1DP->urutTerakhir = $nomorUrut;
$anggaranKasSKPD1DP->tahun = $tahun;
$anggaranKasSKPD1DP->jenisAnggaran = $jenisAnggaran;
$anggaranKasSKPD1DP->idTahap = $idTahap;

$anggaranKasSKPD1DP->username = $_COOKIE['coID'];


if(empty($anggaranKasSKPD1DP->tahun)){
	...
	/*$anggaranKasSKPD1DP->tahun = "select max(id_anggaran) as max from view_dpa_1 where nama_modul = 'anggaranKasSKPD1DP'";*/
	$anggaranKasSKPD1DP->tahun  = $get2['tahun'];
	$anggaranKasSKPD1DP->jenisAnggaran = $get2['jenis_anggaran'];
	$anggaranKasSKPD1DP->urutTerakhir = $get2['no_urut'];
	$anggaranKasSKPD1DP->jenisFormTerakhir = $get2['jenis_form_modul'];
	$anggaranKasSKPD1DP->urutSebelumnya = $anggaranKasSKPD1DP->urutTerakhir - 1;
	...
	$anggaranKasSKPD1DP->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$anggaranKasSKPD1DP->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];

	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$anggaranKasSKPD1DP->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$anggaranKasSKPD1DP->idTahap'"));
	$anggaranKasSKPD1DP->currentTahap = $getCurrenttahap['nama_tahap'];

	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$anggaranKasSKPD1DP->idTahap'"));
	$anggaranKasSKPD1DP->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$anggaranKasSKPD1DP->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$anggaranKasSKPD1DP->provinsi = $setting['provinsi'];
$anggaranKasSKPD1DP->kota = $setting['kota'];
$anggaranKasSKPD1DP->pengelolaBarang = $setting['pengelolaBarang'];
$anggaranKasSKPD1DP->pejabatPengelolaBarang = $setting['pejabat'];
$anggaranKasSKPD1DP->pengurusPengelolaBarang = $setting['pengurus'];
$anggaranKasSKPD1DP->nipPengelola = $setting['nipPengelola'];
$anggaranKasSKPD1DP->nipPengurus = $setting['nipPengurus'];
$anggaranKasSKPD1DP->nipPejabat = $setting['nipPejabat'];
