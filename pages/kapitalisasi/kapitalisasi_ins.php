...
class kapitalisasi_insObj extends DaftarObj2{
	var $Prefix = 'kapitalisasi_ins';
	var $SHOW_CEK = TRUE;	
	...
	var $FormName = 'kapitalisasi_insForm';
	var $pagePerHal = 25;
	
	var $PageTitle = 'RECLASS EXTRA';
	var $PageIcon = 'images/penatausahaan_ico.gif';
	var $ico_width = '20';
	var $ico_height = '30';
	
	var $fileNameExcel='Daftar RECLASS EXTRA.xls';
	var $Cetak_Judul = 'RECLASS EXTRA';	
	var $Cetak_Mode=2;
	...
	//get data -----------------
	 ...
	 $staset_baru = 10;
	 $ket = $_REQUEST['ket'];$ket= $_REQUEST['ket'];
	 //$err.='tgll='.$tgl_sk;
	...
	if($err =='' && compareTanggal($tgl_buku, date('Y-m-d'))==2  ) $err = 'TANGGAL BUKU tidak lebih besar dari Hari ini!';	
		...
		if($err==''){
			$limit=2;
			$getcnt = "select * from buku_induk where id in ($idplh) and staset!=$staset_baru and idawal not in(select idbi_awal from t_kapitalisasi where idbi_awal in($idplh)) limit 0,$limit";			
			//$err='qyr='.$getcnt;
			...
			$content->jml = $jml;	
				...
				while($old = mysql_fetch_array($result)){
					...
					$tgl_closing=getTglClosing($old['c'],$old['d'],$old['e'],$old['e1'],$old['c1']); 
					$tgl_susutAkhir = mysql_fetch_array(mysql_query("select tgl from penyusutan where idbi_awal='$idbi' order by tgl desc limit 1"));
					if($err=='' && $old['staset']==$staset_baru) $err = "Tidak ada perubahan status aset!";
					if($err=='' && $tgl_buku<=$tgl_closing)$err ='Tanggal sudah Closing !';				
					if($err=='' && $tgl_buku<=$tgl_susutAkhir['tgl'])$err ='Sudah ada penyusutan !';					
					$maxtrans = mysql_fetch_array(mysql_query("select max(tgl_buku) as maxtgl from t_transaksi where idawal='$idbiawal'"));
					if($err=='' && compareTanggal($tgl_buku, $maxtrans['maxtgl'])==0  ) $err = 'TANGGAL BUKU tidak lebih kecil dari tanggal transaksi sebelumnya!';
					...
					if($err==''){
						...
						" values('$tgl_buku','$idbi','$uid',now(),'$staset','$staset_baru','$ket', '$idbiawal','$nilai_buku','$nilai_susut','$no_sk','$tgl_sk',6) "; $cek .= $aqry;
						$qry = mysql_query($aqry);
						$newid= mysql_insert_id();
						if($qry){
							mysql_query("update buku_induk set staset = '$staset_baru' where  id='$idbi' ");
							//jurnal							
							//$jur = jurnalKapitalisasi($bi, $idbi,$uid,$tgl_buku, 1, FALSE, $newid);						
							...
							mysql_query(
								...
								" values ".
								"('$tgl_buku','$idbi','$uid',now(),'$staset','$staset_baru','".($staset_baru-$staset)."','$idbiawal',3,'$newid' )"
							);
						}							
					}
				}//end while	
	...
	$qrybi = "SELECT * FROM $this->TblName where id='".$cekid[0]."'";
	...
	$progress = 
		...
		$TampilOpt =
			...
			genFilterBar(
				...
				genFilterBar(
				array(
					"<span id='inputpenerimaanbarang' style='color:black;font-size:14px;font-weight:bold;'/>SK RECLASS EXTRA</span>",
				),'','','').
				genFilterBar(
				array(
					$this->isiform(
						array(	
							...
							array(
								...
								'name'=>'tgl_sk',
								'label-width'=>'200px;',
								'value'=>"<input type='text' name='tgl_sk' id='tgl_sk' value='$tgl_sk' class='datepicker' size='6'>"
								,
							),
							array(
								'label'=>'TANGGAL BUKU',
	...
	function setPage_OtherScript(){
		...
		"<script type='text/javascript' src='js/kapitalisasi/".strtolower($this->Prefix).".js' language='JavaScript' ></script>".
		$scriptload;
	}
...
$kapitalisasi_ins = new kapitalisasi_insObj();
