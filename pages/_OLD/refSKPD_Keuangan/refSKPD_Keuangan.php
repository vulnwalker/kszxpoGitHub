...
class refSKPD_KeuanganObj  extends DaftarObj2{	
	var $Prefix = 'refSKPD_Keuangan';
	var $elCurrPage="HalDefault";
	var $SHOW_CEK = TRUE;
	var $TblName = 'v1_refskpd_Keuangan'; //bonus
	var $TblName_Hapus = 'ref_skpd_keuangan';
	var $MaxFlush = 10;
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='SKPDKeuangan.xls';
	var $namaModulCetak='REFERENSI DATA';
	var $Cetak_Judul = 'Ref SKPD Keuangan';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'refSKPD_KeuanganForm';
	var $noModul=14; 
	var $TampilFilterColapse = 0; //0
	
	function setTitle(){
		return 'SKPD Keuangan';
	}
	...
	//get data -----------------
	 ...
	 $c_d=mysql_fetch_array(mysql_query("select * from ref_skpd_keuangan where c='$c' AND d='$d'"));	 
			...
			if($fmST == 0){
				if($err==''){
					if(empty($c_d['c']) && empty($c_d['d'])){
						$aqry = "INSERT into ref_skpd_keuangan (c,d,bk,ck,dk) values('$c','$d','$bk','$ck','$dk')";	$cek .= $aqry;						
					}else{
						$aqry = "UPDATE ref_skpd_keuangan
							set "." bk = '$bk',
							ck = '$ck',
							dk = '$dk'".
	...
	function setPage_OtherScript(){
		...
		return 	
			"<script type='text/javascript' src='js/master/refSKPD_Keuangan/".strtolower($this->Prefix).".js' language='JavaScript' ></script>".
			$scriptload;
	}
	...
	function setFormBaru(){
  	...
  	function setFormEdit(){
		...
		$this->form_fmST = 1;						
		//get data 
		$aqry = "SELECT * FROM  v1_refSKPD_Keuangan WHERE c='".$kode[0]."' AND d='".$kode[1]."'"; $cek.=$aqry;
...
$refSKPD_Keuangan = new refSKPD_KeuanganObj();
?>