...
class ref_skpd_keuanganObj  extends DaftarObj2{	
	var $Prefix = 'ref_skpd_keuangan';
	...
	var $pagePerHal ='';
	var $cetak_xls=TRUE ;
	var $fileNameExcel='ref_skpd_keuangan.xls';
	var $Cetak_Judul = 'Daftar SKPD Keuangan';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'ref_skpd_keuanganForm'; 	
			
	function setTitle(){
		return 'Daftar SKPD Keuangan';
	}
	...
	function simpan(){
	 ...
	 $kondisi1=" concat(bk,ck,dk)='".$bk.$ck.$dk."' ";$cek.=$kondisi1;
			...
			$ck_urusan = mysql_fetch_array(mysql_query("SELECT count(*) as cnt FROM ref_skpd_keuangan WHERE $kondisi1 "));
				if($ck_urusan['cnt']>0 && $err=="") $err="Kode Urusan Sudah Ada!";
		
				if($err==''){ 
					$aqry1 = "INSERT into ref_urusan (bk,ck,dk,nm_urusan,stpakai)
							 "."values('$bk','$ck','$dk','$nama_urusan',00)";	$cek .= $aqry1;	
	...
	case 'formBaru':{				
		...
		$get = mysql_fetch_array( mysql_query("select * from ref_skpd_keuangan where bk=$bk and ck=$ck and dk=$dk"));
	...
	function setPage_OtherScript(){
		...
		return 
			 "<script type='text/javascript' src='js/master/ref_skpd_keuangan/".strtolower($this->Prefix).".js' language='JavaScript' ></script>".		
			$scriptload;
	}
	...
	function Hapus_Validasi($ids){
		...
		if($errmsg=='' && 
				mysql_num_rows(mysql_query(
					"select * from ref_skpd_keuangan where ".$Kondisi." and stpakai=1")
				) >0 )
	...
	function setFormBaru(){
  	...
  	function setFormEdit(){
		...
		$dk=$kode[2]; 
		//query ambil data ref_urusan
		$aqry = "select * from ref_skpd_keuangan where concat(bk,ck,dk)='".$bk.$ck.$dk."'"; $cek.=$aqry;
	...
	function setKolomHeader($Mode=1, $Checkbox=''){
	 ...
	 <tr>
	   ...
	   <th class='th01' align='center' width='800'>Daftar SKPD</th>	   	   	   
	   </tr>
	   </thead>";
	...
	$TampilOpt = 
			...
			<td>".
			cmbQuery1("fmURUSAN",$fmURUSAN,"select bk,nm_urusan from ref_urusan where bk<>'00' and ck='00' ","onChange=\"$this->Prefix.refreshList(true)\"",'Pilih','').
			"</td>
			</tr><tr>
			<td>BIDANG</td><td>:</td>
			<td>".
			cmbQuery1("fmBIDANG",$fmBIDANG,"select ck,nm_urusan from ref_urusan where bk='$fmURUSAN' and ck<>'00' and dk='00' ","onChange=\"$this->Prefix.refreshList(true)\"",'Pilih','').
			"</td>
			</tr><tr>
			<td>DINAS</td><td>:</td>
			<td>".
			cmbQuery1("fmDINAS",$fmDINAS,"select dk,nm_urusan from ref_urusan where bk='$fmURUSAN' and ck ='$fmBIDANG' and dk<>'00'","onChange=\"$this->Prefix.refreshList(true)\"",'Pilih','').
			"</td>
			</tr>
			...
			"<table style='width:100%'>
			<tr><td>
				Kode : <input type='text' id='fmKODE' name='fmKODE' value='".$fmKODE."' maxlength='9' size=10px title='Kode (ex: 1.1.1)'>&nbsp
				Nama : <input type='text' id='fmAKUN' name='fmAKUN' value='".$fmAKUN."' size=30px>&nbsp
...
$ref_skpd_keuangan = new ref_skpd_keuanganObj();
