...
class RefStatusBarangObj  extends DaftarObj2{	
	var $Prefix = 'RefStatusBarang';
	...
	//get data -----------------
	 ...
	 $stbarang= $_REQUEST['id_status_barang']; 
 	 $idbi= $_REQUEST['idbi']; 
			...
			if($fmST == 0){
				...
				if($err==''){
					...
					for($i=0;$i<count($bi);$i++){
						$aqry= "UPDATE buku_induk set ref_idstatussurvey='$stbarang' WHERE id='".$bi[$i]."'";						
						$cek .= $aqry;
						$qry = mysql_query($aqry) or die(mysql_error());
					}	
				}
...
$RefStatusBarang = new RefStatusBarangObj();
?>