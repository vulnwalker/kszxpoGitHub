...
class ref_potongan2Obj  extends DaftarObj2{	
	var $Prefix = 'ref_potongan2';
	  ...
	  switch($tipe){
		...
		case 'getdata':{
			...
			//	$get = mysql_fetch_array( mysql_query("SELECT `system`.`nm_system`, `system_modul`.`nm_modul`, `system_modul`.`Id_system`,`system_modul`.`Id_modul` FROM `system` RIGHT JOIN `system_modul` ON `system`.`Id_system` = `system_modul`.`Id_system`  where system_modul.Id_modul='$Id'"));
				...
				$content = array('id_potongan' => $get['Id'],'nama_potongan' => $get['nama_potongan'], 'persen' => $get['persen']);
	...
	//  $Koloms[] = array('align="left"',$isi['nama_potongan']);
	 $Koloms[] = array('align="center" width="" ',"<a style='cursor:pointer;' onclick=ref_potongan2.windowSave('".$Id."')>".$isi['nama_potongan']."</a>");
...
$ref_potongan2 = new ref_potongan2Obj();
?>