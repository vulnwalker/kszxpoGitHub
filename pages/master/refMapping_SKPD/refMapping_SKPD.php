...
class refMapping_SKPDObj  extends DaftarObj2{	
	var $Prefix = 'refMapping_SKPD';
	var $elCurrPage="HalDefault";
	var $SHOW_CEK = TRUE;
	var $TblName = 'v1_refMapping_SKPD'; //bonus
	var $TblName_Hapus = 'refMapping_SKPD';
	var $MaxFlush = 10;
	...
	var $FormName = 'refMapping_SKPDForm';
	var $noModul=14; 
	...
	//get data -----------------
	 ...
	 $c_d=mysql_fetch_array(mysql_query("select * from refMapping_SKPD where c='$c' AND d='$d'"));	 
			...
			if($fmST == 0){
				if($err==''){
					if(empty($c_d['c']) && empty($c_d['d'])){
						$aqry = "INSERT into refMapping_SKPD (c,d,bk,ck,dk) values('$c','$d','$bk','$ck','$dk')";	$cek .= $aqry;						
					}else{
						$aqry = "UPDATE refMapping_SKPD
							set "." bk = '$bk',
							ck = '$ck',
							dk = '$dk'".
	...
	function setPage_OtherScript(){
		...
		return 	
			"<script type='text/javascript' src='js/master/refMapping_SKPD/".strtolower($this->Prefix).".js' language='JavaScript' ></script>".
			$scriptload;
	}
	...
	function setFormBaru(){
  	...
  	function setFormEdit(){
		...
		$this->form_fmST = 1;						
		//get data 
		$aqry = "SELECT * FROM  v1_refMapping_SKPD WHERE c='".$kode[0]."' AND d='".$kode[1]."'"; $cek.=$aqry;
...
$refMapping_SKPD = new refMapping_SKPDObj();
?>