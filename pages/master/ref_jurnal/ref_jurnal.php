...
class RefAkunObj  extends DaftarObj2{	
	var $Prefix = 'RefAkun';
...
//	$nm_acc= $_REQUEST['nm_account'];
	...
	if($fmST == 0){
		if($err==''){
			$aqry = "INSERT into ref_jurnal (ka,kb,kc,kd,ke,kf,nm_account) values('$ka','$kb','$kc','$kd','$ke','0','$nm_acc')";			$cek .= $aqry;	
	...
	$aqry = "UPDATE ref_jurnal set ka='$ka',kb='$kb',kc='$kc',kd='$kd',ke='$ke',kf='0',nm_account='$nm_acc' where concat (ka,' ',kb,' ',kc,' ',kd,' ',ke,' ',kf)='".$idplh."'";$cek .= $aqry;
						$qry = mysql_query($aqry);
				}
			} //end else'".$idplh."'
	...
	$aqry = "UPDATE ref_jurnal set ka='$eka',kb='$ekb',kc='$ekc',kd='$ekd',ke='$eke',kf='0',nm_account='$nama' where concat (ka,' ',kb,' ',kc,' ',kd,' ',ke,' ',kf)='".$idplh."'";$cek .= $aqry;
						$qry = mysql_query($aqry);
				}
	...
	if( $err=='' && $nama =='' ) $err= 'Nama Kode Kelompok Belum Di Isi !!';
		if($fmST == 0){
			if($err==''){
				$aqry = "INSERT into ref_jurnal (ka,kb,nm_account) values('$ka01','$kb','$nama')";	
				$cek .= $aqry;	
				$qry = mysql_query($aqry);
				$content=$kb;	
				}
	...
	if( $err=='' && $nama =='' ) $err= 'Nama Kode Jenis Belum Di Isi !!';
		if($fmST == 0){
			if($err==''){
				$aqrykc = "INSERT into ref_jurnal (ka,kb,kc,nm_account) values('$ka01','$kb','$kc','$nama')";	
				$cek .= $aqrykc;	
	...
	if( $err=='' && $nama =='' ) $err= 'Nama Kode Objek Belum Di Isi !!';
		if($fmST == 0){
			if($err==''){
				$aqrykd = "INSERT into ref_jurnal (ka,kb,kc,kd,nm_account) values('$ka01','$kb','$kc','$kd','$nama')";	
				$cek .= $aqrykd;	
	...
	case 'formBaru':{				
	   ...
	   /*	case 'getdata':{
		...
		$content = array('kode_jurnal'=>$kode_account, 'nm_account'=>$get['nm_account']);//, 'tahun_account'=>$get['thn_akun']);	
		break;
	   }*/
	  ...
	  // =1 1 1 2 0 0
				...
				$kd = substr($Id, 6,1);
				$ke = substr($Id, 8,2);
				$get = mysql_fetch_array( mysql_query("select *, concat(ka,'.',kb,'.',kc,'.',kd,'.',ke) as kodejurnal  from ref_jurnal where ka='$ka' AND kb='$kb' AND kc='$kc' AND kd='$kd' AND ke='$ke'"));
			
				
				$content = array('kode_jurnal' => $get['kodejurnal'], 'nm_account' => $get['nm_account']);
	...
	function setPage_OtherScript(){
		...
		return 
			 "<script type='text/javascript' src='js/master/ref_jurnal/ref_jurnal.js' language='JavaScript' ></script>".		
			$scriptload;
	}
	...
	function BaruKD($dt){	
	 ...
	 $this->form_height = 120;
	  if ($this->form_fmST==0) {
		...
		$queryKA1=mysql_fetch_array(mysql_query("SELECT ka, nm_account FROM ref_jurnal where ka='$KA1'"));  
		$queryKB1=mysql_fetch_array(mysql_query("SELECT kb, nm_account FROM ref_jurnal where ka='$KA1' and kb='$KB1' and kc=0 and kd=0 and ke=0 and kf=0"));  
		$queryKC1=mysql_fetch_array(mysql_query("SELECT kc, nm_account FROM ref_jurnal where ka='$KA1' and kb='$KB1' and kc='$KC1' and kd=0 and ke=0 and kf=0"));  
		$cek.="SELECT kc, nm_account FROM ref_jurnal where ka='$KA1' and kb='$KB1' and kc='$KC1' and kd=0 and ke=0 and kf=0";
//		
		$datak1=$KA1.".".$queryKA1['nm_account'];
		$datak2=$queryKB1['kb'].".".$queryKB1['nm_account'];
		$datak3=$queryKC1['kc'].".".$queryKC1['nm_account'];
...
//		$lastkode=$get['maxno'] + 1;
	...
	function setFormBaru(){
	  ...
	  //	$cek.="SELECT * FROM ref_jurnal WHERE ka='$data_ka' and kb='$data_kb' and kc='$data_kc' and kd='$data_kd' and ke='$data_ke' and kf='$data_kf'";
		...
		if($dt['ka']!='0' && $dt['kb']=='0' && $dt['kc']=='0' && $dt['kd']=='0' && $dt['ke']=='0'){
		$fm = $this->setFormEditdata1($dt);	
		}elseif($dt['ka']!='0' && $dt['kb']!='0' && $dt['kc']=='0' && $dt['kd']=='0' && $dt['ke']=='0'){
		$fm = $this->setFormEditdata2($dt);	
		}elseif($dt['ka']!='0' && $dt['kb']!='0' && $dt['kc']!='0' && $dt['kd']=='0' && $dt['ke']=='0'){
		$fm = $this->setFormEditdata3($dt);	
		}elseif($dt['ka']!='0' && $dt['kb']!='0' && $dt['kc']!='0' && $dt['kd']!='0' && $dt['ke']=='0'){
		$fm = $this->setFormEditdata4($dt);	
		}else{
		$fm = $this->setFormEditdata5($dt);
		}
		return	array ('cek'=>$cek.$fm['cek'], 'err'=>$fm['err'], 'content'=>$fm['content']);
	}
	...
	//	$datke=sprintf("%02s",$queryKEedit['ke'])." .  ".$queryKEedit['nm_account'];
       ...
       //items ----------------------
		  ...
		  'kode_Akun' => array( 
						...
						<input type='text' name='kax' id='kax' value='".$queryKAedit['ka']."' style='width:20px;' readonly>
       ...
       //items ----------------------
		  ...
		  'kode_Akun' => array( 
			...
			'kode_kelompok' => array( 
						...
						'labelWidth'=>100, 
						'value'=>"<div style='float:left;'>
						<input type='text' name='kb' id='kb' value='".$queryKBedit['kb']."' style='width:20px;' readonly>
						<input type='text' name='nm_account' id='nm_account' value='".$dt['nm_account']."' size='36px'>
						</div>", 
						 ),
       ...
       //items ----------------------
		  ...
		  'kode_Akun' => array( 
			...
			'kode_Jenis' => array( 
						...
						'labelWidth'=>100, 
						'value'=>"<div style='float:left;'>
						<input type='text' name='kc' id='kc' value='".$queryKCedit['kc']."' style='width:20px;' readonly>
						<input type='text' name='nm_account' id='nm_account' value='".$dt['nm_account']."' size='36px'>
						</div>", 
						 ),
       ...
       //items ----------------------
		  ...
		  'kode_Akun' => array( 
			...
			'kode_Objek' => array( 
						...
						<input type='text' name='kd' id='kd' value='".$queryKDedit['kd']."' style='width:20px;' readonly>
						<input type='text' name='nm_account' id='nm_account' value='".$dt['nm_account']."' size='36px'>
						</div>", 
						 ),
       ...
       //items ----------------------
		  ...
		  'kode_Akun' => array( 
			...
			'Kode_Rincian_Objek' => array( 
						...
						'labelWidth'=>100, 
						'value'=>"<div style='float:left;'>
						<input type='text' name='ke' id='ke' value='".$datke."' style='width:20px;' readonly>
						<input type='text' name='nm_account' id='nm_account' value='".$dt['nm_account']."' size='36px'>
						</div>", 
						 ),			 			 			 		 
			);
