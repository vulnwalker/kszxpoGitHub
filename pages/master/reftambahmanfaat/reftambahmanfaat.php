...
class RefTambahManfaatObj  extends DaftarObj2{	
	var $Prefix = 'RefTambahManfaat';
	var $elCurrPage="HalDefault";
	var $SHOW_CEK = TRUE;
	var $TblName = 'v1_ref_tambah_manfaat'; //daftar
	var $TblName_Hapus = 'ref_tambah_manfaat';
	var $MaxFlush = 10;
	...
	var $Cetak_Judul = 'PENAMBAHAN MASA MANFAAT';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'RefTambahManfaatForm'; 
	var $kdbrg = '';	
			
	function setTitle(){
		return 'Penambahan Masa Manfaat';
	}
	...
	function simpan(){
	 ...
	 $persen1_lama = str_replace(",",".",$_REQUEST['persen1_lama']);
	 $persen2_lama = str_replace(",",".",$_REQUEST['persen2_lama']);
	 ...
	 if($persen1 < $persen1_lama ){
	 	$err= "Persentase Pertama tidak boleh kurang dari $persen1_lama";
	 }
	 if($persen2 < $persen1 ){
	 	$err= "Persentase Kedua tidak boleh kurang dari $persen1";
	 }
	 
	 /*$ck_simpan = mysql_num_rows(mysql_query("select * from ref_tambah_manfaat where f ='$f' and g='$g' and h='$h' and i='$i' and j='$j' and persen1='$persen1' and persen2='$persen2'"));
	  if($ck_simpan > 0){
		  	$errmsg = "Barang sudah ada";
		  }*/
 
 	 /*---------------------------------------------------------------------------*/
	//$cek_dt = mysql_fetch_array(mysql_query("select count(*) as cnt from v1_ref_tambah_manfaat  where f='$f' and g='$g' and 	h='$h' and i='$i' and j='$j'")); //$cek.="select count(*) as cnt from v1_ref_tambah_manfaat  where f='$f' and g='$g' and 	h='$h' and i='$i' and j='$j'";
	...
	/*if($cek_dt['cnt']>0){
		$err= 'Barang sudah ada !';
			...
			if($fmST == 0){ //input ref_tambah_manfaat
				
				if($persen1 < $persen1_lama ){
				 	$err= "Persentase Nilai Kedua tidak boleh kurang dari $persen1_lama";
				 }
				if($persen2 < $persen1 ){
				 	$err= "Persentase Kedua tidak boleh kurang dari $persen1";
				}
				...
				if($persen2 < $persen2_lama ){
					$selisih = abs($persen2_lama - $persen2);
					$content['selisih']=$selisih;
					$nilai = 0;
					$content['nilai']=0;
				// 	$err= "Terdapat selisih nilai Persentase Kedua sesudahnya dengan nilai Persentasi = '$selisih'";
				 }
				if($persen2 > $persen2_lama ){
				$cek_dt = mysql_fetch_array(mysql_query("select * from ref_tambah_manfaat  where f='$f' and g='$g' and h='$h' and i='$i' and j='$j' and persen1>=$persen2")); 
				$cek.="select * from ref_tambah_manfaat  where f='$f' and g='$g' and h='$h'and i='$i' and j='$j' and persen1>=$persen2 ";
					if($persen2 < $cek_dt['persen1']){
				//	$err= "Persentase Kedua tidak boleh kurang dari ";	
					}else{
					$err= "Persentase Kedua tidak boleh lebih dari $persen2 ";	
					}
				 	//$err= "Persentase Kedua tidak boleh kurang dari ";
				}
				...
				if($err==''){
						...
						$aqry2 = "UPDATE ref_tambah_manfaat
			        			  set "." 
								  persen1 ='$persen1',
								  persen2 ='$persen2',
	...
	function Hapus_Data($id){//id -> multi id with space delimiter
		...
		//$Kondisi = 	"Id='".$id."'";
		$TmbManfaat=mysql_fetch_array(mysql_query("select * from ref_tambah_manfaat where Id='$id'"));
		$cek.="select * from ref_tambah_manfaat where Id='$id'";
		$HpsManfaat=mysql_fetch_array(mysql_query("SELECT count(*) as cnt from penyusutan where f='".$TmbManfaat['f']."' and g='".$TmbManfaat['g']."' and  masa_manfaat_rehab!=0 and masa_manfaat_rehab is not null and CONCAT(f,'.',g,'.',h,'.',i,'.',j)"));
		$cek.="SELECT count(*) as cnt from penyusutan where f='".$TmbManfaat['f']."' and g='".$TmbManfaat['g']."' and  masa_manfaat_rehab!=0 and masa_manfaat_rehab is not null and CONCAT(f,'.',g,'.',h,'.',i,'.',j)";
	
		if($HpsManfaat['cnt'] > 0)$err='Tidak bisa di Hapus, sudah digunakan Penyusutan !!';
	...
	function setPage_OtherScript(){
		...
		return 
			 "<script type='text/javascript' src='js/master/reftambahmanfaat/".strtolower($this->Prefix).".js' language='JavaScript' ></script>".
	...
	function setFormBaru(){
  	...
  	function setFormEdit(){
		...
		$MasaManfaatRehab=mysql_fetch_array(mysql_query("SELECT count(*) as cnt from penyusutan where f='".$dt['f']."' and g='".$dt['g']."' and masa_manfaat_rehab!=0 and masa_manfaat_rehab is not null and CONCAT(f,'.',g,'.',h,'.',i,'.',j)"));
		$cek.="SELECT count(*) as cnt from penyusutan where f='".$dt['f']."' and g='".$dt['g']."' and masa_manfaat_rehab!=0 and masa_manfaat_rehab is not null and CONCAT(f,'.',g,'.',h,'.',i,'.',j)";
		if($MasaManfaatRehab['cnt'] > 0)$err='Tidak bisa di Edit, sudah digunakan Penyusutan !!';
       ...
       //items ----------------------
		  $this->form_fields = array(
			'kode_barang' => array( 
								'label'=>'Kode barang',
								'labelWidth'=>100, 
								'value'=>"<input type='text' name='kode_barang' value='".$dt['kode_barang']."' maxlength='$lengthKodeBrg' size='10px' id='kode_barang' readonly>
										<input type='text' id='nama_barang' name='nama_barang' value='".$brg['nm_barang']."' style='width:400px;' size='50px' readonly>" 
									 ),	
									 
			'persentase' => array( 
								'label'=>'Persentase',
								'labelWidth'=>100, 
								'value'=>"> &nbsp&nbsp<input type='text' name='persen1' value='".str_replace(".",",",$dt['persen1'])."' maxlength='6' size='5px' id='persen1' onkeypress='return isNumberKey(event);' $readonly> % &nbsp&nbsp s/d &nbsp&nbsp
										<input type='text' name='persen2' value='".str_replace(".",",",$dt['persen2'])."' maxlength='6' size='5px' id='persen2' onkeypress='return isNumberKey(event);'> %" 
									 ),
			
			'masa_manfaat' => array( 
								'label'=>'Masa Manfaat',
								'labelWidth'=>100, 
								'value'=>"<input type='text' name='masa_manfaat' value='".$dt['masa_manfaat']."' maxlength='4' size='8px' id='masa_manfaat' >&nbsp; Tahun" 
									 ),
		...
		//tombol
		$this->form_menubawah =	
		"<input type='hidden' name='persen1_lama' value='".str_replace(".",",",$dt['persen1'])."' maxlength='6' size='5px' id='persen1_lama' onkeypress='return isNumberKey(event);' $readonly>".
		"<input type='hidden' name='persen2_lama' value='".str_replace(".",",",$dt['persen2'])."' maxlength='6' size='5px' id='persen2_lama' onkeypress='return isNumberKey(event);' $readonly>".
	...
	function setKolomHeader($Mode=1, $Checkbox=''){
	 ...
	 <tr>
	   ...
	   <th class='th01' align='center' width='100'>Persentase</th>
	   <th class='th01' align='center' width='100'>Penambahan <br> Masa Manfaat (Tahun)</th>	   
	   </tr>
	   </thead>";
	...
	function setKolomData($no, $isi, $Mode, $TampilCheckBox){
	 ...
	 if($kode_barang == $this->kdbrg){
	 	$Koloms[] = array('align="center" width="100"','');
		$this->kdbrg = $kode_barang;
	 }else{
	 	$Koloms[] = array('align="center" width="100"',$kode_barang);
		$this->kdbrg = $kode_barang;
	 }
 	 $Koloms[] = array('align="left" width="200"',$brg['nm_barang']);	 	 	 	 
 	 $Koloms[] = array('align="left" width="200"',str_replace(".",",",$isi['persen1']).'% &nbsp; s/d &nbsp; '.str_replace(".",",",$isi['persen2']).'%');	 
	 $Koloms[] = array('align="center" width="200"',$isi['masa_manfaat']);
	 return $Koloms;
	...
	$TampilOpt = 
			...
			<tr>
			<td style='width:150px'>JENIS</td><td style='width:10px'>:</td>
			<td>".
	...
	function getDaftarOpsi($Mode=1){
		...
		$arrOrders = array();
		switch($fmORDER1){
			case '': $arrOrders[] = " concat(f,g,h,i,j), persen1 " ;break;
...
$RefTambahManfaat = new RefTambahManfaatObj();
