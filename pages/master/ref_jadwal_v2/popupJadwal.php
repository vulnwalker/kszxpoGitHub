...
class popupJadwal_v2Obj  extends DaftarObj2{	
	var $Prefix = 'popupJadwal_v2';
	var $elCurrPage="HalDefault";
	var $SHOW_CEK = TRUE;
	var $TblName = 'ref_jadwal_v2'; //daftar
	var $TblName_Hapus = 'ref_jadwal_v2';
	var $MaxFlush = 10;
	...
	var $FormName = 'popupJadwal_v2Form'; 	
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){
		...
		case 'windowshow':{
	   	...
	   	case 'getdata':{
			...
			if(empty($tahun)){
				...
				mysql_query("delete from ref_tahap_anggaran_v2 where tahun = '$tahun' and anggaran = '$jenis_anggaran'");
				$getData = mysql_query("select * from ref_jadwal_v2 where status_aktif = 'AKTIF'  order by id");
				$nomorUrut = 1 ;
	   ...
	   case 'save':{
			...
			$query = VulnWalkerUpdate('ref_jadwal_v2', $data, " id ='$id'");
			if(empty($namaTahap)){
				$err = "Isi nama tahap";
		...
		break;
	   	...
	   	case 'newRow':{
			$getMaxNoUrut = mysql_fetch_array(mysql_query("select max(no_urut) from ref_jadwal_v2"));
			$no_urut = $getMaxNoUrut['max(no_urut)'];
			mysql_query("insert into ref_jadwal_v2 (nama_tahap) values ('')");
		break;
	   }
	   	case 'cekDataAda':{
			foreach ($_REQUEST as $key => $value) { 
			...
			if(mysql_num_rows(mysql_query("select * from ref_tahap_anggaran_v2 where tahun ='$tahun' and anggaran ='$jenis_anggaran'")) > 0){
				$status = 'ya';
			}else{
				$status = 'tidak';
			}
	   ...
	   case 'modulChanged':{
		...
		break;
	   	...
	   	case 'remove':{
			$id =  $_REQUEST['id'];
			mysql_query("delete from ref_jadwal_v2 where id='$id'");
		break;
	   }
	    	case 'cancel':{
			$id =  $_REQUEST['id'];
			mysql_query("delete from ref_jadwal_v2 where nama_tahap =''");
		break;
	   }
	 ...
	 $headerTable =
	  "<thead>
	   ...
	   <th class='th01' width='200'><span id='atasbutton'>
								<a href='javascript:popupJadwal_v2.newRow();' id='linkAtasButton' /><img id='gambarAtasButton' src='datepicker/add-256.png' style='width:20px;height:20px;' /></a>
								</span></th>
	   </tr>
	   </thead>";
...
$popupJadwal_v2 = new popupJadwal_v2Obj();
