...
class ref_tahap_anggaran_v2Obj  extends DaftarObj2{	
	var $Prefix = 'ref_tahap_anggaran_v2';
	var $elCurrPage="HalDefault";
	var $SHOW_CEK = TRUE;
	var $TblName = 'ref_tahap_anggaran_v2'; //bonus
	var $TblName_Hapus = 'ref_tahap_anggaran_v2';
	var $MaxFlush = 10;
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='ref_tahap_anggaran_v2.xls';
	var $namaModulCetak='JADWAL';
	var $Cetak_Judul = 'JADWAL';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'ref_tahap_anggaran_v2Form';
	var $noModul=14; 
		...
		$err ="TANGGAL SALAH";
				...
				if($err==''){
					$statusJadwal= 'NORMAL';
						...
						$nomor_urut = 1;
						$queryNoUrut = "select * from ref_tahap_anggaran_v2 where  anggaran ='$anggaran' and  tahun = '$tahun' order by id_tahap asc";
					...
					$query = VulnWalkerInsert("ref_tahap_anggaran_v2",$data);
					mysql_query($query);
					$getIDTahap = mysql_fetch_array(mysql_query("select max(id_tahap) as ID_TAHAP from ref_tahap_anggaran_v2"));
					...
					$dataDefault = array("tahun" => $tahun,
					 ...
					 $nomor_urut = 1;
								$execute = mysql_query("select * from ref_tahap_anggaran_v2 where  anggaran='$anggaran' and  tahun = '$tahun' ORDER BY id_tahap");
								while($rows = mysql_fetch_array($execute)){
									$dataUpdateAll = array("no_urut" => $nomor_urut);
									$currentIdTahap = $rows['id_tahap'];
									mysql_query(VulnWalkerUpdate('ref_tahap_anggaran_v2',$dataUpdateAll," id_tahap = '$currentIdTahap'") );
				...
				if($err==''){
						    ...
						    $data = array(
								   ...
								   'user' => $user
									...
									$query = VulnWalkerUpdate("ref_tahap_anggaran_v2",$data," id_tahap = '$idplh'");
									mysql_query($query);
   ...
   function setPage_OtherScript(){
		...
		return 	
			"<script type='text/javascript' src='js/master/ref_tahap_anggaran_v2/ref_tahap_anggaran.js' language='JavaScript' ></script> 
			<script type='text/javascript' src='js/master/ref_tahap_anggaran_v2/popupModul.js' language='JavaScript' ></script>
			<script type='text/javascript' src='js/master/ref_tahap_anggaran_v2/popupHistori.js' language='JavaScript' ></script>
	...
	function setFormBaru(){
  	...
  	function setFormEdit(){
		...
		$this->form_fmST = 1;				
		if($err == ''){
			$aqry = "SELECT * FROM  ref_tahap_anggaran_v2 WHERE id_tahap='".$this->form_idplh."' "; $cek.=$aqry;
	...
	function setForm($dt){	
	 ...
	 $this->form_height = 400;
	  if ($this->form_fmST==0) {
		...
		$status = "PASIF";
		$getMaxNomorAku = mysql_fetch_array(mysql_query("select max(id_tahap) max from ref_tahap_anggaran_v2"));
		$maxNomorAku = $getMaxNomorAku['max'];
		$getNomorAku = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran_v2 where id_tahap = '$maxNomorAku'"));
		...
		$idTahap = $dt['id_tahap'];
		if(mysql_num_rows(mysql_query("select * from tabel_anggaran_v2 where id_tahap = '$idTahap'")) != 0){
		$adaGak = "disabled";
	}	
	  }
   ...
   $arrayAnggaran = array(	array('MURNI','MURNI'),
    ...
    $findModul = "<input type='text' id ='namaModul' name = 'namaModul' value='$namaModul' style='width : 300px;' readonly > <input type='hidden' name='idModul' id='idModul' value='$idModul'>  <button type='button' onclick='ref_tahap_anggaran_v2.CariModul()'>CARI </button>";
   
	$maxNoUrut = $nomor_aku ;
	$getDataMax = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran_v2 where no_urut = '$maxNoUrut' and anggaran = '$maxJenisAnggaran' and tahun = '$maxTahun'  " ));
...
$getDataMaxSetelah = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran_v2 where no_urut = '$maxNoUrutSesudahnya' and anggaran = '$maxJenisAnggaran' and tahun = '$maxTahun' " ));
	...
	function setKolomData($no, $isi, $Mode, $TampilCheckBox){
	 ...
	 $Koloms[] = array('align="left"',"<a onclick=ref_tahap_anggaran_v2.histori('$ID_TAHAP') style='cursor : pointer;'>".$isi['status_jadwal']."</a>");
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	function getDaftarOpsi($Mode=1){
		...
		$grabAllTahap = mysql_query("select * from ref_tahap_anggaran_v2");
		while($rows = mysql_fetch_array($grabAllTahap)){
			...
			mysql_query(VulnWalkerUpdate('ref_tahap_anggaran_v2',$data,"id_tahap ='$id_tahap'")); 
	...
	function Hapus($ids){ //validasi hapus ref_kota
		...
		for($i = 0; $i<count($ids); $i++)	{
			...
			$cekAdaData = mysql_num_rows(mysql_query("select * from tabel_anggaran_v2 where id_tahap = '$ids[$i]'"));
			if($cekAdaData != 0){
...
$ref_tahap_anggaran_v2 = new ref_tahap_anggaran_v2Obj();
?>