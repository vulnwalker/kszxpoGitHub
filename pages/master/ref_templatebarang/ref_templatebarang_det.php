...
class ref_templatebarang_detObj  extends DaftarObj2{	
	var $Prefix = 'ref_templatebarang_det';
	var $elCurrPage="HalDefault";
	var $SHOW_CEK = TRUE;
	var $TblName = 'v1_ref_templatebarang_det'; //daftar 
	var $TblName_Hapus = 'ref_templatebarang_det';
	var $MaxFlush = 10;
	...
	var $SumValue = array();
	var $fieldSum_lokasi = array(5,7,9);
	...
	var $FormName = 'ref_templatebarang_detForm'; 
	...
	function setFormBaru(){
  	...
  	function setFormEdit(){
		...
		$this->form_fmST = 1;				
		//get data 
		$aqry = "SELECT rtd.*, rb.nm_barang FROM  ref_templatebarang_det rtd LEFT JOIN ref_barang rb ON (rtd.f1=rb.f1 AND rtd.f2=rb.f2 AND rtd.f=rb.f AND rtd.g=rb.g AND rtd.h=rb.h AND rtd.i=rb.i AND rtd.j=rb.j) WHERE Id='".$this->form_idplh."' "; $cek.=$aqry;
	...
	function setKolomData($no, $isi, $Mode, $TampilCheckBox){
	 ...
	 $Koloms[] = array('align="center"', $no.'.' );
	  ...
	  $Koloms[] = array('align="center"',$barangDistribusi);
	  $Koloms[] = array('align="left"',$isi['keterangan']);
	  $Koloms[] = array('align="center"',
	  	"<a href='javascript:".$this->Prefix.".GandakanPenerimaan(`".$isi['Id']."`)' />
	...
	function getDaftarOpsi($Mode=1){
		...
		$ref_id = cekPOST("ref_templatebarang_idplh");
		...
		$arrKondisi[] = " refid_templatebarang='$ref_id'";
		$arrKondisi[] = " status!='2'";
	...
	function DataCopy(){
		...
		$qry = $DataPengaturan->QyrTmpl1Brs("ref_templatebarang_det", "*", "WHERE Id='$IdDet' AND refid_templatebarang='$Idplh' AND status!='2'");$cek.=$qry["cek"];
		$dt = $qry["hasil"];
		...
		if($dt['Id'] == "" || $dt["Id"] == NULL){
			...
			//for($i=0;$i<=1000;$i++){
				$qry_ins = $DataPengaturan->QryInsData("ref_templatebarang_det", $data);$cek.=" | ".$qry_ins["cek"];
			//}
...
$ref_templatebarang_det = new ref_templatebarang_detObj();
?>