...
class cari_akunObj  extends DaftarObj2{	
	var $Prefix = 'cari_akun';
...
//	$nm_acc= $_REQUEST['nm_account'];
	...
	case 'formBaru':{				
	  ...
	  // =1 1 1 2 0 0
		...
		$cek.="select *, concat(ka,'.',kb,'.',kc,'.',kd,'.',ke,'.',kf) as kodejurnal  from ref_jurnal where ka='$ka' AND kb='$kb' AND kc='$kc' AND kd='$kd' AND ke='$ke' and kf='$kf'";		
		$kode_jurnal = $get['kodejurnal'];	
		$nama_jurnal = $get['nm_account'];
		$content = array('kode_jurnal' => $kode_jurnal, 'nama_jurnal' => $nama_jurnal);
...
//		$lastkode=$get['maxno'] + 1;
	...
	$newkee = sprintf("%02s",$isi['ke']);
	 ...
	 $Koloms[] = array('align="left" width="" ',"<a style='cursor:pointer;' onclick=cari_akun.windowSave('".$kode_jurnal."')>".$isi['nm_account']."</a>");	 	 	 
	 return $Koloms;
	}
