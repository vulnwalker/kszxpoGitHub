...
//	var $TblName = 'ref_tandatangan'; //daftar
	...
	// if( $err=='' && $eselon =='' ) $err= 'ESELON Belum Di Pilih !!';
			...
			if($err=='' && $oldy['cnt']>0) $err="NIP '$nippegawai' Sudah Ada";
				if($err==''){
					$aqry = "INSERT into ref_tandatangan (c1,c,d,e,e1,nama,nip,jabatan,pangkat,gol,ruang,eselon,ref_kategori) values('$kode1','$kode2','$kode3','$kode4','$kode5','$namapegawai','$nippegawai','$jabatan','$p1','$golongan[0]','$golongan[1]','$eselon','$kategori')";	$cek .= $aqry;	
					$qry = mysql_query($aqry);
				}
			}else{						
				if($err==''){
				$aqry = "UPDATE ref_tandatangan SET nama='$namapegawai', nip='$nippegawai', jabatan='$jabatan' ,pangkat='$p1', gol='$golongan[0]' ,ruang='$golongan[1]',eselon='$eselon' ,ref_kategori='$kategori' WHERE Id='".$idplh."'";	$cek .= $aqry;
	...
	//	$cek.=$data;
		...
		$kategori="select * from ref_kategori_tandatangan";	$cek.=$kategori;
		...
		$date1=$querye1['e1'].".".$querye1['nm_skpd'];
		 ...
		 $this->form_fields = array(
			...
			'kategori' => array( 
						...
						'labelWidth'=>150, 
						'value'=>
						"<div id='cont_gd'>".cmbQuery('kategori',$dt['ref_kategori'],$kategori,'style="width:250px;"onchange="'.$this->Prefix.'.()"','--PILIH--')."&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp"."<input type ='hidden' name='kategori_ttd' id='kategori_ttd' value='".$kategori['kategori_tandatangan']."'></div>",
...
//	$kategori=mysql_fetch_array(mysql_query("select kategori_tandatangan as kategori from ref_kategori_tandatangan where id='".$isi['ref_kategori']."'"));
	 ...
	 $skpd=mysql_fetch_array(mysql_query(" select nm_skpd from ref_skpd where c='$c' and d='$d' and e='$e' and e1='$e1' "));
	 $Koloms = array();
	 $Koloms[] = array('align="center"', $no.'.' );
	  if ($Mode == 1) $Koloms[] = array(" align='center' ", $TampilCheckBox);
	//$Koloms[] = array('align="left"',$kategori['kategori']);	
...
&nbsp"							
	...
	function getDaftarOpsi($Mode=1){
		...
		if($ref_tandatanganSkpdfmURUSAN!='00' and $ref_tandatanganSkpdfmSKPD !='')$arrKondisi[]= "c1='$ref_tandatanganSkpdfmURUSAN'";
			if($ref_tandatanganSkpdfmSKPD!='00' and $ref_tandatanganSkpdfmSKPD !='')$arrKondisi[]= "c='$ref_tandatanganSkpdfmSKPD'";
			if($ref_tandatanganSkpdfmUNIT!='00' and $ref_tandatanganSkpdfmUNIT !='')$arrKondisi[]= "d='$ref_tandatanganSkpdfmUNIT'";
			if($ref_tandatanganSkpdfmSUBUNIT!='00' and $ref_tandatanganSkpdfmSUBUNIT !='')$arrKondisi[]= "e='$ref_tandatanganSkpdfmSUBUNIT'";
			if($ref_tandatanganSkpdfmSEKSI!='00' and $ref_tandatanganSkpdfmSEKSI !='')$arrKondisi[]= "e1='$ref_tandatanganSkpdfmSEKSI'";	
