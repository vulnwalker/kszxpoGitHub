...
class rincianBelanjaModalObj  extends DaftarObj2{
	var $Prefix = 'rincianBelanjaModal';
	...
	var $Cetak_Judul = 'DAFTAR RINCIAN BELANJA MODAL';
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'rincianBelanjaModalForm';
	var $kode_skpd = '';
	var $username = '';
	var $WHERE_O = "";

	function setTitle(){
		return 'DAFTAR RINCIAN BELANJA MODAL';
	}
	...
	function simpan(){
	 ...
	 if(mysql_num_rows(mysql_query("select * from temp_rincian_belanja_modal where username = '$this->username' and status ='checked'")) == 0){
	 	$err ="Pilih Barang !";
	 	...
	 	$err ="Pilih Kode Rekening !";
				...
				if($err==''){
						...
						$getData = mysql_query("select * from temp_rincian_belanja_modal where username = '$this->username'  ");
	...
	function set_selector_other($tipe){
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){
		...
		case 'formBaru':{
			mysql_query("delete from temp_rincian_belanja_modal where username ='$this->username'");
			$fm = $this->setFormBaru();
			$cek = $fm['cek'];
			$err = $fm['err'];
			$content = $fm['content'];
		...
		case 'editData':{
			...
			$kodeRekening = explode(" ",  $rincianBelanjaModal_cb[0]);
			$k = $kodeRekening[0];
			$l = $kodeRekening[1];
			$m = $kodeRekening[2];
			$n = $kodeRekening[3];
			...
			mysql_query("delete from temp_rincian_belanja_modal where username ='$this->username'");
			$getData = mysql_query("select * from ref_barang where k11='".$k."' and l11='".$l."' and m11='".$m."' and n11='".$n."' and o11='".$o."' ");
			...
			while ($rows = mysql_fetch_array($getData)) {
				...
				$query = VulnWalkerInsert("temp_rincian_belanja_modal",$data);
				mysql_query($query);
			}
	...
	function setPage_OtherScript(){
		...
		return
			"<script type='text/javascript' src='js/skpd.js' language='JavaScript' ></script>".
			 "<script type='text/javascript' src='js/master/rincianBelanjaModal/rincianBelanjaModal.js' language='JavaScript' ></script>
	...
	function setForm($kodeRekening){
	 ...
	 $this->form_height = 200;
       ...
       //items ----------------------
		  $this->form_fields = array(
			...
			'asdas' => array(
						...
						<div id='listBarang' style='height:5px'>
							"."<div id='listBarangBelanjaModal_cont_title' style='position:relative'></div>".
			"<div id='listBarangBelanjaModal_cont_opsi' style='position:relative'>".
			"</div>".
			"<div id='listBarangBelanjaModal_cont_daftar' style='position:relative'></div>".
			"<div id='listBarangBelanjaModal_cont_hal' style='position:relative'></div>"."
						</div>",
		...
		$this->form_menubawah =
			"
			<input type='hidden' id ='c1' name ='c1' value='$rincianBelanjaModalSkpdfmUrusan'>
			<input type='hidden' id ='c' name ='c' value='$rincianBelanjaModalSkpdfmSKPD'>
			<input type='hidden' id ='d' name ='d' value='$rincianBelanjaModalSkpdfmUNIT'>
			<input type='hidden' id ='e' name ='e' value='$rincianBelanjaModalSkpdfmSUBUNIT'>
			<input type='hidden' id ='e1' name ='e1' value='$rincianBelanjaModalSkpdfmSEKSI'>
	...
	function genForm2($withForm=TRUE){
		$form_name = 'listBarangBelanjaModalForm';
	...
	function setKolomData($no, $isi, $Mode, $TampilCheckBox){
	 ...
	 if(mysql_num_rows(mysql_query("select * from ref_barang where k11='$k' and l11='$l' and m11='$m' and n11='$n' and o11='$o'")) == 0){
		 	$warnaStatus="red";
			$listKodebarang = "";
			$listNomorUrut = "";
			$listNamaBarang = "";
	 }else{
		 	$getAllBarangFromRekening = mysql_query("select * from ref_barang where k11='$k' and l11='$l' and m11='$m' and n11='$n' and o11='$o' $this->kondisiBarang");
			$nomorUrut = 1;
			while($dataBarang = mysql_fetch_array($getAllBarangFromRekening)){
					...
					$listNamaBarang.=$dataBarang['f'].".".$dataBarang['g'].".".$dataBarang['h'].".".$dataBarang['i'].".".$dataBarang['j']." &nbsp ".$getBarang['nm_barang']."<br>";
					$nomorUrut += 1;
			}
	 }
				...
				<td width=\"40%\" valign=\"top\"><h2>BARANG</h2>
					<table>
						<tr>".
							...
							<td>".
							cmbQuery1("cmbJenis",$cmbJenis,"select f as valueCmbJenis, nm_barang from ref_barang where f1 = '$cmbAkun' and f2 = '$cmbKelompok' and f != '00'  and g ='00' and h ='00' and i='00' and j = '000' and f !='06'  and f !='08'","onChange=$this->Prefix.refreshList(true) ",'Pilih','').
							"</td>
							</tr><tr>
	...
	function getDaftarOpsi($Mode=1){
		...
		// $arrKondisi[] = "concat(k11,'.',l11,'.',m11,'.',n11,'.',o11) !='....'";
		// $arrKondisi[] = "concat(k11,'.',l11,'.',m11,'.',n11,'.',o11) !='0.0.0.0.0'";
		...
		if(!empty($this->kondisiBarang)){
				$getBarang = mysql_query("select * from ref_barang where j!='000' and k11='5' and l11='2' and m11='3' $this->kondisiBarang");
				while($dataBarang = mysql_fetch_array($getBarang)){
							$concatRekeningBarang = $dataBarang['k11'].".".$dataBarang['l11'].".".$dataBarang['m11'].".".$dataBarang['n11'].".".$dataBarang['o11'];
				...
				if(sizeof($arrayExceptRekening) > 0 ){
						$getAllRekening = mysql_query("select * from ref_rekening where k='5' and l='2' and m='3' and  $kondisiExcept ");
		...
		//$arrOrders[] = "k11,l11,m11,n11,o11,f,g,h,i,j ASC " ;
	...
	function editData($id){
	 ...
	 $this->form_height = 200;
		...
		$getNamaRekening =  mysql_fetch_array(mysql_query("select * from ref_barang where concat(k,'.',l,'.',m,'.',n,'.',o) = '$kodeRekening' "));
		$namaRekening = $getNamaRekening['nm_rekening'];
...
$rincianBelanjaModal = new rincianBelanjaModalObj();
$rincianBelanjaModal->username = $_COOKIE['coID'];
if($Main->REK_DIGIT_O == 0){
	$rincianBelanjaModal->WHERE_O = "00";
}else{
	$rincianBelanjaModal->WHERE_O = "000";
}
?>
