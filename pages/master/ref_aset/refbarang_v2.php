...
class RefBarang_v2Obj  extends DaftarObj2{	
	var $Prefix = 'RefBarang_v2';
	var $elCurrPage="HalDefault";
	var $SHOW_CEK = TRUE;
	var $TblName = 'ref_barang_v2'; //daftar
	var $TblName_Hapus = 'ref_barang_v2';
	var $MaxFlush = 10;
 	 ...
 	 if($err=='' && $nama_barang =='' ) $err= 'Nama Barang belum diisi';	 	 
			...
			if($fmST == 0){ 
				if($err==''){ 
						...
						$qry =  mysql_query(VulnWalkerInsert("ref_barang_v2",$data));
						if($qry){
				...
				if($err==''){
						...
						$qry = mysql_query(VulnWalkerUpdate('ref_barang_v2',$data,"concat(f1,'.',f2,'.',f,'.',g,'.',h,'.',i,'.',j) = '$forWhere'"));
						$cek = VulnWalkerUpdate('ref_barang_v2',$data,"concat(f1,'.',f2,'.',f,'.',g,'.',h,'.',i,'.',j) = '$forWhere'");
						if($qry==FALSE)
						{ 
							$err="Gagal menyimpan barang";
						}else{
	...
	function set_selector_other($tipe){
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){
		...
		$get = mysql_fetch_array( mysql_query("select * from ref_barang_v2 where f=$f and g=$g and h=$h and i=$i and j=$j"));
	...
	function setPage_OtherScript(){
		...
		return 
			 "<script type='text/javascript' src='js/master/ref_aset/refbarang_v2.js' language='JavaScript' ></script>".
	...
	function Hapus_Validasi($id){//id -> multi id with space delimiter
		...
		if($f2 =='0'){
		  $hasRow = mysql_num_rows(mysql_query("select * from ref_barang_v2 where f1 ='$f1' and f2!='0' "));
		  if($hasRow > 0){
		  	$errmsg = "Data tidak dapat di hapus";
		  }
		}else{
		   if($f =='0' ){
		   $hasRow = mysql_num_rows(mysql_query("select * from ref_barang_v2 where f1 ='$f1' and f2='$f2' and f !='0' and g ='00' and h ='00' and i ='00' and j = '000' "));
			  if($hasRow > 0){
			  	$errmsg = "Data tidak dapat di hapus";
		 	 }
		  }else{
			  	if( $g=='00'){
			  $hasRow = mysql_num_rows(mysql_query("select * from ref_barang_v2 where f1 ='$f1' and f2='$f2' and f='$f' and g !='00' and h ='00' and i ='00' and j = '000' "));
			  if($hasRow > 0){
			  	$errmsg = "Data tidak dapat di hapus";
			  }
			}else{
				if( $h=='00'){
		  $hasRow = mysql_num_rows(mysql_query("select * from ref_barang_v2 where f1 ='$f1' and f2='$f2' and f='$f' and g ='$g' and h !='00' and i ='00' and j = '000' "));
		  if($hasRow > 0){
		  	$errmsg = "Data tidak dapat di hapus";
		  }
		}else{
			if( $i=='00'){
		  $hasRow = mysql_num_rows(mysql_query("select * from ref_barang_v2 where f1 ='$f1' and f2='$f2' and f='$f' and g ='$g' and h='$h' and i!='00'  and j = '000' "));
		  if($hasRow > 0){
		  	$errmsg = "Data tidak dapat di hapus";
		  }
		}else{
			if( $j=='000'){
		  $hasRow = mysql_num_rows(mysql_query("select * from ref_barang_v2 where f1 ='$f1' and f2='$f2' and f='$f' and g ='$g' and h='$h'  and i='$i' and j!='000' "));
		  if($hasRow > 0){
		  	$errmsg = "Data tidak dapat di hapus";
		  }
		}
	...
	function setFormBaru(){
  	...
  	function setFormEdit(){
		...
		$j=$kode[6]; 
		$bulan=date('Y-m-')."1";
		//query ambil data ref_barang_v2
		$aqry = "select * from ref_barang_v2 where concat(f1,'.',f2,'.',f,'.',g,'.',h,'.',i,'.',j)='".$f1.'.'.$f2.'.'.$f.'.'.$g.'.'.$h.'.'.$i.'.'.$j."'"; $cek.=$aqry;
	...
	function setForm($dt){	
	 ...
	 $kode_account_beban_penyusutan=$dt['n1'].'.'.$dt['n2'].'.'.$dt['n3'].'.'.$dt['n4'].'.'.$dt['n5'];
		...
		$sampleKodeBrg = "*1.0.00.00.00.00.000" ;
		
		//query ref_batal
		$queryKB = "SELECT f,nama_barang FROM ref_barang_v2_persediaan where f !='00' and g='00'";
...
/*			'' => array( 
			...
			'kode_barang' => array( 
								'label'=>'KODE BARANG',
								'labelWidth'=>200, 
								'value'=>"<input type='text' id='f1' name='f1' style='width:20px; ' maxlength='1' onkeypress='return event.charCode >= 48 && event.charCode <= 57' value='$f1' $chmod644> &nbsp <input type='text' id='f2' name='f2' style='width:20px;' maxlength='1' onkeypress='return event.charCode >= 48 && event.charCode <= 57' value='$f2' $chmod644> &nbsp <input type='text' id='f' name='f' style='width:25px;' maxlength='2' onkeypress='return event.charCode >= 48 && event.charCode <= 57' value='$f' $chmod644> &nbsp <input type='text' id='g' name='g' style='width:25px;' maxlength='2' onkeypress='return event.charCode >= 48 && event.charCode <= 57' value='$g' $chmod644> &nbsp <input type='text' id='h' name='h' style='width:25px;' maxlength='2' onkeypress='return event.charCode >= 48 && event.charCode <= 57' value='$h' $chmod644> &nbsp <input type='text' id='i' name='i' style='width:25px;' maxlength='2' onkeypress='return event.charCode >= 48 && event.charCode <= 57' value='$i' $chmod644> &nbsp <input type='text' id='j' name='j' style='width:30px;' maxlength='3' onkeypress='return event.charCode >= 48 && event.charCode <= 57' value='$j' $chmod644> &nbsp  
...
*/
	...
	$TampilOpt = 
			...
			<td>".
			cmbQuery1("cmbJenis",$cmbJenis,"select f as valueCmbJenis, nm_barang from ref_barang_v2 where f1 = '$cmbAkun' and f2 = '$cmbKelompok' and f != '00'  and g ='00' and h ='00' and i='00' and j = '000'","onChange=\"$this->Prefix.refreshList(true)\"",'Pilih','').
			"</td>
			</tr><tr>
			<td style='width:170px;'>OBYEK</td><td>:</td>
			<td>".
			cmbQuery1("cmbObyek",$cmbObyek,"select g as valueCmbObyek, nm_barang from ref_barang_v2 where f1 = '$cmbAkun' and f2 = '$cmbKelompok' and f = '$cmbJenis'  and g !='00' and h ='00' and i='00' and j = '000'","onChange=\"$this->Prefix.refreshList(true)\"",'Pilih','').
			"</td>
			</tr><tr>
			<td style='width:170px;'>RINCIAN OBYEK</td><td>:</td>
			<td>".
			cmbQuery1("cmbRincianObyek",$cmbRincianObyek,"select h as valueCmbRincianObyek, nm_barang from ref_barang_v2 where f1 = '$cmbAkun' and f2 = '$cmbKelompok' and f = '$cmbJenis'  and g ='$cmbObyek' and h !='00' and i='00' and j = '000'","onChange=\"$this->Prefix.refreshList(true)\"",'Pilih','').
			"</td>
				</tr>
			<tr>
			<td style='width:170px;'>SUB RINCIAN OBYEK</td><td>:</td>
			<td>".
			cmbQuery1("cmbSubRincianObyek",$cmbSubRincianObyek,"select i as valueCmbSubRincianObyek, nm_barang from ref_barang_v2 where f1 = '$cmbAkun' and f2 = '$cmbKelompok' and f = '$cmbJenis'  and g ='$cmbObyek' and h ='$cmbRincianObyek' and i != '00' and j = '000'","onChange=\"$this->Prefix.refreshList(true)\"",'Pilih','').
			"</td>
				</tr>
			<tr>
			<td style='width:170px;'>SUB-SUB RINCIAN OBYEK</td><td>:</td>
			<td>".
			cmbQuery1("cmbSubSubRincianObyek",$cmbSubSubRincianObyek,"select j as valueCmbSubSubRincianObyek, nm_barang from ref_barang_v2 where f1 = '$cmbAkun' and f2 = '$cmbKelompok' and f = '$cmbJenis'  and g ='$cmbObyek' and h ='$cmbRincianObyek' and i='$cmbSubRincianObyek' and j != '000'","onChange=\"$this->Prefix.refreshList(true)\"",'Pilih','').
			"</td>
				</tr>
...
$RefBarang_v2 = new RefBarang_v2Obj();
