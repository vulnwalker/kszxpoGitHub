...
class refstdbutuh_v3Obj  extends DaftarObj2{	
	var $Prefix = 'refstdbutuh_v3';
	var $elCurrPage="HalDefault";
	var $SHOW_CEK = TRUE;
	var $TblName = "ref_std_kebutuhan_v3 inner join ref_barang_v3 on concat(ref_std_kebutuhan_v3.f,'.',ref_std_kebutuhan_v3.g,'.',ref_std_kebutuhan_v3.h,'.',ref_std_kebutuhan_v3.i,'.',ref_std_kebutuhan_v3.j) = concat(ref_barang_v3.f,'.',ref_barang_v3.g,'.',ref_barang_v3.h,'.',ref_barang_v3.i,'.',ref_barang_v3.j)"; //daftar
	var $TblName_Hapus = 'ref_std_kebutuhan_v3';
	var $MaxFlush = 10;
	...
	var $FormName = 'refstdbutuh_v3Form'; 	
	var $kode_skpd = '';
			...
			if($fmST == 0){ 
				...
				if($err==''){ 
						$data = array ('c1' => $cmbUrusanForm,
						 ...
						 $cek .= VulnWalkerInsert("ref_std_kebutuhan_v3",$data);
						$input =  mysql_query(VulnWalkerInsert("ref_std_kebutuhan_v3",$data));
						if($input){
						...
						$cek .= VulnWalkerUpdate("ref_std_kebutuhan_v3",$data,"concat(c1,' ',c,' ',d,' ',e,' ',e1' ',f,' ',g,' ',h,' ',i,' ',j) = '$idplh'");	
						mysql_query(VulnWalkerUpdate("ref_std_kebutuhan_v3",$data,"concat(c1,' ',c,' ',d,' ',e,' ',e1,' ',f,' ',g,' ',h,' ',i,' ',j) = '$idplh'"));				
	...
	function setPage_OtherScript(){
		...
		return 
			"<script type='text/javascript' src='js/skpd.js' language='JavaScript' ></script>".		
			 "<script type='text/javascript' src='js/master/refstdbutuh_v3/refstdbutuh.js' language='JavaScript' ></script>".		
			 "<script type='text/javascript' src='js/master/refstandarharga_v3/refbarang.js' language='JavaScript' ></script>".		
			$scriptload;
	}
	...
	function setFormBaru(){
  	...
  	function setFormEdit(){
		...
		$aqry = "select * from ref_std_kebutuhan_v3 where concat(c1,' ',c,' ',d,' ',e,' ',e1,' ',f,' ',g,' ',h,' ',i,' ',j) ='".$this->form_idplh."' "; $cek.=$aqry;
...
$comboBoxSubUnit =  cmbQuery('cmbSubUnitForm', $selectedSubUnit, $codeAndNameSubUnit,' '.$cmbRo.' onChange=\''.$this->Prefix.'.BidangAfterform()\'','-- Pilih Semua --');          
		  ...
		  $this->form_fields = array(
			...
			'satuan' => array(
				...
				'jumlah' => array( 
						'label'=>'JUMLAH',
						'labelWidth'=>120, 
						'value'=> "<input type='text' name='jumlah' id='jumlah' value='$jumlah' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='document.getElementById(`bantu`).innerHTML = popupBarang_v3.formatCurrency2(this.value);' style='text-align:right'> <font color='red' id='bantu' name='bantu'></font>"  
							  ),									 	
	...
	$TampilOpt = 
			...
			<td width=\"100%\" valign=\"top\">" . 
				WilSKPD_ajxVW("refstdbutuh_v3Skpd") . 
			"</td>
			<td >" . 		
			"</td></tr>
			<tr><td>
	...
	function getDaftarOpsi($Mode=1){
		global $Main, $HTTP_COOKIE_VARS;
		$UID  = $_COOKIE['coID']; 
		$c1   = $_REQUEST['refstdbutuh_v3SkpdfmUrusan'];
		$c    = $_REQUEST['refstdbutuh_v3SkpdfmSKPD'];
		$d    = $_REQUEST['refstdbutuh_v3SkpdfmUNIT'];
		$e    = $_REQUEST['refstdbutuh_v3SkpdfmSUBUNIT'];
		$e1   = $_REQUEST['refstdbutuh_v3SkpdfmSEKSI'];	
			$fmKODE = $_REQUEST['fmKODE'];
	$fmBARANG = $_REQUEST['fmBARANG'];
		...
		if(!empty($e1) && $e1!="000")$arrKondisi[] = "e1 = $e1";
 		if(empty($fmKODE)){
			...
			$arrKondisi[]= "concat(ref_std_kebutuhan_v3.f1,'.',ref_std_kebutuhan_v3.f2,'.',ref_std_kebutuhan_v3.f,'.',ref_std_kebutuhan_v3.g,'.',ref_std_kebutuhan_v3.h,'.',ref_std_kebutuhan_v3.i,'.',ref_std_kebutuhan_v3.j) like '$fmKODE%'";
		}
		if(empty($fmBARANG)){
			
		}else{
			/*$query = "select * from ref_barang_v3 where nm_barang like '%$fmBARANG%'";
		...
		$arrOrders[] = " concat(ref_std_kebutuhan_v3.f1,'.',ref_std_kebutuhan_v3.f2,'.',ref_std_kebutuhan_v3.f,'.',ref_std_kebutuhan_v3.g,'.',ref_std_kebutuhan_v3.h,'.',ref_std_kebutuhan_v3.i,'.',ref_std_kebutuhan_v3.j) ASC " ;
...
$refstdbutuh_v3 = new refstdbutuh_v3Obj();
