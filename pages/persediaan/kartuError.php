...
class kartuErrorObj  extends DaftarObj2{
	var $Prefix = 'kartuError';
	var $elCurrPage="HalDefault";
	var $SHOW_CEK = TRUE;
	var $TblName = 'temp_lock'; //bonus
	var $TblName_Hapus = 'temp_lock';
	var $MaxFlush = 10;
...
function setKolomHeader($Mode=1, $Checkbox=''){
	...
	function selectAllCheckBox(){
	  $hsl = '';
	    ...
	    $hsl ="<input type='checkbox' name='kartuError_toggle' id='kartuError_toggle' value='' onclick=kartuError.checkSemua($jumlahData,'kartuError_cb','kartuError_toggle','kartuError_jmlcek')>";
	  /*}*/
	  return $hsl;
	}
	function setKolomData($no, $isi, $Mode, $TampilCheckBox){
	 ...
	 if($saldo < 0)$warna = "red";
	 $Koloms = array();
	 $Koloms[] = array('align="center" style="color:'.$warna.';"', $no.'.' );
	 $Koloms[] = array('align="center" style="color:'.$warna.';"', $this->generateDate($tanggal));
	 if($type == '2'){
		 	$uraian = "<span style='cursor:pointer;margin:10px;' onclick=daftarPersediaanBarang.detailPengeluaranTemp($id_t_kartu_persediaan)>$uraian</span>";
	 }
	 $Koloms[] = array('align="left" style="color:'.$warna.';"', $uraian);
	 $Koloms[] = array('align="right style="color:'.$warna.';""', number_format($masuk,0,',','.'));
	 $Koloms[] = array('align="right style="color:'.$warna.';""', number_format($keluar,0,',','.'));
	 $Koloms[] = array('align="right style="color:'.$warna.';""', number_format($saldo,0,',','.'));
	...
	function genDaftarOpsi(){
	 ...
	 $getDataLock = mysql_fetch_array(mysql_query("select * from t_kartu_persediaan where id = '$idPersediaan'"));
	 ...
	 $getMaxData = mysql_fetch_array(mysql_query("select max(id) from temp_lock where id_daftar = '$idPersediaan'"));
	 $getSaldo = mysql_fetch_array(mysql_query("select * from temp_lock where id = '".$getMaxData['max(id)']."'"));
		 ...
		 "</td>
	 	 ...
	 	 <td>".
		 $getSaldo['saldo']." &nbsp".$getNamaBarang['satuan'].
		 "</td>
	 	 </tr>
	 		 ";
	...
	function getDaftarOpsi($Mode=1){
		...
		$arrKondisi[] = "id_daftar = '$idPersediaan'";
	...
	function copyDataBarang($kondisiBarang){
		$getAllBarang = mysql_query("select * from temp_lock where j!='000' $kondisiBarang");
		while($rows = mysql_fetch_array($getAllBarang)){
			foreach ($rows as $key => $value) {
				  $$key = $value;
			 }
			 $kodeBarang =$f.".".$g.".".$h.".".$i.".".$j;
			 if(mysql_num_rows(mysql_query("select * from temp_lock where username = '$this->username' and concat(f,'.',g,'.',h,'.',i,'.',j) = '$kodeBarang'")) == 0){
			 		...
			 		$query = VulnWalkerInsert('temp_lock',$data);
			 		mysql_query($query);
			 }
...
$kartuError = new kartuErrorObj();
$kartuError->username = $_COOKIE['coID'];
?>
