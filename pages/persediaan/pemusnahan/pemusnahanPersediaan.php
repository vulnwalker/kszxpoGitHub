...
class pemusnahanPersediaanObj  extends DaftarObj2{
	var $Prefix = 'pemusnahanPersediaan';
	var $elCurrPage="HalDefault";
	var $SHOW_CEK = TRUE;
	var $TblName = 'cek_fisik'; //bonus
	var $TblName_Hapus = 'cek_fisik';
	var $MaxFlush = 10;
	...
	var $PageTitle = 'PEMUSNAHAN';
	var $PageIcon = 'images/masterData_01.gif';
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='pemusnahanPersediaan.xls';
	var $namaModulCetak='PEMUSNAHAN';
	var $Cetak_Judul = 'PEMUSNAHAN';
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'pemusnahanPersediaanForm';
	var $modul = "PEMUSNAHAN";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	var $jenisAnggaran = "";
  ...
  //  $dat_urusan= $_REQUEST['dat_urusan'];
    ...
    if ($urusan=='0'){
      ...
      $dt['c1'] = $_REQUEST[$this->Prefix.'pemusnahanPersediaanSKPD2fmURUSAN'];
      $dt['c'] = $_REQUEST[$this->Prefix.'pemusnahanPersediaanSKPD2fmSKPD'];
      $dt['d'] = $_REQUEST[$this->Prefix.'pemusnahanPersediaanSKPD2fmUNIT'];
      $dt['e'] = $_REQUEST[$this->Prefix.'pemusnahanPersediaanSKPD2fmSUBUNIT'];
      $dt['e1'] = $_REQUEST[$this->Prefix.'pemusnahanPersediaanSKPD2fmSEKSI'];
      $fm = $this->setForm($dt);
    }
  ...
  $oldy=mysql_fetch_array(
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){
			...
			case 'pemusnahanPersediaan':{
				...
				$fm = $this->pemusnahanPersediaan($idPersediaan);
				$cek .= $fm['cek'];
				$err .= $fm['err'];
				$content = $fm['content'];

				// $content = array(
				// 									'spanBerkurang' => "<input type='text' name='berkurang$idPersediaan' id='berkurang$idPersediaan' value='".$getDatapemusnahanPersediaan['berkurang']."' style='width:40px;text-align:right;' onkeyup=$this->Prefix.bantu('berkurang$idPersediaan','spanBerkurangBantu$idPersediaan',$idPersediaan)> <span id='spanBerkurangBantu$idPersediaan'></span>",
				// 									'spanBertambah' => "<input type='text' name='bertambah$idPersediaan' id='bertambah$idPersediaan' value='".$getDatapemusnahanPersediaan['bertambah']."' style='width:40px;text-align:right;'  onkeyup=$this->Prefix.bantu('bertambah$idPersediaan','spanBertambahBantu$idPersediaan',$idPersediaan)> <span id='spanBertambahBantu$idPersediaan'></span>",
				// 									'spanKeterangan' => "<input type='text' name='keterangan$idPersediaan' id='keterangan$idPersediaan' value='".$getDatapemusnahanPersediaan['keterangan']."' style='width:100%;'>",
				// 									'spanJumlah' => "<span id='spanJumlah$idPersediaan' style='text-align:right;'> <span>",
				// 									'spanAction' => "<span id='spanAction$idPersediaan' style='text-align:center;'><img src='datepicker/save.png' style='width:20px;height:20px;cursor:pointer;' onclick= $this->Prefix.savepemusnahanPersediaan($idPersediaan); >
    ...
    break;
		...
		case 'savepemusnahanPersediaan':{
			...
			if($jenisPersediaan == '4'){
					$jnspemusnahanPersediaan = "4";
					$jenisPersediaan = '1';
					$bertambah = $jumlahCheck;
			}elseif($jenisPersediaan == '5'){
					$jnspemusnahanPersediaan = "5";
					$jenisPersediaan = '2';
					$berkurang = $jumlahCheck;
					$hargaSatuan = "0";
			}
			$getData = mysql_fetch_array(mysql_query("select * from t_kartu_persediaan where id = '$idFisik'"));
			if(mysql_num_rows(mysql_query("select * from t_persediaan_lock_barang where  c1 ='".$getData['c1']."' and c ='".$getData['c']."' and d ='".$getData['d']."' and e ='".$getData['e']."' and e1 ='".$getData['e1']."' and f1 ='".$getData['f1']."' and f2 ='".$getData['f2']."' and f ='".$detailPengeluaran['f']."' and g ='".$getData['g']."' and h ='".$getData['h']."' and i ='".$getData['i']."' and j ='".$getData['j']."' and j1 ='".$getData['j1']."' and tahun = '".$_COOKIE['coThnAnggaran']."' and semester = '$semesterPosting' ")) != 0)
			{
			...
			if(empty($err)){
				...
				if(mysql_num_rows(mysql_query("select * from cek_fisik where id_persediaan = '$idFisik' and semester = '$periode'")) == 0){
					$datapemusnahanPersediaan = array(
									'c1' => $getData['c1'],
									'c' => $getData['c'],
									'd' => $getData['d'],
									'e' => $getData['e'],
					...
					$query = VulnWalkerInsert("cek_fisik",$datapemusnahanPersediaan);
					mysql_query($query);
					...
					$query = VulnWalkerUpdate("cek_fisik",$datapemusnahanPersediaan,"id_persediaan ='$idFisik' and semester = '$periode'");
					mysql_query($query);
		...
		case 'editTab':{
			...
			mysql_query("delete from temp_detail_rincian_distribusi where username = '$this->username'");
			mysql_query("delete from delete_temp_detail_rincian_distribusi where username = '$this->username'");
			$id = $_REQUEST['pemusnahanPersediaan_cb'];
			foreach ($_REQUEST as $key => $value) {
				  $$key = $value;
			}
		...
		case 'Remove':{
			...
			$get = mysql_fetch_array(mysql_query("select * from tabel_PEMUSNAHAN where id ='$id[0]'"));
			$kodeRek = $get['k'].".".$get['l'].".".$get['m'].".".$get['n'].".".$get['o'] ;

			$getAll = mysql_query("select * from tabel_PEMUSNAHAN where concat(k,'.',l,'.',m,'.',n,'.',o) ='$kodeRek' and id_tahap='$this->idTahap' and rincian_perhitungan !=''   and c1='$cmbUrusan' and c='$cmbBidang' and d='$cmbSKPD'  and status_validasi !='1' order by o1, rincian_perhitungan");
			...
			while($rows = mysql_fetch_array($getAll)){
				...
				mysql_query("delete from tabel_PEMUSNAHAN where id = '$id'");
				//mysql_query("delete from tabel_PEMUSNAHAN where concat(k,'.',l,'.',m,'.',n,'.',o) ='$kodeRek' and o1 ='$o1' and c1='$cmbUrusan' and c='$cmbBidang' and d='$cmbSKPD' and e='$cmbUnit' and e1='$cmbSubUnit' and jenis_rka='2.1' and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'");
	    ...
	    case 'Validasi':{
				...
				$this->form_idplh = $_REQUEST['id'];
					...
					$qry = "SELECT * FROM tabel_PEMUSNAHAN WHERE id = '$idplh' ";$cek=$qry;
					$aqry = mysql_query($qry);
		 ...
		 case 'Catatan':{
			...
			$getData = mysql_fetch_array(mysql_query("SELECT * FROM tabel_PEMUSNAHAN WHERE id = '$idAwal'"));
			foreach ($getData as $key => $value) {
				  $$key = $value;
			}
			$getMaxID = mysql_fetch_array(mysql_query("select max(id) as maxID from tabel_PEMUSNAHAN where tahun = '$tahun'  and c1 ='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and p='$p' and q='$q' and jenis_anggaran = '$jenis_anggaran'  "));
			$maxID = $getMaxID['maxID'];
			$aqry = "select * from tabel_PEMUSNAHAN where id ='$maxID' ";
   ...
   function setPage_OtherScript(){
		...
		return
			...
			<script type='text/javascript' src='js/persediaan/pemusnahan/pemusnahanPersediaan.js' language='JavaScript' ></script>
	...
	// }
  	...
  	function setFormEdit(){
		...
		$this->form_fmST = 1;
		if($err == ''){
			$aqry = "SELECT * FROM  tabel_PEMUSNAHAN WHERE id='".$this->form_idplh."' "; $cek.=$aqry;
			$dt = mysql_fetch_array(mysql_query($aqry));
			$fm = $this->setForm($dt);
		}
	...
	function Info(){
	 ...
	 $this->form_caption = 'INFO PEMUSNAHAN';
	...
	function setKolomHeader($Mode=1, $Checkbox=''){
		...
		$headerTable =
		  "<thead>
		   ...
		   <th class='th01' width='100'>TOTAL</th>
		   </tr>
	...
	function setKolomData($no, $isi, $Mode, $TampilCheckBox){
	 ...
	 foreach ($_REQUEST as $key => $value) {
				...
				$getIdLockBarang = mysql_fetch_array(mysql_query("select * from t_persediaan_lock_barang where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and f='$f' and g='$g' and h='$h' and i = '$i' and j ='$j' and j1 ='$j1'"));
				if($semester == '1'){
						$maxFilter = $_COOKIE['coThnAnggaran']."06";
						$kondisiSemester = " and replace(left(tanggal_buku,6),'-','') <= '$maxFilter'";
				}elseif($semester == '2'){
						$minFilter = $_COOKIE['coThnAnggaran']."07";
						$maxFilter = $_COOKIE['coThnAnggaran']."12";
						$kondisiSemester = " and replace(left(tanggal_buku,6),'-','') >= '$minFilter' and replace(left(tanggal_buku,6),'-','') <= '$maxFilter'";
				}elseif($semester == '3'){
						$maxFilter = $_COOKIE['coThnAnggaran']."12";
						$kondisiSemester = " and replace(left(tanggal_buku,6),'-','') <= '$maxFilter'";
				}
				$getIdKartu = mysql_query("select * from t_kartu_persediaan where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and f='$f' and g='$g' and h='$h' and i = '$i' and j ='$j' and j1 ='$j1' $kondisiSemester");
				while ($dataIDKartu = mysql_fetch_array($getIdKartu)) {
					$arrKondisiKartu[] = " id_t_kartu_persediaan ='".$dataIDKartu['id']."'";
				}
				$kondiriKartuPersediaan = join(" or ",$arrKondisiKartu);
				$getDataLockBarangKurang = mysql_fetch_array(mysql_query("select * from rincian_lock_barang where id_lock = '".$getIdLockBarang['id']."' and uraian='CEK FISIK KURANG' and ($kondiriKartuPersediaan)"));

				$Koloms[] = array(' align="left"',$getNamaBarang['nm_barang']);
				$Koloms[] = array(' align="left"',$getNamaBarang['satuan']);
				$Koloms[] = array(' align="right"',number_format($berkurang,0,',','.'));
				$Koloms[] = array(' align="right"',number_format($getDataLockBarangKurang['total'],0,',','.'));
	...
	function Validasi($dt){
	 ...
	 $this->form_caption = 'VALIDASI PEMUSNAHAN 2.1';
...
$TampilOpt =
	...
	function getDaftarOpsi($Mode=1){
		...
		$cmbKelompok = "0";
		$cmbJenis = "08";
		if($pemusnahanPersediaanSKPD2fmURUSAN !='00'){
				$arrKondisi[] = "c1 = '$pemusnahanPersediaanSKPD2fmURUSAN'";
		}
		if($pemusnahanPersediaanSKPD2fmSKPD !='00'){
				$arrKondisi[] = "c = '$pemusnahanPersediaanSKPD2fmSKPD'";
		}
		if($pemusnahanPersediaanSKPD2fmUNIT !='00'){
				$arrKondisi[] = "d = '$pemusnahanPersediaanSKPD2fmUNIT'";
		}
		if($pemusnahanPersediaanSKPD2fmSUBUNIT !='00'){
				$arrKondisi[] = "e = '$pemusnahanPersediaanSKPD2fmSUBUNIT'";
		}
		if($pemusnahanPersediaanSKPD2fmSEKSI !='00'){
				$arrKondisi[] = "e1 = '$pemusnahanPersediaanSKPD2fmSEKSI'";
		}
		...
		$arrKondisi[] = "left(tanggal_cek,4) = '".$_COOKIE['coThnAnggaran']."'";
		...
		if($filterPeriode == '1'){
				$maxFilter = $_COOKIE['coThnAnggaran']."06";
				$arrKondisi[] = "replace(left(tanggal_cek,6),'-','') <= '$maxFilter'";
		}elseif($filterPeriode == '2'){
				$minFilter = $_COOKIE['coThnAnggaran']."07";
				$maxFilter = $_COOKIE['coThnAnggaran']."12";
				$arrKondisi[] = "replace(left(tanggal_cek,6),'-','') >= '$minFilter' and replace(left(tanggal_cek,6),'-','') <= '$maxFilter'";
		}elseif($filterPeriode == '3'){
				$maxFilter = $_COOKIE['coThnAnggaran']."12";
				$arrKondisi[] = "replace(left(tanggal_cek,6),'-','') <= '$maxFilter'";
		}
		$arrKondisi[] = "berkurang != '0'";
		$arrKondisi[] = "berkurang != ''";
...
function Laporan2($xls =FALSE){
    ...
    $arrKondisi[] = "f != '06' and f!='07' and f!='08'";
		...
		echo "
								...
								<thead>
									<tr>
										...
										<th class='th01' style='width: 4%;'>TOTAL</th>
								</thead>
		...
		while($dataKartuPersediaan = mysql_fetch_array($queryKartuPersediaan)){
			...
			$getIdLockBarang = mysql_fetch_array(mysql_query("SELECT * from t_persediaan_lock_barang where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and f = ".$dataKartuPersediaan[f]." and g = ".$dataKartuPersediaan[g]." and h = ".$dataKartuPersediaan[h]." and i = ".$dataKartuPersediaan[i]." and j = ".$dataKartuPersediaan[j]." and j1 = ".$dataKartuPersediaan[j1]." "));
				...
				$getIdKartu = mysql_query("SELECT * from t_kartu_persediaan where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and f = ".$dataKartuPersediaan[f]." and g = ".$dataKartuPersediaan[g]." and h = ".$dataKartuPersediaan[h]." and i = ".$dataKartuPersediaan[i]." and j = ".$dataKartuPersediaan[j]." and j1 = ".$dataKartuPersediaan[j1]." $kondisiSemester");
...
function Laporan($dt){
	 ...
	 $c1 = $dt['pemusnahanPersediaanSKPD2fmURUSAN'];
	 $c = $dt['pemusnahanPersediaanSKPD2fmSKPD'];
	 $d = $dt['pemusnahanPersediaanSKPD2fmUNIT'];
	 $e = $dt['pemusnahanPersediaanSKPD2fmSUBUNIT'];
	 $e1 = $dt['pemusnahanPersediaanSKPD2fmSEKSI'];

	 	$arrayJenisLaporan = array(
							   array('Laporan2', 'DAFTAR PEMUSNAHAN BARANG'),
      ...
      <input type='button' value='TTD' onclick ='".$this->Prefix.".Baru()' title='TTD' >
			...
			"<input type='button' value='Batal' onclick ='pemusnahanPersediaan.Close()' >";
...
function Laporann($xls =FALSE){
		...
		if($this->jenisForm == 'PENYUSUNAN'){
			$getAllParent = mysql_query("select * from tabel_PEMUSNAHAN where id_tahap ='$this->idTahap'  and o1 !='0' and k='0' and n='0' and rincian_perhitungan=''  ");
			while($rows = mysql_fetch_array($getAllParent)){
				foreach ($rows as $key => $value) {
			 	 $$key = $value;
				}
				$cekPekerjaan = mysql_num_rows(mysql_query("select * from tabel_PEMUSNAHAN where o1 = '$o1' and id_tahap ='$this->idTahap' $kondisiRekening $kondisiSKPD and (rincian_perhitungan !='' or rincian_perhitungan!='') "));
				if($cekPekerjaan == 0){
					$arrKondisi[] = "o1 !='$o1'";
				}else{
					$arrKondisi[] = " id_tahap='$this->idTahap'  or id = '$id' ";
					$getAllRekening = mysql_query("select * from tabel_PEMUSNAHAN where id_tahap = '$this->idTahap'  and c1 = '0' and j ='000' and rincian_perhitungan = '' and k!='0' and n !='0'  ");
					while($row2s = mysql_fetch_array($getAllRekening)){
						foreach ($row2s as $key => $value) {
					 	 $$key = $value;
						}
						$cekRekening = mysql_num_rows(mysql_query("select * from tabel_PEMUSNAHAN where k ='$k' and l='$l' and m='$m' and n='$n' and o='$o' and id_tahap ='$this->idTahap' $kondisiRekening $kondisiSKPD and (rincian_perhitungan !='' or rincian_perhitungan!='') "));
						if($cekRekening == 0){
							$concat = $k.".".$l.".".$m.".".$n.".".$o;
				...
				$grabNonMapingRekening= mysql_query("select * from tabel_PEMUSNAHAN where id_tahap ='$this->idTahap' and (LENGTH(k) > 1) and rincian_perhitungan=''");
				while($got = mysql_fetch_array($grabNonMapingRekening)){
					if(mysql_num_rows(mysql_query("select * from tabel_PEMUSNAHAN where id_tahap ='$this->idTahap' and k ='".$got['k']."' and l ='".$got['l']."' and m ='".$got['m']."' and n ='".$got['n']."' and o ='".$got['o']."' and rincian_perhitungan!=''  $kondisiSKPD ")) != 0){
						$arrKondisi[] = "id_tahap = '$this->idTahap' or id ='".$got['id']."'";
					}

				}
				$grabNonHostedRekening= mysql_query("select * from tabel_PEMUSNAHAN where id_tahap ='$this->idTahap' and (LENGTH(k) = 1 and k !='0') and rincian_perhitungan=''");
			...
			$getRApbd = mysql_fetch_array(mysql_query("select * from tabel_PEMUSNAHAN where no_urut = '$nomorUrutSebelumnya' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' "));
			if($getRApbd['jenis_form_modul'] != "PENYUSUNAN" && $getRApbd['jenis_form_modul'] != "KOREKSI"){
				$nomorUrutSebelumnya = $nomorUrutSebelumnya - 1;
			}
			$getLastTahap = mysql_fetch_array(mysql_query("select * from tabel_PEMUSNAHAN where no_urut = '$nomorUrutSebelumnya' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'"));
			$blackList = "";
			if($getLastTahap['jenis_form_modul'] == "PENYUSUNAN" && $this->wajibValidasi == TRUE){
				$getAllChild = mysql_query("select * from tabel_PEMUSNAHAN where id_tahap = '".$getLastTahap['id_tahap']."' and (rincian_perhitungan !='' or j !='000'  and status_validasi !='1' )");
			...
			$getAllParent = mysql_query("select * from tabel_PEMUSNAHAN where id_tahap = '$idTahap'  and o1 !='0' and k='0' and n='0' and rincian_perhitungan=''  ");
			while($rows = mysql_fetch_array($getAllParent)){
				foreach ($rows as $key => $value) {
			 	 $$key = $value;
				}
				$cekPekerjaan = mysql_num_rows(mysql_query("select * from tabel_PEMUSNAHAN where o1 = '$o1' and id_tahap = '$idTahap' $kondisiRekening $kondisiSKPD $blackList and (rincian_perhitungan !='' or rincian_perhitungan!='') "));
				if($cekPekerjaan == 0){
					$arrKondisi[] = "o1 !='$o1'";
				}else{
					$arrKondisi[] = " id_tahap = '$idTahap'  or id = '$id' ";
					$getAllRekening = mysql_query("select * from tabel_PEMUSNAHAN where id_tahap = '$idTahap' and c1 = '0' and j ='000' and rincian_perhitungan = '' and k!='0' and n !='0'  ");
					while($row2s = mysql_fetch_array($getAllRekening)){
						foreach ($row2s as $key => $value) {
					 	 $$key = $value;
						}
						$cekRekening = mysql_num_rows(mysql_query("select * from tabel_PEMUSNAHAN where k ='$k' and l='$l' and m='$m' and n='$n' and o='$o' and id_tahap = '$idTahap' $kondisiRekening $kondisiSKPD $blackList and (rincian_perhitungan !='' or rincian_perhitungan!='') "));
						if($cekRekening == 0){
							$concat = $k.".".$l.".".$m.".".$n.".".$o;
							$arrKondisi[] = "concat(k,'.',l,'.',m,'.',n,'.',o)  !='$concat'";
						}else{
							$arrKondisi[] = " id_tahap = '$idTahap'  or id = '$id'  ";
							$getAllProgram = mysql_query("select * from tabel_PEMUSNAHAN where id_tahap = '$idTahap' and p != '0' and q ='0' and j ='000' and rincian_perhitungan = ''   ");
							while($row3s = mysql_fetch_array($getAllProgram)){
								foreach ($row3s as $key => $value) {
							 	 $$key = $value;
								}
								$cekProgram = mysql_num_rows(mysql_query("select * from tabel_PEMUSNAHAN where bk ='$bk' and ck='$ck' and p='$p' and id_tahap = '$idTahap' $kondisiRekening $kondisiSKPD $blackList and (rincian_perhitungan !='' or rincian_perhitungan!='') "));
								if($cekProgram == 0){
									$concat = $bk.".".$ck.".".$p;
									$arrKondisi[] = "concat(bk,'.',ck,'.',p)  !='$concat'";
								}else{
									$arrKondisi[] = " id_tahap = '$idTahap'  or id = '$id'   ";
									$getAllKegiatan = mysql_query("select * from tabel_PEMUSNAHAN where id_tahap = '$idTahap' and bk ='$bk' and ck='$ck' and p='$p' and q != '0'  and j ='000' and rincian_perhitungan = ''   ");
									while($row4s = mysql_fetch_array($getAllKegiatan)){
										foreach ($row4s as $key => $value) {
									 	 $$key = $value;
										}
										$cekKegiatan = mysql_num_rows(mysql_query("select * from tabel_PEMUSNAHAN where bk ='$bk' and ck='$ck' and p='$p' and q='$q' and id_tahap = '$idTahap' $kondisiRekening $kondisiSKPD  $blackList and (rincian_perhitungan !='' or rincian_perhitungan!='') "));
										if($cekKegiatan == 0){
											$concat = $bk.".".$ck.".".$p;
											$arrKondisi[] = "concat(bk,'.',ck,'.',p)  !='$concat'";
										}else{
				...
				$grabNonMapingRekening= mysql_query("select * from tabel_PEMUSNAHAN where id_tahap = '$idTahap' and (LENGTH(k) > 1) and rincian_perhitungan=''");
				while($got = mysql_fetch_array($grabNonMapingRekening)){
					if(mysql_num_rows(mysql_query("select * from tabel_PEMUSNAHAN where id_tahap = '$idTahap' and k ='".$got['k']."' and l ='".$got['l']."' and m ='".$got['m']."' and n ='".$got['n']."' and o ='".$got['o']."' and rincian_perhitungan!='' $kondisiRekening $kondisiSKPD $blackList")) != 0){
						$arrKondisi[] = "id_tahap = '$idTahap' or id ='".$got['id']."'";
					}

				}

				$grabNonHostedRekening= mysql_query("select * from tabel_PEMUSNAHAN where id_tahap = '$idTahap' and (LENGTH(k) = 1 and k !='0') and rincian_perhitungan=''");
				while($got = mysql_fetch_array($grabNonHostedRekening)){
					if(mysql_num_rows(mysql_query("select * from tabel_PEMUSNAHAN where id_tahap = '$idTahap' and k ='".$got['k']."' and l ='".$got['l']."' and m ='".$got['m']."' and n ='".$got['n']."' and o ='".$got['o']."' and rincian_perhitungan!='' $kondisiRekening $kondisiSKPD $blackList")) != 0){
						$arrKondisi[] = "id_tahap = '$idTahap' or id ='".$got['id']."'";
					}
			...
			if($this->jenisFormTerakhir == "KOREKSI"){
				...
				while($rows = mysql_fetch_array($getAllParent)){
					...
					$cekPekerjaan = mysql_num_rows(mysql_query("select * from tabel_PEMUSNAHAN where o1 = '$o1' and id_tahap ='$idTahap' $kondisiRekening $kondisiSKPD and (rincian_perhitungan !='' or rincian_perhitungan!='') "));
					if($cekPekerjaan == 0){
						$arrKondisi[] = "o1 !='$o1'";
					}else{
						$arrKondisi[] = " id_tahap='$idTahap'  or id = '$id' ";
						...
						while($row2s = mysql_fetch_array($getAllRekening)){
							...
							$cekRekening = mysql_num_rows(mysql_query("select * from tabel_PEMUSNAHAN where k ='$k' and l='$l' and m='$m' and n='$n' and o='$o' and id_tahap ='$idTahap' $kondisiRekening $kondisiSKPD and (rincian_perhitungan !='' or rincian_perhitungan!='') "));
							if($cekRekening == 0){
								$concat = $k.".".$l.".".$m.".".$n.".".$o;
								$arrKondisi[] = "concat(k,'.',l,'.',m,'.',n,'.',o)  !='$concat'";
							}else{
								...
								while($row3s = mysql_fetch_array($getAllProgram)){
									...
									$cekProgram = mysql_num_rows(mysql_query("select * from tabel_PEMUSNAHAN where bk ='$bk' and ck='$ck' and p='$p' and id_tahap ='$idTahap' $kondisiRekening $kondisiSKPD and (rincian_perhitungan !='' or rincian_perhitungan!='') "));
									if($cekProgram == 0){
										$concat = $bk.".".$ck.".".$p;
										$arrKondisi[] = "concat(bk,'.',ck,'.',p)  !='$concat'";
									}else{
										...
										while($row4s = mysql_fetch_array($getAllKegiatan)){
											...
											$cekKegiatan = mysql_num_rows(mysql_query("select * from tabel_PEMUSNAHAN where bk ='$bk' and ck='$ck' and p='$p' and q='$q' and id_tahap ='$idTahap' $kondisiRekening $kondisiSKPD and (rincian_perhitungan !='' or rincian_perhitungan!='') "));
											if($cekKegiatan == 0){
												$concat = $bk.".".$ck.".".$p;
												$arrKondisi[] = "concat(bk,'.',ck,'.',p)  !='$concat'";
											}else{
				...
				$grabNonMapingRekening= mysql_query("select * from tabel_PEMUSNAHAN where id_tahap ='$idTahap' and (LENGTH(k) > 1) and rincian_perhitungan=''");
				while($got = mysql_fetch_array($grabNonMapingRekening)){
					if(mysql_num_rows(mysql_query("select * from tabel_PEMUSNAHAN where id_tahap ='$idTahap' and k ='".$got['k']."' and l ='".$got['l']."' and m ='".$got['m']."' and n ='".$got['n']."' and o ='".$got['o']."' and rincian_perhitungan!='' $kondisiRekening $kondisiSKPD ")) != 0){
						$arrKondisi[] = "id_tahap = '$idTahap' or id ='".$got['id']."'";
					}
		...
		$qry ="select * from tabel_PEMUSNAHAN where $Kondisi ";
		$aqry = mysql_query($qry);
		...
		";
		$getTotal = mysql_fetch_array(mysql_query("select sum(jumlah_harga) from tabel_PEMUSNAHAN where $Kondisi  "));
		...
		while($daqry = mysql_fetch_array($aqry)){
			...
			if($k == '0' && $n =='0' ){
				...
				$getSumJumlahHarga = mysql_fetch_array(mysql_query("select sum(jumlah_harga) from tabel_PEMUSNAHAN where  o1 ='$o1' $kondisiSKPD $kondisiFilter  "));
				...
				$getSumJumlahHarga = mysql_fetch_array(mysql_query("select sum(jumlah_harga) from tabel_PEMUSNAHAN where  k = '$k' and l='$l' and m='$m' and n='$n' and o='$o'  $kondisiSKPD $kondisiFilter"));
...
function formAlokasi($dt){
	...
	function formRincianVolume($dt){
	 ...
	 $jumlahHargaForm = $dt;
	 $username =$_COOKIE['coID'];
	//  $getRincianVolume = mysql_fetch_array(mysql_query("select * from tabel_PEMUSNAHAN where id ='$dt'"));
...
// <input type='hidden' id='volumeRek' value='$volume_rek'>
	...
	function pemusnahanPersediaan($idPersediaan){
	 global $SensusTmp;
	 $cek = ''; $err=''; $content='';
	 $json = TRUE;	//$ErrMsg = 'tes';
	 $form_name = $this->Prefix.'_form';
	 ...
	 //items ----------------------
	  ...
	  $this->form_fields = array(
	  	...
	  	'sdsds' => array(
					'label'=>'HASIl CEK',
					'labelWidth'=>100,
					'value'=>	cmbArray('jenispemusnahanPersediaan',$jenispemusnahanPersediaan,$arrayJenispemusnahanPersediaan,'-- PERHITUNGAN --',"onchange=$this->Prefix.pemusnahanPersediaanChanged();"),
				),
	  	'asd' => array(
					'label'=>'JUMLAH CEK',
					'labelWidth'=>100,
	  	...
	  	'keterangan' => array(
					'label'=>'',
					'labelWidth'=>100,
					'value'=>	"<textarea id='keterangan' name='keterangan' style = 'width:100%;' row ='3'>".$getDatapemusnahanPersediaan['keterangan']."</textarea>",
					'type'=>	"merge",
				),
		...
		//tombol
		$this->form_menubawah =
			"<input type='button' value='Simpan' onclick ='".$this->Prefix.".savepemusnahanPersediaan($idPersediaan)' > &nbsp".
...
$pemusnahanPersediaan = new pemusnahanPersediaanObj();
$pemusnahanPersediaan->username = $_COOKIE['coID'];
$getTanggalCek = mysql_fetch_array(mysql_query("select * from pengaturan_persediaan where tahun = '".$_COOKIE['coThnAnggaran']."'"));
$pemusnahanPersediaan->tanggalCek = $getTanggalCek['tanggal_cek'];
