...
class popupProgramPersediaanObj  extends DaftarObj2{
	var $Prefix = 'popupProgramPersediaan';
	...
	var $FormName = 'popupProgramPersediaanForm';
	...
	case 'formBaru':{
		...
		case 'windowshow':{
	   	...
	   	case 'getdata':{
			...
			$kodeSKPD = $explodeKodeSKPD[0].".".$explodeKodeSKPD[1].".".$explodeKodeSKPD[2];
			$getListKegiatan = mysql_query("select * from view_dpa_2_2_1 where tahun ='2018' and jenis_anggaran = 'MURNI' and rincian_perhitungan !='' and concat(c1,'.',c,'.',d) ='$kodeSKPD' and bk='$bk' and ck='$ck' and dk='$dk' and p ='$p'");
			while ($dataGet = mysql_fetch_array($getListKegiatan)) {
				$concatKegiatan = $dataGet['bk'].".".$dataGet['ck'].".".$dataGet['dk'].".".$dataGet['p'].".".$dataGet['q'];
				if(in_array($concatKegiatan,$arrayKegiatan)){
				}else{
					$arrayKegiatan[] = " concat(bk,'.',ck,'.',dk,'.',p,'.',q) != '$concatKegiatan'";
				}
			}
			$kondisiKegiatan= join(' and ',$arrayKegiatan);
			if(sizeof($arrayKegiatan) !=0){
					$kondisiKegiatan = " and $kondisiKegiatan";
			}
			$getExceptKegiatan  = mysql_query("select * from ref_program where p!='0' and q!='0' $kondisiKegiatan");
			while($exceptKegiatan = mysql_fetch_array($getExceptKegiatan)){
					$concatExceptKegiatan = $exceptKegiatan['bk'].".".$exceptKegiatan['ck'].".".$exceptKegiatan['dk'].".".$exceptKegiatan['p'].".".$exceptKegiatan['q'];
					$arrayExceptKegiatan[] = "concat(bk,'.',ck,'.',dk,'.',p,'.',q) != '$concatExceptKegiatan'";
			}
			$kondisiExceptKegiatan= join(' and ',$arrayExceptKegiatan);
			if(sizeof($arrayExceptKegiatan) !=0){
					$kondisiExceptKegiatan = " and $kondisiExceptKegiatan";
			}
			$codeAndNameKegiatan  = "select q, nama from  ref_program where bk = '$bk' and ck = '$ck' and dk='$dk' and p='$p' and q != '0' $kondisiExceptKegiatan";
			$cmbKegiatan = cmbQuery('cmbKegiatan', $get['q'], $codeAndNameKegiatan,'','-- Pilih Kegiatan --');
			$get = mysql_fetch_array(mysql_query("select * from ref_program where concat(bk,'.',ck,'.',dk,'.',p,'.',q) = '$id' "));
			$content = array("p" => $get['p'] ,  "nama" => $get['nama'], 'cmbKegiatan' => $cmbKegiatan, "bk" => $bk, "ck" => $ck , "dk" => $dk);
		...
		$form_name = $this->FormName;
			...
			$form = centerPage(
					...
					createDialog(
						...
						'',
						"
						<input type='hidden' name='filterSKPD' id='filterSKPD' value='".$_REQUEST['filterSKPD']."' >
	...
	function setKolomData($no, $isi, $Mode, $TampilCheckBox){
	 ...
	 $kode_jurnal=$isi['bk'].'.'.genNumber($isi['ck']).'.'.genNumber($isi['dk']).".".genNumber($isi['p']).'.'.genNumber($isi['q']);
	 $Koloms = array();
	 $Koloms[] = array('align="center" width=""', $no.'.' );
	 $Koloms[] = array('align="center" width="" ',$kode_jurnal);
 	 $kodeProgram =$isi['bk'].'.'.$isi['ck'].'.'.$isi['dk'].".".$isi['p'].'.'.$isi['q'];
	 $Koloms[] = array('align="left" width=""',"<a style='cursor:pointer;' onclick=popupProgramPersediaan.windowSave('$kodeProgram')>".$isi['nama']."</a>");
	 return $Koloms;
	}
	...
	function getDaftarOpsi($Mode=1){
		...
		//kondisi -----------------------------------
		$explodeKodeSKPD = explode('.',$_REQUEST['filterSKPD']);
		$kodeSKPD = $explodeKodeSKPD[0].".".$explodeKodeSKPD[1].".".$explodeKodeSKPD[2];
		$urusan = $_REQUEST['fmURUSAN'];
		$bidang = $_REQUEST['fmBIDANG'];
		$dinas = $_REQUEST['fmDINAS'];
		$p = $_REQUEST['p'];
		$arrKondisi = array();
		if(!empty($urusan)){
		...
		$getListProgramKegiatan = mysql_query("select * from view_dpa_2_2_1 where tahun ='2018' and jenis_anggaran = 'MURNI' and rincian_perhitungan !='' and concat(c1,'.',c,'.',d) ='$kodeSKPD'");
		while ($dataGet = mysql_fetch_array($getListProgramKegiatan)) {
			$concatProgram = $dataGet['bk'].".".$dataGet['ck'].".".$dataGet['dk'].".".$dataGet['p'];
			if(in_array($concatProgram,$arrayProgram)){
			}else{
				$arrayProgram[] = " concat(bk,'.',ck,'.',dk,'.',p) != '$concatProgram'";
			}
		}
		$kondisiProgram= join(' and ',$arrayProgram);
		if(sizeof($arrayProgram) !=0){
				$kondisiProgram = " and $kondisiProgram";
		}
		$getExceptProgram  = mysql_query("select * from ref_program where p!='0' and q='0' $kondisiProgram");
		while($exceptProgram = mysql_fetch_array($getExceptProgram)){
				$concatExceptProgram = $exceptProgram['bk'].".".$exceptProgram['ck'].".".$exceptProgram['dk'].".".$exceptProgram['p'];
				$arrKondisi[] = "concat(bk,'.',ck,'.',dk,'.',p) != '$concatExceptProgram'";
		}
...
$popupProgramPersediaan = new popupProgramPersediaanObj();
