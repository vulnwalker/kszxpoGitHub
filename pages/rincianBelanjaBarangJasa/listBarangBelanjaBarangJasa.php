	  ...
	  switch($tipe){
		...
		case 'checkboxChanged':{
			foreach ($_REQUEST as $key => $value) {
			 ...
			 if($jenis !="checked"){
			 	mysql_query("update temp_rincian_belanja_barang_jasa set status='' where concat(f1,'.',f2,'.',f,'.',g,'.',h,'.',i,'.',j) ='$id' and username ='$this->username'");
			 }else{
...
function setKolomHeader($Mode=1, $Checkbox=''){
	...
	function setKolomData($no, $isi, $Mode, $TampilCheckBox){
	 ...
	 $getStatusCheckBox = mysql_fetch_array(mysql_query("select * from temp_rincian_belanja_barang_jasa where concat(f1,'.',f2,'.',f,'.',g,'.',h,'.',i,'.',j) = '$kodeBarangLengkap' and username = '$this->username'"));
	 $status = $getStatusCheckBox['status'];

	 $TampilCheckBox =  $this->setCekBox($kodeBarangLengkap, $kodeBarangLengkap, $status);
	...
	$TampilOpt =
			"<form id='listBarangBelanjaBarangJasaForm'><div class='FilterBar'>".
			...
			<td>".
			cmbQuery1("cmbJenis",$cmbJenis,"select f as valueCmbJenis, nm_barang from ref_barang where f1 = '$cmbAkun' and f2 = '$cmbKelompok' and f != '00'  and g ='00' and h ='00' and i='00' and j = '000' and f ='08'","onChange=listBarangBelanjaBarangJasa.refreshList(true) ",'Pilih','').
			"</td>
			</tr><tr>
			<td style='width:170px;'>OBYEK</td><td>:</td>
			<td>".
			cmbQuery1("cmbObyek",$cmbObyek,"select g as valueCmbObyek, nm_barang from ref_barang where f1 = '$cmbAkun' and f2 = '$cmbKelompok' and f = '$cmbJenis'  and g !='00' and h ='00' and i='00' and j = '000'","onChange=\"listBarangBelanjaBarangJasa.refreshList(true)\"",'Pilih','').
			"</td>
			</tr><tr>
			<td style='width:170px;'>RINCIAN OBYEK</td><td>:</td>
			<td>".
			cmbQuery1("cmbRincianObyek",$cmbRincianObyek,"select h as valueCmbRincianObyek, nm_barang from ref_barang where f1 = '$cmbAkun' and f2 = '$cmbKelompok' and f = '$cmbJenis'  and g ='$cmbObyek' and h !='00' and i='00' and j = '000'","onChange=\"listBarangBelanjaBarangJasa.refreshList(true)\"",'Pilih','').
			"</td>
				</tr>
			<tr>
			<td style='width:170px;'>SUB RINCIAN OBYEK</td><td>:</td>
			<td>".
			cmbQuery1("cmbSubRincianObyek",$cmbSubRincianObyek,"select i as valueCmbSubRincianObyek, nm_barang from ref_barang where f1 = '$cmbAkun' and f2 = '$cmbKelompok' and f = '$cmbJenis'  and g ='$cmbObyek' and h ='$cmbRincianObyek' and i != '00' and j = '000'","onChange=\"listBarangBelanjaBarangJasa.refreshList(true)\"",'Pilih','').
			"</td>
				</tr>
			<tr>
			<td style='width:170px;'>SUB-SUB RINCIAN OBYEK</td><td>:</td>
			<td>".
			cmbQuery1("cmbSubSubRincianObyek",$cmbSubSubRincianObyek,"select j as valueCmbSubSubRincianObyek, nm_barang from ref_barang where f1 = '$cmbAkun' and f2 = '$cmbKelompok' and f = '$cmbJenis'  and g ='$cmbObyek' and h ='$cmbRincianObyek' and i='$cmbSubRincianObyek' and j != '000'","onChange=\"listBarangBelanjaBarangJasa.refreshList(true)\"",'Pilih','').
			"</td>
				</tr>
			...
			<tr><td>
				...
				<input type='button' id='btTampil' value='Tampilkan' onclick='listBarangBelanjaBarangJasa.refreshList(true)'>
			</td></tr>
			</table>".
			"</div>".
