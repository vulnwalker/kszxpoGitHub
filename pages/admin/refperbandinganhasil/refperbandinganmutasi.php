...
class refperbandinganmutasiObj  extends DaftarObj2{	
	var $Prefix = 'refperbandinganmutasi';
	var $elCurrPage="HalDefault";
	var $SHOW_CEK = TRUE;
	var $TblName = 'v1_refperbandinganmutasi'; //bonus
	var $TblName_Hapus = 'refperbandinganmutasi';
	var $MaxFlush = 10;
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='refperbandinganmutasi.xls';
	var $namaModulCetak='REFERENSI DATA';
	var $Cetak_Judul = 'refperbandinganmutasi';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'refperbandinganmutasiForm';
	var $noModul=14; 
	var $TampilFilterColapse = 0; //0
	
	function setTitle(){
		return 'Perbandingan Mutasi';
	}
	...
	//get data -----------------
	 ...
	 $c_d=mysql_fetch_array(mysql_query("select * from refperbandinganmutasi where c='$c' AND d='$d'"));	 
			...
			if($fmST == 0){
				if($err==''){
					if(empty($c_d['c']) && empty($c_d['d'])){
						$aqry = "INSERT into refperbandinganmutasi (c,d,bk,ck,dk) values('$c','$d','$bk','$ck','$dk')";	$cek .= $aqry;						
					}else{
						$aqry = "UPDATE refperbandinganmutasi
							set "." bk = '$bk',
							ck = '$ck',
							dk = '$dk'".
	...
	function setPage_OtherScript(){
		...
		return 	
			"<script type='text/javascript' src='js/admin/refperbandinganmutasi/".strtolower($this->Prefix).".js' language='JavaScript' ></script>".
			$scriptload;
	}
	...
	function setFormBaru(){
  	...
  	function setFormEdit(){
		...
		$this->form_fmST = 1;						
		//get data 
		$aqry = "SELECT * FROM  v1_refperbandinganmutasi WHERE c='".$kode[0]."' AND d='".$kode[1]."'"; $cek.=$aqry;
...
$refperbandinganmutasi = new refperbandinganmutasiObj();
?>