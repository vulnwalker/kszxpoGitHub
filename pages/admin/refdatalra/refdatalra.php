...
class refdatalraObj  extends DaftarObj2{	
	var $Prefix = 'refdatalra';
	var $elCurrPage="HalDefault";
	var $SHOW_CEK = TRUE;
	var $TblName = 'v1_refdatalra'; //bonus
	var $TblName_Hapus = 'refdatalra';
	var $MaxFlush = 10;
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='refdatalra.xls';
	var $namaModulCetak='REFERENSI DATA';
	var $Cetak_Judul = 'refdatalra';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'refdatalraForm';
	var $noModul=14; 
	...
	//get data -----------------
	 ...
	 $c_d=mysql_fetch_array(mysql_query("select * from refdatalra where c='$c' AND d='$d'"));	 
			...
			if($fmST == 0){
				if($err==''){
					if(empty($c_d['c']) && empty($c_d['d'])){
						$aqry = "INSERT into refdatalra (c,d,bk,ck,dk) values('$c','$d','$bk','$ck','$dk')";	$cek .= $aqry;						
					}else{
						$aqry = "UPDATE refdatalra
							set "." bk = '$bk',
							ck = '$ck',
							dk = '$dk'".
	...
	function setPage_OtherScript(){
		...
		return 	
			"<script type='text/javascript' src='js/admin/refdatalra/".strtolower($this->Prefix).".js' language='JavaScript' ></script>".
			$scriptload;
	}
	...
	function setFormBaru(){
  	...
  	function setFormEdit(){
		...
		$this->form_fmST = 1;						
		//get data 
		$aqry = "SELECT * FROM  refdatalra WHERE c='".$kode[0]."' AND d='".$kode[1]."'"; $cek.=$aqry;
