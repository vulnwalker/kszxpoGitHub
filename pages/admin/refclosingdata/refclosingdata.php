...
class refclosingdataObj  extends DaftarObj2{	
	...
	var $Cetak_Judul = 'CLOSING DATA PEROLEHAN';	
	var $Cetak_Mode=2;
	...
	function Hapus_Validasi($id){
		...
		$old = mysql_fetch_array(mysql_query(
			" select year(tgl) as thnPerolehan, year(tgl_susut) as thnSusut, t_closing.* from t_closing where c1='".$arrid[0]."' and c='".$arrid[1]."' and d='".$arrid[2]."' and e='".$arrid[3]."' and e1='".$arrid[4]."'  "
		));	
		if($errmsg =='' && $old['thnSusut'] >= $thnAnggaran ){
			$errmsg = "  ";
		}
		/*if($errmsg=='' && 
	...
	//get data -----------------
	 ...
	 $data= $_REQUEST['data'];
	  ...
	  $tahun= $HTTP_COOKIE_VARS['coThnAnggaran'];
	  ...
	  $dataQuery =",'$tahunAnggaranRequest-12-31','$keterangan','$tanggalUpdate','',''";
	 ...
	 $arrKondSKPD = array();
	 if($c1!='0') $arrKondSKPD[] = " c1='$c1' ";
	 if($c!='00') $arrKondSKPD[] = " c='$c' ";
	 if($d!='00') $arrKondSKPD[] = " d='$d' ";
	 if($e!='00') $arrKondSKPD[] = " e='$e' ";
	 if($e1!='000') $arrKondSKPD[] = " e1='$e1' ";
	 $kondSKPD = join(' and ',$arrKondSKPD);
	 
	 
	 $c_d=mysql_fetch_array(mysql_query("select * from t_closing where c1='$c1' AND c='$c' AND d='$d' AND e='$e' AND e1='$e1' "));	 
	 $old=mysql_fetch_array(mysql_query("select * from t_closing where c1='$c1' AND c='$c' AND d='$d' AND e='$e' AND e1='$e1' "));
 	 
	  
	//$arrKond = array();	if($c1 <> )

    //cek tahun tidak boleh > thn anggaran
	if ($explodeTahun[0] > $tahun) {$err = 'Tahun tidak boleh melebihi Tahun Anggaran'; }
	//cek tahun tidak boleh kurang dari tahun periode sebelumnya
	if($err=='' && $explodeTahun[0] < $tahun-1 ) $err = 'Tahun tidak boleh lebih kecil dari '.($tahun-1);
	/**
	//jika tahun  < thn yg disimpan sebelumnya , cek tidak boleh ada penyusutan di dalam skpd nya
	$arrTglOld = explode('-', $old['tgl']);
	if($err==''){// && $explodeTahun[0]<$arrTglOld[0] ){
		$kondSKPD1= $kondSKPD==''? '' : ' and '.$kondSKPD; 
		$aqry = "select count(*) as cnt from t_closing ".
			" where tgl_susut <>'0000-00-00' and year(tgl_susut)>'".$explodeTahun[0]."' $kondSKPD1 "; $cek .= ' cek susut '.$aqry;
		$susut = mysql_fetch_array(mysql_query(
			$aqry
		));
		if($susut['cnt']>0 ) $err = ' Ada SKPD sudah Closing Penyusutan  ';
		
	}
	/**/
	//$arrTglOld = explode('-', $old['tgl']);
	$arrTglSusutOld = explode('-', $old['tgl_susut']);
	if($err=='' && $tahunAnggaranRequest<$arrTglSusutOld[0] ){// && $explodeTahun[0]<$arrTglOld[0] ){
		$err = " Tahun Closing masih ".$arrTglSusutOld[0].", belum di batalkan ";
	}		
			...
			if($fmST == 0){ 
				if($err==''){
					...
					if($cnt['cnt']==0){
						$tgl_susut = ($HTTP_COOKIE_VARS['coThnAnggaran']-1).'-12-31';
						$aqry = "INSERT into t_closing (c1,c,d,e,e1,uid,tgl,ket,tgl_update, tgl_susut, tgl_update_susut) ".
							" values('$c1','$c','$d','$e','$e1','$userId' , '".$tahunAnggaranRequest."-12-31', '$keterangan',now(), '$tgl_susut', now()  )";

						$cek .= $aqry;		
					}else{
						$aqry = "UPDATE t_closing set tgl_update = now(), tgl = '$tahunAnggaranRequest-12-31', uid='$userId', ket='$keterangan'".
				...
				if($err==''){
					if(empty($c_d['c1']) && empty($c_d['c']) && empty($c_d['d']) && empty($c_d['e']) && empty($c_d['e1'])){
						...
						$aqry = "UPDATE t_closing set tgl_update = '$tanggalUpdate', tgl = '$tahunAnggaranRequest-12-31', ket='$keterangan'".
							"WHERE c1='".$c1."' AND c='".$c."' AND d='".$d."' AND e='".$e."' AND e1='".$e1."'";
							$cek .= $aqry;
				     	$cek .= $aqry;
					}	
 ...
 //     $CBdata = explode(' ',$idplh);
	...
	// 		$queryClosing = "DELETE from t_closing where c1 =".$c1CB." and c =00 and d=00 and e=00 and e1=000 and tgl_susut = '0000-00-00'";
	// 	}

	// 	if($cCB != "00"){

	// 		 $queryClosing = "DELETE from t_closing where c1 =".$c1CB." and c =".$cCB." and d=00 and e=00 and e1=000 and tgl_susut = '0000-00-00'";
	// 	}

	// 	if($dCB != '00'){
			 
	// 		 $queryClosing = "DELETE from t_closing  where c1 =".$c1CB." and c =".$cCB." and d=".$dCB." and e=00 and e1=000 and tgl_susut = '0000-00-00'";
	// 	}
	// 	if($eCB != '00'){

	// 		$queryClosing = "DELETE from t_closing  where c1 =".$c1CB." and c =".$cCB." and d=".$dCB." and e=".$eCB." and e1=000 and tgl_susut = '0000-00-00'";
	// 	}
	// 	if($e1CB != '000'){
			
	// 		$queryClosing = "DELETE from t_closing where c1 =".$c1CB." and c =".$cCB." and d=".$dCB." and e=".$eCB." and e1=".$e1CB."  and tgl_susut = '0000-00-00'";

	// 	}

	// 	mysql_query($queryClosing);
 ...
 //    }	
	...
	$cbid = $_REQUEST[$this->Prefix.'_cb'];
		...
		//if($DataOption['skpd'] == 1){
			$dt['c1'] = $_REQUEST['refclosingdataSKPD2fmURUSAN'];
			$dt['c'] = $_REQUEST['refclosingdataSKPD2fmSKPD'];
			$dt['d'] = $_REQUEST['refclosingdataSKPD2fmUNIT'];
			$dt['e'] = $_REQUEST['refclosingdataSKPD2fmSUBUNIT'];
			$dt['e1'] = $_REQUEST['refclosingdataSKPD2fmSEKSI'];
	...
	$cekRequest = count($_REQUEST['refclosingdata_cb']);

	if ($cekRequest > 1) {  $err = "PILIH DATA HANYA 1"; }

	  if ($_REQUEST['refclosingdata_cb'] == "") {// 
		...
		$readonly='readonly';
		$dataCB = $_REQUEST['refclosingdata_cb'];
...
if ($dataClosing['tgl'] == "") {
   	  ...
   	  $explodeTahun = explode('-', $dataClosing['tgl']);
   	  $tahunAnggaran = $explodeTahun[0];
   }     
				
	
	//$vthn = "<input type='text' name='tahun'  id='tahun' size='4' maxlength='4' value='".$tahunAnggaran."'style='margin-left: -3px; padding-left: 3px;width: 17%;'>";
	$arrThn = array();
	...
	<A href=\"pages.php?Pg=refclosingdata\" title='PENGADAAN' style='color:blue;'>PEROLEHAN</a> | 
	<A href=\"pages.php?Pg=refclosingdataPenyusutan\" title='PEMELIHARAAN'>PENYUSUTAN</a>  |
	...
	function setKolomHeader($Mode=1, $Checkbox=''){
	 ...
	 $headerTable =
	  "<thead>
	   ...
	   <th class='th01' width='300' title='Closing s/d Tahun'>CLOSING </th>
    ...
    if ($isi['c1'] != "0" && $isi['c'] != "00" && $isi['d'] != "00" && $isi['e'] != "00" && $isi['e1'] != "000") {
	 ...
	 $date=date_create($isi['tgl_update']);
	 $vtglUpdate = $isi['tgl_update']=='' || $isi['tgl_update']=='0000-00-00 00:00:00' ? '' : date_format($date,"d , M Y  -  H:i:s");
	$arrtgl=explode('-',$isi['tgl']);
