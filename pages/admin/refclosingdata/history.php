...
class refclosingdataHistoryObj  extends DaftarObj2{	
	var $Prefix = 'refclosingdataHistory';
	var $elCurrPage="HalDefault";
	var $SHOW_CEK = TRUE;
	var $TblName = 't_closing_hist'; //bonus
	var $TblName_Hapus = 't_closing_hist';
	var $MaxFlush = 10;
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='refclosingdataHistory.xls';
	var $namaModulCetak='REFERENSI DATA';
	var $Cetak_Judul = 'CLOSING DATA PEROLEHAN';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'refclosingdataHistoryForm';
	var $noModul=14; 
	var $TampilFilterColapse = 0; //0
	
	function setTitle(){
		return 'HISTORY CLOSING';
	}
...
if ($explodeTahun[0] > $tahun) {
	$err = 'Tahun tidak boleh melebihi Tahun Anggaran';
}
			...
			if($fmST == 0){
				if($err==''){
					if(empty($c_d['c1']) && empty($c_d['c']) && empty($c_d['d']) && empty($c_d['e']) && empty($c_d['e1'])){
						$aqry = "INSERT into t_closing (c1,c,d,e,e1,uid,tgl,ket,tgl_update,tgl_susut,tgl_persediaan) values('$c1','$c','$d','$e','$e1','$userId' $dataQuery)";
	...
	function setPage_OtherScript(){
		...
		return 	
			"<script type='text/javascript' src='js/skpd.js' language='JavaScript' ></script>".		
			"<script type='text/javascript' src='js/admin/refclosingdata/history.js' language='JavaScript' ></script>".
			$scriptload;
	}
 ...
 //    }	
	...
	$cbid = $_REQUEST[$this->Prefix.'_cb'];
		...
		//if($DataOption['skpd'] == 1){
			$dt['c1'] = $_REQUEST['refclosingdataHistorySKPD2fmURUSAN'];
			$dt['c'] = $_REQUEST['refclosingdataHistorySKPD2fmSKPD'];
			$dt['d'] = $_REQUEST['refclosingdataHistorySKPD2fmUNIT'];
			$dt['e'] = $_REQUEST['refclosingdataHistorySKPD2fmSUBUNIT'];
			$dt['e1'] = $_REQUEST['refclosingdataHistorySKPD2fmSEKSI'];
	...
	function setFormrefclosingdataHistory(){
		$dt=array();
		//$this->form_idplh ='';
		$this->form_fmST = 0;
		$dt['mode'] = $_REQUEST['mode'];
  	...
  	function setFormBatal(){
		...
		$this->form_fmST = 1;						
		//get data 
		$aqry = "SELECT * FROM  v1_refclosingdataHistory WHERE c='".$kode[0]."' AND d='".$kode[1]."'"; $cek.=$aqry;
	...
	$cekRequest = count($_REQUEST['refclosingdataHistory_cb']);

if ($cekRequest > 1) {
   $err = "PILIH DATA HANYA 1";
}

	  if ($_REQUEST['refclosingdataHistory_cb'] == "") {
		...
		$readonly='readonly';
		$dataCB = $_REQUEST['refclosingdataHistory_cb'];
...
if ($dataClosing['tgl'] == "") {
	...
	<A href=\"pages.php?Pg=refclosingdata\" title='PENGADAAN' >PEROLEHAN</a> | 
	<A href=\"pages.php?Pg=refclosingdataPenyusutan\" title='PEMELIHARAAN'>PENYUSUTAN</a>  |
		<A href=\"pages.php?Pg=refclosingdataHistory\" title='refclosingdataHistory' style='color:blue;'>HISTORY</a>  
	$retensi
	&nbsp&nbsp&nbsp	
	</td></tr></table>";
	}
    ...
    if ($isi['c1'] != "0" && $isi['c'] != "00" && $isi['d'] != "00" && $isi['e'] != "00" && $isi['e1'] != "000") {
	 ...
	 $Koloms[] = array('align="center"',date_format($date,"d , M Y  -  H:i:s"));
...
$refclosingdataHistory = new refclosingdataHistoryObj();
?>