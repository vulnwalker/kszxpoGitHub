...
class refmigrasiObj  extends DaftarObj2{	
	var $Prefix = 'refmigrasi';
	var $elCurrPage="HalDefault";
	var $SHOW_CEK = TRUE;
	var $TblName = 'v1_refmigrasi'; //bonus
	var $TblName_Hapus = 'refmigrasi';
	var $MaxFlush = 10;
	...
	var $Cetak_Judul = 'refmigrasi';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'refmigrasiForm';
	var $noModul=14; 
			...
			if($fmST == 0){
				if($err==''){
					if(empty($c_d['c']) && empty($c_d['d'])){
						$aqry = "INSERT into refmigrasi (c,d,bk,ck,dk) values('$c','$d','$bk','$ck','$dk')";	$cek .= $aqry;						
					}else{
						$aqry = "UPDATE refmigrasi
							set "." bk = '$bk',
							ck = '$ck',
							dk = '$dk'".
	...
	function setPage_OtherScript(){
		...
		return 	
			"<script type='text/javascript' src='js/admin/refmigrasi/".strtolower($this->Prefix).".js' language='JavaScript' ></script>".
			$scriptload;
	}
