...
class tesmenuv5Obj  extends DaftarObj2{	
	var $Prefix = 'tesmenuv5';
	...
	$set_urusan = $Main->URUSAN;
		...
		"<td>".genPanelIcon("javascript:".$this->Prefix.".KetetapanTanpaUsulan($set_urusan)","new_f2.png","Tanpa Usulan", 'Ketetapan Tanpa Usulan','','','','','style=width:80px')."</td>".
	...
	function simpan(){
		...
		if($Main->KD_BARANG_P108){
			$f1=$expkdbarang['0']; $f2=$expkdbarang['1']; $f=$expkdbarang['2']; $g=$expkdbarang['3']; $h=$expkdbarang['4']; $i=$expkdbarang['5']; $j=$expkdbarang['6'];
		}else{
		...
		$kd_brg = $Main->KD_BARANG_P108?$f1=='' && $f2=='' && $f=='' && $g=='' && $h=='' && $i=='' && $j=='':$f=='' && $g=='' && $h=='' && $i=='' && $j=='';
		if ($kd_brg){
		...
		if ($err==''){
			if($fmST==0){
				$aqry="insert into $tabel (id_bukuinduk,idbi_awal,a1,a,b,c1,c,d,e,e1,f1,f2,f,g,h,i,j,no_usul,tgl_usul,tahun,thn_perolehan,noreg,kepada_nama,
					...
					'$c1','$c','$d','$e','$e1','$f1','$f2','$f','$g','$h','$i','$j','$no_usul','$tgl_usul','$tahun','$tahun','$noreg','$kepada_nama',
				...
				f1='$f1',f2='$f2',f='$f',g='$g',h='$h',i='$i',j='$j',id_bukuinduk='$id_bukuinduk',
				no_usul='$no_usul',
				tgl_usul='$tgl_usul',
				tahun='$tahun',
				noreg='$noreg',
	...
	function simpanPilih(){
		...
		if($err==''){
			...
			$brg = mysql_fetch_array(mysql_query("select * from ref_barang where concat($concatkd)='$kdBarang'")) ;
			$nilai_buku = getNilaiBuku($bi['id'],$tgl,0);
		
			$content->fmIDBARANG = $Main->KD_BARANG_P108?$bi['f1'].'.'.$bi['f2'].'.'.$bi['f'].'.'.$bi['g'].'.'.$bi['h'].'.'.$bi['i'].'.'.$bi['j']:$bi['f'].'.'.$bi['g'].'.'.$bi['h'].'.'.$bi['i'].'.'.$bi['j'];// '1';
	...
	function simpanSK(){
		...
		$dtbi = mysql_fetch_array(mysql_query("select c1,c,d,e,e1,staset from buku_induk where id='$id_bukuinduk'"));
		...
		//if ($tgl_gantirugi_thn<=$Main->TAHUN_CLOSING){
			if($Main->VERSI_NAME != 'JABAR'){
				if( sudahClosing($tgl_gantirugi,$dtbi['c'],$dtbi['d'],$dtbi['e'],$dtbi['e1'],$dtbi['c1']) ){
			...
			if ($tgl_gantirugi==''){
				$err="Tanggal Pembukuan Tidak Boleh Kosong";
			}
			if($errmsg=='' && !cektanggal($tgl_gantirugi)){
				$err = "Tanggal Pembukuan $tgl_gantirugi Salah !";	
			}			
			if ($harga==''){
		...
		function simpanSKTU(){
			...
			if($Main->KD_BARANG_P108){
				...
				$f1=0; $f2=0; $f=$expkdbarang['0']; $g=$expkdbarang['1']; $h=$expkdbarang['2']; $i=$expkdbarang['3']; $j=$expkdbarang['4'];
			}
			$tahun=$_REQUEST['tahun'];
			$harga=$_REQUEST['harga'];
			$harga_perolehan=str_replace('.','',$_REQUEST['harga_perolehan']);
			...
			$get_susut = mysql_fetch_array(mysql_query($query_susut));
			$get_cd = mysql_fetch_array(mysql_query("select c1,c,d,e,e1,staset,idawal from buku_induk where id='$id_bukuinduk'"));
			...
			if($Main->VERSI_NAME != 'JABAR'){
				...
				$param_closing=$fmTANGGALgantirugi.",".$get_cd['c'].",".$get_cd['d'].",".$get_cd['e'].",".$get_cd['e1'].",".$get_cd['c1'];
			}else{
				$e1='001';
				$field='harga_perolehan';
				$vfield="'$harga_perolehan'";
			...
			if($err==''){
				$aqry="insert into $tabel (id_bukuinduk,a1,a,b,c1,c,d,e,e1,f1,f2,f,g,h,i,j,no_usul,tgl_usul,tahun,thn_perolehan,noreg,kepada_nama,
					kepada_alamat,uraian,ket,tgl_gantirugi,stat,uid,tgl_update,no_sk,tgl_sk,jml_harga,harga,$field) values
					('$id_bukuinduk','".$Main->DEF_KEPEMILIKAN."','".$Main->DEF_PROPINSI."','".$Main->DEF_WILAYAH."','$c1','$c','$d','$e','$e1','$f1','$f2','$f','$g','$h','$i','$j','$no_usul','$tgl_usul','$tahun','$tahun','$noreg','$kepada_nama',
	...
	if($cek_dt['cnt']>0){
		...
		function setPage_OtherScript(){
			...
			"<script type='text/javascript' src='js/_TES/menuv5/".strtolower($this->Prefix).".js' language='JavaScript' ></script>".
		...
		function setPage_Header(){		
			global $Menu, $Menuv5;
			/**
			//ambil title dari menu 
		    $menu=$Menu->getData();
			if($menu != NULL){
				$this->PageTitle =$menu['page_title'];
				$this->PageIcon =$menu['page_icon'];
			}
			...
			* **/
			...
			return $Menuv5->Nav1.$Menuv5->Nav2;
		}
		function setPage_Footer($param=''){		
			global $Main, $Menuv5;
			//ambil title dari menu 
		    //return $Main->CopyRight;
			return $Menuv5->footer1.$Menuv5->footer2;
		}
		...
		function BatalKetetapan(){//id -> multi id with space delimiter
			...
			//if($thn_gantirugi_lama<=$Main->TAHUN_CLOSING){
				...
				if($Main->VERSI_NAME != 'JABAR'){
					$param_closing=$tgl_gantirugi.','.$dtTGR['c'].','.$dtTGR['d'].','.$dtTGR['e'].','.$dtTGR['e1'].','.$dtTGR['c1'];
				}else{
				...
				function setForm($dt){	
					...
					*/
					$kdbarang = $Main->KD_BARANG_P108?$dt['f1'].'.'.$dt['f2'].'.'.$dt['f'].'.'.$dt['g'].'.'.$dt['h'].'.'.$dt['i'].'.'.$dt['j']:$dt['f'].'.'.$dt['g'].'.'.$dt['h'].'.'.$dt['i'].'.'.$dt['j'];
					...
					if ($this->form_fmST==0 || $this->form_fmST==1){ //baru /edit usulan
						...
						"&nbsp;<input type='button' value='cari' onclick=\"".$this->Prefix.".caribarang1(1,$settingurusan)\" >"			 
						),
						'tahun' => array( 
					...
					elseif($this->form_fmST==3){ //ketetapan tanpa usulan
						...
						"&nbsp;<input type='text' size='60' name='fmNMBARANG' id='fmNMBARANG' value='".$brg['nm_barang']."' readonly=''>".
						"&nbsp;<input type='button' value='cari' onclick=\"".$this->Prefix.".caribarang1(2,$settingurusan)\" >"			 
						),
						'tahun' => array( 
						'label'=>'Tahun Perolehan',
						'labelWidth'=>150, 
					...
					"<input type='hidden' value='".$dt['c1']."' name='c1' id='c1'>".
				...
				function setKolomData($no, $isi, $Mode, $CheckBox){
					...
					$dt2=mysql_fetch_array(mysql_query("select * from ref_barang where concat($concatkd)='$kdBarang'"));
					...
					$kode_barang = $Main->KD_BARANG_P108?$dtTGR['f1'].'.'.$dtTGR['f2'].'.'.$dtTGR['f'].'.'.$dtTGR['g'].'.'.$dtTGR['h'].'.'.$dtTGR['i'].'.'.$dtTGR['j']:$dtTGR['f'].'.'.$dtTGR['g'].'.'.$dtTGR['h'].'.'.$dtTGR['i'].'.'.$dtTGR['j'];
					...
					$Koloms[] = array('align="left" ',$kode_barang."/<br>".$isi['id_bukuinduk']);
