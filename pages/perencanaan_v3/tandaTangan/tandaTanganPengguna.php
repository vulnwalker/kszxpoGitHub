...
class tandaTanganPenggunaBarang_v3Obj  extends DaftarObj2{	
	var $Prefix = 'tandaTanganPenggunaBarang_v3';
	var $elCurrPage="HalDefault";
	var $SHOW_CEK = TRUE;
	var $TblName = 'tandatanganpenggunabarang_v3'; //daftar
	var $TblName_Hapus = 'tandatanganpenggunabarang_v3';
	var $MaxFlush = 10;
	...
	var $pagePerHal ='';
	var $cetak_xls=TRUE ;
	var $fileNameExcel='tandaTanganPenggunaBarang_v3.xls';
	var $Cetak_Judul = 'TANDA TANGAN';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'tandaTanganPenggunaBarang_v3Form'; 
	var $kdbrg = '';	
	var $arrEselon = array( 
		array('1','ESELON I'),
		array('2','ESELON II'),
	...
	<A href=\"pages.php?Pg=tandaTanganKuasaPenggunaBarang_v3\" title='TANDA TANGAN PENGGUNA BARANG'  > KUASA PENGGUNA BARANG </a> |
	<A href=\"pages.php?Pg=tandaTanganPenggunaBarang_v3\" title='TANDA TANGAN PENGGUNA BARANG'  style='color : blue;' > PENGGUNA BARANG </a> |
	...
	//get data -----------------
	 ...
	 if( $err=='' && $jabatan =='' ) $err= 'Jabatan Belum Di Isi !!';
			...
			if($fmST == 0){
			$ck1=mysql_fetch_array(mysql_query("Select * from $this->TblName where c1='$kode1' and c='$kode2' and d ='$kode3' and kategori = '$kategori'"));
	...
	function setPage_OtherScript(){
		...
		return 
			 ...
			 <script type='text/javascript' src='js/perencanaan_v3/tandaTangan/tandaTanganPengguna.js' language='JavaScript' ></script>
			 ".
	...
	function setFormBaru(){
		...
		if(empty($cmbSKPD))$err = "SKPD Belum Di Pilih";
	...
	//	$cek.=$data;
		...
		$queryPangkat2="select pangkat,concat(pangkat,' (',gol,'/',ruang,')')as nama from tandaTanganPenggunaBarang_v3 where pangkat='".$dt['pangkat']."' and gol='".$dt['gol']."' and ruang='".$dt['ruang']."'";
	//	$queryPangkat2=mysql_fetch_array(mysql_query("select pangkat,concat(pangkat,' (',gol,'/',ruang,')')as nama from tandaTanganPenggunaBarang_v3 where pangkat='".$dt['pangkat']."' and gol='".$dt['gol']."' and ruang='".$dt['ruang']."'"));
		$cek.="select pangkat,concat(pangkat,' (',gol,'/',ruang,')')as nama from tandaTanganPenggunaBarang_v3 where pangkat='".$dt['pangkat']."' and gol='".$dt['gol']."' and ruang='".$dt['ruang']."'";
       //items ----------------------
	...
	foreach ($_COOKIE as $key => $value) { 
		...
		$urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=tandaTanganPenggunaBarang_v3.refreshList(true);','-- URUSAN --');
		
		$codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
		$bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=tandaTanganPenggunaBarang_v3.refreshList(true);','-- BIDANG --');
		
		$codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
		$skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=tandaTanganPenggunaBarang_v3.refreshList(true);','-- SKPD --');
		
		
		$arrayKategori = array(
						   array('PEJABAT','PEJABAT'),
						   array('PENGURUS','PENGURUS'),
						   array('PENGGUNA','PENGGUNA')
					);
		$cmbKategori = cmbArray('kategori',$_REQUEST['kategori'],$arrayKategori,'-- KATEGORI--','onchange=tandaTanganPenggunaBarang_v3.refreshList(true);');
...
$tandaTanganPenggunaBarang_v3 = new tandaTanganPenggunaBarang_v3Obj();
?>