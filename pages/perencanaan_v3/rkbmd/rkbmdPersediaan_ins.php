<?php
class rkbmdPersediaan_ins_v3Obj  extends DaftarObj2{
	var $Prefix = 'rkbmdPersediaan_ins_v3';
	var $elCurrPage="HalDefault";
	var $SHOW_CEK = TRUE;
	var $TblName = 'temp_rkbmd_persediaan'; //bonus
	...
	var $FormName = 'rkbmdPersediaan_ins_v3Form';
	var $noModul=14;
	...
	var $idTahap = "";
	var $username = '';
	var $kondisiBarang = " and f!='01' and f!='02' and f!='03' and f!='04' and f!='05' and f!='06' ";
	var $statusPenetapan = "";
	function setTitle(){
	    $id = $_REQUEST['ID_RENJA'];
	    $getTahun = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$id'"));
		return 'RKBMD PERSEDIAAN TAHUN '.$getTahun['tahun'] ;
	}
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){
		...
		case 'subDelete':{
			...
			mysql_query("delete from temp_rkbmd_persediaan where id='$id'");
			$getRow = mysql_num_rows(mysql_query("select * from temp_rkbmd_persediaan where user = '$username'"));
			if($getRow > 0){
				$content = "ada";
			}else{
				$content = "";
		...
		case 'subEdit':{
			...
			if($jumlah > $jumlahKebutuhanRiil && $jumlahKebutuhanRiil != '0' && !empty($jumlahKebutuhanOptimal) && !empty($jumlahKebutuhanMaksimal)){
				...
				mysql_query(VulnWalkerUpdate('temp_rkbmd_persediaan',$data,"id = '$id'"));
			}

			$content = VulnWalkerUpdate('temp_rkbmd_persediaan',$data,"id = '$id'");
		...
		case 'finish':{
			$username = $_COOKIE['coID'];
			$execute = mysql_query("select * from temp_rkbmd_persediaan where user='$username'");
			...
			}elseif(checkPenetapanRKBMD($this->tahun,$this->jenisAnggaran,"PERSEDIAAN",$getDataSKPD['c1'],$getDataSKPD['c'],$getDataSKPD['d']) != 0 ){
			...
			mysql_query("delete from tabel_anggaran where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and bk='$bk' and ck='$ck' and dk='$dk' and p='$p' and q='$q' and tahun='$this->tahun' and id_tahap ='$this->idTahap' and ((id_jenis_pemeliharaan = '0' and f !='00') or uraian_pemeliharaan = 'RKBMD PERSEDIAAN') and uraian_pemeliharaan != 'RKBMD PENGADAAN' $this->kondisiBarang and status_validasi !='1' $exceptID ");
			...
			$cekProgram = mysql_num_rows(mysql_query("select * from view_rkbmd where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and bk = '$bk' and dk = '$dk' and ck = '$ck' and p = '$p' and q= '0' and id_tahap='$this->idTahap' "));
			if($cekProgram < 1){
				$data = array('jenis_anggaran' => $this->jenisAnggaran,
							  'tahun' => $this->tahun,
							  'c1' => $c1,
			...
			$cekKegiatan = mysql_num_rows(mysql_query("select * from view_rkbmd where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and bk = '$bk' and dk = '$dk' and ck = '$ck' and p = '$p' and q= '$q' and  f ='00' and id_tahap='$this->idTahap' and uraian_pemeliharaan = 'RKBMD PERSEDIAAN'"));
			if($cekKegiatan < 1){
				$data = array('jenis_anggaran' => $this->jenisAnggaran,
							  'tahun' => $this->tahun,
							  'c1' => $c1,
			...
			$execute2 = mysql_query("select * from temp_rkbmd_persediaan where user='$username'");
		...
		case 'Simpan':{
			...
			$username = $_COOKIE['coID'];
			$cekAda = mysql_num_rows(mysql_query("select * from temp_rkbmd_persediaan where  f='$f' and g='$g' and h='$h' and i='$i' and j='$j' and j1='$j1' and user='$username'"));
			if($cekAda == 1){
				$err = "Barang sudah ada";
			}elseif(mysql_num_rows(mysql_query("select * from view_rkbmd where id_tahap = '$this->idTahap' and c1= '$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and bk='$bk' and ck='$ck' and dk='$dk' and p='$p' and q='$q' and f='$f' and g='$g' and h='$h' and i='$i' and j='$j' and j1='$j1'")) != 0){
				$err = "Barang sudah ada";
			}
			if($err == ''){
				$data  = array("c1" => $c1,
				...
				$query = VulnWalkerInsert("temp_rkbmd_persediaan",$data);

				mysql_query($query);
					$codeAndNameKegiatan = "select q, concat(q,'. ', nama) from ref_program where bk = '$bk' and dk = '$dk' and ck = '$ck' and dk = '$dk' and p = '$p' and q !='0' ";
	$cmbKegiatan = cmbQuery('q', $q, $codeAndNameKegiatan,' disabled','-- KEGIATAN --');
		...
		case 'CekAda':{
			...
			$getAll = mysql_query("select * from view_rkbmd where tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and jenis_form_modul ='PENYUSUNAN' and id_tahap='$this->idTahap' and j !='000' and id_jenis_pemeliharaan ='0' and c1='$c1' and c='$c' and d='$d' and e='$e' and e1 ='$e1' and bk='".$_REQUEST['bk']."' and ck='".$_REQUEST['ck']."' and dk='".$_REQUEST['dk']."' and p='".$_REQUEST['p']."' and q='".$_REQUEST['q']."' and status_validasi !='1' $exceptID $this->kondisiBarang");
			...
			$content = array("status" => $tergantung, "query" => "select * from view_rkbmd where tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and jenis_form_modul ='PENYUSUNAN' and id_tahap='$this->idTahap' and j !='000' and j1!='' and id_jenis_pemeliharaan ='0' and c1='$c1' and c='$c' and d='$d' and e='$e' and e1 ='$e1' and bk='$bk' and ck='$ck' and dk='$dk' and p='$p' and q='$q'",'checkek' => $checker);
		...
		case 'subShowEdit':{
			...
			$get = mysql_fetch_array(mysql_query("select * from temp_rkbmd_persediaan where id = '$id'"));
			foreach ($get as $key => $value) {
			  $$key = $value;
			}
		    $concat2 = $f.".".$g.".".$h.".".$i.".".$j.".".$j1;
			$concat = $c1.".".$c.".".$d.".".$e.".".$e1.".".$f.".".$g.".".$h.".".$i.".".$j.".".$j1 ;
       	    $getKebutuhanMaksimal = mysql_fetch_array(mysql_query("select * from ref_std_kebutuhan where concat(c1,'.',c,'.',d,'.',e,'.',e1,'.',f,'.',g,'.',h,'.',i,'.',j,'.',j1) = '$concat'"));


		   $getKebutuhanOptimal = mysql_fetch_array(mysql_query("select sum(jml_barang) as kebutuhanOptimal from buku_induk where concat(c1,'.',c,'.',d,'.',e,'.',e1,'.',f,'.',g,'.',h,'.',i,'.',j,'.',j1) = '$concat' and status_barang = '1' and (kondisi = '1' or kondisi = '2') "));
		   $kebutuhanOptimal = $getKebutuhanOptimal['kebutuhanOptimal'];
		   $getBarang = mysql_fetch_array( mysql_query("select * from ref_barang where  f=$f and g=$g and h=$h and i=$i and j=$j and j1=$j1"));
   ...
   function setPage_OtherScript(){
		...
		return
			...
			"<script type='text/javascript' src='js/perencanaan_v3/rkbmd/rkbmdPersediaan_ins.js' language='JavaScript' ></script>".
			...
			<script type='text/javascript' src='js/perencanaan_v3/rkbmd/popupBarangPersediaan.js' language='JavaScript' ></script>
	...
	$Koloms = array();
	$concat = $f.".".$g.".".$h.".".$i.".".$j.".".$j1;
	$getNamaBarang = mysql_fetch_array(mysql_query("select * from ref_barang where concat(f,'.',g,'.',h,'.',i,'.',j,'.',j1) = '$concat'"));
	$Koloms[] = array('align="center" width="20"', $no.'.' );
    $Koloms[] = array('align="center"', $f.".".$g.".".$h.".".$i.".".$j.".".$j1  );
	...
	$aksi  = "<img src='images/administrator/images/invalid.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkbmdPersediaan_ins_v3.hapus('$id');></img>&nbsp &nbsp <img src='images/administrator/images/edit_f2.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkbmdPersediaan_ins_v3.edit('$id');></img> ";
	...
	$cekMayoritas = mysql_fetch_array(mysql_query("select * from temp_rkbmd_persediaan where user = '$this->username'"));
	$e = $cekMayoritas['e'];
	$e1 = $cekMayoritas['e1'];
	...
	$codeAndNameKegiatan = "select q, concat(q,'. ', nama) from ref_program where bk = '$selectedBK' and ck = '$selectedCK' and dk = '$selectedDK' and p = '$selectedP' and q ='$selecteQ' ";
	$cmbKegiatan = cmbQuery('q', $selecteQ, $codeAndNameKegiatan,' onchange=rkbmdPersediaan_ins_v3.CekAda(); ','-- KEGIATAN --');

	$cekGordon = mysql_num_rows(mysql_query("select * from temp_rkbmd_persediaan where user = '$username'"));
	if($cekGordon > 0){
		$cokot = mysql_fetch_array(mysql_query("select * from temp_rkbmd_persediaan where user = '$username'"));
		$selectedBK = $cokot['bk'];
		$selectedCK = $cokot['ck'];
		$selectedDK = $cokot['dk'];
		$selectedP = $cokot['p'];
		$selecteQ = $cokot['q'];
		$getNamaProgram = mysql_fetch_array(mysql_query("select * from ref_program where bk='$selectedBK' and ck='$selectedCK' and dk='$selectedDK' and p = '$selectedP' and q='0'"));
					...
					$this->isiform(
						array(
							...
							array(
								...
								<input type='button' value='Cari' id='findProgram' onclick ='rkbmdPersediaan_ins_v3.CariProgram($ID_RENJA)'  title='Cari Program dan Kegiatan' $disabledCariProgram >"
							),
							array(
								'label'=>'KEGIATAN',
								'label-width'=>'200px;',
						...
						array(
							...
							array(
									...
									'value'=>"<input type='text' name='kodeBarang' onkeyup='cariBarang.pilBar2(this.value)' id='kodeBarang' placeholder='KODE BARANG' style='width:150px;' value='".$dt['kodebarangnya']."' readonly />
										...
										<input type='button' id='findBarang' value='CARI' onclick='rkbmdPersediaan_ins_v3.CariBarang();'/>
									",
								),
							array(
									'label'=>'JUMLAH',
...
/*							array(  'name' => "jumlahKebutuhanRiil",
				...
				genFilterBar(
					...
					<td class='border:none'>
						<a class='toolbar' id='btsave' href='javascript:rkbmdPersediaan_ins_v3.$tujuan'>
					...
					<tbody><tr valign='middle' align='center'>
					<td class='border:none'>
						<a class='toolbar' id='btcancel' href='javascript:rkbmdPersediaan_ins_v3.closeTab()'>
					...
					<tbody><tr valign='middle' align='center'>
					<td class='border:none'>
						<a class='toolbar' id='finish' href='javascript:rkbmdPersediaan_ins_v3.finish()'>
...
$rkbmdPersediaan_ins_v3 = new rkbmdPersediaan_ins_v3Obj();

$arrayResult = tahapPerencanaanV3($rkbmdPersediaan_ins_v3->modul,"MURNI");
...
$rkbmdPersediaan_ins_v3->jenisForm = $jenisForm;
$rkbmdPersediaan_ins_v3->nomorUrut = $nomorUrut;
$rkbmdPersediaan_ins_v3->urutTerakhir = $nomorUrut;
$rkbmdPersediaan_ins_v3->tahun = $tahun;
$rkbmdPersediaan_ins_v3->jenisAnggaran = $jenisAnggaran;
$rkbmdPersediaan_ins_v3->idTahap = $idTahap;
$rkbmdPersediaan_ins_v3->username = $_COOKIE['coID'];
$rkbmdPersediaan_ins_v3->wajibValidasi = $Main->wajibValidasi;
if($rkbmdPersediaan_ins_v3->wajibValidasi == TRUE){
	$rkbmdPersediaan_ins_v3->sqlValidasi = " and status_validasi ='1' ";
}else{
	$rkbmdPersediaan_ins_v3->sqlValidasi = " ";
}
$rkbmdPersediaan_ins_v3->statusPenetapan = checkPenetapanRKBMD($rkbmdPersediaan_ins_v3->tahun,$rkbmdPersediaan_ins_v3->jenisAnggaran, "PERSEDIAAN");
