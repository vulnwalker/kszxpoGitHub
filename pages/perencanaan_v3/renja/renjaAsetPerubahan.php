...
class renjaAsetPerubahanObj  extends DaftarObj2{
	var $Prefix = 'renjaAsetPerubahan';
	var $elCurrPage="HalDefault";
	var $SHOW_CEK = TRUE;
	var $TblName = 'view_renja'; //bonus
	var $TblName_Hapus = 'renjaAsetPerubahan';
	var $MaxFlush = 10;
	...
	var $pagePerHal ='';
	var $fileNameExcel='renjaAsetPerubahan.xls';
	var $namaModulCetak='DAFTAR renjaAsetPerubahan';
	var $Cetak_Judul = 'DAFTAR renjaAsetPerubahan';
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'renjaAsetPerubahanForm';
	...
	function setMenuEdit(){
		 ...
		 if ($this->jenisForm == "PENYUSUNAN"){
		 	$listMenu =
						...
						;
			if($this->settingAnggaran != "PERUBAHAN"){
					$listMenu =
	...
	function setPage_HeaderOther(){
		...
		<tr><td class=\"menudottedline\" width=\"40%\" height=\"20\" style='text-align:right'><B>
		<A href=\"pages.php?Pg=renjaAsetPerubahan\" title='RENJA' style='color : blue;' > RENJA </a> |
	...
	function set_selector_other($tipe){
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){
		...
		case 'sync':{
			$getRenjaKeuangan = mysql_query("select * from view_renja where jumlah !='' and jumlah !='0' and q!='0' and jenis_anggaran = 'PERUBAHAN'");
		...
		case 'newTab':{
			 ...
			 foreach ($_REQUEST as $key => $value) {
				...
				$ID_PLAFON = $grabID['id'];
				$content = array('idrenjaAsetPerubahan' => $ID_PLAFON, "status" => $statunya );
			break;
		    }
			...
			case 'editTab':{
			 ...
			 $urutArray = explode(" ",$renjaAsetPerubahan_cb[0]);
			 $urut = $renjaAsetPerubahan_cb[0];
			 $query = "select * from view_renja where id_anggaran = '$renjaAsetPerubahan_cb[0]' ";
			 $getViewrenjaAsetPerubahan = mysql_fetch_array(mysql_query($query));
			 foreach ($getViewrenjaAsetPerubahan as $key => $value) {
				  $$key = $value;
			 }
			 $IDEDIT = $id_anggaran;
			 ...
			 $content = array('ID_PLAFON' => $ID_PLAFON,  'ID_EDIT' => $renjaAsetPerubahan_cb[0] , 'query ' =>"select * from view_renja where jenis_anggaran = '$this->jenisAnggaran' and tahun = '$this->tahun' and c1 = '$c1' and c = '$c' and d='$d' and e='00' and e1='000' and bk='0' and ck = '0' and dk = '0' and p ='0' and q = '0' and no_urut = '$nomor' ");
			...
			case 'jenisChanged':{
				...
				if($jenisKegiatan == 'baru'){
					...
					$plus = "<input type='text' name ='plus' id ='plus' style='width:200px; text-align:right;'  onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='document.getElementById(`keyPP`).textContent = `Rp. ` + popupProgramrenjaAsetPerubahan.formatCurrency(this.value);' > ";
					$minus = "<input type='text' name ='minus' id ='minus' style='width:200px; text-align:right;'  onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='document.getElementById(`keyMM`).textContent = `Rp. ` + popupProgramrenjaAsetPerubahan.formatCurrency(this.value);'> ";
				}
			...
			case 'comboPlafon':{
				...
				$getidTahapPlafon = mysql_fetch_array(mysql_query("select max(id_tahap) as max from view_plafon_v3 where tahun='$this->tahun' and jenis_anggaran='$this->jenisAnggaran' "));
				$idTahapPlafon= $getidTahapPlafon['max'];
				$getAngkaPlafon = mysql_fetch_array(mysql_query("select plafon from view_plafon_v3 where c1='$c1' and c='$c' and d='$d' and id_tahap='$idTahapPlafon'"));
		...
		case 'SaveValid':{
	    	foreach ($_REQUEST as $key => $value) {
			 ...
			 $getSKPD = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$renjaAsetPerubahan_idplh'"));
			 ...
			 $query = VulnWalkerUpdate("tabel_anggaran",$data," id_anggaran = '$renjaAsetPerubahan_idplh'");
			 mysql_query($query);
		...
		case 'koreksi':{
			...
			if($this->jenisForm !='KOREKSI'){
			 ...
			 $getIDPlafon = mysql_fetch_array(mysql_query("select max(id_anggaran) as idPlafon from view_plafon_v3 where tahun = '$this->tahun' and jenis_anggaran = '$this->jenisAnggaran' and c1 = '$cmbUrusanForm' and c = '$cmbBidangForm' and d = '$cmbSKPDForm'"));
			 $idPlafon = $getIDPlafon['idPlafon'];
			 $getJumlahPlafon = mysql_fetch_array(mysql_query("select plafon from view_plafon_v3 where id_anggaran = '$idPlafon'"));
   ...
   function setPage_OtherScript(){
	  ...
	  /* if($this->jenisForm == "KOREKSI"){
		...
		return
			...
			<script type='text/javascript' src='js/perencanaan_v3/renja/renjaAsetPerubahan.js' language='JavaScript' ></script>
			<script type='text/javascript' src='js/perencanaan_v3/renja/renjaAsetPerubahan_ins.js' language='JavaScript' ></script>
	...
	function setFormBaru(){
  	...
  	function setFormEdit(){
		...
		if($cnt['cnt'] > 0) $err = "renjaAsetPerubahan Tidak Bisa Diubah ! Sudah Digunakan Di Ref Barang.";
		if($err == ''){
	...
	function Info(){
	 ...
	 $getidTahapPlafon = mysql_fetch_array(mysql_query("select max(id_tahap) as max from view_plafon_v3 where tahun='$this->tahun' and jenis_anggaran='$this->jenisAnggaran' "));
	 $idTahapPlafon= $getidTahapPlafon['max'];
	 $getAngkaPlafon = mysql_fetch_array(mysql_query("select plafon from view_plafon_v3 where c1='$selectedC1' and c='$selectedC' and d='$selectedD' and id_tahap='$idTahapPlafon'"));
	 $angkaPlafon = number_format($getAngkaPlafon['plafon'],2,',','.');
	 $codeAndNameUrusan = "select c1, concat(c1, '. ', nm_skpd) from ref_skpd where c='00' and d='00' and e='00' and e1='000' ";
	 $urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=renjaAsetPerubahan.comboChanged(); disabled','-- URUSAN --');

	 $codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
	 $bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=renjaAsetPerubahan.comboChanged(); disabled','-- BIDANG --');

	 $codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
	 $skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=renjaAsetPerubahan.comboChanged(); disabled' ,'-- SKPD --');
	/* if($this->jenisFormTerakhir == "VALIDASI"){
	 	$getJumlahSKPDYangMengisiPlafon = mysql_num_rows(mysql_query("select * from view_plafon_v3 where tahun='$this->tahun' and jenis_anggaran = '$this->jenisAnggaran' and no_urut ='$this->noUrutTerakhirPlafon' and d!='00' and status_validasi = '1' "));
	 }else{
	 	$getJumlahSKPDYangMengisiPlafon = mysql_num_rows(mysql_query("select * from view_plafon_v3 where tahun='$this->tahun' and jenis_anggaran = '$this->jenisAnggaran' and no_urut ='$this->noUrutTerakhirPlafon' and d!='00' "));
	 }*/
	...
	$fmBIDANG = cekPOST('fmBIDANG');
	 ...
	 $arr = array(
			...
			array('nama_renjaAsetPerubahan','NAMA renjaAsetPerubahan'),
			array('alamat','ALAMAT'),
	 ...
	 //data order ------------------------------
	 $arrOrder = array(
			     	array('1','NAMA renjaAsetPerubahan'),
					array('2','ALAMAT'),
	...
	"<div class='FilterBar' style='margin-top:10px;'><table style='width:100%'>".
			CmbUrusanBidangSkpd('renjaAsetPerubahan').
"</table></div>";
	   ...
	   if(!isset($ref_skpdSkpdfmUrusan) ){
		...
		if($this->settingAnggaran != "MURNI"){
			$grabAllData = mysql_query("select * from view_renja where tahun = '$this->tahun' and jenis_anggaran = 'MURNI' and q !='0'");
			while($rows = mysql_fetch_array($grabAllData)){
				foreach ($rows as $key => $value) {
					  $$key = $value;
				 }
				 if(mysql_num_rows(mysql_query("select * from pindah_renja_v3 where id_anggaran = '$id_anggaran'")) != 0){

				 }else{
					 if(mysql_num_rows(mysql_query("select * from view_renja where c1 ='$c1' and c= '00' and id_tahap = '$this->idTahap'")) == 0){
				 		$data = array(
									    'c1' => $c1,
										'c' => '00',
										'd' => '00',
				 ...
				 if(mysql_num_rows(mysql_query("select * from view_renja where c1 ='$c1' and c= '$c' and d='00' and id_tahap = '$this->idTahap'")) == 0){
				 		$data = array(
									    'c1' => $c1,
										'c' => $c,
										'd' => '00',
				 ...
				 if(mysql_num_rows(mysql_query("select * from view_renja where c1 ='$c1' and c= '$c' and d='$d' and e='$e' and e1='$e1' and p ='0' and id_tahap = '$this->idTahap'")) == 0){
				 		$data = array(
									    'c1' => $c1,
										'c' => $c,
										'd' => $d,
				 ...
				 if(mysql_num_rows(mysql_query("select * from view_renja where c1 ='$c1' and c= '$c' and d='$d' and e='$e' and e1='$e1' and bk='$bk' and ck='$ck' and dk='$dk' and p ='$p' and q='0' and id_tahap = '$this->idTahap'")) == 0){
				 		$data = array(
									    'c1' => $c1,
										'c' => $c,
										'd' => $d,
				 ...
				 if(mysql_num_rows(mysql_query("select * from view_renja where c1 ='$c1' and c= '$c' and d='$d' and e='$e' and e1='$e1' and bk='$bk' and ck='$ck' and dk='$dk' and p ='$p' and q='$q' and id_tahap = '$this->idTahap'")) == 0){
				 		$data = array(
									    'c1' => $c1,
										'c' => $c,
										'd' => $d,
						...
						$query = VulnWalkerInsert("tabel_anggaran", $data);
						mysql_query($query);
						$getIdBarusan = mysql_fetch_array(mysql_query("select * from view_renja where c1 ='$c1' and c= '$c' and d='$d' and e='$e' and e1='$e1' and bk='$bk' and ck='$ck' and dk='$dk' and p ='$p' and q='$q' and id_tahap = '$this->idTahap'"));
						$maxIDTadi = $getIdBarusan['id_anggaran'];

						$grabDetailRenja = mysql_fetch_array(mysql_query("select * from detail_renja where id_anggaran = '$id_anggaran'"));
						foreach ($grabDetailRenja as $key => $value) {
							  $$key = $value;
						 }
						$data = array(
									...
									'tahun_jamak' => $tahun_jamak,
									'capaian_program_tk' => $capaian_program_tk,
									'capaian_program_tuk' => $capaian_program_tuk,
									'masuk_tuk' => $masuk_tuk,
									'masuk_tk' => $masuk_tk,
									'keluaran_tuk' => $keluaran_tuk,
									'keluaran_tk' => $keluaran_tk,
									'hasil_tuk' => $hasil_tuk,
									'hasil_tk' => $hasil_tk,
									'kelompok_sasaran_kegiatan' => $kelompok_sasaran_kegiatan,
									'id_tahap' => $this->idTahap,
									'output' => $output,
									'id_anggaran' => $maxIDTadi

									);
						$query = VulnWalkerInsert("detail_renja", $data);
						mysql_query($query);
						...
						mysql_query(VulnWalkerInsert('pindah_renja_v3',$dataPindah));
...
$renjaAsetPerubahan = new renjaAsetPerubahanObj();

$arrayResult = tahapPerencanaanV3($renjaAsetPerubahan->modul,'PERUBAHAN');
...
$renjaAsetPerubahan->jenisForm = $jenisForm;
$renjaAsetPerubahan->nomorUrut = $nomorUrut;
$renjaAsetPerubahan->tahun = $tahun;
$renjaAsetPerubahan->jenisAnggaran = $jenisAnggaran;
$renjaAsetPerubahan->idTahap = $idTahap;
$renjaAsetPerubahan->username = $_COOKIE['coID'];
$renjaAsetPerubahan->wajibValidasi = $arrayResult['wajib_validasi'];
$renjaAsetPerubahan->settingAnggaran = $arrayResult['settingAnggaran'];
