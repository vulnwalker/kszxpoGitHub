...
class penghapusan_insObj extends DaftarObj2{
	var $Prefix = 'penghapusan_ins';
	var $SHOW_CEK = TRUE;	
	var $TblName = 'penghapusan';//view2_sensus';
	var $TblName_Hapus = 'penghapusan';
	var $TblName_Edit = 'penghapusan';
	...
	var $FormName = 'penghapusan_insForm';
	var $pagePerHal = 25;
	...
	var $fileNameExcel='Daftar PENGHAPUSAN.xls';
	var $Cetak_Judul = 'PENGHAPUSAN';	
	var $Cetak_Mode=2;
	...
	for($i=0;$i<count($idarr);$i++){
		$getbi = "select c1,c,d,e,e1,tgl_buku,status_barang from buku_induk where id='".$idarr[$i]."' ";
		$bi =table_get_rec($getbi);
		$c1 = $bi['c1'];
		$c = $bi['c'];
		$d = $bi['d'];
		$e = $bi['e'];
		...
		$tgl_closing=getTglClosing($c,$d,$e,$e1,$c1);
		if($err=='' && $tgl_buku<=$tgl_closing)$err ='TANGGAL BUKU harus lebih besar dari Tanggal Closing !';
		if($err=='' &&  compareTanggal($tgl_buku, $fmTglBuku)==0 ){ $err = 'TANGGAL BUKU tidak lebih kecil dari Tanggal Buku Barang!'; }		
		//cek tgl BA
		if($err==''){
		...
		if($err == ''){
			...
			if($err=='' && $tgl_buku <= $ceksusut['tgl_penyusutan']) $err = 'Gagal penghapusan, sudah penyusutan !';
		}
		if ($bi['status_barang'] == 3) $errmsg = 'Barang sudah di Penghapusan!';
		if ($bi['status_barang'] == 5) $errmsg = 'Barang sudah di hapuskan di Tuntutan Ganti Rugi!';
		//if ($bi['status_barang'] == 4) $errmsg = 'Barang sudah di Pemindahtanganan!';
		//if ($err=='' && $bi['status_barang'] == 5) $errmsg = 'Barang sudah di Tuntutan Ganti Rugi!';
		...
		if($err==''){
			...
			$content->jml = $jml;	
				...
				while($old = mysql_fetch_array($result)){
					...
					//$hps = mysql_fetch_array(mysql_query("select count(*)as cnt from penghapusan where mutasi=0 and idbi_awal='$idbiawal'"));
					//if ($err=='' && $hps['cnt'] > 0) $err = 'Id Barang '.$idbiawal.' sudah di Penghapusan !';
					...
					if($err==''){
						...
						"nilai_buku,nilai_susut,tgl_create,uid_create,hapus_dari,status_barang) values ('$tgl_buku',0,0,'$no_sk','$tgl_sk',".
						...
						"'$nilai_buku','$nilai_susut',NOW(),'$uid','$databaru','$status_barang')";
						$qry_hps = mysql_query($query_hps);	
						
						//update buku induk set status barang lama =3
						$upd_bi = "UPDATE buku_induk set status_barang=3 WHERE id='$idbi'";//$err='tes='.$upd_bi;
						$qryupdbi = mysql_query($upd_bi);
						if($qryupdbi==FALSE)$err="Gagal Simpan data ".mysql_error();						
					}
					
					if($err=='' && $databaru==2){//dari Pemindahtangan
						$strdel = "update pemindahtanganan set st_hapus=1 WHERE id_bukuinduk='$idbi'";
						$qrydel = mysql_query($strdel);
						if($qrydel==FALSE)$err="Gagal update data ".mysql_error();
					}	
					
					if($err=='' && $databaru==3){//dari Pemusnahan
						$strdel = "update pemusnahan_det set st_hapus=1 WHERE id_bukuinduk='$idbi'";
						$qrydel = mysql_query($strdel);
						if($qrydel==FALSE)$err="Gagal update data ".mysql_error();
					}
					
					if($err=='' && $databaru==4){//dari gantirugi
						$strdel = "update gantirugi set stat=2 WHERE id_bukuinduk='$idbi'";
						$qrydel = mysql_query($strdel);
						if($qrydel==FALSE)$err="Gagal update data ".mysql_error();
					}	
				}//end while	
	...
	$progress = 
		...
		$TampilOpt =
			...
			genFilterBar(
				...
				genFilterBar(
				array(
					"<span id='inputpenerimaanbarang' style='color:black;font-size:14px;font-weight:bold;'/>SK PENGHAPUSAN</span>",
				),'','','').
				genFilterBar(
				array(
					$this->isiform(
	...
	function pageShow(){
		...
		if($baru==1){//dari Penatausaha
			$cbid = $_REQUEST['cidBI'];
			$idplh = implode(" ",$cbid);
		}elseif($baru==2){//dari Pemindahtangan
			$cbid = $_REQUEST['Pindahtangan_cb'];	
			$tbl='pemindahtanganan';		
		}elseif($baru==3){//pemusnahan	
			$cbid = $_REQUEST['pemusnahan_cb'];
			$tbl='pemusnahan_det';
		}else{//dari gantirugi
			$cbid = $_REQUEST['gantirugiprogres_cb'];
			$tbl='gantirugi';		
		}
			$valuearr = array();
			for($x = 0; $x<sizeof($cbid); $x++){ 
				$strSql ="select id_bukuinduk from $tbl where id = '".$cbid[$x]."'";
			...
			$idplh = $baru==1 ? implode(" ",$cbid):$valuestr;
		return
	...
	function setPage_OtherScript(){
		...
		"<script type='text/javascript' src='js/penghapusan/".strtolower($this->Prefix).".js' language='JavaScript' ></script>".
		$scriptload;
	}
...
$penghapusan_ins = new penghapusan_insObj();
