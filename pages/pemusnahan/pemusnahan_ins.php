...
class pemusnahan_insObj extends DaftarObj2{
	var $Prefix = 'pemusnahan_ins';
	var $SHOW_CEK = TRUE;	
	var $TblName = 'pemusnahan_det';//view2_sensus';
	...
	var $FormName = 'pemusnahan_insForm';
	var $pagePerHal = 25;
	...
	var $fileNameExcel='Daftar PEMUSNAHAN.xls';
	...
	//get data -----------------
	 ...
	 $c1_lama= $_REQUEST['urusan_lama'];
	 $c_lama= $_REQUEST['skpd_lama'];
	 $d_lama= $_REQUEST['unit_lama'];
	...
	if($err=='' && sudahClosing($tgl_buku,$c_lama,$d_lama,'','',$c1_lama))$err = 'Tanggal sudah Closing !';	
		...
		if($err==''){
			...
			$content->jml = $jml;	
				...
				while($old = mysql_fetch_array($result)){
					...
					if($err==''){
						...
						"values ('$idbi','$idbiawal','$c1','$c','$d','$e','$e1',".
			...
			if($qry==TRUE){					
				...
				$aqry = 
					"update pemusnahan ".
					"set ".
					" c1='$c1',c='$c',d='$d',no_ba='$no_sk',tgl_ba='$tgl_sk',tgl_buku='$tgl_buku',cara_pemusnahan='$cr_pemusnahan',".
					" ket='$ket',thn_anggaran='$tahun',".
					" uid='$uid', tgl_update= now(),sttemp='0' ".
					"where id='$id_musnah'";	
				//$aqry = "update pemusnahan_det set refid_pemusnahan='$id_musnah' where id in($idplh)";	
				//$cek .= $aqry;
	...
	$progress = 
		...
		$TampilOpt =
			...
			genFilterBar(
				...
				genFilterBar(
				array(
					"<span id='inputpenerimaanbarang' style='color:black;font-size:14px;font-weight:bold;'/>SK PEMUSNAHAN</span>",
				),'','','').
				genFilterBar(
				array(
					$this->isiform(
						array(	
							...
							array(
								...
								'name'=>'cr_pemusnahan',
								'label-width'=>'200px;',
								'value'=>"<input type='text' name='cr_pemusnahan' id='cr_pemusnahan' value='' size=29/>",
							),	
							array(
									'label'=>'KETERANGAN ',
									'name'=>'ket_koreksi',
...
$pemusnahan_ins = new pemusnahan_insObj();
