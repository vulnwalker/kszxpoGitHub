...
class pemusnahan_detObj extends DaftarObj2{
	...
	function setKolomHeader($Mode=1, $Checkbox=''){
	 ...
	 $Pelihara_Menu=
			...
			<li><a  href='javascript:pemusnahan_det.caribarang1(".$Main->URUSAN.")' title='Tambah' class='btadd'></a>
				<ul id='bgn_ulEntry'>
					<li style='width:470;top:-4;z-index:99;'>	</li>
				</ul>
			</li>
	...
	$TampilCheckBox = $Cetak? '' : "<TH class=\"th01\" rowspan=2 style='width:30'>
	 ...
	 "<thead>
	 <tr>
	   <th colspan='12'>$Pelihara_Menu</th>
	  </tr>
	  <tr>
	   ...
	   <th class='th01'  rowspan=2 width=100>Tanggal Buku</th>
	  </tr>
	  <tr>
	...
	//get dinas		
	 ...
	 $get_brg = mysql_fetch_array(mysql_query("select * from ref_barang where concat($concatkd)='$kdBarang'"));
	 ...
	 $Koloms[] = array('', 
			$kdBarang.'/'.$isi['id_bukuinduk'].'/<br>'.$get_brg['nm_barang']				
		);
	 ...
	 $Koloms[] = array("align='left'", TglInd($bi['tgl_buku']));
		...
		switch($tipe){
			...
			for($x = 0; $x<sizeof($ref_pilih); $x++){ 
				...
				while($bi = mysql_fetch_array($qry)){
					...
					$aqry2 = "update buku_induk set staset=14 where id='$id_bukuinduk'";$cek.=$aqry2;
					$qry2 = mysql_query($aqry2);					
				}				
				$valuearr[]="('$id_bukuinduk','$idawal','$c','$d','$e','$e1','$f1','$f2','$f','$g','$h','$i','$j',".
			...
			if($err==''){
				$querypr = "INSERT INTO $this->TblName(id_bukuinduk,idbi_awal,c,d,e,e1,f1,f2,f,g,h,i,j,".
