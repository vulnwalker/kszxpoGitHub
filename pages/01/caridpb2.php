...
$sqry = "select * from v1_pengadaan ".$Kondisi." order by f,g,h,i,j limit 0,200 "; $cek .= $sqry;
$cek .= '<br> sqry='.$sqry;
...
while($isi=mysql_fetch_array($Qry))
{
	...
	$List.= 
		...
		"</td>
		<td class='GarisDaftar' align='left'>".$isi['merk_barang']."</td>
		
		<td class='GarisDaftar' align='left'>".$isi['nama_perusahaan']."</td>
		<td class='GarisDaftar' align='left'>".$isi['spk_tgl']."</td>
...
<script>
	...
	function KlikGunakan(idbi,isi1,isi2, merk, jml, satuan, harga,tahun,idpengadaan)
	{
		...
		var id = parent.document.getElementById('penerimaan_idplh').value;
		...
		//alert('pages.php?Pg=dpb&ajx=1&tipe=hitPengadaanSebelumnya&idpengadaan='+idpengadaan);
		$.ajax({
			
			url: '../../pages.php?Pg=penerimaan&ajx=1&tipe=hitPenerimaanSebelumnya&idpengadaan='+idpengadaan
				,
		  	success: function(data) {		
				var resp = eval('(' + data + ')');							
				if(resp.err==''){
					//alert('c='+c+'d='+d+'e='+e+ 'jml='+resp.content.jml+' cek'+resp.cek);
					/*console.info('jmlada='+resp.content.jmlada+
						'jmldpb='+resp.content.jmldpb +
						'jml='+jml+
						'sisa='+jmlsisa
					);
					*/
					...
					parent.document.getElementById('jml_dpb').value= resp.content.jmldpb;
					var jmlsisa = parseInt(resp.content.jmldpb) - (  parseInt(resp.content.jmlada) +  parseInt(jml) );
					...
					parent.document.getElementById('id_pengadaan').value=idpengadaan;
