...
$fmFiltThnHapus = $_POST['fmFiltThnHapus']==''?$thn_login:$_POST['fmFiltThnHapus'];
...
if($mutasi=='1' && $Main->PENERIMAAN_P19==1){//tombol fungsi untuk daftar mutasi
	//$cbkriteria = cmb2D_v2('kriteriaM', $kriteriaM, $arrKriteria, '', 'Pilih Kriteria', '').
	$cbkriteria = "";
	$menubatal="<td>".PanelIcon1("javascript:Batal_Mutasi()","delete_f2.png","Batal")."</td>";
	$menuhapus="";
}elseif($mutasi=='2' && $Main->PENERIMAAN_P19==1){//tombol fungsi untuk daftar reclass
	$cbkriteria = "<input type=hidden name=mutasi id=mutasi value='2'>";
	$menubatal="<td>".PanelIcon1("javascript:Batal_Reclass()","delete_f2.png","Batal")."</td>";
	$menuhapus="";
}else{//tombol fungsi untuk daftar penggaungan dan double cacat
	$cbkriteria = "<input type=hidden name=mutasi id=mutasi value='$mutasi'>";
	$menubatal="";
	$menuhapus="<td>".PanelIcon1("javascript:Penghapusan_Hapus()","delete_f2.png","Batal")."</td>";
}
...
/* $thndefault =cmbQuery('fmFiltThnHapus',$fmFiltThnHapus,'select year(tgl_penghapusan)as thnhapus,year(tgl_penghapusan)as thnhapus from penghapusan group by thnhapus desc',
			'', 'Semua Thn. Hapus', ''); */
$thndefault ="<input type=text name='fmFiltThnHapus' id='fmFiltThnHapus' value='$fmFiltThnHapus' readonly size=4>";
$ListFilter = "
...
$script ="
	<script>
		...
		function Batal_Mutasi(){
			var errmsg = '';			
			if((errmsg=='') && (adminForm.boxchecked.value ==0 )){	errmsg= 'Data belum di pilih!';	}
			if (errmsg ==''){
				if(confirm('Yakin '+adminForm.boxchecked.value+' data akan di batalkan??')){
					adminForm.Act.value='BatalMutasi';
					adminForm.action='';
					adminForm.target='';
					adminForm.submit();
				}
		...
		function Batal_Reclass(){
			var errmsg = '';			
			if((errmsg=='') && (adminForm.boxchecked.value ==0 )){	errmsg= 'Data belum di pilih!';	}
			if (errmsg ==''){
				if(confirm('Yakin '+adminForm.boxchecked.value+' data akan di batalkan??')){
					adminForm.Act.value='BatalReclass';
					adminForm.action='';
					adminForm.target='';
					adminForm.submit();
				}
		...
		function Penghapusan_CetakAll(){
			adminForm.action='?Pg=PR&SPg=penghapusan_cetak&SSPg=$SSPg&ctk=$jmlDataHPS&mutasi=$mutasi';
			adminForm.target='_blank';
			adminForm.submit();
			adminForm.target='';		
		}
		function Penghapusan_exportXls(){
			adminForm.action='?Pg=PR&SPg=penghapusan_cetak&SSPg=$SSPg&ctk=1&mutasi=$mutasi&xls=1';
			adminForm.target='';
			adminForm.submit();
			adminForm.target='';
		}
	...
	";
	$menuedit="";
	//$menuedit="<td>".PanelIcon1("javascript:Penghapusan_Edit()","edit_f2.png","Ubah")."</td>";
...
//title -----------------------------------------------------------------------------
switch($mutasi){
	case '1' : $titlepage ='Mutasi Pindah Skpd'; break;
	case '2' : $titlepage ='Mutasi Reklas KIB'; break;
	case '4' : $titlepage ='Mutasi Penggabungan'; break;
	case '5' : $titlepage ='Mutasi (Double Catat) '; break;
