<?php
class footerObj  extends DaftarObj2{	
	var $Prefix = 'footer';
	var $elCurrPage="HalDefault";
	var $SHOW_CEK = TRUE;
	var $TblName = 'footer'; //daftar 
	var $TblName_Hapus = 'footer';
	var $MaxFlush = 10;
	...
	var $PageTitle = 'FOOTER';
	var $PageIcon = 'images/masterdata_ico.gif';
	var $pagePerHal ='';
	var $cetak_xls=TRUE ;
	var $fileNameExcel='usulansk.xls';
	var $Cetak_Judul = 'footer';	
	var $Cetak_Mode=2;
	...
	function setPage_HeaderOther(){
		...
		<A href=\"pages.php?Pg=footer\"   style='color : blue;' > FOOTER </a> |
	...
	function formURL(){	
	 ...
	 $get = mysql_fetch_array(mysql_query("select * from $this->TblName where id ='$footer_cb[0]'"));
	 ...
	 //items ----------------------
	  $this->form_fields = array(
		...
		//tombol
		$this->form_menubawah =
			"<input type='button' value='Simpan' onclick ='".$this->Prefix.".saveURL($footer_cb[0])' title='Simpan' >"."&nbsp&nbsp".
	...
	//get data -----------------
	 ...
	 if(empty($title)){
	 	$err = "Isi title";
	 }elseif(!empty($nomorUrut) && mysql_num_rows(mysql_query("select * from $this->TblName where nomor_urut = '$nomorUrut'  and id!='$idEdit'")) !=0){
	  ...
	  switch($tipe){	
		...
		case 'saveNewGrup':{				
			foreach ($_REQUEST as $key => $value) { 
			 ...
			 if(empty($namaGrup)){
			 	$err = "Isi nama grup ";
			 }elseif(mysql_num_rows(mysql_fetch_array("select * from grup where nama ='$namaGrup'")) !=0 ){
			 	$err = "Nama grup sudah ada ";
			 }else{
			 	$data = array("nama"=> $namaGrup);
				$query = VulnWalkerInsert("grup",$data);
				mysql_query($query);
				$getMaxId = mysql_fetch_array(mysql_query("select max(id) from grup"));
				$comboGrup = cmbQuery('cmbGrup',$getMaxId['max(id)'],"select id, nama from grup","",'-- Pilih Grup --');
		...
		case 'saveEditGrup':{				
			foreach ($_REQUEST as $key => $value) { 
			 ...
			 if(empty($namaGrup)){
			 	$err = "Isi nama grup ";
			 }elseif(mysql_num_rows(mysql_fetch_array("select * from grup where nama ='$namaGrup' and id !='$id'")) !=0 ){
			 	$err = "Nama grup sudah ada ";
			 }else{
			 	$data = array("nama"=> $namaGrup);
				$query = VulnWalkerUpdate("grup",$data,"id='$id'");
				mysql_query($query);
				$comboGrup = cmbQuery('cmbGrup',$id,"select id, nama from grup","",'-- Pilih Grup --');
		...
		case 'levelChanged':{				
		 ...
		 $getDataUpline = mysql_query("select * from footer where level = '$levelUpline'");
	...
	function setPage_OtherScript(){
		...
		return 
			...
			"<script src='js/jquery-ui.custom.js'></script>".
			 "<script type='text/javascript' src='js/footer/footer.js' language='JavaScript' ></script>".
	...
	function setForm($dt){	
	 ...
	 $tgl_update = date('d-m-Y');	
	  if ($this->form_fmST==0) {
		$this->form_caption = 'FOOTER - Baru';
		$grup = $_REQUEST['filterGrup'];
		
		$status = "1";
		
	  }else{
		$this->form_caption = 'FOOTER - Edit';			
	    ...
	    $comboGrup = cmbQuery('cmbGrup',$grup,"select id, nama from grup","",'-- Pilih Grup --');
		$arrayRow = array();
		$arrayKolom = array();
		$getMaxPosisi = mysql_fetch_array(mysql_query("select * from setting_kolom"));
		for($i = 1;$i <= $getMaxPosisi['row'];$i++ ){
			$arrayRow[] = array($i,$i);
		}
		
		for($a = 1;$a <= $getMaxPosisi['kolom'];$a++ ){
	...
	function Gambar($id){	
	 ...
	 $this->form_height = 250;
		...
		$this->form_caption = 'footer - GAMBAR';
	 ...
	 $headerTable =
	  "<thead>
	   <tr>
  	   ...
  	   $Checkbox		
	   <th class='th01' width='50' align='center'>NOMOR URUT</th>
	...
	function setKolomData($no, $isi, $Mode, $TampilCheckBox){
	 ...
	 $Koloms[] = array('align="right"',$nomor_urut);

	 $getNama = mysql_fetch_array(mysql_query("select * from system where id='$id_system'"));
	 $nama = $getNama['nama'];
	 $Koloms[] = array('align="left"',$margin.$title);
	  $Koloms[] = array('align="left"',$margin.$hint);
	  $Koloms[] = array('align="left"',$margin.$url);
	 if($status == "1"){
	 	$status = "AKTIF";
	 }else{
	 	$status = "TIDAK AKTIF";
	...
	function genDaftarOpsi(){
	 ...
	 $queryCmbGrup = "select id, nama from grup ";
	 $comboGrup = cmbQuery('filterGrup',$filterGrup,$queryCmbGrup," onchange =$this->Prefix.refreshList(true); style='width:400px;'",'-- Semua Grup --');
	 ...
	 //items ----------------------
	  $this->form_fields = array(
			...
			'asd' => array( 
						'label'=>'NAMA GRUP',
						'labelWidth'=>100, 
						'value'=>"<input type = 'text' name='namaGrup' id='namaGrup' style='width:250px;' value=''>", 
						 ),
	 ...
	 //items ----------------------
	  $this->form_fields = array(
			...
			'asd' => array( 
						'label'=>'NAMA GRUP',
						'labelWidth'=>100, 
						'value'=>"<input type = 'text' name='namaGrup' id='namaGrup' style='width:250px;' value='$nama'>", 
						 ),
			);
		//tombol
		$this->form_menubawah =
