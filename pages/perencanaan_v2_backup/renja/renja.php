...
class renja_v2Obj  extends DaftarObj2{	
	var $Prefix = 'renja_v2';
	var $elCurrPage="HalDefault";
	var $SHOW_CEK = TRUE;
	var $TblName = 'view_renja'; //bonus
	var $TblName_Hapus = 'renja_v2';
	var $MaxFlush = 10;
	...
	var $pagePerHal ='';
	var $fileNameExcel='renja_v2.xls';
	var $namaModulCetak='DAFTAR renja_v2';
	var $Cetak_Judul = 'DAFTAR renja_v2';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'renja_v2Form';
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){	
		...
		echo "
								...
								<table table width='100%' class='cetak' border='1' style='margin:4 0 0 0;width:100%;'>
									<tr>
										...
										<th class='th01' >UNIT KERJA PELAKSANA</th>
		...
		while($daqry = mysql_fetch_array($aqry)){
		 ...
		 if($q!='0' && $e1 !='000'){
		 	$unitPelaksana = $getUnitPelaksana['nm_skpd'];
		 }else{
		 	$unitPelaksana = "";
		 }
			...
			echo "
								<tr valign='top'>
									...
									<td align='left' class='GarisCetak' >".$unitPelaksana."</td>
								</tr>
			";
		...
		case 'newTab':{
			 ...
			 foreach ($_REQUEST as $key => $value) { 
				...
				if($cekKeberadaanMangkluk != 0){
					...
					if($getDataPlafon['jenis_form_modul']  == 'PENYUSUNAN' && $this->wajibValidasi == TRUE ){
				...
				$content = array('idrenja_v2' => $idPlafon, "status" => $statunya );
			break;
		    }
			...
			case 'editTab':{
			 ...
			 $urutArray = explode(" ",$renja_v2_cb[0]);
			 $urut = $renja_v2_cb[0];
			 $query = "select * from view_renja where id_anggaran = '$renja_v2_cb[0]' ";
			 $getViewrenja_v2 = mysql_fetch_array(mysql_query($query)); 
			 foreach ($getViewrenja_v2 as $key => $value) { 
				  $$key = $value; 
			 }
			 $IDEDIT = $id_anggaran;
			 ...
			 $content = array('ID_PLAFON' => $ID_PLAFON,  'ID_EDIT' => $renja_v2_cb[0] , 'query ' =>"select * from view_renja where jenis_anggaran = '$this->jenisAnggaran' and tahun = '$this->tahun' and c1 = '$c1' and c = '$c' and d='$d' and e='00' and e1='000' and bk='0' and ck = '0'  and p ='0' and q = '0' and no_urut = '$nomor' ");
		...
		case 'SaveValid':{
	    	foreach ($_REQUEST as $key => $value) { 
			 ...
			 $getSKPD = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$renja_v2_idplh'"));
			 ...
			 $query = VulnWalkerUpdate("tabel_anggaran",$data," id_anggaran = '$renja_v2_idplh'");
			 mysql_query($query);
   ...
   if($this->jenisForm == "KOREKSI"){
		...
		return 	
			...
			<script type='text/javascript' src='js/perencanaan_v2/renja/renja.js' language='JavaScript' ></script>
			<script type='text/javascript' src='js/perencanaan_v2/renja/renja_ins.js' language='JavaScript' ></script>
	...
	function setFormBaru(){
  	...
  	function setFormEdit(){
		...
		if($cnt['cnt'] > 0) $err = "renja_v2 Tidak Bisa Diubah ! Sudah Digunakan Di Ref Barang.";
		if($err == ''){
	...
	function Info(){	
	 ...
	 $angkaPlafon = number_format($getAngkaPlafon['plafon'],2,',','.');
	 $getAngkaRenja = mysql_fetch_array(mysql_query("select sum(jumlah) from view_renja where c1='$selectedC1' and c='$selectedC' and d='$selectedD' and tahun = '$this->tahun' and jenis_anggaran = '$this->jenisAnggaran'"));
	 $angkaRenja = number_format($getAngkaRenja['sum(jumlah)'],2,',','.');
	 $sisaPlafon = $getAngkaPlafon['plafon'] - $getAngkaRenja['sum(jumlah)'];
	 $sisaPlafon = number_format($sisaPlafon,2,',','.');
	 $codeAndNameUrusan = "select c1, concat(c1, '. ', nm_skpd) from ref_skpd where c='00' and d='00' and e='00' and e1='000' ";
	 $urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=renja_v2.comboChanged(); disabled','-- URUSAN --');
	
	 $codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
	 $bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=renja_v2.comboChanged(); disabled','-- BIDANG --');
	
	 $codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
	 $skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=renja_v2.comboChanged(); disabled' ,'-- SKPD --');
	 ...
	 //items ----------------------
	  $this->form_fields = array(
			...
			'9' => array( 
						'label'=>'PLAFON',
						'labelWidth'=>150, 
						'value'=> "<span id='angkaPlafon' style='color:red;' >Rp. <input type='text' value = '$angkaPlafon' style='width:150;text-align:right;color:red;border:0;background: transparent;' readonly></span>",
						 ),	
			'10' => array( 
						'label'=>'TERPAKAI',
						'labelWidth'=>150, 
						'value'=> "<span id='terpakai' style='color:red;' >Rp. <input type='text' value = '$angkaRenja' style='width:150;text-align:right;color:red;border:0;background: transparent;' readonly></span>",
						 ),	
			'11' => array( 
						'label'=>'SISA',
						'labelWidth'=>150, 
						'value'=> "<span id='sisa' style='color:red;' >Rp. <input type='text' value = '$sisaPlafon' style='width:150;text-align:right;color:red;border:0;background: transparent;' readonly></span>",
						 ),	
	...
	function setKolomData($no, $isi, $Mode, $TampilCheckBox){
	 ...
	 if($this->jenisForm == "PENYUSUNAN"){
		 ...
		 if ($status_validasi =='1') {
			 ...
			 $aksi  = "<img src='images/administrator/images/valid.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=renja_v2.sesuai('$id_anggaran');></img>&nbsp  &nbsp <img src='images/administrator/images/invalid.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=renja_v2.koreksi('$id_anggaran');></img>&nbsp &nbsp <img src='images/administrator/images/edit_f2.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=renja_v2.catatan('$id_anggaran');></img> ";
			 if ($q == '00') {
			 $aksi = "";
			 }
	...
	$fmBIDANG = cekPOST('fmBIDANG');
	 ...
	 $arr = array(
			...
			array('nama_renja_v2','NAMA renja_v2'),		
			array('alamat','ALAMAT'),	
	...
	function getDaftarOpsi($Mode=1){
	   ...
	   if(!isset($ref_skpdSkpdfmUrusan) ){
		...
		while($rows = mysql_fetch_array($execute)){
			...
			$p = $rows['p'];
			$q = $rows['q'];
			$getUrusan = mysql_num_rows(mysql_query("select * from view_renja where c1 = '$c1' and c !='00' and d !='00'  and p!='0' and q !='0' and tahun = '$this->tahun' and jenis_anggaran = '$this->jenisAnggaran'"));
			if($getUrusan > 0){
				$queryGetBidang = "select * from view_renja where c1='$c1' and c = '$c'  and d != '00'  and p!='0' and q !='0' and tahun = '$this->tahun' and jenis_anggaran = '$this->jenisAnggaran'";
				$getBidang = mysql_num_rows(mysql_query($queryGetBidang));
				if($getBidang > 0){
					$queryGetSKPD = "select * from view_renja where c1='$c1' and c = '$c'  and d = '$d' and p!='0' and q !='0' and tahun = '$this->tahun' and jenis_anggaran = '$this->jenisAnggaran'";
					$getSKPD = mysql_num_rows(mysql_query($queryGetSKPD));
					if($getSKPD > 0){
						$queryGetUNIT = "select * from view_renja where c1='$c1' and c = '$c'  and d = '$d' and e = '$e' and e1 !='000' and p!='0' and q !='0' and tahun = '$this->tahun' and jenis_anggaran = '$this->jenisAnggaran'";
						$getUNIT = mysql_num_rows(mysql_query($queryGetUNIT));
...
$renja_v2 = new renja_v2Obj();

$arrayResult = tahapPerencanaanV3($renja_v2->modul,"MURNI");
...
$renja_v2->jenisForm = $jenisForm;
$renja_v2->nomorUrut = $nomorUrut;
$renja_v2->tahun = $tahun;
$renja_v2->jenisAnggaran = $jenisAnggaran;
$renja_v2->idTahap = $idTahap;
$renja_v2->username = $_COOKIE['coID'];
$renja_v2->wajibValidasi = $arrayResult['wajib_validasi'];
$renja_v2->settingAnggaran = $arrayResult['settingAnggaran'];
