<?php
class formRkaPpkd21_v2Obj  extends DaftarObj2{	
	var $Prefix = 'formRkaPpkd21_v2';
	...
	var $FormName = 'formRkaPpkd21_v2Form';
	var $noModul=14; 
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){	
		...
		case  'SaveAlokasi':{
			...
			$query = VulnWalkerInsert('temp_alokasi_rka_ppkd_21_v2',$data);
		...
		case 'clearAlokasi':{				
			$username = $_COOKIE['coID'];
			mysql_query("delete from temp_alokasi_rka_ppkd_21_v2 where user = '$username'");	
			$content = "delete from temp_alokasi_rka_ppkd_21_v2 where user = '$username'";										
		break;
		}
		...
		case 'finish':{		
			...
			}elseif(mysql_num_rows(mysql_query("select * from temp_rka_ppkd_21  where user ='$username'  and delete_status = '0'")) == 0){
				$err = "Data kosong";
			}
			
			if(empty($err)){
				$execute = mysql_query("select * from temp_rka_ppkd_21  where user ='$username' ");
				while($rows = mysql_fetch_array($execute)){
					foreach ($rows as $key => $value) { 
					  $$key = $value; 
					}	
					$queryCekRekening = "select * from view_rka_ppkd_2_1 where c1='0' and f = '00' and rincian_perhitungan = '' and k='$k' and l='$l' and m='$m' and n='$n' and o='$o' and id_tahap='$this->idTahap' ";
					...
					if(mysql_num_rows(mysql_query($queryCekForUpdate)) > 0){
						...
						if(mysql_num_rows(mysql_query("select * from view_rka_ppkd_2_1 where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and k='$lamaK' and l='$lamaL' and m='$lamaM' and n='$lamaN' and o='$lamaO'  and (rincian_perhitungan !='' or f!='00') and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and id_tahap='$this->idTahap'")) == 0){
		...
		case 'edit':{
				...
				mysql_query(VulnWalkerInsert('temp_alokasi_rka_ppkd_21_v2',$dataAlokasi));
				/*if($satuan_total == ''){
					$statusAlokasi = 'false';
				}else{*/
					$statusAlokasi = 'true';
   ...
   function setPage_OtherScript(){
		...
		return
			...
			"<script type='text/javascript' src='js/perencanaan_v2/rka/formRkaPpkd2.1.js' language='JavaScript' ></script>".
			...
			genFilterBar(
				...
				genFilterBar(
					...
					<td class='border:none'> 
						<a class='toolbar' id='btsave' href='javascript:formRkaPpkd21_v2.$tujuan'> 
					...
					<tbody><tr valign='middle' align='center'> 
					<td class='border:none'> 
						<a class='toolbar' id='btcancel' href='javascript:formRkaPpkd21_v2.closeTab()'> 
					...
					<tbody><tr valign='middle' align='center'> 
					<td class='border:none'> 
						<a class='toolbar' id='finish' href='javascript:formRkaPpkd21_v2.finish()'> 
...
$formRkaPpkd21_v2 = new formRkaPpkd21_v2Obj();
...
$formRkaPpkd21_v2->jenisForm = $jenisForm;
$formRkaPpkd21_v2->nomorUrut = $nomorUrut;
$formRkaPpkd21_v2->tahun = $tahun;
$formRkaPpkd21_v2->jenisAnggaran = $jenisAnggaran;
$formRkaPpkd21_v2->idTahap = $idTahap;
