...
class rkaSKPD_v2_koreksiObj  extends DaftarObj2{	
	var $Prefix = 'rkaSKPD_v2_koreksi';
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='rkaSKPD_v2_koreksi.xls';
	var $namaModulCetak='RKA';
	var $Cetak_Judul = 'RKA';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'rkaSKPD_v2_koreksiForm';
	var $modul = "RKA-SKPD";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	...
	function setTitle(){
		return 'RKA-SKPD  '.$this->jenisAnggaran.' TAHUN '.$this->tahun."&nbsp ".  strtoupper($this->namaTahap);
	}
	...
	$getJumlahBarang = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$rkaSKPD_v2_koreksi_idplh'"));
	...
	$sisaPaguIndikatif = $paguIndikatif - $getPaguYangTelahTerpakai['paguYangTerpakai'];
	 ...
	 if(mysql_num_rows(mysql_query("select * from view_rka where c1='0' and f1 = '0' and k = '$k' and l ='$l' and m='$m' and n='$n' and o='$o'  and id_tahap='$this->idTahap' ")) > 0){
 	 ...
 	 if(empty($cmbJenisRKAForm) ){
		...
		$query = VulnWalkerUpdate('tabel_anggaran',$data,"id_anggaran = '$rkaSKPD_v2_koreksi_idplh'");
		mysql_query($query);
	 }
	...
	<A href=\"pages.php?Pg=rka-skpd_v2_koreksi\" title='RKA-SKPKD MURNI'  style='color:blue;' > RKA-SKPD  </a> |
   ...
   function setPage_OtherScript(){
		...
		return 	
			...
			<script type='text/javascript' src='js/perencanaan_v2/rka/rkaSKPD_koreksi.js' language='JavaScript' ></script> 
	 ...
	 $this->form_height = 400;
	  if ($this->form_fmST==0) {
		...
		foreach ($dt as $key => $value) { 
		 ...
		 $jenis_rka = $jenis_rka;
		 $cmbJenisRKA = cmbArray('cmbJenisRKAForm',$jenis_rka,$arrayJenisRKA,'-- JENIS RKA --','onchange=rkaSKPD_v2_koreksi.unlockFindRekening();');
	 	 if(empty($jenis_rka)){
	  ...
	  $this->form_fields = array(
			...
			'kode7' => array( 
						...
						'value'=> "<input type='text' id='kodeRekening' name='kodeRekening' style='width:120px;' readonly value = '$kodeRekening'> &nbsp&nbsp
						 ...
						 <button type='button' id='findRekening' onclick=rkaSKPD_v2_koreksi.findRekening('$jenis_rka'); $tergantungJenis> CARI </button> "
						 ),
			'kode8' => array( 
						'label'=>'HARGA SATUAN',
						'labelWidth'=>150, 
						'value'=> "<input type='text' name='hargaSatuan' id='hargaSatuan' value='$hargaSatuan' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='rkaSKPD_v2_koreksi.bantu();' > <span id='bantu' style='color:red;'> </span>"
						 ),	
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	foreach ($_COOKIE as $key => $value) { 
		...
		$urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=rkaSKPD_v2_koreksi.refreshList(true);','-- URUSAN --');
		
		$codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
		$bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=rkaSKPD_v2_koreksi.refreshList(true);','-- BIDANG --');
		
		$codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
		$skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=rkaSKPD_v2_koreksi.refreshList(true);','-- SKPD --');
		...
		$unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=rkaSKPD_v2_koreksi.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=rkaSKPD_v2_koreksi.refreshList(true);','-- SUB UNIT --');
			...
			while($rows = mysql_fetch_array($getAllRekeningFromRka)){
				...
				if(strlen($k) > 1){
					...
					$data = array(
					  ...
					  if(mysql_num_rows(mysql_query("select * from view_rka where k='$k' and l='$l' and m='$m' and n='$n' and o ='$o'")) == 0){
...
$rkaSKPD_v2_koreksi = new rkaSKPD_v2_koreksiObj();
...
$rkaSKPD_v2_koreksi->jenisForm = $jenisForm;
$rkaSKPD_v2_koreksi->nomorUrut = $nomorUrut;
$rkaSKPD_v2_koreksi->tahun = $tahun;
$rkaSKPD_v2_koreksi->jenisAnggaran = $jenisAnggaran;
$rkaSKPD_v2_koreksi->idTahap = $idTahap;
$getNamaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$idTahap'"));
$rkaSKPD_v2_koreksi->namaTahap = $getNamaTahap['nama_tahap'];

$rkaSKPD_v2_koreksi->username = $_COOKIE['coID'];

$rkaSKPD_v2_koreksi->wajibValidasi = $Main->wajibValidasi;
if($Main->wajibValidasi == TRUE){
	$rkaSKPD_v2_koreksi->sqlValidasi = " and status_validasi ='1' ";
}else{
	$rkaSKPD_v2_koreksi->sqlValidasi = " ";
}

if(empty($rkaSKPD_v2_koreksi->tahun)){
	...
	/*$rkaSKPD_v2_koreksi->tahun = "select max(id_anggaran) as max from view_rka where nama_modul = 'rkaSKPD_v2_koreksi'";*/
	$rkaSKPD_v2_koreksi->tahun  = $get2['tahun'];
	$rkaSKPD_v2_koreksi->jenisAnggaran = $get2['jenis_anggaran'];
	$rkaSKPD_v2_koreksi->urutTerakhir = $get2['no_urut'];
	$rkaSKPD_v2_koreksi->jenisFormTerakhir = $get2['jenis_form_modul'];
	$rkaSKPD_v2_koreksi->urutSebelumnya = $rkaSKPD_v2_koreksi->urutTerakhir - 1;
	...
	$rkaSKPD_v2_koreksi->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$rkaSKPD_v2_koreksi->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];
	
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$rkaSKPD_v2_koreksi->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$rkaSKPD_v2_koreksi->idTahap'"));
	$rkaSKPD_v2_koreksi->currentTahap = $getCurrenttahap['nama_tahap'];
	
	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$rkaSKPD_v2_koreksi->idTahap'"));
	$rkaSKPD_v2_koreksi->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$rkaSKPD_v2_koreksi->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$rkaSKPD_v2_koreksi->provinsi = $setting['provinsi'];
$rkaSKPD_v2_koreksi->kota = $setting['kota'];
$rkaSKPD_v2_koreksi->pengelolaBarang = $setting['pengelolaBarang'];
$rkaSKPD_v2_koreksi->pejabatPengelolaBarang = $setting['pejabat'];
$rkaSKPD_v2_koreksi->pengurusPengelolaBarang = $setting['pengurus'];
$rkaSKPD_v2_koreksi->nipPengelola = $setting['nipPengelola'];
$rkaSKPD_v2_koreksi->nipPengurus = $setting['nipPengurus'];
$rkaSKPD_v2_koreksi->nipPejabat = $setting['nipPejabat'];


if($rkaSKPD_v2_koreksi->jenisForm != "KOREKSI"){
$get1 = mysql_fetch_array(mysql_query("select max(id_anggaran)  from view_rka_2_2_1 where jenis_form_modul = 'KOREKSI' "));
	...
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$rkaSKPD_v2_koreksi->currentTahap = $arrayHasil['currentTahap'];
			$rkaSKPD_v2_koreksi->jenisForm = "";
			$rkaSKPD_v2_koreksi->jenisFormTerakhir = "KOREKSI";
			$rkaSKPD_v2_koreksi->idTahap = $get2['id_tahap'];
			$rkaSKPD_v2_koreksi->nomorUrut = $get2['no_urut'];
			$rkaSKPD_v2_koreksi->urutTerakhir = $get2['no_urut'];
