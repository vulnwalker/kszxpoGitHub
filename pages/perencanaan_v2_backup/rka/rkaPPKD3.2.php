...
class rkaPPKD32_v2Obj  extends DaftarObj2{	
	var $Prefix = 'rkaPPKD32_v2';
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='rkaPPKD32_v2.xls';
	var $namaModulCetak='RKA';
	var $Cetak_Judul = 'RKA';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'rkaPPKD32_v2Form';
	var $modul = "RKA-PPKD";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	...
	$getJumlahBarang = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$rkaPPKD32_v2_idplh'"));
	...
	$sisaPaguIndikatif = $paguIndikatif - $getPaguYangTelahTerpakai['paguYangTerpakai'];
	 ...
	 if(mysql_num_rows(mysql_query("select * from view_rka_ppkd_3_2 where c1='0' and f = '00' and k = '$k' and l ='$l' and m='$m' and n='$n' and o='$o'  and id_tahap='$this->idTahap' ")) > 0){
 	 ...
 	 if(empty($cmbJenisRKAForm) ){
		...
		$query = VulnWalkerUpdate('tabel_anggaran',$data,"id_anggaran = '$rkaPPKD32_v2_idplh'");
		mysql_query($query);
	 }
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){	
		...
		case 'Gruping':{	
			...
			if(!isset($rkaPPKD32_v2_cb)){
				$err = "Pilih Data";
			}
			$dt = implode(',',$rkaPPKD32_v2_cb);
		...
		case 'sesuai':{
			...
			$getrkaPPKD32_v2nya = mysql_fetch_array(mysql_query($queryRows));
			foreach ($getrkaPPKD32_v2nya as $key => $value) { 
				  $$key = $value; 
			} 
			$cmbUrusanForm = $c1;
			$cmbBidangForm = $c;
	...
	<A href=\"pages.php?Pg=rka-ppkd-1_v2\" title='RKA-PPKD MURNI'  > RKA-PPKD 1 </a> |
	...
	<A href=\"pages.php?Pg=rka-ppkd-3.2_v2\" title='RKA-PPKD MURNI' style='color:blue;'  > RKA-PPKD 3.2 </a> |
   ...
   function setPage_OtherScript(){
		...
		return 	
			...
			<script type='text/javascript' src='js/perencanaan_v2/rka/rkaPPKD3.2.js' language='JavaScript' ></script> 
	 ...
	 $this->form_height = 400;
	  if ($this->form_fmST==0) {
		...
		foreach ($dt as $key => $value) { 
		 ...
		 $jenis_rka = $jenis_rka;
		 $cmbJenisRKA = cmbArray('cmbJenisRKAForm',$jenis_rka,$arrayJenisRKA,'-- JENIS RKA --','onchange=rkaPPKD32_v2.unlockFindRekening();');
	 	 if(empty($jenis_rka)){
	  ...
	  $this->form_fields = array(
			...
			'kode7' => array( 
						...
						'value'=> "<input type='text' id='kodeRekening' name='kodeRekening' style='width:120px;' readonly value = '$kodeRekening'> &nbsp&nbsp
						 ...
						 <button type='button' id='findRekening' onclick=rkaPPKD32_v2.findRekening('$jenis_rka'); $tergantungJenis> CARI </button> "
						 ),
			'kode8' => array( 
						'label'=>'HARGA SATUAN',
						'labelWidth'=>150, 
						'value'=> "<input type='text' name='hargaSatuan' id='hargaSatuan' value='$hargaSatuan' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='rkaPPKD32_v2.bantu();' > <span id='bantu' style='color:red;'> </span>"
						 ),	
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	foreach ($_COOKIE as $key => $value) { 
		...
		$urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=rkaPPKD32_v2.refreshList(true);','-- URUSAN --');
		
		$codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
		$bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=rkaPPKD32_v2.refreshList(true);','-- BIDANG --');
		
		$codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
		$skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=rkaPPKD32_v2.refreshList(true);','-- SKPD --');
		...
		$unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=rkaPPKD32_v2.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=rkaPPKD32_v2.refreshList(true);','-- SUB UNIT --');
...
$rkaPPKD32_v2 = new rkaPPKD32_v2Obj();
...
$rkaPPKD32_v2->jenisForm = $jenisForm;
$rkaPPKD32_v2->nomorUrut = $nomorUrut;
$rkaPPKD32_v2->urutTerakhir = $nomorUrut;
$rkaPPKD32_v2->tahun = $tahun;
$rkaPPKD32_v2->jenisAnggaran = $jenisAnggaran;
$rkaPPKD32_v2->idTahap = $idTahap;

$rkaPPKD32_v2->wajibValidasi = $Main->wajibValidasi;
if($Main->wajibValidasi == TRUE){
	$rkaPPKD32_v2->sqlValidasi = " and status_validasi ='1' ";
}else{
	$rkaPPKD32_v2->sqlValidasi = " ";
}

if(empty($rkaPPKD32_v2->tahun)){
	...
	/*$rkaPPKD32_v2->tahun = "select max(id_anggaran) as max from view_rka_ppkd_3_2 where nama_modul = 'rkaPPKD32_v2'";*/
	$rkaPPKD32_v2->tahun  = $get2['tahun'];
	$rkaPPKD32_v2->jenisAnggaran = $get2['jenis_anggaran'];
	$rkaPPKD32_v2->urutTerakhir = $get2['no_urut'];
	$rkaPPKD32_v2->jenisFormTerakhir = $get2['jenis_form_modul'];
	$rkaPPKD32_v2->urutSebelumnya = $rkaPPKD32_v2->urutTerakhir - 1;
	...
	$rkaPPKD32_v2->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$rkaPPKD32_v2->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];
	
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$rkaPPKD32_v2->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$rkaPPKD32_v2->idTahap'"));
	$rkaPPKD32_v2->currentTahap = $getCurrenttahap['nama_tahap'];
	
	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$rkaPPKD32_v2->idTahap'"));
	$rkaPPKD32_v2->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$rkaPPKD32_v2->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$rkaPPKD32_v2->provinsi = $setting['provinsi'];
$rkaPPKD32_v2->kota = $setting['kota'];
$rkaPPKD32_v2->pengelolaBarang = $setting['pengelolaBarang'];
$rkaPPKD32_v2->pejabatPengelolaBarang = $setting['pejabat'];
$rkaPPKD32_v2->pengurusPengelolaBarang = $setting['pengurus'];
$rkaPPKD32_v2->nipPengelola = $setting['nipPengelola'];
$rkaPPKD32_v2->nipPengurus = $setting['nipPengurus'];
$rkaPPKD32_v2->nipPejabat = $setting['nipPejabat'];
$rkaPPKD32_v2->username = $_COOKIE['coID'];

if($rkaPPKD32_v2->jenisForm != "PENYUSUNAN"){
$get1 = mysql_fetch_array(mysql_query("select max(id_anggaran)  from view_rka_ppkd_3_2 where jenis_form_modul = 'PENYUSUNAN' "));
	$maxAnggaran = $get1['max(id_anggaran)'];
	$get2 = mysql_fetch_array(mysql_query("select * from view_rka_ppkd_3_2 where id_anggaran = '$maxAnggaran'"));
	$rkaPPKD32_v2->tahun  = $get2['tahun'];
	$rkaPPKD32_v2->jenisAnggaran = $get2['jenis_anggaran'];
	...
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$rkaPPKD32_v2->currentTahap = $arrayHasil['currentTahap'];
			$rkaPPKD32_v2->jenisForm = "";
			$rkaPPKD32_v2->jenisFormTerakhir = "PENYUSUNAN";
			$rkaPPKD32_v2->idTahap = $get2['id_tahap'];
			$rkaPPKD32_v2->nomorUrut = $get2['no_urut'];
			$rkaPPKD32_v2->urutTerakhir = $get2['no_urut'];
}
