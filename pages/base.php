...
$SPg = $_REQUEST['SPg'];
if($Main->MENU_VERSI==3){
	if ($Pg=='05' && $SPg=='belumsensus'){
		$Pg_ = "sensus";
	}
	else if(($Pg=='05' && $jns!='' && $jns!='penyusutan' ) || ($Pg=='09' && $mutasi<>'') || $Pg=='Rekap1' || $Pg=='Rekap2' || $Pg=='Rekap3' || $Pg=='pemusnahan')
	{
		$Pg_ = "pelaporan";
	
	}else if($Pg=='05' && $jns=='penyusutan'){
		$Pg_ = "penyusutan";
...
$icon = "images/". $Main->Modul[$Pg_][2];
$title = trim($Main->Modul[$Pg_][1]);

//ambil title dan icon dari menu by url
$menu= $Menu->getData();
if($menu != NULL){
	 $title=$menu['page_title'];
	$icon =$menu['page_icon'];
}
