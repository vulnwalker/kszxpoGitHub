...
class popupShortcutObj  extends DaftarObj2{
	var $Prefix = 'popupShortcut';
	var $elCurrPage="HalDefault";
	var $SHOW_CEK = TRUE;
	var $TblName = 'grup'; //daftar
	var $TblName_Hapus = 'grup';
	var $MaxFlush = 10;
	...
	var $FormName = 'popupShortcutForm';
	var $WHERE_O = "";
	...
	//urutan kode skpd
		if($j==0){
			$ck=mysql_fetch_array(mysql_query("Select * from grup where k!='0' and l ='0' and m ='0' and n ='00' and o ='00' Order By k DESC limit 1"));
			if($kode1=='0') {$err= 'Format Kode Akun salah';}
			elseif($kode1!=5){ $err= 'Format Kode Akun salah';}

		}elseif($j==1){
			$ck=mysql_fetch_array(mysql_query("Select * from grup where k='".$kode1."' and l !='0' and m ='0' and n ='00' and o ='00' Order By l DESC limit 1"));
			if ($kode2>sprintf("%02s",$ck['l']+1)) {$err= 'Format Kode Kelompok Belanja Harus berurutan';}

		}elseif($j==2){
			$ck=mysql_fetch_array(mysql_query("Select * from grup where k='".$kode1."' and l ='".$kode2."' and m !='0' and n ='00' and o ='00' Order By m DESC limit 1"));
			if ($kode3>sprintf("%02s",$ck['m']+1)) {$err= 'Format Kode Jenis Belanja Salah';}

		}elseif($j==3){
			$ck=mysql_fetch_array(mysql_query("Select * from grup where k='".$kode1."' and l ='".$kode2."' and m ='".$kode3."' and n!='00' and o='$this->WHERE_O' Order By n DESC limit 1"));
			if ($kode4>sprintf("%02s",$ck['n']+1)) {$err= 'Format Kode Objek Belanja Harus berurutan';}

		}elseif($j==4){
			$ck=mysql_fetch_array(mysql_query("Select * from grup where k='".$kode1."' and l ='".$kode2."' and m ='".$kode3."' and n ='".$kode4."' and o!='00' Order By o DESC limit 1"));
			...
			if($fmST == 0){
			$ck1=mysql_fetch_array(mysql_query("Select * from grup where k='$kode1' and l ='$kode2' and m ='$kode3' and n='$kode4' and o='$kode5'"));
			if ($ck1>=1)$err= 'Gagal Simpan'.mysql_error();
				if($err==''){
					$aqry = "INSERT into grup (k,l,m,n,o,nm_rekening) values('$kode1','$kode2','$kode3','$kode4','$kode5','$nama_rekening')";	$cek .= $aqry;
					$qry = mysql_query($aqry);
				}
			}else{
				if($err==''){
				$aqry = "UPDATE grup SET nm_rekening='$nama_rekening' WHERE k='$kode1' and l='$kode2' and m='$kode3' and n='$kode4' and o='$kode5'";	$cek .= $aqry;
	...
	function setFormBaru(){
  	...
  	function setFormEdit(){
		...
		$this->form_fmST = 1;
		//get data
		$aqry = "SELECT * FROM  grup WHERE k='$k' and l='$l' and m='$m' and n='$n' and o='$o' "; $cek.=$aqry;
	...
	$NomorColSpan = $Mode==1? 2: 1;
	 $headerTable =
	  "<thead>
	   ...
	   <th class='th01' width='200' align='center'>HEIGHT</th>
	   <th class='th01' width='200' align='center'>ROW</th>
	   <th class='th01' width='200' align='center'>COLOM</th>
	   <th class='th01' width='50' align='center'>CONTENT BACKGROUND</th>
	...
	function setKolomData($no, $isi, $Mode, $TampilCheckBox){
	 ...
	 $Koloms[] = array('align="left"',"<span style='color:red;cursor:pointer;' onclick=$this->Prefix.windowSave($id)>$title</span>");

	 $Koloms[] = array('align="right"',$height);
	 $Koloms[] = array('align="right"',$max_row);
	 $Koloms[] = array('align="right"',$max_kolom);
	 $Koloms[] = array('align="left"',"<span style='color:$content_background;'>COLOR</span>");
	...
	function genDaftarOpsi(){
	 ...
	 foreach ($_REQUEST as $key => $value) {
	  ...
	  $queryCmbSystem = "select id_grup, nama from $this->TblName where id_grup != '0' and id_modul = '0' and id_sub_modul = '0' and id_sub_sub_modul = '0' and id_sub_sub_sub_modul = '0'";
	  $comboSystem = cmbQuery('filterSystem',$filterSystem,$queryCmbSystem,"' onchange =$this->Prefix.refreshList(true); ",'-- Semua System --');
	  $queryCmbModul = "select id_modul, nama from $this->TblName where id_grup = '$filterSystem' and id_modul != '0' and id_sub_modul = '0' and id_sub_sub_modul = '0' and id_sub_sub_sub_modul = '0'";
	  $comboModul = cmbQuery('filterModul',$filterModul,$queryCmbModul,"' onchange=$this->Prefix.refreshList(true);",'-- Semua Modul --');

	  $querySubModul = "select id_sub_modul, nama from $this->TblName where id_grup = '$filterSystem' and id_modul = '$filterModul' and id_sub_modul != '0' and id_sub_sub_modul = '0' and id_sub_sub_sub_modul = '0'";
	  $comboSubModul = cmbQuery('filterSubModul',$filterSubModul,$querySubModul,"' onchange=$this->Prefix.refreshList(true);",'-- Semua Sub Modul --');

	  $querySubSubModul = "select id_sub_sub_modul, nama from $this->TblName where id_grup = '$filterSystem' and id_modul = '$filterModul' and id_sub_modul = '$filterSubModul' and id_sub_sub_modul != '0' and id_sub_sub_sub_modul = '0'";
	  ...
	  $querySubSubSubModul = "select id_sub_sub_sub_modul, nama from $this->TblName where id_grup = '$filterSystem' and id_modul = '$filterModul' and id_sub_modul = '$filterSubModul' and id_sub_sub_modul = '$filterSubSubModul' and id_sub_sub_sub_modul != '0'";
	...
	function getDaftarOpsi($Mode=1){
		...
		$Asc1 = $fmDESC1 ==''? '': 'desc';
		$arrOrders = array();
		//$arrOrders[] = "concat(right((100 +id_grup),2),'.',right((100 +id_modul),2),'.',right((1000 +id_sub_modul),3),'.',right((1000 +id_sub_sub_modul),3),'.',right((1000 +id_sub_sub_sub_modul),3))";
...
$popupShortcut = new popupShortcutObj();
if($Main->REK_DIGIT_O == 0){
	$popupShortcut->WHERE_O = "00";
}else{
	$popupShortcut->WHERE_O = "000";
}
