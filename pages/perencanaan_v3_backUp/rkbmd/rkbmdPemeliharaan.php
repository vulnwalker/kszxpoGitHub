...
class rkbmdPemeliharaan_v3Obj  extends DaftarObj2{	
	...
	function setTitle(){
		...
		return 'RKBMD PEMELIHARAAN KUASA PENGGUNA BARANG';
	}
	...
	<A href=\"pages.php?Pg=rkbmdPemeliharaan_v3\" title='RKBMD PEMELIHARAAN MURNI'  style='color : blue;' > PEMELIHARAAN </a> |
	...
	"<table width=\"100%\" class=\"menubar\" cellpadding=\"0\" cellspacing=\"0\" border=\"0\" style='margin:0 0 0 0'>
	<tr><td class=\"menudottedline\" width=\"40%\" height=\"20\" style='text-align:right'><B>
	<A href=\"pages.php?Pg=rkbmdPemeliharaan_v3\" title='RKBMD PENGADAAN MURNI' style='color : blue;' > INPUT </a> |
	<A href=\"pages.php?Pg=koreksiPenggunaPemeliharaan\" title='RKBMD PENGADAAN MURNI' > KOREKSI </a> |
		...
		case 'editTab':{
			...
			foreach ($_REQUEST as $key => $value) { 
				  $$key = $value; 
					...
					$execute = mysql_query("select * from view_rkbmd where  c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and bk='$bk' and ck='$ck' and p='$p' and q='$q' and tahun='$this->tahun' and id_tahap ='$this->idTahap' and f !='00' and id_jenis_pemeliharaan !='0'  $exceptID  ");
	   ...
	   if($this->jenisForm=="PENYUSUNAN"){
				...
				if($f == '00' && $q =='0')$TampilCheckBox = "";
					...
					if($this->wajibValidasi == TRUE)$Koloms.= "<td class='$cssclass' align='left'></td>";
					 ...
					 $getSumVolumeBarang = mysql_fetch_array(mysql_query("select sum(volume_barang) as jumlah from view_rkbmd where concat(c1,'.',c,'.',d,'.',e,'.',e1,'.',bk,'.',ck,'.',p,'.',q,'.',f,'.',g,'.',h,'.',i,'.',j) ='$customConcat' and id_jenis_pemeliharaan != '0' and id_tahap = '$this->idTahap'"));
	   ...
	   elseif($this->jenisForm=="KOREKSI PENGGUNA"){
				if($f == '00' && $q =='0')$TampilCheckBox = "";
					...
					$Koloms.= "<td class='$cssclass' align='left'></td>";
					 ...
					 $aksi  = "<img src='images/administrator/images/valid.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkbmdPemeliharaan_v3.sesuai('$id_anggaran');></img>&nbsp  &nbsp <img src='images/administrator/images/invalid.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkbmdPemeliharaan_v3.koreksi('$id_anggaran');></img> ";
		...
		while($rows = mysql_fetch_array($grabProgram)){
			...
			$grabKegiatan = mysql_query("select * from view_rkbmd where no_urut = '$lastNomorUrut' and jenis_anggaran = '$this->jenisAnggaran' and tahun = '$this->tahun'   and uraian_pemeliharaan !='RKBMD PENGADAAN' and uraian_pemeliharaan !='RKBMD PERSEDIAAN'  and bk='$bk' and ck='$ck'  and p ='$p' and q!='0' and j='000'");
				while($row2s = mysql_fetch_array($grabKegiatan)){
					...
					if(mysql_num_rows(mysql_query("select * from view_rkbmd where no_urut = '$lastNomorUrut' and jenis_anggaran = '$this->jenisAnggaran' and tahun = '$this->tahun'   and uraian_pemeliharaan !=''  and bk='$bk' and ck='$ck' and p ='$p' and q='$q' and j!='000' $this->kondisiBarang $jenisSKPD ")) == 0){
						if($q != '0'){
							$concat = $bk.".".$ck.".".$p.".".$q;
						...
						while($row3s = mysql_fetch_array($grabBarang)){
							...
							$concat = $f.".".$g.".".$h.".".$i.".".$j;
							if(mysql_num_rows(mysql_query("select * from view_rkbmd where no_urut = '$lastNomorUrut' and jenis_anggaran = '$this->jenisAnggaran' and tahun = '$this->tahun'  and uraian_pemeliharaan !=''  and bk='$bk' and ck='$ck' and p ='$p' and q='$q' and concat(f,'.',g,'.',h,'.',i,'.',j) = '$concat' $this->kondisiBarang $jenisSKPD ")) == 0){
								if($j != '000'){
...
function Pemeliharaan2($xls =FALSE){
		...
		while($rows = mysql_fetch_array($grabProgram)){
			...
			if(mysql_num_rows(mysql_query("select * from view_rkbmd where no_urut = '$lastNomorUrut' and jenis_anggaran = '$this->jenisAnggaran' and tahun = '$this->tahun'  and uraian_pemeliharaan !=''  and bk='$bk' and ck='$ck' and p ='$p' and j!='000' $this->kondisiBarang $jenisSKPD")) == 0){
				$concat = $bk.".".$ck.".".$p;
				$arrKondisi[] = " concat(bk,'.',ck,'.',p) !='$concat'";
			}else{
			$grabKegiatan = mysql_query("select * from view_rkbmd where no_urut = '$lastNomorUrut' and jenis_anggaran = '$this->jenisAnggaran' and tahun = '$this->tahun'   and uraian_pemeliharaan !='RKBMD PENGADAAN' and bk='$bk' and ck='$ck'  and p ='$p' and q!='0' and j='000' $this->kondisiBarang");
				while($row2s = mysql_fetch_array($grabKegiatan)){
					foreach ($row2s as $key => $value) { 
						  $$key = $value; 
					}
					if(mysql_num_rows(mysql_query("select * from view_rkbmd where no_urut = '$lastNomorUrut' and jenis_anggaran = '$this->jenisAnggaran' and tahun = '$this->tahun'   and uraian_pemeliharaan !=''  and bk='$bk' and ck='$ck' and p ='$p' and q='$q' and j!='000' $this->kondisiBarang $jenisSKPD ")) == 0){
						...
						$grabBarang = mysql_query("select * from view_rkbmd where no_urut = '$lastNomorUrut' and jenis_anggaran = '$this->jenisAnggaran' and tahun = '$this->tahun'   and uraian_pemeliharaan !='RKBMD PENGADAAN' and bk='$bk' and ck='$ck'  and p ='$p' and q='$q' and j!='000' $this->kondisiBarang");
						while($row3s = mysql_fetch_array($grabBarang)){
							foreach ($row3s as $key => $value) { 
								  $$key = $value; 
							}
		...
		while($daqry = mysql_fetch_array($aqry)){
			...
			if($q == '0' && $j =='000'){
				...
				$getJumlahBarangSebelumnya = mysql_fetch_array(mysql_query("select * from view_rkbmd where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and bk = '$bk' and ck ='$ck' and p = '$p' and q= '$q' and f='$f' and g='$g' and h='$h' and i='$i' and j ='$j' and id_jenis_pemeliharaan ='$id_jenis_pemeliharaan' and uraian_pemeliharaan = '$uraian_pemeliharaan' and no_urut='$nomorurutSebelumnya' and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' "));
