...
class inputPengadaanPenggunaObj  extends DaftarObj2{	
	var $Prefix = 'inputPengadaanPengguna';
	...
	var $FormName = 'inputPengadaanPenggunaForm'; 	
	var $kode_skpd = '';
	var $modul = "RKBMD";
	var $jenisForm = "";
	var $tahun = "";
	...
	<A href=\"pages.php?Pg=koreksiPenggunaPengadaan\" title='RKBMD PENGADAAN MURNI' style='color : blue;' > PENGADAAN </a> |
	...
	<table width=\"100%\" class=\"menubar\" cellpadding=\"0\" cellspacing=\"0\" border=\"0\" style='margin:0 0 0 0'>
	<tr><td class=\"menudottedline\" width=\"40%\" height=\"20\" style='text-align:right'><B>
	<A href=\"pages.php?Pg=inputPengadaanPengguna\" title='RKBMD PENGADAAN MURNI'  style='color : blue;' > INPUT </a> |
	<A href=\"pages.php?Pg=koreksiPenggunaPengadaan\" title='RKBMD PEMELIHARAAN MURNI'  > KOREKSI </a> |
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  case 'excel':{
			foreach ($_REQUEST as $key => $value) { 
			 ...
			 if($inputPengadaanPenggunaSkpdfmUrusan =='0'){
			 	$err = "Pilih Urusan";
			 }elseif($inputPengadaanPenggunaSkpdfmSKPD =='00'){
			 	$err = "Pilih Bidang";
			 }elseif($inputPengadaanPenggunaSkpdfmUNIT =='00'){
			 	$err = "Pilih SKPD";
			 }elseif($inputPengadaanPenggunaSkpdfmSUBUNIT =='00'){
			 	$err = "Pilih Unit";
			 }elseif($inputPengadaanPenggunaSkpdfmSEKSI =='000'){
			 	$err = "Pilih Sub Unit";
			 }else{
	...
	case 'formBaru':{				
		...
		case 'newTab':{
			 ...
			 $cekKeberadaanMangkluk =  mysql_num_rows(mysql_query("select * from view_renja where jenis_anggaran = '$this->jenisAnggaran' and tahun = '$this->tahun' and c1 = '$inputPengadaanPenggunaSkpdfmUrusan' and c = '$inputPengadaanPenggunaSkpdfmSKPD' and d='$inputPengadaanPenggunaSkpdfmUNIT' and e = '$inputPengadaanPenggunaSkpdfmSUBUNIT' and e1='$inputPengadaanPenggunaSkpdfmSEKSI'  and q!='0' and no_urut ='$nomorUrutSebelumnya' "));
			 $getDatarenja = mysql_fetch_array(mysql_query("select * from view_renja where jenis_anggaran = '$this->jenisAnggaran' and tahun = '$this->tahun' and c1 = '$inputPengadaanPenggunaSkpdfmUrusan' and c = '$inputPengadaanPenggunaSkpdfmSKPD' and d='$inputPengadaanPenggunaSkpdfmUNIT' and e = '$inputPengadaanPenggunaSkpdfmSUBUNIT' and e1='$inputPengadaanPenggunaSkpdfmSEKSI' and q!='0' and no_urut = '$nomorUrutSebelumnya'"));	 
			 $lastID = $getDatarenja['id_anggaran'];
			 $parentC1 = $inputPengadaanPenggunaSkpdfmUrusan;
		     $parentC = $inputPengadaanPenggunaSkpdfmSKPD;
			 $parentD = $inputPengadaanPenggunaSkpdfmUNIT;
			 $parentE = $inputPengadaanPenggunaSkpdfmSUBUNIT;
			 $parentE1= $inputPengadaanPenggunaSkpdfmSEKSI;
				...
				$parentE1= '000';
			 	...
			 	if($cekKeberadaanMangkluk != 0){
					if($getDatarenja['jenis_form_modul']  == 'PENYUSUNAN' ){
						$getJumlahRenjaValidasi = mysql_num_rows(mysql_query("select * from view_renja where jenis_anggaran = '$this->jenisAnggaran' and tahun = '$this->tahun' and c1 = '$inputPengadaanPenggunaSkpdfmUrusan' and c = '$inputPengadaanPenggunaSkpdfmSKPD' and d='$inputPengadaanPenggunaSkpdfmUNIT' and e = '$parentE' and e1='$parentE1' and q!='0' $this->sqlValidasi and no_urut = '$nomorUrutSebelumnya'"));
					...
					if($cmbJenisinputPengadaanPengguna == 'PENGADAAN'){
						$kemana = 'ins';
					}else{
		...
		case 'editTab':{
			 $id = $_REQUEST['inputPengadaanPengguna_cb'];
			foreach ($_REQUEST as $key => $value) { 
				  $$key = $value; 
			 } 
					$nomorUrutSebelumnya = $this->nomorUrut - 1;
					$getParentinputPengadaanPengguna = mysql_fetch_array(mysql_query("select * from view_rkbmd where id_anggaran = '$id[0]'"));
					foreach ($getParentinputPengadaanPengguna as $key => $value) { 
				 		 $$key = $value; 
					 } 
					$parentC1 = $getParentinputPengadaanPengguna['c1'];
					$parentC = $getParentinputPengadaanPengguna['c'];
					$parentD = $getParentinputPengadaanPengguna['d'];
					$parentE = '00';
					$parentE1= '000';
					$parentBK = $bk;
					$parentCK = $ck;
					...
					$arrayExcept = array();
					$grabExceptID = mysql_query("select * from view_rkbmd where no_urut = '$nextUrut' and jenis_anggaran = '$this->jenisAnggaran' and tahun = '$this->tahun' and j!='000' and id_jenis_pemeliharaan ='0' and c1='$c1' and c='$c' and d='$d' and e='00' and e1 ='000'");
					...
					$execute = mysql_query("select * from view_rkbmd where  c1='$parentC1' and c='$parentC' and d='$parentD' and e='$parentE' and e1='$parentE1' and bk='$parentBK' and ck='$parentCK' and p='$parentP' and q='$parentQ' and tahun='$this->tahun' and id_tahap ='$this->idTahap' and f !='00' and id_jenis_pemeliharaan = '0' $exceptID ");
				...
				$content = array('idrenja' => $idrenja, "kemana" =>$kemana, "qry " => "select * from view_rkbmd where  c1='$parentC1' and c='$parentC' and d='$parentD' and e='00' and e1='00' and bk='$parentBK' and ck='$parentCK' and p='$parentP' and q='$parentQ' and tahun='$this->tahun' and id_tahap ='$this->idTahap' and f !='00' and id_jenis_pemeliharaan = '0' $exceptID ");
			break;
		}
		case 'Laporan':{
			foreach ($_REQUEST as $key => $value) { 
		...
		case 'SaveValid':{
	    	foreach ($_REQUEST as $key => $value) { 
			 ...
			 $getSKPD = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$inputPengadaanPengguna_idplh'"));
			 ...
			 $query = VulnWalkerUpdate("tabel_anggaran",$data," id_anggaran = '$inputPengadaanPengguna_idplh'");
			 mysql_query($query);
	   ...
	   case 'remove':{				
		...
		case 'koreksi':{
			...
			$getinputPengadaanPenggunanya = mysql_fetch_array(mysql_query($queryRows));
			foreach ($getinputPengadaanPenggunanya as $key => $value) { 
				  $$key = $value; 
			} 
			 ...
			 $dataSesuai = array(           'jenis_anggaran' => $this->jenisAnggaran,
			  ...
			  if($cekKoreksi > 0 ){
			   	 $getID = mysql_fetch_array(mysql_query("select * from tabel_anggaran where c1 = '$c1' and c='$c' and d = '$d' and e='$e' and e1='$e1' and bk='$bk' and ck='$ck' and p = '$p' and q='$q'  and f1='$f1' and f2='$f2' and f='$f' and g='$g' and h='$h' and i='$i' and j='$j' and id_jenis_pemeliharaan ='$id_jenis_pemeliharaan' and tahun = '$this->tahun' and jenis_anggaran = '$this->jenisAnggaran' and id_tahap = '$this->idTahap' "));
						...
						if($angkaKoreksi <= $kebutuhanRiil || (empty($jumlahOptimal) && empty($kebutuhanMax) && $getinputPengadaanPenggunanya['volume_barang'] >= $angkaKoreksi )  ){
							mysql_query("update tabel_anggaran set volume_barang = '$angkaKoreksi' , cara_pemenuhan = '$caraPemenuhan' where id_anggaran='$idnya'");
						}elseif($getinputPengadaanPenggunanya['volume_barang'] < $angkaKoreksi){
	   ...
	   if($this->jenisForm == "KOREKSI"){
		...
		return 
			...
			<script type='text/javascript' src='js/skpd.js' language='JavaScript' ></script>".		
			 "<script type='text/javascript' src='js/perencanaan_v3/rkbmd/inputPengadaanPengguna.js' language='JavaScript' ></script>".			
			$scriptload;
	}
...
/*		$nomorUrutSebelumnya = $this-> -1;*/
	...
	function Laporan($dt){	
	 ...
	 $c1 = $dt['inputPengadaanPenggunaSkpdfmUrusan'];
	 $c = $dt['inputPengadaanPenggunaSkpdfmSKPD'];
	 $d = $dt['inputPengadaanPenggunaSkpdfmUNIT'];
	 $e = $dt['inputPengadaanPenggunaSkpdfmSUBUNIT'];
	 $e1 = $dt['inputPengadaanPenggunaSkpdfmSEKSI'];
	...
	$TampilOpt = 
			...
			<input type='hidden' name='cmbJenisinputPengadaanPengguna' id='cmbJenisinputPengadaanPengguna' value='PENGADAAN'>
	...
	function getDaftarOpsi($Mode=1){
		global $Main, $HTTP_COOKIE_VARS;
		$UID  = $_COOKIE['coID']; 
		$c1   = $_REQUEST['inputPengadaanPenggunaSkpdfmUrusan'];
		$c    = $_REQUEST['inputPengadaanPenggunaSkpdfmSKPD'];
		$d    = $_REQUEST['inputPengadaanPenggunaSkpdfmUNIT'];
		$e    = $_REQUEST['inputPengadaanPenggunaSkpdfmSUBUNIT'];
		$e1   = $_REQUEST['inputPengadaanPenggunaSkpdfmSEKSI'];
		...
		$fmKODE = $_REQUEST['fmKODE'];
		$fmBARANG = $_REQUEST['fmBARANG'];
		$cmbJenisinputPengadaanPengguna = $_REQUEST['cmbJenisinputPengadaanPengguna'];
		$arrKondisi = array();		
		...
		if($this->jenisForm == "PENYUSUNAN"){
			$getAllParent = mysql_query("select * from view_rkbmd where id_tahap='$this->idTahap' and f='00' and q = '0' and e1 ='000' and d!= '00' ");
...
$inputPengadaanPengguna = new inputPengadaanPenggunaObj();
$arrayResult = tahapPerencanaanV3($inputPengadaanPengguna->modul,"MURNI");
...
$inputPengadaanPengguna->jenisForm = $jenisForm;
$inputPengadaanPengguna->nomorUrut = $nomorUrut;
$inputPengadaanPengguna->urutTerakhir = $nomorUrut;
$inputPengadaanPengguna->tahun = $tahun;
$inputPengadaanPengguna->jenisAnggaran = $jenisAnggaran;
$inputPengadaanPengguna->idTahap = $idTahap;
$inputPengadaanPengguna->username = $_COOKIE['coID'];
$inputPengadaanPengguna->wajibValidasi = $arrayResult['wajib_validasi'];
if($inputPengadaanPengguna->wajibValidasi == TRUE){
	$inputPengadaanPengguna->sqlValidasi = " and status_validasi ='1' ";
}else{
	$inputPengadaanPengguna->sqlValidasi = " ";
}
$inputPengadaanPengguna->provinsi = $arrayResult['provinsi'];
$inputPengadaanPengguna->kota = $arrayResult['kota'];

$inputPengadaanPengguna->pengelolaBarang = $arrayResult['pengelolaBarang'];
$inputPengadaanPengguna->pejabatPengelolaBarang = $arrayResult['pejabat'];
$inputPengadaanPengguna->pengurusPengelolaBarang = $arrayResult['pengurus'];
$inputPengadaanPengguna->nipPengelola = $arrayResult['nipPengelola'];
$inputPengadaanPengguna->nipPengurus = $arrayResult['nipPengurus'];
$inputPengadaanPengguna->nipPejabat = $arrayResult['nipPejabat'];
