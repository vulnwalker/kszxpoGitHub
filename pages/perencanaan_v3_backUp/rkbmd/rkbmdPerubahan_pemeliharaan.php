...
class rkbmdPerubahan_pemeliharaan_v3Obj  extends DaftarObj2{	
	  ...
	  switch($tipe){	
			...
			$nextUrut = $this->nomorUrut + 1;
					...
					while($getKoreksi = mysql_fetch_array($grabExceptID)){
						foreach ($getKoreksi as $key => $value) { 
						 ...
						 $getIDawal = mysql_fetch_array(mysql_query("select * from view_rkbmd where id_tahap = '$this->idTahap' and c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and bk='$bk' and ck ='$ck' and p='$p' and $q='$q' and f='$f' and g='$g' and h = '$h' and i='$i' and j='$j' and id_jenis_pemeliharaan ='$id_jenis_pemeliharaan'"));
						 $arrayExcept[] = "id_anggaran !='".$getIDawal['id_anggaran']."'"; 
					}
					$exceptID= join(' and ',$arrayExcept);	
				    $exceptID = $exceptID =='' ? '':' and '.$exceptID;
			$getAll = mysql_query("select * from view_rkbmd where tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and jenis_form_modul ='PENYUSUNAN' and id_tahap='$this->idTahap' and j !='000' and id_jenis_pemeliharaan !='0' and c1='$c1' and c='$c' and d='$d' and e='$e' and e1 ='$e1' and bk='$bk' and ck='$ck' and p='$p' and q='$q' and status_validasi !='1' $exceptID");
		...
		case 'finish':{		
			...
			if(mysql_num_rows(mysql_query("select * from rkbmd_pemeliharaan where user='$username'")) == 0){
			    ...
			    mysql_query("delete from tabel_anggaran where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and bk='$bk' and ck='$ck' and p='$p' and q='$q' and tahun='$this->tahun' and id_tahap ='$this->idTahap' and ((id_jenis_pemeliharaan != '0' and f !='00') or uraian_pemeliharaan = 'RKBMD PEMELIHARAAN') and status_validasi !='1' $exceptID ");
   ...
   function setPage_OtherScript(){
	...
	if($hitungData > 1){
		$getMin = mysql_fetch_array(mysql_query("select min(concat(c1,'.',c,'.',d,'.',e,'.',e1,'.',bk,'.',ck,'.',p,'.',q,'.',f,'.',g,'.',h,'.',i,'.',j,'.',id_jenis_pemeliharaan)) as min from rkbmd_pemeliharaan where concat(f,'.',g,'.',h,'.',i,'.',j) = '$concat2' and user = '$username'"));
		$min = $getMin['min'];
