...
class pemasukanObj  extends DaftarObj2{
   ...
   function setPage_OtherScript(){
		...
		return
			...
			fn_TagScript('js/pengadaanpenerimaan/Persediaan/pemasukan_baru.js').
	...
	$this->form_height = 225;
		 ...
		 $this->form_caption = 'LAPORAN PENERIMAAN PERSEDIAAN BARANG';
	...
	function setPage_HeaderOther(){
		...
		if($halman == "3" && $this->STATUS_MODULPERSEDIAAN() == TRUE)$data="<a href=\"pages.php?Pg=pemasukan&halman=3\" title='PENERIMAAN' $setnya2 >PENERIMAAN</a> | <A href=\"pages.php?Pg=penerimaanDistribusi\" title='DISTRIBUSI'> DISTRIBUSI </a>";
	...
	function setKolomData_RekeningAtribusi($no, $isi, $Mode, $TampilCheckBox, $cssclass){
		...
		$Koloms="";
		$qry = "SELECT * FROM $this->ViewName_pnermn_atr WHERE refid_terima='".$isi['Id']."'";
		$aqry = mysql_query($qry);
		$idDokAtribusi = 0;
	...
	function setKolomData_Barang($no, $isi, $Mode, $TampilCheckBox, $cssclass){
		...
		while($dt = mysql_fetch_assoc($aqry)){
			...
			if($isi["asal_usul"] == "1"){
				switch($i){
					case 0:
						$tgl_pilih = $hit>1?$tgl_doksum."<br>".$tgl_buku:$tgl_doksum;
	...
	function BersihkanData(){
		$cek='';
		//Penerimaan Rekening
		$hapusrek = "DELETE FROM t_penerimaan_rekening WHERE tgl_update < DATE_SUB(NOW(), INTERVAL 4 HOUR) AND sttemp!='0'"; $cek.="| ".$hapusrek;
		$qry_hapusrek = mysql_query($hapusrek);

		$updrek = "UPDATE t_penerimaan_rekening SET status='0' WHERE tgl_update < DATE_SUB(NOW(), INTERVAL 4 HOUR) AND sttemp='0'";$cek.="| ".$updrek;
		$qry_updrek = mysql_query($updrek);


		//Penerimaan Detail -----------------------------------------------------------------------------------
		$hapuspenerimaan_det = "DELETE FROM t_penerimaan_barang_det WHERE tgl_update < DATE_SUB(NOW(), INTERVAL 4 HOUR) AND sttemp!='0'"; $cek.="| ".$hapuspenerimaan_det;
		$qry_hapuspenerimaan_det	= mysql_query($hapuspenerimaan_det);

		$updpenerimaan_det =  "UPDATE t_penerimaan_barang_det SET status='0' WHERE tgl_update < DATE_SUB(NOW(), INTERVAL 4 HOUR) AND sttemp='0'"; $cek.='| '.$updpenerimaan_det;
		...
		//ATRIBUSI_RINCIAN
		$hapus_atribusiRincian = "DELETE FROM t_atribusi_rincian WHERE tgl_update < DATE_SUB(NOW(), INTERVAL  4 HOUR) AND sttemp='1'"; $cek.="| ".$hapus_atribusiRincian;
		$qryhapus_atribusiRincian = mysql_query($hapus_atribusiRincian);

		$upd_atribusirincian = "UPDATE t_atribusi_rincian SET status='0' WHERE tgl_update < DATE_SUB(NOW(), INTERVAL  4 HOUR) AND sttemp='0'";$cek.="| ".$upd_atribusirincian;
