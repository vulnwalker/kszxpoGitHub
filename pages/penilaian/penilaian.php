...
class PenilaianObj extends DaftarObj2{
	...
	$set_urusan = $Main->URUSAN;	
		return
			...
			"<td>".genPanelIcon("javascript:".$this->Prefix.".Baru($set_urusan)","new_f2.png","Baru",'Baru')."</td>".
	...
	function setPage_HeaderOther(){	
		...
		switch ($Pg){
			...
			//index.php?Pg=09
			$menu = $Main->MENU_VERSI=3?"":"<table width=\"100%\" class=\"menubar\" cellpadding=\"0\" cellspacing=\"0\" border=\"0\" style='margin:0 0 4 0'>
	...
	function genDaftarOpsi(){
		...
		$thn = $_REQUEST['fmthn']==''?$thn_login:$_REQUEST['fmthn'];
		...
		$jmlperpage = $_REQUEST['jmlperpage']==''?$Main->PagePerHal:$_REQUEST['jmlperpage'];
					...
					<tr valign=\"top\">
						<td>
							<div style=\"float:left\">". 
								...
								Baris per halaman &nbsp;<input type=\"text\" name=\"jmlperpage\" id=\"jmlperpage\" size=\"5\" value=\"$jmlperpage\">&nbsp;&nbsp;".
	...
	function getDaftarOpsi($Mode=1){
		...
		$fmURUSAN = isset($HTTP_COOKIE_VARS['coURUSAN'])? $HTTP_COOKIE_VARS['cofmURUSAN']: cekPOST($this->Prefix.'SkpdfmURUSAN');
		...
		if(!empty($kd_barang)) $arrKondisi[] = "concat($concatkd) like '%$kdbrg%'";
		if(!empty($nm_barang)) $arrKondisi[] = "nm_barang like '%$nm_barang%'";	
		//if(!empty($thn)) 
		$arrKondisi[] = !empty($thn)?"year(tgl_penilaian)='$thn' ":"year(tgl_penilaian)='$thn_login' ";	
		...
		*/
		...
		$jmlperpage = $_REQUEST['jmlperpage'];
		//limit --------------------------------------
		$pagePerHal = $jmlperpage =='' ? $Main->PagePerHal: $jmlperpage; 
	...
	function set_selector_other($tipe){
		...
		switch($tipe){	
		   ...
		   case 'getdata':{
				...
				$alamat = $kib_c['alamat'];					
				$kdbrg = $Main->KD_BARANG_P108?$bi['f1'].'.'.$bi['f2'].'.'.$bi['f'].'.'.$bi['g'].'.'.$bi['h'].'.'.$bi['i'].'.'.$bi['j']:$bi['f'].'.'.$bi['g'].'.'.$bi['h'].'.'.$bi['i'].'.'.$bi['j'];
				...
				$br = mysql_fetch_array(mysql_query("select * from ref_barang where concat($concatkd) = '$kd_barang'"));
				$nilai_buku = getNilaiBuku($ids[0],$tgl,0);
	...
	function GetHrg_Asal(){
		...
		$harga = getNilaiPerolehan($idbi_awal,$tgl_perolehan);
	...
	function setFormEdit(){
		...
		$aqry2 = "select f1,f2,f,g,h,i,j from buku_induk where id ='".$dt['id_bukuinduk']."'";
		$row = mysql_fetch_array(mysql_query($aqry2));
		$dt['kd_barang'] = $Main->KD_BARANG_P108?$row['f1'].'.'.$row['f2'].'.'.$row['f'].'.'.$row['g'].'.'.$row['h'].'.'.$row['i'].'.'.$row['j']:$row['f'].'.'.$row['g'].'.'.$row['h'].'.'.$row['i'].'.'.$row['j'];
		$kd = str_replace('.','',$dt['kd_barang']);
		$concatkd = $Main->KD_BARANG_P108?"f1,f2,f,g,h,i,j":"f,g,h,i,j";
		$aqry3 = "select nm_barang from ref_barang where concat($concatkd) = '$kd'";
	...
	function setForm($dt){	
		...
		if ($this->form_fmST==0) {
			...
			$nip	 = '';
			$inptbtn = "<input type='button' value='Cari' onclick ='".$this->Prefix.".TambahBarang()' >";
		}else{
		...
		$this->form_fields = array(	
			...
			'tgl_perolehan' => array('label'=>'Tanggal Nilai Perolehan',
	...
	function simpan(){
	 ...
	 if ($err=='' && $fmST==0){//baru
		...
		if($errmsg=='' && $tgl_perolehan<$gentgl_perolehan)$errmsg ='Tanggal Perolehan tidak lebih kecil dari Tanggal BAST/Buku Barang !';
		//$err='tgl='.$tgl_closing;			
	 }else{//edit
		$old_penilaian = mysql_fetch_array(mysql_query("select * from penilaian where id='$idplh'"));
			...
			if($err=='' && $old_penilaian['tgl_penilaian']<$tgl_susutAkhir['tgl']){
			...
			if($errmsg=='' && $old_penilaian['tgl_perolehan']<$gentgl_perolehan ){
				if($errmsg=='' && $old_penilaian['tgl_perolehan']!=$tgl_perolehan)$errmsg ='Tanggal Perolehan tidak lebih kecil dari Tanggal BAST/Buku Barang !';
			}			 
		}	 	 	 
	 		if($err==''){ 
				if($fmST == 0){ //input penggunaan				
					$aqry = "INSERT INTO penilaian (id_bukuinduk,a1,a,b,c1,c,d,e,e1,f1,f2,f,g,h,i,j,
							...
							VALUES ('$id_bukuinduk','$a1','$a','$b','$c1','$c','$d','$e','$e1','$f1','$f2','$f','$g','$h','$i','$j',
	...
	function batal(){
	 ...
	 if($err=='' && sudahClosing($tgl_penilaian,$c,$d,$e,$e1,$c1))$err = 'Tanggal sudah Closing !';	 
		/*if($err==''){ 
