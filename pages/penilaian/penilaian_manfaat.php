<?php
class Penilaian_manfaatObj extends DaftarObj2{
	var $Prefix = 'Penilaian_manfaat';
	var $SHOW_CEK = TRUE;	
	var $TblName = 'penilaian_manfaat';//'v1_rkb'
	var $TblName_Hapus = 'penilaian_manfaat';
	var $TblName_Edit = 'penilaian_manfaat';
	...
	var $FormName = 'Penilaian_manfaatForm';
	//var $pagePerHal = 7;
	...
	var $fileNameExcel='HASIL_PENILAIAN_MANFAAT.xls';
	var $Cetak_Judul = 'HASIL PENILAIAN PEMANFAATAN BARANG MILIK DAERAH';	
	var $Cetak_Mode=2;
	...
	function setTitle(){
		return 'Hasil Penilaian - Pemanfaatan';
	}
	
	function setMenuEdit(){		
		return
			
			"<td>".genPanelIcon("javascript:".$this->Prefix.".Baru()","new_f2.png","Baru",'Penilaian Pemanfaatan Baru')."</td>".
			"<td>".genPanelIcon("javascript:".$this->Prefix.".Edit()","edit_f2.png","Edit", 'Edit Penilaian Pemanfaatan')."</td>".
			"<td>".genPanelIcon("javascript:".$this->Prefix.".Hapus()","delete_f2.png","Hapus", 'Hapus Penilaian Pemanfaatan')."</td>";
	}
	...
	function setNavAtas(){
		return
			...
			<td class="menudottedline" width="60%" height="20" style="text-align:right"><b>
				<a style="color:blue;" href="pages.php?Pg=Penilaian_manfaat" title="Penilaian pemanfaatan barang" >Pemanfaatan</a> |			
	...
	function simpan(){
		...
		$tgl_pemanfaatan=$_REQUEST['tgl_buku'];
		...
		if($err==''){
			if($fmST == 0){//baru
				...
				$aqry = 
					...
					" f='$f',g='$g',h='$h',i='$i',j='$j',".
					" noreg='$noreg',thn_perolehan='$thn_perolehan',tgl_pemanfaatan='$tgl_pemanfaatan',".
					...
					" thn_akun='$thn_akun',bentuk_pemanfaatan='$bentuk_pemanfaatan',".					
	...
	function setFormBaru(){
		...
		$dt['tgl_pemanfaatan'] = date("Y-m-d");//tanggal buku
		$this->form_fmST = 0;
		$fm = $this->setForm($dt);
	  ...
	  if ($this->form_fmST==0) {
		 ...
		 *****************************************************************/
		 ...
		 $this->form_fields = array(									 
			...
			'tgl_buku' => array(
							'label'=>'Tanggal Buku', 
							'value'=> createEntryTgl('tgl_buku',$dt['tgl_pemanfaatan'] ), 
							'type'=>''
			),			
			'nama_penilai' => array(
							'label'=>'Nama Penilai',
	...
	function SetPilihCariBI(){
		...
		if($err==''){
			...
			$harga_buku=getNilaiBuku($bi['id'],$tgl_pemanfaatan,0);	
		...
		$headerTable =
			"<thead>
				<tr>
					...
					<th class='th01' rowspan=2 width=''>Bentuk Pemanfaatan</th>
	...
	function setKolomData($no, $isi, $Mode, $TampilCheckBox){
		...
		$Koloms[] = array("align=''",  $Main->BentukPemanfaatan[$isi['bentuk_pemanfaatan']-1][1] );
		...
		$Koloms[] = array("align='center'",  TglInd($isi['tgl_pemanfaatan'] )."/<br/>".$isi['ket'] );
		return $Koloms;
	}
...
$Penilaian_manfaat = new Penilaian_manfaatObj();
