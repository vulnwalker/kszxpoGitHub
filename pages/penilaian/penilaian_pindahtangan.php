<?php
class Penilaian_pindahtanganObj extends DaftarObj2{
	var $Prefix = 'Penilaian_pindahtangan';
	var $SHOW_CEK = TRUE;	
	var $TblName = 'penilaian_pindahtangan';//'v1_rkb'
	var $TblName_Hapus = 'penilaian_pindahtangan';
	var $TblName_Edit = 'penilaian_pindahtangan';
	var $KeyFields = array('id');
	var $FieldSum = array('nilai_barang');
	var $fieldSum_lokasi = array( 10);
	var $SumValue = array();
	var $FieldSum_Cp1 = array( 9, 8, 8);
	var $FieldSum_Cp2 = array( 4, 4, 4);	
	var $totalCol = 15; //total kolom daftar
	var $FormName = 'Penilaian_pindahtanganForm';
	//var $pagePerHal = 7;
	...
	var $fileNameExcel='HASIL_PENILAIAN_PEMINDAHTANGANAN.xls';
	var $Cetak_Judul = 'HASIL PENILAIAN PEMINDAHTANGANAN BARANG MILIK DAERAH';	
	var $Cetak_Mode=2;
	...
	function setTitle(){
		return 'Hasil Penilaian - Pemindahtanganan';
	}
	
	function setMenuEdit(){		
		return
			
			"<td>".genPanelIcon("javascript:".$this->Prefix.".Baru()","new_f2.png","Baru",'Penilaian Pemindahtanganan Baru')."</td>".
			"<td>".genPanelIcon("javascript:".$this->Prefix.".Edit()","edit_f2.png","Edit", 'Edit Penilaian Pemindahtanganan')."</td>".
			"<td>".genPanelIcon("javascript:".$this->Prefix.".Hapus()","delete_f2.png","Hapus", 'Hapus Penilaian Pemindahtanganan')."</td>";
	}
	...
	function setNavAtas(){
		return
			...
			<td class="menudottedline" width="60%" height="20" style="text-align:right"><b>
				...
				<a style="color:blue;" href="pages.php?Pg=Penilaian_pindahtangan" title="Penilaian pemindahtanganan barang">Pemindahtanganan</a>  |  
				<a href="pages.php?Pg=Penilaian_koreksi" title="Koreksi penilaian">Koreksi</a> 
				  &nbsp;&nbsp;&nbsp;
			</b></td>
			</tr>
			</table>';
	...
	function simpan(){
		...
		$tgl_pemindahtanganan=$_REQUEST['tgl_buku'];
		$nilai_barang_asal=str_replace(".","",$_REQUEST['fmharga_perolehan']);
		$harga_buku=str_replace(".","",$_REQUEST['fmharga_buku']);
		$nilai_barang=$_REQUEST['fmharga_penilaian'];
		$penilai_instansi=$_REQUEST['fmnm_penilai'];
		$penilai_alamat=$_REQUEST['fmalamat_penilai'];
		...
		//-- validasi
		if($err=='' && sudahClosing($tgl_pemindahtanganan,$c,$d))$err = "Tanggal Sudah Closing!";
		...
		if($err=='' && $nilai_barang == '')$err = "Harga penilaian belum diisi!";
		if($err=='' && $penilai_instansi == '')$err = "Nama penilai belum diisi!";
		...
		if($err==''){
			if($fmST == 0){//baru
				...
				$aqry = 
					...
					" nilai_barang_asal,nilai_barang,penilai_instansi,penilai_alamat,".
					" surat_no,surat_tgl,ket,tahun,".
					" ka,kb,kc,kd,ke,kf,".
					" thn_akun,bentuk_pemindahtanganan,".					
				...
				if($err=='' && sudahClosing($old['tgl_pemindahtanganan'],$c,$d))$err = "Tanggal Sudah Closing!";
				$aqry = 
					"update $this->TblName_Edit  ".
					"set ".
					" a1='$a1',a='$a',b='$b',c='$c',d='$d',e='$e',e1='$e1',".
					" f='$f',g='$g',h='$h',i='$i',j='$j',".
					" noreg='$noreg',thn_perolehan='$thn_perolehan',tgl_pemindahtanganan='$tgl_pemindahtanganan',".
					" nilai_barang_asal='$nilai_barang_asal',nilai_barang='$nilai_barang',penilai_instansi='$penilai_instansi',penilai_alamat='$penilai_alamat',".
					" surat_no='$surat_no',surat_tgl='$surat_tgl',ket='$ket',tahun='$tahun',".
					" ka='$ka',kb='$kb',kc='$kc',kd='$kd',ke='$ke',kf='$kf',".
					" thn_akun='$thn_akun',bentuk_pemindahtanganan='$bentuk_pemindahtanganan',".
	...
	function setFormBaru(){
		...
		$dt['tgl_pemindahtanganan'] = date("Y-m-d");//tanggal Buku
		$this->form_fmST = 0;
		$fm = $this->setForm($dt);
	  ...
	  if ($this->form_fmST==0) {
		 ...
		 *****************************************************************/
		 ...
		 $this->form_fields = array(									 
			...
			'harga_penilaian' => array( 
							'label'=>'Harga Penilaian',
							'labelWidth'=>150, 
							'value'=>inputFormatRibuan("fmharga_penilaian", '',$dt['nilai_barang']) ,
				),
			'tgl_penilaian' => array(
							'label'=>'Tanggal Buku', 
							'value'=> createEntryTgl('tgl_buku',$dt['tgl_pemindahtanganan'] ), 
							'type'=>''
			),			
			'nama_penilai' => array(
							'label'=>'Nama Penilai',
							'labelWidth'=>150, 
							'value'=>"<INPUT type=text name='fmnm_penilai' id='fmnm_penilai' value='".$dt['penilai_instansi']."' >",
							'type'=>''
				),
			'alamat_penilai' => array( 
							'label'=>'Alamat', 
							'labelWidth'=>150,
							'value'=> "<textarea style='width:438;' id='fmalamat_penilai' name='fmalamat_penilai' >".$dt['penilai_alamat']."</textarea> ", 
							'row_params'=>"valign='top'",
							'labelWidth'=>116, 'type'=>'' 
				),
	...
	function SetPilihCariBI(){
		...
		if($err==''){
			...
			$harga_buku=getNilaiBuku($bi['id'],$tgl_pemindahtanganan,0);	
		...
		$headerTable =
			"<thead>
				<tr>
					...
					<th class='th01' rowspan=2 width=''>Bentuk Pemindahtanganan</th>
					<th class='th01' rowspan=2 width=''>Harga Penilaian</th>
					<th class='th01' rowspan=2>Nama Penilai/ Alamat</th>
	...
	function setKolomData($no, $isi, $Mode, $TampilCheckBox){
		...
		$Koloms[] = array("align=''",  $Main->BentukPemindahtanganan[$isi['bentuk_pemindahtanganan']-1][1] );
		$Koloms[] = array("align='right'", number_format($isi['nilai_barang'] ,2,',','.' ));
		$Koloms[] = array("align=''", $isi['penilai_instansi']."/<br/>".$isi['penilai_alamat']	);
		$Koloms[] = array("align='right'", $isi['surat_no'] );
		$Koloms[] = array("align='center'",  TglInd($isi['surat_tgl'] ) );		
		$Koloms[] = array("align='center'",  TglInd($isi['tgl_pemindahtanganan'] )."/<br/>".$isi['ket'] );
		return $Koloms;
	}
	...
	function getDaftarOpsi($Mode=1){
		...
		if(!($fmThnAnggaran=='') ) $arrKondisi[] = "YEAR(tgl_pemindahtanganan)='$fmThnAnggaran'";
		
		if ($fmThnAnggaranDari == $fmThnAnggaranSampai){
		
			if(!($fmThnAnggaranDari=='')  && !($fmThnAnggaranSampai=='')) $arrKondisi[] = "YEAR(tgl_pemindahtanganan)>='$fmThnAnggaranDari' and YEAR(tgl_pemindahtanganan)<='$fmThnAnggaranSampai' ";
			switch($fmPILSEMESTER){			
			case '1': $arrKondisi[] = " tgl_pemindahtanganan>='".$fmThnAnggaranDari."-01-01' and  cast(tgl_pemindahtanganan as DATE)<='".$fmThnAnggaranSampai."-06-30' "; break;
			case '2': $arrKondisi[] = " tgl_pemindahtanganan>='".$fmThnAnggaranDari."-07-01' and  cast(tgl_pemindahtanganan as DATE)<='".$fmThnAnggaranSampai."-12-31' "; break;
			default :""; break;
			}
		}else{
			if(!($fmThnAnggaranDari=='') && !($fmThnAnggaranSampai=='')) $arrKondisi[] = "YEAR(tgl_pemindahtanganan)>='$fmThnAnggaranDari' and YEAR(tgl_pemindahtanganan)<='$fmThnAnggaranSampai' ";
...
$Penilaian_pindahtangan = new Penilaian_pindahtanganObj();
