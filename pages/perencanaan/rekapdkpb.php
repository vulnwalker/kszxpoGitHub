...
class rekapdkpbObj  extends DaftarObj2{	
	var $Prefix = 'rekapdkpb';
	...
	var $FieldSum = array('jml_barang','jml_biaya');//array('jml_harga');
	var $SumValue = array();
	...
	var $Cetak_Judul = 'Rekap DKPBMD';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'rekapdkpbForm'; 	
	...
	function setNavAtas(){
		...
		return
			...
			<td class="menudottedline" width="60%" height="20" style="text-align:right"><b>
				...
				<a style="color:blue;" href="pages.php?Pg=rekapdkpb" title="Rekap Daftar Kebutuhan Pemeliharaan Barang Milik Daerah">Rekap DKPBMD</a> |
...
//		if( $fmBerdasarkan == 2 ) $arrKondisi[] = " concat(ka, '.', kb, '.', kc, '.', kd, '.', ke) like '$kdakun%' ";	
		...
		if($berdasarkan==2){
			$aqry = "select '$fmSKPD' as c,'$fmUNIT' as d, aa.ka, aa.kb, aa.kc, aa.kd, aa.ke, aa.kf, aa.nm_account,bb.jml_barang, bb.jml_biaya, bb.ket, $fmThnAnggaran as tahun ".
//ifNULL(bb.tahun,'0000') as tahun
					...
					"sum(jml_barang) as jml_barang, sum(jml_biaya) as jml_biaya, ket, tahun ".
					"from dkpb ". 
			...
			$aqry = "select '$fmSKPD' as c, '$fmUNIT' as d,aa.f,aa.g,aa.h,aa.i,aa.j,
					aa.nm_barang,bb.jml_barang,bb.jml_biaya,bb.ket, ".
...
//ifNULL(bb.tahun,'0000') as tahun
		...
		switch($fmLevel){
			case '1':case '2':case '3':default:{
				if($kode1!=0  && $kode2!=0 && $kode3==0){
					...
					$qh=mysql_fetch_array(mysql_query("select sum(jml_barang) as jml_barang, sum(jml_biaya) as jml_biaya from dkpb WHERE k='$kode1' and l='$kode2' and tahun='$fmThnAnggaran' $KondThnAkun $KondSKPD"));
					$jumlah_barang=$qh['jml_barang'];	
					$harga_barang=$qh['jml_biaya'];		
				}else{
					$jumlah_barang=$isi['jml_barang'];	
					$harga_barang=$isi['jml_biaya'];				
				}				
			} break;
			case '4':{
				if($kode1!=0  && $kode2!=0 && $kode3==0){
					...
					$qh=mysql_fetch_array(mysql_query("select sum(jml_barang) as jml_barang, sum(jml_biaya) as jml_biaya from dkpb WHERE k='$kode1' and l='$kode2' and m='$kode3' and tahun='$fmThnAnggaran' $KondThnAkun $KondSKPD"));
					$jumlah_barang=$qh['jml_barang'];	
					$harga_barang=$qh['jml_biaya'];		
				}else{
					$jumlah_barang=$isi['jml_barang'];	
			...
			case '5':{
				if($kode1!=0  && $kode2!=0 && $kode3==0){
					...
					//query get jml_harga level 3 selanjutnya
					$qh=mysql_fetch_array(mysql_query("select sum(jml_barang) as jml_barang, sum(jml_biaya) as jml_biaya from dkpb WHERE k='$kode1' and l='$kode2' and m='$kode3' and n='$kode4' and tahun='$fmThnAnggaran' $KondThnAkun $KondSKPD"));
					$jumlah_barang=$qh['jml_barang'];	
					$harga_barang=$qh['jml_biaya'];		
				}else{
					$jumlah_barang=$isi['jml_barang'];	
 ...
 //echo $aqry;
	...
	function setSumHal2_query($Kondisi, $fsum){
		...
		if($berdasarkan==2){
			return "select $fsum from dkpb WHERE $KondTahun $KondSKPD $KondThnAkun and concat(k, '.', l, '.', m, '.', n, '.', o, '.', kf) like '$kdakun%' ";	
		}else{
			return "select $fsum from dkpb WHERE $KondTahun $KondSKPD $KondThnAkun  ";
		} //echo $aqry;
	}	
