...
class rekaprkpbObj  extends DaftarObj2{	
	var $Prefix = 'rekaprkpb';
	...
	var $Cetak_Judul = 'Rekap RKPBMD';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'rekaprkpbForm'; 	
	...
	function setNavAtas(){
		...
		return
			...
			<td class="menudottedline" width="60%" height="20" style="text-align:right"><b>
				...
				<a style="color:blue;" href="pages.php?Pg=rekaprkpb" title="Rekap Rencana Kebutuhan Pemeliharaan Barang Milik Daerah">Rekap RKPBMD</a> |
...
//ifNULL(bb.tahun,'0000') as tahun
		...
		switch($fmLevel){
			case '1':case '2':case '3':default:{
				if($kode1!=0  && $kode2!=0 && $kode3==0){
					...
					$qh=mysql_fetch_array(mysql_query("select sum(jml_barang) as jml_barang, sum(jml_biaya) as jml_biaya from rkpb WHERE k='$kode1' and l='$kode2' and tahun='$fmThnAnggaran' $KondThnAkun $KondSKPD"));
			...
			case '4':{
				if($kode1!=0  && $kode2!=0 && $kode3==0){
					...
					$qh=mysql_fetch_array(mysql_query("select sum(jml_barang) as jml_barang, sum(jml_biaya) as jml_biaya from rkpb WHERE k='$kode1' and l='$kode2' and m='$kode3' and tahun='$fmThnAnggaran' $KondThnAkun $KondSKPD"));
					$jumlah_barang=$qh['jml_barang'];	
					$harga_barang=$qh['jml_biaya'];		
				}else{
					$jumlah_barang=$isi['jml_barang'];	
			...
			case '5':{
				if($kode1!=0  && $kode2!=0 && $kode3==0){
					...
					//query get jml_harga level 3 selanjutnya
					$qh=mysql_fetch_array(mysql_query("select sum(jml_barang) as jml_barang, sum(jml_biaya) as jml_biaya from rkpb WHERE k='$kode1' and l='$kode2' and m='$kode3' and n='$kode4' and tahun='$fmThnAnggaran' $KondThnAkun $KondSKPD"));
					$jumlah_barang=$qh['jml_barang'];	
					$harga_barang=$qh['jml_biaya'];		
				}else{
					$jumlah_barang=$isi['jml_barang'];	
	...
	function setSumHal2_query($Kondisi, $fsum){
		...
		$berdasarkan=$_REQUEST['fmBerdasarkan'];	
		if($berdasarkan==2){
			return "select $fsum from rkpb $Kondisi and concat(k, '.', l, '.', m, '.', n, '.', o, '.', kf) like '$kdakun%' ";	
		}else{
			return "select $fsum from rkpb $Kondisi  ";
		}
 //echo $aqry;
	}	
