...
class RkpbObj extends DaftarObj2{
	...
	var $FieldSum = array('jml_barang','jml_biaya');
	var $SumValue = array();
	var $FieldSum_Cp1 = array( 8, 7, 7);
	var $FieldSum_Cp2 = array( 4, 4,4);
	var $fieldSum_lokasi = array( 5,10); 
	...
	function setKolomData($no, $isi, $Mode, $TampilCheckBox){
		...
		$nmopd = //$fmSKPD.'-'.$fmUNIT.'-'.$fmSUBUNIT.' '.
			join(' - ', $nmopdarr );
		$get=mysql_fetch_array(mysql_query("select * from ref_skpd where c='".$isi['c']."' and d='".$isi['d']."' and e='".$isi['e']."' and e1='$kdSubUnit0'"));
		$subunit = $get['nm_skpd'];	
		...
		$Koloms[] = array("align='right'", $isi['jml_barang']*$isi['kuantitas']  );	
		...
		$Koloms[] = array("align='center'",  $subunit);
	...
	function setForm($dt){	
		...
		*/
		$value_info=$dt['jml_barang']*$dt['kuantitas'];
		...
		$this->form_fields = array(	
			...
			'jml_barang' => array(  'label'=>'Jumlah Barang', 
				'value'=> 
				"<input name=\"jml_barang\" id='jml_barang' type=\"text\" value='".$dt['jml_barang']."' onkeypress='return isNumberKey(event)' onkeyup='".$this->Prefix.".Getinfo_volume()'/>",				 
				//inputFormatRibuan("jml_barang", '',$dt['jml_barang']) ,
				'type'=>'' 
			),
			'kuantitas' => array( 'label'=>'Volume / Kuantitas', 
				'value'=> 
				"<input name=\"kuantitas\" id='kuantitas' type=\"text\" value='".$dt['kuantitas']."' onkeypress='return isNumberKey(event)' onkeypress='return isNumberKey(event)' onkeyup='".$this->Prefix.".Getinfo_volume()'/>".
				" Satuan ".
				"<input name=\"satuan\" id='satuan' type=\"text\" value='".$dt['satuan']."' />&nbsp;Info Volume&nbsp;<input type='text' name='info_volume' id='info_volume' value='$value_info' readonly>", 
				//inputFormatRibuan("jml_barang", '',$dt['jml_barang']) ,
				'type'=>'' 
			),						
	...
	function setFormDKPB(){
		...
		"<input type=hidden id='e' name='e' value='".$dt['e']."'> ".
		"<input type=hidden id='e1' name='e1' value='".$dt['e1']."'>".
		"<input type='button' value='Simpan' onclick ='".$this->Prefix.".SimpanDKPB()' >".
		"<input type='button' value='Batal' onclick ='".$this->Prefix.".Close()' >";
		$form = $this->genForm();		
	...
	function simpanDKPB(){
		...
		if($err =='' && $rkpb['id']=='') $err = "RKPB Tidak Ada! select * from $this->TblName where id='$idrkpb'";
		...
		if($err == ''){		
			...
			*/
			$aqry = "insert into dkpb (jml_barang,kuantitas,harga,satuan,jml_biaya,ket,uraian,jns_pelihara,kapitalisasi,nm_barang,nm_account,".
				"idrkpb,a1,a,b,c,d,e,e1,f,g,h,i,j,k,l,m,n,o,kf,tahun,thn_perolehan,thn_anggaran,thn_akun, uid,tgl_update)".
				" values ".
				"('$jml_barang','$kuantitas','$harga','$satuan','$jml_harga','$ket','$uraian','$jns_pelihara','$kapitalisasi','$nm_barang','$nm_account',".
				"'$idrkpb','$a1','$a','$b','$c','$d','$e','$e1','$f','$g','$h','$i','$j','$k','$l','$m','$n','$o','$kf','$tahun', '$thn_perolehan' ,'$fmTAHUN','$thn_akun','$UID',now())"; $cek .= $aqry;
		...
		if($err==''){
			...
			if($fmST == 0){//baru
				...
				$aqry = 
					...
					"ket='$ket',nm_barang='$nm_barang',nm_account='$nm_account',idbi='$idbi', ".
