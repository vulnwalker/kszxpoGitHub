...
class rekaprptbmdObj  extends DaftarObj2{	
	var $Prefix = 'rekaprptbmd';
	...
	var $Cetak_Judul = 'Rekap RPTBMD';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'rekaprptbmdForm'; 	
	...
	function setNavAtas(){
		...
		return
			...
			<td class="menudottedline" width="60%" height="20" style="text-align:right"><b>
				...
				<a style="color:blue;" href="pages.php?Pg=rpebmd" title="Pemindahtanganan">Pemindahtanganan</a>  |
			...
			<tr>
			<td class="menudottedline" width="60%" height="20" style="text-align:right"><b>
				<a href="pages.php?Pg=rpebmd" title="Rencana Pemindahtanganan Barang Milik Daerah">RPTBMD</a>  |
				<a style="color:blue;" href="pages.php?Pg=rekaprptbmd" title="Rekap Rencana Pemindahtanganan Barang Milik Daerah">Rekap RPTBMD</a>  |	
				<a href="pages.php?Pg=rekaprpebmd_skpd" title="Rekap Rencana Pemindahtanganan Barang Milik Daerah">Rekap RPTBMD (SKPD)</a>  		
				  &nbsp;&nbsp;&nbsp;
			</b></td>
			</tr>
			</table>';
...
//ifNULL(bb.tahun,'0000') as tahun
					...
					"from t_rencana_pemindahtanganan ". 
		...
		switch($fmLevel){
			case '1':case '2':case '3':default:{
				if($kode1!=0  && $kode2!=0 && $kode3==0){
					...
					$qh=mysql_fetch_array(mysql_query("select count(*) as jml_barang, sum(harga) as harga from t_rencana_pemindahtanganan WHERE ka='$kode1' and kb='$kode2' and thn_anggaran='$fmThnAnggaran' $KondThnAkun $KondSKPD"));
			...
			case '4':{
				if($kode1!=0  && $kode2!=0 && $kode3==0){
					...
					$qh=mysql_fetch_array(mysql_query("select count(*) as jml_barang, sum(harga) as harga from t_rencana_pemindahtanganan WHERE ka='$kode1' and kb='$kode2' and kc='$kode3' and thn_anggaran='$fmThnAnggaran' $KondThnAkun $KondSKPD"));
					$jumlah_barang=$qh['jml_barang'];	
					$harga_barang=$qh['harga'];		
				}else{
					$jumlah_barang=$isi['jml_barang'];	
			...
			case '5':{
				if($kode1!=0  && $kode2!=0 && $kode3==0){
					...
					//query get jml_harga level 3 selanjutnya
					$qh=mysql_fetch_array(mysql_query("select count(*) as jml_barang, sum(harga) as harga from t_rencana_pemindahtanganan WHERE ka='$kode1' and kb='$kode2' and kc='$kode3' and kd='$kode4' and thn_anggaran='$fmThnAnggaran' $KondThnAkun $KondSKPD"));
					$jumlah_barang=$qh['jml_barang'];	
					$harga_barang=$qh['harga'];		
				}else{
					$jumlah_barang=$isi['jml_barang'];	
	...
	function setSumHal2_query($Kondisi, $fsum){
		...
		if($berdasarkan==2){
			return "select $fsum from (select *, 
					'1' as jml_barang 
					from t_rencana_pemindahtanganan WHERE $KondTahun $KondSKPD $KondThnAkun and concat(ka, '.', kb, '.', kc, '.', kd, '.', ke, '.', kf) like '$kdakun%') as aa ";	
		}else{
			return "select $fsum from (select *, 
					'1' as jml_barang 
					from t_rencana_pemindahtanganan WHERE $KondTahun $KondSKPD $KondThnAkun) as aa  ";
		}
...
$rekaprptbmd = new rekaprptbmdObj();
