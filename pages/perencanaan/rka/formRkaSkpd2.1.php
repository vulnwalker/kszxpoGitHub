<?php
class formRkaSkpd21Obj  extends DaftarObj2{	
	var $Prefix = 'formRkaSkpd21';
	...
	var $FormName = 'formRkaSkpd21Form';
	var $noModul=14; 
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){	
		...
		case 'finish':{		
			...
			}elseif(mysql_num_rows(mysql_query("select * from temp_rka_21  where user ='$username' and o2 !='0' and delete_status = '0'")) == 0){
				$err = "Data kosong";
			}
			
			if(empty($err)){
				$execute = mysql_query("select * from temp_rka_21  where user ='$username' and o2 !='0'");
				while($rows = mysql_fetch_array($execute)){
					foreach ($rows as $key => $value) { 
					  $$key = $value; 
					}	
					
					$queryCekRekening = "select * from view_rka_2_1 where c1='0' and f1 = '0' and rincian_perhitungan = '' and k='$k' and l='$l' and m='$m' and n='$n' and o='$o' and id_tahap='$this->idTahap' ";
					...
					$queryCekPekerjaan = "select * from view_rka_2_1 where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and k='$k' and l='$l' and m='$m' and n='$n' and o='$o' and o1='$o1' and f1 = '0' and rincian_perhitungan = ''  and id_tahap='$this->idTahap' ";
					...
					if(mysql_num_rows(mysql_query($queryCekForUpdate)) > 0){
						...
						if(mysql_num_rows(mysql_query("select * from view_rka_2_1 where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and k='$lamaK' and l='$lamaL' and m='$lamaM' and n='$lamaN' and o='$lamaO' and o1='$lamaO1' and (rincian_perhitungan !='' or f1!='0') and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and id_tahap='$this->idTahap'")) == 0){
		...
		case 'saveEdit':{
	    	foreach ($_REQUEST as $key => $value) { 
			 ...
			 $ekseGetTempData = mysql_query("select * from temp_rka_21 where user = '$username' and o2 != '0'  and id !='$id' and delete_status = '0'");
			 $kondisiIDAwal = "";
			 ...
			 $getJumlahPaguDiTabelAnggaran = mysql_fetch_array(mysql_query("select sum(jumlah_harga) from view_rka_2_1  where id_tahap='$this->idTahap' and c1='$c1' and c='$c' and d='$d' and  e='$e' and e1='$e1'  and o1 !='0' and rincian_perhitungan !='' $kondisiIDAwal "));
			 ...
			 $o = $kodeRekening[4];
			 if($err == ''){
			 	 if(mysql_num_rows(mysql_query("select * from temp_rka_21 where  o1 ='$noPekerjaan' and rincian_perhitungan ='' and user ='$username'")) > 0){
   ...
   function setPage_OtherScript(){
		...
		return
			...
			"<script type='text/javascript' src='js/perencanaan/rka/formRkaSkpd2.1.js' language='JavaScript' ></script>".
			...
			genFilterBar(
				...
				genFilterBar(
					...
					<td class='border:none'> 
						<a class='toolbar' id='btsave' href='javascript:formRkaSkpd21.$tujuan'> 
					...
					<tbody><tr valign='middle' align='center'> 
					<td class='border:none'> 
						<a class='toolbar' id='btcancel' href='javascript:formRkaSkpd21.closeTab()'> 
					...
					<tbody><tr valign='middle' align='center'> 
					<td class='border:none'> 
						<a class='toolbar' id='finish' href='javascript:formRkaSkpd21.finish()'> 
	...
	function getDaftarOpsi($Mode=1){
		...
		$arrKondisi = array();		
		$username = $_COOKIE['coID'];
		$getAll = mysql_query("select * from temp_rka_21 where rincian_perhitungan ='' and user = '$username' and o1!='0'");
		while($rows = mysql_fetch_array($getAll)){
			foreach ($rows as $key => $value) { 
				 	 	$$key = $value; 
			}
			if(mysql_num_rows(mysql_query("select * from temp_rka_21 where user ='$username' and rincian_perhitungan !='' and o1='$o1' and delete_status = '0'")) == 0){
				mysql_query("delete from temp_rka_21 where id='$id'");						
			}else{
...
$formRkaSkpd21 = new formRkaSkpd21Obj();

$arrayResult = VulnWalkerTahap($formRkaSkpd21->modul);
...
$formRkaSkpd21->jenisForm = $jenisForm;
$formRkaSkpd21->nomorUrut = $nomorUrut;
$formRkaSkpd21->tahun = $tahun;
$formRkaSkpd21->jenisAnggaran = $jenisAnggaran;
$formRkaSkpd21->idTahap = $idTahap;
