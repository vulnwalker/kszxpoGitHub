...
class rkaSKPD21Obj  extends DaftarObj2{	
	var $Prefix = 'rkaSKPD21';
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='rkaSKPD21.xls';
	var $namaModulCetak='RKA';
	var $Cetak_Judul = 'RKA';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'rkaSKPD21Form';
	var $modul = "RKA-SKPD";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	...
	function genRowSum($ColStyle, $Mode, $Total){
		...
		$getData = mysql_fetch_array(mysql_query("select sum(jumlah_harga) from tabel_anggaran where o1 !='0' and (rincian_perhitungan !='' or f1 !='0' ) and id_tahap='$idTahap' and tahun ='$this->tahun' and jenis_anggaran = '$this->jenisAnggaran' and jenis_rka = '2.1' and nama_modul='RKA-SKPD' $kondisiSKPD $kondisiRekening"));
		...
		$ContentTotalHal=''; $ContentTotal='';
			...
			if($this->jenisForm == "PENYUSUNAN"){
				...
				$getDataSebelumnya = mysql_fetch_array(mysql_query("select sum(jumlah_harga) from view_rka_2_1 where o1 !='0' and (rincian_perhitungan !='' or f1 !='0' ) and no_urut='$urutSebelumnya' and tahun ='$this->tahun' and jenis_anggaran = '$this->jenisAnggaran'  $kondisiSKPD $kondisiRekening"));
				...
				if($TotalBertambahBerkurang   > 0){
					...
					$TotalBertambahBerkurang = $Total  -  $TotalSebelumnya;
					$TotalBertambahBerkurang = number_format($TotalBertambahBerkurang,2,',','.');
				}
				...
				"<tr>
					...
					<td class='GarisDaftar' align='right'><b><div  id='{$this->Prefix}_cont_sum'>".number_format($TotalSebelumnya,2,',','.')."</div></td>
	...
	$getJumlahBarang = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$rkaSKPD21_idplh'"));
	...
	$getPaguYangTelahTerpakai = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as paguYangTerpakai from view_rka_2_1 where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and bk='$bk' and ck='$ck' and p='$p' and q='$q' and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'  and no_urut = '$this->nomorUrut' and id_anggaran!='$rkaSKPD21_idplh' "));
	$sisaPaguIndikatif = $paguIndikatif - $getPaguYangTelahTerpakai['paguYangTerpakai'];
    
	 if(mysql_num_rows(mysql_query("select * from view_rka_2_1 where c1='0' and f1 = '0' and k = '$k' and l ='$l' and m='$m' and n='$n' and o='$o'  and id_tahap='$this->idTahap' ")) > 0){
 	 ...
 	 if(empty($cmbJenisRKAForm) ){
		...
		$query = VulnWalkerUpdate('tabel_anggaran',$data,"id_anggaran = '$rkaSKPD21_idplh'");
		mysql_query($query);
	 }
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){	
		...
		case 'sesuai':{
			...
			$getrkaSKPD21nya = mysql_fetch_array(mysql_query($queryRows));
			foreach ($getrkaSKPD21nya as $key => $value) { 
				  $$key = $value; 
			} 
			$cmbUrusanForm = $c1;
			$cmbBidangForm = $c;
		...
		case 'koreksi':{
			...
			foreach ($getrkaSKPD21nya as $key => $value) { 
			 ...
			 $getSisaPagu = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as paguYangTerpakai  from view_rka_2_1 where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and bk='$bk' and p='$p' and q='$q' and id_tahap='$this->idTahap' and concat(c1,'.',c,'.',d,'.',e,'.',e1,'.',bk,'.',ck,'.',p,'.',q,'.',f1,'.',f2,'.',f,'.',g,'.',h,'.',i,'.',j,'.',id_jenis_pemeliharaan,'.',k,'.',l,'.',m,'.',n,'.',o) != '$filterPagu'  and o1 !='0' and rincian_perhitungan !=''"));
			 $sisaPagu = $paguIndikatif - $getSisaPagu['paguYangTerpakai'];
			 $content .= "Sisa PAGU : ".$sisaPagu;
			 $content .= "getSIsaPAGU nya : "."select sum(jumlah_harga) as paguYangTerpakai  from view_rka_2_1 where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and bk='$bk' and p='$p' and q='$q' and id_tahap='$this->idTahap' and concat(c1,'.',c,'.',d,'.',e,'.',e1,'.',bk,'.',ck,'.',p,'.',q,'.',f1,'.',f2,'.',f,'.',g,'.',h,'.',i,'.',j,'.',id_jenis_pemeliharaan,'.',k,'.',l,'.',m,'.',n,'.',o) != '$filterPagu' ";
			
			 if($err == ""){
				 	if(mysql_num_rows(mysql_query("select * from view_rka_2_1 where o1='0' and rincian_perhitungan ='' and c1='0' and k='$k' and l='$l' and m='$m' and n='$n' and o='$o' and id_tahap='$this->idTahap'  ")) > 0){
					 ...
					 if(mysql_num_rows(mysql_query("select * from view_rka_2_1 where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and k = '$k' and l ='$l' and m='$m' and n='$n' and o='$o'  and o1='$o1' and f1='0' and rincian_perhitungan ='' and id_tahap='$this->idTahap' ")) > 0){
		...
		case 'editTab':{
			...
			$getAll = mysql_query("select * from view_rka_2_1 where concat(k,'.',l,'.',m,'.',n,'.',o) ='$kodeRek' and id_tahap='$this->idTahap' and rincian_perhitungan !=''   and c1='$cmbUrusan' and c='$cmbBidang' and d='$cmbSKPD' and e='$cmbUnit' and e1='$cmbSubUnit'  order by o1, rincian_perhitungan");
			mysql_query("delete from temp_rka_21 where user='$username'");
			mysql_query("delete from temp_rincian_volume_21 where user='$username'");
		    mysql_query("delete from temp_alokasi_rka_21 where user='$username'");
			$noUrutPekerjaan = 0;
			$angkaO2 = 0;
			$lastO1 = 0;
			while($rows = mysql_fetch_array($getAll)){
				...
				if($getLastO1['o1'] != $lastO1){
			 	...
			 	if($o1 != '0'){
					if(mysql_num_rows(mysql_query("select * from temp_rka_21 where  o1 ='$o1' and rincian_perhitungan ='' and user ='$username'")) > 0){
			 	...
			 	$data = array( 'c1' => $c1,
					...
					mysql_query(VulnWalkerInsert("temp_rka_21",$data)); 
				 }
				}
				$data = array(
								'c1' => $c1,
	...
	<A href=\"pages.php?Pg=rka-skpd-2.2.1\" title='RKA-SKPD MURNI' > RKA-SKPD 2.2.1 </a> |
	<A href=\"pages.php?Pg=rka-skpd-2.2\" title='RKA-SKPKD MURNI' > RKA-SKPD 2.2 </a> |
	<A href=\"pages.php?Pg=rka-skpd-2.1\" title='RKA-SKPKD MURNI'  style='color:blue;' > RKA-SKPD 2.1 </a> |
   ...
   function setPage_OtherScript(){
		...
		return 	
			...
			<script type='text/javascript' src='js/perencanaan/rka/rkaSKPD2.1.js' language='JavaScript' ></script> 
	 ...
	 $this->form_height = 400;
	  if ($this->form_fmST==0) {
		...
		foreach ($dt as $key => $value) { 
		 ...
		 $jenis_rka = $jenis_rka;
		 $cmbJenisRKA = cmbArray('cmbJenisRKAForm',$jenis_rka,$arrayJenisRKA,'-- JENIS RKA --','onchange=rkaSKPD21.unlockFindRekening();');
	 	 if(empty($jenis_rka)){
	  ...
	  $this->form_fields = array(
			...
			'kode7' => array( 
						...
						'value'=> "<input type='text' id='kodeRekening' name='kodeRekening' style='width:120px;' readonly value = '$kodeRekening'> &nbsp&nbsp
						 ...
						 <button type='button' id='findRekening' onclick=rkaSKPD21.findRekening('$jenis_rka'); $tergantungJenis> CARI </button> "
						 ),
			'kode8' => array( 
						'label'=>'HARGA SATUAN',
						'labelWidth'=>150, 
						'value'=> "<input type='text' name='hargaSatuan' id='hargaSatuan' value='$hargaSatuan' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='rkaSKPD21.bantu();' > <span id='bantu' style='color:red;'> </span>"
						 ),	
	...
	function setKolomData($no, $isi, $Mode, $TampilCheckBox){
	 ...
	 if($this->jenisForm == 'PENYUSUNAN'){
			 ...
			 if($c1 == '0'){
				...
				$ilustrasi = "";
				$getSumJumlahBarang = mysql_fetch_array(mysql_query("select sum(volume_rek) as total from view_rka_2_1 where k='$k' and l='$l' and m='$m' and n='$n' and o='$o' $kondisiSKPD and id_tahap='$this->idTahap'"));
				...
				$Koloms[] = array('align="right"',  );
			 	...
			 	if(empty($rincian_perhitungan) && $f1 == '0' ){
					...
					$getSumPekerjaan = mysql_fetch_array(mysql_query("select sum(jumlah_harga), sum(volume_rek), sum(jumlah) from view_rka_2_1 where k='$k' and l='$l' and m='$m' and n='$n' and o='$o' $kondisiSKPD and o1='$o1' and id_tahap='$this->idTahap' "));
			 ...
			 $getSumSatuanRek = mysql_fetch_array(mysql_query("select sum(jumlah) as total from view_rka_2_1 where k='$k' and l='$l' and m='$m' and n='$n'  and o='$o' $kondisiSKPD and id_tahap='$this->idTahap'"));
			 ...
			 if($c1 == '0'){
			 $getTotalJumalhHarga = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as total from view_rka_2_1 where k='$k' and l='$l' and m='$m' and n='$n' and o='$o' $kondisiSKPD and id_tahap='$this->idTahap'"));
	 ...
	 //TAHAP PENYUSUNAN
			 ...
			 if($c1 == '0'){
				...
				$ilustrasi = "";
				//$Koloms[] = array('align="left"',"" );
				$getSumJumlahBarang = mysql_fetch_array(mysql_query("select sum(volume_rek) as total from view_rka_2_1 where k='$k' and l='$l' and m='$m' and n='$n' and o='$o' $kondisiSKPD and no_urut ='$nomorBefore' and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'"));
				...
				$Koloms[] = array('align="right"',  );
			 	...
			 	if(empty($rincian_perhitungan) && $f1 == '0' ){
					...
					$getSumPekerjaan = mysql_fetch_array(mysql_query("select sum(jumlah_harga), sum(volume_rek), sum(jumlah) from view_rka_2_1 where k='$k' and l='$l' and m='$m' and n='$n' and o='$o' $kondisiSKPD and o1='$o1' and no_urut ='$nomorBefore' and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' "));
					$jumlah_harga = $getSumPekerjaan['sum(jumlah_harga)'];
					$volume_rek = $getSumPekerjaan['sum(volume_rek)'];
					$jumlah = $getSumPekerjaan['sum(jumlah)'];
					$Koloms[] = array('align="left"',"<span style='margin-left:5px;'>- ".$namaPekerjaan."</span>" );
			 ...
			 $Koloms[] = array('align="left"', $satuan_rek );
			 $getSumSatuanRek = mysql_fetch_array(mysql_query("select sum(jumlah) as total from view_rka_2_1 where k='$k' and l='$l' and m='$m' and n='$n'  and o='$o' $kondisiSKPD and no_urut ='$nomorBefore' and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'"));
			 $sumSatuanRek = $getSumSatuanRek['total'];
			 if($c1 == '0'){
			 	//$Koloms[] = array('align="right"', number_format($sumSatuanRek ,2,',','.') );
				$Koloms[] = array('align="right"','' );
			 ...
			 if($c1 == '0'){
			 $getTotalJumalhHarga = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as total from view_rka_2_1 where k='$k' and l='$l' and m='$m' and n='$n' and o='$o' $kondisiSKPD and no_urut ='$nomorBefore' and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'"));
			 $Koloms[] = array('align="right"', "<span style='font-weight:bold;'>".number_format($getTotalJumalhHarga['total'] ,2,',','.')."</span>" );
			 ...
			 $getAngkaKoreksi = mysql_fetch_array(mysql_query("select * from view_rka_2_1 where c1='$c1' and c='$c' and d='$d'  and e='$e' and e1='$e1'  and k='$k' and l='$l' and m='$m' and n='$n' and o='$o' and o1='$o1' and rincian_perhitungan ='$rincian_perhitungan'  and id_tahap='$this->idTahap'"));
			 ...
			 if(empty($rincian_perhitungan)){
			    ...
			    $Koloms[] = array('align="right"',"");
				if($o1 !='0'){
				 	$getTotalJumalhHargaKoreksi = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as total from view_rka_2_1 where k='$k' and l='$l' and m='$m' and n='$n' and o='$o' and o1='$o1' $kondisiSKPD and id_tahap ='$this->idTahap' and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'"));
				 	
					$Koloms[] = array('align="right"',number_format($getTotalJumalhHargaKoreksi['total'] ,2,',','.'));
				 }else{
				 	$getTotalJumalhHargaKoreksi = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as total from view_rka_2_1 where k='$k' and l='$l' and m='$m' and n='$n' and o='$o' $kondisiSKPD and id_tahap ='$this->idTahap' and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'"));
			 ...
			 $aksi  = "<img src='images/administrator/images/valid.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkaSKPD21.sesuai('$id_anggaran');></img>&nbsp  &nbsp <img src='images/administrator/images/invalid.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkaSKPD21.koreksi('$id_anggaran');></img> ";
			 if ($rincian_perhitungan == '') {
			 $aksi = "";
			 }
			 if(empty($rincian_perhitungan)){
	 	...
	 	if($this->jenisFormTerakhir == "KOREKSI"){
			 ...
			 $getAngkaKoreksi = mysql_fetch_array(mysql_query("select * from view_rka_2_1 where c1='$c1' and c='$c' and d='$d'  and e='$e' and e1='$e1'  and id_jenis_pemeliharaan='$id_jenis_pemeliharaan' and f1='$f1' and f2='$f2' and f='$f' and g='$g' and h='$h' and i='$i' and j='$j' and k='$k' and l='$l' and m='$m' and n='$n' and o='$o' and o1='$o1' and rincian_perhitungan ='$rincian_perhitungan'  and no_urut = '$this->urutTerakhir' and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'"));
			 $koreksiVolumeBarang = number_format($getAngkaKoreksi['volume_rek'] ,0,',','.');
			 $koreksiSatuanHarga = number_format($getAngkaKoreksi['jumlah'] ,2,',','.');
			 $koreksiJumlahHarga = number_format($getAngkaKoreksi['volume_rek']  * $getAngkaKoreksi['jumlah'] ,2,',','.');
			 if($getAngkaKoreksi['volume_rek'] > $isi['volume_rek'] ){
			 ...
			 if(empty($rincian_perhitungan)){
				...
				if($o1 !='0'){
					$getTotalJumalhHargaKoreksi = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as total from view_rka_2_1 where k='$k' and l='$l' and m='$m' and n='$n' and o='$o' and o1='$o1' $kondisiSKPD and no_urut ='$this->urutTerakhir' and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'"));
					$Koloms[] = array('align="right"', number_format($getTotalJumalhHargaKoreksi['total'],2,',','.') );
				}else{
					$getTotalJumalhHargaKoreksi = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as total from view_rka_2_1 where k='$k' and l='$l' and m='$m' and n='$n' and o='$o'  $kondisiSKPD and no_urut ='$this->urutTerakhir' and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'"));
					$Koloms[] = array('align="right"',"<span style='font-weight:bold;'>". number_format($getTotalJumalhHargaKoreksi['total'],2,',','.')."</span>");
				}
			 ...
			 if(empty($rincian_perhitungan)){
				 ...
				 if($o1 !='0'){
					...
					if($kurangLebihPekerjaan == 0){
				 	...
				 	$getTotalJumalhHargaKoreksi = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as total from view_rka_2_1 where k='$k' and l='$l' and m='$m' and n='$n' and o='$o' $kondisiSKPD and id_tahap ='$this->urutTerakhir' and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'"));
			 ...
			 if($c1 == '0'){
				...
				$ilustrasi = "";
				$Koloms[] = array('align="left"',"" );
				$getSumJumlahBarang = mysql_fetch_array(mysql_query("select sum(volume_rek) as total from view_rka_2_1 where k='$k' and l='$l' and m='$m' and n='$n' and o='$o' $kondisiSKPD and no_urut ='$this->urutTerakhir' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'"));
				...
				$Koloms[] = array('align="right"',  );
			 	...
			 	if(empty($rincian_perhitungan) && $f1 == '0' ){
					...
					$getSumPekerjaan = mysql_fetch_array(mysql_query("select sum(jumlah_harga), sum(volume_rek), sum(jumlah) from view_rka_2_1 where k='$k' and l='$l' and m='$m' and n='$n' and o='$o' $kondisiSKPD and o1='$o1' and no_urut ='$this->urutTerakhir' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' "));
					$jumlah_harga = $getSumPekerjaan['sum(jumlah_harga)'];
					$volume_rek = $getSumPekerjaan['sum(volume_rek)'];
					$jumlah = $getSumPekerjaan['sum(jumlah)'];
					$Koloms[] = array('align="left"',"<span style='margin-left:5px;'>- ".$namaPekerjaan."</span>" );
			 ...
			 if($c1 == '0'){
			 $getTotalJumalhHarga = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as total from view_rka_2_1 where k='$k' and l='$l' and m='$m' and n='$n' and o='$o' $kondisiSKPD and no_urut ='$this->urutTerakhir' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'"));
			 $Koloms[] = array('align="right"', "<span style='font-weight:bold;'>".number_format($getTotalJumalhHarga['total'] ,2,',','.')."</span>" );
			 }else{
			 	$Koloms[] = array('align="right"', number_format($jumlah_harga ,2,',','.') );
			 ...
			 if($c1 == '0'){
				...
				$getSumJumlahBarang = mysql_fetch_array(mysql_query("select sum(volume_rek) as total from view_rka_2_1 where k='$k' and l='$l' and m='$m' and n='$n' and o='$o' $kondisiSKPD and no_urut ='$nomorurutSebelumnya' and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'"));
				...
				$Koloms[] = array('align="right"', '' );
			 	...
			 	if(empty($rincian_perhitungan) && $f1 == '0' ){
					...
					$getSumPekerjaan = mysql_fetch_array(mysql_query("select sum(jumlah_harga), sum(volume_rek), sum(jumlah) from view_rka_2_1 where k='$k' and l='$l' and m='$m' and n='$n' and o='$o' $kondisiSKPD and o1='$o1' and no_urut ='$nomorurutSebelumnya' and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' "));
					$jumlah_harga = $getSumPekerjaan['sum(jumlah_harga)'];
					$volume_rek = $getSumPekerjaan['sum(volume_rek)'];
					$jumlah = $getSumPekerjaan['sum(jumlah)'];
					$Koloms[] = array('align="left"',"<span style='margin-left:5px;'>- ".$namaPekerjaan."</span>" );
			 ...
			 if($c1 == '0'){
			 $getTotalJumalhHarga = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as total from view_rka_2_1 where k='$k' and l='$l' and m='$m' and n='$n' and o='$o' $kondisiSKPD and no_urut ='$nomorurutSebelumnya' and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'"));
			 $Koloms[] = array('align="right"', "<span style='font-weight:bold;'>".number_format($getTotalJumalhHarga['total'] ,2,',','.')."</span>" );
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	foreach ($_COOKIE as $key => $value) { 
		...
		$urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=rkaSKPD21.refreshList(true);','-- URUSAN --');
		
		$codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
		$bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=rkaSKPD21.refreshList(true);','-- BIDANG --');
		
		$codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
		$skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=rkaSKPD21.refreshList(true);','-- SKPD --');
		...
		$unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=rkaSKPD21.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=rkaSKPD21.refreshList(true);','-- SUB UNIT --');
	...
	function getDaftarOpsi($Mode=1){
		...
		if($this->jenisForm == 'PENYUSUNAN'){
			...
			$getAllParent = mysql_query("select * from view_rka_2_1 where id_tahap='$this->idTahap'  and o1 ='0' ");
			$angka = 1;
			...
			while($rows = mysql_fetch_array($getAllParent)){
				...
				$cekRekening = mysql_num_rows(mysql_query("select * from view_rka_2_1 where id_tahap = '$this->idTahap' and k='$k' and l='$l' and m='$m' and n='$n' and o='$o' $kondisiSKPD and rincian_perhitungan != ''   "));
				if($cekRekening == 0){
					$concat = $k.".".$l.".".$m.".".$n.".".$o;
			...
			$nomorUrutSebelumnya = $this->nomorUrut - 1;
			$getJenisTahapSebelumnya = mysql_fetch_array(mysql_query("select * from view_rka_2_1 where no_urut = '$nomorUrutSebelumnya'  and tahun = '$this->tahun' and jenis_anggaran = '$this->jenisAnggaran' and o1 !='0' and  rincian_perhitungan !=''"));
			$jenisTahapSebelumnya = $getJenisTahapSebelumnya['jenis_form_modul'];
			$getAllTahapSebelumnya = mysql_query("select * from view_rka_2_1 where tahun = '$this->tahun' and jenis_anggaran = '$this->jenisAnggaran' and no_urut ='$nomorUrutSebelumnya' and o1 !='0' and (rincian_perhitungan !='' or f1 !='0' )  ");
			while($rows = mysql_fetch_array($getAllTahapSebelumnya)){
				if( $jenisTahapSebelumnya == "VALIDASI" && $rows['status_validasi'] != '1' ){
						...
						if(mysql_num_rows(mysql_query("select * from view_rka_2_1 where c1='$cmbUrusanForm' and c='$cmbBidangForm' and d='$cmbSKPDForm' and e='$cmbUnitForm' and e1='$cmbSubUnitForm' and k = '$k' and l ='$l' and m='$m' and n='$n' and o='$o'  and o1='$o1' and f1='0' and rincian_perhitungan ='' and id_tahap='$this->idTahap' ")) > 0){
				...
				if($cekRekening == 0){
					...
					//VulnWalker
						if(!empty($hiddenP)){
							...
							$getCheckingPekerjaan =  mysql_query("select * from view_rka_2_1 where concat(k,'.',l,'.',m,'.',n,'.',o) != '$concat' and id_tahap = '$this->idTahap' and o1 !='0' and rincian_perhitungan ='' and c1='$cmbUrusan' and c='$cmbBidang' and d='$cmbSKPD' and e='$cmbUnit' and e1='$cmbSubUnit'") ;
							while($chroot = mysql_fetch_array($getCheckingPekerjaan)){
								$chrootO1 = $chroot['o1'];
								$chrootIdAnggaran = $chroot['id_anggaran'];
								if(mysql_num_rows(mysql_query("select * from view_rka_2_1 where id_tahap ='$this->idTahap' and o1= '$chrootO1' and concat(k,'.',l,'.',m,'.',n,'.',o) != '$concat'  and rincian_perhitungan != '' $kondisiSKPD")) > 0){
			...
			$nomorUrutSebelumnya = $this->nomorUrut - 1;
			$getAllParent = mysql_query("select * from view_rka_2_1 where no_urut='$nomorUrutSebelumnya' and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'  and o1 ='0'  ");
			$angka = 1;	
			...
			while($rows = mysql_fetch_array($getAllParent)){
				...
				if($jenis_form_modul == "VALIDASI"){
					...
					$getDataValidasi = mysql_query("select * from view_rka_2_1 where no_urut='$nomorUrutSebelumnya' and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'  and o1 !='0' and (rincian_perhitungan !='' or f1 !='0')");
					while($got = mysql_fetch_array($getDataValidasi)){
						if($got['status_validasi'] !='1'){
							...
							$gotO = $got['o'];
							$gotO1 = $got['o1'];
							$getDataPekerjaanValidasi = mysql_fetch_array(mysql_query("select * from view_rka_2_1 where c1='$gotC1' and c='$gotC' and d='$gotD' and e='$gotE' and e1='$gotE1' and k='$gotK' and l='$gotL' and m='$gotM' and n='$gotN' and o ='$gotO' and o1='$gotO1' and rincian_perhitungan ='' and f1='0' and no_urut='$nomorUrutSebelumnya' and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'"));
							$idValidasiPekerjaan = $getDataPekerjaanValidasi['id_anggaran'];
							if(mysql_num_rows(mysql_query("select * from view_rka_2_1 where c1='$gotC1' and c='$gotC' and d='$gotD' and e='$gotE' and e1='$gotE1' and k='$gotK' and l='$gotL' and m='$gotM' and n='$gotN' and o ='$gotO' and o1='$gotO1'  and no_urut='$nomorUrutSebelumnya' and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and status_validasi ='1' and (rincian_perhitungan !='' or f1!='0')")) == 0){
								$arrKondisi[] = "id_anggaran !='$idValidasiPekerjaan'";
							}
							//$arrKondisi[] = "coba coba "."select * from view_rka_2_1 where c1='$gotC1' and c='$gotC' and d='$gotD' and e='$gotE' and e1='$gotE1' and k='$gotK' and l='$gotL' and m='$gotM' and n='$gotN' and o ='$gotO' and o1='$gotO1'  and no_urut='$nomorUrutSebelumnya' and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and (rincian_perhitungan !='' or f1!='0')"."       coba coba  ";
						}
					}				
				}
				$cekRekening = mysql_num_rows(mysql_query("select * from view_rka_2_1 where no_urut='$nomorUrutSebelumnya' and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and k='$k' and l='$l' and m='$m' and n='$n' and o='$o' $kondisiSKPD and rincian_perhitungan != '' $kondisiFilter   "));
				if($cekRekening == 0){
					$concat = $k.".".$l.".".$m.".".$n.".".$o;
					$arrKondisi[] = "concat(k,'.',l,'.',m,'.',n,'.',o) != '$concat'";
					...
					//VulnWalker
						if(!empty($hiddenP)){
							...
							$getCheckingPekerjaan =  mysql_query("select * from view_rka_2_1 where concat(k,'.',l,'.',m,'.',n,'.',o) != '$concat' and no_urut='$nomorUrutSebelumnya' and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and o1 !='0' and rincian_perhitungan ='' and c1='$cmbUrusan' and c='$cmbBidang' and d='$cmbSKPD' and e='$cmbUnit' and e1='$cmbSubUnit'") ;
							while($chroot = mysql_fetch_array($getCheckingPekerjaan)){
								$chrootO1 = $chroot['o1'];
								$chrootIdAnggaran = $chroot['id_anggaran'];
								if(mysql_num_rows(mysql_query("select * from view_rka_2_1 where no_urut='$nomorUrutSebelumnya' and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and o1= '$chrootO1' and concat(k,'.',l,'.',m,'.',n,'.',o) != '$concat'  and rincian_perhitungan != '' $kondisiSKPD")) > 0){
			...
			if($this->jenisFormTerakhir == "KOREKSI"){
				...
				$getAllParent = mysql_query("select * from view_rka_2_1 where no_urut='$this->urutTerakhir' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'  and o1 ='0'  ");
			$angka = 1;	
			...
			while($rows = mysql_fetch_array($getAllParent)){
				foreach ($rows as $key => $value) { 
			 	 $$key = $value; 
					...
					$cekRekening = mysql_num_rows(mysql_query("select * from view_rka_2_1 where no_urut='$this->urutTerakhir' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and k='$k' and l='$l' and m='$m' and n='$n' and o='$o' $kondisiSKPD and rincian_perhitungan != ''   "));
					if($cekRekening == 0){
						$concat = $k.".".$l.".".$m.".".$n.".".$o;
						$arrKondisi[] = "concat(k,'.',l,'.',m,'.',n,'.',o) != '$concat'";
						...
						//VulnWalker
							if(!empty($hiddenP)){
								...
								$getCheckingPekerjaan =  mysql_query("select * from view_rka_2_1 where concat(k,'.',l,'.',m,'.',n,'.',o) != '$concat' and no_urut='$this->urutTerakhir' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and o1 !='0' and rincian_perhitungan ='' and c1='$cmbUrusan' and c='$cmbBidang' and d='$cmbSKPD' and e='$cmbUnit' and e1='$cmbSubUnit'") ;
								while($chroot = mysql_fetch_array($getCheckingPekerjaan)){
									$chrootO1 = $chroot['o1'];
									$chrootIdAnggaran = $chroot['id_anggaran'];
									if(mysql_num_rows(mysql_query("select * from view_rka_2_1 where no_urut='$this->urutTerakhir' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and o1= '$chrootO1' and concat(k,'.',l,'.',m,'.',n,'.',o) != '$concat'  and rincian_perhitungan != '' $kondisiSKPD")) > 0){
				...
				while($rows = mysql_fetch_array($getAllParent)){
					...
					$cekRekening = mysql_num_rows(mysql_query("select * from view_rka_2_1 where no_urut='$nomorUrutSebelumnya' and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'  and k='$k' and l='$l' and m='$m' and n='$n' and o='$o' $kondisiSKPD and rincian_perhitungan != ''   "));
					if($cekRekening == 0){
						$concat = $k.".".$l.".".$m.".".$n.".".$o;
						$arrKondisi[] = "concat(k,'.',l,'.',m,'.',n,'.',o) != '$concat'";
...
function Laporan($xls =FALSE){
		...
		if($this->jenisForm == 'PENYUSUNAN'){
			...
			while($rows = mysql_fetch_array($getAllParent)){
				...
				if($jenis_form_modul == "VALIDASI"){
					$kondisiFilter = " and status_validasi = '1' ";
					$getDataValidasi = mysql_query("select * from view_rka_2_1 where no_urut='$nomorUrutSebelumnya' and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'  and o1 !='0' and (rincian_perhitungan !='' or f !='00')");
					while($got = mysql_fetch_array($getDataValidasi)){
						if($got['status_validasi'] !='1'){
							$gotID = $got['id_anggaran'];
							$arrKondisi[] = " id_anggaran !='$gotID' ";
							...
							$gotO = $got['o'];
							$gotO1 = $got['o1'];
							$getDataPekerjaanValidasi = mysql_fetch_array(mysql_query("select * from view_rka_2_1 where c1='$gotC1' and c='$gotC' and d='$gotD' and e='$gotE' and e1='$gotE1' and k='$gotK' and l='$gotL' and m='$gotM' and n='$gotN' and o ='$gotO' and o1='$gotO1' and rincian_perhitungan ='' and f='00' and no_urut='$nomorUrutSebelumnya' and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'"));
							$idValidasiPekerjaan = $getDataPekerjaanValidasi['id_anggaran'];
		...
		$qry ="select * from view_rka_2_1 where $Kondisi ";
		$aqry = mysql_query($qry);
		...
		while($daqry = mysql_fetch_array($aqry)){
			...
			if($o1 == 0){
				...
				$getSumJumlahHarga = mysql_fetch_array(mysql_query("select sum(jumlah_harga) from view_rka_2_1 where $Kondisi and k = '$k' and l='$l' and m='$m' and n='$n' and o='$o'  "));
				...
				$getSumJumlahHarga = mysql_fetch_array(mysql_query("select sum(jumlah_harga) from view_rka_2_1 where $Kondisi and o1 ='$o1'  "));
...
$rkaSKPD21 = new rkaSKPD21Obj();

$arrayResult = VulnWalkerTahap($rkaSKPD21->modul);
$jenisForm = $arrayResult['jenisForm'];
$nomorUrut = $arrayResult['nomorUrut'];
$tahun = $arrayResult['tahun'];
$jenisAnggaran = $arrayResult['jenisAnggaran'];
...
$rkaSKPD21->jenisForm = $jenisForm;
$rkaSKPD21->nomorUrut = $nomorUrut;
$rkaSKPD21->tahun = $tahun;
$rkaSKPD21->jenisAnggaran = $jenisAnggaran;
$rkaSKPD21->idTahap = $idTahap;


if(empty($rkaSKPD21->tahun)){
	...
	/*$rkaSKPD21->tahun = "select max(id_anggaran) as max from view_rka_2_1 where nama_modul = 'rkaSKPD21'";*/
	$rkaSKPD21->tahun  = $get2['tahun'];
	$rkaSKPD21->jenisAnggaran = $get2['jenis_anggaran'];
	$rkaSKPD21->urutTerakhir = $get2['no_urut'];
	$rkaSKPD21->jenisFormTerakhir = $get2['jenis_form_modul'];
	$rkaSKPD21->urutSebelumnya = $rkaSKPD21->urutTerakhir - 1;
	...
	$rkaSKPD21->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$rkaSKPD21->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];
	
	$arrayHasil =  VulnWalkerLASTTahap();
	$rkaSKPD21->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$rkaSKPD21->idTahap'"));
	$rkaSKPD21->currentTahap = $getCurrenttahap['nama_tahap'];
	
	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$rkaSKPD21->idTahap'"));
	$rkaSKPD21->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$rkaSKPD21->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
