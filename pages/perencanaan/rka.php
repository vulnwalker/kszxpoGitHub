...
class RKAObj  extends DaftarObj2{	
	...
	case 'formBaru':{				
	   ...
	   case 'kegiatan':{
			...
			$dk=$_REQUEST['dk'];	
				...
				{
					$kegiatan.="<option value='".$dt['q']."'>".$dt['q'].'. '.$dt['nama']."</option>";
				}
	   ...
	   case 'fmkegiatan':{
			...
			$queryqqq = "select * from ref_skpd_urusan WHERE c='".$fmBidang."' AND d='".$fmSKPD."'"; $cek .= $queryqqq;
				...
				while ($dt = mysql_fetch_array($hasil))
				{
					$fmkegiatan.="<option value='".$dt['q']."'>".$dt['q'].'. '.$dt['nama']."</option>";
				}
	   ...
	   case 'urusan':{
			...
			$ck=$_REQUEST['ck'];	
				//opsi bidang		
				$query = "SELECT ck,nm_urusan FROM ref_urusan WHERE bk='$bk' AND ck!='0' AND dk='0'"; //$cek .= $query;
				$hasil = mysql_query($query);
				$opsi = "<option value=''>-- PILIH Bidang --</option>";
				while ($dt = mysql_fetch_array($hasil))
				{
					$opsi.="<option value='".$dt['ck']."'>".$dt['ck'].'. '.$dt['nm_urusan']."</option>";
				}
				...
				$hasil2 = mysql_query($query2);
					...
					{
						$opsi2.="<option value='".$dt2['p']."'>".$dt2['p'].'. '.$dt2['nama']."</option>";
					}
	   ...
	   case 'bidang':{
			...
			$ck=$_REQUEST['ck'];	
				...
				$query = "SELECT dk,nm_urusan FROM ref_urusan WHERE bk='$bk' AND ck='$ck' AND dk!='0'"; //$cek .= $query;
				$hasil = mysql_query($query);
				$opsi = "<option value=''>-- PILIH Skpd --</option>";
				while ($dt = mysql_fetch_array($hasil))
				{
					$opsi.="<option value='".$dt['dk']."'>".$dt['dk'].'. '.$dt['nm_urusan']."</option>";
				}
	   ...
	   case 'skpd':{
			...
			$ck=$_REQUEST['ck'];			
				...
				//opsi program				
				$query2 = "SELECT p,nama FROM ref_program WHERE bk='$bk' AND ck='$ck' AND dk='0' and p<>0 and q='0' ";$cek .= $query;
				$hasil2 = mysql_query($query2);
					$opsi2 = "<option value=''>-- PILIH Program --</option>";
					while ($dt2 = mysql_fetch_array($hasil2))
					{
	   ...
	   case 'fmUrusan':{
			$fmUrusan = $_REQUEST['fmUrusan'];
				...
				$query = "SELECT ck,nm_urusan FROM ref_urusan WHERE bk='".$fmUrusan."' AND ck!='0' AND dk='0'"; $cek .= $query;
				$hasil = mysql_query($query);
				$fmskpd = "<option value=''>-- PILIH BIDANG --</option>";
				while ($dt = mysql_fetch_array($hasil))
				{
					$fmskpd.="<option value='".$dt['ck']."'>".$dt['ck'].'. '.$dt['nm_urusan']."</option>";
				}
		...
		break;
	  	...
	  	case 'fmbidang':{
			...
			$fmBidang = $_REQUEST['fmBidang'];
				...
				$query = "SELECT dk,nm_urusan FROM ref_urusan WHERE bk='$fmUrusan' AND ck='$fmBidang' AND dk!='0'"; $cek .= $query;
				$hasil = mysql_query($query);
				$fmskpd = "<option value=''>-- PILIH SKPD--</option>";
				while ($dt = mysql_fetch_array($hasil))
				{
					$fmskpd.="<option value='".$dt['dk']."'>".$dt['dk'].'. '.$dt['nm_urusan']."</option>";
				}
		...
		break;
	  	...
	  	case 'fmskpd':{
			...
			//$fmSKPD = (empty($_REQUEST['fmSKPD']))?'00':$_REQUEST['fmSKPD'];
				...
				while ($dt = mysql_fetch_array($hasil))
				{
					$fmprogram.="<option value='".$dt['p']."'>".$dt['p'].'. '.$dt['nama']."</option>";
				}				
  ...
  function Hapus(){
	...
	function setForm($dt){	
	 ...
	 $this->form_height = 500; 
	  if ($this->form_fmST==0) {
		...
		*/
		$querybk = "SELECT bk,nm_urusan FROM ref_urusan WHERE bk!='0' AND ck='0' AND dk='0'";
		$queryck = "SELECT ck,nm_urusan FROM ref_urusan WHERE bk='".$dt['bk']."' AND ck!='0' AND dk='0'";
		$querydk = "SELECT dk,nm_urusan FROM ref_urusan WHERE bk='".$dt['bk']."' AND ck='".$dt['ck']."' AND dk!='0'";
	...
	function genDaftarOpsi(){
		...
		$querybk = "SELECT bk,nm_urusan FROM ref_urusan WHERE bk!='0' AND ck='0' AND dk='0'";
		$queryck = "SELECT ck,nm_urusan FROM ref_urusan WHERE bk='$fmUrusan' AND ck!='0' AND dk='0'";
		$querydk = "SELECT dk,nm_urusan FROM ref_urusan WHERE bk='$fmUrusan' AND ck='$fmBidang' AND dk!='0'";
		
		//query Bidang		
		$queryBidang = "SELECT ck,nm_urusan FROM ref_urusan ";
 		//query SKPD
		$querySKPD = "SELECT dk,nm_urusan FROM ref_urusan WHERE bk='".$dt['bk']."' AND ck='".$dt['ck']."' AND dk!='0'";
