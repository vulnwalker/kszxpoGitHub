...
class rekaprphbmd_skpdObj  extends DaftarObj2{	
	var $Prefix = 'rekaprphbmd_skpd';
	...
	var $pagePerHal ='';
	var $cetak_xls=TRUE ;
	var $fileNameExcel='Rekap RPHBMD SKPD.xls';
	var $Cetak_Judul = 'REKAP RPHBMD SKPD';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	
	var $FormName = 'rekaprphbmd_skpdForm'; 	
			
	function setTitle(){
		return 'Rekap Rencana Penghapusan Barang Milik Daerah (SKPD)';
	}
	...
	function set_selector_other($tipe){
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){
		...
		case 'BidangAfter':{
			$content= $this->cmbQuerySKPD('fmSKPDskpd',$fmSKPDskpd,'','onchange=rekaprphbmd_skpd.refreshList(true)','--- Pilih SKPD ---','00');
		break;
	    }
		case 'SKPDAfter':{
	...
	function setNavAtas(){
		...
		return
			...
			<tr>
			<td class="menudottedline" width="60%" height="20" style="text-align:right"><b>
				<a href="pages.php?Pg=rphbmd" title="Rencana Penghapusan Barang Milik Daerah">RHBMD</a> |	
				<a href="pages.php?Pg=rekaprphbmd" title="Rekap Rencana Penghapusan Barang Milik Daerah">Rekap RHBMD</a> |
				<a style="color:blue;" href="pages.php?Pg=rekaprphbmd_skpd" title="Rencana Penghapusan Barang Milik Daerah">Rekap RHBMD (SKPD)</a>
				  &nbsp;&nbsp;&nbsp;
			</b></td>
			</tr>
			</table>';
	...
	$TampilOpt = 
			...
			"<table><tr><td width='100'>Bidang</td><td width='10'>:</td><td>".
				$this->cmbQueryBidang('fmSKPDBidang',$fmSKPDBidang,'','onchange=rekaprphbmd_skpd.BidangAfter() '.$disabled1,'--- Pilih BIDANG ---','00')."</td></tr>".
			"<tr><td width='100'>SKPD</td><td width='10'>:</td><td>".
				$this->cmbQuerySKPD('fmSKPDskpd',$fmSKPDskpd,'','onchange=rekaprphbmd_skpd.SKPDAfter() '.$disabled1,'--- Pilih SKPD ---','00').
			"</td></tr></table>".
			"</td>
			</tr></table>".
	...
	function setDaftar_query($Kondisi='', $Order='', $Limit=''){		
		...
		(
		select c,d,  
		sum(if(f='01',1,0)) as jml_brg_a,sum(if(f='01',harga,0)) as jml_hrg_a,
		sum(if(f='02',1,0)) as jml_brg_b,sum(if(f='02',harga,0)) as jml_hrg_b,
		sum(if(f='03',1,0)) as jml_brg_c,sum(if(f='03',harga,0)) as jml_hrg_c,
		sum(if(f='04',1,0)) as jml_brg_d,sum(if(f='04',harga,0)) as jml_hrg_d,
		sum(if(f='05',1,0)) as jml_brg_e,sum(if(f='05',harga,0)) as jml_hrg_e,
		sum(if(f='06',1,0)) as jml_brg_f,sum(if(f='06',harga,0)) as jml_hrg_f,
		sum(if(f='07',1,0)) as jml_brg_g,sum(if(f='07',harga,0)) as jml_hrg_g
...
$rekaprphbmd_skpd = new rekaprphbmd_skpdObj();
