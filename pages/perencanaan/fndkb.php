...
class DkbObj extends DaftarObj2{
	...
	/*function setMenuView(){
 	...
 	function setFormBaru(){
		...
		$dt['ref_id']=$qr['id'];
		//query program
		$qrqp = "SELECT p,nama FROM ref_program WHERE bk='".$qr['bk']."' AND ck='".$qr['ck']."' AND dk='".$qr['dk']."' AND p='".$qr['p']."' AND q='0'";
		$cek .= ' qrqp = '.$qrqp;
		$qp=mysql_fetch_array(mysql_query($qrqp));
		//query Kegiatan
		$qrqr = "SELECT q,nama FROM ref_program WHERE bk='".$qr['bk']."' AND ck='".$qr['ck']."' AND dk='".$qr['dk']."' AND p='".$qr['p']."' AND q='".$qr['q']."'";
		$cek .= ' qrqr= '.$qrqr;
		$qq=mysql_fetch_array(mysql_query($qrqr));
	  ...
	  if ($this->form_fmST==0) {
		...
		if ($Main->VERSI_NAME == 'PANDEGLANG'){
			...
			$queryProgram = "SELECT p,nama FROM ref_program WHERE bk='".$dt['bk']."' AND ck='".$dt['ck']."' AND dk='0' AND p!='0' AND q='0'";
 			//query Kegiatan
			$queryKegiatan = "SELECT q,nama FROM ref_program WHERE bk='".$dt['bk']."' AND ck='".$dt['ck']."' AND dk='0' AND p='".$dt['program_dkbmd']."' AND q!='0'";				
		}else{
			//query program
			$queryProgram = "SELECT p,nama FROM ref_program WHERE bk='".$dt['bk']."' AND ck='".$dt['ck']."' AND dk='0' AND p!='0' AND q='0'";
 			//query Kegiatan
	 ...
	 //items ----------------------
	  $this->form_fields = array(
			...
			'bidang' => array( 
							'label'=>'BIDANG',
							'labelWidth'=>100, 
							'value'=>"<input type='text' name='bidang' value='".$dt['bidang']."' size='70px' id='bidang' readonly>&nbsp&nbsp
			...
			'skpd' => array( 
							'label'=>'SKPD',
							'labelWidth'=>100, 
							'value'=>"<input type='text' name='skpd' value='".$dt['skpd']."' size='70px' id='skpd' readonly>&nbsp&nbsp
			...
			'unit' => array( 
							'label'=>'UNIT',
							'labelWidth'=>100, 
							'value'=>"<input type='text' name='unit' value='".$dt['unit']."' size='70px' id='unit' readonly>&nbsp&nbsp
			...
			'subunit' => array( 
							'label'=>'SUB UNIT',
							'labelWidth'=>100, 
							'value'=>"<input type='text' name='subunit' value='".$dt['subunit']."' size='70px' id='subunit' readonly>&nbsp&nbsp
			...
			'program_dkbmd' => array( 
						...
						"<div style='display:none'>$queryProgram</div>",
						),
