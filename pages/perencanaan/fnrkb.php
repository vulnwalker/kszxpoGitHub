...
class RkbObj extends DaftarObj2{
	...
	function setForm($dt){	
		...
		//$paramskpd = "&c={$dt['c']}&d={$dt['d']}&e={$dt['e']}&e1={$dt['e1']}";
		$vkdbarang = //$this->form_fmST==1?	$fmIDBARANG.' - '.$dt['nm_barang'] :
			cariInfo3("adminForm","pages/01/caribarang1.php","pages/01/caribarang2a.php",
					"fmIDBARANG",
					"fmNMBARANG",
					"readonly","$DisAbled",'',//'');
					$paramskpd);
	...
	function simpan2(){
		...
		//thn_akun
		$akn=mysql_fetch_array(mysql_query("select * from ref_jurnal where concat(ka,'.',kb,'.',kc,'.',kd,'.',ke,'.',kf)='".$_REQUEST['fmIDREKENING']."'"));
		...
		if($err==''){
			if($fmST == 0){//baru
				$aqry = 
					...
					(merk_barang,jml_barang,jml_bi,jml_standar,thn_akun, harga,satuan,jml_harga,ket,a,b,c,d,e,e1,f,g,h,i,j,k,l,m,n,o,kf,tahun,
					uid,tgl_update,nm_barang,nm_account,jml_max) 
					values 
					('$merk_barang','$jml_barang', '$jml_bi', '$jml_standar','$thn_akun','$harga','$satuan','$tot_harga','$ket',
