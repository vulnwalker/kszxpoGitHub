...
class rekaprkpbmd_skpdObj  extends DaftarObj2{	
	var $Prefix = 'rekaprkpbmd_skpd';
	var $elCurrPage="HalDefault";
	var $SHOW_CEK = TRUE;
	var $TblName = 'rkpb'; //daftar
	var $TblName_Hapus = 'rpkb';
	var $MaxFlush = 10;
	var $TblStyle = array( 'koptable', 'cetak','cetak'); //berdasar mode
	var $ColStyle = array( 'GarisDaftar', 'GarisCetak','GarisCetak');
	var $KeyFields = array('c','d');
	var $FieldSum = array('jml_brg_a','jml_biaya_a',
						  'jml_brg_b','jml_biaya_b',
						  'jml_brg_c','jml_biaya_c',
						  'jml_brg_d','jml_biaya_d',
						  'jml_brg_e','jml_biaya_e',
						  'jml_brg_f','jml_biaya_f',
						  'jml_brg_g','jml_biaya_g',
						  'tot_brg','tot_biaya');//array('jml_harga');
	...
	var $pagePerHal ='';
	var $cetak_xls=TRUE ;
	var $fileNameExcel='Rekap RKPBMD SKPD.xls';
	var $Cetak_Judul = 'REKAP RKPBMD SKPD';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	
	var $FormName = 'rekaprkpbmd_skpdForm'; 	
			
	function setTitle(){
		return 'Rekap Rencana Kebutuhan Pemeliharaan Barang Milik Daerah (SKPD)';
	}
	...
	function set_selector_other($tipe){
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){
		...
		case 'BidangAfter':{
			$content= $this->cmbQuerySKPD('fmSKPDskpd',$fmSKPDskpd,'','onchange=rekaprkpbmd_skpd.refreshList(true)','--- Pilih SKPD ---','00');
		break;
	    }
		case 'SKPDAfter':{
	...
	function setNavAtas(){
		...
		return
			...
			<td class="menudottedline" width="60%" height="20" style="text-align:right"><b>
				...
				<a style="color:blue;"  href="pages.php?Pg=rekaprkpbmd_skpd" title="Rencana Kebutuhan Pemeliharaan Barang Milik Daerah">Rekap RKPBMD (SKPD)</a>  |	
				<a href="pages.php?Pg=rka&jns=1" title="Rekap Rencana Kebutuhan Pemeliharaan Barang Milik Daerah">RKA</a> | 		
				<a href="pages.php?Pg=dkpb" title="Daftar Kebutuhan Barang Milik Daerah">DKPBMD</a>  |  
				<a href="pages.php?Pg=rekapdkpb" title="Rekap Daftar Kebutuhan Barang Milik Daerah">Rekap DKPBMD</a>  |  
				<a href="pages.php?Pg=rekapdkpb_skpd" title="Daftar Kebutuhan Pemeliharaan Barang Milik Daerah">Rekap DKPBMD (SKPD)</a>
				  &nbsp;&nbsp;&nbsp;
			</b></td>
			</tr>
			</table>';
	...
	function setKolomData($no, $isi, $Mode, $TampilCheckBox){
	 ...
	 global $Main;
		...
		$Koloms[] = array('align=right', number_format($isi['jml_biaya_a'],2,',','.'));
		$Koloms[] = array('align=right', number_format($isi['jml_brg_b'],0,',','.'));
		$Koloms[] = array('align=right', number_format($isi['jml_biaya_b'],2,',','.'));
		$Koloms[] = array('align=right', number_format($isi['jml_brg_c'],0,',','.'));
		$Koloms[] = array('align=right', number_format($isi['jml_biaya_c'],2,',','.'));
		$Koloms[] = array('align=right', number_format($isi['jml_brg_d'],0,',','.'));
		$Koloms[] = array('align=right', number_format($isi['jml_biaya_d'],2,',','.'));
		$Koloms[] = array('align=right', number_format($isi['jml_brg_e'],0,',','.'));
		$Koloms[] = array('align=right', number_format($isi['jml_biaya_e'],2,',','.'));
		$Koloms[] = array('align=right', number_format($isi['jml_brg_f'],0,',','.'));
		$Koloms[] = array('align=right', number_format($isi['jml_biaya_f'],2,',','.'));
		$Koloms[] = array('align=right', number_format($isi['jml_brg_g'],0,',','.'));
		$Koloms[] = array('align=right', number_format($isi['jml_biaya_g'],2,',','.'));
		$Koloms[] = array('align=right', number_format($isi['tot_brg'],0,',','.'));
		$Koloms[] = array('align=right', number_format($isi['tot_biaya'],2,',','.'));
		return $Koloms;
	}
	...
	$TampilOpt = 
			...
			"<table><tr><td width='100'>Bidang</td><td width='10'>:</td><td>".
				$this->cmbQueryBidang('fmSKPDBidang',$fmSKPDBidang,'','onchange=rekaprkpbmd_skpd.BidangAfter() '.$disabled1,'--- Pilih BIDANG ---','00')."</td></tr>".
			"<tr><td width='100'>SKPD</td><td width='10'>:</td><td>".
				$this->cmbQuerySKPD('fmSKPDskpd',$fmSKPDskpd,'','onchange=rekaprkpbmd_skpd.SKPDAfter() '.$disabled1,'--- Pilih SKPD ---','00').
			"</td></tr></table>".
			"</td>
			</tr></table>".
	...
	function setDaftar_query($Kondisi='', $Order='', $Limit=''){		
		...
		bb.jml_brg_a,bb.jml_biaya_a,bb.jml_brg_b,bb.jml_biaya_b,bb.jml_brg_c,bb.jml_biaya_c,
		bb.jml_brg_d,bb.jml_biaya_d,bb.jml_brg_e,bb.jml_biaya_e,bb.jml_brg_f,bb.jml_biaya_f,
		bb.jml_brg_g,bb.jml_biaya_g,
		(bb.jml_brg_a+bb.jml_brg_b+bb.jml_brg_c+bb.jml_brg_d+bb.jml_brg_e+bb.jml_brg_f+bb.jml_brg_g) as tot_brg,
		(bb.jml_biaya_a+bb.jml_biaya_b+bb.jml_biaya_c+bb.jml_biaya_d+bb.jml_biaya_e+bb.jml_biaya_f+bb.jml_biaya_g) as tot_biaya
		from ref_skpd aa
		left join
		(
		select c,d,  
		sum(if(f='01',jml_barang,0)) as jml_brg_a,sum(if(f='01',jml_biaya,0)) as jml_biaya_a,
		sum(if(f='02',jml_barang,0)) as jml_brg_b,sum(if(f='02',jml_biaya,0)) as jml_biaya_b,
		sum(if(f='03',jml_barang,0)) as jml_brg_c,sum(if(f='03',jml_biaya,0)) as jml_biaya_c,
		sum(if(f='04',jml_barang,0)) as jml_brg_d,sum(if(f='04',jml_biaya,0)) as jml_biaya_d,
		sum(if(f='05',jml_barang,0)) as jml_brg_e,sum(if(f='05',jml_biaya,0)) as jml_biaya_e,
		sum(if(f='06',jml_barang,0)) as jml_brg_f,sum(if(f='06',jml_biaya,0)) as jml_biaya_f,
		sum(if(f='07',jml_barang,0)) as jml_brg_g,sum(if(f='07',jml_biaya,0)) as jml_biaya_g
		 from rkpb 
		 $kondSKPD 
		 group by c,d
		) bb
...
$rekaprkpbmd_skpd = new rekaprkpbmd_skpdObj();
