...
class rekaprkbObj  extends DaftarObj2{	
	var $Prefix = 'rekaprkb';
	...
	var $Cetak_Judul = 'Rekap RKBMD';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'rekaprkbForm'; 	
	...
	function setNavAtas(){
		...
		return
			...
			<td class="menudottedline" width="60%" height="20" style="text-align:right"><b>
				...
				<a style="color:blue;" href="pages.php?Pg=rekaprkb" title="Rekap Rencana Kebutuhan Barang Milik Daerah">Rekap RKBMD</a> |
...
//ifNULL(bb.tahun,'0000') as tahun
		...
		switch($fmLevel){
			case '1':case '2':case '3':default:{
				if($kode1!=0  && $kode2!=0 && $kode3==0){
					...
					$qh=mysql_fetch_array(mysql_query("select sum(jml_barang) as jml_barang, sum(jml_harga) as jml_harga from rkb WHERE k='$kode1' and l='$kode2' and tahun='$fmThnAnggaran' $KondThnAkun $KondSKPD"));
			...
			case '4':{
				if($kode1!=0  && $kode2!=0 && $kode3==0){
					...
					$qh=mysql_fetch_array(mysql_query("select sum(jml_barang) as jml_barang, sum(jml_harga) as jml_harga from rkb WHERE k='$kode1' and l='$kode2' and m='$kode3' and tahun='$fmThnAnggaran' $KondThnAkun $KondSKPD"));
					$jumlah_barang=$qh['jml_barang'];	
					$harga_barang=$qh['jml_harga'];		
				}else{
					$jumlah_barang=$isi['jml_barang'];	
			...
			case '5':{
				if($kode1!=0  && $kode2!=0 && $kode3==0){
					...
					//query get jml_harga level 3 selanjutnya
					$qh=mysql_fetch_array(mysql_query("select sum(jml_barang) as jml_barang, sum(jml_harga) as jml_harga from rkb WHERE k='$kode1' and l='$kode2' and m='$kode3' and n='$kode4' and tahun='$fmThnAnggaran' $KondThnAkun $KondSKPD"));
					$jumlah_barang=$qh['jml_barang'];	
					$harga_barang=$qh['jml_harga'];		
				}else{
					$jumlah_barang=$isi['jml_barang'];	
	...
	$Koloms[] = array('align="right"',$bold.number_format($jumlah_barang,0,',','.').$bold_ttp);	 	 	 	 
	...
	function setSumHal2_query($Kondisi, $fsum){
		...
		if($berdasarkan==2){
			return "select $fsum from rkb WHERE $KondTahun $KondSKPD $KondThnAkun and concat(k, '.', l, '.', m, '.', n, '.', o, '.', kf) like '$kdakun%' ";	
		}else{
			return "select $fsum from rkb WHERE $KondTahun $KondSKPD $KondThnAkun  ";
		}
