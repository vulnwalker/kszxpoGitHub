...
class rekaprphbmdObj  extends DaftarObj2{	
	var $Prefix = 'rekaprphbmd';
	...
	var $FieldSum = array('jml_barang','harga');//array('jml_harga');
	var $SumValue = array();
	...
	var $Cetak_Judul = 'Rekap RPHBMD';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'rekaprphbmdForm'; 	
	...
	function setNavAtas(){
		...
		return
			...
			<td class="menudottedline" width="60%" height="20" style="text-align:right"><b>
				...
				<a style="color:blue;" href="pages.php?Pg=rphbmd" title="Penghapusan">Penghapusan</a> '.
					$persediaan.
				  '&nbsp;&nbsp;&nbsp;
			</b></td>
			</tr>
			<tr>
			<td class="menudottedline" width="60%" height="20" style="text-align:right"><b>
				<a href="pages.php?Pg=rphbmd" title="Rencana Penghapusan Barang Milik Daerah">RPHBMD</a>  |
				<a style="color:blue;" href="pages.php?Pg=rekaprphbmd" title="Rekap Rencana Penghapusan Barang Milik Daerah">Rekap RPHBMD</a>  |	
				<a href="pages.php?Pg=rekaprphbmd_skpd" title="Rekap Rencana Penghapusan Barang Milik Daerah">Rekap RPHBMD (SKPD)</a>  		
				  &nbsp;&nbsp;&nbsp;
			</b></td>
			</tr>
			</table>';
	...
	function setDaftar_query($Kondisi='', $Order='', $Limit=''){
		...
		//Kondisi Tahun
		if(!empty($fmThnAnggaran)) $KondTahun = "thn_anggaran='$fmThnAnggaran'";
		//===================================Kondisi SKPD===================================================//
		$arrKondSKPD=array();
...
//		if( $fmBerdasarkan == 2 ) $arrKondisi[] = " concat(ka, '.', kb, '.', kc, '.', kd, '.', ke) like '$kdakun%' ";	
		...
		switch($fmLevel){
			...
			case '2' : {
				...
				$KondJoinAkun='aa.ka=bb.ka and aa.kb=bb.kb';
				$GroupbyBarang='f,g'; 
				$GroupbyAkun='ka,kb'; 
				break;
			}
			case '3' : {
				...
				$KondJoinAkun='aa.ka=bb.ka and aa.kb=bb.kb and aa.kc=bb.kc';
				$GroupbyBarang='f,g,h'; 
			...
			case '4' : {
				...
				$KondJoinAkun='aa.ka=bb.ka and aa.kb=bb.kb and aa.kc=bb.kc and aa.kd=bb.kd';
			...
			default: case '5' : {
				...
				$KondJoinAkun='aa.ka=bb.ka and aa.kb=bb.kb and aa.kc=bb.kc and aa.kd=bb.kd and aa.ke=bb.ke';
		...
		if($berdasarkan==2){
			$aqry = "select '$fmSKPD' as c,'$fmUNIT' as d, aa.ka, aa.kb, aa.kc, aa.kd, aa.ke, aa.kf, aa.nm_account,bb.jml_barang, bb.harga, bb.ket, $fmThnAnggaran as thn_anggaran ".
//ifNULL(bb.tahun,'0000') as tahun
					...
					"LEFT JOIN ".
					"(select c,d,ifNULL(ka,'ftot') as ka, ifNULL(kb,'0') as kb, ifNULL(kc,'0') as kc, ifNULL(kd,'0') as kd, ifNULL(ke,'0') as ke, ifNULL(kf,'0') as kf, ".
					"count(*) as jml_barang, sum(harga) as harga, ket, thn_anggaran ".
					"from t_rencana_penghapusan ". 
			...
			$aqry = "select '$fmSKPD' as c, '$fmUNIT' as d,aa.f,aa.g,aa.h,aa.i,aa.j,
					aa.nm_barang,bb.jml_barang,bb.harga,bb.ket, ".
					" $fmThnAnggaran as thn_anggaran ".
//ifNULL(bb.tahun,'0000') as tahun
				"from (
	...
	function genDaftar($Kondisi='', $Order='', $Limit='', $noAwal = 0, $Mode=1, $vKondisi_old=''){
		...
		$berdasarkan=$_REQUEST['fmBerdasarkan'];
		if($berdasarkan==2){
			$this->KeyFields=array('c','d','ka','kb','kc','kd','ke','thn_anggaran');				
			$KondIf=$isi['ka']!='0' && $isi['kb']=='0' && $isi['kc']=='0' && $isi['kd']=='0' && $isi['ke']=='0';
		}else{
			$this->KeyFields=array('c','d','f','g','h','i','j','thn_anggaran');
		...
		switch($fmLevel){
			case '1':case '2':case '3':default:{
				if($kode1!=0  && $kode2!=0 && $kode3==0){
					...
					$qh=mysql_fetch_array(mysql_query("select count(*) as jml_barang, sum(harga) as harga from t_rencana_penghapusan WHERE ka='$kode1' and kb='$kode2' and thn_anggaran='$fmThnAnggaran' $KondThnAkun $KondSKPD"));
					$jumlah_barang=$qh['jml_barang'];	
					$harga_barang=$qh['harga'];		
				}else{
					$jumlah_barang=$isi['jml_barang'];	
					$harga_barang=$isi['harga'];				
				}				
			} break;
			case '4':{
				if($kode1!=0  && $kode2!=0 && $kode3==0){
					...
					$qh=mysql_fetch_array(mysql_query("select count(*) as jml_barang, sum(harga) as harga from t_rencana_penghapusan WHERE ka='$kode1' and kb='$kode2' and kc='$kode3' and thn_anggaran='$fmThnAnggaran'$KondThnAkun $KondSKPD"));
					$jumlah_barang=$qh['jml_barang'];	
					$harga_barang=$qh['harga'];		
				}else{
					$jumlah_barang=$isi['jml_barang'];	
			...
			case '5':{
				if($kode1!=0  && $kode2!=0 && $kode3==0){
					...
					//query get jml_harga level 3 selanjutnya
					$qh=mysql_fetch_array(mysql_query("select count(*) as jml_barang, sum(harga) as harga from t_rencana_penghapusan WHERE ka='$kode1' and kb='$kode2' and kc='$kode3' and kd='$kode4' and thn_anggaran='$fmThnAnggaran' $KondThnAkun $KondSKPD"));
					$jumlah_barang=$qh['jml_barang'];	
					$harga_barang=$qh['harga'];		
				}else{
					$jumlah_barang=$isi['jml_barang'];	
	...
	function setSumHal2_query($Kondisi, $fsum){
		...
		if($berdasarkan==2){
			return "select $fsum from (select *, 
					'1' as jml_barang 
					from t_rencana_penghapusan WHERE $KondTahun $KondSKPD $KondThnAkun and concat(ka, '.', kb, '.', kc, '.', kd, '.', ke, '.', kf) like '$kdakun%') as aa ";	
		}else{
			return "select $fsum from (select *, 
					'1' as jml_barang 
					from t_rencana_penghapusan WHERE $KondTahun $KondSKPD $KondThnAkun) as aa  ";
		}
...
$rekaprphbmd = new rekaprphbmdObj();
