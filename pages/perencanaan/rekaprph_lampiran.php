...
class Rekaprph_lampiranObj  extends DaftarObj2{	
	var $Prefix = 'Rekaprph_lampiran';
	...
	var $SumValue = array();
	var $fieldSum_lokasi = array( 3,4); 
	var $FieldSum_Cp1 = array( 2, 2, 2);//berdasar mode
	var $FieldSum_Cp2 = array( 0, 0, 0);	
	var $checkbox_rowspan = 1;
	var $PageTitle = 'Lampiran Rekap RPHBMD ';
	var $PageIcon = 'images/perencanaan_ico.png';
	var $ico_width = '';
	var $ico_height = '';	
	var $pagePerHal ='';
	var $cetak_xls=TRUE ;
	var $fileNameExcel='rekaprph_lampiran.xls';
	var $Cetak_Judul = 'Rekap RPH Lampiran';	
	var $Cetak_Mode=1;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'Rekaprph_lampiran_form'; 
	var $totalCol = 4;	
	
	function setTitle(){
		return 'Rekap RPH Lampiran';
	}	
	...
	function setDaftar_query($Kondisi='', $Order='', $Limit=''){		
		$aqry = "select id,c,d,count(*) as jml_barang, sum(harga) as jml_harga from $this->TblName $Kondisi $Order $Limit ";	
		return $aqry;		
	}
	
	function setSumHal_query($Kondisi, $fsum){
		//return "select $fsum from $this->TblName $Kondisi "; //echo $aqry;
		return "select $fsum from (select id,c,d,count(*) as jml_barang, sum(harga) as jml_harga from $this->TblName $Kondisi ) aa "; //echo $aqry;
   ...
   function setKolomHeader($Mode=1, $Checkbox=''){
	...
	function setKolomData($no, $isi, $Mode, $TampilCheckBox){
	 ...
	 //$query_rkb="SELECT SUM(jml_barang) as jml_barang,SUM(jml_harga) as jml_harga FROM $this->TblName where c='$c' and d='$d' and tahun='$tahun' GROUP BY e";
	 //$query_rkb="SELECT SUM(jml_barang) as jml_barang,SUM(jml_harga) as jml_harga FROM $this->TblName where c='$c' and d='$d' and f='$f' and g='$g' and h='$h' and i='$i' and j='$j' and tahun='$tahun' GROUP BY c,d,e";
	 //$dt=mysql_fetch_array(mysql_query($query_rkb));
	 ...
	 //$get_jml=mysql_fetch_array(mysql_query("select sum(jml_barang) from rkb where 
	 //c='$c' and d='$d' and f='$f' and g='$g' and h='$h' and i='$i' and j='$j' GROUB BY e"));	
	 $Koloms = array();
	...
	function getDaftarOpsi($Mode=1){
		...
		if(!($k=='' || $k=='00') ) $arrKondisi[] = "ka='$k'";	
		if(!($l=='' || $l=='00') ) $arrKondisi[] = "kb='$l'";	
		if(!($m=='' || $m=='00') ) $arrKondisi[] = "kc='$m'";	
		if(!($n=='' || $n=='00') ) $arrKondisi[] = "kd='$n'";	
		if(!($o=='' || $o=='00') ) $arrKondisi[] = "ke='$o'";	
		if(!($tahun=='' || $tahun=='00') ) $arrKondisi[] = "thn_anggaran='$tahun' GROUP BY c,d,e";		
...
$Rekaprph_lampiran = new Rekaprph_lampiranObj();
