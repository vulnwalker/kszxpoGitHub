...
class rekaprpbmdObj  extends DaftarObj2{	
	var $Prefix = 'rekaprpbmd';
	...
	var $PageIcon = 'images/pemanfaatan_ico.png';
	var $ico_width = '';
	var $ico_height = '';	
	//var $pagePerHal ='100';
	var $cetak_xls=TRUE ;
	var $fileNameExcel='usulansk.xls';
	var $Cetak_Judul = 'Rekap RPBMD';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'rekaprpbmdForm'; 	
	...
	function setNavAtas(){
		...
		return
			...
			<td class="menudottedline" width="60%" height="20" style="text-align:right"><b>
				...
				<a style="color:blue;" href="pages.php?Pg=rekaprpbmd" title="Rekap Rencana Pemanfaatan Barang Milik Daerah">Rekap RPBMD</a>  |	
				<a href="pages.php?Pg=rekaprpbmd_skpd" title="Rekap Rencana Pemanfaatan Barang Milik Daerah">Rekap RPBMD (SKPD)</a> 		
				  &nbsp;&nbsp;&nbsp;
			</b></td>
			</tr>
			</table>';
		...
		switch($fmLevel){
			case '1':case '2':case '3':default:{
				if($kode1!=0  && $kode2!=0 && $kode3==0){
					...
					$qh=mysql_fetch_array(mysql_query("select count(*) as jml_barang, sum(harga) as harga from t_rencana_pemanfaatan WHERE ka='$kode1' and kb='$kode2' and thn_anggaran='$fmThnAnggaran' $KondThnAkun $KondSKPD"));
			...
			case '4':{
				if($kode1!=0  && $kode2!=0 && $kode3==0){
					...
					$qh=mysql_fetch_array(mysql_query("select count(*) as jml_barang, sum(harga) as harga from t_rencana_pemanfaatan WHERE ka='$kode1' and kb='$kode2' and kc='$kode3' and thn_anggaran='$fmThnAnggaran' $KondThnAkun $KondSKPD"));
					$jumlah_barang=$qh['jml_barang'];	
					$harga_barang=$qh['harga'];		
				}else{
					$jumlah_barang=$isi['jml_barang'];	
			...
			case '5':{
				if($kode1!=0  && $kode2!=0 && $kode3==0){
					...
					//query get jml_harga level 3 selanjutnya
					$qh=mysql_fetch_array(mysql_query("select count(*) as jml_barang, sum(harga) as harga from t_rencana_pemanfaatan WHERE ka='$kode1' and kb='$kode2' and kc='$kode3' and kd='$kode4' and thn_anggaran='$fmThnAnggaran' $KondThnAkun $KondSKPD"));
					$jumlah_barang=$qh['jml_barang'];	
					$harga_barang=$qh['harga'];		
				}else{
					$jumlah_barang=$isi['jml_barang'];	
	...
	function setSumHal2_query($Kondisi, $fsum){
		...
		if($berdasarkan==2){
			return "select $fsum from (select *, 
					'1' as jml_barang 
					from t_rencana_pemanfaatan WHERE $KondTahun $KondSKPD $KondThnAkun and concat(ka, '.', kb, '.', kc, '.', kd, '.', ke, '.', kf) like '$kdakun%') as aa ";	
		}else{
			return "select $fsum from (select *, 
					'1' as jml_barang 
					from t_rencana_pemanfaatan WHERE $KondTahun $KondSKPD $KondThnAkun) as aa  ";
		}
