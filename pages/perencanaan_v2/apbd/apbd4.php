...
class apbd4_v2Obj  extends DaftarObj2{	
	var $Prefix = 'apbd4_v2';
	var $elCurrPage="HalDefault";
	var $SHOW_CEK = TRUE;
	var $TblName = 'view_apbd4'; //bonus
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='apbd4_v2.xls';
	var $namaModulCetak='RKA';
	var $Cetak_Judul = 'RKA';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'apbd4_v2Form';
	var $modul = "APBD";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	...
	$getJumlahBarang = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$apbd4_v2_idplh'"));
	...
	$getPaguYangTelahTerpakai = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as paguYangTerpakai from view_apbd4 where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and bk='$bk' and ck='$ck' and p='$p' and q='$q' and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'  and no_urut = '$this->nomorUrut' and id_anggaran!='$apbd4_v2_idplh' "));
	$sisaPaguIndikatif = $paguIndikatif - $getPaguYangTelahTerpakai['paguYangTerpakai'];
    
	 if(mysql_num_rows(mysql_query("select * from view_apbd4 where c1='0' and f = '00' and k = '$k' and l ='$l' and m='$m' and n='$n' and o='$o'  and id_tahap='$this->idTahap' ")) > 0){
 	 ...
 	 if(empty($cmbJenisRKAForm) ){
		...
		$query = VulnWalkerUpdate('tabel_anggaran',$data,"id_anggaran = '$apbd4_v2_idplh'");
		mysql_query($query);
	 }
	  ...
	  switch($tipe){	
		...
		case 'sync':{	
			...
			while($rows = mysql_fetch_array($getAllFromRka221)){
				...
				if(mysql_fetch_array(mysql_query("select * from view_apbd4 where c1 = '$c1' and c='00' and d='00' and e='00' and e1='000' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and id_tahap = '$this->idTahap' ")) > 0){
				}else{
					$data = array(
									'c1' => $c1,
									'c' => '00',
				...
				if(mysql_fetch_array(mysql_query("select * from view_apbd4 where c1 = '$c1' and c='$c' and d='00' and e='00' and e1='000' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and id_tahap = '$this->idTahap' ")) > 0){
				}else{
					$data = array(
									'c1' => $c1,
									'c' => $c,
				...
				if(mysql_fetch_array(mysql_query("select * from view_apbd4 where c1 = '$c1' and c='$c' and d='$d' and e='00' and e1='000' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and id_tahap = '$this->idTahap'")) > 0){
				}else{
					$data = array(
									'c1' => $c1,
									'c' => $c,
				...
				if(mysql_fetch_array(mysql_query("select * from view_apbd4 where  c1='$c1' and c='$c' and d='$d' and e='00' and e1='000' and bk='$bk' and ck='$ck' and p='$p' and q='0' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and id_tahap = '$this->idTahap'")) > 0){
				}else{
					$data = array(
									'c1' => $c1,
									'c' => $c,
				...
				if(mysql_fetch_array(mysql_query("select * from view_apbd4 where c1='$c1' and c='$c' and d='$d' and e='00' and e1='000' and bk='$bk' and ck='$ck' and p='$p' and q='$q' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'  and id_tahap = '$this->idTahap'")) > 0){
				}else{
					$data = array(
									'c1' => $c1,
									'c' => $c,
   ...
   function setPage_OtherScript(){
		...
		//if($this->jenisForm == "READ")			
		$read = "<script type='text/javascript' src='js/perencanaan_v2/apbd/apbd4.js' language='JavaScript' ></script> ";
		return 	
			"
	 ...
	 $this->form_height = 400;
	  if ($this->form_fmST==0) {
		...
		foreach ($dt as $key => $value) { 
		 ...
		 $jenis_rka = $jenis_rka;
		 $cmbJenisRKA = cmbArray('cmbJenisRKAForm',$jenis_rka,$arrayJenisRKA,'-- JENIS RKA --','onchange=apbd4_v2.unlockFindRekening();');
	 	 if(empty($jenis_rka)){
	  ...
	  $this->form_fields = array(
			...
			'kode7' => array( 
						...
						'value'=> "<input type='text' id='kodeRekening' name='kodeRekening' style='width:120px;' readonly value = '$kodeRekening'> &nbsp&nbsp
						 ...
						 <button type='button' id='findRekening' onclick=apbd4_v2.findRekening('$jenis_rka'); $tergantungJenis> CARI </button> "
						 ),
			'kode8' => array( 
						'label'=>'HARGA SATUAN',
						'labelWidth'=>150, 
						'value'=> "<input type='text' name='hargaSatuan' id='hargaSatuan' value='$hargaSatuan' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='apbd4_v2.bantu();' > <span id='bantu' style='color:red;'> </span>"
						 ),	
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	if($_COOKIE['VulnWalkerUrusan'] != '0' || $_COOKIE['VulnWalkerUrusan'] != '00' ){
		...
		$urusan = cmbQuery('cmbUrusan',$VulnWalkerUrusan,$codeAndNameUrusan,'onchange=apbd4_v2.refreshList(true);','-- URUSAN --');
		...
		$bidang = cmbQuery('cmbBidang',$VulnWalkerBidang,$codeAndNameBidang,'onchange=apbd4_v2.refreshList(true);','-- BIDANG --');
		...
		$skpd= cmbQuery('cmbSKPD',$VulnWalkerSKPD,$codeAndNameSKPD,'onchange=apbd4_v2.refreshList(true);','-- SKPD --');
		...
		$unit = cmbQuery('cmbUnit',$VulnWalkerUnit,$codeAndNameUnit,'onchange=apbd4_v2.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$VulnWalkerSubUnit,$codeAndNameSubUnit,'onchange=apbd4_v2.refreshList(true);','-- SUB UNIT --');*/
		...
		$urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=apbd4_v2.refreshList(true);','-- URUSAN --');
		
		$codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
		$bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=apbd4_v2.refreshList(true);','-- BIDANG --');
		
		$codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
		$skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=apbd4_v2.refreshList(true);','-- SKPD --');
...
/*		$codeAndNameUnit = "select e, concat(e, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d='$selectedD' and e!='00' and e1='000' ";
		$unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=apbd4_v2.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=apbd4_v2.refreshList(true);','-- SUB UNIT --');*/
	}
	...
	if($this->jenisForm == "KOREKSI" || $this->jenisForm == "PENYUSUNAN" || $this->jenisForm == "VALIDASI"){
		...
		$getPaguYangTerpakai =  mysql_fetch_array(mysql_query("select sum(jumlah_harga) as paguYangTerpakai from view_apbd4 where c1= '$selectedC1' and c='$selectedC' and d='$selectedD' and e='$selectedE' and e1='$selectedE1' and bk='$selectedBK' and ck='$selectedCK' and p='$selectedP' and q='$selectedQ' and id_tahap = '$this->idTahap'  "));
		...
		$getPaguYangTerpakai =  mysql_fetch_array(mysql_query("select sum(jumlah_harga) as paguYangTerpakai from view_apbd4 where c1= '$selectedC1' and c='$selectedC' and d='$selectedD' and e='$selectedE' and e1='$selectedE1' and bk='$selectedBK' and ck='$selectedCK' and p='$selectedP' and q='$selectedQ' and no_urut = '$this->urutTerakhir' and tahun ='$this->tahun' and jenis_anggaran = '$this->jenisAnggaran'  "));
		$sisaPagu = $getPaguIndikatif['jumlah'] - $getPaguYangTerpakai['paguYangTerpakai'];
		$sisaPagu =  number_format($sisaPagu ,2,',','.');
		$paguIndikatif = "<input type='text' value='$angkaPaguIndikatif' readonly> &nbsp &nbsp &nbsp SISA PAGU  :  <input type='text' value='$sisaPagu' readonly>";
	...
	function getDaftarOpsi($Mode=1){
		...
		/*$getAllProgram = mysql_query("select * from view_apbd4 where q != '0'");
		$arrayProgram = array();
		while($rows = mysql_fetch_array($getAllProgram)){
			foreach ($rows as $key => $value) { 
				  $$key = $value; 
			}
			if(mysql_num_rows(mysql_query("select * from view_apbd4 where   (rincian_perhitungan !='' or f ='00' ) and bk='$bk' and ck='$ck' and p='$p' and q='$q'  $kondisiFilter $kondisiSKPD")) == 0){
				$arrKondisi[] = "id_anggaran !='$id_anggaran'";	
				if(mysql_num_rows(mysql_query("select * from view_apbd4 where   (rincian_perhitungan !='' or f ='00' ) and bk='$bk' and ck='$ck' and p='$p'   $kondisiFilter $kondisiSKPD")) == 0){
					$concat = $bk.".".$ck.".".$p;
					$arrKondisi[] = "concat(bk,'.',ck,'.',p) !='$concat'";
				}
			}else{
					$getIDProgram =  mysql_fetch_array(mysql_query("select * from view_apbd4 where bk='$bk' and ck='$ck' and p='$p' and  q = '0'"));
...
function Laporan($xls =FALSE){
	 ...
	 $getIdTahapTerakhir = mysql_fetch_array(mysql_query("select max(id_tahap) from tabel_anggaran where tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and nama_modul = 'RKA-SKPD' and id_tahap < $this->idTahap"));
		...
		$Kondisi= join(' and ',$arrKondisi);
		$qry ="select * from view_apbd4 where $Kondisi  ";
...
$apbd4_v2 = new apbd4_v2Obj();

$arrayResult = VulnWalkerTahap_v2($apbd4_v2->modul);
$jenisForm = $arrayResult['jenisForm'];
$nomorUrut = $arrayResult['nomorUrut'];
$tahun = $arrayResult['tahun'];
$jenisAnggaran = $arrayResult['jenisAnggaran'];
...
$apbd4_v2->jenisForm = $jenisForm;
$apbd4_v2->nomorUrut = $nomorUrut;
$apbd4_v2->tahun = $tahun;
$apbd4_v2->jenisAnggaran = $jenisAnggaran;
$apbd4_v2->idTahap = $idTahap;
$apbd4_v2->username = $_COOKIE['coID'];

$apbd4_v2->wajibValidasi = $Main->wajibValidasi;
if($Main->wajibValidasi == TRUE){
	$apbd4_v2->sqlValidasi = " and status_validasi ='1' ";
}else{
	$apbd4_v2->sqlValidasi = " ";
}

if(empty($apbd4_v2->tahun)){
    
	$get1 = mysql_fetch_array(mysql_query("select max(id_anggaran)  from view_apbd4 "));
	$maxAnggaran = $get1['max(id_anggaran)'];
	$get2 = mysql_fetch_array(mysql_query("select * from view_apbd4 where id_anggaran = '$maxAnggaran'"));
	/*$apbd4_v2->tahun = "select max(id_anggaran) as max from view_apbd4 where nama_modul = 'apbd4_v2'";*/
	$apbd4_v2->tahun  = $get2['tahun'];
	$apbd4_v2->jenisAnggaran = $get2['jenis_anggaran'];
	$apbd4_v2->urutTerakhir = $get2['no_urut'];
	$apbd4_v2->jenisFormTerakhir = $get2['jenis_form_modul'];
	$apbd4_v2->urutSebelumnya = $apbd4_v2->urutTerakhir - 1;
	
	
	$idtahapTerakhir = $get2['id_tahap'];
	$apbd4_v2->idTahap = $idtahapTerakhir;
	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$idtahapTerakhir'"));
	$apbd4_v2->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$apbd4_v2->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];
	
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$apbd4_v2->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$apbd4_v2->idTahap'"));
	$apbd4_v2->currentTahap = $getCurrenttahap['nama_tahap'];
	
	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$apbd4_v2->idTahap'"));
	$apbd4_v2->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$apbd4_v2->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$apbd4_v2->provinsi = $setting['provinsi'];
$apbd4_v2->kota = $setting['kota'];
$apbd4_v2->pengelolaBarang = $setting['pengelolaBarang'];
$apbd4_v2->pejabatPengelolaBarang = $setting['pejabat'];
$apbd4_v2->pengurusPengelolaBarang = $setting['pengurus'];
$apbd4_v2->nipPengelola = $setting['nipPengelola'];
$apbd4_v2->nipPengurus = $setting['nipPengurus'];
$apbd4_v2->nipPejabat = $setting['nipPejabat'];
