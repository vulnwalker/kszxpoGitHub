...
class dpaSKPD1_v2Obj  extends DaftarObj2{	
	var $Prefix = 'dpaSKPD1_v2';
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='dpaSKPD1_v2.xls';
	var $namaModulCetak='DPA-SKPD';
	var $Cetak_Judul = 'DPA-SKPD';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'dpaSKPD1_v2Form';
	var $modul = "DPA";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	...
	$getJumlahBarang = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$dpaSKPD1_v2_idplh'"));
	...
	$sisaPaguIndikatif = $paguIndikatif - $getPaguYangTelahTerpakai['paguYangTerpakai'];
	 ...
	 if(mysql_num_rows(mysql_query("select * from view_dpa_1 where c1='0' and f = '00' and k = '$k' and l ='$l' and m='$m' and n='$n' and o='$o'  and id_tahap='$this->idTahap' ")) > 0){
 	 ...
 	 if(empty($cmbJenisDPAForm) ){
		...
		$query = VulnWalkerUpdate('tabel_anggaran',$data,"id_anggaran = '$dpaSKPD1_v2_idplh'");
		mysql_query($query);
	 }
	...
	<A href=\"pages.php?Pg=dpa-skpd-1_v2\" title='DPA-SKPKD MURNI' style='color:blue;' > DPA-SKPD 1 </a> |
   ...
   function setPage_OtherScript(){
		...
		return 	
			...
			<script type='text/javascript' src='js/perencanaan_v2/dpa/dpaSKPD1.js' language='JavaScript' ></script> 
	 ...
	 $this->form_height = 400;
	  if ($this->form_fmST==0) {
		...
		foreach ($dt as $key => $value) { 
		 ...
		 $jenis_dpa = $jenis_dpa;
		 $cmbJenisDPA = cmbArray('cmbJenisDPAForm',$jenis_dpa,$arrayJenisDPA,'-- JENIS DPA --','onchange=dpaSKPD1_v2.unlockFindRekening();');
	 	 if(empty($jenis_dpa)){
	  ...
	  $this->form_fields = array(
			...
			'kode7' => array( 
						...
						'value'=> "<input type='text' id='kodeRekening' name='kodeRekening' style='width:120px;' readonly value = '$kodeRekening'> &nbsp&nbsp
						 ...
						 <button type='button' id='findRekening' onclick=dpaSKPD1_v2.findRekening('$jenis_dpa'); $tergantungJenis> CARI </button> "
						 ),
			'kode8' => array( 
						'label'=>'HARGA SATUAN',
						'labelWidth'=>150, 
						'value'=> "<input type='text' name='hargaSatuan' id='hargaSatuan' value='$hargaSatuan' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='dpaSKPD1_v2.bantu();' > <span id='bantu' style='color:red;'> </span>"
						 ),	
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	foreach ($_COOKIE as $key => $value) { 
		...
		$urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=dpaSKPD1_v2.refreshList(true);','-- URUSAN --');
		
		$codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
		$bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=dpaSKPD1_v2.refreshList(true);','-- BIDANG --');
		
		$codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
		$skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=dpaSKPD1_v2.refreshList(true);','-- SKPD --');
		...
		$unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=dpaSKPD1_v2.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=dpaSKPD1_v2.refreshList(true);','-- SUB UNIT --');
...
$dpaSKPD1_v2 = new dpaSKPD1_v2Obj();

$arrayResult = VulnWalkerTahap_v2($dpaSKPD1_v2->modul);
$jenisForm = $arrayResult['jenisForm'];
$nomorUrut = $arrayResult['nomorUrut'];
$tahun = $arrayResult['tahun'];
$jenisAnggaran = $arrayResult['jenisAnggaran'];
...
$dpaSKPD1_v2->jenisForm = $jenisForm;
$dpaSKPD1_v2->nomorUrut = $nomorUrut;
$dpaSKPD1_v2->urutTerakhir = $nomorUrut;
$dpaSKPD1_v2->tahun = $tahun;
$dpaSKPD1_v2->jenisAnggaran = $jenisAnggaran;
$dpaSKPD1_v2->idTahap = $idTahap;
$dpaSKPD1_v2->username = $_COOKIE['coID'];

$dpaSKPD1_v2->wajibValidasi = $Main->wajibValidasi;
if($Main->wajibValidasi == TRUE){
	$dpaSKPD1_v2->sqlValidasi = " and status_validasi ='1' ";
}else{
	$dpaSKPD1_v2->sqlValidasi = " ";
}



if(empty($dpaSKPD1_v2->tahun)){
	...
	/*$dpaSKPD1_v2->tahun = "select max(id_anggaran) as max from view_dpa_1 where nama_modul = 'dpaSKPD1_v2'";*/
	$dpaSKPD1_v2->tahun  = $get2['tahun'];
	$dpaSKPD1_v2->jenisAnggaran = $get2['jenis_anggaran'];
	$dpaSKPD1_v2->urutTerakhir = $get2['no_urut'];
	$dpaSKPD1_v2->jenisFormTerakhir = $get2['jenis_form_modul'];
	$dpaSKPD1_v2->urutSebelumnya = $dpaSKPD1_v2->urutTerakhir - 1;
	...
	$dpaSKPD1_v2->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$dpaSKPD1_v2->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];
	
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$dpaSKPD1_v2->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$dpaSKPD1_v2->idTahap'"));
	$dpaSKPD1_v2->currentTahap = $getCurrenttahap['nama_tahap'];
	
	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$dpaSKPD1_v2->idTahap'"));
	$dpaSKPD1_v2->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$dpaSKPD1_v2->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$dpaSKPD1_v2->provinsi = $setting['provinsi'];
$dpaSKPD1_v2->kota = $setting['kota'];
$dpaSKPD1_v2->pengelolaBarang = $setting['pengelolaBarang'];
$dpaSKPD1_v2->pejabatPengelolaBarang = $setting['pejabat'];
$dpaSKPD1_v2->pengurusPengelolaBarang = $setting['pengurus'];
$dpaSKPD1_v2->nipPengelola = $setting['nipPengelola'];
$dpaSKPD1_v2->nipPengurus = $setting['nipPengurus'];
$dpaSKPD1_v2->nipPejabat = $setting['nipPejabat'];
