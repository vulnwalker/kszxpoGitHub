...
class rkaSKPD_v2Obj  extends DaftarObj2{	
	var $Prefix = 'rkaSKPD_v2';
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='rkaSKPD_v2.xls';
	var $namaModulCetak='RKA';
	var $Cetak_Judul = 'RKA';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'rkaSKPD_v2Form';
	var $modul = "RKA-SKPD";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	...
	$getJumlahBarang = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$rkaSKPD_v2_idplh'"));
	...
	$sisaPaguIndikatif = $paguIndikatif - $getPaguYangTelahTerpakai['paguYangTerpakai'];
	 ...
	 if(mysql_num_rows(mysql_query("select * from view_rka where c1='0' and f1 = '0' and k = '$k' and l ='$l' and m='$m' and n='$n' and o='$o'  and id_tahap='$this->idTahap' ")) > 0){
 	 ...
 	 if(empty($cmbJenisRKAForm) ){
		...
		$query = VulnWalkerUpdate('tabel_anggaran',$data,"id_anggaran = '$rkaSKPD_v2_idplh'");
		mysql_query($query);
	 }
	...
	<A href=\"pages.php?Pg=rka-skpd_v2\" title='RKA-SKPKD MURNI'  style='color:blue;' > RKA-SKPD  </a> |
   ...
   function setPage_OtherScript(){
		...
		return 	
			...
			<script type='text/javascript' src='js/perencanaan_v2/rka/rkaSKPD.js' language='JavaScript' ></script> 
	 ...
	 $this->form_height = 400;
	  if ($this->form_fmST==0) {
		...
		foreach ($dt as $key => $value) { 
		 ...
		 $jenis_rka = $jenis_rka;
		 $cmbJenisRKA = cmbArray('cmbJenisRKAForm',$jenis_rka,$arrayJenisRKA,'-- JENIS RKA --','onchange=rkaSKPD_v2.unlockFindRekening();');
	 	 if(empty($jenis_rka)){
	  ...
	  $this->form_fields = array(
			...
			'kode7' => array( 
						...
						'value'=> "<input type='text' id='kodeRekening' name='kodeRekening' style='width:120px;' readonly value = '$kodeRekening'> &nbsp&nbsp
						 ...
						 <button type='button' id='findRekening' onclick=rkaSKPD_v2.findRekening('$jenis_rka'); $tergantungJenis> CARI </button> "
						 ),
			'kode8' => array( 
						'label'=>'HARGA SATUAN',
						'labelWidth'=>150, 
						'value'=> "<input type='text' name='hargaSatuan' id='hargaSatuan' value='$hargaSatuan' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='rkaSKPD_v2.bantu();' > <span id='bantu' style='color:red;'> </span>"
						 ),	
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	foreach ($_COOKIE as $key => $value) { 
		...
		$urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=rkaSKPD_v2.refreshList(true);','-- URUSAN --');
		
		$codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
		$bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=rkaSKPD_v2.refreshList(true);','-- BIDANG --');
		
		$codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
		$skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=rkaSKPD_v2.refreshList(true);','-- SKPD --');
		...
		$unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=rkaSKPD_v2.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=rkaSKPD_v2.refreshList(true);','-- SUB UNIT --');
...
$rkaSKPD_v2->jenisForm = $jenisForm;
$rkaSKPD_v2->nomorUrut = $nomorUrut;
$rkaSKPD_v2->tahun = $tahun;
$rkaSKPD_v2->jenisAnggaran = $jenisAnggaran;
$rkaSKPD_v2->idTahap = $idTahap;

$rkaSKPD_v2->username = $_COOKIE['coID'];

$rkaSKPD_v2->wajibValidasi = $Main->wajibValidasi;
if($Main->wajibValidasi == TRUE){
	$rkaSKPD_v2->sqlValidasi = " and status_validasi ='1' ";
}else{
	$rkaSKPD_v2->sqlValidasi = " ";
}

if(empty($rkaSKPD_v2->tahun)){
	...
	/*$rkaSKPD_v2->tahun = "select max(id_anggaran) as max from view_rka where nama_modul = 'rkaSKPD_v2'";*/
	$rkaSKPD_v2->tahun  = $get2['tahun'];
	$rkaSKPD_v2->jenisAnggaran = $get2['jenis_anggaran'];
	$rkaSKPD_v2->urutTerakhir = $get2['no_urut'];
	$rkaSKPD_v2->jenisFormTerakhir = $get2['jenis_form_modul'];
	$rkaSKPD_v2->urutSebelumnya = $rkaSKPD_v2->urutTerakhir - 1;
	...
	$rkaSKPD_v2->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$rkaSKPD_v2->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];
	
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$rkaSKPD_v2->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$rkaSKPD_v2->idTahap'"));
	$rkaSKPD_v2->currentTahap = $getCurrenttahap['nama_tahap'];
	
	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$rkaSKPD_v2->idTahap'"));
	$rkaSKPD_v2->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$rkaSKPD_v2->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$rkaSKPD_v2->provinsi = $setting['provinsi'];
$rkaSKPD_v2->kota = $setting['kota'];
$rkaSKPD_v2->pengelolaBarang = $setting['pengelolaBarang'];
$rkaSKPD_v2->pejabatPengelolaBarang = $setting['pejabat'];
$rkaSKPD_v2->pengurusPengelolaBarang = $setting['pengurus'];
$rkaSKPD_v2->nipPengelola = $setting['nipPengelola'];
$rkaSKPD_v2->nipPengurus = $setting['nipPengurus'];
$rkaSKPD_v2->nipPejabat = $setting['nipPejabat'];


if($rkaSKPD_v2->jenisForm != "PENYUSUNAN"){
$get1 = mysql_fetch_array(mysql_query("select max(id_anggaran)  from view_rka_2_2_1 where jenis_form_modul = 'PENYUSUNAN' "));
	...
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$rkaSKPD_v2->currentTahap = $arrayHasil['currentTahap'];
			$rkaSKPD_v2->jenisForm = "";
			$rkaSKPD_v2->jenisFormTerakhir = "PENYUSUNAN";
			$rkaSKPD_v2->idTahap = $get2['id_tahap'];
			$rkaSKPD_v2->nomorUrut = $get2['no_urut'];
			$rkaSKPD_v2->urutTerakhir = $get2['no_urut'];
}
