...
class rkaPPKD31_v2_koreksiObj  extends DaftarObj2{	
	var $Prefix = 'rkaPPKD31_v2_koreksi';
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='rkaPPKD31_v2_koreksi.xls';
	var $namaModulCetak='RKA';
	var $Cetak_Judul = 'RKA';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'rkaPPKD31_v2_koreksiForm';
	var $modul = "RKA-PPKD";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	...
	function setTitle(){
		return 'RKA-PPKD 3.1 '.$this->jenisAnggaran.' TAHUN '.$this->tahun."&nbsp ".  strtoupper($this->namaTahap);
	}
	...
	function genRowSum($ColStyle, $Mode, $Total){
		...
		$getData = mysql_fetch_array(mysql_query("select sum(jumlah_harga) from tabel_anggaran where  (rincian_perhitungan !='' or f ='00' ) and id_tahap='$idTahap' and tahun ='$this->tahun' and jenis_anggaran = '$this->jenisAnggaran' and nama_modul ='RKA-PPKD' and jenis_rka = '3.1' $kondisiSKPD $kondisiRekening"));
		...
		$ContentTotalHal=''; $ContentTotal='';
			...
			if($this->jenisForm == "PENYUSUNAN"){
				...
				$getRApbd = mysql_fetch_array(mysql_query("select * from view_rka_ppkd_3_1 where no_urut = '$urutSebelumnya' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' "));
				if($getRApbd['jenis_form_modul'] != "PENYUSUNAN" && $getRApbd['jenis_form_modul'] != "KOREKSI"){
					$urutSebelumnya = $urutSebelumnya - 1;		
				}
				$getDataSebelumnya = mysql_fetch_array(mysql_query("select sum(jumlah_harga) from view_rka_ppkd_3_1 where  (rincian_perhitungan !='' or f ='00' ) and no_urut='$urutSebelumnya' and tahun ='$this->tahun' and jenis_anggaran = '$this->jenisAnggaran'  $kondisiSKPD $kondisiRekening"));
	...
	$getJumlahBarang = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$rkaPPKD31_v2_koreksi_idplh'"));
	...
	$sisaPaguIndikatif = $paguIndikatif - $getPaguYangTelahTerpakai['paguYangTerpakai'];
	 ...
	 if(mysql_num_rows(mysql_query("select * from view_rka_ppkd_3_1 where c1='0' and f = '00' and k = '$k' and l ='$l' and m='$m' and n='$n' and o='$o'  and id_tahap='$this->idTahap' ")) > 0){
 	 ...
 	 if(empty($cmbJenisRKAForm) ){
		...
		$query = VulnWalkerUpdate('tabel_anggaran',$data,"id_anggaran = '$rkaPPKD31_v2_koreksi_idplh'");
		mysql_query($query);
	 }
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){	
		...
		case 'setGrup':{
				...
				if(mysql_num_rows(mysql_query("select * from tabel_anggaran where o1 = '$pekerjaan' and id_tahap = '$this->idTahap' and jenis_rka = '3.1' and nama_modul ='RKA-PPKD'")) == 0){
					$data = array(
												 'c1' => '0',
												 'c' => '00',
												 'd' => '00',
		...
		case 'SaveEditJob':{
			...
			$getUrut = mysql_fetch_array(mysql_query("select * from temp_rka_231_v2 where o1='$o1'"));
			$urut = $getUrut['urut'];
		...
		case 'Gruping':{	
			...
			if(!isset($rkaPPKD31_v2_koreksi_cb)){
				$err = "Pilih Data";
			}
			$dt = implode(',',$rkaPPKD31_v2_koreksi_cb);
		...
		case 'SaveJob':{
			...
			$getCurrentInsert = mysql_fetch_array(mysql_query("select max(id) from ref_pekerjaan"));
			$cmbPekerjaan = cmbQuery('pekerjaan', $getCurrentInsert['max(id)'], $codeAndNamePekerjaan,"  ",'-- PEKERJAAN --');
			$getMaxUrut = mysql_fetch_array(mysql_query("select max(urut) from temp_rka_231_v2 where user ='$username'"));
		...
		case 'sesuai':{
			...
			$getrkaPPKD31_v2_koreksinya = mysql_fetch_array(mysql_query($queryRows));
			foreach ($getrkaPPKD31_v2_koreksinya as $key => $value) { 
				  $$key = $value; 
			} 
			$cmbUrusanForm = $c1;
			$cmbBidangForm = $c;
		...
		case 'koreksi':{
			...
			foreach ($getrkaPPKD31_v2_koreksinya as $key => $value) { 
			 ...
			 if($this->jenisForm !='KOREKSI'){
			 	...
			 	if($err == ""){
				 	if(mysql_num_rows(mysql_query("select * from view_rka_ppkd_3_1 where  o1='$o1' and rincian_perhitungan='' and rincian_perhitungan ='' and id_tahap='$this->idTahap' ")) > 0){
				 	...
				 	if(mysql_num_rows(mysql_query("select * from view_rka_ppkd_3_1 where  rincian_perhitungan ='' and c1='0' and k='$k' and l='$l' and m='$m' and n='$n' and o='$o' and id_tahap='$this->idTahap'  ")) > 0){
					...
					$cekRKA =  mysql_num_rows(mysql_query("select * from view_rka_ppkd_3_1 where c1 = '$c1' and c='$c' and d = '$d' and e='$e' and e1='$e1'  and k='$k' and l='$l' and m='$m' and n='$n' and o='$o' and tahun = '$this->tahun' and jenis_anggaran = '$this->jenisAnggaran' and id_tahap = '$this->idTahap' and o1='$o1' and rincian_perhitungan = '$rincian_perhitungan'"));
							if($cekRKA > 0 ){
								$getID = mysql_fetch_array(mysql_query("select * from view_rka_ppkd_3_1 where c1 = '$c1' and c='$c' and d = '$d' and e='$e' and e1='$e1'  and k='$k' and l='$l' and m='$m' and n='$n' and o='$o'  and rincian_perhitungan !='' and tahun = '$this->tahun' and jenis_anggaran = '$this->jenisAnggaran' and id_tahap = '$this->idTahap' "));
							    $idnya = $getID['id_anggaran'];
		...
		case 'editTab':{
			$id = $_REQUEST['rkaPPKD31_v2_koreksi_cb'];
			foreach ($_REQUEST as $key => $value) { 
				  $$key = $value; 
			} 
			...
			$getAll = mysql_query("select * from view_rka_ppkd_3_1 where concat(k,'.',l,'.',m,'.',n,'.',o) ='$kodeRek' and id_tahap='$this->idTahap' and rincian_perhitungan !=''   and c1='$cmbUrusan' and c='$cmbBidang' and d='$cmbSKPD' and status_validasi !='1'  order by o1, rincian_perhitungan");
			mysql_query("delete from temp_rka_ppkd_31 where user='$username'");
			mysql_query("delete from temp_rincian_volume_31 where user='$username'");
		    mysql_query("delete from temp_alokasi_rka_ppkd_31_v2 where user='$username'");
			$noUrutPekerjaan = 0;
			$angkaO2 = 0;
			$lastO1 = 0;
	...
	<A href=\"pages.php?Pg=rka-ppkd-3.1_v2_koreksi\" title='RKA-PPKD MURNI'  style='color:blue;' > RKA-PPKD 3.1 </a> |
   ...
   function setPage_OtherScript(){
		...
		return 	
			...
			<script type='text/javascript' src='js/perencanaan_v2/rka/rkaPPKD3.1_koreksi.js' language='JavaScript' ></script> 
	 ...
	 $this->form_height = 400;
	  if ($this->form_fmST==0) {
		...
		foreach ($dt as $key => $value) { 
		 ...
		 $jenis_rka = $jenis_rka;
		 $cmbJenisRKA = cmbArray('cmbJenisRKAForm',$jenis_rka,$arrayJenisRKA,'-- JENIS RKA --','onchange=rkaPPKD31_v2_koreksi.unlockFindRekening();');
	 	 if(empty($jenis_rka)){
	  ...
	  $this->form_fields = array(
			...
			'kode7' => array( 
						...
						'value'=> "<input type='text' id='kodeRekening' name='kodeRekening' style='width:120px;' readonly value = '$kodeRekening'> &nbsp&nbsp
						 ...
						 <button type='button' id='findRekening' onclick=rkaPPKD31_v2_koreksi.findRekening('$jenis_rka'); $tergantungJenis> CARI </button> "
						 ),
			'kode8' => array( 
						'label'=>'HARGA SATUAN',
						'labelWidth'=>150, 
						'value'=> "<input type='text' name='hargaSatuan' id='hargaSatuan' value='$hargaSatuan' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='rkaPPKD31_v2_koreksi.bantu();' > <span id='bantu' style='color:red;'> </span>"
						 ),	
	...
	function setKolomData($no, $isi, $Mode, $TampilCheckBox){
	 ...
	 if($this->jenisForm == 'PENYUSUNAN'){
			 ...
			 if($c1 == '0'){
			 	if(strlen($k) > 1){
					...
					if($k == '0' && $n == '0'){
						...
						$jarak = '15px';
				 		$cekAdaParent = mysql_num_rows(mysql_query("select * from view_rka_ppkd_3_1 where id_tahap = '$this->idTahap' and o1 = '$o1' and o1 !='0' and o1 !='' "));
			 ...
			 if($c1 == '0'){
			 if($k == '0' && $n =='0'){
			 	$getTotalJumalhHarga = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as total from view_rka_ppkd_3_1 where o1='$o1' $kondisiSKPD $kondisiBelanja and id_tahap='$this->idTahap'"));
			 }else{
	 ...
	 //TAHAP PENYUSUNAN
			 ...
			 $getRApbd = mysql_fetch_array(mysql_query("select * from view_rka_ppkd_3_1 where no_urut = '$nomorBefore' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' "));
			 if($getRApbd['jenis_form_modul'] != "PENYUSUNAN" && $getRApbd['jenis_form_modul'] != "KOREKSI"){
					$nomorBefore = $nomorBefore - 1;		
			 }
			 $getLastTahap = mysql_fetch_array(mysql_query("select * from view_rka_ppkd_3_1 where no_urut = '$nomorBefore' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'"));
			 $blackList = "";
			 if($getLastTahap['jenis_form_modul'] == "PENYUSUNAN" && $this->wajibValidasi == TRUE){
				$getAllChild = mysql_query("select * from view_rka_ppkd_3_1 where id_tahap = '".$getLastTahap['id_tahap']."' and (rincian_perhitungan !='' or j !='000'  and status_validasi !='1' )");			
			 ...
			 if($c1 == '0'){
			 	if(strlen($k) > 1){
					...
					if($k == '0' && $n == '0'){
						...
						$jarak = '15px';
				 		$cekAdaParent = mysql_num_rows(mysql_query("select * from view_rka_ppkd_3_1 where no_urut = '$nomorBefore' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and o1 = '$o1' and o1 !='0' and o1 !='' "));
						if($cekAdaParent == 0)$jarak = "0px";
						$Koloms[] = array('align="left"',"<span style='margin-left:$jarak;font-weight:bold;'>".$namaRekening."</span>" );
					}
				}
			 ...
			 if($c1 == '0'){
			 if($k == '0' && $n =='0'){
			 	$getTotalJumalhHarga = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as total from view_rka_ppkd_3_1 where o1='$o1' $kondisiSKPD $kondisiBelanja and no_urut ='$nomorBefore' and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' $blackList"));
			 }else{
			 	$getTotalJumalhHarga = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as total from view_rka_ppkd_3_1 where k='$k' and l='$l' and m='$m' and n='$n' and o='$o' $kondisiSKPD and no_urut ='$nomorBefore' and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' $blackList"));	
			 }
			 $Koloms[] = array('align="right"', " <span style='font-weight:bold;'>".number_format($getTotalJumalhHarga['total'] ,2,',','.')." </span>" );
			 ...
			 if(empty($rincian_perhitungan)){
			    ...
			    $Koloms[] = array('align="right"',"");
				 if($c1 =='0'){
				 	if($k == '0' && $n =='0'){
					 	$getTotalJumalhHargaKoreksi = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as total from view_rka_ppkd_3_1 where  o1='$o1' $kondisiSKPD and id_tahap ='$this->idTahap' and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'"));
					 }else{
	 	...
	 	if($this->jenisFormTerakhir == "KOREKSI"){
			 ...
			 if(empty($rincian_perhitungan)){
			    ...
			    $Koloms[] = array('align="right"',"");
				 if($c1 =='0'){
				 	if($k == '0' && $n =='0'){
					 	$getTotalJumalhHargaKoreksi = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as total from view_rka_ppkd_3_1 where  o1='$o1' $kondisiSKPD and no_urut='$this->urutTerakhir' and tahun='$this->tahun' and jenis_anggaran = '$this->jenisAnggaran'"));
					 }else{
			 ...
			 if($c1 == '0'){
			 	if(strlen($k) > 1){
					...
					if($k == '0' && $n == '0'){
						...
						$jarak = '15px';
				 		$cekAdaParent = mysql_num_rows(mysql_query("select * from view_rka_ppkd_3_1 where id_tahap = '$idTahap' and o1 = '$o1' and o1 !='0' and o1 !='' "));
						if($cekAdaParent == 0)$jarak = "0px";
						$Koloms[] = array('align="left"',"<span style='margin-left:$jarak;font-weight:bold;'>".$namaRekening."</span>" );
					}
				...
				$ilustrasi = "";
				$getSumJumlahBarang = mysql_fetch_array(mysql_query("select sum(volume_rek) as total from view_rka_ppkd_3_1 where k='$k' and l='$l' and m='$m' and n='$n' and o='$o' $kondisiSKPD and id_tahap='$idTahap'"));
				$Koloms[] = array('align="left"',"" );
				$jumlahBarang = $getSumJumlahBarang['total'];
				//$Koloms[] = array('align="right"', number_format($jumlahBarang ,0,',','.') );
			 ...
			 $Koloms[] = array('align="left"', $satuan_rek );
			 $getSumSatuanRek = mysql_fetch_array(mysql_query("select sum(jumlah) as total from view_rka_ppkd_3_1 where k='$k' and l='$l' and m='$m' and n='$n'  and o='$o' $kondisiSKPD and id_tahap='$idTahap'"));
			 $sumSatuanRek = $getSumSatuanRek['total'];
			 if($c1 == '0'){
			 	//$Koloms[] = array('align="right"', number_format($sumSatuanRek ,2,',','.') );
				$Koloms[] = array('align="right"','' );
			 ...
			 if($c1 == '0'){
			 if($k == '0' && $n =='0'){
			 	$getTotalJumalhHarga = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as total from view_rka_ppkd_3_1 where o1='$o1' $kondisiSKPD $kondisiBelanja and id_tahap='$idTahap'"));
			 }else{
			 	$getTotalJumalhHarga = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as total from view_rka_ppkd_3_1 where k='$k' and l='$l' and m='$m' and n='$n' and o='$o' $kondisiSKPD and id_tahap='$idTahap'"));	
			 }
			 $Koloms[] = array('align="right"', "<span style='font-weight:bold;'>".number_format($getTotalJumalhHarga['total'] ,2,',','.')."</span>" );
			 if($this->wajibValidasi == TRUE)$Koloms[] = array('align="center"', "");
			 }else{
	...
	function Validasi($dt){	
	 ...
	 $this->form_caption = 'VALIDASI RKA-PPKD 3.1';
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	foreach ($_COOKIE as $key => $value) { 
		...
		$urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=rkaPPKD31_v2_koreksi.refreshList(true);','-- URUSAN --');
		
		$codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
		$bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=rkaPPKD31_v2_koreksi.refreshList(true);','-- BIDANG --');
		
		$codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
		$skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=rkaPPKD31_v2_koreksi.refreshList(true);','-- SKPD --');
		...
		$unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=rkaPPKD31_v2_koreksi.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=rkaPPKD31_v2_koreksi.refreshList(true);','-- SUB UNIT --');
	...
	function getDaftarOpsi($Mode=1){
	  ...
	  if(isset($cmbUrusan) && $cmbUrusan== ''){
			...
			$arrKondisi[] = "c1 != '0'";
		if($this->jenisForm == 'PENYUSUNAN'){
			$getAllParent = mysql_query("select * from view_rka_ppkd_3_1 where id_tahap ='$this->idTahap'  and o1 !='0' and k='0' and n='0' and rincian_perhitungan=''  ");	
			while($rows = mysql_fetch_array($getAllParent)){
				foreach ($rows as $key => $value) { 
			 	 $$key = $value; 
				}
				$cekPekerjaan = mysql_num_rows(mysql_query("select * from view_rka_ppkd_3_1 where o1 = '$o1' and id_tahap ='$this->idTahap' $kondisiRekening $kondisiSKPD and (rincian_perhitungan !='' or rincian_perhitungan!='') "));
				if($cekPekerjaan == 0){
					$arrKondisi[] = "o1 !='$o1'";
				}else{
					$arrKondisi[] = " id_tahap='$this->idTahap'  or id_anggaran = '$id_anggaran' ";
					$getAllRekening = mysql_query("select * from view_rka_ppkd_3_1 where id_tahap = '$this->idTahap'  and c1 = '0' and j ='000' and rincian_perhitungan = '' and k!='0' and n !='0'  ");				
					while($row2s = mysql_fetch_array($getAllRekening)){
						foreach ($row2s as $key => $value) { 
					 	 $$key = $value; 
						}
						$cekRekening = mysql_num_rows(mysql_query("select * from view_rka_ppkd_3_1 where k ='$k' and l='$l' and m='$m' and n='$n' and o='$o' and id_tahap ='$this->idTahap' $kondisiRekening $kondisiSKPD and (rincian_perhitungan !='' or rincian_perhitungan!='') "));
						if($cekRekening == 0){
							$concat = $k.".".$l.".".$m.".".$n.".".$o;
				...
				$grabNonMapingRekening= mysql_query("select * from view_rka_ppkd_3_1 where id_tahap ='$this->idTahap' and (LENGTH(k) > 1) and rincian_perhitungan=''");
				while($got = mysql_fetch_array($grabNonMapingRekening)){
					if(mysql_num_rows(mysql_query("select * from view_rka_ppkd_3_1 where id_tahap ='$this->idTahap' and k ='".$got['k']."' and l ='".$got['l']."' and m ='".$got['m']."' and n ='".$got['n']."' and o ='".$got['o']."' and rincian_perhitungan!=''  $kondisiSKPD ")) != 0){
						$arrKondisi[] = "id_tahap = '$this->idTahap' or id_anggaran ='".$got['id_anggaran']."'";
					}
					
				}
				$grabNonHostedRekening= mysql_query("select * from view_rka_ppkd_3_1 where id_tahap ='$this->idTahap' and (LENGTH(k) = 1 and k !='0') and rincian_perhitungan=''");
			...
			$nomorUrutSebelumnya = $this->nomorUrut - 1;
			$getRApbd = mysql_fetch_array(mysql_query("select * from view_rka_ppkd_3_1 where no_urut = '$nomorUrutSebelumnya' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' "));
			if($getRApbd['jenis_form_modul'] != "PENYUSUNAN" && $getRApbd['jenis_form_modul'] != "KOREKSI"){
				$nomorUrutSebelumnya = $nomorUrutSebelumnya - 1;		
			}
			$getLastTahap = mysql_fetch_array(mysql_query("select * from view_rka_ppkd_3_1 where no_urut = '$nomorUrutSebelumnya' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'"));
			$blackList = "";
			if($getLastTahap['jenis_form_modul'] == "PENYUSUNAN" && $this->wajibValidasi == TRUE){
				$getAllChild = mysql_query("select * from view_rka_ppkd_3_1 where id_tahap = '".$getLastTahap['id_tahap']."' and (rincian_perhitungan !='' or j !='000'  and status_validasi !='1' )");			
				while($black = mysql_fetch_array($getAllChild)){
			...
			$idTahap = $getIDTahap['id_tahap'];
			$getAllParent = mysql_query("select * from view_rka_ppkd_3_1 where id_tahap = '$idTahap'  and o1 !='0' and k='0' and n='0' and rincian_perhitungan=''  ");	
			while($rows = mysql_fetch_array($getAllParent)){
				foreach ($rows as $key => $value) { 
			 	 $$key = $value; 
				}
				$cekPekerjaan = mysql_num_rows(mysql_query("select * from view_rka_ppkd_3_1 where o1 = '$o1' and id_tahap = '$idTahap' $kondisiRekening $kondisiSKPD $blackList and (rincian_perhitungan !='' or rincian_perhitungan!='') "));
				if($cekPekerjaan == 0){
					$arrKondisi[] = "o1 !='$o1'";
				}else{
					$arrKondisi[] = " id_tahap = '$idTahap'  or id_anggaran = '$id_anggaran' ";
					$getAllRekening = mysql_query("select * from view_rka_ppkd_3_1 where id_tahap = '$idTahap' and c1 = '0' and j ='000' and rincian_perhitungan = '' and k!='0' and n !='0'  ");				
					while($row2s = mysql_fetch_array($getAllRekening)){
						foreach ($row2s as $key => $value) { 
					 	 $$key = $value; 
						}
						$cekRekening = mysql_num_rows(mysql_query("select * from view_rka_ppkd_3_1 where k ='$k' and l='$l' and m='$m' and n='$n' and o='$o' and id_tahap = '$idTahap' $kondisiRekening $kondisiSKPD $blackList and (rincian_perhitungan !='' or rincian_perhitungan!='') "));
						if($cekRekening == 0){
							$concat = $k.".".$l.".".$m.".".$n.".".$o;
							$arrKondisi[] = "concat(k,'.',l,'.',m,'.',n,'.',o)  !='$concat'";
						}else{
							$arrKondisi[] = " id_tahap = '$idTahap'  or id_anggaran = '$id_anggaran'  ";
							$getAllProgram = mysql_query("select * from view_rka_ppkd_3_1 where id_tahap = '$idTahap' and p != '0' and q ='0' and j ='000' and rincian_perhitungan = ''   ");				
							while($row3s = mysql_fetch_array($getAllProgram)){
								foreach ($row3s as $key => $value) { 
							 	 $$key = $value; 
								}
								$cekProgram = mysql_num_rows(mysql_query("select * from view_rka_ppkd_3_1 where bk ='$bk' and ck='$ck' and p='$p' and id_tahap = '$idTahap' $kondisiRekening $kondisiSKPD $blackList and (rincian_perhitungan !='' or rincian_perhitungan!='') "));
								if($cekProgram == 0){
									$concat = $bk.".".$ck.".".$p;
									$arrKondisi[] = "concat(bk,'.',ck,'.',p)  !='$concat'";
								}else{
									$arrKondisi[] = " id_tahap = '$idTahap'  or id_anggaran = '$id_anggaran'   ";
									$getAllKegiatan = mysql_query("select * from view_rka_ppkd_3_1 where id_tahap = '$idTahap' and bk ='$bk' and ck='$ck' and p='$p' and q != '0'  and j ='000' and rincian_perhitungan = ''   ");				
									while($row4s = mysql_fetch_array($getAllKegiatan)){
										foreach ($row4s as $key => $value) { 
									 	 $$key = $value; 
										}
										$cekKegiatan = mysql_num_rows(mysql_query("select * from view_rka_ppkd_3_1 where bk ='$bk' and ck='$ck' and p='$p' and q='$q' and id_tahap = '$idTahap' $kondisiRekening $kondisiSKPD  $blackList and (rincian_perhitungan !='' or rincian_perhitungan!='') "));
										if($cekKegiatan == 0){
											$concat = $bk.".".$ck.".".$p;
											$arrKondisi[] = "concat(bk,'.',ck,'.',p)  !='$concat'";
										}else{
				...
				$grabNonMapingRekening= mysql_query("select * from view_rka_ppkd_3_1 where id_tahap = '$idTahap' and (LENGTH(k) > 1) and rincian_perhitungan=''");
				while($got = mysql_fetch_array($grabNonMapingRekening)){
					if(mysql_num_rows(mysql_query("select * from view_rka_ppkd_3_1 where id_tahap = '$idTahap' and k ='".$got['k']."' and l ='".$got['l']."' and m ='".$got['m']."' and n ='".$got['n']."' and o ='".$got['o']."' and rincian_perhitungan!='' $kondisiRekening $kondisiSKPD $blackList")) != 0){
						$arrKondisi[] = "id_tahap = '$idTahap' or id_anggaran ='".$got['id_anggaran']."'";
					}
					
				}
				
				$grabNonHostedRekening= mysql_query("select * from view_rka_ppkd_3_1 where id_tahap = '$idTahap' and (LENGTH(k) = 1 and k !='0') and rincian_perhitungan=''");
				while($got = mysql_fetch_array($grabNonHostedRekening)){
					if(mysql_num_rows(mysql_query("select * from view_rka_ppkd_3_1 where id_tahap = '$idTahap' and k ='".$got['k']."' and l ='".$got['l']."' and m ='".$got['m']."' and n ='".$got['n']."' and o ='".$got['o']."' and rincian_perhitungan!='' $kondisiRekening $kondisiSKPD $blackList")) != 0){
						$arrKondisi[] = "id_tahap = '$idTahap' or id_anggaran ='".$got['id_anggaran']."'";
					}
			...
			if($this->jenisFormTerakhir == "KOREKSI"){
				...
				while($rows = mysql_fetch_array($getAllParent)){
					...
					$cekPekerjaan = mysql_num_rows(mysql_query("select * from view_rka_ppkd_3_1 where o1 = '$o1' and id_tahap ='$idTahap' $kondisiRekening $kondisiSKPD and (rincian_perhitungan !='' or rincian_perhitungan!='') "));
					if($cekPekerjaan == 0){
						$arrKondisi[] = "o1 !='$o1'";
					}else{
						$arrKondisi[] = " id_tahap='$idTahap'  or id_anggaran = '$id_anggaran' ";
						...
						while($row2s = mysql_fetch_array($getAllRekening)){
							...
							$cekRekening = mysql_num_rows(mysql_query("select * from view_rka_ppkd_3_1 where k ='$k' and l='$l' and m='$m' and n='$n' and o='$o' and id_tahap ='$idTahap' $kondisiRekening $kondisiSKPD and (rincian_perhitungan !='' or rincian_perhitungan!='') "));
							if($cekRekening == 0){
								$concat = $k.".".$l.".".$m.".".$n.".".$o;
								$arrKondisi[] = "concat(k,'.',l,'.',m,'.',n,'.',o)  !='$concat'";
							}else{
								...
								while($row3s = mysql_fetch_array($getAllProgram)){
									...
									$cekProgram = mysql_num_rows(mysql_query("select * from view_rka_ppkd_3_1 where bk ='$bk' and ck='$ck' and p='$p' and id_tahap ='$idTahap' $kondisiRekening $kondisiSKPD and (rincian_perhitungan !='' or rincian_perhitungan!='') "));
									if($cekProgram == 0){
										$concat = $bk.".".$ck.".".$p;
										$arrKondisi[] = "concat(bk,'.',ck,'.',p)  !='$concat'";
									}else{
										...
										while($row4s = mysql_fetch_array($getAllKegiatan)){
											...
											$cekKegiatan = mysql_num_rows(mysql_query("select * from view_rka_ppkd_3_1 where bk ='$bk' and ck='$ck' and p='$p' and q='$q' and id_tahap ='$idTahap' $kondisiRekening $kondisiSKPD and (rincian_perhitungan !='' or rincian_perhitungan!='') "));
											if($cekKegiatan == 0){
												$concat = $bk.".".$ck.".".$p;
												$arrKondisi[] = "concat(bk,'.',ck,'.',p)  !='$concat'";
											}else{
				...
				$grabNonMapingRekening= mysql_query("select * from view_rka_ppkd_3_1 where id_tahap ='$idTahap' and (LENGTH(k) > 1) and rincian_perhitungan=''");
				while($got = mysql_fetch_array($grabNonMapingRekening)){
					if(mysql_num_rows(mysql_query("select * from view_rka_ppkd_3_1 where id_tahap ='$idTahap' and k ='".$got['k']."' and l ='".$got['l']."' and m ='".$got['m']."' and n ='".$got['n']."' and o ='".$got['o']."' and rincian_perhitungan!='' $kondisiRekening $kondisiSKPD ")) != 0){
						$arrKondisi[] = "id_tahap = '$idTahap' or id_anggaran ='".$got['id_anggaran']."'";
					}
		...
		setcookie('publicKondisiPPKD31',$Kondisi);
		//Order -------------------------------------
...
function Laporan($xls =FALSE){
		...
		while($daqry = mysql_fetch_array($aqry)){
			...
			if($k == '0' && $n =='0' ){
				...
				$getSumJumlahHarga = mysql_fetch_array(mysql_query("select sum(jumlah_harga) from view_rka_ppkd_3_1 where  o1 ='$o1' $kondisiSKPD $kondisiFilter  "));
				...
				$getSumJumlahHarga = mysql_fetch_array(mysql_query("select sum(jumlah_harga) from view_rka_ppkd_3_1 where  k = '$k' and l='$l' and m='$m' and n='$n' and o='$o'  $kondisiSKPD $kondisiFilter"));
...
function formAlokasi($dt){	
	...
	function chooseReport($dt){	
	 ...
	 $this->form_width = 300;
	 $this->form_height = 80;
	 $this->form_caption = 'LAPORAN RKA PPKD 3.1';
	...
	function KertasKerja($listTabel,$xls =FALSE){
		...
		$qry ="select * from $this->TblName ".$_COOKIE['publicKondisiPPKD31']. " order by urut, rincian_perhitungan  asc";
		$aqry = mysql_query($qry);
		$getKuasapenggunaBarang = mysql_fetch_array(mysql_query("select * from ref_skpd where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1'"));
		$kuasaPenggunaBarang = $getKuasapenggunaBarang['nm_skpd'];		
...
$rkaPPKD31_v2_koreksi = new rkaPPKD31_v2_koreksiObj();
...
$rkaPPKD31_v2_koreksi->jenisForm = $jenisForm;
$rkaPPKD31_v2_koreksi->nomorUrut = $nomorUrut;
$rkaPPKD31_v2_koreksi->urutTerakhir = $nomorUrut;
$rkaPPKD31_v2_koreksi->tahun = $tahun;
$rkaPPKD31_v2_koreksi->jenisAnggaran = $jenisAnggaran;
$rkaPPKD31_v2_koreksi->idTahap = $idTahap;
$rkaPPKD31_v2_koreksi->username = $_COOKIE['coID'];
$getNamaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$idTahap'"));
$rkaPPKD31_v2_koreksi->namaTahap = $getNamaTahap['nama_tahap'];


$rkaPPKD31_v2_koreksi->wajibValidasi = $Main->wajibValidasi;
if($Main->wajibValidasi == TRUE){
	$rkaPPKD31_v2_koreksi->sqlValidasi = " and status_validasi ='1' ";
}else{
	$rkaPPKD31_v2_koreksi->sqlValidasi = " ";
}

if(empty($rkaPPKD31_v2_koreksi->tahun)){
	...
	/*$rkaPPKD31_v2_koreksi->tahun = "select max(id_anggaran) as max from view_rka_ppkd_3_1 where nama_modul = 'rkaPPKD31_v2_koreksi'";*/
	$rkaPPKD31_v2_koreksi->tahun  = $get2['tahun'];
	$rkaPPKD31_v2_koreksi->jenisAnggaran = $get2['jenis_anggaran'];
	$rkaPPKD31_v2_koreksi->urutTerakhir = $get2['no_urut'];
	$rkaPPKD31_v2_koreksi->jenisFormTerakhir = $get2['jenis_form_modul'];
	$rkaPPKD31_v2_koreksi->urutSebelumnya = $rkaPPKD31_v2_koreksi->urutTerakhir - 1;
	...
	$rkaPPKD31_v2_koreksi->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$rkaPPKD31_v2_koreksi->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];
	
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$rkaPPKD31_v2_koreksi->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$rkaPPKD31_v2_koreksi->idTahap'"));
	$rkaPPKD31_v2_koreksi->currentTahap = $getCurrenttahap['nama_tahap'];
	
	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$rkaPPKD31_v2_koreksi->idTahap'"));
	$rkaPPKD31_v2_koreksi->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$rkaPPKD31_v2_koreksi->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$rkaPPKD31_v2_koreksi->provinsi = $setting['provinsi'];
$rkaPPKD31_v2_koreksi->kota = $setting['kota'];
$rkaPPKD31_v2_koreksi->pengelolaBarang = $setting['pengelolaBarang'];
$rkaPPKD31_v2_koreksi->pejabatPengelolaBarang = $setting['pejabat'];
$rkaPPKD31_v2_koreksi->pengurusPengelolaBarang = $setting['pengurus'];
$rkaPPKD31_v2_koreksi->nipPengelola = $setting['nipPengelola'];
$rkaPPKD31_v2_koreksi->nipPengurus = $setting['nipPengurus'];
$rkaPPKD31_v2_koreksi->nipPejabat = $setting['nipPejabat'];

if($rkaPPKD31_v2_koreksi->jenisForm != "KOREKSI"){
$get1 = mysql_fetch_array(mysql_query("select max(id_anggaran)  from view_rka_ppkd_3_1 where jenis_form_modul = 'KOREKSI' "));
	$maxAnggaran = $get1['max(id_anggaran)'];
	$get2 = mysql_fetch_array(mysql_query("select * from view_rka_ppkd_3_1 where id_anggaran = '$maxAnggaran'"));
	$rkaPPKD31_v2_koreksi->tahun  = $get2['tahun'];
	$rkaPPKD31_v2_koreksi->jenisAnggaran = $get2['jenis_anggaran'];
	...
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$rkaPPKD31_v2_koreksi->currentTahap = $arrayHasil['currentTahap'];
			$rkaPPKD31_v2_koreksi->jenisForm = "";
			$rkaPPKD31_v2_koreksi->jenisFormTerakhir = "KOREKSI";
			$rkaPPKD31_v2_koreksi->idTahap = $get2['id_tahap'];
			$rkaPPKD31_v2_koreksi->nomorUrut = $get2['no_urut'];
			$rkaPPKD31_v2_koreksi->urutTerakhir = $get2['no_urut'];
