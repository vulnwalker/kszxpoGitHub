...
class rkaSKPD221_v2Obj  extends DaftarObj2{	
	var $Prefix = 'rkaSKPD221_v2';
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='rkaSKPD221_v2.xls';
	var $namaModulCetak='RKA';
	var $Cetak_Judul = 'RKA';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'rkaSKPD221_v2Form';
	var $modul = "RKA-SKPD";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	...
	$getJumlahBarang = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$rkaSKPD221_v2_idplh'"));
	...
	$sisaPaguIndikatif = $paguIndikatif - $getPaguYangTelahTerpakai['paguYangTerpakai'];
	 ...
	 if(mysql_num_rows(mysql_query("select * from view_rka_2_2_1 where c1='0' and f = '00' and k = '$k' and l ='$l' and m='$m' and n='$n' and o='$o'  and id_tahap='$this->idTahap' ")) > 0){
 	 ...
 	 if(empty($cmbJenisRKAForm) ){
		...
		$query = VulnWalkerUpdate('tabel_anggaran',$data,"id_anggaran = '$rkaSKPD221_v2_idplh'");
		mysql_query($query);
	 }
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){	
		...
		case 'Gruping':{	
			...
			if(!isset($rkaSKPD221_v2_cb)){
				$err = "Pilih Data";
			}
			$dt = implode(',',$rkaSKPD221_v2_cb);
		...
		case 'Remove':{
			...
			if(empty($cmbUnit))$cmbUnit = "00";
			if(empty($cmbSubUnit))$cmbSubUnit = "000";
		...
		case 'sesuai':{
			...
			$getrkaSKPD221_v2nya = mysql_fetch_array(mysql_query($queryRows));
			foreach ($getrkaSKPD221_v2nya as $key => $value) { 
				  $$key = $value; 
			} 
			$cmbUrusanForm = $c1;
			$cmbBidangForm = $c;
		...
		case 'editTab':{
			...
			$cek = "select * from view_rka_2_2_1 where concat(k,'.',l,'.',m,'.',n,'.',o) ='$kodeRek' and id_tahap='$this->idTahap' and (rincian_perhitungan !='' or f!='00' )   and c1='$cmbUrusan' and c='$cmbBidang' and d='$cmbSKPD'  and bk='$bk' and ck='$ck' and p='$hiddenP' and q='$q' and status_validasi !='1' order by o1, rincian_perhitungan";
	...
	<A href=\"pages.php?Pg=rka-skpd-2.2.1_v2\" title='RKA-SKPD MURNI' style='color:blue;' > RKA-SKPD 2.2.1 </a> |
	<A href=\"pages.php?Pg=rka-skpd-2.2_v2\" title='RKA-SKPKD MURNI' > RKA-SKPD 2.2 </a> |
	<A href=\"pages.php?Pg=rka-skpd-2.1_v2\" title='RKA-SKPKD MURNI' > RKA-SKPD 2.1 </a> |
   ...
   function setPage_OtherScript(){
		...
		return 	
			...
			<script type='text/javascript' src='js/perencanaan_v2/rka/rkaSKPD2.2.1.js' language='JavaScript' ></script> 
	 ...
	 $this->form_height = 400;
	  if ($this->form_fmST==0) {
		...
		foreach ($dt as $key => $value) { 
		 ...
		 $jenis_rka = $jenis_rka;
		 $cmbJenisRKA = cmbArray('cmbJenisRKAForm',$jenis_rka,$arrayJenisRKA,'-- JENIS RKA --','onchange=rkaSKPD221_v2.unlockFindRekening();');
	 	 if(empty($jenis_rka)){
	  ...
	  $this->form_fields = array(
			...
			'kode7' => array( 
						...
						'value'=> "<input type='text' id='kodeRekening' name='kodeRekening' style='width:120px;' readonly value = '$kodeRekening'> &nbsp&nbsp
						 ...
						 <button type='button' id='findRekening' onclick=rkaSKPD221_v2.findRekening('$jenis_rka'); $tergantungJenis> CARI </button> "
						 ),
			'kode8' => array( 
						'label'=>'HARGA SATUAN',
						'labelWidth'=>150, 
						'value'=> "<input type='text' name='hargaSatuan' id='hargaSatuan' value='$hargaSatuan' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='rkaSKPD221_v2.bantu();' > <span id='bantu' style='color:red;'> </span>"
						 ),	
...
/*		 $arrayResult = VulnWalkerTahap_v2('RKA');
	...
	function setKolomData($no, $isi, $Mode, $TampilCheckBox){
	 ...
	 //TAHAP PENYUSUNAN
			 ...
			 $aksi  = "<img src='images/administrator/images/valid.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkaSKPD221_v2.sesuai('$id_anggaran');></img>&nbsp  &nbsp <img src='images/administrator/images/invalid.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkaSKPD221_v2.koreksi('$id_anggaran');></img> ";
			 if ($rincian_perhitungan == '') {
			 $aksi = "";
			 }
			 if(empty($rincian_perhitungan)){
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	foreach ($_COOKIE as $key => $value) { 
		...
		$urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=rkaSKPD221_v2.refreshList(true);','-- URUSAN --');
		
		$codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
		$bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=rkaSKPD221_v2.refreshList(true);','-- BIDANG --');
		
		$codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
		$skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=rkaSKPD221_v2.refreshList(true);','-- SKPD --');
		...
		$unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=rkaSKPD221_v2.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=rkaSKPD221_v2.refreshList(true);','-- SUB UNIT --');
	...
	$program = "<input type='hidden' id='bk' name='bk' value='$selectedBK'> <input type='hidden' id='ck' name='ck' value='$selectedCK'> <input type='hidden' id='hiddenP' name='hiddenP' value='$selectedP'>".cmbArray('p',$_REQUEST['p'],$arrayP,'-- PROGRAM --','onchange=rkaSKPD221_v2.programChanged();');
	...
	$kegiatan = cmbArray('q',$_REQUEST['q'],$arrayQ,'-- KEGIATAN --','onchange=rkaSKPD221_v2.refreshList(true);');
	...
	$cmbBelanja = cmbArray('cmbBelanja',$_REQUEST['cmbBelanja'],$arrayBelanja,'-- JENIS BELANJA--','onchange=rkaSKPD221_v2.refreshList(true);');
...
$rkaSKPD221_v2 = new rkaSKPD221_v2Obj();
...
$rkaSKPD221_v2->jenisForm = $jenisForm;
$rkaSKPD221_v2->nomorUrut = $nomorUrut;
$rkaSKPD221_v2->urutTerakhir = $nomorUrut;
$rkaSKPD221_v2->tahun = $tahun;
$rkaSKPD221_v2->jenisAnggaran = $jenisAnggaran;
$rkaSKPD221_v2->idTahap = $idTahap;

$rkaSKPD221_v2->username = $_COOKIE['coID'];

$rkaSKPD221_v2->wajibValidasi = $Main->wajibValidasi;
if($Main->wajibValidasi == TRUE){
	$rkaSKPD221_v2->sqlValidasi = " and status_validasi ='1' ";
}else{
	$rkaSKPD221_v2->sqlValidasi = " ";
}


if(empty($rkaSKPD221_v2->tahun)){
	...
	/*$rkaSKPD221_v2->tahun = "select max(id_anggaran) as max from view_rka_2_2_1 where nama_modul = 'rkaSKPD221_v2'";*/
	$rkaSKPD221_v2->tahun  = $get2['tahun'];
	$rkaSKPD221_v2->jenisAnggaran = $get2['jenis_anggaran'];
	$rkaSKPD221_v2->urutTerakhir = $get2['no_urut'];
	$rkaSKPD221_v2->jenisFormTerakhir = $get2['jenis_form_modul'];
	$rkaSKPD221_v2->urutSebelumnya = $rkaSKPD221_v2->urutTerakhir - 1;
	...
	$rkaSKPD221_v2->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$rkaSKPD221_v2->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];
	
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$rkaSKPD221_v2->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$rkaSKPD221_v2->idTahap'"));
	$rkaSKPD221_v2->currentTahap = $getCurrenttahap['nama_tahap'];
	
	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$rkaSKPD221_v2->idTahap'"));
	$rkaSKPD221_v2->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$rkaSKPD221_v2->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$rkaSKPD221_v2->provinsi = $setting['provinsi'];
$rkaSKPD221_v2->kota = $setting['kota'];
$rkaSKPD221_v2->pengelolaBarang = $setting['pengelolaBarang'];
$rkaSKPD221_v2->pejabatPengelolaBarang = $setting['pejabat'];
$rkaSKPD221_v2->pengurusPengelolaBarang = $setting['pengurus'];
$rkaSKPD221_v2->nipPengelola = $setting['nipPengelola'];
$rkaSKPD221_v2->nipPengurus = $setting['nipPengurus'];
$rkaSKPD221_v2->nipPejabat = $setting['nipPejabat'];


if($rkaSKPD221_v2->jenisForm != "PENYUSUNAN"){
$get1 = mysql_fetch_array(mysql_query("select max(id_anggaran)  from view_rka_2_2_1 where jenis_form_modul = 'PENYUSUNAN' "));
	...
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$rkaSKPD221_v2->currentTahap = $arrayHasil['currentTahap'];
			$rkaSKPD221_v2->jenisForm = "";
			$rkaSKPD221_v2->jenisFormTerakhir = "PENYUSUNAN";
			$rkaSKPD221_v2->idTahap = $get2['id_tahap'];
			$rkaSKPD221_v2->nomorUrut = $get2['no_urut'];
			$rkaSKPD221_v2->urutTerakhir = $get2['no_urut'];
}
