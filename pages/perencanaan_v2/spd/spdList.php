...
class spdList_v2Obj  extends DaftarObj2{	
	var $Prefix = 'spdList_v2';
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='spdList_v2.xls';
	var $namaModulCetak='RKA';
	var $Cetak_Judul = 'RKA';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'spdList_v2Form';
	var $modul = "SPD";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	...
	$getJumlahBarang = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$spdList_v2_idplh'"));
	...
	$sisaPaguIndikatif = $paguIndikatif - $getPaguYangTelahTerpakai['paguYangTerpakai'];
	 ...
	 if(mysql_num_rows(mysql_query("select * from tabel_spd where c1='0' and f = '00' and k = '$k' and l ='$l' and m='$m' and n='$n' and o='$o'  and id_tahap='$this->idTahap' ")) > 0){
 	 ...
 	 if(empty($cmbJenisRKAForm) ){
		...
		$query = VulnWalkerUpdate('tabel_anggaran',$data,"id_anggaran = '$spdList_v2_idplh'");
		mysql_query($query);
	 }
	...
	<A href=\"pages.php?Pg=spd_v2\" title='INPUT'  > INPUT </a> |
	<A href=\"pages.php?Pg=spdList_v2\" title='LIST' style='color:blue;' > LIST </a> 
	&nbsp&nbsp&nbsp	
	</td></tr>
	</table>";
	}
   function setPage_OtherScript(){
		...
		//if($this->jenisForm == "PENYUSUNAN")
		$read = "<script type='text/javascript' src='js/perencanaan_v2/spd/spdList.js' language='JavaScript' ></script> ";
		return 	
			"
	 ...
	 $this->form_height = 400;
	  if ($this->form_fmST==0) {
		...
		foreach ($dt as $key => $value) { 
		 ...
		 $jenis_rka = $jenis_rka;
		 $cmbJenisRKA = cmbArray('cmbJenisRKAForm',$jenis_rka,$arrayJenisRKA,'-- JENIS RKA --','onchange=spdList_v2.unlockFindRekening();');
	 	 if(empty($jenis_rka)){
	  ...
	  $this->form_fields = array(
			...
			'kode7' => array( 
						...
						'value'=> "<input type='text' id='kodeRekening' name='kodeRekening' style='width:120px;' readonly value = '$kodeRekening'> &nbsp&nbsp
						 ...
						 <button type='button' id='findRekening' onclick=spdList_v2.findRekening('$jenis_rka'); $tergantungJenis> CARI </button> "
						 ),
			'kode8' => array( 
						'label'=>'HARGA SATUAN',
						'labelWidth'=>150, 
						'value'=> "<input type='text' name='hargaSatuan' id='hargaSatuan' value='$hargaSatuan' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='spdList_v2.bantu();' > <span id='bantu' style='color:red;'> </span>"
						 ),	
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	foreach ($_COOKIE as $key => $value) { 
		...
		$urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=spdList_v2.refreshList(true);','-- URUSAN --');
		
		$codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
		$bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=spdList_v2.refreshList(true);','-- BIDANG --');
		
		$codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
		$skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=spdList_v2.refreshList(true);','-- SKPD --');
		...
		$unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=spdList_v2.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=spdList_v2.refreshList(true);','-- SUB UNIT --');
	...
	$program = "<input type='hidden' id='bk' name='bk' value='$selectedBK'> <input type='hidden' id='ck' name='ck' value='$selectedCK'> <input type='hidden' id='hiddenP' name='hiddenP' value='$selectedP'>".cmbArray('p',$_REQUEST['p'],$arrayP,'-- PROGRAM --','onchange=spdList_v2.programChanged();');
	...
	$kegiatan = cmbArray('q',$_REQUEST['q'],$arrayQ,'-- KEGIATAN --','onchange=spdList_v2.refreshList(true);');
	...
	//$cmbBelanja = cmbArray('cmbBelanja',$_REQUEST['cmbBelanja'],$arrayBelanja,'-- JENIS BELANJA--','onchange=spdList_v2.refreshList(true);');
	$cmbDPA = cmbArray('cmbDPA',$_REQUEST['cmbDPA'],$arrayDPA,'-- JENIS DPA --','onchange=spdList_v2.refreshList(true);');
...
$spdList_v2 = new spdList_v2Obj();

$arrayResult = VulnWalkerTahap_v2($spdList_v2->modul);
...
$spdList_v2->jenisForm = $jenisForm;
$spdList_v2->nomorUrut = $nomorUrut;
$spdList_v2->tahun = $tahun;
$spdList_v2->jenisAnggaran = $jenisAnggaran;
$spdList_v2->idTahap = $idTahap;
$spdList_v2->username = $_COOKIE['coID'];

$spdList_v2->wajibValidasi = $Main->wajibValidasi;
if($Main->wajibValidasi == TRUE){
	$spdList_v2->sqlValidasi = " and status_validasi ='1' ";
}else{
	$spdList_v2->sqlValidasi = " ";
}

if(empty($spdList_v2->tahun)){
	...
	/*$spdList_v2->tahun = "select max(id_anggaran) as max from tabel_spd where nama_modul = 'spdList_v2'";*/
	$spdList_v2->tahun  = $get2['tahun'];
	$spdList_v2->jenisAnggaran = $get2['jenis_anggaran'];
	$spdList_v2->urutTerakhir = $get2['no_urut'];
	$spdList_v2->jenisFormTerakhir = $get2['jenis_form_modul'];
	$spdList_v2->urutSebelumnya = $spdList_v2->urutTerakhir - 1;
	
	
	$idtahapTerakhir = $get2['id_tahap'];
	$spdList_v2->idTahap = $idtahapTerakhir;
	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$idtahapTerakhir'"));
	$spdList_v2->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$spdList_v2->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];
	
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$spdList_v2->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$spdList_v2->idTahap'"));
	$spdList_v2->currentTahap = $getCurrenttahap['nama_tahap'];
	
	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$spdList_v2->idTahap'"));
	$spdList_v2->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$spdList_v2->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$spdList_v2->provinsi = $setting['provinsi'];
$spdList_v2->kota = $setting['kota'];
$spdList_v2->pengelolaBarang = $setting['pengelolaBarang'];
$spdList_v2->pejabatPengelolaBarang = $setting['pejabat'];
$spdList_v2->pengurusPengelolaBarang = $setting['pengurus'];
$spdList_v2->nipPengelola = $setting['nipPengelola'];
$spdList_v2->nipPengurus = $setting['nipPengurus'];
$spdList_v2->nipPejabat = $setting['nipPejabat'];
