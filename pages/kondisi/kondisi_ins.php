...
class kondisi_insObj extends DaftarObj2{
	var $Prefix = 'kondisi_ins';
	var $SHOW_CEK = TRUE;	
	...
	var $FormName = 'kondisi_insForm';
	var $pagePerHal = 25;
	
	var $PageTitle = 'RECLASS RB';
	var $PageIcon = 'images/penatausahaan_ico.gif';
	var $ico_width = '20';
	var $ico_height = '30';
	
	var $fileNameExcel='Daftar KONDISI.xls';
	var $Cetak_Judul = 'RECLASS RB';	
	var $Cetak_Mode=2;
	...
	/*if($err=='' && sudahClosing($tgl,$old['c'],$old['d'],$old['e'],$old['e1'],$old['c1']))$err = 'Tanggal sudah Closing !';
	// - tanggal harus <= tgl hari ini
	//tgl >=tgl_buku
	if( compareTanggal( $tgl,$old['tgl_buku'])==0  ) $err = "TANGGAL BUKU tidak lebih kecil dari TANGGAL BUKU BARANG!";
	...
	if($err=='' && compareTanggal($tgl, $maxtrans['maxtgl'])==0  ) $err = 'TANGGAL BUKU tidak lebih kecil dari tanggal transaksi sebelumnya!';*/
		...
		if($err==''){
			$limit=2;
			$getcnt = "select * from buku_induk where id in ($idplh) and kondisi !=$kondisi_baru and idawal not in(select idbi_awal from t_kondisi where idbi_awal in($idplh) and kond_awal !=0) limit 0,$limit";			
			//$err='qyr='.$getcnt;
			...
			$content->jml = $jml;	
				...
				while($old = mysql_fetch_array($result)){
					...
					if($err==''){
						...
						" values('$tgl','$idbi','$uid',now(),'$ket', '$kondisi','$kondisi_baru','".$bi['idawal']."','$dif_kondisi','$no_sk','$tgl_sk','6','$staset_lama') "; $cek .= $aqry;
	...
	$progress = 
		...
		$TampilOpt =
			...
			genFilterBar(
				...
				genFilterBar(
				array(
					"<span id='inputpenerimaanbarang' style='color:black;font-size:14px;font-weight:bold;'/>SK RECLASS RB</span>",
				),'','','').
				genFilterBar(
				array(
					$this->isiform(
	...
	function set_selector_other($tipe){
		...
		switch($tipe){
			...
			case 'cek_kondisi':{
				...
				$cekdata=mysql_fetch_array(mysql_query("select kondisi from view_buku_induk2 where id='".$cbid[$i]."'"));
						
					if($cekdata['kondisi']==$Main->KondisiBarang[2][0]){
						//$errmsg="Id Barang ".$cbid[$i]." sudah ".$Main->KondisiBarang[2][1];
						//$get= $this->cek_kondisi();
						$cek = $get['cek'];
						$err = $errmsg;
						$content = $get['content'];
					}	
...
$kondisi_ins = new kondisi_insObj();
