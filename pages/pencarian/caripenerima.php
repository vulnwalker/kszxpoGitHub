...
class caripenerimaObj  extends DaftarObj2{	
	var $Prefix = 'caripenerima';
   ...
   function setPage_OtherScript(){
	...
	function setKolomData($no, $isi, $Mode, $TampilCheckBox){
	 global $Ref;
	 $nama_penyedia = $isi['nama_penyedia'];
	 ...
	 $Koloms[] = array('align="left"', $isi['alamat']." ".$isi['kota']);
	 $Koloms[] = array('align="left"', $isi['nama_pimpinan']);
	 $Koloms[] = array('align="left"', $isi['nama_bank']."/ <br>".$isi['norek_bank']);
	...
	function genDaftarOpsi(){
	 ...
	 $arr = array(
				array('nama_penyedia','NAMA'),		
				array('alamat','ALAMAT'),		
				array('kota','KOTA'),		
				array('nama_pimpinan','PIMPINAN'),		
				array('no_npwp','NO NPWP'),		
				array('nama_bank','NAMA BANK'),		
			);
	...
	function pilData(){
		...
		if($err == ""){
			$qry = $DataPengaturan->QryHitungData("ref_penyedia","WHERE id='$id' AND c1='$c1' AND c='$c' AND d='$d'");
...
$caripenerima = new caripenerimaObj();
?>