...
//		 $ErrMsg.=$ID.' - '.$_POST['tgl_pemindahtanganan'].' - '.$_POST['Pindahtangan_fmST'].' - '.$Pindahtangan->Prefix.'_fmST';
		...
		if ($ErrMsg==''){
			if ($_POST[$PindahTangan->Prefix.'_fmST']==0){//baru
				...
				if ($ErrMsg =='' && $idbi == '') $ErrMsg = "Data dengan id '{$_POST[$PindahTangan->Prefix.'_idplh']}' tidak ada!";
				...
				if($ErrMsg=='' && $old['tgl_pemindahtanganan']<=$tgl_closing){ //jika data pemeliharaan dicatat sebelum closing
					if($ErrMsg=='' && $old['tgl_pemindahtanganan']!=$_POST['tgl_pemindahtanganan'])$ErrMsg = 'Tanggal Buku Pemindahtanganan tidak bisa di edit,karena sudah closing !';
					if($ErrMsg=='' && $old['nosk']!=$_POST['nosk'])$ErrMsg = 'Nomor SK tidak bisa di edit,karena sudah closing !';
					if($ErrMsg=='' && $old['tglsk']!=$_POST['tglsk'])$ErrMsg = 'Tgl. SK	 tidak bisa di edit,karena sudah closing !';
					if($ErrMsg=='' && $old['kepada_nama']!=$_POST['kepada_nama'])$ErrMsg = 'Nama tidak bisa di edit,karena sudah closing !';
					if($ErrMsg=='' && $old['kepada_alamat']!=$_POST['kepada_alamat'])$ErrMsg = 'Alamat tidak bisa di edit,karena sudah closing !';
					if($ErrMsg=='' && $old['uraian']!=$_POST['uraian'])$ErrMsg = 'Uraian tidak bisa di edit,karena sudah closing !';
				}	
			}			
			$pelihara = mysql_fetch_array( mysql_query ("select max(tgl_pemeliharaan) as maxtgl from pemeliharaan where id_bukuinduk = '$idbi'"	));
			if ($ErrMsg =='' && compareTanggal($_POST['tgl_pemindahtanganan'], $pelihara['maxtgl'])==0) 
			...
			if ($ErrMsg =='' && compareTanggal($_POST['tgl_pemindahtanganan'], $pengaman['maxtgl'])==0) 
			...
			if ($ErrMsg =='' && compareTanggal($_POST['tgl_pemindahtanganan'], $pemanfaat['maxtgl'])==0) 
		...
		if($Main->VERSI_NAME <> 'SERANG'){
			...
			$get_cd = mysql_fetch_array(mysql_query("select c1,c,d,e,e1 from buku_induk where id='$idbi'"));
			...
			if ($_POST[$PindahTangan->Prefix.'_fmST']==0){
				...
				if($ErrMsg=='' && sudahClosing($fmTANGGALpemindahtanganan,$get_cd['c'],$get_cd['d'],$get_cd['e'],$get_cd['e1'],$get_cd['c1']))$ErrMsg = 'Tanggal sudah Closing !';
			}else{
				...
				$get_cd = mysql_fetch_array(mysql_query("select c1,c,d,e,e1 from buku_induk where id='$idbi'"));
				if($ErrMsg=='' && sudahClosing($fmTANGGALpemindahtanganan,$get_cd['c'],$get_cd['d'],$get_cd['e'],$get_cd['e1'],$get_cd['c1'])){
		...
		$cbid= $_POST[$PindahTangan->Prefix.'_cb'];	
		 ...
		 $err=Pindahtangan_CekdataCutoff('hapus',$cbid[$i],$_POST['tgl_pemindahtanganan'],$idbi);	
			...
			if($Main->VERSI_NAME <> 'SERANG'){
				...
				$get_cd = mysql_fetch_array(mysql_query("select c1,c,d,e,e1 from buku_induk where id='$idbi'"));
				if(sudahClosing($fmTANGGALpemindahtanganan,$get_cd['c'],$get_cd['d'],$get_cd['e'],$get_cd['e1'],$get_cd['c1'])){
		...
		$script ="
			...
			if (empty($disModul09) && empty($ridModul09)){
				$toolbar_atas_edit = 
					...
					<td>".PanelIcon1("javascript:Pindahtangan.Hapus()","delete_f2.png","Batal")."</td>";				
			}	
