...
class saldo_awal_keuangan_lo_insObj  extends DaftarObj2{	
	var $Prefix = 'saldo_awal_keuangan_lo_ins';
	...
	var $PageTitle = 'Data Saldo Awal Keuangan - LO';
	...
	var $FormName = 'saldo_awal_keuangan_lo_insForm';
	var $noModul=14; 
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){	
		...
		case 'Editinput':{
			...
			$content['koderek'] = "
				<input type='text' onkeyup='setTimeout(function myFunction() {saldo_awal_keuangan_lo_ins.namajurnal();},1000);' name='Id_jurnal' id='Id_jurnal' value='".$dt['ka'].".".$dt['kb'].".".$dt['kc'].".".$dt['kd'].".".$dt['ke']."' style='width:70px;' maxlength='25' />"
			...
			$content['jml_saldo'] = "<input type='text' name='jml_saldo' id='jml_saldo' value='".floatval($jml_saldo_)."' style='text-align:right;' onkeypress='return isNumberKey(event)' onkeyup='document.getElementById(`formatsaldo`).innerHTML = saldo_awal_keuangan_lo_ins.formatCurrency(this.value);' />
							<span id='formatsaldo'></span>";
							
			$content['jml_anggaran'] = "<input type='text' name='jml_anggaran' id='jml_anggaran' value='".floatval($dt['anggaran'])."' style='text-align:right;' onkeypress='return isNumberKey(event)' onkeyup='document.getElementById(`formatanggaran`).innerHTML = saldo_awal_keuangan_lo_ins.formatCurrency(this.value);' />
							<span id='formatanggaran'></span>";
											
			$content['idrek'] = $idrek;
			$content['option'] = "
				<a href='javascript:saldo_awal_keuangan_lo_ins.updKodeJurnal()' />
			...
			$content['atasbutton'] = "<a href='javascript:saldo_awal_keuangan_lo_ins.dat_saldo_awal()' /><img src='datepicker/cancel.png' style='width:20px;height:20px;' /></a>";
		break;
	    }
		...
		$IDUBAH = $_REQUEST['idubah'];
		if ($idplh!=0){
			$qry_jurnal_det="INSERT INTO t_saldo_keu_det (refid_jurnal,status,tipe_jurnal,thn_anggaran) values ('$idplh','1','3','$coThnAnggaran')";$cek.=$qry_jurnal_det;
			$qry_jurnal_keuangan_det = mysql_query($qry_jurnal_det);
			
		}else{
			$qry_jurnal_det="INSERT INTO t_saldo_keu_det (refid_jurnal,status,tipe_jurnal,thn_anggaran) values ('$IDUBAH','1','3','$coThnAnggaran')";$cek.=$qry_jurnal_det;
		$qry_jurnal_keuangan_det = mysql_query($qry_jurnal_det);
		}
			$get= $this->data_saldo_awal();
			$cek = $get['cek'];
		...
		//	$qry = "SELECT * FROM t_saldo_keu_det WHERE Id='$idrek' and status='2'";$cek.=$qry;
			$qry = "SELECT * FROM t_saldo_keu_det WHERE Id='$idrek'";$cek.=$qry;
			$aqry = mysql_query($qry);
		...
		//	if($cek_status['status']!='2'){
			...
			$kode = explode(".",$Id_jurnal);
				...
				$kd_6='6';
				$kd_7='7';
		//	$kode_jurnal=mysql_fetch_array(mysql_query("select * from ref_jurnal where ka in(1,2,3)"));	
			if($ka ==$kd_1) $err="Kode Akun '$ka.$kb.$kc.$kd.$ke' tidak bisa di input";
			if($ka ==$kd_2) $err="Kode Akun '$ka.$kb.$kc.$kd.$ke' tidak bisa di input";
			if($ka ==$kd_3) $err="Kode Akun '$ka.$kb.$kc.$kd.$ke' tidak bisa di input";
			if($ka ==$kd_4) $err="Kode Akun '$ka.$kb.$kc.$kd.$ke' tidak bisa di input";
			if($ka ==$kd_5) $err="Kode Akun '$ka.$kb.$kc.$kd.$ke' tidak bisa di input";
			if($ka ==$kd_6) $err="Kode Akun '$ka.$kb.$kc.$kd.$ke' tidak bisa di input";
			if($ka ==$kd_7) $err="Kode Akun '$ka.$kb.$kc.$kd.$ke' tidak bisa di input";
		...
		//	if($ka ==$ka && $kb==$kb && $kc=='') $err='Kode Akun 2';
		//	if($ka ==$ka && $kb==$kb && $kc==$kc && $kd=='') $err='Kode Akun 3';
			...
			$cek_dat=mysql_fetch_array(mysql_query("select count(*) as cnt from t_saldo_keu_det where ka='$ka' and kb='$kb' and kc='$kc' and kd='$kd' and ke='$ke' and c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and tipe_jurnal='3'"));
			$cek.="select count(*) as cnt from t_saldo_keu_det where ka='$ka' and kb='$kb' and kc='$kc' and kd='$kd' and ke='$ke' and c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and tipe_jurnal='3'";
		...
		//	$kode_jurnal=mysql_fetch_array(mysql_query("select * from ref_jurnal where ka in(1,2,3)"));	
			...
			$cek_dat=mysql_fetch_array(mysql_query("select * from t_saldo_keu_det where ka='$ka' and kb='$kb' and kc='$kc' and kd='$kd' and ke='$ke' and c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and tipe_jurnal='3'"));
			$cek_dat2=mysql_fetch_array(mysql_query("select count(*) as cnt,Id from t_saldo_keu_det where ka='$ka' and kb='$kb' and kc='$kc' and kd='$kd' and ke='$ke' and c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and tipe_jurnal='3'"));
   ...
   function setNavAtas(){
	...
	if($total!=0){
		...
		$aqry ="UPDATE t_saldo_keu_det set status='3' ,jns_jurnal='$jnsJurnal',tgl_anggaran='$tgl_anggaran2',tgl_saldo_awal='$tgl_saldo_awal2',tipe_jurnal='3',tgl_create=NOW(),uid='$uid' where refid_jurnal='".$IDUBAH."'";$cek.=$aqry;
	...
	if($total==0){
		...
		$aqry ="UPDATE t_saldo_keu_det set status='0' ,jns_jurnal='$jnsJurnal',tgl_saldo_awal='$tgl_saldo_awal2',tgl_anggaran='$tgl_anggaran2',tipe_jurnal='3',tgl_create=NOW(),uid='$uid' where refid_jurnal='".$IDUBAH."'";$cek.=$aqry;
		$qry = mysql_query($aqry);
	...
	$jurnal_cek=mysql_fetch_array(mysql_query("select count(*) as cnt from t_saldo_keu_det where refid_jurnal='$IDUBAH'"));
		...
		genFilterBar(
				array("<table >			
						<tr>
						<span style='color:black;font-size:14px;font-weight:bold;'>RINCIAN SALDO AWAL KEUANGAN - LO</span>
						</tr>
						</table>
				"),'','').
   ...
   function TampilDetailJurnal(){
		...
		$cek.=$hapus_det;
		$pil_jns_tran = "<select style='width:300px;' onchange='saldo_awal_keuangan_lo_ins.bukarekening()' id='jns_transaksi' name='jns_transaksi'>";
   ...
   function data_saldo_awal(){
	...
	//new	
		...
		while($dt = mysql_fetch_array($aqry)){
			...
			$kode_dt = sprintf("%02s",$dt['kd']);
			$kode_de = sprintf("%02s",$dt['ke']);
			$kode = "<a href='javascript:saldo_awal_keuangan_lo_ins.Editinput(`".$dt['Id']."`,`".$dt['ka'].".".$dt['kb'].".".$dt['kc'].".".$kode_dt.".".$kode_de.".".$dt['kf']."`);' />
				...
				$btn ="
				<a href='javascript:saldo_awal_keuangan_lo_ins.HapusJurnal_1(`".$dt['Id']."`)' />
			...
			$kode = "<input type='text' onkeyup='setTimeout(function myFunction() {saldo_awal_keuangan_lo_ins.namajurnal();},1000);' name='Id_jurnal' id='Id_jurnal' value='".$dt['ka'].".".$dt['kb'].".".$dt['kc'].".".$dt['kd'].".".$dt['ke']."' style='width:70px;' maxlength='25' />"
			."<a href='javascript:cr_jurnal.windowShow(".$dt['Id'].");'> <img src='datepicker/search.png' style='width:20px;height:20px;margin-bottom:-5px;'  /></a>";	
			...
			$nama_jurnal='';			
			$jml_saldo ="<input type='text' name='jml_saldo' id='jml_saldo' value='".floatval($dt['jumlah'])."' style='text-align:right' ;' onkeypress='return isNumberKey(event)' onkeyup='document.getElementById(`formatsaldo`).innerHTML = saldo_awal_keuangan_lo_ins.formatCurrency(this.value);' />
							<span id='formatsaldo'></span>";
			$jml_anggaran ="<input type='text' name='jml_anggaran' id='jml_anggaran' value='".floatval($dt['jumlah'])."' style='text-align:right' ;' onkeypress='return isNumberKey(event)' onkeyup='document.getElementById(`formatanggaran`).innerHTML = saldo_awal_keuangan_lo_ins.formatCurrency(this.value);' />
							<span id='formatanggaran'></span>";
		...
		$kode_de = sprintf("%02s",$dt['ke']);
			...
			if($dt['status'] == '0'){
				$kode = "
					<a href='javascript:saldo_awal_keuangan_lo_ins.Editinput(`".$dt['Id']."`,`".$dt['ka'].".".$dt['kb'].".".$dt['kc'].".".$dt['kd'].".".$kode_de.".".$dt['kf']."`);' />
						".$dt['ka'].".".$dt['kb'].".".$dt['kc'].".".$kode_dt.".".$kode_de."
					</a>";
			...
			$jml_saldo_ ="<input type='text' name='jml_saldo' id='jml_saldo' value='".floatval($dt['jumlah'])."' style='text-align:right' ;' onkeypress='return isNumberKey(event)' onkeyup='document.getElementById(`formatsaldo`).innerHTML = saldo_awal_keuangan_lo_ins.formatCurrency(this.value);' />
							<span id='formatsaldo'></span>";
			$jml_anggaran_ ="<input type='text' name='jml_anggaran' id='jml_anggaran' value='".floatval($dt['jumlah'])."' style='text-align:right' ;' onkeypress='return isNumberKey(event)' onkeyup='document.getElementById(`formatanggaran`).innerHTML = saldo_awal_keuangan_lo_ins.formatCurrency(this.value);' />
							<span id='formatanggaran'></span>";
			...
			$kode = "
					<a href='javascript:saldo_awal_keuangan_lo_ins.Editinput(`".$dt['Id']."`,`".$dt['ka'].".".$dt['kb'].".".$dt['kc'].".".$dt['kd'].".".$dt['ke'].".".$dt['kf']."`);' />
		...
		$TombolBaru = "<a href='javascript:saldo_awal_keuangan_lo_ins.input_Jurnal()' /><img src='datepicker/add-256.png' style='width:20px;height:20px;' /></a>";
	...
	$content['atasbutton'] = "<a href='javascript:saldo_awal_keuangan_lo_ins.dat_saldo_awal(`".$dt['Id']."`)' /><img src='datepicker/cancel.png' style='width:20px;height:20px;' /></a>";
		return	array ('cek'=>$cek, 'err'=>$err, 'content'=>$content);
	}
   ...
   function SimpanAll(){
	...
	function setPage_OtherScript(){
		...
		return 	
			...
			"<script type='text/javascript' src='js/saldo_awal_keuangan/saldo_awal_keuangan_lo_ins.js' language='JavaScript' ></script>".
			'
	...
	function pageShow(){
		...
		$cbid = $_REQUEST['saldo_awal_keuangan_lo_cb'];
		if(addslashes($_REQUEST['YN']) == '2')$cbid[0]='';
		
			$c1input = $_REQUEST['saldo_awal_keuangan_loSKPDfmURUSAN'];
			$cinput = $_REQUEST['saldo_awal_keuangan_loSKPDfmSKPD'];
			$dinput = $_REQUEST['saldo_awal_keuangan_loSKPDfmUNIT'];
			$einput = $_REQUEST['saldo_awal_keuangan_loSKPDfmSUBUNIT'];
			$e1input = $_REQUEST['saldo_awal_keuangan_loSKPDfmSEKSI'];
		...
		"<html>".
			...
			"<table id='KerangkaHal' class='menubar' cellspacing='0' cellpadding='0' border='0' width='100%' height='100%' >".
				...
				"<tr height='*' valign='top'> <td >".
					...
					"<div class='main' >".
					$form1.
					"<input type='hidden' name='saldo_awal_keuangan_loSKPDfmURUSAN' value='".$c1input."' />".
					"<input type='hidden' name='saldo_awal_keuangan_loSKPDfmSKPD' value='".$cinput."' />".
					"<input type='hidden' name='saldo_awal_keuangan_loSKPDfmUNIT' value='".$dinput."' />".
					"<input type='hidden' name='saldo_awal_keuangan_loSKPDfmSUBUNIT' value='".$einput."' />".
					"<input type='hidden' name='saldo_awal_keuangan_loSKPDfmSEKSI' value='".$e1input."' />".
	...
	//	if(addslashes($_REQUEST['databaru'] == '1')){
		...
		$c1 = $_REQUEST['saldo_awal_keuangan_loSKPDfmURUSAN'];
		$c = $_REQUEST['saldo_awal_keuangan_loSKPDfmSKPD'];
		$d = $_REQUEST['saldo_awal_keuangan_loSKPDfmUNIT'];
		$e = $_REQUEST['saldo_awal_keuangan_loSKPDfmSUBUNIT'];
		$e1 = $_REQUEST['saldo_awal_keuangan_loSKPDfmSEKSI'];
	...
	"<tr><td>".
		...
		genFilterBar(
			...
			"<table>			
			<tr>
			<td style='width:130px'>URUSAN 1</td><td style='width:10px'>:</td>
			<td>
			<input type='text' id='c1x' name='c1x' value='".$dat_c1."' size=50px readonly>
			<input type='hidden' id='c1' name='c1' value='".$qry_c1['c1']."' size=50px>
			</td>
...
$saldo_awal_keuangan_lo_ins = new saldo_awal_keuangan_lo_insObj();
?>