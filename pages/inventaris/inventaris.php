...
class InventarisObj extends DaftarObj2{
	var $Prefix = 'Inventaris';
	var $SHOW_CEK = TRUE;
	...
	var $fileNameExcel='Inventaris.xls';
	var $Cetak_Judul = 'Inventaris';
	var $Cetak_Mode=2;
...
".
					...
					"Gedung &nbsp; ".						
						...
						genComboBoxQry2( 'fmPILGEDUNG', $fmPILGEDUNG, 
							...
							array('c1','c','d','e','e1','p'), 'nm_gedung', '-- Semua Gedung --',"onChange=\"Inventaris.pilihGedungOnchange()\"" ).
						"</span>".
					"</div>".
					"<div style='float:left;margin: 0 8 0 4;border-left: 1px solid #E5E5E5;height:22'></div>
					<div  style='float:left;padding: 0 4 0 0;height:22;padding: 4 4 0 0'> ".
					"Ruang &nbsp".
						...
						genComboBoxQry( 'fmPILRUANG', $fmPILRUANG, 
							...
							'q', 'nm_ruang', '-- Semua Ruang --',"style='' onChange=\"Inventaris.refreshList(true);Inventaris.tampilPJRuang();\" "  ).
						"</span>".
					"</div>".
					"<div style='float:left;margin: 0 8 0 4;border-left: 1px solid #E5E5E5;height:22'></div>".										
					"<div  style='float:left;padding: 0 4 0 0;height:22;padding: 4 4 0 0'> ".
	...
	function genDaftarInitialForm($fmId){
		$vOpsi = $this->genDaftarOpsi();
		$Id=explode("-",$fmId);
		$kdbarang=$fmId==""?"":$Id[0].".".$Id[1].".".$Id[2].".".$Id[3].".".$Id[4];
		return
...
//							$ISI7 = "{$isiKIB_E['jenis']}";
		...
		if($dtInv){
			$tmplKondisi="<span id='kondisi_".$isi['id']."'><a href='javascript:Inventaris.ProsesInventaris(2,".$isi['id'].")' />$kondisi</a></span>";
			$tmplAda="<span id='ada_".$isi['id']."'><a href='javascript:Inventaris.ProsesInventaris(2,".$isi['id'].")' />$Ada</a></span>";
			$tmplstatus="<span id='status_".$isi['id']."'><a href='javascript:Inventaris.ProsesInventaris(2,".$isi['id'].")' style='color:#0000ff;'/>Sudah</a></span>";				
			$tmpltombol="<span id='tombol_".$isi['id']."'><a href='javascript:Inventaris.ProsesInventaris(3,".$isi['id'].")' /><img src='datepicker/remove2.png' title='Hapus' style='width:20px;height:20px;' /></a></span>";	
		}else{
			$tmplKondisi="<span id='kondisi_".$isi['id']."'><a href='javascript:Inventaris.ProsesInventaris(1,".$isi['id'].")' title='Baru'/>-</a></span>";
			$tmplAda="<span id='ada_".$isi['id']."'><a href='javascript:Inventaris.ProsesInventaris(1,".$isi['id'].")' title='Baru'/>-</a></span>";
			$tmplstatus="<span id='status_".$isi['id']."'><a href='javascript:Inventaris.ProsesInventaris(1,".$isi['id'].")' title='Baru'/>Belum</a></span>";
	...
	function prosesInventaris(){ 
		global $Main, $HTTP_COOKIE_VARS;
			...
			if($mode==1){ //baru
				...
				$content->tombol="<a href='javascript:Inventaris.Updinventaris(1,$idbi)' />
									<img src='datepicker/save.png' title='Simpan' style='width:20px;height:20px;' />
								</a>
								<a href='javascript:Inventaris.ProsesInventaris(4,$idbi)' />
				...
				$content->tombol="<a href='javascript:Inventaris.Updinventaris(2,$idbi)' />
									<img src='datepicker/save.png' title='Simpan' style='width:20px;height:20px;' />
								</a>
								<a href='javascript:Inventaris.ProsesInventaris(4,$idbi)' />
									<img src='datepicker/remove2.png' title='Batal' style='width:20px;height:20px;' />
			...
			if($idbiplh != 0){ 
				...
				if($dtInvawl){
					$content->kondisiawl="<a href='javascript:Inventaris.ProsesInventaris(2,".$idbiplh.")' />$kondisi</a>";
					$content->adaawl="<a href='javascript:Inventaris.ProsesInventaris(2,".$idbiplh.")' />$Ada</a>";
					$content->statusawl="<a href='javascript:Inventaris.ProsesInventaris(2,".$idbiplh.")'  style='color:#0000ff;'/>Sudah</a>";				
					$content->tombolawl="<a href='javascript:Inventaris.ProsesInventaris(3,".$idbiplh.")' /><img src='datepicker/remove2.png' style='width:20px;height:20px;' /></a>";	
				}else{
					$content->kondisiawl="<a href='javascript:Inventaris.ProsesInventaris(1,".$idbiplh.")' />-</a>";
					$content->adaawl="<a href='javascript:Inventaris.ProsesInventaris(1,".$idbiplh.")' />-</a>";
					$content->statusawl="<a href='javascript:Inventaris.ProsesInventaris(1,".$idbiplh.")' />Belum</a>";
					$content->tombolawl="";
				}
			}
			//$content = $tmplInfo;
	...
	function getData(){ 
		...
		while($row=mysql_fetch_array($aqry)){
			$queryInventaris="select count(*) as jml_data from inventaris WHERE idbi='".$row['id']."'";
			$cntI=mysql_num_rows(mysql_query($queryInventaris));
			$dtI=mysql_fetch_array(mysql_query($queryInventaris));
			if($cntI['jml_data']==0){ //input ke inventaris jika belum ada di inventaris
		...
		$content->ada=$jmlAda;	
		$content->tidak=$jmlTidak;			
		$content->status=$tmplstatus;
		$content->baik=$jmlBaik;
		$content->kurang=$jmlKurang;
		$content->rusak=$jmlRusak;
		$content->color=$color;						
		return	array ('cek'=>$cek, 'err'=>$err, 'content'=>$content, 'json'=>$json);
	}		
