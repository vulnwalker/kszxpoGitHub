...
$fmcaraperolehan = cekPOST("fmcaraperolehan");
...
$fmKIB = $_POST['fmKIB'];
if($fmKIB==01){
	$genKonKIB ="and concat(f1,'.',f2,'.',f)= '1.3.01'";
}elseif($fmKIB==02){
	$genKonKIB ="and concat(f1,'.',f2,'.',f)= '1.3.02'";
}elseif($fmKIB==03){
	$genKonKIB ="and concat(f1,'.',f2,'.',f)= '1.3.03'";
}elseif($fmKIB==04){
	$genKonKIB ="and concat(f1,'.',f2,'.',f)= '1.3.04'";
}elseif($fmKIB==05){
	$genKonKIB ="and concat(f1,'.',f2,'.',f)= '1.3.05'";
}elseif($fmKIB==06){
	$genKonKIB ="and concat(f1,'.',f2,'.',f)= '1.3.06'";
}elseif($fmKIB==07){
	$genKonKIB ="and concat(f1,'.',f2,'.',f)= '1.5.03'";
}else{
	$genKonKIB ="";
}
...
$fmcaraperolehan = $_POST['fmcaraperolehan'];
$kondF = $Main->KD_BARANG_P108?$genKonKIB:"and f='$fmKIB'";
...
if (!empty($fmKIB)) $Kondisi .= $kondF ;
//if (!empty($fmPilihThn))
$Kondisi .= !empty($fmPilihThn) ? " and year(tgl_pemeliharaan)='$fmPilihThn'" : " and year(tgl_pemeliharaan)='$thn_login'";
if (!empty($fmPilihThnPerolehan)) $Kondisi .= " and year(tgl_perolehan)='$fmPilihThnPerolehan' ";
if (!empty($fmcaraperolehan)) $Kondisi .= " and cara_perolehan='$fmcaraperolehan' ";
if(!empty($fmcr_data)){
	switch($fmcr_data){			
...
$PilihCaper = "<div style='float:left;padding:1 8 0 8; border-right:1px solid #E5E5E5;'> 
Cara Perolehan : ".cmb2D_v2('fmcaraperolehan',$fmcaraperolehan,$ArrCaper,'','--- PILIH ---')."
</div>";	
...
cmbQuery('fmPilihThn',$fmPilihThn,'select year(tgl_pemeliharaan)as thn,year(tgl_pemeliharaan)as thn from v_pemelihara group by thn desc',
'', 'Tahun Pemeliharaan', '').
"</div>";*/
$PilihThn = " <div style='float:left;padding:1 8 0 8; border-right:1px solid #E5E5E5;'>
Tahun : <input type=text name='fmPilihThn' value='$fmPilihThn' size='4' readonly></div>";
