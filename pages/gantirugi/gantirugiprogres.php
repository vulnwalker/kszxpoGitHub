...
class gantirugiprogresObj  extends DaftarObj2{	
	...
	function HapusSK(){
		 $err=''; $cek='';
		 $id_plh = $_REQUEST['gantirugiprogres_cb'];
		for($i = 0; $i<count($id_plh); $i++){
			//cek id terakhir
			$cekdata=mysql_fetch_array(mysql_query("select * from v_gantirugi where id='".$id_plh[$i]."'"));
			if($cekdata['sisa']!=0 ){
				$err="Tidak bisa dihapus, Pembayaran belum Lunas!";
			}		 
	...
	function setForm($dt){	
	 ...
	 $form_name = $this->Prefix.'_form';				
	  ...
	  if ($this->form_fmST==0) {
	   ...
	   $kode_skpd = $Main->KD_BARANG_P108?$dt['f1'].'.'.$dt['f2'].'.'.$dt['f'].'.'.$dt['g'].'.'.$dt['h'].'.'.$dt['i'].'.'.$dt['j']:$dt['f'].'.'.$dt['g'].'.'.$dt['h'].'.'.$dt['i'].'.'.$dt['j'] ;	   
       //items ----------------------
	...
	//Get data
		...
		if(!($fmURUSAN=='' || $fmURUSAN=='0') && $Main->URUSAN==1 ) $arrKondisi[] = "(select c1 from gantirugi where id= v_gantirugi.id)  like '%$fmURUSAN%'";
		...
		//if(!empty($fmtahun)) 
		$arrKondisi[]= !empty($fmtahun)?" (select year(tgl_gantirugi) from gantirugi where id= v_gantirugi.id) like '%$fmtahun%'" : " (select year(tgl_gantirugi) from gantirugi where id= v_gantirugi.id) like '%$thn_login%'";
	...
	function genToolbarBawah(){
		...
		$PnlPenghapusan = $Main->MODUL_PENGAPUSAN ?"<table width=\"70\"><tr><td>" . 
						//PanelIcon3("javascript:penghapusan_ins.penghapusanbaru(4)", "PENGHAPUSAN", $ridModul09, $disModul09,'',2,90) . 
						PanelIcon3("javascript:".$this->Prefix.".HapusSK()", "PENGHAPUSAN", $ridModul09, $disModul09,'',2,90) . 
					"</td></tr></table>":"";
