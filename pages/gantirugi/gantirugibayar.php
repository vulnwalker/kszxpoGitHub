...
class gantirugibayarObj  extends DaftarObj2{	
	...
	function simpan(){
	 ...
	 //get data -----------------
		...
		$c1=$expkdskpd['0']; $c=$expkdskpd['1']; $d=$expkdskpd['2']; $e=$expkdskpd['3']; $e1=$expkdskpd['4'];
		...
		if($err=='' && $fmST == 0){
			...
			if ($err==''){
			$aqry="insert into $tabel (ref_idgantirugi,id_bukuinduk,a1,a,b,c1,c,d,e,e1,tgl,dari_nama,harga,tgl_pembukuan,ket,uid,tgl_update) values
						('$ref_idgantirugi','$id_bukuinduk','".$Main->DEF_KEPEMILIKAN."','".$Main->DEF_PROPINSI."','".$Main->DEF_WILAYAH."','$c1','$c','$d','$e','$e1','$tgl','$dari_nama','$bayar','$tgl_pembukuan','$ket','$uid','$tgl')";$cek.=$aqry;
	...
	function simpanPilih(){
		...
		if($err==''){
			...
			$tgl_sk_tgl=$tgl_sk[2];
			$sisa=$tgr1['sisa'];			
			$content->fmIDBARANG = $Main->KD_BARANG_P108?$tgr['f1'].'.'.$tgr['f2'].'.'.$tgr['f'].'.'.$tgr['g'].'.'.$tgr['h'].'.'.$tgr['i'].'.'.$tgr['j']:$tgr['f'].'.'.$tgr['g'].'.'.$tgr['h'].'.'.$tgr['i'].'.'.$tgr['j'] ;// '1';
			...
			$content->kd_skpd = $Main->URUSAN?$tgr['c1'].'.'.$tgr['c'].'.'.$tgr['d'].'.'.$tgr['e'].'.'.$tgr['e1']:$tgr['c'].'.'.$tgr['d'].'.'.$tgr['e'].'.'.$tgr['e1'];			
		}
	 ...
	 $form_name = $this->Prefix.'_form';				
	  ...
	  if ($this->form_fmST==0) {
		...
		$dtpembayaran=mysql_fetch_array(mysql_query("select * from gantirugi_bayar where id='".$dt['id']."'"));
		  $kode_skpd = $Main->KD_BARANG_P108?$dt1['f1'].'.'.$dt1['f2'].'.'.$dt1['f'].'.'.$dt1['g'].'.'.$dt1['h'].'.'.$dt1['i'].'.'.$dt1['j']:$dt1['f'].'.'.$dt1['g'].'.'.$dt1['h'].'.'.$dt1['i'].'.'.$dt1['j'] ;
		  $qrbrg = "select * from ref_barang where concat($concatkd)='$kdBarang'";
	...
	if($Main->VERSI_NAME=='JABAR'){
	  ...
	  $kode_barang = $Main->KD_BARANG_P108?$dt1['f1'].'.'.$dt1['f2'].'.'.$dt1['f'].'.'.$dt1['g'].'.'.$dt1['h'].'.'.$dt1['i'].'.'.$dt1['j']:$dt1['f'].'.'.$dt1['g'].'.'.$dt1['h'].'.'.$dt1['i'].'.'.$dt1['j'] ;
	 $Koloms = array();
	...
	//Get data
		...
		if(!($fmURUSAN=='' || $fmURUSAN=='0') && ($Main->URUSAN==1)) $arrKondisi[] = "(select c1 from gantirugi_bayar where id= v_gantirugi_bayar.id)  like '%$fmURUSAN%'";
		...
		if(!empty($fmkode_barang)) $arrKondisi[]= " (select concat($concatkd) from gantirugi where id= v_gantirugi_bayar.ref_idgantirugi)  like '$fmkode_barang%'";
		if(!empty($fmid_barang)) $arrKondisi[]= " (select id_bukuinduk from gantirugi where id= v_gantirugi_bayar.ref_idgantirugi)  like '$fmid_barang%'";
		//if(!empty($fmtahun)) 
		$arrKondisi[]= !empty($fmtahun)?" (select year(tgl_gantirugi) from gantirugi where id= v_gantirugi_bayar.ref_idgantirugi) like '%$fmtahun%'" : " (select year(tgl_gantirugi) from gantirugi where id= v_gantirugi_bayar.ref_idgantirugi) like '%$thn_login%'";
