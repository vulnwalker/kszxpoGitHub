...
class gantirugi_cariObj  extends DaftarObj2{	
	 ...
	 //get data -----------------
		...
		if ($err==''){
		 if($fmST==0){
			...
			$aqry="insert into $tabel (id_bukuinduk,c,d,e,e1,f1,f2,f,g,h,i,j,no_usul,tgl_usul,tahun,noreg,kepada_nama,
					kepada_alamat,uraian,ket,tgl_gantirugi,stat,uid,tgl_update) values
					('$id_bukuinduk','$c','$d','$e','$e1','$f1','$f2','$f','$g','$h','$i','$j','$no_usul','$tgl_usul','$tahun','$noreg','$kepada_nama',
	...
	function setForm($dt){	
	 ...
	 $form_name = $this->Prefix.'_form';				
	  ...
	  if ($this->form_fmST==0) {
	   ...
	   $kode_skpd = $Main->KD_BARANG_P108?$dtTGR['f1'].'.'.$dtTGR['f2'].'.'.$dtTGR['f'].'.'.$dtTGR['g'].'.'.$dtTGR['h'].'.'.$dtTGR['i'].'.'.$dtTGR['j']:$dtTGR['f'].'.'.$dtTGR['g'].'.'.$dtTGR['h'].'.'.$dtTGR['i'].'.'.$dtTGR['j'] ;	   
       //items ----------------------
	...
	//Get data
		...
		if(!($fmURUSAN=='' || $fmURUSAN=='00') && ($Main->URUSAN==1)) $arrKondisi[] = "(select c1 from gantirugi where id= v_gantirugi.id)  like '%$fmURUSAN%'";
		...
		if(!empty($fmkode_barang)) $arrKondisi[]= " (select concat($concatkd) from gantirugi where id= v_gantirugi.id)  like '$fmkode_barang%'";
