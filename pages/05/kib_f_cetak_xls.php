...
$Qry = mysql_query("select * from view_kib_f where $Kondisi order by $Urutkan a1,a,b,c,d,e,f,g,h,i,j,tahun,noreg  $LimitHalKIB_F");
...
if ($jmlDataKIB_F>$maxrow) {
$tmpfname = tempnam(sys_get_temp_dir(), 'Tux');	
$handle = fopen($tmpfname, "w+");
fwrite($handle, $ListBarangKIB_F);
...
while ($isi = mysql_fetch_array($Qry))
{
	...
	$ListBarangKIB_F .= list_table($no,$isi['id_barang'],$isi['noreg'],$isi['nm_barang'],
	ifempty($Main->Bangunan[$isi['bangunan']-1][1],'-'),
	ifempty($Main->Tingkat[$isi['konstruksi_tingkat']-1][1],'-'),
	ifempty($Main->Beton[$isi['konstruksi_beton']-1][1],'-'),
	$isi['luas'],ifempty($isi['alamat'],'-').$Kel.$Kec.$kota,
...
$tampilHarga = !empty($cbxDlmRibu) ? $JmlTotalHargaListKIB_F/1000: $JmlTotalHargaListKIB_F;
...
$ListBarangKIB_F  .= list_tablefooter($tampilHarga,$cbxDlmRibu);
...
$ListBarangKIB_F .= list_footer($TITIMANGSA,$JABATANSKPD,$NAMASKPD,$NIPSKPD,$JABATANSKPD1,$NAMASKPD1,$NIPSKPD1);

$Main->Isi = $ListBarangKIB_F;
