...
switch ($idpilih){
	...
	default:{ //kerangka hal		
		...
		// tampil bidang ----------------------------------
		$fmKEPEMILIKAN = $Main->DEF_KEPEMILIKAN;		
		$fmURUSAN = cekPOST('fmURUSAN'); //echo  '<br> fmURUSAN1  = '.$fmURUSAN;//? 
		$fmSKPD = cekPOST('fmSKPD'); //echo  '<br> fmSKPD  = '.$fmSKPD;//? 
		$fmUNIT = cekPOST('fmUNIT'); //echo  '<br> fmUNIT  = '.$fmUNIT;//?
		$fmSUBUNIT = cekPOST('fmSUBUNIT');
		$fmSEKSI = cekPOST('fmSEKSI');
		...
		if($skpdro==1){
			...
			$filterSKPD =  
				...
				WilSKPD_ajx3('', '100%', 100, TRUE) ;

		}else{
			setWilSKPD(); //echo "<br> fmURUSAN3 ".$fmURUSAN;	
			$filterSKPD =  WilSKPD1();
		}
	...
	//	switch($tipebi){
		...
		$fmSEKSI = cekPOST('fmSEKSI');
		*/
			$loading_js = ($fmSKPD !='' && $fmSKPD !='00') && ($fmUNIT !='' && $fmUNIT !='00')&& 
				($fmSUBUNIT !='' && $fmSUBUNIT!='00') && ($fmSEKSI !='' && $fmSEKSI!='00')?
				'Penatausaha.loading();': '';
		...
		<script src='js/reclass.js' type='text/javascript'></script>						
					...
					<form action=\"\" method=\"post\" name=\"adminForm\" id=\"adminForm\">
						<input type='hidden' id='Penghapusan_Baru' name='Penghapusan_Baru' value='$Penghapusan_Baru'>" . 
							$divBlock . "
						<table width='100%' style='min-width:1580px'><tr><td>" .
		        			"<div id = 'cntTitle' >".
								$Penatausaha->genTitle($_GET['SPg'], $Penatausaha->genToolbarAtas()).
							"</div>".
		       				$opt_wil .
							...
							**/
							...
							" .
							"<div id='$Penatausaha->elContentHal'>						
								Klik Tampilkan ... <img src='images/wait.gif'  style='height:16;position:relative;'> ".
		        				"<input type='hidden' id='HalDefault' name='HalDefault' value='1'>".
							"</div>".
							/**
							"<div id='$Penatausaha->elContentHal'>						
