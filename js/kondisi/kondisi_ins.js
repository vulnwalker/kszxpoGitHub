var kondisi_ins = new DaftarObj2({
	prefix : 'kondisi_ins',
	url : 'pages.php?Pg=kondisi_ins', 
	formName : 'kondisi_insForm',
	...
	kondisibaru: function(){	
		var me = this; var errmsg ='';	
		$.ajax({
				type:'POST', 
				data:$('#adminForm').serialize(),
				url: this.url+'&tipe=cek_kondisi',
	...
	FormKondisi: function(){
		var me = this; var errmsg ='';
		/*if((errmsg=='') && ( (document.getElementById('fmKONDBRG').value == 0)||(document.getElementById('fmKONDBRG').value == '')  )){
			errmsg= 'Filter kondisi barang belum dipilih!';
		}*/
		...
		if(errmsg ==''){ 
			...
			if(confirm('Reclass RB '+jmlcek+' Data ?')){
				if(errmsg ==''){ 							
				...
				success: function(data) {	
					...
					if(resp.err==''){
						...
						if(persen<100) {
							...
							document.getElementById('progressbar').style.width = persen +'%';				
							setTimeout(function(){ alert('Reclass RB Selesai !'); }, 1000);
