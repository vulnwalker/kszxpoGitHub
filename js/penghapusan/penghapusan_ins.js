var penghapusan_ins = new DaftarObj2({
	...
	var me = this; var errmsg ='';
		...
		}else if(baru==3){//dari Pemusnahan
			var jmlcek = document.getElementById('pemusnahan_jmlcek').value ;	
		}else{//dari gantirugi
			var jmlcek = document.getElementById('gantirugiprogres_jmlcek').value ;	
		}
		if(baru!=4){
			var nmform = 'adminForm';
		}else{
			var nmform = 'gantirugiprogresForm';
		}
		...
		if(errmsg ==''){ 
			if(confirm('Penghapusan '+jmlcek+' Data ?')){
				...
				var aForm = document.getElementById(nmform);		
				aForm.action= 'pages.php?Pg=penghapusan_ins&baru='+baru;//'?Op='+op+'&Pg=2&idprs=cetak_hal';		
				aForm.target='_blank';
				aForm.submit();	
				aForm.target='';
			}
