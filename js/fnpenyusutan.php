...
class PenyusutanObj extends DaftarObj2{
	...
	function batalSusutMetode(){ //batal penyusutan per barang
		...
		$arrKondisiMetode[]=" id in (select idbi from penyusutan where tahun>='$thnbatal' ) ";		
		...
		//foreach ($dt as $bi){		
//$cek .= 'test';
			$aqrySusut="select * from penyusutan where idbi='{$bi['id']}' and tahun>='$thnbatal' order by Id DESC"; $cek .= $aqrySusut;
		...
		if($jml_error>0) {
			$content->msg_jml_error = "Terdapat ".$jml_error." ID Barang yang gagal disusutkan!";
		}else{
	...
	*/
	...
	function susut_koreksi_semester($idbi, $tglsusut, $susut_koreksi = FALSE){
		...
		" and tgl_pemeliharaan <= '$tglsusut' and tgl_perolehan <='$maxTglSusutOld' and tgl_pemeliharaan >'$maxTglSusutOld' ".	// tgl buku transaksi tidak lebih dari tgl susut, kalau tgl perolehan > tgl susut sebelumnya --> maka penyusutan tidak perlu di hitung ulang
		" and ( 
		...
		" and tgl_pengamanan <= '$tglsusut'  and tgl_perolehan <='$maxTglSusutOld' and tgl_pengamanan >'$maxTglSusutOld' ".
		...
		" and tgl_penghapusan <= '$tglsusut'  and tgl_perolehan <='$maxTglSusutOld' and tgl_penghapusan >'$maxTglSusutOld' ".
		...
		" and tgl <= '$tglsusut'  and tgl_perolehan <='$maxTglSusutOld' and tgl >'$maxTglSusutOld' ".
		...
		/**$query_penilaian = "select * from penilaian where idbi_awal = '".$isi['idawal']."' ".
			...
			***/
		$query_penilaian = "select * from penilaian where idbi_awal = '".$isi['idawal']."'". 		
			" and tgl_penilaian <= '$tglsusut'  and tgl_perolehan <='$maxTglSusutOld' and tgl_penilaian >'$maxTglSusutOld' ".
			"and (year(tgl_perolehan) <> year(tgl) 
			or (year(tgl_perolehan) = year(tgl) 
			and if(month(tgl_perolehan)>=1 and month(tgl_perolehan)<=6,1,2) <> if(month(tgl)>=1 and month(tgl)<=6,1,2)))
...
*/	
	...
	function susut1barang_bandung($idbi, $tglsusut, $jns_trans2, $sesi, $susut_koreksi=FALSE){
		...
		if($jns_trans2==31){
			...
			//$aqry = "select  sf_susut_mode7($idbi, '$tglsusut' , '$UID', $jns_trans2,$idk)  as cek;"; $cek .= $aqry;
			$aqry = "select  sf_susut_mode12($idbi, '$tglsusut' , '$UID', $jns_trans2,$idk)  as cek;"; $cek .= $aqry;
			$isi =mysql_fetch_array( mysql_query($aqry));			
		}
...
/*function susut1barang_jabar2($idbi, $thnsusutak, $blnsusutak, $tglSusut){
	...
	function genQuery($mode = 1, $fmURUSAN, $fmSKPD, $fmUNIT, $fmSUBUNIT, $fmSEKSI, $thnsusut, $blnsusut=6, $NoSesi, $limit=50  ){
		...
		/*elseif($Main->SUSUT_MODE==3){
			...
			$arrKondisi[] = " (aa.thn_susut_ak < $thnsusut )"; //belum disusutkan pd tgl susut ini	
			$arrKondisi[] = " year(aa.tgl_buku) <=  $thnsusut "; // periode tgl susut harus >= periode tgl buku	
		}elseif($Main->SUSUT_MODE==8 && $Main->VERSI_NAME=='CIREBON_KAB'){
			$thnblnsusut = $thnsusut*2 +( $blnsusut>6? 2 :1);
			$arrKondisi[] = " ( if(aa.thn_perolehan > $Main->TAHUN_MULAI_SUSUT , aa.thn_perolehan, $Main->TAHUN_MULAI_SUSUT )*2+if(month(aa.tgl_buku)>6,2,1)  )<=$thnblnsusut  ";	//sudah harus disusutkan					
			$arrKondisi[] = " ( (aa.thn_susut_ak*2)+if(ifnull(aa.bln_susut_ak,0) > 6, 2,1)<((aa.thn_susut_aw+aa.masa_manfaat-1)*2)+if(ifnull(aa.bln_susut_aw,0)>6,2,1) or aa.thn_susut_aw=0 )"; //masih dalam masa manfaat
			$arrKondisi[] = " (aa.thn_susut_ak*2)+ if(ifnull(aa.bln_susut_ak,0)>6,2,1) < $thnblnsusut "; //belum disusutkan pd tgl susut ini
		...
		}elseif($Main->SUSUT_MODE==5 && $Main->VERSI_NAME=='SERANG_KOTA'){
			$arrKondisi[] = " ( if(aa.thn_perolehan > $Main->TAHUN_MULAI_SUSUT , aa.thn_perolehan, $Main->TAHUN_MULAI_SUSUT )*12+month(aa.tgl_buku)) <= $thnblnsusut  ";	//sudah harus disusutkan					
			$arrKondisi[] = " ( (aa.thn_susut_ak*12)+ifnull(aa.bln_susut_ak,0) < (((aa.thn_susut_aw+aa.masa_manfaat)*12)-1)+ifnull(aa.bln_susut_aw,0) or aa.thn_susut_aw=0 ) "; //masih dalam masa manfaat	
			$arrKondisi[] = " (aa.thn_susut_ak*12)+ifnull(aa.bln_susut_ak,0) < $thnblnsusut "; //belum selesai disusutkan
			$arrKondisi[] = " year(aa.tgl_buku) <=  $thnsusut "; 				
		}elseif($Main->SUSUT_MODE==12 && $Main->VERSI_NAME=='KOTA_BANDUNG'){
			$arrKondisi[] = " ( if(aa.thn_perolehan > $Main->TAHUN_MULAI_SUSUT , aa.thn_perolehan, $Main->TAHUN_MULAI_SUSUT )<=$thnsusut  )";	//sudah harus disusutkan					
			$arrKondisi[] = " ( aa.thn_susut_ak < aa.thn_susut_aw + aa.masa_manfaat-1  or aa.thn_susut_aw=0 )"; //masih dalam masa manfaat
			$arrKondisi[] = " (aa.thn_susut_ak < $thnsusut )"; //belum disusutkan pd tgl susut ini	
			$arrKondisi[] = " year(aa.tgl_buku) <=  $thnsusut "; // periode tgl susut harus >= periode tgl buku	
	...
	function susutSemester($thnsusut=2014,$blnsusut=12,$tglsusut='2014-12-31',$semsusut=2,$jns_trans2=30,$NoSesi=''){
		global $Main, $HTTP_COOKIE_VARS;
		$cek = ''; $err=''; $content=''; $json=TRUE;
		...
		$get = $this->genQuery(1, $fmURUSAN, $fmSKPD, $fmUNIT, $fmSUBUNIT, $fmSEKSI, $thnsusut, $blnsusut, $NoSesi);
		...
		while($isi=mysql_fetch_array($qry)){
			...
			if($CekBI==''){
				...
				}else if($Main->SUSUT_MODE==11){//Karawang -> dihitung tahun, disimpan tahun, tgl susut otomatis
					$susut = $this->susut1barang_karawang($isi['id'], $tglsusut, $jns_trans2,$NoSesi);
				}else if($Main->SUSUT_MODE==12){//Bandung -> dihitung tahun, disimpan tahun, tgl susut otomatis
	...
	function susutSatu($idbi=0,$thnsusut=2014,$blnsusut=12,$tglsusut='2014-12-31',$semsusut=2,$jns_trans2=30,$NoSesi=''){
		...
		else if($Main->SUSUT_MODE==11){//Karawang -> dihitung bulan, disimpan semesteran, tgl susut otomatis
			$susut = $this->susut1barang_karawang($idbi, $tglsusut, $jns_trans2,$NoSesi);
		}
		else if($Main->SUSUT_MODE==12){//KOta Bandung -> dihitung tahun, disimpan tahuan, tgl susut otomatis
	...
	function genFormSusut(){
		...
		if($Main->SUSUT_MODE==2 || $Main->SUSUT_MODE==6 || $Main->SUSUT_MODE==7 || $Main->SUSUT_MODE==9 || $Main->SUSUT_MODE==10 || $Main->SUSUT_MODE==12){
	...
	function genFormSusutSatu(){
		...
		//if($tglclosing=='' || $tglclosing==0 || $tglclosing==NULL || $Main->SUSUT_MODE==2 || $Main->SUSUT_MODE==3 || $Main->SUSUT_MODE==7 || $Main->SUSUT_MODE==10 || $Main->SUSUT_MODE==11 || $Main->SUSUT_MODE==8 || $Main->SUSUT_MODE==9 || $Main->SUSUT_MODE==5 || $Main->SUSUT_MODE==12){ //sementara
		//	$thnsusut = $thnskr-1;				
	...
	function genFormBatal(){
		...
		if($fmSUBUNIT !='' && $fmSUBUNIT != NULL && $fmSUBUNIT !='00') $arrKondisi[] = " e='$fmSUBUNIT' ";		
		if($fmSEKSI !='' && $fmSEKSI != NULL && $fmSEKSI !=$kdseksikos) $arrKondisi[] = " e1='$fmSEKSI' ";
		$arrKondisi[] = " id in (select idbi from penyusutan where tahun>='".($thnskr-1)."') ";
		$kondisi = join(' and ',$arrKondisi); //$cek .=$Kondisi;
		if($kondisi !='') $kondisi = ' Where '.$kondisi;					
		...
		$bi = mysql_fetch_array(mysql_query($aqry));			
		//$tglclosing = getTglClosing($bi['c'],$bi['d'],$bi['e'],$bi['e1']); $cek .= $tglclosing;
		//if($tglclosing=='' || $tglclosing==0 || $tglclosing==NULL || $Main->SUSUT_MODE==3 || $Main->SUSUT_MODE==9){
			$thnbatal = $thnskr-1;				
		...
		//}else{
			...
			$vthnbatal = "<input type='text' id='thnbatal' name='thnbatal' value='$thnbatal' readonly>";
		//}
		//$Semester = cmb2D_v2('fmSemester',$fmSemester,$Main->ArSemester,'','Semester I',"  " );		
		
		if($Main->SUSUT_MODE==2 || ($Main->SUSUT_MODE==3 && $bi['thn_perolehan'] <=2014) ||$Main->SUSUT_MODE==6 || $Main->SUSUT_MODE==7 || $Main->SUSUT_MODE==9 || ($Main->SUSUT_MODE==8 && $bi['thn_perolehan'] <=2014)){
		...
		$progress = 
			...
			"<div id ='progreserrormsg' name='progreserrormsg' style='float:left;width:100%;'></div></br>".
			"<div id='' style='float:right; padding: 2 0 0 0'>".
			"<img id='daftaropsierror_slide_img' src='' onclick='".$this->Prefix.".daftaropsierrorbatal_click(270)' style='cursor:pointer'>".
			"</div>".
			"<div id='daftaropsisusuterror_div' style='height: 0px; overflow-y: hidden;float:left;'>".
			"<div id ='progreserror' name='progreserror'></div>".
			"</div>".			
		...
		$this->form_fields = array(				
			'urusan' => array(  'label'=>'URUSAN', 'value'=> $urusan, 'labelWidth'=>120, 'type'=>'' ),
			'bidang' => array(  'label'=>'BIDANG', 'value'=> $bidang, 'labelWidth'=>120, 'type'=>'' ),
			'unit' => array(  'label'=>'SKPD', 'value'=> $unit, 'labelWidth'=>120, 'type'=>'' ),
			'subunit' => array(  'label'=>'UNIT', 'value'=> $subunit, 'labelWidth'=>120, 'type'=>'' ),
			'seksi' => array(  'label'=>'SUBUNIT', 'value'=> $seksi, 'labelWidth'=>120, 'type'=>'' ),
	...
	function changeTahun(){
		...
		if ($thnsusut>0){
			...
			$aqry = $this->genQuery(2,$fmURUSAN, $fmSKPD, $fmUNIT, $fmSUBUNIT, $fmSEKSI,$thnsusut, $blnsusut);	 $cek .= $aqry['content'];	
			$get = mysql_fetch_array(mysql_query( $aqry['content'] ));
	...
	function set_selector_other($tipe){
		...
		switch($tipe){			
			...
			/*case 'batalNext':{
				...
				$fmSUBUNIT = $_REQUEST['e'];	
				$fmSEKSI = $_REQUEST['e1'];
				$kd_barang = explode(".",$_REQUEST['kd_barang']);
				#get pilih kode barang
				$f = $kd_barang[0];
				$g = $kd_barang[1];
				$h = $kd_barang[2];
				$i = $kd_barang[3];
				...
				//urusan		
				if($fmURUSAN != '' && $fmURUSAN != NULL && $fmURUSAN !='00') {
					$get=mysql_fetch_array(mysql_query("select * from ref_skpd where c1='".$fmURUSAN."' and c='00' and d='00' "));
					$urusan = $get['nm_skpd'];	
				}
				//bidang		
				if($fmSKPD != '' && $fmSKPD != NULL && $fmSKPD !='00') {
				...
				if($batalMetode=="batalIdbi"){
					//id barang
					$content="Batalkan penyusutan untuk id $idbi.";	
					$kondisiMetode=" aa.idbi='$idbi' ";				
				}elseif($batalMetode=="batalTahun"){
					//tahun periode
					$content="Tahun : ".cmbArray('thnbatal', $thnskr, $arrthn, 'PILIH'," onchange='".$this->Prefix.".changeTahun()' "  );
					$kondisiMetode=" aa.tahun>='$thnbatal' ";							
				}elseif($batalMetode=="batalSKPD"){
					//skpd
					$content="<table>
							<tr>
					...
					$kondisiMetode=" bb.c1='".$fmURUSAN."' and bb.c='".$fmSKPD."' and bb.d='".$fmUNIT."' and bb.e='".$fmSUBUNIT."' and bb.e1='$fmSEKSI' ";													
				}else{
					//kode barang
					$content="Kode barang : <input type='text' id='kd_barang' name='kd_barang'>";
					$kondisiMetode=" bb.f='".$f."' and bb.g='".$g."' and bb.h='".$h."' and bb.i='".$i."' and bb.j='$j' ";	
					
				}				
				
				//get jml data
				$test="select count(*) as jmlData from penyusutan aa inner join buku_induk bb on aa.idbi=bb.id where $kondisiMetode group by aa.idbi ";
				$get=mysql_fetch_array(mysql_query("select count(*) as jmlData from penyusutan aa inner join buku_induk bb on aa.idbi=bb.id where $kondisiMetode group by aa.idbi "));
					$jmlData = $get['jmlData'];	
				$err = $test;//$get['err'];
				$cek = $get['cek'];
				$content->jmlData = $jmlData;
			...
			case 'susut':{
				...
				$get = $this->susutSemester($thnsusut,$blnsusut,$tglsusut,$fmSemester,30,$NoSesi);
				//$get = $this->susut();
				$err = $get['err'];
				$cek = $get['cek'];
				$content = $get['content'];				
	...
	*/
	...
	function setKolomHeader($Mode=1, $Checkbox=''){
		...
		if(($Main->SUSUT_MODE==3 && $get['thn_perolehan'] > 2014) || $Main->SUSUT_MODE==11 || $Main->SUSUT_MODE==5){
			$vbln = "(Bulan)";
			$vtittle = "";				
	...
	function setKolomData($no, $isi, $Mode, $TampilCheckBox){
		...
		if(($Main->SUSUT_MODE==3 && $get['thn_perolehan'] > 2014) || $Main->SUSUT_MODE==4 || $Main->SUSUT_MODE==8 || $Main->SUSUT_MODE==11 || $Main->SUSUT_MODE==5 || $Main->SUSUT_MODE==9){
		...
		if($Main->SUSUT_MODE==2 || $Main->SUSUT_MODE==3 || $Main->SUSUT_MODE==4 || $Main->SUSUT_MODE==5 || $Main->SUSUT_MODE==6 || $Main->SUSUT_MODE==7 || $Main->SUSUT_MODE==8 || $Main->SUSUT_MODE==9 || $Main->SUSUT_MODE==10 || $Main->SUSUT_MODE==11 || $Main->SUSUT_MODE==5 || $Main->SUSUT_MODE==5){
		...
		if($Main->SUSUT_MODE==2 || $Main->SUSUT_MODE==3 || $Main->SUSUT_MODE==4 || $Main->SUSUT_MODE==5 || $Main->SUSUT_MODE==6 || $Main->SUSUT_MODE==7 || $Main->SUSUT_MODE==8 || $Main->SUSUT_MODE==9 || $Main->SUSUT_MODE==10 || $Main->SUSUT_MODE==11 || $Main->SUSUT_MODE==5){
	...
	function setDaftar_after_getrow($list_row, $isi, $no='', $Mode='', $TampilCheckBox='', $RowAtr='', $KolomClassStyle=''){
		...
		}else if($Main->SUSUT_MODE==12){//  && $Main->VERSI_NAME=='BANDUNG'){//BANDUNG
