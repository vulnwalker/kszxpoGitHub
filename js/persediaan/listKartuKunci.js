var listKartuKunci = new DaftarObj2({
	prefix : 'listKartuKunci',
	url : 'pages.php?Pg=listKartuKunci',
	formName : 'listKartuKunciForm',


	loading: function(idLock){
		//alert('loading');
		this.topBarRender();
		this.filterRender(idLock);
		this.daftarRender(idLock);
		this.sumHalRender();

	},
	filterRender:  function(idLock){
		var me=this;
		//render filter
		$.ajax({
		  url: this.url+'&tipe=filter',
		  type:'POST',
		  data:$('#'+this.formName).serialize()+"&idLock="+idLock,
 ...
 sum : function(id) {
	...
	windowSave: function(){
		var me= this;
			...
			$.ajax({
			type : 'POST',
				url: 'pages.php?Pg=listKartuKunci&tipe=setValueTemplate',
				data:{id : document.getElementById('idTemplate').value,
					  c  : document.getElementById('c').value,
					  d  : document.getElementById('d').value,
					  e  : document.getElementById('cmbUnit').value,
					  currentPosition : document.getElementById("listKartuKunci_hal_fmHAL").value
					 },
			  	success: function(data) {
					var resp = eval('(' + data + ')');
					var limit = resp.cek;
