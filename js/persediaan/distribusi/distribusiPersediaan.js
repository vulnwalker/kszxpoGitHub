var distribusiPersediaanSKPD2 = new SkpdCls({
	prefix : 'distribusiPersediaanSKPD2',
	formName: 'distribusiPersediaanForm',

	pilihUrusanfter : function(){distribusiPersediaan.refreshList(true);},
	pilihBidangAfter : function(){distribusiPersediaan.refreshList(true);},
	pilihUnitAfter : function(){distribusiPersediaan.refreshList(true);},
	pilihSubUnitAfter : function(){distribusiPersediaan.refreshList(true);},
	pilihSeksiAfter : function(){distribusiPersediaan.refreshList(true);}
});
var distribusiPersediaan = new DaftarObj2({
	prefix : 'distribusiPersediaan',
	url : 'pages.php?Pg=distribusiPersediaan',
	formName : 'distribusiPersediaanForm',
	distribusiPersediaan_form : '0',//default js distribusiPersediaan
	loading: function(){
		//alert('loading');
		this.topBarRender();
		this.filterRender();
  	...
  	Report: function(){
		...
		$.ajax({
			...
			data: { jenisKegiatan : $("#jenisKegiatan").val(),
					...
					ttd			  : $("#ttd").val(),
					c1			  : $("#distribusiPersediaanSKPD2fmURUSAN").val(),
					c 			  : $("#distribusiPersediaanSKPD2fmSKPD").val(),
					d 			  : $("#distribusiPersediaanSKPD2fmUNIT").val(),
					e  			  : $("#distribusiPersediaanSKPD2fmSUBUNIT").val(),
					e1 			  : $("#distribusiPersediaanSKPD2fmSEKSI").val(),
			...
			url: this.url+'&tipe=Report',
		  	success: function(data) {
				...
				if(resp.err==''){
					...
					aForm.action= url2+'&tipe='+resp.content.to+'&urusan='+$("#distribusiPersediaanSKPD2fmURUSAN").val()+'&bidang='+$("#distribusiPersediaanSKPD2fmSKPD").val()+'&skpd='+$("#distribusiPersediaanSKPD2fmUNIT").val()+'&unit='+$("#distribusiPersediaanSKPD2fmSUBUNIT").val()+'&subunit='+$("#distribusiPersediaanSKPD2fmSEKSI").val()+'&kota='+resp.content.namaPemda+'&tanggalCetak='+resp.content.cetakjang+'&ttd='+resp.content.ttd;
					aForm.target='_blank';
					aForm.submit();
					aForm.target='';
					me.Close();
	...
	programChanged : function(){
		...
		$("#hiddenP").val(hiddenP);
		$("#q").val('');
		distribusiPersediaan.refreshList(true);
	},
	detail: function(){
		//alert('detail');
		var me = this;
...
/*				me.sumHalRender();*/
	...
	BaruTTD: function(){
		var me = this;
		var err='';
		var urusan = document.getElementById('distribusiPersediaanSKPD2fmURUSAN').value; 
		var bidang = document.getElementById('distribusiPersediaanSKPD2fmSKPD').value; 
		var skpd = document.getElementById('distribusiPersediaanSKPD2fmUNIT').value;
		var unit = document.getElementById('distribusiPersediaanSKPD2fmSUBUNIT').value;
		var subunit = document.getElementById('distribusiPersediaanSKPD2fmSEKSI').value;
			...
			alert("Pilih Sub Unit");
				...
				data : {c1 : $("#"+me.prefix+"SKPD2fmURUSAN").val(),
			  	...
			  	success: function(data) {
					var resp = eval('(' + data + ')');
					if(resp.err == ''){
						aForm.action= 'pages.php?Pg=distribusiPersediaan_ins&skpd='+skpd+"&nomor="+resp.content.nomor;
						aForm.target='_blank';
						aForm.submit();
						aForm.target='';
					}else{
	...
	unlockFindRekening: function(){
		...
		if( $("#cmbJenisRKAForm").val() == ""){
			...
			$("#findRekening").attr('onclick',"distribusiPersediaan.findRekening('"+$("#cmbJenisRKAForm").val()+"');");
			...
			if(errmsg ==''){
				...
				$.ajax({
				  ...
				  success: function(data) {
						var resp = eval('(' + data + ')');
						if(resp.err == "") {
							aForm.action= 'pages.php?Pg=distribusiPersediaan_ins&skpd='+resp.content.skpd+"&nomor="+resp.content.nomor;
							aForm.target='_blank';
							aForm.submit();
							aForm.target='';
						}else{
	...
	bantuVolumeBarang : function(id){
		document.getElementById('bantuVolumeBarang'+id).innerHTML = distribusiPersediaan.formatCurrency2($("#volumeBarang"+id).val());
		var hasilKali = Number($("#volumeBarang"+id).val()) * Number($("#satuanHarga"+id).val()) ;
		$("#spanJumlahHarga"+id).text(distribusiPersediaan.formatCurrency(hasilKali));
	},
	bantuSatuanHarga : function(id){
		document.getElementById('bantuSatuanHarga'+id).innerHTML =  distribusiPersediaan.formatCurrency($("#satuanHarga"+id).val());
		var hasilKali = Number($("#volumeBarang"+id).val()) * Number($("#satuanHarga"+id).val()) ;
		$("#spanJumlahHarga"+id).text(distribusiPersediaan.formatCurrency(hasilKali));
	},
	...
	bantu : function(){
		$("#bantu").text("Rp. "+distribusiPersediaan.formatCurrency($("#hargaSatuan").val())  );
			...
			$.ajax({
				...
				data:$('#'+this.formName).serialize(),
			  	...
			  	success: function(data) {
					...
					//setTimeout(function myFunction() {distribusiPersediaan.jam()},100);
					//me.AfterFormBaru();
			  	}
			});
		}else{
		...
		$.ajax({
			...
			url: this.url+'&tipe=simpan',
		  	success: function(data) {
				...
				//document.getElementById(cover).innerHTML = resp.content;
				if(resp.err==''){
					if(me.distribusiPersediaan_form==0){
						me.Close();
						me.AfterSimpan();
					}else{
						me.Close();
						barang.refreshCombodistribusiPersediaan();
					}
	...
	jenisAlokasiChanged : function(){
		...
		if($("#jenisAlokasi").val() == "BULANAN"){
			...
			$.ajax({
				...
				success: function(data) {
					distribusiPersediaan.formAlokasi();
				}
			});
			...
			$.ajax({
				...
				url: this.url+'&tipe=clearAlokasi',
				success: function(data) {
					distribusiPersediaan.formAlokasiTriwulan();
				}
			});
