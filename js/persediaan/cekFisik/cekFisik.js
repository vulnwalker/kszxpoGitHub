var cekFisikSKPD2 = new SkpdCls({
	prefix : 'cekFisikSKPD2',
	formName: 'cekFisikForm',

	pilihUrusanfter : function(){cekFisik.refreshList(true);},
	pilihBidangAfter : function(){cekFisik.refreshList(true);},
	pilihUnitAfter : function(){cekFisik.refreshList(true);},
	pilihSubUnitAfter : function(){cekFisik.refreshList(true);},
	pilihSeksiAfter : function(){cekFisik.refreshList(true);}
});
var cekFisik = new DaftarObj2({
	prefix : 'cekFisik',
	url : 'pages.php?Pg=cekFisik',
	formName : 'cekFisikForm',
...
/*				me.sumHalRender();*/
	...
	// 				var resp = eval('(' + data + ')');
	// 				if(resp.err == ''){
	// 					aForm.action= 'pages.php?Pg=cekFisik_ins&skpd='+skpd+"&nomor="+resp.content.nomor;
	// 					aForm.target='_blank';
	// 					aForm.submit();
	// 					aForm.target='';
	// 				}else{
			...
			if(errmsg ==''){
				...
				$.ajax({
				  ...
				  success: function(data) {
						var resp = eval('(' + data + ')');
						if(resp.err == "") {
							aForm.action= 'pages.php?Pg=cekFisik_ins&skpd='+resp.content.skpd+"&nomor="+resp.content.nomor;
							aForm.target='_blank';
							aForm.submit();
							aForm.target='';
						}else{
	...
	bantu : function(from,to,idPersediaan){
		$("#"+to).text(cekFisik.formatCurrency2($("#"+from).val())  );
		var total;
		...
		$("#spanJumlah"+idPersediaan).text(cekFisik.formatCurrency2(total.toString() ) ) ;
	...
	saveCekFisik: function(idFisik){
		...
		$.ajax({
			...
			data : {
								...
								jenisPersediaan : $("#jenisCekFisik").val(),
								//tanggalCek : $("#tanggalCek").val(),
								hargaSatuan : $("#hargaSatuan").val(),
								periode : $("#filterPeriode").val(),
						 },
			url: this.url+'&tipe=saveCekFisik',
		  	success: function(data) {
					delElem(cover);
				var resp = eval('(' + data + ')');
				if(resp.err==''){
   ...
   SaveCatatan: function(id){
	...
	cekFisikChanged : function(){
		if($("#jenisCekFisik").val() == "4" ){
