var popupProgramREF = new DaftarObj2({
	prefix : 'popupProgramREF',
	url : 'pages.php?Pg=popupProgramREF',
	formName : 'popupProgramREFForm',
	kodeProgram : 'p',
	namaProgram : 'program',
	filterAkun : '',
	filterSKPD : '',
	...
	windowSave: function(id){
		...
		//alert('save');
			...
			var cover = 'popupProgramREF_getdata';
			addCoverPage2(cover,1,true,false);
			$.ajax({
				url: 'pages.php?Pg=popupProgramREF&tipe=getdata&id='+id+"&kodeSKPD="+$("#filterSKPD").val(),
			  	success: function(data) {
					var resp = eval('(' + data + ')');
					delElem(cover);
					if(resp.err==''){
	...
	formatCurrency:function(num) {
		...
		var sisanya = $("#plafon").val() - $("#paguIndikatif").val() - $("#sisaPlafonDariDB").val();
			$("#sisaPlafon").val("Rp. " + popupProgramREF.uang(sisanya));
		}else{
