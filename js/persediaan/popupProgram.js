var popupProgramPersediaan = new DaftarObj2({
	prefix : 'popupProgramPersediaan',
	url : 'pages.php?Pg=popupProgramPersediaan',
	formName : 'popupProgramPersediaanForm',
	kodeProgram : 'p',
	namaProgram : 'program',
	filterAkun : '',
	filterSKPD : '',
	...
	windowShow: function(){
		...
		$.ajax({
			type:'POST',
			data:$('#'+this.formName).serialize(),
			url: this.url+'&tipe=windowshow&filterSKPD='+this.filterSKPD,
		  	success: function(data) {
				var resp = eval('(' + data + ')');
				if(resp.err==''){
					document.getElementById(cover).innerHTML = resp.content;
					me.loading();
					// $("#filterSKPD").val(this.filterSKPD);
				}else{
					alert(resp.err);
					delElem(cover);
					document.body.style.overflow='auto';
	...
	windowSave: function(id){
		...
		//alert('save');
			...
			var cover = 'popupProgramPersediaan_getdata';
			addCoverPage2(cover,1,true,false);
			$.ajax({
				url: 'pages.php?Pg=popupProgramPersediaan&tipe=getdata&id='+id+"&kodeSKPD="+$("#filterSKPD").val(),
			  	success: function(data) {
					var resp = eval('(' + data + ')');
					delElem(cover);
					if(resp.err==''){
	...
	formatCurrency:function(num) {
		...
		var sisanya = $("#plafon").val() - $("#paguIndikatif").val() - $("#sisaPlafonDariDB").val();
			$("#sisaPlafon").val("Rp. " + popupProgramPersediaan.uang(sisanya));
		}else{
