...
//	this.MAP_center_point = '-6.895160 107.633580'; -6.115333 106.152317
	this.MAP_center_point = '-6.517387, 107.718064';//subang '-6.115333 106.152317';//serang
...
//			var skoord =  '-6.895160 107.633580';//removeCR(removeSpaces(document.getElementById('koordinat_gps').value)); -6.115333 106.152317
			...
			var skoord = this.MAP_center_point;
			var me = this;
			//if(skoord!='' ){					
	...
	this.gambar_lokasi= function(rows){
		...
		for(var i=0; i<rows.length; i++){
			if (rows[i].koord != null){
				...
				if(skoord!='')	this.addPoint(rows[i].koord,rows[i].id);
			}else{
		...
		//set center map ke posisi koordinat akhir 
		var akoord = skoord.split(','); 
		this.map.setCenter(new GLatLng(akoord[0], akoord[1]), this.MAP_zoom);
...
//			var skoord =  '-6.476766 106.824293'; // -6.115333 106.152317
			var skoord = me.MAP_center_point;// '-6.115333 106.152317';//serang // -6.115333 106.152317
...
var Map = new MapObj({
	prefix  : 'Map',	
	MAP_center_point : '-6.517387 107.718064',//subang '-6.476766 106.824293',//serang // -0.789275, 113.921327  -3.800738 114.785254 -6.115333,106.152317 -6.115333 106.152317
	MAP_zoom : 8
});
