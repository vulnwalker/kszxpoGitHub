var saldo_awal_keuangan_neracaSKPD = new SkpdCls({
	prefix : 'saldo_awal_keuangan_neracaSKPD', formName:'saldo_awal_keuangan_neracaForm', kolomWidth:120,
	
});

var saldo_awal_keuangan_neraca_ins = new DaftarObj2({
	prefix : 'saldo_awal_keuangan_neraca_ins',
	url : 'pages.php?Pg=saldo_awal_keuangan_neraca_ins', 
	formName : 'saldo_awal_keuangan_neraca_insForm',
	withPilih:true,
	...
	filterRenderAfter : function(){
		...
		if (baru!=''){
			...
			setTimeout(function myFunction() {me.TampilDataRincian()},100);
		}
		if (edit!=''){
		...
		$.ajax({
			...
			success: function(data) {	
				...
				if(resp.err==''){
					...
					setTimeout(function myFunction() {me.nyalakandatepicker2()},1000);
					setTimeout(function myFunction() {me.cek_saldo()},100);
					setTimeout(function myFunction() {me.dat_saldo_awal()},100);
				//	}	
					/*document.getElementById('tbl_jurnal_detail').innerHTML = resp.content;
					setTimeout(function myFunction() {me.bukarekening()},1000);*/
				//	me.bukarekening()
		...
		$.ajax({
			...
			success: function(data) {	
				...
				if(resp.err==''){
					if (edit!=''){
					//	document.getElementById('tbl_jurnal_detail').innerHTML = resp.content;
					setTimeout(function myFunction() {me.dat_saldo_awal()},1000);
					}else{
						document.getElementById('tbl_jurnal_detail').innerHTML = resp.content;
					setTimeout(function myFunction() {me.dat_saldo_awal()},1000);
					}	
	...
	dat_saldo_awal: function(){
		var me = this;
		$.ajax({
			type:'POST', 
			data:$('#'+this.formName).serialize(),
			url: this.url+'&tipe=dat_saldo_awal',
			success: function(data) {	
				var resp = eval('(' + data + ')');			
				if(resp.err==''){
	...
	namajurnal: function(){
		$.ajax({
			...
			success: function(data) {	
				var resp = eval('(' + data + ')');			
				if(resp.err==''){
					document.getElementById('namajurnal_'+resp.content.idrek).innerHTML = resp.content.namarekening;
				}else{
					alert(resp.err);
				}
			}
	...
	//alert('tes');
		$.ajax({
				...
				success: function(data) {	
					...
					//	document.getElementById('SaldoAwal_'+resp.content.idrek).innerHTML = resp.content.saldo_awal;
						document.getElementById('jml_saldo_'+resp.content.idrek).innerHTML = resp.content.jml_saldo;
						document.getElementById('jml_anggaran_'+resp.content.idrek).innerHTML = resp.content.jml_anggaran;
	...
	//	me.bukarekening();
		...
		setTimeout(function myFunction() {saldo_awal_keuangan_neraca_ins.Editinput2(idna)},100);
	...
	input_Jurnal: function(){
		
		$.ajax({
			type:'POST', 
			data:$('#'+this.formName).serialize(),
			url: this.url+'&tipe=input_Jurnal',
			success: function(data) {	
				var resp = eval('(' + data + ')');			
				if(resp.err==''){
					document.getElementById('tbl_jurnal').innerHTML = resp.content.tabel;
		...
		$.ajax({
			...
			success: function(data) {	
				...
				if (edit==''){
					...
					setTimeout(function myFunction() {me.EditJurnal()},100);
				}
