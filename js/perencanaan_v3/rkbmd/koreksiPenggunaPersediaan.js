var koreksiPenggunaPersediaanSkpd = new SkpdCls({
	prefix : 'koreksiPenggunaPersediaanSkpd',
	formName: 'koreksiPenggunaPersediaanForm',
	pilihBidangAfter : function(){koreksiPenggunaPersediaan.refreshList(true);},
	pilihUnitAfter : function(){koreksiPenggunaPersediaan.refreshList(true);},
	pilihSubUnitAfter : function(){koreksiPenggunaPersediaan.refreshList(true);},
	pilihSeksiAfter : function(){koreksiPenggunaPersediaan.refreshList(true);}
});

var koreksiPenggunaPersediaan = new DaftarObj2({
	prefix : 'koreksiPenggunaPersediaan',
	url : 'pages.php?Pg=koreksiPenggunaPersediaan',
	formName : 'koreksiPenggunaPersediaanForm',
	...
	daftarRender:function(){
		var jenisKegiatan = document.getElementById('cmbJeniskoreksiPenggunaPersediaan');
		/*cmbJeniskoreksiPenggunaPersediaan.remove(0);*/
	...
	remove:function(){
		...
		if(errmsg ==''){
			...
			$.ajax({
			  ...
			  success: function(data) {
					var resp = eval('(' + data + ')');
					koreksiPenggunaPersediaan.refreshList(true);
	...
	Laporan:function(){
		var me = this;
			...
			$.ajax({
				type:'POST',
				data:$('#'+this.formName).serialize(),
			  	url: 'pages.php?Pg=koreksiPersediaanNew'+'&tipe=Laporan',
			  	success: function(data) {
					var resp = eval('(' + data + ')');
					if(resp.err == ''){
						document.getElementById(cover).innerHTML = resp.content;
	...
	InputBaru: function(){
		...
		if($("#koreksiPenggunaPersediaanSkpdfmUrusan").val() == '00'){
			errmsg = "Pilih Urusan";
		}else if($("#koreksiPenggunaPersediaanSkpdfmSKPD").val() == '00'){
			errmsg = "Pilih Bidang";
		}else if($("#koreksiPenggunaPersediaanSkpdfmUNIT").val() == '00'){
			errmsg = "Pilih SKPD";
		}else if($("#koreksiPenggunaPersediaanSkpdfmSUBUNIT").val() == '00'){
			errmsg = "Pilih UNIT";
		}else if($("#koreksiPenggunaPersediaanSkpdfmSEKSI").val() == '000'){
			errmsg = "Pilih SUB UNIT";
		}else if($("#cmbJeniskoreksiPenggunaPersediaan").val() == ''){
			errmsg = "Pilih JENIS koreksiPenggunaPersediaan";
		}
	...
	koreksi:function(id){
		...
		$("#spanJumlah"+id).html("<input type='text' name='jumlah"+id+"' id='jumlah"+id+"' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup=koreksiPenggunaPersediaan.bantu('Jumlah"+id+"'); style='text-align: right;'>  ");
		$.ajax({
				type:'POST',
				data :{id : id},
		...
		$("#save"+id).html("<img src='datepicker/save.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=koreksiPenggunaPersediaan.submitKoreksi('"+id+"');></img> &nbsp  &nbsp <img src='images/administrator/images/invalid.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=koreksiPenggunaPersediaan.cancelKoreksi('"+id+"');></img> ");
	},
	sesuai:function(id){
	...
	if(angkaKoreksi == ''){
	    ...
	    $.ajax({
				...
				data:{idAwal : id,
					  ...
					  jeniskoreksiPenggunaPersediaan  : $("#cmbJeniskoreksiPenggunaPersediaan").val()
					  },
	...
	bantu : function(id){
		...
		$("#bantu"+id).text(koreksiPenggunaPersediaan.formatCurrency(angka));
	},
