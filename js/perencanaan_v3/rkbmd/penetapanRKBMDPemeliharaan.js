var penetapanRKBMDPemeliharaanSkpd = new SkpdCls({
	prefix : 'penetapanRKBMDPemeliharaanSkpd',
	formName: 'penetapanRKBMDPemeliharaanForm',
	pilihBidangAfter : function(){penetapanRKBMDPemeliharaan.refreshList(true);},
	pilihUnitAfter : function(){penetapanRKBMDPemeliharaan.refreshList(true);},
	pilihSubUnitAfter : function(){penetapanRKBMDPemeliharaan.refreshList(true);},
	pilihSeksiAfter : function(){penetapanRKBMDPemeliharaan.refreshList(true);}
});

var penetapanRKBMDPemeliharaan = new DaftarObj2({
	prefix : 'penetapanRKBMDPemeliharaan',
	url : 'pages.php?Pg=penetapanRKBMDPemeliharaan',
	formName : 'penetapanRKBMDPemeliharaanForm',
	...
	daftarRender:function(){
		var jenisKegiatan = document.getElementById('cmbJenispenetapanRKBMDPemeliharaan');
		/*cmbJenispenetapanRKBMDPemeliharaan.remove(0);*/
	...
	remove:function(){
		...
		if(errmsg ==''){
			...
			$.ajax({
			  ...
			  success: function(data) {
					var resp = eval('(' + data + ')');
					penetapanRKBMDPemeliharaan.refreshList(true);
	...
	Laporan:function(){
		var me = this;
			...
			$.ajax({
				type:'POST',
				data:$('#'+this.formName).serialize(),
			  	url: 'pages.php?Pg=penetapanRKBMDPemeliharaanNew'+'&tipe=Laporan',
			  	success: function(data) {
					var resp = eval('(' + data + ')');
					if(resp.err == ''){
						document.getElementById(cover).innerHTML = resp.content;
		...
		$.ajax({
			...
			data: { jenisKegiatan : $("#jenisKegiatan").val(),
					...
					e 			  : $("#penetapanRKBMDPemeliharaanSkpdfmSUBUNIT").val(),
					e1 			  : $("#penetapanRKBMDPemeliharaanSkpdfmSEKSI").val(),
			...
			url: this.url+'&tipe=Report',
		  	success: function(data) {
				...
				if(resp.err==''){
					...
					aForm.action= url2+'&tipe='+resp.content.to+'&urusan='+$("#penetapanRKBMDPemeliharaanSkpdfmUrusan").val()+'&bidang='+$("#penetapanRKBMDPemeliharaanSkpdfmSKPD").val()+'&skpd='+$("#penetapanRKBMDPemeliharaanSkpdfmUNIT").val()+'&unit='+$("#penetapanRKBMDPemeliharaanSkpdfmSUBUNIT").val()+'&subunit='+$("#penetapanRKBMDPemeliharaanSkpdfmSEKSI").val()+'&kota='+resp.content.namaPemda+'&tanggalCetak='+resp.content.cetakjang;
					aForm.target='_blank';
					aForm.submit();
					aForm.target='';
					me.Close();
	...
	bantu : function(id){
		...
		$("#bantu"+id).text(penetapanRKBMDPemeliharaan.formatCurrency(angka));
	},
