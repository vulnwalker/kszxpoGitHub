var rpebmdSkpd = new SkpdCls({
	prefix : 'rpebmdSkpd', 
	formName: 'rpebmdForm',// 'adminForm'
	pilihBidangAfter:function(){
		//rpebmd.refreshList(true);
	},
...
var rpebmd = new DaftarObj2({
	prefix : 'rpebmd',
	url : 'pages.php?Pg=rpebmd', 
	formName : 'rpebmdForm',
	...
	formSetDetailEntry : function(){
		...
		document.getElementById('rpebmd_formkiba').style.display = 'none';
		document.getElementById('rpebmd_formkibb').style.display = 'none';
		document.getElementById('rpebmd_formkibc').style.display = 'none';
		document.getElementById('rpebmd_formkibd').style.display = 'none';
		document.getElementById('rpebmd_formkibe').style.display = 'none';
		document.getElementById('rpebmd_formkibf').style.display = 'none';
		switch(jnsbrg){
			case '01': {
				document.getElementById('rpebmd_formkiba').style.display = 'block';
				break;
			}
			case '02': {
				document.getElementById('rpebmd_formkibb').style.display = 'block';
				break;
			}
			case '03': {
				document.getElementById('rpebmd_formkibc').style.display = 'block';
				break;
			}
			case '04':{
				document.getElementById('rpebmd_formkibd').style.display = 'block';
				break;
			}
			case '05': {
				document.getElementById('rpebmd_formkibe').style.display = 'block';
				break;
			}
			case '06': {
				document.getElementById('rpebmd_formkibf').style.display = 'block';
				break;
			}
		}	
	...
	CetakSemua:function(){
		var me = this;
			...
			$.ajax({
				type:'POST',
				data:$('#'+this.formName).serialize(),
			  	url: 'pages.php?Pg=pemindahtangananNew'+'&tipe=CetakSemua',
			  	success: function(data) {
					var resp = eval('(' + data + ')');
					if(resp.err == ''){
						document.getElementById(cover).innerHTML = resp.content;
	...
	CetakHal:function(){
		var me = this;
			...
			$.ajax({
				type:'POST',
				data:$('#'+this.formName).serialize(),
			  	url: 'pages.php?Pg=pemindahtangananNew'+'&tipe=CetakHal',
			  	success: function(data) {
					var resp = eval('(' + data + ')');
					if(resp.err == ''){
						document.getElementById(cover).innerHTML = resp.content;
