var koreksiPengelolaPersediaanSkpd = new SkpdCls({
	prefix : 'koreksiPengelolaPersediaanSkpd',
	formName: 'koreksiPengelolaPersediaanForm',
	pilihBidangAfter : function(){koreksiPengelolaPersediaan.refreshList(true);},
	pilihUnitAfter : function(){koreksiPengelolaPersediaan.refreshList(true);},
	pilihSubUnitAfter : function(){koreksiPengelolaPersediaan.refreshList(true);},
	pilihSeksiAfter : function(){koreksiPengelolaPersediaan.refreshList(true);}
});

var koreksiPengelolaPersediaan = new DaftarObj2({
	prefix : 'koreksiPengelolaPersediaan',
	url : 'pages.php?Pg=koreksiPengelolaPersediaan',
	formName : 'koreksiPengelolaPersediaanForm',
	...
	daftarRender:function(){
		var jenisKegiatan = document.getElementById('cmbJeniskoreksiPengelolaPersediaan');
		/*cmbJeniskoreksiPengelolaPersediaan.remove(0);*/
	...
	remove:function(){
		...
		if(errmsg ==''){
			...
			$.ajax({
			  ...
			  success: function(data) {
					var resp = eval('(' + data + ')');
					koreksiPengelolaPersediaan.refreshList(true);
	...
	Laporan:function(){
		var me = this;
			...
			$.ajax({
				type:'POST',
				data:$('#'+this.formName).serialize(),
			  	url: 'pages.php?Pg=koreksiPengelolaPersediaanNew'+'&tipe=Laporan',
			  	success: function(data) {
					var resp = eval('(' + data + ')');
					if(resp.err == ''){
						document.getElementById(cover).innerHTML = resp.content;
	...
	InputBaru: function(){
		...
		if($("#koreksiPengelolaPersediaanSkpdfmUrusan").val() == '00'){
			errmsg = "Pilih Urusan";
		}else if($("#koreksiPengelolaPersediaanSkpdfmSKPD").val() == '00'){
			errmsg = "Pilih Bidang";
		}else if($("#koreksiPengelolaPersediaanSkpdfmUNIT").val() == '00'){
			errmsg = "Pilih SKPD";
		}else if($("#koreksiPengelolaPersediaanSkpdfmSUBUNIT").val() == '00'){
			errmsg = "Pilih UNIT";
		}else if($("#koreksiPengelolaPersediaanSkpdfmSEKSI").val() == '000'){
			errmsg = "Pilih SUB UNIT";
		}else if($("#cmbJeniskoreksiPengelolaPersediaan").val() == ''){
			errmsg = "Pilih JENIS koreksiPengelolaPersediaan";
		}
	...
	koreksi:function(id){
		...
		$("#spanJumlah"+id).html("<input type='text' name='jumlah"+id+"' id='jumlah"+id+"' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup=koreksiPengelolaPersediaan.bantu('Jumlah"+id+"'); style='text-align: right;'>  ");
		$.ajax({
				type:'POST',
				data :{id : id},
		...
		$("#save"+id).html("<img src='datepicker/save.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=koreksiPengelolaPersediaan.submitKoreksi('"+id+"');></img> &nbsp  &nbsp <img src='images/administrator/images/invalid.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=koreksiPengelolaPersediaan.cancelKoreksi('"+id+"');></img> ");
	},
	sesuai:function(id){
	...
	if(angkaKoreksi == ''){
	    ...
	    $.ajax({
				...
				data:{idAwal : id,
					  ...
					  jeniskoreksiPengelolaPersediaan  : $("#cmbJeniskoreksiPengelolaPersediaan").val()
					  },
	...
	bantu : function(id){
		...
		$("#bantu"+id).text(koreksiPengelolaPersediaan.formatCurrency(angka));
	},
