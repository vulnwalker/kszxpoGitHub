var koreksiPenggunaPengadaanSkpd = new SkpdCls({
	prefix : 'koreksiPenggunaPengadaanSkpd',
	formName: 'koreksiPenggunaPengadaanForm',
	pilihBidangAfter : function(){koreksiPenggunaPengadaan.refreshList(true);},
	pilihUnitAfter : function(){koreksiPenggunaPengadaan.refreshList(true);},
	pilihSubUnitAfter : function(){koreksiPenggunaPengadaan.refreshList(true);},
	pilihSeksiAfter : function(){koreksiPenggunaPengadaan.refreshList(true);}
});

var koreksiPenggunaPengadaan = new DaftarObj2({
	prefix : 'koreksiPenggunaPengadaan',
	url : 'pages.php?Pg=koreksiPenggunaPengadaan',
	formName : 'koreksiPenggunaPengadaanForm',
	...
	daftarRender:function(){
		var jenisKegiatan = document.getElementById('cmbJeniskoreksiPenggunaPengadaan');
		/*cmbJeniskoreksiPenggunaPengadaan.remove(0);*/
	...
	remove:function(){
		...
		if(errmsg ==''){
			...
			$.ajax({
			  ...
			  success: function(data) {
					var resp = eval('(' + data + ')');
					koreksiPenggunaPengadaan.refreshList(true);
			...
			url: this.url+'&tipe=Report',
		  	success: function(data) {
				...
				if(resp.err==''){
					...
					window.open('pages.php?Pg=koreksiPengadaanNew'+'&tipe='+resp.content.to+'&urusan='+$("#koreksiPenggunaPengadaanSkpdfmUrusan").val()+'&bidang='+$("#koreksiPenggunaPengadaanSkpdfmSKPD").val()+'&skpd='+$("#koreksiPenggunaPengadaanSkpdfmUNIT").val()+'&unit='+$("#koreksiPenggunaPengadaanSkpdfmSUBUNIT").val()+'&subunit='+$("#koreksiPenggunaPengadaanSkpdfmSEKSI").val()+'&kota='+resp.content.namaPemda+'&tanggalCetak='+resp.content.cetakjang, '_blank');
					me.Close();
				}else{
					alert(resp.err);
				}
	...
	InputBaru: function(){
		...
		if($("#koreksiPenggunaPengadaanSkpdfmUrusan").val() == '00'){
			errmsg = "Pilih Urusan";
		}else if($("#koreksiPenggunaPengadaanSkpdfmSKPD").val() == '00'){
			errmsg = "Pilih Bidang";
		}else if($("#koreksiPenggunaPengadaanSkpdfmUNIT").val() == '00'){
			errmsg = "Pilih SKPD";
		}else if($("#koreksiPenggunaPengadaanSkpdfmSUBUNIT").val() == '00'){
			errmsg = "Pilih UNIT";
		}else if($("#koreksiPenggunaPengadaanSkpdfmSEKSI").val() == '000'){
			errmsg = "Pilih SUB UNIT";
		}else if($("#cmbJeniskoreksiPenggunaPengadaan").val() == ''){
			errmsg = "Pilih JENIS koreksiPenggunaPengadaan";
		}
	...
	koreksi:function(id){
		...
		$("#spanJumlah"+id).html("<input type='text' name='jumlah"+id+"' id='jumlah"+id+"' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup=koreksiPenggunaPengadaan.bantu('Jumlah"+id+"'); style='text-align: right;'>  ");
		$.ajax({
				type:'POST',
				data :{id : id},
		...
		$("#save"+id).html("<img src='datepicker/save.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=koreksiPenggunaPengadaan.submitKoreksi('"+id+"');></img> &nbsp  &nbsp <img src='images/administrator/images/invalid.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=koreksiPenggunaPengadaan.cancelKoreksi('"+id+"');></img> ");
	},
	sesuai:function(id){
	...
	if(angkaKoreksi == ''){
	    ...
	    $.ajax({
				...
				data:{idAwal : id,
					  ...
					  jeniskoreksiPenggunaPengadaan  : $("#cmbJeniskoreksiPenggunaPengadaan").val()
					  },
	...
	bantu : function(id){
		...
		$("#bantu"+id).text(koreksiPenggunaPengadaan.formatCurrency(angka));
	},
