var koreksiPengelolaPengadaanSkpd = new SkpdCls({
	prefix : 'koreksiPengelolaPengadaanSkpd',
	formName: 'koreksiPengelolaPengadaanForm',
	pilihBidangAfter : function(){koreksiPengelolaPengadaan.refreshList(true);},
	pilihUnitAfter : function(){koreksiPengelolaPengadaan.refreshList(true);},
	pilihSubUnitAfter : function(){koreksiPengelolaPengadaan.refreshList(true);},
	pilihSeksiAfter : function(){koreksiPengelolaPengadaan.refreshList(true);}
});

var koreksiPengelolaPengadaan = new DaftarObj2({
	prefix : 'koreksiPengelolaPengadaan',
	url : 'pages.php?Pg=koreksiPengelolaPengadaan',
	formName : 'koreksiPengelolaPengadaanForm',
	...
	daftarRender:function(){
		var jenisKegiatan = document.getElementById('cmbJeniskoreksiPengelolaPengadaan');
		/*cmbJeniskoreksiPengelolaPengadaan.remove(0);*/
	...
	remove:function(){
		...
		if(errmsg ==''){
			...
			$.ajax({
			  ...
			  success: function(data) {
					var resp = eval('(' + data + ')');
					koreksiPengelolaPengadaan.refreshList(true);
	...
	Laporan:function(){
		var me = this;
			...
			$.ajax({
				type:'POST',
				data:$('#'+this.formName).serialize(),
			  	url: 'pages.php?Pg=koreksiPengelolaPengadaanNew'+'&tipe=Laporan',
			  	success: function(data) {
					var resp = eval('(' + data + ')');
					if(resp.err == ''){
						document.getElementById(cover).innerHTML = resp.content;
	...
	InputBaru: function(){
		...
		if($("#koreksiPengelolaPengadaanSkpdfmUrusan").val() == '00'){
			errmsg = "Pilih Urusan";
		}else if($("#koreksiPengelolaPengadaanSkpdfmSKPD").val() == '00'){
			errmsg = "Pilih Bidang";
		}else if($("#koreksiPengelolaPengadaanSkpdfmUNIT").val() == '00'){
			errmsg = "Pilih SKPD";
		}else if($("#koreksiPengelolaPengadaanSkpdfmSUBUNIT").val() == '00'){
			errmsg = "Pilih UNIT";
		}else if($("#koreksiPengelolaPengadaanSkpdfmSEKSI").val() == '000'){
			errmsg = "Pilih SUB UNIT";
		}else if($("#cmbJeniskoreksiPengelolaPengadaan").val() == ''){
			errmsg = "Pilih JENIS koreksiPengelolaPengadaan";
		}
	...
	koreksi:function(id){
		...
		$("#spanJumlah"+id).html("<input type='text' name='jumlah"+id+"' id='jumlah"+id+"' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup=koreksiPengelolaPengadaan.bantu('Jumlah"+id+"'); style='text-align: right;'>  ");
		$.ajax({
				type:'POST',
				data :{id : id},
		...
		$("#save"+id).html("<img src='datepicker/save.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=koreksiPengelolaPengadaan.submitKoreksi('"+id+"');></img> &nbsp  &nbsp <img src='images/administrator/images/invalid.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=koreksiPengelolaPengadaan.cancelKoreksi('"+id+"');></img> ");
	},
	sesuai:function(id){
	...
	if(angkaKoreksi == ''){
	    ...
	    $.ajax({
				...
				data:{idAwal : id,
					  ...
					  jeniskoreksiPengelolaPengadaan  : $("#cmbJeniskoreksiPengelolaPengadaan").val()
					  },
	...
	bantu : function(id){
		...
		$("#bantu"+id).text(koreksiPengelolaPengadaan.formatCurrency(angka));
	},
