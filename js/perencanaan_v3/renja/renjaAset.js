var renjaAset = new DaftarObj2({
	prefix : 'renjaAset',
	url : 'pages.php?Pg=renjaAset',
	formName : 'renjaAsetForm',
	renjaAset_form : '0',
	loading: function(){
		this.topBarRender();
		this.filterRender();
		...
		if(errmsg ==''){
			...
			$.ajax({
			  ...
			  success: function(data) {
					var resp = eval('(' + data + ')');
					if(resp.err == "") {
						var IDrenjaAset = Number(resp.content.idrenjaAset);
						aForm.action= 'pages.php?Pg=renjaAset_ins&YN=1&id='+IDrenjaAset;
						aForm.target='_blank';
						aForm.submit();
						aForm.target='';
					}else{
	...
	Baru: function(){
		...
		if (err =='' ){
			...
			if(me.renjaAset_form==0){//baru dari renjaAset
	...
	sesuai:function(id){
		...
		/*$("#span"+id).html("<input type='text' name='"+id+"' id='"+id+"' onkeyup='return renjaAset.submitKoreksi(event,id)'  value='"+$("#valueJumlah"+id).val()+"'  style='text-align: right;'>  <span id='bantu"+id+"' >"+popupBarang.formatCurrency($("#valueJumlah"+id).val())+"</span>");*/

	},
	koreksi:function(id){
		$("#span"+id).html("<input type='text' name='"+id+"' id='"+id+"' onkeyup='return renjaAset.submitKoreksi(event,id)' onkeypress='return event.charCode >= 48 && event.charCode <= 57'  style='text-align: right;'>  <span id='bantu"+id+"' ></span>");
	},
   ...
   SaveCatatan: function(id){
	...
	detailrenjaAset: function(kode){
		...
		var me = this;
			...
			$.ajax({
				type:'POST',
				data: $('#'+this.formName).serialize()+"&renjaAset_cb%5B%5D=1.08.01.01.01" ,
				url: this.url+'&tipe=lihatrenjaAset',
			  	success: function(data) {
					var resp = eval('(' + data + ')');
					if (resp.err ==''){
						document.getElementById(cover).innerHTML = resp.content;
			...
			$.ajax({
			  ...
			  success: function(data) {
					...
					if(resp.err ==''){
						...
						aForm.action= 'pages.php?Pg=renjaAset_ins&id='+ID_PLAFON+"&ID_RENJA="+ID_EDIT;
						aForm.target='_blank';
						aForm.submit();
						aForm.target='';
					}else{
	...
	Simpan: function(){
		...
		$.ajax({
			...
			$.ajax({
			  ...
			  type : 'POST',
			  data:$('#'+this.formName).serialize()+'&cetakjang='+$("#cetakjang").val(),
			  success: function(data) {
					var resp = eval('(' + data + ')');
					if(resp.err !=''){
						alert(resp.err);
					}else{
						window.open('pages.php?Pg=renjaAsetNew&tipe=Laporan'+'&urusan='+resp.content.urusan+'&bidang='+resp.content.bidang+'&skpd='+resp.content.skpd+'&kota='+resp.content.namaPemda+'&tanggalCetak='+resp.content.cetakjang, '_blank');
					}
			  }
			});
	...
	TanggalCetak:function(){
		var me = this;
			...
			$.ajax({
				type:'POST',
				data:$('#'+this.formName).serialize(),
			  	url: 'pages.php?Pg=renjaAsetNew&tipe=TanggalCetak',
			  	success: function(data) {
					var resp = eval('(' + data + ')');
					if(resp.err == ''){
						document.getElementById(cover).innerHTML = resp.content;
