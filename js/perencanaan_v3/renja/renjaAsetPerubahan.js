var renjaAsetPerubahan = new DaftarObj2({
	prefix : 'renjaAsetPerubahan',
	url : 'pages.php?Pg=renjaAsetPerubahan',
	formName : 'renjaAsetPerubahanForm',
	renjaAsetPerubahan_form : '0',
	loading: function(){
		this.topBarRender();
		this.filterRender();
		this.daftarRender();
		...
		if(errmsg ==''){
			...
			$.ajax({
			  ...
			  success: function(data) {
					var resp = eval('(' + data + ')');
					if(resp.err == "") {
						var IDrenjaAsetPerubahan = Number(resp.content.idrenjaAsetPerubahan);
						aForm.action= 'pages.php?Pg=renjaAsetPerubahan_ins&YN=1&id='+IDrenjaAsetPerubahan;
						aForm.target='_blank';
						aForm.submit();
						aForm.target='';
					}else{
	...
	BidangAfter2: function(){
		renjaAsetPerubahan.refreshList(true);
	},
	...
	Baru: function(){
		...
		if (err =='' ){
			...
			if(me.renjaAsetPerubahan_form==0){//baru dari renjaAsetPerubahan
	...
	sesuai:function(id){
		...
		/*$("#span"+id).html("<input type='text' name='"+id+"' id='"+id+"' onkeyup='return renjaAsetPerubahan.submitKoreksi(event,id)'  value='"+$("#valueJumlah"+id).val()+"'  style='text-align: right;'>  <span id='bantu"+id+"' >"+popupBarang.formatCurrency($("#valueJumlah"+id).val())+"</span>");*/

	},
	koreksi:function(id){
		$("#span"+id).html("<input type='text' name='"+id+"' id='"+id+"' onkeyup='return renjaAsetPerubahan.submitKoreksi(event,id)' onkeypress='return event.charCode >= 48 && event.charCode <= 57'  style='text-align: right;'>  <span id='bantu"+id+"' ></span>");
	},
   ...
   SaveCatatan: function(id){
		...
		$.ajax({
			...
			url: this.url+'&tipe=SaveCatatan',
		  	success: function(data) {
				...
				delElem(cover);
				if(resp.err==''){
					if(me.renjaAsetPerubahan_form==0){
						me.Close();
						me.AfterSimpan();
					}
	...
	,
	detailrenjaAsetPerubahan: function(kode){
		...
		var me = this;
			...
			$.ajax({
				type:'POST',
				data: $('#'+this.formName).serialize()+"&renjaAsetPerubahan_cb%5B%5D=1.08.01.01.01" ,
				url: this.url+'&tipe=lihatrenjaAsetPerubahan',
			  	success: function(data) {
					var resp = eval('(' + data + ')');
					if (resp.err ==''){
						document.getElementById(cover).innerHTML = resp.content;
			...
			$.ajax({
			  ...
			  success: function(data) {
					...
					if(resp.err ==''){
						...
						aForm.action= 'pages.php?Pg=renjaAsetPerubahan_ins&id='+ID_PLAFON+"&ID_RENJA="+ID_EDIT;
						aForm.target='_blank';
						aForm.submit();
						aForm.target='';
					}else{
		  	...
		  	success: function(data) {
				...
				if(resp.err==''){
					if(me.renjaAsetPerubahan_form==0){
						...
						me.Close();
						me.refreshComborenjaAsetPerubahan();
					}
			...
			$.ajax({
			  ...
			  success: function(data) {
					...
					if(resp.err !=''){
						...
						aForm.action= 'pages.php?Pg=renjaAsetPerubahan&tipe=Laporan';
						aForm.target='_blank';
						aForm.submit();
						aForm.target='';
					}
