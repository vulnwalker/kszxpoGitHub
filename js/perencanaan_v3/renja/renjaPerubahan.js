var renjaPerubahan_v3 = new DaftarObj2({
	prefix : 'renjaPerubahan_v3',
	url : 'pages.php?Pg=renjaPerubahan_v3', 
	formName : 'renjaPerubahan_v3Form',
	renjaPerubahan_v3_form : '0',
	loading: function(){
		this.topBarRender();
		this.filterRender();
		this.daftarRender();
		...
		if(errmsg ==''){
			...
			$.ajax({
			  ...
			  success: function(data) {
					var resp = eval('(' + data + ')');
					if(resp.err == "") {
						var IDrenjaPerubahan_v3 = Number(resp.content.idrenjaPerubahan_v3);
						aForm.action= 'pages.php?Pg=renjaPerubahan_ins_v3&YN=1&id='+IDrenjaPerubahan_v3;
						aForm.target='_blank';
						aForm.submit();	
						aForm.target='';
					}else{
	...
	BidangAfter2: function(){
		renjaPerubahan_v3.refreshList(true);
	},
	...
	Baru: function(){	
		...
		if (err =='' ){		
			...
			if(me.renjaPerubahan_v3_form==0){//baru dari renjaPerubahan_v3
	...
	sesuai:function(id){
		...
		/*$("#span"+id).html("<input type='text' name='"+id+"' id='"+id+"' onkeyup='return renjaPerubahan_v3.submitKoreksi(event,id)'  value='"+$("#valueJumlah"+id).val()+"'  style='text-align: right;'>  <span id='bantu"+id+"' >"+popupBarang.formatCurrency($("#valueJumlah"+id).val())+"</span>");*/

	},
	koreksi:function(id){
		$("#span"+id).html("<input type='text' name='"+id+"' id='"+id+"' onkeyup='return renjaPerubahan_v3.submitKoreksi(event,id)' onkeypress='return event.charCode >= 48 && event.charCode <= 57'  style='text-align: right;'>  <span id='bantu"+id+"' ></span>");
	},
   ...
   SaveCatatan: function(id){
		...
		$.ajax({
			...
			url: this.url+'&tipe=SaveCatatan',
		  	success: function(data) {		
				...
				delElem(cover);		
				if(resp.err==''){
					if(me.renjaPerubahan_v3_form==0){
						me.Close();
						me.AfterSimpan();						
					}
	...
	,	
	detailrenjaPerubahan_v3: function(kode){	
		...
		var me = this;
			...
			$.ajax({
				type:'POST', 
				data: $('#'+this.formName).serialize()+"&renjaPerubahan_v3_cb%5B%5D=1.08.01.01.01" ,
				url: this.url+'&tipe=lihatrenjaPerubahan_v3',
			  	success: function(data) {		
					var resp = eval('(' + data + ')');	
					if (resp.err ==''){		
						document.getElementById(cover).innerHTML = resp.content;
			...
			$.ajax({
			  ...
			  success: function(data) {
					...
					if(resp.err ==''){
						...
						aForm.action= 'pages.php?Pg=renjaPerubahan_ins_v3&id='+ID_PLAFON+"&ID_RENJA="+ID_EDIT;
						aForm.target='_blank';
						aForm.submit();	
						aForm.target='';
					}else{
		  	...
		  	success: function(data) {		
				...
				if(resp.err==''){
					if(me.renjaPerubahan_v3_form==0){
						...
						me.Close();
						me.refreshComborenjaPerubahan_v3();
					}
			...
			$.ajax({
			  ...
			  success: function(data) {
					...
					if(resp.err !=''){
						...
						aForm.action= 'pages.php?Pg=renjaPerubahan_v3&tipe=Laporan';
						aForm.target='_blank';
						aForm.submit();	
						aForm.target='';
					}
