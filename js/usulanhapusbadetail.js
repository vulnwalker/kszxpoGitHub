var UsulanHapusdetailSkpd = new SkpdCls({
	prefix : 'UsulanHapusbadetailSkpd', formName:'adminForm'
});

var UsulanHapusbadetail = new DaftarObj2({
	prefix : 'UsulanHapusbadetail',
	url : 'pages.php?Pg=usulanhapusbadetail', 
	//formName : 'UsulanHapus_form',// 'ruang_form',
	formName : 'UsulanHapusba_form', 
	...
	daftarRender : function(){
		...
		//if(document.getElementById(this.formName)) var formName = this.formname;
		//if(document.getElementById('UsulanHapus_form')) var formName = 'UsulanHapus_form';
	...
	//	alert('tes');
	//	if(document.getElementById('barcodeUsulanHapusBA_input')) barcodeUsulanHapusBA.reset();
	//},
	sumHalRender : function(){
		var me =this; //render sumhal
		addCoverPage2(
			'daftar_cover',	999, 	true, 	true,{renderTo: this.prefix+'_cont_hal',
			imgsrc: 'images/wait.gif',
			style: {position:'absolute', top:'5', left:'5'}
			}
		);
   ...
   periksaEntry:function(){
		...
		if(jmlcek ==0){
			...
			if(confirm('Pengecekan '+jmlcek+' Data ?')){
		this.daftarPilih=[]	
