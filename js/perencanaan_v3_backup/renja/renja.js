var renja_v3 = new DaftarObj2({
	prefix : 'renja_v3',
	url : 'pages.php?Pg=renja_v3', 
	formName : 'renja_v3Form',
	renja_v3_form : '0',
	loading: function(){
		this.topBarRender();
		this.filterRender();
		...
		if(errmsg ==''){
			...
			$.ajax({
			  ...
			  success: function(data) {
					var resp = eval('(' + data + ')');
					if(resp.err == "") {
						var IDrenja_v3 = Number(resp.content.idrenja_v3);
						aForm.action= 'pages.php?Pg=renja_ins_v3&YN=1&id='+IDrenja_v3;
						aForm.target='_blank';
						aForm.submit();	
						aForm.target='';
					}else{
	...
	Baru: function(){	
		...
		if (err =='' ){		
			...
			if(me.renja_v3_form==0){//baru dari renja_v3
	...
	sesuai:function(id){
		...
		/*$("#span"+id).html("<input type='text' name='"+id+"' id='"+id+"' onkeyup='return renja_v3.submitKoreksi(event,id)'  value='"+$("#valueJumlah"+id).val()+"'  style='text-align: right;'>  <span id='bantu"+id+"' >"+popupBarang.formatCurrency($("#valueJumlah"+id).val())+"</span>");*/

	},
	koreksi:function(id){
		$("#span"+id).html("<input type='text' name='"+id+"' id='"+id+"' onkeyup='return renja_v3.submitKoreksi(event,id)' onkeypress='return event.charCode >= 48 && event.charCode <= 57'  style='text-align: right;'>  <span id='bantu"+id+"' ></span>");
	},
   ...
   SaveCatatan: function(id){
	...
	detailrenja_v3: function(kode){	
		...
		var me = this;
			...
			$.ajax({
				type:'POST', 
				data: $('#'+this.formName).serialize()+"&renja_v3_cb%5B%5D=1.08.01.01.01" ,
				url: this.url+'&tipe=lihatrenja_v3',
			  	success: function(data) {		
					var resp = eval('(' + data + ')');	
					if (resp.err ==''){		
						document.getElementById(cover).innerHTML = resp.content;
			...
			$.ajax({
			  ...
			  success: function(data) {
					...
					if(resp.err ==''){
						...
						aForm.action= 'pages.php?Pg=renja_ins_v3&id='+ID_PLAFON+"&ID_RENJA="+ID_EDIT;
						aForm.target='_blank';
						aForm.submit();	
						aForm.target='';
					}else{
			...
			$.ajax({
			  ...
			  success: function(data) {
					...
					if(resp.err !=''){
						...
						aForm.action= 'pages.php?Pg=renja_v3&tipe=Laporan';
						aForm.target='_blank';
						aForm.submit();	
						aForm.target='';
					}
