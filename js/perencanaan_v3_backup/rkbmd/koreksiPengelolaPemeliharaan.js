...
var koreksiPengelolaPemeliharaan = new DaftarObj2({
	...
	koreksi:function(id){
		...
		$("#spanJumlah"+id).html("<input type='text' name='jumlah"+id+"' id='jumlah"+id+"' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup=koreksiPengelolaPemeliharaan.bantu('Jumlah"+id+"'); style='text-align: right;'>  ");
			$.ajax({
				type:'POST',
				data :{id : id},
	...
	sesuai:function(id){
		...
		$("#updatePengelola"+id).attr('align','center');
		    ...
		    $.ajax({
					...
					data:{idAwal : id,
						  angkaKoreksi : isi,
						  jeniskoreksiPengelolaPemeliharaan  : $("#cmbJeniskoreksiPengelolaPemeliharaan").val()	
						  },
