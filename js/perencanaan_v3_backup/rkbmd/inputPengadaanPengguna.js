var inputPengadaanPenggunaSkpd = new SkpdCls({
	prefix : 'inputPengadaanPenggunaSkpd', 
	formName: 'inputPengadaanPenggunaForm',
	pilihBidangAfter : function(){inputPengadaanPengguna.refreshList(true);},
	pilihUnitAfter : function(){inputPengadaanPengguna.refreshList(true);},
	pilihSubUnitAfter : function(){inputPengadaanPengguna.refreshList(true);},
	pilihSeksiAfter : function(){inputPengadaanPengguna.refreshList(true);}
});

var inputPengadaanPengguna = new DaftarObj2({
	prefix : 'inputPengadaanPengguna',
	url : 'pages.php?Pg=inputPengadaanPengguna', 
	formName : 'inputPengadaanPenggunaForm',
	...
	DownloadExcel: function(){
		...
		$.ajax({
			...
			data: { jenisKegiatan : $("#jenisKegiatan").val(),
					c1 			  : $("#inputPengadaanPenggunaSkpdfmUrusan").val(),
					c 			  : $("#inputPengadaanPenggunaSkpdfmSKPD").val(),
					d 			  : $("#inputPengadaanPenggunaSkpdfmUNIT").val(),
					e 			  : $("#inputPengadaanPenggunaSkpdfmSUBUNIT").val(),
					e1 			  : $("#inputPengadaanPenggunaSkpdfmSEKSI").val(),
	...
	daftarRender:function(){
		var jenisKegiatan = document.getElementById('cmbJenisinputPengadaanPengguna');
		/*cmbJenisinputPengadaanPengguna.remove(0);*/
			...
			$.ajax({
			  ...
			  success: function(data) {
					...
					if(resp.err == "") {
						...
						aForm.action= 'pages.php?Pg=pengadaanPengguna_'+resp.content.kemana+'&YN=1&id='+IDrenja;
						aForm.target='_blank';
						aForm.submit();	
						aForm.target='';
					}else{
	...
	remove:function(){
		...
		if(errmsg ==''){ 
			...
			$.ajax({
			  ...
			  success: function(data) {
					var resp = eval('(' + data + ')');
					inputPengadaanPengguna.refreshList(true);
	...
	InputBaru: function(){
		...
		if($("#inputPengadaanPenggunaSkpdfmUrusan").val() == '0'){
			errmsg = "Pilih Urusan";
		}else if($("#inputPengadaanPenggunaSkpdfmSKPD").val() == '00'){
			errmsg = "Pilih Bidang";
		}else if($("#inputPengadaanPenggunaSkpdfmUNIT").val() == '00'){
			errmsg = "Pilih SKPD";
		}
	...
	koreksi:function(id){
		...
		$("#spanJumlah"+id).html("<input type='text' name='jumlah"+id+"' id='jumlah"+id+"' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup=inputPengadaanPengguna.bantu('Jumlah"+id+"'); style='text-align: right;'>  ");
		$.ajax({
				type:'POST',
				data :{id : id},
		...
		$("#save"+id).html("<img src='datepicker/save.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=inputPengadaanPengguna.submitKoreksi('"+id+"');></img> &nbsp  &nbsp <img src='images/administrator/images/invalid.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=inputPengadaanPengguna.cancelKoreksi('"+id+"');></img> ");
	},
	sesuai:function(id){
	...
	if(angkaKoreksi <= '0'){
	    ...
	    $.ajax({
				...
				data:{idAwal : id,
					  ...
					  jenisinputPengadaanPengguna  : $("#cmbJenisinputPengadaanPengguna").val()	
					  },
	...
	bantu : function(id){
		...
		$("#bantu"+id).text(inputPengadaanPengguna.formatCurrency(angka));
	},
