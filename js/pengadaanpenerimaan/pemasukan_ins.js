...
var pemasukan_ins = new DaftarObj2({
	...
	filterRenderAfter : function(){
		...
		setTimeout(function myFunction() {
			...
			me.SetPenerima();
			me.Get_TanggalDok_Sumber();
			me.nyalakandatepicker();
			me.nyalakandatepicker2();
			me.OptionPenyedia();
			
			parent.window.onunload=me.TutupHalaman();
		},1000);						
	},
	...
	inputpenerimaan: function(){
		...
		var cover = this.prefix+'_forminputdetail';
	...
	rincianpenerimaan: function(Bersih=1){
		var me=this;
		//UPDATE DPA 27 DESEMBER 2017 ----------------------------
		if(Bersih == 1){
			DataPengaturan.Set_cekDcID_val("GetRekeningDPA","");
			DataPengaturan.Set_cekDcID_val("IdRekeningDPA","");
		}
		me.inputpenerimaan();
	...
	tabelRekening: function(hapus = 1){
		...
		$.ajax({
			...
			success: function(data) {	
				...
				if(resp.err==''){
					...
					me.rincianpenerimaan();
					//setTimeout(function myFunction() {me.reload_carabayar()},1000);					
				}else{
					alert(resp.err);
				}
			}
		...
		setTimeout(function myFunction() {me.CekSesuai()},1000);
	},
	
	caraperolehan_After: function(BIRMS=0, konten=''){
		var me =this;
		document.getElementById('pilCaraPerolehan').innerHTML = konten;
		setTimeout(function myFunction() {
			me.tabelRekening();
			me.GetData_DPA_ProgKeg();
			me.inputpenerimaan();
		},100);			
		...
		if(asalusul == '1'){
			$.ajax({
				...
				success: function(data) {	
					var resp = eval('(' + data + ')');			
					if(resp.err==''){
						me.caraperolehan_After(BIRMS, resp.content);							
					}else{
						alert(resp.err);
					}
				}
	...
	CariProgram: function(){
		...
		cariprogram.def_c1 = document.getElementById("c1nya").value;	
		cariprogram.def_c = document.getElementById("cnya").value;	
		cariprogram.def_d = document.getElementById("dnya").value;	
	},
	...
	CariPenyedia: function(){
		var me=this;
		cariPenyedia.windowShow(this.formName);
		cariPenyedia.GetData_After= function(konten){
			$("#penyedian").val(konten);
			me.OptionPenyedia();
		};
	},
	...
	SimpanPenerima: function(parram=0){
		var me= this;	
		this.OnErrorClose = false	
		document.body.style.overflow='hidden';
		var cover = this.prefix+'_formsimpan';
			...
			url: this.url+'&tipe=simpanPenerima',
		  	success: function(data) {		
				...
				if(resp.err==''){
					me.Close();
					if(parram == 1){
						pemasukan_baru.TutupForm('pemasukan_baru_formcover_TTD');
						$('#cmb_penerima').html(resp.content.cmb_penerima);
						$('#cmb_mengetahui').html(resp.content.cmb_mengetahui);
					}					
	...
	reload_carabayar: function(){
		...
		if(jns_transaksi == "1"){
			if(cara_bayar == '1' || cara_bayar == '2'){
				...
				var status_kdp = confirm("Kode Barang Jadi Kontruksi Dalam Pengerjaan (KDP) ?");
				if(status_kdp == true){
					$("#status_kdp").val("1");
				}else{
					$("#status_kdp").val("0");
				}
				me.Get_Btn_CariTermin();
			}else{
	...
	cariDPA: function(){
		var me=this;
		cariDPA.windowShow(me.formName);	
		cariDPA.AfterwindowSave= function(dt){
			me.GetData_RefreshFromDPA();
		};	
	},
	
	GetData_RefreshFromDPA: function(){
		var me=this;
		me.rincianpenerimaan();
		me.Set_SaveRekeningForDPA();
		me.GetData_DPA_ProgKeg();
	},
	
	GetData_DPA_ProgKeg:function(){
		var me = this;
		var err='';
		...
		$.ajax({
			type:'POST', 
			data:$('#'+this.formName).serialize(),
		  	url: this.url+'&tipe=GetData_DPA_ProgKeg',
		  	success: function(data) {		
				var resp = eval('(' + data + ')');	
				delElem(cover);	
				document.body.style.overflow='auto';
				if(resp.err == ""){	
					if(resp.content.status_dpa == 1){
						$("#program").val(resp.content.nm_program);
						$("#prog").val(resp.content.kd_program);
						$("#dafkeg").html(resp.content.nm_kegiatan);
						$("#status_dpa").val(resp.content.status_dpa);
					}					
				}else{
					alert(resp.err);
				}
	...
	Set_SaveRekeningForDPA:function(){
		var me = this;
		var err='';
		...
		$.ajax({
			type:'POST', 
			data:$('#'+this.formName).serialize(),
		  	url: this.url+'&tipe=Set_SaveRekeningForDPA',
		  	success: function(data) {		
				var resp = eval('(' + data + ')');	
				delElem(cover);	
				document.body.style.overflow='auto';
	...
	RincianRekeningDPA:function(Datanya, Idnya){
		var me = this;
		document.getElementById("GetRekeningDPA").value=Datanya;
		document.getElementById("IdRekeningDPA").value=Idnya;
		me.rincianpenerimaan(0);
		me.inputpenerimaan();
	},
	
	Get_jmlMaksBarang_DPA:function(){
		var me = this;
		var err='';
		...
		$.ajax({
			type:'POST', 
			data:$('#'+this.formName).serialize(),
		  	url: this.url+'&tipe=Get_jmlMaksBarang_DPA',
		  	success: function(data) {		
				var resp = eval('(' + data + ')');	
				delElem(cover);	
				document.body.style.overflow='auto';
				if(resp.err == ""){	
					$("#jml_maksimal").val(resp.content.jml_maksimal);
					$("#hrg_maksimal").val(resp.content.hrg_maksimal);
					$("#kuantitas_maksimal").val(resp.content.kuantitas_maksimal);
					$("#konten_BTNRealisasi").html(resp.content.konten_BTNRealisasi);
					if(document.getElementById("barang_didistribusi"))document.getElementById("barang_didistribusi").style.marginLeft = "35px";			
				}else{
					alert(resp.err);
				}
		  	}
	...
	Get_Btn_CariTermin:function(){
		var me = this;
		var err='';
		
		var cover = this.prefix+'_formcover_CariTermin';
		document.body.style.overflow='hidden';
		addCoverPage2(cover,1,true,false);	
		$.ajax({
			type:'POST', 
			data:$('#'+this.formName).serialize(),
		  	url: this.url+'&tipe=Get_Btn_CariTermin',
		  	success: function(data) {		
				var resp = eval('(' + data + ')');	
				delElem(cover);	
				document.body.style.overflow='auto';
				if(resp.err == ""){	
					$("#reload_carabayar").html(resp.content);
				}else{
					alert(resp.err);
				}
		  	}
	...
	cariDokumenKontrak: function(){
		var me=this;
		ref_dokumen_kontrak.windowShow(me.formName, 1, 1);	
		ref_dokumen_kontrak.AfterwindowSave= function(dt){
			me.ambilNomorDokumen(dt.nomor_dok,"y");
			me.cariDokumenKontrak_After();
		};	
	},
	
	cariDokumenKontrak_After:function(){
		var me = this;
		var err='';
		
		var cover = this.prefix+'_formcover_CariTermin1';
		document.body.style.overflow='hidden';
		addCoverPage2(cover,1,true,false);	
		$.ajax({
			type:'POST', 
			data:$('#'+this.formName).serialize(),
		  	url: this.url+'&tipe=cariDokumenKontrak_After',
		  	success: function(data) {		
				var resp = eval('(' + data + ')');	
				delElem(cover);	
				document.body.style.overflow='auto';
				if(resp.err == ""){	
					$("#program").val(resp.content.program);				
					$("#prog").val(resp.content.prog);				
					$("#pekerjaan").val(resp.content.pekerjaan);				
					$("#dafkeg").html(resp.content.dafkeg);		
					me.tabelRekening();							
				}else{
					alert(resp.err);
				}
	...
	Get_jmlBarangRealisasi_DPA:function(){
		var me = this;
		var err='';
		...
		$.ajax({
			type:'POST', 
			data:$('#'+this.formName).serialize(),
		  	url: this.url+'&tipe=Get_jmlBarangRealisasi_DPA',
		  	success: function(data) {		
				var resp = eval('(' + data + ')');	
				delElem(cover);	
				document.body.style.overflow='auto';
				if(resp.err == ""){	
					$("#konten_BTNRealisasi").html(resp.content);	
					if(document.getElementById("barang_didistribusi"))document.getElementById("barang_didistribusi").style.marginLeft = "5px";			
				}else{
					alert(resp.err);
				}
		  	}
	...
	/*tabelRekening_DataFormDPA_jmlMaksBarang:function(){
		var me = this;
		var err='';
		...
		$.ajax({
			type:'POST', 
			data:$('#'+this.formName).serialize(),
		  	url: this.url+'&tipe=tabelRekening_DataFormDPA_jmlMaksBarang',
		  	success: function(data) {		
				var resp = eval('(' + data + ')');	
				delElem(cover);	
				document.body.style.overflow='auto';
				if(resp.err == ""){	
					var konten = resp.content;
					for(i=0;i<konten.length;i++){
						alert(konten.i.kdrek);
						$("#fm_JmlBrgDPA_"+konten.i.kdrek).html(konten.i.jml);
					}
				}else{
					alert(resp.err);
				}
