...
var pemasukan = new DaftarObj2({
	...
	Edit:function(){
		...
		if(errmsg ==''){ 
			var box = this.GetCbxChecked();
			var cover1 = this.prefix+'_formUbahcover2';
			document.body.style.overflow='hidden';
			addCoverPage2(cover1,1,true,false);
			$.ajax({
				type:'POST', 
				data:$('#'+this.formName).serialize(),
	...
	var me = this;
		...
		if(errmsg =='Data belum dipilih!'){ 
			...
			var cover1 = this.prefix+'_formAtrcover2';
			document.body.style.overflow='hidden';
			addCoverPage2(cover1,1,true,false);
			$.ajax({
				type:'POST', 
	...
	SimpanPosting: function(){
		...
		var cover1 = this.prefix+'_formMulaiposting1';
		addCoverPage2(cover1,1,true,false);
										...
										switch(resp.content.jns_trans){
											case "1":
												if(resp.content.konfirmasi == "Batalkan Posting Data ?"){
													...
													if(document.getElementById('idbarangnya_0')){					
														me.JML_POSTING=0;
														me.ProsesPosting_Pengadaan(0);
													}
												}
											break;
											case "2":
		...
		tot_jmlbarang = document.getElementById('tot_jmlbarang').value;
			...
			$.ajax({
				...
				url: this.url+'&tipe=HapusPosting',
					...
					var cover = 'pemasukan_formcover_load';
						if(resp.err==''){
							$("#ket_tot_jml_post").html(resp.content.JumlahBarang);
	...
	ProsesPosting_Pengadaan_After: function(urutan, konten){
		var me=this;
		jmlSetPOST = 0;
		if(document.getElementById("jml_brg_nextPOST"))jmlSetPOST = document.getElementById("jml_brg_nextPOST").value;	
		me.JML_POSTING = Number(me.JML_POSTING) + Number(konten.jml_proses);	
			
		if(konten.Langsung == "1")urutan+=1;		
		me.function_persenan(konten.persen);
		$("#ket_tot_jml_post").html(konten.jml_barang);
		
		if(konten.selesai == 1){
				me.UpdatePostingBerhasil();
		}else{		
			if(me.JML_POSTING >= jmlSetPOST){
				var tanya = confirm("Data Sukses Di Posting ! Mau di Lanjutkan ?");
				if(tanya){
					me.JML_POSTING=0;
					me.ProsesPosting_Pengadaan(0);
				}
				if(document.getElementById("pemasukan_formcover_load"))delElem("pemasukan_formcover_load");
			}else{				
				me.ProsesPosting_Pengadaan(urutan);	
			}
		}		
	...
	ProsesPosting_Pengadaan: function(urutan){
		var me = this;			
		var cover = this.prefix+'_formcover_posting';
		addCoverPage2(cover,1,true,false);	
		document.body.style.overflow='hidden';
		//IdDet = document.getElementById('idbarangnya_'+urutan).value;
		IdDet = $("#idbarangnya_"+urutan).val();
		//alert(urutan+" A"+IdDet);
		
		$.ajax({
			type:'POST', 
			data:$('#'+this.prefix+"_form").serialize(),
			url: this.url+'&tipe=ProsesPosting_Pengadaan&IdPenDet='+IdDet+"&TotalSudahPosting="+me.JML_POSTING,
		  	success: function(data) {		
				var resp = eval('(' + data + ')');					
				delElem(cover);
				if (resp.err ==''){
					me.ProsesPosting_Pengadaan_After(urutan, resp.content);
				}else{
					alert(resp.err);
					document.body.style.overflow='auto';
				}
	...
	HitungTotalSemua:function(){
		var me = this;
		var cover = this.prefix+'_formcover_Hrg';
		document.body.style.overflow='hidden';
		addCoverPage2(cover,1,true,false);
		$.ajax({
			type:'POST', 
			data:$('#'+this.formName).serialize(),
		  	url: this.url+'&tipe=HitungTotalSemua',
		  	success: function(data) {		
				var resp = eval('(' + data + ')');
				delElem(cover);
				document.body.style.overflow='auto';
				if(resp.err == ''){
					$("#Tot_Barang").html(resp.content.Tot_Barang);
					$("#Tot_Rek").html(resp.content.Tot_Rek);
					$("#Tot_Beli").html(resp.content.Tot_Beli);
					$("#Tot_Atribusi").html(resp.content.Tot_Atribusi);
					$("#Tot_Perolehan").html(resp.content.Tot_Perolehan);
				}else{
					alert(resp.err);
				}	
		  	}
