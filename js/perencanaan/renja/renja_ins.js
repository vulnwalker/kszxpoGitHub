var renja = new DaftarObj2({
	prefix : 'renja',
	url : 'pages.php?Pg=renja', 
	formName : 'renjaForm',
	renja_form : '0',
	loading: function(){
		this.topBarRender();
			...
			$.ajax({
			  ...
			  success: function(data) {
					...
					if(resp.err == "") {
						...
						aForm.action= 'pages.php?Pg=renja_ins&YN=1&id='+IDrenja;
						aForm.target='_blank';
						aForm.submit();	
						aForm.target='';
					}else{
	...
	Baru: function(){	
		...
		if (err =='' ){		
			...
			if(me.renja_form==0){//baru dari renja
	...
	sesuai:function(id){
		...
		/*$("#span"+id).html("<input type='text' name='"+id+"' id='"+id+"' onkeyup='return renja.submitKoreksi(event,id)'  value='"+$("#valueJumlah"+id).val()+"'  style='text-align: right;'>  <span id='bantu"+id+"' >"+popupBarang.formatCurrency($("#valueJumlah"+id).val())+"</span>");*/

	},
	koreksi:function(id){
		$("#span"+id).html("<input type='text' name='"+id+"' id='"+id+"' onkeyup='return renja.submitKoreksi(event,id)' onkeypress='return event.charCode >= 48 && event.charCode <= 57'  style='text-align: right;'>  <span id='bantu"+id+"' ></span>");
	},
   ...
   SaveCatatan: function(id){
				...
				data:$('#'+this.formName).serialize(),
			  	...
			  	success: function(data) {		
					...
					if(resp.err == ''){
						...
						delElem(cover);
						if(resp.err == 'Tahap Valdisasi Telah Habis'){
							alert(resp.err);
							window.location.reload();
						}else{
							alert(resp.err);
	...
	detailrenja: function(kode){	
		...
		var me = this;
			...
			$.ajax({
				type:'POST', 
				data: $('#'+this.formName).serialize()+"&renja_cb%5B%5D=1.08.01.01.01" ,
				url: this.url+'&tipe=lihatrenja',
			  	success: function(data) {		
					var resp = eval('(' + data + ')');	
					if (resp.err ==''){		
						document.getElementById(cover).innerHTML = resp.content;
		...
		if(errmsg ==''){ 
			...
			$.ajax({
			  ...
			  success: function(data) {
					...
					aForm.action= 'pages.php?Pg=renja_ins&id='+ID_PLAFON+"&ID_RENJA="+ID_EDIT;
					aForm.target='_blank';
					aForm.submit();	
					aForm.target='';
			  }
			...
			$.ajax({
			  ...
			  success: function(data) {
					...
					if(resp.err !=''){
						...
						aForm.action= 'pages.php?Pg=renja&tipe=Laporan';
						aForm.target='_blank';
						aForm.submit();	
						aForm.target='';
					}
