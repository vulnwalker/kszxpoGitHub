var rkbmdPengadaanSkpd = new SkpdCls({
	prefix : 'rkbmdPengadaanSkpd', 
	formName: 'rkbmdPengadaanForm',
	pilihBidangAfter : function(){rkbmdPengadaan.refreshList(true);},
	pilihUnitAfter : function(){rkbmdPengadaan.refreshList(true);},
	pilihSubUnitAfter : function(){rkbmdPengadaan.refreshList(true);},
	pilihSeksiAfter : function(){rkbmdPengadaan.refreshList(true);}
});

var rkbmdPengadaan = new DaftarObj2({
	prefix : 'rkbmdPengadaan',
	url : 'pages.php?Pg=rkbmdPengadaan', 
	formName : 'rkbmdPengadaanForm',
	...
	daftarRender:function(){
		var jenisKegiatan = document.getElementById('cmbJenisrkbmdPengadaan');
		/*cmbJenisrkbmdPengadaan.remove(0);*/
	...
	remove:function(){
		...
		if(errmsg ==''){ 
			...
			$.ajax({
			  ...
			  success: function(data) {
					var resp = eval('(' + data + ')');
					rkbmdPengadaan.refreshList(true);
	...
	InputBaru: function(){
		...
		if($("#rkbmdPengadaanSkpdfmUrusan").val() == '00'){
			errmsg = "Pilih Urusan";
		}else if($("#rkbmdPengadaanSkpdfmSKPD").val() == '00'){
			errmsg = "Pilih Bidang";
		}else if($("#rkbmdPengadaanSkpdfmUNIT").val() == '00'){
			errmsg = "Pilih SKPD";
		}else if($("#rkbmdPengadaanSkpdfmSUBUNIT").val() == '00'){
			errmsg = "Pilih UNIT";
		}else if($("#rkbmdPengadaanSkpdfmSEKSI").val() == '000'){
			errmsg = "Pilih SUB UNIT";
		}else if($("#cmbJenisrkbmdPengadaan").val() == ''){
			errmsg = "Pilih JENIS rkbmdPengadaan";
		}
	...
	koreksi:function(id){
		...
		$("#spanJumlah"+id).html("<input type='text' name='jumlah"+id+"' id='jumlah"+id+"' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup=rkbmdPengadaan.bantu('Jumlah"+id+"'); style='text-align: right;'>  ");
		$.ajax({
				type:'POST',
				data :{id : id},
		...
		$("#save"+id).html("<img src='datepicker/save.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkbmdPengadaan.submitKoreksi('"+id+"');></img> &nbsp  &nbsp <img src='images/administrator/images/invalid.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkbmdPengadaan.cancelKoreksi('"+id+"');></img> ");
	},
	sesuai:function(id){
	...
	if(angkaKoreksi <= '0'){
	    ...
	    $.ajax({
				...
				data:{idAwal : id,
					  ...
					  jenisrkbmdPengadaan  : $("#cmbJenisrkbmdPengadaan").val()	
					  },
	...
	bantu : function(id){
		...
		$("#bantu"+id).text(rkbmdPengadaan.formatCurrency(angka));
	},
	...
	Laporan:function(){
		...
		$.ajax({
			...
			data: { jenisKegiatan : $("#jenisKegiatan").val(),
					c1 			  : $("#rkbmdPengadaanSkpdfmUrusan").val(),
					c 			  : $("#rkbmdPengadaanSkpdfmSKPD").val(),
					d 			  : $("#rkbmdPengadaanSkpdfmUNIT").val(),
					e 			  : $("#rkbmdPengadaanSkpdfmSUBUNIT").val(),
					e1 			  : $("#rkbmdPengadaanSkpdfmSEKSI").val(),
