var formRkaSkpd1SKPD = new SkpdCls({
	prefix : 'formRkaSkpd1SKPD', formName:'formRkaSkpd1Form', kolomWidth:120,
...
var formRkaSkpd1 = new DaftarObj2({
	prefix : 'formRkaSkpd1',
	url : 'pages.php?Pg=formRkaSkpd1', 
	formName : 'formRkaSkpd1Form',
	...
	hitungSelisih:function(namaBantu){
		...
		$("#"+namaBantu).text("Rp. "+formRkaSkpd1.formatCurrency($("#"+idPost).val())  );
		...
		$("#selisih").val(selisih);	
		$("#bantuPenjumlahan").text("Rp. "+formRkaSkpd1.formatCurrency(jumlahHargaAlokasi)  );		
		$("#bantuSelisih").text("Rp. "+formRkaSkpd1.formatCurrency(selisih)  );				
	},
					...
					data : {
				  	...
				  	success: function(data) {		
						...
						if(resp.err == ''){
							...
							$("#bantuJumlahHarga").text("Rp. "+formRkaSkpd1.formatCurrency($("#jumlahHarga").val()));
						}else{
							alert(resp.err);
						}			
				  	}
	...
	bantu : function(){
		$("#bantuSatuanHarga").text("Rp. "+formRkaSkpd1.formatCurrency($("#hargaSatuan").val())  );
		$("#jumlahHarga").val($("#hargaSatuan").val() * $("#volumeBarang").val());
		$("#bantuJumlahHarga").text("Rp. "+formRkaSkpd1.formatCurrency($("#jumlahHarga").val()));
		$("#teralokasi").val('');
	},
	...
	hitung: function(){
		if($("#jenisAlokasi").val() == '' ){
			...
			if($("#jenisAlokasi").val() == "BULANAN"){
				...
				formRkaSkpd1.hitungSelisih('bantuJan');
				formRkaSkpd1.hitungSelisih('bantuFeb');
				formRkaSkpd1.hitungSelisih('bantuApr');
				formRkaSkpd1.hitungSelisih('bantuMei');
				formRkaSkpd1.hitungSelisih('bantuJul');
				formRkaSkpd1.hitungSelisih('bantuAgu');
				formRkaSkpd1.hitungSelisih('bantuOkt');
				formRkaSkpd1.hitungSelisih('bantuNop');
				formRkaSkpd1.hitungSelisih('bantuMar');
				formRkaSkpd1.hitungSelisih('bantuJun');
				formRkaSkpd1.hitungSelisih('bantuSep');
				formRkaSkpd1.hitungSelisih('bantuDes');
	...
	jenisAlokasiChanged : function(){
		...
		if($("#jenisAlokasi").val() == "BULANAN"){
			...
			$.ajax({
				...
				url: this.url+'&tipe=clearAlokasi',
				success: function(data) {	
					formRkaSkpd1.formAlokasiTriwulan();
				}
			});	
	...
	cancelEdit : function(){
		formRkaSkpd1.refreshList(true);
		setTimeout(function myFunction() {formRkaSkpd1.refreshList(true);},1000);
	},
				...
				success: function(data) {	
					...
					if(resp.content.kunci == '0'){
						...
						if(resp.content.jenis_alokasi_kas == "TRIWULAN"){
							$("#tombolAlokasi").attr("onclick","formRkaSkpd1.formAlokasiTriwulan();");
						}else{
							$("#tombolAlokasi").attr("onclick","formRkaSkpd1.formAlokasi();");
						}
						...
						$("#btsave").attr('href','javascript:formRkaSkpd1.saveEdit('+id+')');
						$("#btcancel").attr('href','javascript:formRkaSkpd1.cancelEdit()');
				...
				success: function(data) {	
					...
					if(resp.err == ''){
						...
						if(resp.err == "Lengkapi Alokasi"){
							...
							formRkaSkpd1.formRincianVolume();
						}else{
							alert(resp.err);
						}
