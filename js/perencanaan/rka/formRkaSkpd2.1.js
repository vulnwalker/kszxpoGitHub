var formRkaSkpd21SKPD = new SkpdCls({
	prefix : 'formRkaSkpd21SKPD', formName:'formRkaSkpd21Form', kolomWidth:120,
...
var formRkaSkpd21 = new DaftarObj2({
	prefix : 'formRkaSkpd21',
	url : 'pages.php?Pg=formRkaSkpd21', 
	formName : 'formRkaSkpd21Form',
	...
	loading: function(){
		...
		this.topBarRender();
		this.filterRender();
		formRkaSkpd21.refreshList(true);
	...
	hitungSelisih:function(namaBantu){
		...
		$("#"+namaBantu).text("Rp. "+formRkaSkpd21.formatCurrency($("#"+idPost).val())  );
		...
		$("#selisih").val(selisih);	
		$("#bantuPenjumlahan").text("Rp. "+formRkaSkpd21.formatCurrency(jumlahHargaAlokasi)  );		
		$("#bantuSelisih").text("Rp. "+formRkaSkpd21.formatCurrency(selisih)  );				
	},
					...
					data : {
				  	...
				  	success: function(data) {		
						...
						if(resp.err == ''){
							...
							$("#bantuJumlahHarga").text("Rp. "+formRkaSkpd21.formatCurrency($("#jumlahHarga").val()));
						}else{
							alert(resp.err);
						}			
				  	}
	...
	bantu : function(){
		$("#bantuSatuanHarga").text("Rp. "+formRkaSkpd21.formatCurrency($("#hargaSatuan").val())  );
		$("#jumlahHarga").val($("#hargaSatuan").val() * $("#volumeBarang").val());
		$("#bantuJumlahHarga").text("Rp. "+formRkaSkpd21.formatCurrency($("#jumlahHarga").val()));
		$("#teralokasi").val('');
	},
	...
	hitung: function(){
		if($("#jenisAlokasi").val() == '' ){
			...
			if($("#jenisAlokasi").val() == "BULANAN"){
				...
				formRkaSkpd21.hitungSelisih('bantuJan');
				formRkaSkpd21.hitungSelisih('bantuFeb');
				formRkaSkpd21.hitungSelisih('bantuApr');
				formRkaSkpd21.hitungSelisih('bantuMei');
				formRkaSkpd21.hitungSelisih('bantuJul');
				formRkaSkpd21.hitungSelisih('bantuAgu');
				formRkaSkpd21.hitungSelisih('bantuOkt');
				formRkaSkpd21.hitungSelisih('bantuNop');
				formRkaSkpd21.hitungSelisih('bantuMar');
				formRkaSkpd21.hitungSelisih('bantuJun');
				formRkaSkpd21.hitungSelisih('bantuSep');
				formRkaSkpd21.hitungSelisih('bantuDes');
	...
	jenisAlokasiChanged : function(){
		...
		if($("#jenisAlokasi").val() == "BULANAN"){
			...
			$.ajax({
				...
				url: this.url+'&tipe=clearAlokasi',
				success: function(data) {	
					formRkaSkpd21.formAlokasiTriwulan();
				}
			});	
	...
	cancelEdit : function(){
		formRkaSkpd21.refreshList(true);
		setTimeout(function myFunction() {formRkaSkpd21.refreshList(true);},1000);
	},
				...
				success: function(data) {	
					...
					if(resp.content.kunci == '0'){
						...
						if(resp.content.jenis_alokasi_kas == "TRIWULAN"){
							$("#tombolAlokasi").attr("onclick","formRkaSkpd21.formAlokasiTriwulan();");
						}else{
							$("#tombolAlokasi").attr("onclick","formRkaSkpd21.formAlokasi();");
						}
						...
						$("#btsave").attr('href','javascript:formRkaSkpd21.saveEdit('+id+')');
						$("#btcancel").attr('href','javascript:formRkaSkpd21.cancelEdit()');
				...
				success: function(data) {	
					...
					if(resp.err == ''){
						...
						if(resp.err == "Lengkapi Alokasi"){
							...
							formRkaSkpd21.formRincianVolume();
						}else{
							alert(resp.err);
						}
