var popupJadwal_v2 = new DaftarObj2({
	prefix : 'popupJadwal_v2',
	url : 'pages.php?Pg=popupJadwal_v2', 
	formName : 'popupJadwal_v2Form',
	el_namaHistori : '',
	el_idHistori: '',	
	...
	newRow:function(){
		...
		$.ajax({
			...
			data:$('#'+this.formName).serialize(),
		  	...
		  	success: function(data) {		
				var resp = eval('(' + data + ')');
				popupJadwal_v2.refreshList(true);
	...
	windowSave: function(){
		...
		$.ajax({
			...
			data: {tahun : $("#tahun").val(),
		  	...
		  	url: 'pages.php?Pg=popupJadwal_v2&tipe=cekDataAda',
		  	success: function(data) {		
				var resp = eval('(' + data + ')');
				if(resp.content.ada == 'ya'){
						if (confirm('Data sudah ada ingin menggantinya ?')) {
							popupJadwal_v2.execute();
						} else {
	...
	execute:function(){
		var me= this;
		var cover = 'popupJadwal_v2_getdata';
		addCoverPage2(cover,1,true,false);
		...
		$.ajax({
		  ...
		  var resp = eval('('+data+')');
		  	if(resp.err == ''){
				delElem(cover);
				ref_tahap_anggaran_v2.refreshList(true);
				me.windowClose();
			}else{
				delElem(cover);
				alert(resp.err);
