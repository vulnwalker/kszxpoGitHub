var ref_bank2 = new DaftarObj2({
	prefix : 'ref_bank2',
	url : 'pages.php?Pg=ref_bank2', 
	formName : 'ref_bankForm',
	...
	windowSave: function(id){
		...
		if(errmsg ==''){ 
			...
			addCoverPage2(cover,1,true,false);				
			$.ajax({
				url: 'pages.php?Pg=ref_bank2&tipe=getdata&id='+id,
			  	success: function(data) {		
					var resp = eval('(' + data + ')');			
					delElem(cover);
					if(resp.err==''){
					
						if(document.getElementById('nm_bank'))document.getElementById('nm_bank').value=resp.content.nm_bank;
						if(document.getElementById('rek_bank'))document.getElementById('rek_bank').value=resp.content.rek_bank;
