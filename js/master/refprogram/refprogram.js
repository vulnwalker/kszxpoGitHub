var refprogram = new DaftarObj2({
	...
	programBaru: function(){	
		var me = this;
			...
			$.ajax({
				type:'POST', 
				data:$('#refprogram_form').serialize(),
	...
	SaveProgram: function(){
		...
		document.body.style.overflow='hidden';
		var cover = this.prefix+'_formsimpanProgram';
		addCoverPage2(cover,1,true,false);	
		
		$.ajax({
			type:'POST', 
			data:$('#'+this.prefix+'_KDform').serialize(),
			url: this.url+'&tipe=simpanProgram',
		  	success: function(data) {		
				var resp = eval('(' + data + ')');	
				delElem(cover);		
				
				if(resp.err==''){
					me.refreshProgram(resp.content);
					me.Close4();
				}else{
					alert(resp.err);
				}
	...
	refreshProgram : function(id_ProgramBaru){
	var me = this; //alert('tes');	//alert(this.prefix);
		$.ajax({
		  url: 'pages.php?Pg=refprogram&tipe=refreshProgram&id_ProgramBaru='+id_ProgramBaru,
		  type : 'POST',
		  data:$('#refprogram_form').serialize(),
		  success: function(data) {		
			var resp = eval('(' + data + ')');			
			document.getElementById('cont_program').innerHTML = resp.content.p;
		me.getProgram();
		  }
		});
	},
	...
	var me = this; 
		$.ajax({
		  url: 'pages.php?Pg=refprogram&tipe=pilihProgram',
		  type : 'POST',
		  data:$('#refprogram_form').serialize(),
		  success: function(data) {		
			var resp = eval('(' + data + ')');			
			document.getElementById('kegiatan').value = resp.content.kegiatan;
		  }
		});
	},
	getProgram : function(){
	var me = this; //alert('tes');	//alert(this.prefix);
		...
		$.ajax({
		  url: 'pages.php?Pg=refprogram&tipe=getProgram',
		  type : 'POST',
