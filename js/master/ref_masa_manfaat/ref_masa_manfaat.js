var ref_masa_manfaat = new DaftarObj2({
	prefix : 'ref_masa_manfaat',
	url : 'pages.php?Pg=ref_masa_manfaat',
	formName : 'ref_masa_manfaatForm',
	el_IDBARANG : '',
	el_NMBARANG: '',
	el_kode_account : '',
	el_nama_account : '',
	...
	var me = this; 
		...
		$.ajax({
		  url: 'pages.php?Pg=ref_masa_manfaat&tipe=pilihJenis',
		  type : 'POST',
		  data:$('#ref_masa_manfaat_form').serialize(),
		  success: function(data) {		
			var resp = eval('(' + data + ')');			
	...
	var me = this; 
		$.ajax({
		  url: 'pages.php?Pg=ref_masa_manfaat&tipe=pilihObjek',
		  type : 'POST',
		  data:$('#ref_masa_manfaat_form').serialize(),
		  success: function(data) {		
			var resp = eval('(' + data + ')');			
	...
	pilihRincianObjek : function(){
	var me = this; 
		$.ajax({
		  url: 'pages.php?Pg=ref_masa_manfaat&tipe=pilihRincianObjek',
		  type : 'POST',
		  data:$('#ref_masa_manfaat_form').serialize(),
		  success: function(data) {		
			var resp = eval('(' + data + ')');			
	...
	pilihSubRincianObjek : function(){
	var me = this; 
		$.ajax({
		  url: 'pages.php?Pg=ref_masa_manfaat&tipe=pilihSubRincianObjek',
		  type : 'POST',
		  data:$('#ref_masa_manfaat_form').serialize(),
		  success: function(data) {		
			var resp = eval('(' + data + ')');			
	...
	jenisChanged: function(){
		$.ajax({
			type:'POST',
			data:{
						jenisBarang : $("#cmbJenisBarang").val(),
					 },
			url: this.url+'&tipe=jenisChanged',
				success: function(data) {
				var resp = eval('(' + data + ')');
				if(resp.err==''){
					$("#spanKodeBarang").html(resp.content.isiSpanKodeBarang);
				}
				else{
					alert(resp.err);
				}
