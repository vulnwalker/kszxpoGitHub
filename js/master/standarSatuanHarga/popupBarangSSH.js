var popupBarangSSH = new DaftarObj2({
	prefix : 'popupBarangSSH',
	url : 'pages.php?Pg=popupBarangSSH',
	formName : 'popupBarangSSHForm',
	...
	windowShow: function(kodeRENJA){
		...
		$.ajax({
			type:'POST',
			data:$('#'+this.formName).serialize()+"kodeRENJA="+kodeRENJA,
	...
	windowSave: function(id){
		var me= this;
			...
			addCoverPage2(cover,1,true,false);
			$.ajax({
				url: 'pages.php?Pg=popupBarangSSH&tipe=getdata&id='+id,
			  	success: function(data) {
					var resp = eval('(' + data + ')');
					delElem(cover);
					if(resp.err==''){
						...
						$("#satuan").val(resp.content.satuanBarang);
