var RekapSKPDSkpd = new SkpdCls({
	prefix : 'RekapSKPDSkpd', formName:'adminForm'
});


var RekapSKPD = new DaftarObj2({
	prefix : 'RekapSKPD',
	//url : 'pages.php?Pg=Rekap2Ajx', 
	url : 'pages.php?Pg=RekapSKPD', 
	...
	exportXls2 : function(){
	    ...
	    a.href = tableToExcel('divcetak','neracaskpd');
	    a.download = 'neracaskpd.xls';
	...
	reqTotal : function(href_, idel_,iterator_){
		var me = this;	
		var href_ = href_.replace("Jurnal","RekapSKPD");	
		if(href_!=''){
			document.getElementById(idel_).innerHTML = "<img src='images/wait.gif' height='10px' >";
			this.ajx = $.ajax({
			  	url: href_+'&tipe=rekapskpd&idel='+idel_+'&iter='+iterator_,
			 	type:'POST', 
				data:$('#'+this.formName).serialize(), 
			  	success: function(data) {		
					...
					var jmlcol = 15;
					for(i=1; i<=jmlcol; i++){
						if(document.getElementById('vrekap_'+arrid[1]+'_'+i) ){
							document.getElementById('vrekap_'+arrid[1]+'_'+i).innerHTML = resp.content.totalN[i][3];
							document.getElementById('vrekap_'+arrid[1]+'_'+i).setAttribute('title',resp.content.totalN[i][4] );
							//alert('vrekap_'+arrid[1]+'_'+i);	
						}else{
							//alert('vrekap_'+arrid[1]+'_'+i+' tidak ada!');
						}
						//alert(i+' tidak ada!');
					}
					
					//document.getElementById('vrekap_'+arrid[1]+'_'+21).innerHTML = resp.content.totalN[20][3];
					//document.getElementById('vrekap_'+arrid[1]+'_'+21).setAttribute('title',resp.content.totalN[i][4] );
					/**
					document.getElementById(arrid[0]+'_'+arrid[1]+'_'+1).innerHTML = resp.content.vtotal2 ;
