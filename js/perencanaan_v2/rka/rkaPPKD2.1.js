var rkaPPKD21_v2 = new DaftarObj2({
	prefix : 'rkaPPKD21_v2',
	url : 'pages.php?Pg=rka-ppkd-2.1_v2', 
	formName : 'rkaPPKD21_v2Form',
	rkaPPKD21_v2_form : '0',//default js rkaPPKD21_v2
	loading: function(){
		//alert('loading');
		this.topBarRender();
		this.filterRender();
...
/*				me.sumHalRender();*/
			...
			alert("Pilih SKPD");
				...
				data : {c1 : $("#cmbUrusan").val(),
			  	...
			  	success: function(data) {		
					var resp = eval('(' + data + ')');
					if(resp.err == ''){
						aForm.action= 'pages.php?Pg=formRkaPpkd21_v2&skpd='+skpd;
						aForm.target='_blank';
						aForm.submit();	
						aForm.target='';
					}else{
	...
	unlockFindRekening: function(){
		...
		if( $("#cmbJenisRKAForm").val() == ""){
			...
			$("#findRekening").attr('onclick',"rkaPPKD21_v2.findRekening('"+$("#cmbJenisRKAForm").val()+"');");
				...
				$.ajax({
				  ...
				  success: function(data) {
						...
						if(resp.err == "") {
							...
							aForm.action= 'pages.php?Pg=formRkaPpkd21_v2&rek='+kodeRek+'&skpd='+skpd;
							aForm.target='_blank';
							aForm.submit();	
							aForm.target='';
						}else{
	...
	koreksi:function(id){
		$("#spanVolumeBarang"+id).html("<input type='text' name='volumeBarang"+id+"' id='volumeBarang"+id+"' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='rkaPPKD21_v2.bantuVolumeBarang("+id+")'   style='text-align: right;'>  <span id='bantuVolumeBarang"+id+"' ></span>");
		$("#spanSatuanHarga"+id).html("<input type='text' name='satuanHarga"+id+"' id='satuanHarga"+id+"' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='rkaPPKD21_v2.bantuSatuanHarga("+id+")'    style='text-align: right;'>  <span id='bantuSatuanHarga"+id+"' ></span>");
		$("#buttonSubmitKoreksi"+id).html("<img src='datepicker/save.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkaPPKD21_v2.submitKoreksi('"+id+"');></img>&nbsp  &nbsp <img src='images/administrator/images/invalid.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkaPPKD21_v2.refreshList(true);></img>");
	...
	bantuVolumeBarang : function(id){
		document.getElementById('bantuVolumeBarang'+id).innerHTML = rkaPPKD21_v2.formatCurrency2($("#volumeBarang"+id).val());
		var hasilKali = Number($("#volumeBarang"+id).val()) * Number($("#satuanHarga"+id).val()) ; 
		$("#spanJumlahHarga"+id).text(rkaPPKD21_v2.formatCurrency(hasilKali));
	},
	bantuSatuanHarga : function(id){
		document.getElementById('bantuSatuanHarga'+id).innerHTML =  rkaPPKD21_v2.formatCurrency($("#satuanHarga"+id).val());
		var hasilKali = Number($("#volumeBarang"+id).val()) * Number($("#satuanHarga"+id).val()) ; 
		$("#spanJumlahHarga"+id).text(rkaPPKD21_v2.formatCurrency(hasilKali));
	},
	...
	bantu : function(){
		$("#bantu").text("Rp. "+rkaPPKD21_v2.formatCurrency($("#hargaSatuan").val())  );
		...
		$.ajax({
			...
			url: this.url+'&tipe=simpan',
		  	success: function(data) {		
				...
				//document.getElementById(cover).innerHTML = resp.content;
				if(resp.err==''){
					if(me.rkaPPKD21_v2_form==0){
						me.Close();
						me.AfterSimpan();						
					}else{
						me.Close();
						barang.refreshComborkaPPKD21_v2();
					}
   ...
   SaveCatatan: function(id){
	...
	hitungSelisih:function(namaBantu){
		...
		$("#"+namaBantu).text("Rp. "+rkaPPKD21_v2.formatCurrency($("#"+idPost).val())  );
		...
		$("#selisih").val(selisih);	
		$("#bantuPenjumlahan").text("Rp. "+rkaPPKD21_v2.formatCurrency(jumlahHargaAlokasi)  );		
		$("#bantuSelisih").text("Rp. "+rkaPPKD21_v2.formatCurrency(selisih)  );				
	},
	...
	jenisAlokasiChanged : function(id){
		...
		if($("#jenisAlokasi").val() == "BULANAN"){
			...
			$.ajax({
				...
				url: this.url+'&tipe=clearAlokasi',
				success: function(data) {	
					rkaPPKD21_v2.formAlokasiTriwulan(id);
				}
			});	
	...
	hitung: function(){
		if($("#jenisAlokasi").val() == '' ){
			...
			if($("#jenisAlokasi").val() == "BULANAN"){
				...
				rkaPPKD21_v2.hitungSelisih('bantuJan');
				rkaPPKD21_v2.hitungSelisih('bantuFeb');
				rkaPPKD21_v2.hitungSelisih('bantuApr');
				rkaPPKD21_v2.hitungSelisih('bantuMei');
				rkaPPKD21_v2.hitungSelisih('bantuJul');
				rkaPPKD21_v2.hitungSelisih('bantuAgu');
				rkaPPKD21_v2.hitungSelisih('bantuOkt');
				rkaPPKD21_v2.hitungSelisih('bantuNop');
				rkaPPKD21_v2.hitungSelisih('bantuMar');
				rkaPPKD21_v2.hitungSelisih('bantuJun');
				rkaPPKD21_v2.hitungSelisih('bantuSep');
				rkaPPKD21_v2.hitungSelisih('bantuDes');
			}else if($("#jenisAlokasi").val() == "TRIWULAN"){
				var jumlahHargaForm = $("#jumlahHargaForm").val();
				$("#jan").val('');
				$("#feb").val('');
