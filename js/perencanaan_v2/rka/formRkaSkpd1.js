var formRkaSkpd1_v2SKPD = new SkpdCls({
	prefix : 'formRkaSkpd1_v2SKPD', formName:'formRkaSkpd1_v2Form', kolomWidth:120,
...
var formRkaSkpd1_v2 = new DaftarObj2({
	prefix : 'formRkaSkpd1_v2',
	url : 'pages.php?Pg=formRkaSkpd1_v2', 
	formName : 'formRkaSkpd1_v2Form',
	...
	loading: function(){
		...
		this.topBarRender();
		this.filterRender();
		formRkaSkpd1_v2.refreshList(true);
	...
	hitungSelisih:function(namaBantu){
		...
		$("#"+namaBantu).text("Rp. "+formRkaSkpd1_v2.formatCurrency($("#"+idPost).val())  );
		...
		$("#selisih").val(selisih);	
		$("#bantuPenjumlahan").text("Rp. "+formRkaSkpd1_v2.formatCurrency(jumlahHargaAlokasi)  );		
		$("#bantuSelisih").text("Rp. "+formRkaSkpd1_v2.formatCurrency(selisih)  );				
	},
					...
					data : {
				  	...
				  	success: function(data) {		
						...
						if(resp.err == ''){
							...
							$("#bantuJumlahHarga").text("Rp. "+formRkaSkpd1_v2.formatCurrency($("#jumlahHarga").val()));
						}else{
							alert(resp.err);
						}			
				  	}
	...
	bantu : function(){
		$("#bantuSatuanHarga").text("Rp. "+formRkaSkpd1_v2.formatCurrency($("#hargaSatuan").val())  );
		$("#jumlahHarga").val($("#hargaSatuan").val() * $("#volumeBarang").val());
		$("#bantuJumlahHarga").text("Rp. "+formRkaSkpd1_v2.formatCurrency($("#jumlahHarga").val()));
		$("#teralokasi").val('');
	},
	...
	hitung: function(){
		if($("#jenisAlokasi").val() == '' ){
			...
			if($("#jenisAlokasi").val() == "BULANAN"){
				...
				formRkaSkpd1_v2.hitungSelisih('bantuJan');
				formRkaSkpd1_v2.hitungSelisih('bantuFeb');
				formRkaSkpd1_v2.hitungSelisih('bantuApr');
				formRkaSkpd1_v2.hitungSelisih('bantuMei');
				formRkaSkpd1_v2.hitungSelisih('bantuJul');
				formRkaSkpd1_v2.hitungSelisih('bantuAgu');
				formRkaSkpd1_v2.hitungSelisih('bantuOkt');
				formRkaSkpd1_v2.hitungSelisih('bantuNop');
				formRkaSkpd1_v2.hitungSelisih('bantuMar');
				formRkaSkpd1_v2.hitungSelisih('bantuJun');
				formRkaSkpd1_v2.hitungSelisih('bantuSep');
				formRkaSkpd1_v2.hitungSelisih('bantuDes');
	...
	jenisAlokasiChanged : function(){
		...
		if($("#jenisAlokasi").val() == "BULANAN"){
			...
			$.ajax({
				...
				url: this.url+'&tipe=clearAlokasi',
				success: function(data) {	
					formRkaSkpd1_v2.formAlokasiTriwulan();
				}
			});	
	...
	cancelEdit : function(){
		formRkaSkpd1_v2.refreshList(true);
		setTimeout(function myFunction() {formRkaSkpd1_v2.refreshList(true);},1000);
	},
				...
				success: function(data) {	
					...
					if(resp.content.kunci == '0'){
						...
						if(resp.content.jenis_alokasi_kas == "TRIWULAN"){
							$("#tombolAlokasi").attr("onclick","formRkaSkpd1_v2.formAlokasiTriwulan();");
						}else{
							$("#tombolAlokasi").attr("onclick","formRkaSkpd1_v2.formAlokasi();");
						}
						...
						$("#btsave").attr('href','javascript:formRkaSkpd1_v2.saveEdit('+id+')');
						$("#btcancel").attr('href','javascript:formRkaSkpd1_v2.cancelEdit()');
				...
				success: function(data) {	
					...
					if(resp.err == ''){
						...
						if(resp.err == "Lengkapi Alokasi"){
							...
							formRkaSkpd1_v2.formRincianVolume();
						}else{
							alert(resp.err);
						}
