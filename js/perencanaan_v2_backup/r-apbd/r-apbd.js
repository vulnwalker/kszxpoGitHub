var r_apbd_v2 = new DaftarObj2({
	prefix : 'r_apbd_v2',
	url : 'pages.php?Pg=r-apbd_v2', 
	formName : 'r_apbd_v2Form',
	r_apbd_v2_form : '0',//default js r_apbd_v2
	loading: function(){
		//alert('loading');
		this.topBarRender();
		this.filterRender();
...
/*				me.sumHalRender();*/
	...
	Baru: function(){	
		...
		if (err =='' ){		
			...
			if(me.r_apbd_v2_form==0){//baru dari r_apbd_v2
	...
	koreksi:function(id){
		$("#span"+id).html("<input type='text' name='"+id+"' id='"+id+"' onkeyup='return r_apbd_v2.submitKoreksi(event,id)'   style='text-align: right;'>  <span id='bantu"+id+"' ></span>");
	},
		...
		$.ajax({
			...
			url: this.url+'&tipe=simpan',
		  	success: function(data) {		
				...
				//document.getElementById(cover).innerHTML = resp.content;
				if(resp.err==''){
					if(me.r_apbd_v2_form==0){
						me.Close();
						me.AfterSimpan();						
					}else{
						me.Close();
						barang.refreshCombor_apbd_v2();
					}
