var formRkaPpkd21_v2SKPD = new SkpdCls({
	prefix : 'formRkaPpkd21_v2SKPD', formName:'formRkaPpkd21_v2Form', kolomWidth:120,
...
var formRkaPpkd21_v2 = new DaftarObj2({
	prefix : 'formRkaPpkd21_v2',
	url : 'pages.php?Pg=formRkaPpkd21_v2', 
	formName : 'formRkaPpkd21_v2Form',
	...
	loading: function(){
		...
		this.topBarRender();
		this.filterRender();
		formRkaPpkd21_v2.refreshList(true);
	...
	hitungSelisih:function(namaBantu){
		...
		$("#"+namaBantu).text("Rp. "+formRkaPpkd21_v2.formatCurrency($("#"+idPost).val())  );
		...
		$("#selisih").val(selisih);	
		$("#bantuPenjumlahan").text("Rp. "+formRkaPpkd21_v2.formatCurrency(jumlahHargaAlokasi)  );		
		$("#bantuSelisih").text("Rp. "+formRkaPpkd21_v2.formatCurrency(selisih)  );				
	},
					...
					data : {
				  	...
				  	success: function(data) {		
						...
						if(resp.err == ''){
							...
							$("#bantuJumlahHarga").text("Rp. "+formRkaPpkd21_v2.formatCurrency($("#jumlahHarga").val()));
						}else{
							alert(resp.err);
						}			
				  	}
	...
	bantu : function(){
		$("#bantuSatuanHarga").text("Rp. "+formRkaPpkd21_v2.formatCurrency($("#hargaSatuan").val())  );
		$("#jumlahHarga").val($("#hargaSatuan").val() * $("#volumeBarang").val());
		$("#bantuJumlahHarga").text("Rp. "+formRkaPpkd21_v2.formatCurrency($("#jumlahHarga").val()));
		$("#teralokasi").val('');
	},
	...
	hitung: function(){
		if($("#jenisAlokasi").val() == '' ){
			...
			if($("#jenisAlokasi").val() == "BULANAN"){
				...
				formRkaPpkd21_v2.hitungSelisih('bantuJan');
				formRkaPpkd21_v2.hitungSelisih('bantuFeb');
				formRkaPpkd21_v2.hitungSelisih('bantuApr');
				formRkaPpkd21_v2.hitungSelisih('bantuMei');
				formRkaPpkd21_v2.hitungSelisih('bantuJul');
				formRkaPpkd21_v2.hitungSelisih('bantuAgu');
				formRkaPpkd21_v2.hitungSelisih('bantuOkt');
				formRkaPpkd21_v2.hitungSelisih('bantuNop');
				formRkaPpkd21_v2.hitungSelisih('bantuMar');
				formRkaPpkd21_v2.hitungSelisih('bantuJun');
				formRkaPpkd21_v2.hitungSelisih('bantuSep');
				formRkaPpkd21_v2.hitungSelisih('bantuDes');
	...
	jenisAlokasiChanged : function(){
		...
		if($("#jenisAlokasi").val() == "BULANAN"){
			...
			$.ajax({
				...
				url: this.url+'&tipe=clearAlokasi',
				success: function(data) {	
					formRkaPpkd21_v2.formAlokasiTriwulan();
				}
			});	
	...
	cancelEdit : function(){
		formRkaPpkd21_v2.refreshList(true);
		setTimeout(function myFunction() {formRkaPpkd21_v2.refreshList(true);},1000);
	},
				...
				success: function(data) {	
					...
					if(resp.content.kunci == '0'){
						...
						if(resp.content.jenis_alokasi_kas == "TRIWULAN"){
							$("#tombolAlokasi").attr("onclick","formRkaPpkd21_v2.formAlokasiTriwulan();");
						}else{
							$("#tombolAlokasi").attr("onclick","formRkaPpkd21_v2.formAlokasi();");
						}
						...
						$("#btsave").attr('href','javascript:formRkaPpkd21_v2.saveEdit('+id+')');
						$("#btcancel").attr('href','javascript:formRkaPpkd21_v2.cancelEdit()');
				...
				success: function(data) {	
					...
					if(resp.err == ''){
						...
						if(resp.err == "Lengkapi Alokasi"){
							...
							formRkaPpkd21_v2.formRincianVolume();
						}else{
							alert(resp.err);
						}
