var formRkaPpkd32_v2SKPD = new SkpdCls({
	prefix : 'formRkaPpkd32_v2SKPD', formName:'formRkaPpkd32_v2Form', kolomWidth:120,
...
var formRkaPpkd32_v2 = new DaftarObj2({
	prefix : 'formRkaPpkd32_v2',
	url : 'pages.php?Pg=formRkaPpkd32_v2', 
	formName : 'formRkaPpkd32_v2Form',
	...
	loading: function(){
		...
		this.topBarRender();
		this.filterRender();
		formRkaPpkd32_v2.refreshList(true);
	...
	hitungSelisih:function(namaBantu){
		...
		$("#"+namaBantu).text("Rp. "+formRkaPpkd32_v2.formatCurrency($("#"+idPost).val())  );
		...
		$("#selisih").val(selisih);	
		$("#bantuPenjumlahan").text("Rp. "+formRkaPpkd32_v2.formatCurrency(jumlahHargaAlokasi)  );		
		$("#bantuSelisih").text("Rp. "+formRkaPpkd32_v2.formatCurrency(selisih)  );				
	},
					...
					data : {
				  	...
				  	success: function(data) {		
						...
						if(resp.err == ''){
							...
							$("#bantuJumlahHarga").text("Rp. "+formRkaPpkd32_v2.formatCurrency($("#jumlahHarga").val()));
						}else{
							alert(resp.err);
						}			
				  	}
	...
	bantu : function(){
		$("#bantuSatuanHarga").text("Rp. "+formRkaPpkd32_v2.formatCurrency($("#hargaSatuan").val())  );
		$("#jumlahHarga").val($("#hargaSatuan").val() * $("#volumeBarang").val());
		$("#bantuJumlahHarga").text("Rp. "+formRkaPpkd32_v2.formatCurrency($("#jumlahHarga").val()));
		$("#teralokasi").val('');
	},
	...
	hitung: function(){
		if($("#jenisAlokasi").val() == '' ){
			...
			if($("#jenisAlokasi").val() == "BULANAN"){
				...
				formRkaPpkd32_v2.hitungSelisih('bantuJan');
				formRkaPpkd32_v2.hitungSelisih('bantuFeb');
				formRkaPpkd32_v2.hitungSelisih('bantuApr');
				formRkaPpkd32_v2.hitungSelisih('bantuMei');
				formRkaPpkd32_v2.hitungSelisih('bantuJul');
				formRkaPpkd32_v2.hitungSelisih('bantuAgu');
				formRkaPpkd32_v2.hitungSelisih('bantuOkt');
				formRkaPpkd32_v2.hitungSelisih('bantuNop');
				formRkaPpkd32_v2.hitungSelisih('bantuMar');
				formRkaPpkd32_v2.hitungSelisih('bantuJun');
				formRkaPpkd32_v2.hitungSelisih('bantuSep');
				formRkaPpkd32_v2.hitungSelisih('bantuDes');
	...
	jenisAlokasiChanged : function(){
		...
		if($("#jenisAlokasi").val() == "BULANAN"){
			...
			$.ajax({
				...
				url: this.url+'&tipe=clearAlokasi',
				success: function(data) {	
					formRkaPpkd32_v2.formAlokasiTriwulan();
				}
			});	
	...
	cancelEdit : function(){
		formRkaPpkd32_v2.refreshList(true);
		setTimeout(function myFunction() {formRkaPpkd32_v2.refreshList(true);},1000);
	},
				...
				success: function(data) {	
					...
					if(resp.content.kunci == '0'){
						...
						if(resp.content.jenis_alokasi_kas == "TRIWULAN"){
							$("#tombolAlokasi").attr("onclick","formRkaPpkd32_v2.formAlokasiTriwulan();");
						}else{
							$("#tombolAlokasi").attr("onclick","formRkaPpkd32_v2.formAlokasi();");
						}
						...
						$("#btsave").attr('href','javascript:formRkaPpkd32_v2.saveEdit('+id+')');
						$("#btcancel").attr('href','javascript:formRkaPpkd32_v2.cancelEdit()');
				...
				success: function(data) {	
					...
					if(resp.err == ''){
						...
						if(resp.err == "Lengkapi Alokasi"){
							...
							formRkaPpkd32_v2.formRincianVolume();
						}else{
							alert(resp.err);
						}
