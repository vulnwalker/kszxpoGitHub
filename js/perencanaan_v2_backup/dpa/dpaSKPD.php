...
class dpaSKPD_v2Obj  extends DaftarObj2{	
	var $Prefix = 'dpaSKPD_v2';
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='dpaSKPD_v2.xls';
	var $namaModulCetak='RKA';
	var $Cetak_Judul = 'RKA';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'dpaSKPD_v2Form';
	var $modul = "DPA";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	...
	function genRowSum($ColStyle, $Mode, $Total){
		...
		$ContentTotalHal=''; $ContentTotal='';
			...
			$Kiri2 = $TotalColSpan1 > 0 ? "<td class='$ColStyle' colspan='3' align='center'><b>Total</td>": '';
				...
				"<tr>
					$Kiri2
					<td class='GarisDaftar' align='right'><b><div  id='{$this->Prefix}_cont_sum'>".number_format($Total,2,',','.')."</div>
					</td>
				</tr>" ;
	...
	$getJumlahBarang = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$dpaSKPD_v2_idplh'"));
	...
	$sisaPaguIndikatif = $paguIndikatif - $getPaguYangTelahTerpakai['paguYangTerpakai'];
	 ...
	 if(mysql_num_rows(mysql_query("select * from view_dpa where c1='0' and f1 = '0' and k = '$k' and l ='$l' and m='$m' and n='$n' and o='$o'  and id_tahap='$this->idTahap' ")) > 0){
 	 ...
 	 if(empty($cmbJenisRKAForm) ){
		...
		$query = VulnWalkerUpdate('tabel_anggaran',$data,"id_anggaran = '$dpaSKPD_v2_idplh'");
		mysql_query($query);
	 }
	...
	<A href=\"pages.php?Pg=dpa-skpd-2.2.1_v2\" title='DPA-SKPD MURNI'  > DPA-SKPD 2.2.1 </a> |
	<A href=\"pages.php?Pg=dpa-skpd-2.2_v2\" title='RKA-SKPKD MURNI' > DPA-SKPD 2.2  </a> |
	<A href=\"pages.php?Pg=dpa-skpd-2.1_v2\" title='RKA-SKPKD MURNI' > DPA-SKPD 2.1 </a> |
	<A href=\"pages.php?Pg=dpa-skpd-1_v2\" title='RKA-SKPKD MURNI' > DPA-SKPD 1 </a> |
	<A href=\"pages.php?Pg=dpa-skpd_v2\" title='RKA-SKPKD MURNI'  style='color:blue;' > DPA-SKPD  </a> |
   ...
   function setPage_OtherScript(){
		...
		return 	
			...
			<script type='text/javascript' src='js/perencanaan_v2/dpa/dpaSKPD.js' language='JavaScript' ></script> 
	 ...
	 $this->form_height = 400;
	  if ($this->form_fmST==0) {
		...
		foreach ($dt as $key => $value) { 
		 ...
		 $jenis_rka = $jenis_rka;
		 $cmbJenisRKA = cmbArray('cmbJenisRKAForm',$jenis_rka,$arrayJenisRKA,'-- JENIS RKA --','onchange=dpaSKPD_v2.unlockFindRekening();');
	 	 if(empty($jenis_rka)){
	  ...
	  $this->form_fields = array(
			...
			'kode7' => array( 
						...
						'value'=> "<input type='text' id='kodeRekening' name='kodeRekening' style='width:120px;' readonly value = '$kodeRekening'> &nbsp&nbsp
						 ...
						 <button type='button' id='findRekening' onclick=dpaSKPD_v2.findRekening('$jenis_rka'); $tergantungJenis> CARI </button> "
						 ),
			'kode8' => array( 
						'label'=>'HARGA SATUAN',
						'labelWidth'=>150, 
						'value'=> "<input type='text' name='hargaSatuan' id='hargaSatuan' value='$hargaSatuan' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='dpaSKPD_v2.bantu();' > <span id='bantu' style='color:red;'> </span>"
						 ),	
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	foreach ($_COOKIE as $key => $value) { 
		...
		$urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=dpaSKPD_v2.refreshList(true);','-- URUSAN --');
		
		$codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
		$bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=dpaSKPD_v2.refreshList(true);','-- BIDANG --');
		
		$codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
		$skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=dpaSKPD_v2.refreshList(true);','-- SKPD --');
		...
		$unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=dpaSKPD_v2.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=dpaSKPD_v2.refreshList(true);','-- SUB UNIT --');
	...
	function getDaftarOpsi($Mode=1){
	  ...
	  if(isset($cmbUrusan) && $cmbUrusan== ''){
		...
		if($this->jenisForm = "READ"){
			$getAllRekeningFromRka = mysql_query("select * from tabel_anggaran where jenis_rka !='' and jenis_rka !='RKA-SKPD'  and c1 ='0' and f ='00' and k !='0' and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and nama_modul ='RKA-SKPD' ");	
	...
	function Laporan($xls =FALSE){
		...
		echo 
			...
			"<body >
				<div style='width:$this->Cetak_WIDTH_Landscape;'>
					<table class=\"rangkacetak\" style='33cm;font-family:Times New Roman;margin-left:2cm;margin-top:2cm;'>
						<tr>
		...
		<center><h3> Rencana Pelaksanaan Anggaran <br> 
		Satuan Kerja Perangkat Daerah per triwulan</h3>
		";
	...
	echo "	
	 ...
	 <tr valign='top'>
									<td align='center' class='GarisCetak'>2.2</td>
...
$dpaSKPD_v2 = new dpaSKPD_v2Obj();

$arrayResult = VulnWalkerTahap_v2($dpaSKPD_v2->modul);
$jenisForm = $arrayResult['jenisForm'];
$nomorUrut = $arrayResult['nomorUrut'];
$tahun = $arrayResult['tahun'];
$jenisAnggaran = $arrayResult['jenisAnggaran'];
...
$dpaSKPD_v2->jenisForm = $jenisForm;
$dpaSKPD_v2->nomorUrut = $nomorUrut;
$dpaSKPD_v2->urutTerakhir = $nomorUrut;
$dpaSKPD_v2->tahun = $tahun;
$dpaSKPD_v2->jenisAnggaran = $jenisAnggaran;
$dpaSKPD_v2->idTahap = $idTahap;

$dpaSKPD_v2->username = $_COOKIE['coID'];


$dpaSKPD_v2->wajibValidasi = $Main->wajibValidasi;
if($Main->wajibValidasi == TRUE){
	$dpaSKPD_v2->sqlValidasi = " and status_validasi ='1' ";
}else{
	$dpaSKPD_v2->sqlValidasi = " ";
}


if(empty($dpaSKPD_v2->tahun)){
	...
	/*$dpaSKPD_v2->tahun = "select max(id_anggaran) as max from view_dpa where nama_modul = 'dpaSKPD_v2'";*/
	$dpaSKPD_v2->tahun  = $get2['tahun'];
	$dpaSKPD_v2->jenisAnggaran = $get2['jenis_anggaran'];
	$dpaSKPD_v2->urutTerakhir = $get2['no_urut'];
	$dpaSKPD_v2->jenisFormTerakhir = $get2['jenis_form_modul'];
	$dpaSKPD_v2->urutSebelumnya = $dpaSKPD_v2->urutTerakhir - 1;
	...
	$dpaSKPD_v2->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$dpaSKPD_v2->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];
	
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$dpaSKPD_v2->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$dpaSKPD_v2->idTahap'"));
	$dpaSKPD_v2->currentTahap = $getCurrenttahap['nama_tahap'];
	
	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$dpaSKPD_v2->idTahap'"));
	$dpaSKPD_v2->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$dpaSKPD_v2->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$dpaSKPD_v2->provinsi = $setting['provinsi'];
$dpaSKPD_v2->kota = $setting['kota'];
$dpaSKPD_v2->pengelolaBarang = $setting['pengelolaBarang'];
$dpaSKPD_v2->pejabatPengelolaBarang = $setting['pejabat'];
$dpaSKPD_v2->pengurusPengelolaBarang = $setting['pengurus'];
$dpaSKPD_v2->nipPengelola = $setting['nipPengelola'];
$dpaSKPD_v2->nipPengurus = $setting['nipPengurus'];
$dpaSKPD_v2->nipPejabat = $setting['nipPejabat'];
