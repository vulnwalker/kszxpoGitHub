var RekapBISkpd = new SkpdCls({
	prefix : 'RekapBISkpd', formName:'adminForm'
});


var RekapBI = new DaftarObj2({
	prefix : 'RekapBI',
	url : 'pages.php?Pg=RekapBI', 
	...
	exportXls2 : function(){
	    ...
	    a.href = tableToExcel('divcetak','rekapbi');
	    a.download = 'rekapbi.xls';
	...
	reqTotalbyIter:function(iterator_){
		var me=this;
		var cbxKondisi = document.getElementById('cbxKondisi').checked;	
		//me.iterator++;
		if(me.iterator < me.nodeList.length ){						
			...
			if (node.innerHTML==''){
				var node = me.nodeList[me.iterator];
				if(cbxKondisi){
					var href = node.getAttribute('href_');
				}else{
	...
	reqTotal : function(href_, idel_,iterator_){
		...
		//var href__ = href_.replace("Jurnal","Rekap1");
		var href__ = href_.replace("JurnalBI","RekapBI");
		if(href_!=''){
			...
			this.ajx = $.ajax({
				...
				data:$('#'+this.formName).serialize(), 
			  	success: function(data) {		
					var resp = eval('(' + data + ')');				
					//document.getElementById(resp.content.idel).innerHTML = resp.content.vSaldoAk;
					...
					**/
					...
					if(document.getElementById(arrid[0]+'_'+arrid[1]+'_'+1)) document.getElementById(arrid[0]+'_'+arrid[1]+'_'+1).innerHTML = resp.content.vKondBaik;//+"<input type='hidden' value='"+resp.content.vSaldoAkBrg+"'>"; //saldo akhit					
					...
					if(document.getElementById(arrid[0]+'_'+arrid[1]+'_'+2)) document.getElementById(arrid[0]+'_'+arrid[1]+'_'+2).innerHTML = resp.content.vKondRusak;
					if(document.getElementById(arrid[0]+'_'+arrid[1]+'_'+3)) document.getElementById(arrid[0]+'_'+arrid[1]+'_'+3).innerHTML = resp.content.vKondRusakBrt;
					if(document.getElementById(arrid[0]+'_'+arrid[1]+'_'+4)) document.getElementById(arrid[0]+'_'+arrid[1]+'_'+4).innerHTML = resp.content.vSaldoAkBrg;
					if(document.getElementById(arrid[0]+'_'+arrid[1]+'_'+5)) document.getElementById(arrid[0]+'_'+arrid[1]+'_'+5).innerHTML = resp.content.vSaldoAk;
