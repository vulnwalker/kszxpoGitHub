var rkbmdPersediaanSKPKDSkpd = new SkpdCls({
	prefix : 'rkbmdPersediaanSKPKDSkpd', 
	formName: 'rkbmdPersediaanSKPKDForm',
	pilihBidangAfter : function(){rkbmdPersediaanSKPKD.refreshList(true);},
	pilihUnitAfter : function(){rkbmdPersediaanSKPKD.refreshList(true);},
	pilihSubUnitAfter : function(){rkbmdPersediaanSKPKD.refreshList(true);},
	pilihSeksiAfter : function(){rkbmdPersediaanSKPKD.refreshList(true);}
});

var rkbmdPersediaanSKPKD = new DaftarObj2({
	prefix : 'rkbmdPersediaanSKPKD',
	url : 'pages.php?Pg=rkbmdPersediaanSKPKD', 
	formName : 'rkbmdPersediaanSKPKDForm',
	...
	daftarRender:function(){
		var jenisKegiatan = document.getElementById('cmbJenisrkbmdPersediaanSKPKD');
		/*cmbJenisrkbmdPersediaanSKPKD.remove(0);*/
	...
	remove:function(){
		...
		if(errmsg ==''){ 
			...
			$.ajax({
			  ...
			  success: function(data) {
					var resp = eval('(' + data + ')');
					rkbmdPersediaanSKPKD.refreshList(true);
	...
	Laporan:function(){
		...
		$.ajax({
			...
			data: { jenisKegiatan : $("#jenisKegiatan").val(),
					c1 			  : $("#rkbmdPersediaanSKPKDSkpdfmUrusan").val(),
					c 			  : $("#rkbmdPersediaanSKPKDSkpdfmSKPD").val(),
					d 			  : $("#rkbmdPersediaanSKPKDSkpdfmUNIT").val(),
					e 			  : $("#rkbmdPersediaanSKPKDSkpdfmSUBUNIT").val(),
					e1 			  : $("#rkbmdPersediaanSKPKDSkpdfmSEKSI").val(),
	...
	InputBaru: function(){
		...
		if($("#rkbmdPersediaanSKPKDSkpdfmUrusan").val() == '00'){
			errmsg = "Pilih Urusan";
		}else if($("#rkbmdPersediaanSKPKDSkpdfmSKPD").val() == '00'){
			errmsg = "Pilih Bidang";
		}else if($("#rkbmdPersediaanSKPKDSkpdfmUNIT").val() == '00'){
			errmsg = "Pilih SKPD";
		}else if($("#rkbmdPersediaanSKPKDSkpdfmSUBUNIT").val() == '00'){
			errmsg = "Pilih UNIT";
		}else if($("#rkbmdPersediaanSKPKDSkpdfmSEKSI").val() == '000'){
			errmsg = "Pilih SUB UNIT";
		}else if($("#cmbJenisrkbmdPersediaanSKPKD").val() == ''){
			errmsg = "Pilih JENIS rkbmdPersediaanSKPKD";
		}
	...
	koreksi:function(id){
		...
		$("#spanJumlah"+id).html("<input type='text' name='jumlah"+id+"' id='jumlah"+id+"' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup=rkbmdPersediaanSKPKD.bantu('Jumlah"+id+"'); style='text-align: right;'>  ");
		$.ajax({
				type:'POST',
				data :{id : id},
		...
		$("#save"+id).html("<img src='datepicker/save.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkbmdPersediaanSKPKD.submitKoreksi('"+id+"');></img> &nbsp  &nbsp <img src='images/administrator/images/invalid.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkbmdPersediaanSKPKD.cancelKoreksi('"+id+"');></img> ");
	},
	sesuai:function(id){
	...
	if(angkaKoreksi == ''){
	    ...
	    $.ajax({
				...
				data:{idAwal : id,
					  ...
					  jenisrkbmdPersediaanSKPKD  : $("#cmbJenisrkbmdPersediaanSKPKD").val()	
					  },
	...
	bantu : function(id){
		...
		$("#bantu"+id).text(rkbmdPersediaanSKPKD.formatCurrency(angka));
	},
