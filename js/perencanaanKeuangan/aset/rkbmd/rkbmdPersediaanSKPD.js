var rkbmdPersediaanSKPDSkpd = new SkpdCls({
	prefix : 'rkbmdPersediaanSKPDSkpd', 
	formName: 'rkbmdPersediaanSKPDForm',
	pilihBidangAfter : function(){rkbmdPersediaanSKPD.refreshList(true);},
	pilihUnitAfter : function(){rkbmdPersediaanSKPD.refreshList(true);},
	pilihSubUnitAfter : function(){rkbmdPersediaanSKPD.refreshList(true);},
	pilihSeksiAfter : function(){rkbmdPersediaanSKPD.refreshList(true);}
});

var rkbmdPersediaanSKPD = new DaftarObj2({
	prefix : 'rkbmdPersediaanSKPD',
	url : 'pages.php?Pg=rkbmdPersediaanSKPD', 
	formName : 'rkbmdPersediaanSKPDForm',
	...
	daftarRender:function(){
		var jenisKegiatan = document.getElementById('cmbJenisrkbmdPersediaanSKPD');
		/*cmbJenisrkbmdPersediaanSKPD.remove(0);*/
	...
	remove:function(){
		...
		if(errmsg ==''){ 
			...
			$.ajax({
			  ...
			  success: function(data) {
					var resp = eval('(' + data + ')');
					rkbmdPersediaanSKPD.refreshList(true);
	...
	Laporan:function(){
		...
		$.ajax({
			...
			data: { jenisKegiatan : $("#jenisKegiatan").val(),
					c1 			  : $("#rkbmdPersediaanSKPDSkpdfmUrusan").val(),
					c 			  : $("#rkbmdPersediaanSKPDSkpdfmSKPD").val(),
					d 			  : $("#rkbmdPersediaanSKPDSkpdfmUNIT").val(),
					e 			  : $("#rkbmdPersediaanSKPDSkpdfmSUBUNIT").val(),
					e1 			  : $("#rkbmdPersediaanSKPDSkpdfmSEKSI").val(),
	...
	InputBaru: function(){
		...
		if($("#rkbmdPersediaanSKPDSkpdfmUrusan").val() == '00'){
			errmsg = "Pilih Urusan";
		}else if($("#rkbmdPersediaanSKPDSkpdfmSKPD").val() == '00'){
			errmsg = "Pilih Bidang";
		}else if($("#rkbmdPersediaanSKPDSkpdfmUNIT").val() == '00'){
			errmsg = "Pilih SKPD";
		}else if($("#rkbmdPersediaanSKPDSkpdfmSUBUNIT").val() == '00'){
			errmsg = "Pilih UNIT";
		}else if($("#rkbmdPersediaanSKPDSkpdfmSEKSI").val() == '000'){
			errmsg = "Pilih SUB UNIT";
		}else if($("#cmbJenisrkbmdPersediaanSKPD").val() == ''){
			errmsg = "Pilih JENIS rkbmdPersediaanSKPD";
		}
	...
	koreksi:function(id){
		...
		$("#spanJumlah"+id).html("<input type='text' name='jumlah"+id+"' id='jumlah"+id+"' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup=rkbmdPersediaanSKPD.bantu('Jumlah"+id+"'); style='text-align: right;'>  ");
		$.ajax({
				type:'POST',
				data :{id : id},
		...
		$("#save"+id).html("<img src='datepicker/save.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkbmdPersediaanSKPD.submitKoreksi('"+id+"');></img> &nbsp  &nbsp <img src='images/administrator/images/invalid.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkbmdPersediaanSKPD.cancelKoreksi('"+id+"');></img> ");
	},
	sesuai:function(id){
	...
	if(angkaKoreksi == ''){
	    ...
	    $.ajax({
				...
				data:{idAwal : id,
					  ...
					  jenisrkbmdPersediaanSKPD  : $("#cmbJenisrkbmdPersediaanSKPD").val()	
					  },
	...
	bantu : function(id){
		...
		$("#bantu"+id).text(rkbmdPersediaanSKPD.formatCurrency(angka));
	},
