var rkbmdPengadaanSKPDSkpd = new SkpdCls({
	prefix : 'rkbmdPengadaanSKPDSkpd', 
	formName: 'rkbmdPengadaanSKPDForm',
	pilihBidangAfter : function(){rkbmdPengadaanSKPD.refreshList(true);},
	pilihUnitAfter : function(){rkbmdPengadaanSKPD.refreshList(true);},
	pilihSubUnitAfter : function(){rkbmdPengadaanSKPD.refreshList(true);},
	pilihSeksiAfter : function(){rkbmdPengadaanSKPD.refreshList(true);}
});

var rkbmdPengadaanSKPD = new DaftarObj2({
	prefix : 'rkbmdPengadaanSKPD',
	url : 'pages.php?Pg=rkbmdPengadaanSKPD', 
	formName : 'rkbmdPengadaanSKPDForm',
	...
	daftarRender:function(){
		var jenisKegiatan = document.getElementById('cmbJenisrkbmdPengadaanSKPD');
		/*cmbJenisrkbmdPengadaanSKPD.remove(0);*/
	...
	remove:function(){
		...
		if(errmsg ==''){ 
			...
			$.ajax({
			  ...
			  success: function(data) {
					var resp = eval('(' + data + ')');
					rkbmdPengadaanSKPD.refreshList(true);
	...
	Laporan:function(){
		...
		$.ajax({
			...
			data: { jenisKegiatan : $("#jenisKegiatan").val(),
					c1 			  : $("#rkbmdPengadaanSKPDSkpdfmUrusan").val(),
					c 			  : $("#rkbmdPengadaanSKPDSkpdfmSKPD").val(),
					d 			  : $("#rkbmdPengadaanSKPDSkpdfmUNIT").val(),
					e 			  : $("#rkbmdPengadaanSKPDSkpdfmSUBUNIT").val(),
					e1 			  : $("#rkbmdPengadaanSKPDSkpdfmSEKSI").val(),
	...
	InputBaru: function(){
		...
		if($("#rkbmdPengadaanSKPDSkpdfmUrusan").val() == '00'){
			errmsg = "Pilih Urusan";
		}else if($("#rkbmdPengadaanSKPDSkpdfmSKPD").val() == '00'){
			errmsg = "Pilih Bidang";
		}else if($("#rkbmdPengadaanSKPDSkpdfmUNIT").val() == '00'){
			errmsg = "Pilih SKPD";
		}else if($("#rkbmdPengadaanSKPDSkpdfmSUBUNIT").val() == '00'){
			errmsg = "Pilih UNIT";
		}else if($("#rkbmdPengadaanSKPDSkpdfmSEKSI").val() == '000'){
			errmsg = "Pilih SUB UNIT";
		}else if($("#cmbJenisrkbmdPengadaanSKPD").val() == ''){
			errmsg = "Pilih JENIS rkbmdPengadaanSKPD";
		}
	...
	koreksi:function(id){
		...
		$("#spanJumlah"+id).html("<input type='text' name='jumlah"+id+"' id='jumlah"+id+"' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup=rkbmdPengadaanSKPD.bantu('Jumlah"+id+"'); style='text-align: right;'>  ");
		$.ajax({
				type:'POST',
				data :{id : id},
		...
		$("#save"+id).html("<img src='datepicker/save.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkbmdPengadaanSKPD.submitKoreksi('"+id+"');></img> &nbsp  &nbsp <img src='images/administrator/images/invalid.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkbmdPengadaanSKPD.cancelKoreksi('"+id+"');></img> ");
	},
	sesuai:function(id){
	...
	if(angkaKoreksi == ''){
	    ...
	    $.ajax({
				...
				data:{idAwal : id,
					  ...
					  jenisrkbmdPengadaanSKPD  : $("#cmbJenisrkbmdPengadaanSKPD").val()	
					  },
	...
	bantu : function(id){
		...
		$("#bantu"+id).text(rkbmdPengadaanSKPD.formatCurrency(angka));
	},
