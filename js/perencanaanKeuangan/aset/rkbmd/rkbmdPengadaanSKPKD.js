var rkbmdPengadaanSKPKDSkpd = new SkpdCls({
	prefix : 'rkbmdPengadaanSKPKDSkpd', 
	formName: 'rkbmdPengadaanSKPKDForm',
	pilihBidangAfter : function(){rkbmdPengadaanSKPKD.refreshList(true);},
	pilihUnitAfter : function(){rkbmdPengadaanSKPKD.refreshList(true);},
	pilihSubUnitAfter : function(){rkbmdPengadaanSKPKD.refreshList(true);},
	pilihSeksiAfter : function(){rkbmdPengadaanSKPKD.refreshList(true);}
});

var rkbmdPengadaanSKPKD = new DaftarObj2({
	prefix : 'rkbmdPengadaanSKPKD',
	url : 'pages.php?Pg=rkbmdPengadaanSKPKD', 
	formName : 'rkbmdPengadaanSKPKDForm',
	...
	daftarRender:function(){
		var jenisKegiatan = document.getElementById('cmbJenisrkbmdPengadaanSKPKD');
		/*cmbJenisrkbmdPengadaanSKPKD.remove(0);*/
	...
	remove:function(){
		...
		if(errmsg ==''){ 
			...
			$.ajax({
			  ...
			  success: function(data) {
					var resp = eval('(' + data + ')');
					rkbmdPengadaanSKPKD.refreshList(true);
	...
	Laporan:function(){
		...
		$.ajax({
			...
			data: { jenisKegiatan : $("#jenisKegiatan").val(),
					c1 			  : $("#rkbmdPengadaanSKPKDSkpdfmUrusan").val(),
					c 			  : $("#rkbmdPengadaanSKPKDSkpdfmSKPD").val(),
					d 			  : $("#rkbmdPengadaanSKPKDSkpdfmUNIT").val(),
					e 			  : $("#rkbmdPengadaanSKPKDSkpdfmSUBUNIT").val(),
					e1 			  : $("#rkbmdPengadaanSKPKDSkpdfmSEKSI").val(),
	...
	InputBaru: function(){
		...
		if($("#rkbmdPengadaanSKPKDSkpdfmUrusan").val() == '00'){
			errmsg = "Pilih Urusan";
		}else if($("#rkbmdPengadaanSKPKDSkpdfmSKPD").val() == '00'){
			errmsg = "Pilih Bidang";
		}else if($("#rkbmdPengadaanSKPKDSkpdfmUNIT").val() == '00'){
			errmsg = "Pilih SKPD";
		}else if($("#rkbmdPengadaanSKPKDSkpdfmSUBUNIT").val() == '00'){
			errmsg = "Pilih UNIT";
		}else if($("#rkbmdPengadaanSKPKDSkpdfmSEKSI").val() == '000'){
			errmsg = "Pilih SUB UNIT";
		}else if($("#cmbJenisrkbmdPengadaanSKPKD").val() == ''){
			errmsg = "Pilih JENIS rkbmdPengadaanSKPKD";
		}
	...
	koreksi:function(id){
		...
		$("#spanJumlah"+id).html("<input type='text' name='jumlah"+id+"' id='jumlah"+id+"' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup=rkbmdPengadaanSKPKD.bantu('Jumlah"+id+"'); style='text-align: right;'>  ");
		$.ajax({
				type:'POST',
				data :{id : id},
		...
		$("#save"+id).html("<img src='datepicker/save.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkbmdPengadaanSKPKD.submitKoreksi('"+id+"');></img> &nbsp  &nbsp <img src='images/administrator/images/invalid.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkbmdPengadaanSKPKD.cancelKoreksi('"+id+"');></img> ");
	},
	sesuai:function(id){
	...
	if(angkaKoreksi == ''){
	    ...
	    $.ajax({
				...
				data:{idAwal : id,
					  ...
					  jenisrkbmdPengadaanSKPKD  : $("#cmbJenisrkbmdPengadaanSKPKD").val()	
					  },
	...
	bantu : function(id){
		...
		$("#bantu"+id).text(rkbmdPengadaanSKPKD.formatCurrency(angka));
	},
