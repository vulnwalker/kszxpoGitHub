var renjaAsetSKPDPerubahan = new DaftarObj2({
	prefix : 'renjaAsetSKPDPerubahan',
	url : 'pages.php?Pg=renjaAsetSKPDPerubahan', 
	formName : 'renjaAsetSKPDPerubahanForm',
	renjaAsetSKPDPerubahan_form : '0',
	loading: function(){
		this.topBarRender();
		this.filterRender();
		this.daftarRender();
		...
		if(errmsg ==''){
			...
			$.ajax({
			  ...
			  success: function(data) {
					var resp = eval('(' + data + ')');
					if(resp.err == "") {
						var IDrenjaAsetSKPDPerubahan = Number(resp.content.idrenjaAsetSKPDPerubahan);
						aForm.action= 'pages.php?Pg=renjaAsetSKPDPerubahan_ins&YN=1&id='+IDrenjaAsetSKPDPerubahan;
						aForm.target='_blank';
						aForm.submit();	
						aForm.target='';
					}else{
	...
	BidangAfter2: function(){
		renjaAsetSKPDPerubahan.refreshList(true);
	},
	...
	Baru: function(){	
		...
		if (err =='' ){		
			...
			if(me.renjaAsetSKPDPerubahan_form==0){//baru dari renjaAsetSKPDPerubahan
	...
	sesuai:function(id){
		...
		/*$("#span"+id).html("<input type='text' name='"+id+"' id='"+id+"' onkeyup='return renjaAsetSKPDPerubahan.submitKoreksi(event,id)'  value='"+$("#valueJumlah"+id).val()+"'  style='text-align: right;'>  <span id='bantu"+id+"' >"+popupBarang.formatCurrency($("#valueJumlah"+id).val())+"</span>");*/

	},
	koreksi:function(id){
		$("#span"+id).html("<input type='text' name='"+id+"' id='"+id+"' onkeyup='return renjaAsetSKPDPerubahan.submitKoreksi(event,id)' onkeypress='return event.charCode >= 48 && event.charCode <= 57'  style='text-align: right;'>  <span id='bantu"+id+"' ></span>");
	},
   ...
   SaveCatatan: function(id){
		...
		$.ajax({
			...
			url: this.url+'&tipe=SaveCatatan',
		  	success: function(data) {		
				...
				delElem(cover);		
				if(resp.err==''){
					if(me.renjaAsetSKPDPerubahan_form==0){
						me.Close();
						me.AfterSimpan();						
					}
	...
	,	
	detailrenjaAsetSKPDPerubahan: function(kode){	
		...
		var me = this;
			...
			$.ajax({
				type:'POST', 
				data: $('#'+this.formName).serialize()+"&renjaAsetSKPDPerubahan_cb%5B%5D=1.08.01.01.01" ,
				url: this.url+'&tipe=lihatrenjaAsetSKPDPerubahan',
			  	success: function(data) {		
					var resp = eval('(' + data + ')');	
					if (resp.err ==''){		
						document.getElementById(cover).innerHTML = resp.content;
			...
			$.ajax({
			  ...
			  success: function(data) {
					...
					if(resp.err ==''){
						...
						aForm.action= 'pages.php?Pg=renjaAsetSKPDPerubahan_ins&id='+ID_PLAFON+"&ID_RENJA="+ID_EDIT;
						aForm.target='_blank';
						aForm.submit();	
						aForm.target='';
					}else{
		  	...
		  	success: function(data) {		
				...
				if(resp.err==''){
					if(me.renjaAsetSKPDPerubahan_form==0){
						...
						me.Close();
						me.refreshComborenjaAsetSKPDPerubahan();
					}
			...
			$.ajax({
			  ...
			  success: function(data) {
					...
					if(resp.err !=''){
						...
						aForm.action= 'pages.php?Pg=renjaAsetSKPDPerubahan&tipe=Laporan';
						aForm.target='_blank';
						aForm.submit();	
						aForm.target='';
					}
