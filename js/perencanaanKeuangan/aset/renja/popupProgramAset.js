var popupProgramRenjaAset = new DaftarObj2({
	prefix : 'popupProgramRenjaAset',
	url : 'pages.php?Pg=popupProgramRenjaAset',
	formName : 'popupProgramRenjaAsetForm',
	kodeProgram : 'p',
	namaProgram : 'program',
	filterAkun : '',
	...
	windowSave: function(id){
		...
		//alert('save');
			...
			var cover = 'popupProgramRenjaAset_getdata';
			addCoverPage2(cover,1,true,false);
			$.ajax({
				url: 'pages.php?Pg=popupProgramRenjaAset&tipe=getdata&id='+id,
			  	success: function(data) {
					var resp = eval('(' + data + ')');
					delElem(cover);
					if(resp.err==''){
	...
	formatCurrency:function(num) {
		...
		var sisanya = $("#plafon").val() - $("#paguIndikatif").val() - $("#sisaPlafonDariDB").val();
			$("#sisaPlafon").val("Rp. " + popupProgramRenjaAset.uang(sisanya));
		}else{
