var renjaAsetSKPD = new DaftarObj2({
	prefix : 'renjaAsetSKPD',
	url : 'pages.php?Pg=renjaAsetSKPD', 
	formName : 'renjaAsetSKPDForm',
	renjaAsetSKPD_form : '0',
	loading: function(){
		this.topBarRender();
		this.filterRender();
		this.daftarRender();
		...
		if(errmsg ==''){
			...
			$.ajax({
			  ...
			  success: function(data) {
					var resp = eval('(' + data + ')');
					if(resp.err == "") {
						var IDrenjaAsetSKPD = Number(resp.content.idrenjaAsetSKPD);
						aForm.action= 'pages.php?Pg=renjaAsetSKPD_ins&YN=1&id='+IDrenjaAsetSKPD;
						aForm.target='_blank';
						aForm.submit();	
						aForm.target='';
					}else{
	...
	BidangAfter2: function(){
		renjaAsetSKPD.refreshList(true);
	},
	...
	Baru: function(){	
		...
		if (err =='' ){		
			...
			if(me.renjaAsetSKPD_form==0){//baru dari renjaAsetSKPD
	...
	sesuai:function(id){
		...
		/*$("#span"+id).html("<input type='text' name='"+id+"' id='"+id+"' onkeyup='return renjaAsetSKPD.submitKoreksi(event,id)'  value='"+$("#valueJumlah"+id).val()+"'  style='text-align: right;'>  <span id='bantu"+id+"' >"+popupBarang.formatCurrency($("#valueJumlah"+id).val())+"</span>");*/

	},
	koreksi:function(id){
		$("#span"+id).html("<input type='text' name='"+id+"' id='"+id+"' onkeyup='return renjaAsetSKPD.submitKoreksi(event,id)' onkeypress='return event.charCode >= 48 && event.charCode <= 57'  style='text-align: right;'>  <span id='bantu"+id+"' ></span>");
	},
   ...
   SaveCatatan: function(id){
	...
	,	
	detailrenjaAsetSKPD: function(kode){	
		...
		var me = this;
			...
			$.ajax({
				type:'POST', 
				data: $('#'+this.formName).serialize()+"&renjaAsetSKPD_cb%5B%5D=1.08.01.01.01" ,
				url: this.url+'&tipe=lihatrenjaAsetSKPD',
			  	success: function(data) {		
					var resp = eval('(' + data + ')');	
					if (resp.err ==''){		
						document.getElementById(cover).innerHTML = resp.content;
			...
			$.ajax({
			  ...
			  success: function(data) {
					...
					if(resp.err ==''){
						...
						aForm.action= 'pages.php?Pg=renjaAsetSKPD_ins&id='+ID_PLAFON+"&ID_RENJA="+ID_EDIT;
						aForm.target='_blank';
						aForm.submit();	
						aForm.target='';
					}else{
			...
			$.ajax({
			  ...
			  success: function(data) {
					...
					if(resp.err !=''){
						...
						aForm.action= 'pages.php?Pg=renjaAsetSKPD&tipe=Laporan';
						aForm.target='_blank';
						aForm.submit();	
						aForm.target='';
					}
