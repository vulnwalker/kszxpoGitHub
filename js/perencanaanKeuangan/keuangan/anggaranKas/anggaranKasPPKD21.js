var anggaranKasPPKD21 = new DaftarObj2({
	prefix : 'anggaranKasPPKD21',
	url : 'pages.php?Pg=anggaranKasPPKD21',
	formName : 'anggaranKasPPKD21Form',
	anggaranKasPPKD21_form : '0',//default js anggaranKasPPKD21
	loading: function(){
		//alert('loading');
		this.topBarRender();
		this.filterRender();
...
/*		this.sumHalRender();*/
	...
	programChanged : function(){
		...
		$("#hiddenP").val(hiddenP);
		$("#q").val('');
		anggaranKasPPKD21.refreshList(true);
	},
	detail: function(){
		//alert('detail');
		var me = this;
...
/*				me.sumHalRender();*/
	...
	unlockFindRekening: function(){
		...
		if( $("#cmbJenisRKAForm").val() == ""){
			...
			$("#findRekening").attr('onclick',"anggaranKasPPKD21.findRekening('"+$("#cmbJenisRKAForm").val()+"');");
	...
	koreksi:function(id){
		$("#spanVolumeBarang"+id).html("<input type='text' name='volumeBarang"+id+"' id='volumeBarang"+id+"' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='anggaranKasPPKD21.bantuVolumeBarang("+id+")'   style='text-align: right;'>  <span id='bantuVolumeBarang"+id+"' ></span>");
		$("#spanSatuanHarga"+id).html("<input type='text' name='satuanHarga"+id+"' id='satuanHarga"+id+"' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='anggaranKasPPKD21.bantuSatuanHarga("+id+")'    style='text-align: right;'>  <span id='bantuSatuanHarga"+id+"' ></span>");
		$("#buttonSubmitKoreksi"+id).html("<img src='datepicker/save.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=anggaranKasPPKD21.submitKoreksi('"+id+"');></img>&nbsp  &nbsp <img src='images/administrator/images/invalid.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=anggaranKasPPKD21.refreshList(true);></img>");
	...
	bantuVolumeBarang : function(id){
		document.getElementById('bantuVolumeBarang'+id).innerHTML = anggaranKasPPKD21.formatCurrency2($("#volumeBarang"+id).val());
		var hasilKali = Number($("#volumeBarang"+id).val()) * Number($("#satuanHarga"+id).val()) ;
		$("#spanJumlahHarga"+id).text(anggaranKasPPKD21.formatCurrency(hasilKali));
	},
	bantuSatuanHarga : function(id){
		document.getElementById('bantuSatuanHarga'+id).innerHTML =  anggaranKasPPKD21.formatCurrency($("#satuanHarga"+id).val());
		var hasilKali = Number($("#volumeBarang"+id).val()) * Number($("#satuanHarga"+id).val()) ;
		$("#spanJumlahHarga"+id).text(anggaranKasPPKD21.formatCurrency(hasilKali));
	},
	...
	bantu : function(){
		$("#bantu").text("Rp. "+anggaranKasPPKD21.formatCurrency($("#hargaSatuan").val())  );
		...
		$.ajax({
			...
			url: this.url+'&tipe=simpan',
		  	success: function(data) {
				...
				//document.getElementById(cover).innerHTML = resp.content;
				if(resp.err==''){
					if(me.anggaranKasPPKD21_form==0){
						me.Close();
						me.AfterSimpan();
					}else{
						me.Close();
						barang.refreshComboanggaranKasPPKD21();
					}
	...
	jenisAlokasiChanged : function(){
		...
		if($("#jenisAlokasi").val() == "BULANAN"){
			...
			$.ajax({
				...
				url: this.url+'&tipe=clearAlokasi',
				success: function(data) {
					anggaranKasPPKD21.formAlokasiTriwulan();
				}
			});
