var rkaPPKD32Koreksi = new DaftarObj2({
	prefix : 'rkaPPKD32Koreksi',
	url : 'pages.php?Pg=rkaPPKD32Koreksi', 
	formName : 'rkaPPKD32KoreksiForm',
	rkaPPKD32Koreksi_form : '0',//default js rkaPPKD32Koreksi
	loading: function(){
		//alert('loading');
		this.topBarRender();
		this.filterRender();
...
/*		this.sumHalRender();*/ 
	...
	programChanged : function(){
		...
		$("#hiddenP").val(hiddenP);
		$("#q").val('');
		rkaPPKD32Koreksi.refreshList(true);	
	},
	detail: function(){
		//alert('detail');
		var me = this;
...
/*				me.sumHalRender();*/
			...
			alert("Pilih SKPD");
				...
				data : {c1 : $("#cmbUrusan").val(),
			  	...
			  	success: function(data) {		
					var resp = eval('(' + data + ')');
					if(resp.err == ''){
						aForm.action= 'pages.php?Pg=formrkaPPKD32Koreksi&skpd='+skpd;
						aForm.target='_blank';
						aForm.submit();	
						aForm.target='';
					}else{
	...
	unlockFindRekening: function(){
		...
		if( $("#cmbJenisRKAForm").val() == ""){
			...
			$("#findRekening").attr('onclick',"rkaPPKD32Koreksi.findRekening('"+$("#cmbJenisRKAForm").val()+"');");
				...
				$.ajax({
				  ...
				  success: function(data) {
						...
						if(resp.err == "") {
							...
							aForm.action= 'pages.php?Pg=formrkaPPKD32Koreksi&rek='+kodeRek+'&skpd='+skpd;
							aForm.target='_blank';
							aForm.submit();	
							aForm.target='';
						}else{
	...
	koreksi:function(id){
		$("#spanVolumeBarang"+id).html("<input type='text' name='volumeBarang"+id+"' id='volumeBarang"+id+"' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='rkaPPKD32Koreksi.bantuVolumeBarang("+id+")'   style='text-align: right;'>  <span id='bantuVolumeBarang"+id+"' ></span>");
		$("#spanSatuanHarga"+id).html("<input type='text' name='satuanHarga"+id+"' id='satuanHarga"+id+"' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='rkaPPKD32Koreksi.bantuSatuanHarga("+id+")'    style='text-align: right;'>  <span id='bantuSatuanHarga"+id+"' ></span>");
		$("#buttonSubmitKoreksi"+id).html("<img src='datepicker/save.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkaPPKD32Koreksi.submitKoreksi('"+id+"');></img>&nbsp  &nbsp <img src='images/administrator/images/invalid.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkaPPKD32Koreksi.refreshList(true);></img>");
	...
	bantuVolumeBarang : function(id){
		document.getElementById('bantuVolumeBarang'+id).innerHTML = rkaPPKD32Koreksi.formatCurrency2($("#volumeBarang"+id).val());
		var hasilKali = Number($("#volumeBarang"+id).val()) * Number($("#satuanHarga"+id).val()) ; 
		$("#spanJumlahHarga"+id).text(rkaPPKD32Koreksi.formatCurrency(hasilKali));
	},
	bantuSatuanHarga : function(id){
		document.getElementById('bantuSatuanHarga'+id).innerHTML =  rkaPPKD32Koreksi.formatCurrency($("#satuanHarga"+id).val());
		var hasilKali = Number($("#volumeBarang"+id).val()) * Number($("#satuanHarga"+id).val()) ; 
		$("#spanJumlahHarga"+id).text(rkaPPKD32Koreksi.formatCurrency(hasilKali));
	},
	...
	bantu : function(){
		$("#bantu").text("Rp. "+rkaPPKD32Koreksi.formatCurrency($("#hargaSatuan").val())  );
		...
		$.ajax({
			...
			url: this.url+'&tipe=simpan',
		  	success: function(data) {		
				...
				//document.getElementById(cover).innerHTML = resp.content;
				if(resp.err==''){
					if(me.rkaPPKD32Koreksi_form==0){
						me.Close();
						me.AfterSimpan();						
					}else{
						me.Close();
						barang.refreshComborkaPPKD32Koreksi();
					}
   ...
   SaveCatatan: function(id){
	...
	hitungSelisih:function(namaBantu){
		...
		$("#"+namaBantu).text("Rp. "+rkaPPKD32Koreksi.formatCurrency($("#"+idPost).val())  );
		...
		$("#selisih").val(selisih);	
		$("#bantuPenjumlahan").text("Rp. "+rkaPPKD32Koreksi.formatCurrency(jumlahHargaAlokasi)  );		
		$("#bantuSelisih").text("Rp. "+rkaPPKD32Koreksi.formatCurrency(selisih)  );				
	},
	...
	jenisAlokasiChanged : function(id){
		...
		if($("#jenisAlokasi").val() == "BULANAN"){
			...
			$.ajax({
				...
				success: function(data) {	
					rkaPPKD32Koreksi.formAlokasi(id);
				}
			});	
			...
			$.ajax({
				...
				url: this.url+'&tipe=clearAlokasi',
				success: function(data) {	
					rkaPPKD32Koreksi.formAlokasiTriwulan(id);
				}
			});	
	...
	hitung: function(){
		if($("#jenisAlokasi").val() == '' ){
			...
			if($("#jenisAlokasi").val() == "BULANAN"){
				...
				rkaPPKD32Koreksi.hitungSelisih('bantuJan');
				rkaPPKD32Koreksi.hitungSelisih('bantuFeb');
				rkaPPKD32Koreksi.hitungSelisih('bantuApr');
				rkaPPKD32Koreksi.hitungSelisih('bantuMei');
				rkaPPKD32Koreksi.hitungSelisih('bantuJul');
				rkaPPKD32Koreksi.hitungSelisih('bantuAgu');
				rkaPPKD32Koreksi.hitungSelisih('bantuOkt');
				rkaPPKD32Koreksi.hitungSelisih('bantuNop');
				rkaPPKD32Koreksi.hitungSelisih('bantuMar');
				rkaPPKD32Koreksi.hitungSelisih('bantuJun');
				rkaPPKD32Koreksi.hitungSelisih('bantuSep');
				rkaPPKD32Koreksi.hitungSelisih('bantuDes');
			}else if($("#jenisAlokasi").val() == "TRIWULAN"){
				var jumlahHargaForm = $("#jumlahHargaForm").val();
				$("#jan").val('');
				$("#feb").val('');
