var rkaSKPD1_insSKPD = new SkpdCls({
	prefix : 'rkaSKPD1_insSKPD', formName:'rkaSKPD1_insForm', kolomWidth:120,
...
var rkaSKPD1_ins = new DaftarObj2({
	prefix : 'rkaSKPD1_ins',
	url : 'pages.php?Pg=rkaSKPD1_ins', 
	formName : 'rkaSKPD1_insForm',
	...
	hitungSelisih:function(namaBantu){
		...
		$("#"+namaBantu).text("Rp. "+rkaSKPD1_ins.formatCurrency($("#"+idPost).val())  );
		...
		$("#selisih").val(selisih);	
		$("#bantuPenjumlahan").text("Rp. "+rkaSKPD1_ins.formatCurrency(jumlahHargaAlokasi)  );		
		$("#bantuSelisih").text("Rp. "+rkaSKPD1_ins.formatCurrency(selisih)  );				
	},
					...
					data : {
				  	...
				  	success: function(data) {		
						...
						if(resp.err == ''){
							...
							$("#bantuJumlahHarga").text("Rp. "+rkaSKPD1_ins.formatCurrency($("#jumlahHarga").val()));
						}else{
							alert(resp.err);
						}			
				  	}
	...
	bantu : function(){
		$("#bantuSatuanHarga").text("Rp. "+rkaSKPD1_ins.formatCurrency($("#hargaSatuan").val())  );
		$("#jumlahHarga").val($("#hargaSatuan").val() * $("#volumeBarang").val());
		$("#bantuJumlahHarga").text("Rp. "+rkaSKPD1_ins.formatCurrency($("#jumlahHarga").val()));
		$("#teralokasi").val('');
	},
	...
	hitung: function(){
		if($("#jenisAlokasi").val() == '' ){
			...
			if($("#jenisAlokasi").val() == "BULANAN"){
				...
				rkaSKPD1_ins.hitungSelisih('bantuJan');
				rkaSKPD1_ins.hitungSelisih('bantuFeb');
				rkaSKPD1_ins.hitungSelisih('bantuApr');
				rkaSKPD1_ins.hitungSelisih('bantuMei');
				rkaSKPD1_ins.hitungSelisih('bantuJul');
				rkaSKPD1_ins.hitungSelisih('bantuAgu');
				rkaSKPD1_ins.hitungSelisih('bantuOkt');
				rkaSKPD1_ins.hitungSelisih('bantuNop');
				rkaSKPD1_ins.hitungSelisih('bantuMar');
				rkaSKPD1_ins.hitungSelisih('bantuJun');
				rkaSKPD1_ins.hitungSelisih('bantuSep');
				rkaSKPD1_ins.hitungSelisih('bantuDes');
	...
	jenisAlokasiChanged : function(){
		...
		if($("#jenisAlokasi").val() == "BULANAN"){
			...
			$.ajax({
				...
				url: this.url+'&tipe=clearAlokasi',
				success: function(data) {	
					rkaSKPD1_ins.formAlokasiTriwulan();
				}
			});	
	...
	cancelEdit : function(){
		rkaSKPD1_ins.refreshList(true);
		setTimeout(function myFunction() {rkaSKPD1_ins.refreshList(true);},1000);
	},
				...
				success: function(data) {	
					...
					if(resp.content.kunci == '0'){
						...
						if(resp.content.jenis_alokasi_kas == "TRIWULAN"){
							$("#tombolAlokasi").attr("onclick","rkaSKPD1_ins.formAlokasiTriwulan();");
						}else{
							$("#tombolAlokasi").attr("onclick","rkaSKPD1_ins.formAlokasi();");
						}
						...
						$("#btsave").attr('href','javascript:rkaSKPD1_ins.saveEdit('+id+')');
						$("#btcancel").attr('href','javascript:rkaSKPD1_ins.cancelEdit()');
				...
				success: function(data) {	
					...
					if(resp.err == ''){
						...
						if(resp.err == "Lengkapi Alokasi"){
							...
							rkaSKPD1_ins.formRincianVolume();
						}else{
							alert(resp.err);
						}
