...
class rkaSKPD1PADObj  extends DaftarObj2{
	var $Prefix = 'rkaSKPD1PAD';
	...
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='rkaSKPD1PAD.xls';
	var $namaModulCetak='RKA';
	var $Cetak_Judul = 'RKA';
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'rkaSKPD1PADForm';
	var $modul = "RKA-SKPD";
	var $jenisForm = "";
	var $tahun = "";
	var $nomorUrut = "";
	...
	$getJumlahBarang = mysql_fetch_array(mysql_query("select * from tabel_anggaran where id_anggaran = '$rkaSKPD1PAD_idplh'"));
	...
	$getPaguYangTelahTerpakai = mysql_fetch_array(mysql_query("select sum(jumlah_harga) as paguYangTerpakai from view_rka_1 where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1'  and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'  and no_urut = '$this->nomorUrut' and id_anggaran!='$rkaSKPD1PAD_idplh' "));
	$sisaPaguIndikatif = $paguIndikatif - $getPaguYangTelahTerpakai['paguYangTerpakai'];
	 ...
	 if(mysql_num_rows(mysql_query("select * from view_rka_1 where c1='0' and f = '00' and k = '$k' and l ='$l' and m='$m' and n='$n' and o='$o'  and id_tahap='$this->idTahap' ")) > 0){
 	 ...
 	 if(empty($cmbJenisRKAForm) ){
		...
		$query = VulnWalkerUpdate('tabel_anggaran',$data,"id_anggaran = '$rkaSKPD1PAD_idplh'");
		mysql_query($query);
	 }
	...
	function set_selector_other($tipe){
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){
			...
			case 'editSubRincian':{
					...
					$eventKeyup = 'document.getElementById("formatjumlah'.$id.'").innerHTML = rkaSKPD1PAD.formatCurrency(this.value); ';
		...
		case 'Gruping':{
			...
			if(!isset($rkaSKPD1PAD_cb)){
				$err = "Pilih Data";
			}
			$dt = implode(',',$rkaSKPD1PAD_cb);
		...
		case 'sesuai':{
			...
			$getrkaSKPD1PADnya = mysql_fetch_array(mysql_query($queryRows));
			foreach ($getrkaSKPD1PADnya as $key => $value) {
				  $$key = $value;
			}
			$cmbUrusanForm = $c1;
			$cmbBidangForm = $c;
	 ...
	 <A href=\"pages.php?Pg=rkaSKPD1PAD\" title='PENDAPATAN'  style='color:blue;'> PENDAPATAN </a> |
	 ...
	 <tr><td class=\"menudottedline\" width=\"40%\" height=\"20\" style='text-align:right'><B>
	 <A href=\"pages.php?Pg=rkaSKPD1PAD\" title='PENDAPATAN ASLI DAERAH' style='color:blue;' > PENDAPATAN ASLI DAERAH </a> |
	 ...
	 $this->form_height = 400;
	  if ($this->form_fmST==0) {
		...
		foreach ($dt as $key => $value) {
		 ...
		 $namaRekening = $getNamaRekening['nm_rekening'];
		 $arrayJenisRKA = array(
						array("1 PAD","RKA-SKPD 1 PAD"),
						array("1 PAD","RKA-SKPD 1 PAD")

						);
		 $jenis_rka = $jenis_rka;
		 $cmbJenisRKA = cmbArray('cmbJenisRKAForm',$jenis_rka,$arrayJenisRKA,'-- JENIS RKA --','onchange=rkaSKPD1PAD.unlockFindRekening();');
	 	 if(empty($jenis_rka)){
	  ...
	  $this->form_fields = array(
			...
			'kode7' => array(
						...
						'value'=> "<input type='text' id='kodeRekening' name='kodeRekening' style='width:120px;' readonly value = '$kodeRekening'> &nbsp&nbsp
						 ...
						 <button type='button' id='findRekening' onclick=rkaSKPD1PAD.findRekening('$jenis_rka'); $tergantungJenis> CARI </button> "
						 ),
			'kode8' => array(
						'label'=>'HARGA SATUAN',
						'labelWidth'=>150,
						'value'=> "<input type='text' name='hargaSatuan' id='hargaSatuan' value='$hargaSatuan' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='rkaSKPD1PAD.bantu();' > <span id='bantu' style='color:red;'> </span>"
						 ),
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	foreach ($_COOKIE as $key => $value) {
		...
		$urusan = cmbQuery('cmbUrusan',$selectedC1,$codeAndNameUrusan,'onchange=rkaSKPD1PAD.refreshList(true);','-- URUSAN --');

		$codeAndNameBidang = "select c, concat(c, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c !='00' and d='00' and e='00' and e1='000' ";
		$bidang = cmbQuery('cmbBidang',$selectedC,$codeAndNameBidang,'onchange=rkaSKPD1PAD.refreshList(true);','-- BIDANG --');

		$codeAndNameSKPD = "select d, concat(d, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d!='00' and e='00' and e1='000' ";
		$skpd= cmbQuery('cmbSKPD',$selectedD,$codeAndNameSKPD,'onchange=rkaSKPD1PAD.refreshList(true);','-- SKPD --');
		...
		$unit = cmbQuery('cmbUnit',$selectedE,$codeAndNameUnit,'onchange=rkaSKPD1PAD.refreshList(true);','-- UNIT --');
		...
		$subunit = cmbQuery('cmbSubUnit',$selectedE1,$codeAndNameSubUnit,'onchange=rkaSKPD1PAD.refreshList(true);','-- SUB UNIT --');
...
$rkaSKPD1PAD = new rkaSKPD1PADObj();
...
$rkaSKPD1PAD->jenisForm = $jenisForm;
$rkaSKPD1PAD->nomorUrut = $nomorUrut;
$rkaSKPD1PAD->urutTerakhir = $nomorUrut;
$rkaSKPD1PAD->tahun = $tahun;
$rkaSKPD1PAD->jenisAnggaran = $jenisAnggaran;
$rkaSKPD1PAD->idTahap = $idTahap;
$rkaSKPD1PAD->username = $_COOKIE['coID'];

$rkaSKPD1PAD->wajibValidasi = $Main->wajibValidasi;
if($Main->wajibValidasi == TRUE){
	$rkaSKPD1PAD->sqlValidasi = " and status_validasi ='1' ";
}else{
	$rkaSKPD1PAD->sqlValidasi = " ";
}

if(empty($rkaSKPD1PAD->tahun)){
	...
	$rkaSKPD1PAD->tahun  = $get2['tahun'];
	$rkaSKPD1PAD->jenisAnggaran = $get2['jenis_anggaran'];
	$rkaSKPD1PAD->urutTerakhir = $get2['no_urut'];
	$rkaSKPD1PAD->jenisFormTerakhir = $get2['jenis_form_modul'];
	$rkaSKPD1PAD->urutSebelumnya = $rkaSKPD1PAD->urutTerakhir - 1;
	...
	$rkaSKPD1PAD->namaTahapTerakhir = $namaTahap['nama_tahap'];
	...
	$rkaSKPD1PAD->masaTerakhir = $lastTanggalMulai." JAM : ".$namaTahap['jam_mulai']."  s/d  ".$lastTanggalSelesai." JAM : ".$namaTahap['jam_selesai'];

	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$rkaSKPD1PAD->currentTahap = $arrayHasil['currentTahap'];
}else{
	$getCurrenttahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$rkaSKPD1PAD->idTahap'"));
	$rkaSKPD1PAD->currentTahap = $getCurrenttahap['nama_tahap'];

	$namaTahap = mysql_fetch_array(mysql_query("select * from ref_tahap_anggaran where id_tahap = '$rkaSKPD1PAD->idTahap'"));
	$rkaSKPD1PAD->jenisFormTerakhir =  $namaTahap['jenis_form_modul'];
	$rkaSKPD1PAD->namaTahapTerakhir = $namaTahap['nama_tahap'];
	$arrayMasa = explode("-",$namaTahap['tanggal_mulai']);
	$lastTanggalMulai = $arrayMasa[2]."-".$arrayMasa[1]."-".$arrayMasa[0];
	$arrayMasa2 = explode("-",$namaTahap['tanggal_selesai']);
...
$rkaSKPD1PAD->provinsi = $setting['provinsi'];
$rkaSKPD1PAD->kota = $setting['kota'];
$rkaSKPD1PAD->pengelolaBarang = $setting['pengelolaBarang'];
$rkaSKPD1PAD->pejabatPengelolaBarang = $setting['pejabat'];
$rkaSKPD1PAD->pengurusPengelolaBarang = $setting['pengurus'];
$rkaSKPD1PAD->nipPengelola = $setting['nipPengelola'];
$rkaSKPD1PAD->nipPengurus = $setting['nipPengurus'];
$rkaSKPD1PAD->nipPejabat = $setting['nipPejabat'];


if($rkaSKPD1PAD->jenisForm != "PENYUSUNAN"){
$get1 = mysql_fetch_array(mysql_query("select max(id_anggaran)  from view_rka_1 where jenis_form_modul = 'PENYUSUNAN' "));
	...
	$arrayHasil =  VulnWalkerLASTTahap_v2();
	$rkaSKPD1PAD->currentTahap = $arrayHasil['currentTahap'];
			$rkaSKPD1PAD->jenisForm = "";
			$rkaSKPD1PAD->jenisFormTerakhir = "PENYUSUNAN";
			$rkaSKPD1PAD->idTahap = $get2['id_tahap'];
			$rkaSKPD1PAD->tahun = $get2['tahun'];
			$rkaSKPD1PAD->nomorUrut = $get2['no_urut'];
			$rkaSKPD1PAD->urutTerakhir = $get2['no_urut'];
}
