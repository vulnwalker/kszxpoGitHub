var rkaSKPDKoreksi = new DaftarObj2({
	prefix : 'rkaSKPDKoreksi',
	url : 'pages.php?Pg=rkaSKPDKoreksi', 
	formName : 'rkaSKPDKoreksiForm',
	rkaSKPDKoreksi_form : '0',//default js rkaSKPDKoreksi
	loading: function(){
		//alert('loading');
		this.topBarRender();
		this.filterRender();
...
/*		this.sumHalRender();*/ 
	...
	programChanged : function(){
		...
		$("#hiddenP").val(hiddenP);
		$("#q").val('');
		rkaSKPDKoreksi.refreshList(true);	
	},
	detail: function(){
		//alert('detail');
		var me = this;
...
/*				me.sumHalRender();*/
			...
			alert("Pilih Kegiatan");
				...
				type:'POST', 
			  	...
			  	success: function(data) {		
					...
					aForm.action= 'pages.php?Pg=formrkaSKPDKoreksi1&skpd='+skpd;
					aForm.target='_blank';
					aForm.submit();	
					aForm.target='';
				 }
	...
	unlockFindRekening: function(){
		...
		if( $("#cmbJenisRKAForm").val() == ""){
			...
			$("#findRekening").attr('onclick',"rkaSKPDKoreksi.findRekening('"+$("#cmbJenisRKAForm").val()+"');");
				...
				$.ajax({
				  ...
				  success: function(data) {
						...
						if(resp.err == "") {
							...
							aForm.action= 'pages.php?Pg=formrkaSKPDKoreksi1&rek='+kodeRek+'&skpd='+skpd;
							aForm.target='_blank';
							aForm.submit();	
							aForm.target='';
						}else{
	...
	koreksi:function(id){
		$("#spanVolumeBarang"+id).html("<input type='text' name='volumeBarang"+id+"' id='volumeBarang"+id+"' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='rkaSKPDKoreksi.bantuVolumeBarang("+id+")'   style='text-align: right;'>  <span id='bantuVolumeBarang"+id+"' ></span>");
		$("#spanSatuanHarga"+id).html("<input type='text' name='satuanHarga"+id+"' id='satuanHarga"+id+"' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='rkaSKPDKoreksi.bantuSatuanHarga("+id+")'    style='text-align: right;'>  <span id='bantuSatuanHarga"+id+"' ></span>");
		$("#buttonSubmitKoreksi"+id).html("<img src='datepicker/save.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkaSKPDKoreksi.submitKoreksi('"+id+"');></img>&nbsp  &nbsp <img src='images/administrator/images/invalid.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkaSKPDKoreksi.refreshList(true);></img>");
	...
	bantuVolumeBarang : function(id){
		document.getElementById('bantuVolumeBarang'+id).innerHTML = rkaSKPDKoreksi.formatCurrency2($("#volumeBarang"+id).val());
		var hasilKali = Number($("#volumeBarang"+id).val()) * Number($("#satuanHarga"+id).val()) ; 
		$("#spanJumlahHarga"+id).text(rkaSKPDKoreksi.formatCurrency(hasilKali));
	},
	bantuSatuanHarga : function(id){
		document.getElementById('bantuSatuanHarga'+id).innerHTML =  rkaSKPDKoreksi.formatCurrency($("#satuanHarga"+id).val());
		var hasilKali = Number($("#volumeBarang"+id).val()) * Number($("#satuanHarga"+id).val()) ; 
		$("#spanJumlahHarga"+id).text(rkaSKPDKoreksi.formatCurrency(hasilKali));
	},
	...
	bantu : function(){
		$("#bantu").text("Rp. "+rkaSKPDKoreksi.formatCurrency($("#hargaSatuan").val())  );
		...
		$.ajax({
			...
			url: this.url+'&tipe=simpan',
		  	success: function(data) {		
				...
				//document.getElementById(cover).innerHTML = resp.content;
				if(resp.err==''){
					if(me.rkaSKPDKoreksi_form==0){
						me.Close();
						me.AfterSimpan();						
					}else{
						me.Close();
						barang.refreshComborkaSKPDKoreksi();
					}
