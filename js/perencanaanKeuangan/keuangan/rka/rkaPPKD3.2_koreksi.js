var rkaPPKD32_v2_koreksi = new DaftarObj2({
	prefix : 'rkaPPKD32_v2_koreksi',
	url : 'pages.php?Pg=rka-ppkd-3.2_v2_koreksi', 
	formName : 'rkaPPKD32_v2_koreksiForm',
	rkaPPKD32_v2_koreksi_form : '0',//default js rkaPPKD32_v2_koreksi
	loading: function(){
		//alert('loading');
		this.topBarRender();
		this.filterRender();
...
/*		this.sumHalRender();*/ 
	...
	programChanged : function(){
		...
		$("#hiddenP").val(hiddenP);
		$("#q").val('');
		rkaPPKD32_v2_koreksi.refreshList(true);	
	},
	detail: function(){
		//alert('detail');
		var me = this;
...
/*				me.sumHalRender();*/
			...
			alert("Pilih SKPD");
				...
				data : {c1 : $("#cmbUrusan").val(),
			  	...
			  	success: function(data) {		
					var resp = eval('(' + data + ')');
					if(resp.err == ''){
						aForm.action= 'pages.php?Pg=formrkaPPKD32_v2_koreksi&skpd='+skpd;
						aForm.target='_blank';
						aForm.submit();	
						aForm.target='';
					}else{
	...
	unlockFindRekening: function(){
		...
		if( $("#cmbJenisRKAForm").val() == ""){
			...
			$("#findRekening").attr('onclick',"rkaPPKD32_v2_koreksi.findRekening('"+$("#cmbJenisRKAForm").val()+"');");
				...
				$.ajax({
				  ...
				  success: function(data) {
						...
						if(resp.err == "") {
							...
							aForm.action= 'pages.php?Pg=formrkaPPKD32_v2_koreksi&rek='+kodeRek+'&skpd='+skpd;
							aForm.target='_blank';
							aForm.submit();	
							aForm.target='';
						}else{
	...
	koreksi:function(id){
		$("#spanVolumeBarang"+id).html("<input type='text' name='volumeBarang"+id+"' id='volumeBarang"+id+"' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='rkaPPKD32_v2_koreksi.bantuVolumeBarang("+id+")'   style='text-align: right;'>  <span id='bantuVolumeBarang"+id+"' ></span>");
		$("#spanSatuanHarga"+id).html("<input type='text' name='satuanHarga"+id+"' id='satuanHarga"+id+"' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='rkaPPKD32_v2_koreksi.bantuSatuanHarga("+id+")'    style='text-align: right;'>  <span id='bantuSatuanHarga"+id+"' ></span>");
		$("#buttonSubmitKoreksi"+id).html("<img src='datepicker/save.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkaPPKD32_v2_koreksi.submitKoreksi('"+id+"');></img>&nbsp  &nbsp <img src='images/administrator/images/invalid.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkaPPKD32_v2_koreksi.refreshList(true);></img>");
	...
	bantuVolumeBarang : function(id){
		document.getElementById('bantuVolumeBarang'+id).innerHTML = rkaPPKD32_v2_koreksi.formatCurrency2($("#volumeBarang"+id).val());
		var hasilKali = Number($("#volumeBarang"+id).val()) * Number($("#satuanHarga"+id).val()) ; 
		$("#spanJumlahHarga"+id).text(rkaPPKD32_v2_koreksi.formatCurrency(hasilKali));
	},
	bantuSatuanHarga : function(id){
		document.getElementById('bantuSatuanHarga'+id).innerHTML =  rkaPPKD32_v2_koreksi.formatCurrency($("#satuanHarga"+id).val());
		var hasilKali = Number($("#volumeBarang"+id).val()) * Number($("#satuanHarga"+id).val()) ; 
		$("#spanJumlahHarga"+id).text(rkaPPKD32_v2_koreksi.formatCurrency(hasilKali));
	},
	...
	bantu : function(){
		$("#bantu").text("Rp. "+rkaPPKD32_v2_koreksi.formatCurrency($("#hargaSatuan").val())  );
		...
		$.ajax({
			...
			url: this.url+'&tipe=simpan',
		  	success: function(data) {		
				...
				//document.getElementById(cover).innerHTML = resp.content;
				if(resp.err==''){
					if(me.rkaPPKD32_v2_koreksi_form==0){
						me.Close();
						me.AfterSimpan();						
					}else{
						me.Close();
						barang.refreshComborkaPPKD32_v2_koreksi();
					}
   ...
   SaveCatatan: function(id){
	...
	hitungSelisih:function(namaBantu){
		...
		$("#"+namaBantu).text("Rp. "+rkaPPKD32_v2_koreksi.formatCurrency($("#"+idPost).val())  );
		...
		$("#selisih").val(selisih);	
		$("#bantuPenjumlahan").text("Rp. "+rkaPPKD32_v2_koreksi.formatCurrency(jumlahHargaAlokasi)  );		
		$("#bantuSelisih").text("Rp. "+rkaPPKD32_v2_koreksi.formatCurrency(selisih)  );				
	},
	...
	jenisAlokasiChanged : function(id){
		...
		if($("#jenisAlokasi").val() == "BULANAN"){
			...
			$.ajax({
				...
				success: function(data) {	
					rkaPPKD32_v2_koreksi.formAlokasi(id);
				}
			});	
			...
			$.ajax({
				...
				url: this.url+'&tipe=clearAlokasi',
				success: function(data) {	
					rkaPPKD32_v2_koreksi.formAlokasiTriwulan(id);
				}
			});	
	...
	hitung: function(){
		if($("#jenisAlokasi").val() == '' ){
			...
			if($("#jenisAlokasi").val() == "BULANAN"){
				...
				rkaPPKD32_v2_koreksi.hitungSelisih('bantuJan');
				rkaPPKD32_v2_koreksi.hitungSelisih('bantuFeb');
				rkaPPKD32_v2_koreksi.hitungSelisih('bantuApr');
				rkaPPKD32_v2_koreksi.hitungSelisih('bantuMei');
				rkaPPKD32_v2_koreksi.hitungSelisih('bantuJul');
				rkaPPKD32_v2_koreksi.hitungSelisih('bantuAgu');
				rkaPPKD32_v2_koreksi.hitungSelisih('bantuOkt');
				rkaPPKD32_v2_koreksi.hitungSelisih('bantuNop');
				rkaPPKD32_v2_koreksi.hitungSelisih('bantuMar');
				rkaPPKD32_v2_koreksi.hitungSelisih('bantuJun');
				rkaPPKD32_v2_koreksi.hitungSelisih('bantuSep');
				rkaPPKD32_v2_koreksi.hitungSelisih('bantuDes');
			}else if($("#jenisAlokasi").val() == "TRIWULAN"){
				var jumlahHargaForm = $("#jumlahHargaForm").val();
				$("#jan").val('');
				$("#feb").val('');
