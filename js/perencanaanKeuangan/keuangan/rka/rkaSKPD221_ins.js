var rkaSKPD221_insSKPD = new SkpdCls({
	prefix : 'rkaSKPD221_insSKPD', formName:'rkaSKPD221_insForm', kolomWidth:120,
...
var rkaSKPD221_ins = new DaftarObj2({
	prefix : 'rkaSKPD221_ins',
	url : 'pages.php?Pg=rkaSKPD221_ins',
	formName : 'rkaSKPD221_insForm',
				...
				type:'POST',
			  	...
			  	success: function(data) {
					...
					if(resp.content.status == "OK"){
						rkaSKPD221_ins.edit(resp.content.id);
	...
	deleteSubRincianBelanja:function(id){
		var me = this;
			if(confirm("Yakin hapus sub rincian belanja ?")){
				$.ajax({
					type:'POST',
					data : {id : id,
	...
	hitungSelisih:function(namaBantu){
		...
		$("#"+namaBantu).text("Rp. "+rkaSKPD221_ins.formatCurrency($("#"+idPost).val())  );
		...
		$("#selisih").val(selisih);
		$("#bantuPenjumlahan").text("Rp. "+rkaSKPD221_ins.formatCurrency(jumlahHargaAlokasi)  );
		$("#bantuSelisih").text("Rp. "+rkaSKPD221_ins.formatCurrency(selisih)  );
	},
					...
					data : {
				  	...
				  	success: function(data) {
						...
						if(resp.err == ''){
							...
							$("#bantuJumlahHarga").text("Rp. "+rkaSKPD221_ins.formatCurrency($("#jumlahHarga").val()));
						}else{
							alert(resp.err);
						}
				  	}
	...
	bantu : function(){
		$("#bantuSatuanHarga").text("Rp. "+rkaSKPD221_ins.formatCurrency($("#hargaSatuan").val())  );
		$("#jumlahHarga").val(rkaSKPD221_ins.formatCurrency($("#hargaSatuan").val() * $("#volumeBarang").val()));
		$("#bantuJumlahHarga").text("Rp. "+rkaSKPD221_ins.formatCurrency($("#jumlahHarga").val()));
		$("#teralokasi").val('');
	},
	...
	hitung: function(){
		if($("#jenisAlokasi").val() == '' ){
			...
			if($("#jenisAlokasi").val() == "BULANAN"){
				...
				rkaSKPD221_ins.hitungSelisih('bantuJan');
				rkaSKPD221_ins.hitungSelisih('bantuFeb');
				rkaSKPD221_ins.hitungSelisih('bantuApr');
				rkaSKPD221_ins.hitungSelisih('bantuMei');
				rkaSKPD221_ins.hitungSelisih('bantuJul');
				rkaSKPD221_ins.hitungSelisih('bantuAgu');
				rkaSKPD221_ins.hitungSelisih('bantuOkt');
				rkaSKPD221_ins.hitungSelisih('bantuNop');
				rkaSKPD221_ins.hitungSelisih('bantuMar');
				rkaSKPD221_ins.hitungSelisih('bantuJun');
				rkaSKPD221_ins.hitungSelisih('bantuSep');
				rkaSKPD221_ins.hitungSelisih('bantuDes');
	...
	jenisAlokasiChanged : function(){
		...
		if($("#jenisAlokasi").val() == "BULANAN"){
			...
			$.ajax({
				...
				url: this.url+'&tipe=clearAlokasi',
				success: function(data) {
					rkaSKPD221_ins.formAlokasiTriwulan();
				}
			});
	...
	jenisAtribusiChanged:function(){
		var checkboxStatus = ( $("#checkboxAtribusi").is(':checked') ) ? 1 : 0;
		if(checkboxStatus == 1){
	...
	hapus : function(id){
		$.ajax({
				...
				success: function(data) {
					rkaSKPD221_ins.refreshList(true);
		...
		$.ajax({
				...
				success: function(data) {
					...
					if(resp.content.status == 1){
						...
						rkaSKPD221_ins.closeTab();

					}else{
						rkaSKPD221_ins.refreshList(true);
						setTimeout(function myFunction() {rkaSKPD221_ins.refreshList(true);},1000);
						$("#hargaSatuan").val("");
						$("#jumlahHarga").val("");
					}
				...
				success: function(data) {
					...
					if(resp.content.kunci == '0'){
						...
						if(document.getElementById('pemicuDetailVolume')){
							...
							document.getElementById('kolomButtonCariBarang').innerHTML = "<input type='button' id='findBarang' value='CARI' onclick='rkaSKPD221_ins.CariBarang();'/>";
						}
						...
						if(resp.content.jenis_alokasi_kas == "TRIWULAN"){
							$("#tombolAlokasi").attr("onclick","rkaSKPD221_ins.formAlokasiTriwulan();");
						}else{
							$("#tombolAlokasi").attr("onclick","rkaSKPD221_ins.formAlokasi();");
						}
						$("#namaRekening").val(resp.content.namaRekening);
						$("#bk").val(resp.content.bk);
						$("#ck").val(resp.content.ck);
						$("#dk").val(resp.content.dk);
						...
						$("#btsave").attr('href','javascript:rkaSKPD221_ins.saveEdit('+id+')');
						$("#pemicuDetailVolume").attr('onclick','rkaSKPD221_ins.detailVolume('+id+')');
						$("#statusDetail").val("");
						$("#btcancel").attr('href','javascript:rkaSKPD221_ins.cancelEdit()');
	...
	detailVolume : function(id){
		...
		if($("#rincianPerhitungan").val() == ''){
			...
			"<div id='detailVolume_cont_title' style='position:relative'></div>"+
			"<div id='detailVolume_cont_opsi' style='position:relative'>"+
			"</div>"+
			"<div id='detailVolume_cont_daftar' style='position:relative'></div>"+
			"<div id='detailVolume_cont_hal' style='position:relative'></div>"
			;
					detailVolume.loading();
					}else{
						alert(resp.err);
					}
