var formRkaSkpd221_v2SKPD = new SkpdCls({
	prefix : 'formRkaSkpd221_v2SKPD', formName:'formRkaSkpd221_v2Form', kolomWidth:120,
...
var formRkaSkpd221_v2 = new DaftarObj2({
	prefix : 'formRkaSkpd221_v2',
	url : 'pages.php?Pg=formRkaSkpd221_v2', 
	formName : 'formRkaSkpd221_v2Form',
	...
	loading: function(){
		...
		this.topBarRender();
		this.filterRender();
		formRkaSkpd221_v2.refreshList(true);
	
	},	
	
	filterRenderAfter : function(resp){
	
		$.ajax({
				type:'POST', 
			  	url: this.url+'&tipe=setGenNya',
			  	success: function(data) {		
					var resp = eval('(' + data + ')');
					if(resp.content.status == "OK"){
						formRkaSkpd221_v2.edit(resp.content.id);
					}		
	...
	closeTab : function(){
		...
		if(document.getElementById('rincianPerhitungan').value != ''){
			$.ajax({
				type:'POST', 
	...
	hitungSelisih:function(namaBantu){
		...
		$("#"+namaBantu).text("Rp. "+formRkaSkpd221_v2.formatCurrency($("#"+idPost).val())  );
		...
		$("#selisih").val(selisih);	
		$("#bantuPenjumlahan").text("Rp. "+formRkaSkpd221_v2.formatCurrency(jumlahHargaAlokasi)  );		
		$("#bantuSelisih").text("Rp. "+formRkaSkpd221_v2.formatCurrency(selisih)  );				
	},
	...
	setSatuanHarga:function(){
		...
		var f = arrayKodeBarang[0];
		var g = arrayKodeBarang[1];
		var h = arrayKodeBarang[2];
		var i = arrayKodeBarang[3];
		var j = arrayKodeBarang[4];
					...
					data : {
				  	...
				  	success: function(data) {		
						...
						if(resp.err == ''){
							...
							$("#bantuJumlahHarga").text("Rp. "+formRkaSkpd221_v2.formatCurrency($("#jumlahHarga").val()));
						}else{
							alert(resp.err);
						}			
				  	}
	...
	bantu : function(){
		$("#bantuSatuanHarga").text("Rp. "+formRkaSkpd221_v2.formatCurrency($("#hargaSatuan").val())  );
		$("#jumlahHarga").val($("#hargaSatuan").val() * $("#volumeBarang").val());
		$("#bantuJumlahHarga").text("Rp. "+formRkaSkpd221_v2.formatCurrency($("#jumlahHarga").val()));
		$("#teralokasi").val('');
	},
	...
	CariBarang: function(){
		var me = this;
		popupBarangRKBMD_v3.windowShow();	
	...
	hitung: function(){
		if($("#jenisAlokasi").val() == '' ){
			...
			if($("#jenisAlokasi").val() == "BULANAN"){
				...
				formRkaSkpd221_v2.hitungSelisih('bantuJan');
				formRkaSkpd221_v2.hitungSelisih('bantuFeb');
				formRkaSkpd221_v2.hitungSelisih('bantuApr');
				formRkaSkpd221_v2.hitungSelisih('bantuMei');
				formRkaSkpd221_v2.hitungSelisih('bantuJul');
				formRkaSkpd221_v2.hitungSelisih('bantuAgu');
				formRkaSkpd221_v2.hitungSelisih('bantuOkt');
				formRkaSkpd221_v2.hitungSelisih('bantuNop');
				formRkaSkpd221_v2.hitungSelisih('bantuMar');
				formRkaSkpd221_v2.hitungSelisih('bantuJun');
				formRkaSkpd221_v2.hitungSelisih('bantuSep');
				formRkaSkpd221_v2.hitungSelisih('bantuDes');
	...
	jenisAlokasiChanged : function(){
		...
		if($("#jenisAlokasi").val() == "BULANAN"){
			...
			$.ajax({
				...
				url: this.url+'&tipe=clearAlokasi',
				success: function(data) {	
					formRkaSkpd221_v2.formAlokasiTriwulan();
				}
			});	
	...
	cancelEdit : function(){
		...
		$.ajax({
				type:'POST', 
				url: this.url+'&tipe=checkKondisi',
				success: function(data) {	
					var resp = eval('(' + data + ')');	
					if(resp.content.status == 1){
						document.getElementById('rincianPerhitungan').value = '';
						formRkaSkpd221_v2.closeTab();
						
					}else{
						formRkaSkpd221_v2.refreshList(true);
						setTimeout(function myFunction() {formRkaSkpd221_v2.refreshList(true);},1000);
						$("#hargaSatuan").val("");
						$("#jumlahHarga").val("");
					}
		...
		$.ajax({
				...
				success: function(data) {	
					...
					if(resp.content.kunci == '0'){
						...
						if(document.getElementById('pemicuDetailVolume')){
							
						}else{
							document.getElementById('kolomButtonCariBarang').innerHTML = "<input type='button' id='findBarang' value='CARI' onclick='formRkaSkpd221_v2.CariBarang();'/>";
						}
						...
						if(resp.content.jenis_alokasi_kas == "TRIWULAN"){
							$("#tombolAlokasi").attr("onclick","formRkaSkpd221_v2.formAlokasiTriwulan();");
						}else{
							$("#tombolAlokasi").attr("onclick","formRkaSkpd221_v2.formAlokasi();");
						}
						...
						$("#btsave").attr('href','javascript:formRkaSkpd221_v2.saveEdit('+id+')');
						$("#pemicuDetailVolume").attr('onclick','formRkaSkpd221_v2.detailVolume('+id+')');
						$("#statusDetail").val("");
						$("#btcancel").attr('href','javascript:formRkaSkpd221_v2.cancelEdit()');
				...
				success: function(data) {	
					...
					if(resp.err == ''){
						...
						setTimeout(function myFunction() {
							$("#kodeRekening").val(resp.content.kodeRekening);
							$("#namaRekening").val(resp.content.namaRekening);
							var arrayKodeRekening = resp.content.kodeRekening.split('.');
							if(arrayKodeRekening[0] == '5' && arrayKodeRekening[1] == '2' && arrayKodeRekening[2] == '3' ){
								document.getElementById('kolomButtonCariBarang').innerHTML= "<input type='button' id='findBarang' value='CARI' onclick='formRkaSkpd221_v2.CariBarang();'/>";
								$("#rincianPerhitungan").attr('readonly','true');
							}else{
								$("#rincianPerhitungan").removeAttr("readonly");
								document.getElementById('kolomButtonCariBarang').innerHTML= "";
							}
							formRkaSkpd221_v2.setNoUrut();
							},1000);
						if(resp.content.lanjut == 'finish'){
							formRkaSkpd221_v2.finish();
						}
					}else{
						if(resp.err == "Lengkapi Alokasi"){
							formRkaSkpd221_v2.formAlokasi();
						}else if(resp.err == "Lengkapi Rincian Volume"){
							formRkaSkpd221_v2.formRincianVolume();
						}else{
							alert(resp.err);
						}
	...
	detailVolume : function(id){
		
		var me = this;
		var cover = this.prefix+'_formcover';

		if($("#rincianPerhitungan").val() == ''){
			err = "Rincian Perhitungan Kosong";
		}else{
			$.ajax({
				type:'POST', 
				data : {
			  	...
			  	url: this.url+'&tipe=detailVolume',
			  	success: function(data) {		
					var resp = eval('(' + data + ')');
					if(resp.err == ''){
						document.body.style.overflow='hidden';
						addCoverPage2(cover,999,true,false);	
						document.getElementById(cover).innerHTML = resp.content;
						document.getElementById('listDetailVolume').innerHTML = 
			"<div id='detailVolumeRka221_cont_title' style='position:relative'></div>"+
			"<div id='detailVolumeRka221_cont_opsi' style='position:relative'>"+
			"</div>"+
			"<div id='detailVolumeRka221_cont_daftar' style='position:relative'></div>"+
			"<div id='detailVolumeRka221_cont_hal' style='position:relative'></div>"
			;
					detailVolumeRka221.loading();
					}else{
						alert(resp.err);
					}			
	...
	saveDetailVolume: function(){
		...
		$.ajax({
			type:'POST', 
			url: this.url+'&tipe=saveDetailVolume',
		  	success: function(data) {		
				var resp = eval('(' + data + ')');	
				delElem(cover);		
				if(resp.err==''){
					$("#volumeBarang").val(resp.content.totalVolume);
					$("#statusDetail").val("1");
					formRkaSkpd221_v2.bantu();
					formRkaSkpd221_v2.resetRincian();
					me.Close();
