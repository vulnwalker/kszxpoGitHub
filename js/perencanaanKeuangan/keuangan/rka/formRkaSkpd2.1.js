var formRkaSkpd21_v2SKPD = new SkpdCls({
	prefix : 'formRkaSkpd21_v2SKPD', formName:'formRkaSkpd21_v2Form', kolomWidth:120,
...
var formRkaSkpd21_v2 = new DaftarObj2({
	prefix : 'formRkaSkpd21_v2',
	url : 'pages.php?Pg=formRkaSkpd21_v2', 
	formName : 'formRkaSkpd21_v2Form',
	...
	loading: function(){
		...
		this.topBarRender();
		this.filterRender();
		formRkaSkpd21_v2.refreshList(true);
	...
	hitungSelisih:function(namaBantu){
		...
		$("#"+namaBantu).text("Rp. "+formRkaSkpd21_v2.formatCurrency($("#"+idPost).val())  );
		...
		$("#selisih").val(selisih);	
		$("#bantuPenjumlahan").text("Rp. "+formRkaSkpd21_v2.formatCurrency(jumlahHargaAlokasi)  );		
		$("#bantuSelisih").text("Rp. "+formRkaSkpd21_v2.formatCurrency(selisih)  );				
	},
					...
					data : {
				  	...
				  	success: function(data) {		
						...
						if(resp.err == ''){
							...
							$("#bantuJumlahHarga").text("Rp. "+formRkaSkpd21_v2.formatCurrency($("#jumlahHarga").val()));
						}else{
							alert(resp.err);
						}			
				  	}
	...
	bantu : function(){
		$("#bantuSatuanHarga").text("Rp. "+formRkaSkpd21_v2.formatCurrency($("#hargaSatuan").val())  );
		$("#jumlahHarga").val($("#hargaSatuan").val() * $("#volumeBarang").val());
		$("#bantuJumlahHarga").text("Rp. "+formRkaSkpd21_v2.formatCurrency($("#jumlahHarga").val()));
		$("#teralokasi").val('');
	},
	...
	hitung: function(){
		if($("#jenisAlokasi").val() == '' ){
			...
			if($("#jenisAlokasi").val() == "BULANAN"){
				...
				formRkaSkpd21_v2.hitungSelisih('bantuJan');
				formRkaSkpd21_v2.hitungSelisih('bantuFeb');
				formRkaSkpd21_v2.hitungSelisih('bantuApr');
				formRkaSkpd21_v2.hitungSelisih('bantuMei');
				formRkaSkpd21_v2.hitungSelisih('bantuJul');
				formRkaSkpd21_v2.hitungSelisih('bantuAgu');
				formRkaSkpd21_v2.hitungSelisih('bantuOkt');
				formRkaSkpd21_v2.hitungSelisih('bantuNop');
				formRkaSkpd21_v2.hitungSelisih('bantuMar');
				formRkaSkpd21_v2.hitungSelisih('bantuJun');
				formRkaSkpd21_v2.hitungSelisih('bantuSep');
				formRkaSkpd21_v2.hitungSelisih('bantuDes');
	...
	jenisAlokasiChanged : function(){
		...
		if($("#jenisAlokasi").val() == "BULANAN"){
			...
			$.ajax({
				...
				url: this.url+'&tipe=clearAlokasi',
				success: function(data) {	
					formRkaSkpd21_v2.formAlokasiTriwulan();
				}
			});	
	...
	cancelEdit : function(){
		formRkaSkpd21_v2.refreshList(true);
		setTimeout(function myFunction() {formRkaSkpd21_v2.refreshList(true);},1000);
	},
				...
				success: function(data) {	
					...
					if(resp.content.kunci == '0'){
						...
						if(resp.content.jenis_alokasi_kas == "TRIWULAN"){
							$("#tombolAlokasi").attr("onclick","formRkaSkpd21_v2.formAlokasiTriwulan();");
						}else{
							$("#tombolAlokasi").attr("onclick","formRkaSkpd21_v2.formAlokasi();");
						}
						...
						$("#btsave").attr('href','javascript:formRkaSkpd21_v2.saveEdit('+id+')');
						$("#btcancel").attr('href','javascript:formRkaSkpd21_v2.cancelEdit()');
				...
				success: function(data) {	
					...
					if(resp.err == ''){
						...
						if(resp.err == "Lengkapi Alokasi"){
							...
							formRkaSkpd21_v2.formRincianVolume();
						}else{
							alert(resp.err);
						}
