var rkaPPKD21Koreksi = new DaftarObj2({
	prefix : 'rkaPPKD21Koreksi',
	url : 'pages.php?Pg=rkaPPKD21Koreksi', 
	formName : 'rkaPPKD21KoreksiForm',
	rkaPPKD21Koreksi_form : '0',//default js rkaPPKD21Koreksi
	loading: function(){
		//alert('loading');
		this.topBarRender();
		this.filterRender();
...
/*		this.sumHalRender();*/ 
	...
	programChanged : function(){
		...
		$("#hiddenP").val(hiddenP);
		$("#q").val('');
		rkaPPKD21Koreksi.refreshList(true);	
	},
	detail: function(){
		//alert('detail');
		var me = this;
...
/*				me.sumHalRender();*/
			...
			alert("Pilih SKPD");
				...
				data : {c1 : $("#cmbUrusan").val(),
			  	...
			  	success: function(data) {		
					var resp = eval('(' + data + ')');
					if(resp.err == ''){
						aForm.action= 'pages.php?Pg=formrkaPPKD21Koreksi&skpd='+skpd;
						aForm.target='_blank';
						aForm.submit();	
						aForm.target='';
					}else{
	...
	unlockFindRekening: function(){
		...
		if( $("#cmbJenisRKAForm").val() == ""){
			...
			$("#findRekening").attr('onclick',"rkaPPKD21Koreksi.findRekening('"+$("#cmbJenisRKAForm").val()+"');");
				...
				$.ajax({
				  ...
				  success: function(data) {
						...
						if(resp.err == "") {
							...
							aForm.action= 'pages.php?Pg=formrkaPPKD21Koreksi&rek='+kodeRek+'&skpd='+skpd;
							aForm.target='_blank';
							aForm.submit();	
							aForm.target='';
						}else{
	...
	koreksi:function(id){
		$("#spanVolumeBarang"+id).html("<input type='text' name='volumeBarang"+id+"' id='volumeBarang"+id+"' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='rkaPPKD21Koreksi.bantuVolumeBarang("+id+")'   style='text-align: right;'>  <span id='bantuVolumeBarang"+id+"' ></span>");
		$("#spanSatuanHarga"+id).html("<input type='text' name='satuanHarga"+id+"' id='satuanHarga"+id+"' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='rkaPPKD21Koreksi.bantuSatuanHarga("+id+")'    style='text-align: right;'>  <span id='bantuSatuanHarga"+id+"' ></span>");
		$("#buttonSubmitKoreksi"+id).html("<img src='datepicker/save.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkaPPKD21Koreksi.submitKoreksi('"+id+"');></img>&nbsp  &nbsp <img src='images/administrator/images/invalid.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkaPPKD21Koreksi.refreshList(true);></img>");
	...
	bantuVolumeBarang : function(id){
		document.getElementById('bantuVolumeBarang'+id).innerHTML = rkaPPKD21Koreksi.formatCurrency2($("#volumeBarang"+id).val());
		var hasilKali = Number($("#volumeBarang"+id).val()) * Number($("#satuanHarga"+id).val()) ; 
		$("#spanJumlahHarga"+id).text(rkaPPKD21Koreksi.formatCurrency(hasilKali));
	},
	bantuSatuanHarga : function(id){
		document.getElementById('bantuSatuanHarga'+id).innerHTML =  rkaPPKD21Koreksi.formatCurrency($("#satuanHarga"+id).val());
		var hasilKali = Number($("#volumeBarang"+id).val()) * Number($("#satuanHarga"+id).val()) ; 
		$("#spanJumlahHarga"+id).text(rkaPPKD21Koreksi.formatCurrency(hasilKali));
	},
	...
	bantu : function(){
		$("#bantu").text("Rp. "+rkaPPKD21Koreksi.formatCurrency($("#hargaSatuan").val())  );
		...
		$.ajax({
			...
			url: this.url+'&tipe=simpan',
		  	success: function(data) {		
				...
				//document.getElementById(cover).innerHTML = resp.content;
				if(resp.err==''){
					if(me.rkaPPKD21Koreksi_form==0){
						me.Close();
						me.AfterSimpan();						
					}else{
						me.Close();
						barang.refreshComborkaPPKD21Koreksi();
					}
   ...
   SaveCatatan: function(id){
	...
	hitungSelisih:function(namaBantu){
		...
		$("#"+namaBantu).text("Rp. "+rkaPPKD21Koreksi.formatCurrency($("#"+idPost).val())  );
		...
		$("#selisih").val(selisih);	
		$("#bantuPenjumlahan").text("Rp. "+rkaPPKD21Koreksi.formatCurrency(jumlahHargaAlokasi)  );		
		$("#bantuSelisih").text("Rp. "+rkaPPKD21Koreksi.formatCurrency(selisih)  );				
	},
	...
	jenisAlokasiChanged : function(id){
		...
		if($("#jenisAlokasi").val() == "BULANAN"){
			...
			$.ajax({
				...
				success: function(data) {	
					rkaPPKD21Koreksi.formAlokasi(id);
				}
			});	
			...
			$.ajax({
				...
				url: this.url+'&tipe=clearAlokasi',
				success: function(data) {	
					rkaPPKD21Koreksi.formAlokasiTriwulan(id);
				}
			});	
	...
	hitung: function(){
		if($("#jenisAlokasi").val() == '' ){
			...
			if($("#jenisAlokasi").val() == "BULANAN"){
				...
				rkaPPKD21Koreksi.hitungSelisih('bantuJan');
				rkaPPKD21Koreksi.hitungSelisih('bantuFeb');
				rkaPPKD21Koreksi.hitungSelisih('bantuApr');
				rkaPPKD21Koreksi.hitungSelisih('bantuMei');
				rkaPPKD21Koreksi.hitungSelisih('bantuJul');
				rkaPPKD21Koreksi.hitungSelisih('bantuAgu');
				rkaPPKD21Koreksi.hitungSelisih('bantuOkt');
				rkaPPKD21Koreksi.hitungSelisih('bantuNop');
				rkaPPKD21Koreksi.hitungSelisih('bantuMar');
				rkaPPKD21Koreksi.hitungSelisih('bantuJun');
				rkaPPKD21Koreksi.hitungSelisih('bantuSep');
				rkaPPKD21Koreksi.hitungSelisih('bantuDes');
			}else if($("#jenisAlokasi").val() == "TRIWULAN"){
				var jumlahHargaForm = $("#jumlahHargaForm").val();
				$("#jan").val('');
				$("#feb").val('');
