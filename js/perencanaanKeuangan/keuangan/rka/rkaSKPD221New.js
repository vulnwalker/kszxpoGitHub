var rkaSKPD221New = new DaftarObj2({
	prefix : 'rkaSKPD221New',
	url : 'pages.php?Pg=rkaSKPD221New',
	formName : 'rkaSKPD221NewForm',
	rkaSKPD221New_form : '0',//default js rkaSKPD221New
	loading: function(){
		//alert('loading');
		this.topBarRender();
		this.filterRender();
	...
	programChanged : function(){
		...
		$("#hiddenP").val(hiddenP);
		$("#q").val('');
		rkaSKPD221New.refreshList(true);
	},
	detail: function(){
		//alert('detail');
		var me = this;
...
/*		RefBarangButuh.el_nama_barang= 'nama_barang';		*/
	...
	unlockFindRekening: function(){
		...
		if( $("#cmbJenisRKAForm").val() == ""){
			...
			$("#findRekening").attr('onclick',"rkaSKPD221New.findRekening('"+$("#cmbJenisRKAForm").val()+"');");
				...
				var box = this.GetCbxChecked();
					...
					$.ajax({
				  	...
				  	success: function(data) {
						...
						aForm.action= 'pages.php?Pg=rkaSKPD221New_ins&skpd='+skpd;
						aForm.target='_blank';
						aForm.submit();
						aForm.target='';
					 }
	...
	koreksi:function(id){
		$("#spanVolumeBarang"+id).html("<input type='text' name='volumeBarang"+id+"' id='volumeBarang"+id+"' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='rkaSKPD221New.bantuVolumeBarang("+id+")'   style='text-align: right;'>  <span id='bantuVolumeBarang"+id+"' ></span>");
		$("#spanSatuanHarga"+id).html("<input type='text' name='satuanHarga"+id+"' id='satuanHarga"+id+"' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='rkaSKPD221New.bantuSatuanHarga("+id+")'    style='text-align: right;'>  <span id='bantuSatuanHarga"+id+"' ></span>");
		$("#buttonSubmitKoreksi"+id).html("<img src='datepicker/save.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkaSKPD221New.submitKoreksi('"+id+"');></img>&nbsp  &nbsp <img src='images/administrator/images/invalid.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkaSKPD221New.refreshList(true);></img>");
...
/*			  rkaSKPD221New.formAlokasi(id);*/
	...
	bantuVolumeBarang : function(id){
		document.getElementById('bantuVolumeBarang'+id).innerHTML = rkaSKPD221New.formatCurrency2($("#volumeBarang"+id).val());
		var hasilKali = Number($("#volumeBarang"+id).val()) * Number($("#satuanHarga"+id).val()) ;
		$("#spanJumlahHarga"+id).text(rkaSKPD221New.formatCurrency(hasilKali));
	},
	bantuSatuanHarga : function(id){
		document.getElementById('bantuSatuanHarga'+id).innerHTML =  rkaSKPD221New.formatCurrency($("#satuanHarga"+id).val());
		var hasilKali = Number($("#volumeBarang"+id).val()) * Number($("#satuanHarga"+id).val()) ;
		$("#spanJumlahHarga"+id).text(rkaSKPD221New.formatCurrency(hasilKali));
	},
	...
	bantu : function(){
		$("#bantu").text("Rp. "+rkaSKPD221New.formatCurrency($("#hargaSatuan").val())  );
   ...
   SaveCatatan: function(id){
	...
	hitungSelisih:function(namaBantu){
		...
		$("#"+namaBantu).text("Rp. "+rkaSKPD221New.formatCurrency($("#"+idPost).val())  );
		...
		$("#selisih").val(selisih);
		$("#bantuPenjumlahan").text("Rp. "+rkaSKPD221New.formatCurrency(jumlahHargaAlokasi)  );
		$("#bantuSelisih").text("Rp. "+rkaSKPD221New.formatCurrency(selisih)  );
	},
	...
	jenisAlokasiChanged : function(id){
		...
		if($("#jenisAlokasi").val() == "BULANAN"){
			...
			$.ajax({
				...
				url: this.url+'&tipe=clearAlokasi',
				success: function(data) {
					rkaSKPD221New.formAlokasiTriwulan(id);
				}
			});
	...
	hitung: function(){
		if($("#jenisAlokasi").val() == '' ){
			...
			if($("#jenisAlokasi").val() == "BULANAN"){
				...
				rkaSKPD221New.hitungSelisih('bantuJan');
				rkaSKPD221New.hitungSelisih('bantuFeb');
				rkaSKPD221New.hitungSelisih('bantuApr');
				rkaSKPD221New.hitungSelisih('bantuMei');
				rkaSKPD221New.hitungSelisih('bantuJul');
				rkaSKPD221New.hitungSelisih('bantuAgu');
				rkaSKPD221New.hitungSelisih('bantuOkt');
				rkaSKPD221New.hitungSelisih('bantuNop');
				rkaSKPD221New.hitungSelisih('bantuMar');
				rkaSKPD221New.hitungSelisih('bantuJun');
				rkaSKPD221New.hitungSelisih('bantuSep');
				rkaSKPD221New.hitungSelisih('bantuDes');
			}else if($("#jenisAlokasi").val() == "TRIWULAN"){
				var jumlahHargaForm = $("#jumlahHargaForm").val();
				$("#jan").val('');
				$("#feb").val('');
	...
	hitungVolume: function(id){
			...
			$("#spanTotalJumlah"+id).text(rkaSKPD221New.formatCurrency(hasilKali));
	},
