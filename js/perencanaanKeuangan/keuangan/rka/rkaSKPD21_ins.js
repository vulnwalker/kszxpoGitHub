var rkaSKPD21_insSKPD = new SkpdCls({
	prefix : 'rkaSKPD21_insSKPD', formName:'rkaSKPD21_insForm', kolomWidth:120,
...
var rkaSKPD21_ins = new DaftarObj2({
	prefix : 'rkaSKPD21_ins',
	url : 'pages.php?Pg=rkaSKPD21_ins',
	formName : 'rkaSKPD21_insForm',
	...
	hitungSelisih:function(namaBantu){
		...
		$("#"+namaBantu).text("Rp. "+rkaSKPD21_ins.formatCurrency($("#"+idPost).val())  );
		...
		$("#selisih").val(selisih);
		$("#bantuPenjumlahan").text("Rp. "+rkaSKPD21_ins.formatCurrency(jumlahHargaAlokasi)  );
		$("#bantuSelisih").text("Rp. "+rkaSKPD21_ins.formatCurrency(selisih)  );
	},
					...
					data : {
				  	...
				  	success: function(data) {
						...
						if(resp.err == ''){
							...
							$("#bantuJumlahHarga").text("Rp. "+rkaSKPD21_ins.formatCurrency($("#jumlahHarga").val()));
						}else{
							alert(resp.err);
						}
				  	}
	...
	hitung: function(){
		if($("#jenisAlokasi").val() == '' ){
			...
			if($("#jenisAlokasi").val() == "BULANAN"){
				...
				rkaSKPD21_ins.hitungSelisih('bantuJan');
				rkaSKPD21_ins.hitungSelisih('bantuFeb');
				rkaSKPD21_ins.hitungSelisih('bantuApr');
				rkaSKPD21_ins.hitungSelisih('bantuMei');
				rkaSKPD21_ins.hitungSelisih('bantuJul');
				rkaSKPD21_ins.hitungSelisih('bantuAgu');
				rkaSKPD21_ins.hitungSelisih('bantuOkt');
				rkaSKPD21_ins.hitungSelisih('bantuNop');
				rkaSKPD21_ins.hitungSelisih('bantuMar');
				rkaSKPD21_ins.hitungSelisih('bantuJun');
				rkaSKPD21_ins.hitungSelisih('bantuSep');
				rkaSKPD21_ins.hitungSelisih('bantuDes');
	...
	jenisAlokasiChanged : function(){
		...
		if($("#jenisAlokasi").val() == "BULANAN"){
			...
			$.ajax({
				...
				url: this.url+'&tipe=clearAlokasi',
				success: function(data) {
					rkaSKPD21_ins.formAlokasiTriwulan();
				}
			});
	...
	hapus : function(id){
		$.ajax({
				...
				success: function(data) {
					rkaSKPD21_ins.refreshList(true);
	...
	cancelEdit : function(){
		rkaSKPD21_ins.refreshList(true);
		setTimeout(function myFunction() {rkaSKPD21_ins.refreshList(true);},1000);
	},
				...
				success: function(data) {
					...
					if(resp.content.kunci == '0'){
						...
						if(resp.content.jenis_alokasi_kas == "TRIWULAN"){
							$("#tombolAlokasi").attr("onclick","rkaSKPD21_ins.formAlokasiTriwulan();");
						}else{
							$("#tombolAlokasi").attr("onclick","rkaSKPD21_ins.formAlokasi();");
						}
						...
						$("#btsave").attr('href','javascript:rkaSKPD21_ins.saveEdit('+id+')');
						$("#btcancel").attr('href','javascript:rkaSKPD21_ins.cancelEdit()');
				...
				success: function(data) {
					...
					if(resp.err == ''){
						...
						if(resp.err == "Lengkapi Alokasi"){
							...
							rkaSKPD21_ins.formRincianVolume();
						}else{
							alert(resp.err);
						}
