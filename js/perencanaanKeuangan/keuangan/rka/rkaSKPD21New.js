var rkaSKPD21New = new DaftarObj2({
	prefix : 'rkaSKPD21New',
	url : 'pages.php?Pg=rkaSKPD21New',
	formName : 'rkaSKPD21NewForm',
	rkaSKPD21New_form : '0',//default js rkaSKPD21New
	loading: function(){
		//alert('loading');
		this.topBarRender();
		this.filterRender();
...
/*				me.sumHalRender();*/
	...
	unlockFindRekening: function(){
		...
		if( $("#cmbJenisRKAForm").val() == ""){
			...
			$("#findRekening").attr('onclick',"rkaSKPD21New.findRekening('"+$("#cmbJenisRKAForm").val()+"');");
		...
		if($("#cmbUrusan").val()  == ''){
			...
			if(errmsg ==''){
				...
				$.ajax({
				  ...
				  data:$('#'+this.formName).serialize(),
				  success: function(data) {
							aForm.action= 'pages.php?Pg=rkaSKPD21New_ins&skpd='+skpd;
							aForm.target='_blank';
							aForm.submit();
							aForm.target='';
				  }
	...
	koreksi:function(id){
		$("#spanVolumeBarang"+id).html("<input type='text' name='volumeBarang"+id+"' id='volumeBarang"+id+"' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='rkaSKPD21New.bantuVolumeBarang("+id+")'   style='text-align: right;'>  <span id='bantuVolumeBarang"+id+"' ></span>");
		$("#spanSatuanHarga"+id).html("<input type='text' name='satuanHarga"+id+"' id='satuanHarga"+id+"' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='rkaSKPD21New.bantuSatuanHarga("+id+")'    style='text-align: right;'>  <span id='bantuSatuanHarga"+id+"' ></span>");
		$("#buttonSubmitKoreksi"+id).html("<img src='datepicker/save.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkaSKPD21New.submitKoreksi('"+id+"');></img>&nbsp  &nbsp <img src='images/administrator/images/invalid.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=rkaSKPD21New.refreshList(true);></img>");
	...
	bantuVolumeBarang : function(id){
		document.getElementById('bantuVolumeBarang'+id).innerHTML = rkaSKPD21New.formatCurrency2($("#volumeBarang"+id).val());
		var hasilKali = Number($("#volumeBarang"+id).val()) * Number($("#satuanHarga"+id).val()) ;
		$("#spanJumlahHarga"+id).text(rkaSKPD21New.formatCurrency(hasilKali));
	},
	bantuSatuanHarga : function(id){
		document.getElementById('bantuSatuanHarga'+id).innerHTML =  rkaSKPD21New.formatCurrency($("#satuanHarga"+id).val());
		var hasilKali = Number($("#volumeBarang"+id).val()) * Number($("#satuanHarga"+id).val()) ;
		$("#spanJumlahHarga"+id).text(rkaSKPD21New.formatCurrency(hasilKali));
	},
	...
	bantu : function(){
		$("#bantu").text("Rp. "+rkaSKPD21New.formatCurrency($("#hargaSatuan").val())  );
   ...
   SaveCatatan: function(id){
	...
	hitungSelisih:function(namaBantu){
		...
		$("#"+namaBantu).text("Rp. "+rkaSKPD21New.formatCurrency($("#"+idPost).val())  );
		...
		$("#selisih").val(selisih);
		$("#bantuPenjumlahan").text("Rp. "+rkaSKPD21New.formatCurrency(jumlahHargaAlokasi)  );
		$("#bantuSelisih").text("Rp. "+rkaSKPD21New.formatCurrency(selisih)  );
	},
	...
	jenisAlokasiChanged : function(id){
		...
		if($("#jenisAlokasi").val() == "BULANAN"){
			...
			$.ajax({
				...
				url: this.url+'&tipe=clearAlokasi',
				success: function(data) {
					rkaSKPD21New.formAlokasiTriwulan(id);
				}
			});
	...
	hitung: function(){
		if($("#jenisAlokasi").val() == '' ){
			...
			if($("#jenisAlokasi").val() == "BULANAN"){
				...
				rkaSKPD21New.hitungSelisih('bantuJan');
				rkaSKPD21New.hitungSelisih('bantuFeb');
				rkaSKPD21New.hitungSelisih('bantuApr');
				rkaSKPD21New.hitungSelisih('bantuMei');
				rkaSKPD21New.hitungSelisih('bantuJul');
				rkaSKPD21New.hitungSelisih('bantuAgu');
				rkaSKPD21New.hitungSelisih('bantuOkt');
				rkaSKPD21New.hitungSelisih('bantuNop');
				rkaSKPD21New.hitungSelisih('bantuMar');
				rkaSKPD21New.hitungSelisih('bantuJun');
				rkaSKPD21New.hitungSelisih('bantuSep');
				rkaSKPD21New.hitungSelisih('bantuDes');
			}else if($("#jenisAlokasi").val() == "TRIWULAN"){
				var jumlahHargaForm = $("#jumlahHargaForm").val();
				$("#jan").val('');
				$("#feb").val('');
