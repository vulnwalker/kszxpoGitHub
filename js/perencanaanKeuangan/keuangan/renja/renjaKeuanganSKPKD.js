var renjaKeuanganSKPKD = new DaftarObj2({
	prefix : 'renjaKeuanganSKPKD',
	url : 'pages.php?Pg=renjaKeuanganSKPKD', 
	formName : 'renjaKeuanganSKPKDForm',
	renjaKeuanganSKPKD_form : '0',
	loading: function(){
		this.topBarRender();
		this.filterRender();
		this.daftarRender();
		...
		if(errmsg ==''){
			...
			$.ajax({
			  ...
			  success: function(data) {
					var resp = eval('(' + data + ')');
					if(resp.err == "") {
						var IDrenjaKeuanganSKPKD = Number(resp.content.idrenjaKeuanganSKPKD);
						aForm.action= 'pages.php?Pg=renjaKeuanganSKPKD_ins&YN=1&id='+IDrenjaKeuanganSKPKD;
						aForm.target='_blank';
						aForm.submit();	
						aForm.target='';
					}else{
	...
	BidangAfter2: function(){
		renjaKeuanganSKPKD.refreshList(true);
	},
	...
	Baru: function(){	
		...
		if (err =='' ){		
			...
			if(me.renjaKeuanganSKPKD_form==0){//baru dari renjaKeuanganSKPKD
	...
	sesuai:function(id){
		...
		/*$("#span"+id).html("<input type='text' name='"+id+"' id='"+id+"' onkeyup='return renjaKeuanganSKPKD.submitKoreksi(event,id)'  value='"+$("#valueJumlah"+id).val()+"'  style='text-align: right;'>  <span id='bantu"+id+"' >"+popupBarang.formatCurrency($("#valueJumlah"+id).val())+"</span>");*/

	},
	koreksi:function(id){
		$("#span"+id).html("<input type='text' name='"+id+"' id='"+id+"' onkeyup='return renjaKeuanganSKPKD.submitKoreksi(event,id)' onkeypress='return event.charCode >= 48 && event.charCode <= 57'  style='text-align: right;'>  <span id='bantu"+id+"' ></span>");
	},
   ...
   SaveCatatan: function(id){
		...
		$.ajax({
			...
			url: this.url+'&tipe=SaveCatatan',
		  	success: function(data) {		
				...
				delElem(cover);		
				if(resp.err==''){
					if(me.renjaKeuanganSKPKD_form==0){
						me.Close();
						me.AfterSimpan();						
					}
	...
	,	
	detailrenjaKeuanganSKPKD: function(kode){	
		...
		var me = this;
			...
			$.ajax({
				type:'POST', 
				data: $('#'+this.formName).serialize()+"&renjaKeuanganSKPKD_cb%5B%5D=1.08.01.01.01" ,
				url: this.url+'&tipe=lihatrenjaKeuanganSKPKD',
			  	success: function(data) {		
					var resp = eval('(' + data + ')');	
					if (resp.err ==''){		
						document.getElementById(cover).innerHTML = resp.content;
			...
			$.ajax({
			  ...
			  success: function(data) {
					...
					if(resp.err ==''){
						...
						aForm.action= 'pages.php?Pg=renjaKeuanganSKPKD_ins&id='+ID_PLAFON+"&ID_RENJA="+ID_EDIT;
						aForm.target='_blank';
						aForm.submit();	
						aForm.target='';
					}else{
		  	...
		  	success: function(data) {		
				...
				if(resp.err==''){
					if(me.renjaKeuanganSKPKD_form==0){
						...
						me.Close();
						me.refreshComborenjaKeuanganSKPKD();
					}
			...
			$.ajax({
			  ...
			  success: function(data) {
					...
					if(resp.err !=''){
						...
						aForm.action= 'pages.php?Pg=renjaKeuanganSKPKD&tipe=Laporan';
						aForm.target='_blank';
						aForm.submit();	
						aForm.target='';
					}
