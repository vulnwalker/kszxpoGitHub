var dpaSKPD21New = new DaftarObj2({
	prefix : 'dpaSKPD21New',
	url : 'pages.php?Pg=dpaSKPD21New', 
	formName : 'dpaSKPD21NewForm',
	dpaSKPD21New_form : '0',//default js dpaSKPD21New
	loading: function(){
		//alert('loading');
		this.topBarRender();
		this.filterRender();
...
/*				me.sumHalRender();*/
	...
	unlockFindRekening: function(){
		...
		if( $("#cmbJenisRKAForm").val() == ""){
			...
			$("#findRekening").attr('onclick',"dpaSKPD21New.findRekening('"+$("#cmbJenisRKAForm").val()+"');");
	...
	koreksi:function(id){
		$("#spanVolumeBarang"+id).html("<input type='text' name='volumeBarang"+id+"' id='volumeBarang"+id+"' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='dpaSKPD21New.bantuVolumeBarang("+id+")'   style='text-align: right;'>  <span id='bantuVolumeBarang"+id+"' ></span>");
		$("#spanSatuanHarga"+id).html("<input type='text' name='satuanHarga"+id+"' id='satuanHarga"+id+"' onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='dpaSKPD21New.bantuSatuanHarga("+id+")'    style='text-align: right;'>  <span id='bantuSatuanHarga"+id+"' ></span>");
		$("#buttonSubmitKoreksi"+id).html("<img src='datepicker/save.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=dpaSKPD21New.submitKoreksi('"+id+"');></img>&nbsp  &nbsp <img src='images/administrator/images/invalid.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=dpaSKPD21New.refreshList(true);></img>");
	...
	bantuVolumeBarang : function(id){
		document.getElementById('bantuVolumeBarang'+id).innerHTML = dpaSKPD21New.formatCurrency2($("#volumeBarang"+id).val());
		var hasilKali = Number($("#volumeBarang"+id).val()) * Number($("#satuanHarga"+id).val()) ; 
		$("#spanJumlahHarga"+id).text(dpaSKPD21New.formatCurrency(hasilKali));
	},
	bantuSatuanHarga : function(id){
		document.getElementById('bantuSatuanHarga'+id).innerHTML =  dpaSKPD21New.formatCurrency($("#satuanHarga"+id).val());
		var hasilKali = Number($("#volumeBarang"+id).val()) * Number($("#satuanHarga"+id).val()) ; 
		$("#spanJumlahHarga"+id).text(dpaSKPD21New.formatCurrency(hasilKali));
	},
	...
	bantu : function(){
		$("#bantu").text("Rp. "+dpaSKPD21New.formatCurrency($("#hargaSatuan").val())  );
	...
	jenisAlokasiChanged : function(){
		...
		if($("#jenisAlokasi").val() == "BULANAN"){
			...
			$.ajax({
				...
				url: this.url+'&tipe=clearAlokasi',
				success: function(data) {	
					dpaSKPD21New.formAlokasiTriwulan();
				}
			});	
