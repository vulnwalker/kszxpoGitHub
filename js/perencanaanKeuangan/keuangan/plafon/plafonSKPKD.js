var plafonSKPKD = new DaftarObj2({
	prefix : 'plafonSKPKD',
	url : 'pages.php?Pg=plafonSKPKD', 
	formName : 'plafonSKPKDForm',
	plafonSKPKD_form : '0',//default js plafonSKPKD
	loading: function(){
		//alert('loading');
		this.topBarRender();
		this.filterRender();
...
/*				me.sumHalRender();*/
	...
	Baru: function(){	
		...
		if (err =='' ){		
			...
			if(me.plafonSKPKD_form==0){//baru dari plafonSKPKD
	...
	koreksi:function(id){
		$("#span"+id).html("<input type='text' name='"+id+"' id='"+id+"' onkeyup='return plafonSKPKD.submitKoreksi(event,id)'  onkeypress='return event.charCode >= 48 && event.charCode <= 57'  style='text-align: right;'>  <span id='bantu"+id+"' ></span>");
	},
		...
		$.ajax({
			...
			url: this.url+'&tipe=simpan',
		  	success: function(data) {		
				...
				//document.getElementById(cover).innerHTML = resp.content;
				if(resp.err==''){
					if(me.plafonSKPKD_form==0){
						me.Close();
						me.AfterSimpan();						
					}else{
						me.Close();
						barang.refreshComboplafonSKPKD();
					}
