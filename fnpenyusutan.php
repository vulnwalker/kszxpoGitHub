...
/*function susut1barang_jabar2($idbi, $thnsusutak, $blnsusutak, $tglSusut){
	...
	function genQuery($mode = 1, $fmURUSAN, $fmSKPD, $fmUNIT, $fmSUBUNIT, $fmSEKSI, $thnsusut, $blnsusut=6, $NoSesi, $KdBarang, $limit=50  ){
		...
		}elseif($Main->VERSI_NAME == 'PANDEGLANG' || $Main->VERSI_NAME == 'TASIKMALAYA_KAB'){
	...
	function genFormInformasi(){
		...
		$this->form_fields = array(		
			...
			/*'proses_penyusutan' => array(  'label'=>'Proses Perhitungan', 'value'=> $Main->SettingPenyusutan["periode_susut"][$getSP["periode_hitung"]-1][1], 'labelWidth'=>250, 'type'=>'' ),					
			'proses_penyusutan' => array(  'label'=>'Proses Perhitungan', 'value'=> $Main->SettingPenyusutan["periode_susut"][$getSP["periode_hitung"]-1][1], 'labelWidth'=>250, 'type'=>'' ),		
			'pelaporan_penyusutan' => array(  'label'=>'Pelaporan Penyusutan', 'value'=> $Main->SettingPenyusutan["dilaporkan"][$getSP["pelaporan"]-1][1], 'labelWidth'=>250, 'type'=>'' ),	
			'intra' => array(  'label'=>'Intra Disusutkan', 'value'=> $Main->SettingPenyusutan["intra"][$getSP["intra"]-1][1], 'labelWidth'=>250, 'type'=>'' ),		
			'ekstra' => array(  'label'=>'Ekstra Disusutkan', 'value'=> $Main->SettingPenyusutan["ekstra"][$getSP["ekstra"]-1][1], 'labelWidth'=>250, 'type'=>'' ),
			'all' => array(  'label'=>'Aset Lain-lain Disusutkan', 'value'=> $Main->SettingPenyusutan["aset_lainlain"][$getSP["aset_lain-lain"]-1][1], 'labelWidth'=>250, 'type'=>'' ),		
			'rb' => array(  'label'=>'Rusak Berat Disusutkan', 'value'=> $Main->SettingPenyusutan["rusak_berat"][$getSP["rusak_berat"]-1][1], 'labelWidth'=>250, 'type'=>'' ),
			'kibe' => array(  'label'=>'Kib E Disusutkan', 'value'=> $Main->SettingPenyusutan["kibE"][$getSP["kib_e"]-1][1], 'labelWidth'=>250, 'type'=>'' ),		
			'atb' => array(  'label'=>'Aset Tidak Berwujud Disusutkan', 'value'=> $Main->SettingPenyusutan["ATB"][$getSP["atb"]-1][1], 'labelWidth'=>250, 'type'=>'' ),
			'penambahan_manfaat' => array(  'label'=>'Menambah Masa manfaat', 'value'=> $Main->SettingPenyusutan["penambahan_manfaat"][$getSP["penambahan_masa_manfaat"]-1][1], 'labelWidth'=>250, 'type'=>'' ),		
			'tahun_berlaku' => array(  'label'=>'Tahun Berlaku Menambah Masa Manfaat', 'value'=> $getSP["tahun_berlaku"], 'labelWidth'=>250, 'type'=>'' ),
			'persentase' => array(  'label'=>'Persentase perhitungan terhadap', 'value'=> $Main->SettingPenyusutan["persentase"][$getSP["persentase"]-1][1], 'labelWidth'=>250, 'type'=>'' ),		
			'tabel' => array(  'label'=>'Persentase perhitungan diambil dari tabel', 'value'=> $Main->SettingPenyusutan["tabel_masa_manfaat"][$getSP["tabel_masa_manfaat"]-1][1], 'labelWidth'=>250, 'type'=>'' ),
			'aturan' => array(  'label'=>'Aturan Penambahan Masa Manfaat', 'value'=> "<ul><li>".$Main->SettingPenyusutan["aturan_manfaat"][$getSP["aturan_masa_manfaat1"]-1][1]."</li><li>".
			$Main->SettingPenyusutan["aturan_manfaat"][$getSP["aturan_masa_manfaat2"]-1][1]."</li><li>".
			$Main->SettingPenyusutan["aturan_manfaat"][$getSP["aturan_masa_manfaat3"]-1][1]."</li></ul>"
			, 'labelWidth'=>250, 'labelparam'=> "valign=top",	'type'=>'' ),	*/			
		);
	...
	function genFormKoreksi(){
		...
		if($Main->VERSI_NAME=="SERANG" || ($Main->VERSI_NAME=="JABAR" && $bi['thn_perolehan'] <=2014) ||$Main->VERSI_NAME=="TASIKMALAYA_KAB" || $Main->VERSI_NAME=="PANDEGLANG" || ($Main->VERSI_NAME=="CIREBON_KAB" && $bi['thn_perolehan'] <=2014)){
