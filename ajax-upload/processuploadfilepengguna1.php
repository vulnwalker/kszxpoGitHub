<?php
	...
	$err ='';
	$content = '';
	$fileName = $_FILES["ImageFile"]["name"]; // The file name
	$fileTmpLoc = $_FILES["ImageFile"]["tmp_name"]; // File in the PHP tmp folder
	$fileType = $_FILES["ImageFile"]["type"]; // The type of file it is
	$fileSize = $_FILES["ImageFile"]["size"]; // File size in bytes
	$fileErrorMsg = $_FILES["ImageFile"]["error"]; // 0 for false... and 1 for true
	...
	if(move_uploaded_file($fileTmpLoc, "Media/pengguna/$fileNewName")){
		...
		$qryubah = "UPDATE gambar_upload SET stat = '2' WHERE id_upload='$peng_id' and jns_upload='6'"; $cek .= " || ".$qryubah;
	...
	//	$qry = "INSERT INTO gambar_uploadmenu_aktif (mnu_id, nmfile, nmfile_asli, stat, uid, tgl_create) values ('$mnu_id','$nmfile', '$nmfile_asli', '1', '$uid', NOW())";	$cek .= $qry;	
