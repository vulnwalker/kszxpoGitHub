...
global $Main,$HTTP_COOKIE_VARS;
	...
	$sqry="select aa.f, aa.g,  aa.nm_barang,
			...
			tt.jmlHrgPenilaian_awal,uu.jmlHrgPenilaian_akhir
		...
		"from v_ref_kib_keu_h aa 
			...
			"left join (select IFNULL(f,'00') as f , IFNULL(g,'00') as g, sum(nilai_barang - nilai_barang_asal) as jmlHrgPenilaian_awal from penilaian where $Kondisi and tgl_penilaian < '$tglAwal'   group by f,g with rollup ) tt on aa.f=tt.f  and aa.g=tt.g  ".
			"left join (select IFNULL(f,'00') as f , IFNULL(g,'00') as g, sum(nilai_barang - nilai_barang_asal) as jmlHrgPenilaian_akhir from penilaian where $Kondisi and tgl_penilaian <= '$tglAkhir'   group by f,g with rollup ) uu on aa.f=uu.f  and aa.g=uu.g ".			
		...
		while($isi=mysql_fetch_array($Qry)){
			...
			//penilaian			
			$jmlHrgPenilaian_akhir = $isi['jmlHrgPenilaian_akhir'];   
       ...
       $kdBidang = $isi['g'] == "00" ? "" : $isi['g'];
		...
		if ($isi['g'] == "22")
		{
			...
			if($Main->VERSI_NAME == 'BDG_BARAT' ){
				//kredit tgr karena pembayaran
				$aqry = "select  sum(bayar ) as jmlHrgBI_akhir from gantirugi_bayar aa
					left join buku_induk bb on aa.id_bukuinduk = bb.id
					where $Kondisi and tgl <= '$tglAkhir'  "; //echo $aqry;
				$get= mysql_fetch_array( mysql_query($aqry));  
				$jmlbayar = $get['jmlHrgBI_akhir'];
			}
			
			//$get= mysql_fetch_array( mysql_query("select sum(jml_barang) as jmlBrgBI_akhir, sum(jml_harga ) as jmlHrgBI_akhir from buku_induk 
			//where  $Kondisi and staset=6  and tgl_buku <= '$tglAkhir'  "));  
			
			$get= mysql_fetch_array( mysql_query("select sum(jml_barang) as jmlBrgBI_akhir, sum(jml_harga ) as jmlHrgBI_akhir from v1_gantirugi 
			where  $Kondisi and staset=6  and tgl_gantirugi <= '$tglAkhir'  "));  			
			$jmlBrg_akhir=$get['jmlBrgBI_akhir'];
			$jmlHrg_akhir=$get['jmlHrgBI_akhir'] - $jmlbayar;
			$jmlHrg_akhir_tot=$jmlHrg_akhir_tot+$jmlHrg_akhir;		
			$jmlBrg_akhir_tot=$jmlBrg_akhir_tot+$jmlBrg_akhir;		
