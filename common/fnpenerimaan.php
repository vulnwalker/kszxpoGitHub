...
class PenerimaanObj extends DaftarObj2{
	...
	var $SHOW_CEK = TRUE;	
	var $TblName = 'v1_penerimaan';//'v1_rkb'
	var $TblName_Hapus = 'penerimaan';
	var $TblName_Edit = 'penerimaan';
	...
	global $Main;
		...
		if ( $Main->VERSI_NAME == 'JABAR') {
			...
			<tr>
			<td class="menudottedline" width="60%" height="20" style="text-align:right"><b>
			<a href="pages.php?Pg=penerimaan" style="color:blue;" title="Daftar Penerimaan Barang Milik Daerah">Penerimaan</a>  |  
			<a href="pages.php?Pg=pengeluaran" title="Daftar Pengeluaran Barang Milik Daerah">Pengeluaran</a>  |
	...
	function setMenuEdit(){		
		return
			"<td>".genPanelIcon("javascript:penerimaan.Baru2()","new_f2.png","Baru",'Penerimaan Baru')."</td>".
			"<td>".genPanelIcon("javascript:penerimaan.Edit2()","edit_f2.png","Edit", 'Edit Penerimaan')."</td>".
			"<td>".genPanelIcon("javascript:penerimaan.Hapus()","delete_f2.png","Hapus", 'Hapus Penerimaan')."</td>";
			/*
	...
	function setKolomHeader($Mode=1, $Checkbox=''){
		...
		$headerTable =
			"<thead>
				...
				<th class='th01' width='' rowspan=2>Unit Kerja</th>				
	...
	function setKolomData($no, $isi, $Mode, $TampilCheckBox){
		...
		*/
		...
		$Koloms[] = array("align='center'",  TglInd($isi['tgl_penerimaan']) );
		$Koloms[] = array("align=''", $isi['supplier']);
		
		$Koloms[] = array("align='center'",  TglInd($isi['faktur_tgl'] ) );		
		$Koloms[] = array("align=''",  $isi['faktur_no']  );
	...
	function genDaftarOpsi(){
		...
		$arrOrder = array(
			...
			//get selectbox Order 1
			$fmORDER1 = cekPOST('fmORDER1');
			$fmDESC1 = cekPOST('fmDESC1');
			
			//get selectbox Order 2
			$fmORDER2 = cekPOST('fmORDER2');
			$fmDESC2 = cekPOST('fmDESC2');
			
			//get selectbox Order 3
	...
	function setFormEdit(){		
		...
		*/
		...
		$get = mysql_fetch_array(mysql_query(
			"select * from pengadaan where id='".$dt['id_pengadaan']."'  "
		));
		$dt['nama_perusahaan_dpb'] = $get['nama_perusahaan'];
		$dt['spk_tgl_dpb'] = $get['spk_tgl'];
		$dt['spk_no_dpb'] = $get['spk_no'];
		$dt['jmldpb'] = $get['jml_barang']==NULL || $get['jml_barang']==''?0 : $get['jml_barang'];
		
		//ambil penerimaan sebelumnya ----------------------------
		$aqry = "select sum(jml_barang) as tot from $this->TblName where id_pengadaan='".$dt['id_pengadaan']."' and id<>'".$this->form_idplh."' ";
		$get =  mysql_fetch_array( mysql_query(
			$aqry//"select sum(jml_barang) as tot from $this->TblName where id_pengadaan='".$_REQUEST['id_pengadaan']."' and id<>'".$this->form_idplh."' "
		));
		
		$dt['jmlada'] = $get['tot'] == NULL || $get['tot'] == ''? 0: $get['tot'];
		
		//hit sisa ----------------------------------------------
		$dt['jmlsisa'] = $dt['jmldpb'] - $dt['jmlada'] - $dt['jml_barang'];			
		$dt['cek'].=$aqry;
		
		
		//-- get pegawai
		$pgw = mysql_fetch_array(mysql_query("select * from ref_pegawai where nip ='".$dt['nip_penyimpan']."'"));
		$dt['nip_penyimpan'] = $pgw['nip'];
		$dt['penyimpan'] = $pgw['nama'];
		$dt['jbt_penyimpan'] = $pgw['jabatan'];
		$pgw = mysql_fetch_array(mysql_query("select * from ref_pegawai where nip ='".$dt['nip_penerima']."'"));
		$dt['nip_penerima'] = $pgw['nip'];
		$dt['penerima'] = $pgw['nama'];
		$dt['jbt_penerima'] = $pgw['jabatan'];
		$pgw = mysql_fetch_array(mysql_query("select * from ref_pegawai where nip ='".$dt['nip_pemeriksa']."'"));
		$dt['nip_pemeriksa'] = $pgw['nip'];
		$dt['pemeriksa'] = $pgw['nama'];
		$dt['jbt_pemeriksa'] = $pgw['jabatan'];
	...
	function setForm($dt){	
		...
		$vkdbarang = //$this->form_fmST==1?	$fmIDBARANG.' - '.$dt['nm_barang'] :			
			cariDPB("adminForm","pages/01/caridpb1.php","pages/01/caridpb2.php",
					"fmIDBARANG",
					"fmNMBARANG",
		...
		$vGudang = 
			...
			"<input type='button' onclick=\"document.getElementById('nm_gudang').value='';document.getElementById('id_gudang').value='';\" value='Clear'>".
			"<input type='hidden' id='id_gudang' name='id_gudang' value='".$dt['id_gudang']."'>";		 
			
		
		$title = $this->form_fmST == 1? 'Penerimaan Barang Milik Daerah - Edit' : 'Penerimaan Barang Milik Daerah - Baru';
		$this->form_fields = array(	
			...
			'jml_dpb' => array(  'label'=>'Jumlah Pengadaan', 
				'value'=> 
					"<input name=\"jml_dpb\" id='jml_dpb' type=\"text\" value='".$dt['jmldpb']."' readonly='true' />"					
				,'type'=>'' 
			),	
			'jml_ada' => array(  'label'=>'Penerimaan Sebelumnya', 
				'value'=> 
					"<input name=\"jml_ada\" id='jml_ada' type=\"text\" value='".$dt['jmlada']."' readonly='true' />"					
				,'type'=>'' 
			),				
			'jml_barang' => array(  'label'=>'Jumlah Penerimaan Barang', 
				'value'=> 
					"<input name=\"jml_barang\" id='jml_barang' type=\"text\" value='".$dt['jml_barang']."' onblur='penerimaan.hitungSisa()' />".
					" Satuan ".
			...
			'tgl_penerimaan' => array('label'=>'Tanggal Terima', 
				'value'=> createEntryTgl('tgl_penerimaan',$dt['tgl_penerimaan'] )					
				, 'type'=>''
			),
			...
			'penerima' => array(  
				...
				'value'=> 
					"<input type='hidden' id='ref_idpenerima' name='ref_idpenerima' value='".$dt['ref_idpenerima']."'> ".
					"<input type='text' id='penerima' name='penerima' readonly=true value='".$dt['penerima']."' style='width:250'> &nbsp; ".
					"NIP  &nbsp;<input type='text' id='nip_penerima' name='nip_penerima' readonly=true value='".$dt['nip_penerima']."' style='width:150' > ".					
					"<input type='button' value='Pilih' onclick=\"".$this->Prefix.".pilihPenerima()\">".
			"<input type='button' onclick=\"document.getElementById('penerima').value='';document.getElementById('nip_penerima').value='';document.getElementById('ref_idpenerima').value='';document.getElementById('jbt_penerima').value='';\" value='Clear'>"					
				,
				'type'=>'' 
			), 	
			'jbt1' => array(  'label'=>'', 
				'value'=> "JABATAN  &nbsp;<input type='text' id='jbt_penerima' name='jbt_penerima' readonly=true value='".$dt['jbt_penerima']."' style='width:380'> ",  
				'type'=>'' , 'pemisah'=>' '
			),
			...
			'penyimpan' => array(  
				'label'=>'Petugas Penyimpan', 
				'value'=> 
					"<input type='hidden' id='ref_idpenyimpan' name='ref_idpenyimpan' value='".$dt['ref_idpenyimpan']."'> ".
					"<input type='text' id='penyimpan' name='penyimpan' readonly=true value='".$dt['penyimpan']."' style='width:250'> &nbsp; ".
					"NIP  &nbsp;<input type='text' id='nip_penyimpan' name='nip_penyimpan' readonly=true value='".$dt['nip_penyimpan']."' style='width:150' > ".					
					"<input type='button' value='Pilih' onclick=\"".$this->Prefix.".pilihPenyimpan()\">".
			"<input type='button' onclick=\"document.getElementById('penyimpan').value='';document.getElementById('nip_penyimpan').value='';document.getElementById('ref_idpenyimpan').value='';document.getElementById('jbt_penyimpan').value='';\" value='Clear'>"					
				,
				'type'=>'' 
			), 	
			'jbt2' => array(  'label'=>'', 
				'value'=> "JABATAN  &nbsp;<input type='text' id='jbt_penyimpan' name='jbt_penyimpan' readonly=true value='".$dt['jbt_penyimpan']."' style='width:380'> ",  
				'type'=>'' , 'pemisah'=>' '
			),
			/*
			...
			*/
			...
			'ba' => array('label'=>'', 'value'=>'Berita Acara Pemeriksaan', 'type'=>'merge','row_params'=>" height='21'"),
			'ba_tgl' => array('label'=>'&nbsp;&nbsp;Tanggal', 
				'value'=> createEntryTgl('ba_tgl',$dt['ba_tgl'] ), 'type'=>''
			),
			'ba_no' => array('label'=>'&nbsp;&nbsp;Nomor', 
				'value'=>$dt['ba_no'], 'type'=>'text'
			),
			...
			'pemeriksa' => array(  
				...
				'value'=> 
					"<input type='hidden' id='ref_idpemeriksa' name='ref_idpemeriksa' value='".$dt['ref_idpemeriksa']."'> ".
					"<input type='text' id='pemeriksa' name='pemeriksa' readonly=true value='".$dt['pemeriksa']."' style='width:250'> &nbsp; ".
					"NIP  &nbsp;<input type='text' id='nip_pemeriksa' name='nip_pemeriksa' readonly=true value='".$dt['nip_pemeriksa']."' style='width:150' > ".					
					"<input type='button' value='Pilih' onclick=\"".$this->Prefix.".pilihPemeriksa()\">".
			"<input type='button' onclick=\"document.getElementById('pemeriksa').value='';document.getElementById('nip_pemeriksa').value='';document.getElementById('ref_idpemeriksa').value='';document.getElementById('jbt_pemeriksa').value='';\" value='Clear'>"					
					
				,
				'type'=>'' 
			), 	
			'jbt3' => array(  'label'=>'', 
				'value'=> "JABATAN  &nbsp;<input type='text' id='jbt_pemeriksa' name='jbt_pemeriksa' readonly=true value='".$dt['jbt_pemeriksa']."' style='width:380'> ",  
				'type'=>'' , 'pemisah'=>' '
			),
	...
	*/
	...
	function simpan2(){
		...
		$get = mysql_fetch_array( mysql_query(
			"select count(*)as cnt, sum(jml_barang) as tot from pengadaan where id='$id_pengadaan'"
		));	
		$jmldpb = $get['tot']; 
		//-- cek kebutuhan ada
		if( $err=='' && ($get['cnt'] == NULL || $get['cnt']=='' )  ) $err = "Pengadaan Barang $fmIDBARANG untuk Tahun Anggaran $tahun Tidak Ada!";
		
		//-- get penerimaan sblumnya utk idpengadaan, selain ini
		$get =  mysql_fetch_array( mysql_query(
			"select sum(jml_barang) as tot from $this->TblName where id_pengadaan='".$_REQUEST['id_pengadaan']."' and id<>'$id' "
		));
		$jmlada = $get['tot'];
		...
		if($err==''){
			...
			if($jmlsisa<0) $err="Gagal Simpan, Jumlah Penerimaan lebih besar dari Pengadaan!";
		}
		
		//-- cek sudah Pengeluaran
		if($err==''){
			$aqry = "select count(*) as cnt from pengeluaran where tahun='".$old['tahun']."' ".
				" and c='".$old['c']."' and d='".$old['d']."' and e='".$old['e']."' and e1='".$old['e1']."' ".
				" and f='".$old['f']."' and g='".$old['g']."' and h='".$old['h']."' and i='".$old['i']."' and j='".$old['j']."' "; $cek .= $aqry;
			$pengeluaran = mysql_fetch_array(mysql_query( $aqry	));
			if( $pengeluaran['cnt'] > 0 ) $err='Gagal Simpan! Data Sudah Ada di Pengeluaran!';
		}
		...
		if($err==''){
			$get = $this->simpanData(
				...
				array(
					...
					'nip_penerima'=>"'".$_REQUEST['nip_penerima']."'",
					...
					'faktur_tgl'=>"'".$_REQUEST['faktur_tgl']."'",
					'faktur_no'=>"'".$_REQUEST['faktur_no']."'",
					'f'=>"'".$f."'",
					'g'=>"'".$g."'",
					'h'=>"'".$h."'",
					'i'=>"'".$i."'",
	...
	function getJmlDPB($idpengadaan){
		$aqry = "select * from pengadaan where id='$idpengadaan'";
		$get =  mysql_fetch_array(mysql_query($aqry));
		return $get['jml_barang'];
	}
	function getPengadaan($idpengadaan){
		$aqry = "select * from pengadaan where id='$idpengadaan'";
		$get =  mysql_fetch_array(mysql_query($aqry));
		return $get;
	}
	function getPenerimaanSebelumnya($idpengadaan){
		$idstr = $id==''? '' : " and id<>'$id' ";
		$aqry = "select sum(jml_barang) as tot from penerimaan where ".
				" id_pengadaan='$idpengadaan' "; $cek.=$aqry;
		$get = mysql_fetch_array( mysql_query(
			$aqry
		));	
		return $get['tot'];
	}	
	function hitPenerimaanSebelumnya(){
		$cek=''; $err=''; $content='';
		...
		$j = $fmIDBARANG['12'].$fmIDBARANG['13'].$fmIDBARANG['14'];	
		$id = $_REQUEST['id'];*/
		$idpengadaan = $_REQUEST['idpengadaan'];
				
		$content->jmlada = $this->getPenerimaanSebelumnya( $idpengadaan );		
		$content->jmldpb = $this->getJmlDPB( $idpengadaan ); 
		if($content->jmlada == NULL) $content->jmlada=0;
		if($content->jmldpb == NULL) $content->jmldpb =0;
		
		$content->jml = $content->jmldpb - $content->jmlada;
		$content->jmlsisa = 0;
		
		//ambil data pengadaan
		$get = $this->getPengadaan($idpengadaan);
	...
	function set_selector_other($tipe){
		...
		switch($tipe){
			...
			case 'hitPenerimaanSebelumnya': {
				$get = $this->hitPenerimaanSebelumnya();
				$cek = $get['cek']; $err = $get['err']; $content=$get['content']; 
				break;
			}		
		}
