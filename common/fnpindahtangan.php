...
class PindahTanganObj extends DaftarObj{	
	...
	function getDaftarOpsi($Mode=1){
		global $Main,$HTTP_COOKIE_VARS;// $fmUNIT,$fmSKPD, $fmSUBUNIT;
		...
		$arrKondisi[] = !empty($_POST['thn_pemindahtangan'])? " year(tgl_pemindahtanganan) = '".$_POST['thn_pemindahtangan']."' ":" year(tgl_pemindahtanganan) = '$thn_login' ";
		
		$SSPg = !empty($_GET[SSPg])? $_GET['SSPg']: $_POST['fmSSPg'];
		$fmcr_data = $_REQUEST['fmcr_data'];
		$fmPILCARIvalue = $_REQUEST['fmPILCARIvalue'];
		$vf04 = $Main->KD_BARANG_P108?"concat(f1,'.',f2,'.',f)= '1.3.01'":"f='01'";
		$vf05 = $Main->KD_BARANG_P108?"concat(f1,'.',f2,'.',f)= '1.3.02'":"f='02'";
		$vf06 = $Main->KD_BARANG_P108?"concat(f1,'.',f2,'.',f)= '1.3.03'":"f='03'";
		$vf07 = $Main->KD_BARANG_P108?"concat(f1,'.',f2,'.',f)= '1.3.04'":"f='04'";
		$vf08 = $Main->KD_BARANG_P108?"concat(f1,'.',f2,'.',f)= '1.3.05'":"f='05'";
		$vf09 = $Main->KD_BARANG_P108?"concat(f1,'.',f2,'.',f)= '1.3.06'":"f='06'";
		$vf10 = $Main->KD_BARANG_P108?"concat(f1,'.',f2,'.',f)= '1.5.03'":"f='07'";
		switch($SSPg){
			case '04': $arrKondisi[] =$vf04; break;
			case '05': $arrKondisi[] =$vf05; break;
			case '06': $arrKondisi[] =$vf06; break;
			case '07': $arrKondisi[] =$vf07; break;
			case '08': $arrKondisi[] =$vf08; break;
			case '09': $arrKondisi[] =$vf09; break;				
			case '10': $arrKondisi[] =$vf10; break;				
		}		
		if (!empty($_POST['fmIDBARANGCARI'])) $arrKondisi[] = " id_bukuinduk like '%".$_POST['fmIDBARANGCARI']."%'";
		if (!empty($_POST['fmFiltThnBuku'])) $arrKondisi[] = " year(tgl_buku) = '".$_POST['tgl_buku']."'";
		$concatkd = $Main->KD_BARANG_P108?"f1,'.',f2,'.',f,'.',g,'.',h,'.',i,'.',j":"f,'.',g,'.',h,'.',i,'.',j";
		
		if(!empty($fmcr_data)){
			switch($fmcr_data){			
				case 'selectKode': $arrKondisi[] .= " concat($concatkd) like '%$fmPILCARIvalue%' "; break;
				case 'selectNama': $arrKondisi[] .= " alamat like '%$fmPILCARIvalue%'"; break;								 	
			}
			
		}
		//setWilSKPD();	
		$fmURUSAN = $Main->URUSAN==1?$_POST['fmURUSAN']:"";
		$fmUNIT = $_POST['fmUNIT'];
		$fmSKPD = $_POST['fmSKPD'];
		$fmSUBUNIT = $_POST['fmSUBUNIT'];
		$fmSEKSI = $_POST['fmSEKSI'];
		if (!empty($fmURUSAN) && $fmURUSAN!='0' ) $arrKondisi[] = " c1 = '".$fmURUSAN."'";
	...
	function genDaftarOpsi(){
		...
		//filter ------------------------------------------------
		$SSPg = !empty($_GET[SSPg])? $_GET['SSPg']: $_POST['fmSSPg'];
		$vf04 = $Main->KD_BARANG_P108?"and concat(f1,'.',f2,'.',f)= '1.3.01'":"and f='01'";
		$vf05 = $Main->KD_BARANG_P108?"and concat(f1,'.',f2,'.',f)= '1.3.02'":"and f='02'";
		$vf06 = $Main->KD_BARANG_P108?"and concat(f1,'.',f2,'.',f)= '1.3.03'":"and f='03'";
		$vf07 = $Main->KD_BARANG_P108?"and concat(f1,'.',f2,'.',f)= '1.3.04'":"and f='04'";
		$vf08 = $Main->KD_BARANG_P108?"and concat(f1,'.',f2,'.',f)= '1.3.05'":"and f='05'";
		$vf09 = $Main->KD_BARANG_P108?"and concat(f1,'.',f2,'.',f)= '1.3.06'":"and f='06'";
		$vf10 = $Main->KD_BARANG_P108?"and concat(f1,'.',f2,'.',f)= '1.5.03'":"and f='07'";
		switch($SSPg){
			case '04': $KondisiKib =$vf04; break;
			case '05': $KondisiKib =$vf05; break;
			case '06': $KondisiKib =$vf06; break;
			case '07': $KondisiKib =$vf07; break;
			case '08': $KondisiKib =$vf08; break;
			case '09': $KondisiKib =$vf09; break;	
			case '10': $KondisiKib =$vf10; break;	
			default : $KondisiKib =''; break; 
		}	
		$bentuk = $_REQUEST['bentuk'];
		$filetthn_pindahtangan =comboBySQL2('fmFiltThnHapus', $_POST['fmFiltThnHapus'],
		...
		$TampilOpt =	
			...
			"<table width=\"100%\" height=\"100%\" class=\"adminform\" style='margin:4 0 0 0'>
				<tr valign=\"top\"> 		
					<td> 
					&nbsp;&nbsp Tampilkan : &nbsp;&nbsp".
					cmbQuery('fmFiltThnBuku', $fmFiltThnBuku,
						"select year(tgl_buku)as thnbuku,year(tgl_buku)as thnbuku from v1_pindahtangan where tgl_buku!='' $KondisiKib group by thnbuku desc",
						'', 'Semua Thn. Buku', ''
					).
					"&nbsp;&nbsp Tahun <input type=text name='thn_pemindahtangan' value='$thn_login' size='4'> ".
					$ListOrderBy.
	...
	function setKolomData($no, $isi, $Mode, $TampilCheckBox){
		...
		$get = mysql_fetch_array(mysql_query(
			"select * from ref_skpd where $vc1 c='{$isi['c']}' and d='00' and e='00'"
		));
		$namapengguna .= $get['nm_skpd'].' - ';
		$get = mysql_fetch_array(mysql_query(
			"select * from ref_skpd where $vc1 c='{$isi['c']}' and d='{$isi['d']}' and e='00'"
		));
		$namapengguna .= $get['nm_skpd'].' - ';
		$get = mysql_fetch_array(mysql_query(
			"select * from ref_skpd where $vc1 c='{$isi['c']}' and d='{$isi['d']}' and e='{$isi['e']}' and e1='$kdSubUnit0'"
		));
		$namapengguna .= $get['nm_skpd'];
		$get = mysql_fetch_array(mysql_query(
			"select * from ref_skpd where $vc1 c='{$isi['c']}' and d='{$isi['d']}' and e='{$isi['e']}' and e1='{$isi['e1']}'"
		));
		$namapengguna .= $get['nm_skpd'];
	...
	function genToolbarBawah(){
		...
		$PnlPenghapusan = $Main->MODUL_PENGAPUSAN ?"<table width=\"70\"><tr><td>" . 
						PanelIcon3("javascript:penghapusan_ins.penghapusanbaru(2)", "PENGHAPUSAN", $ridModul09, $disModul09,'',2,90) . 
					"</td></tr></table>":"";
