...
class MapObj{
	...
	function getdata(){
		...
		$fmURUSAN = $_REQUEST['MapSkpdfmURUSAN'];
		$fmSEKSI = $_REQUEST['MapSkpdfmSEKSI'];
		$tampilBidang=$_REQUEST['tampilbidang'];
				
		$Limit = '';//" limit 0,10 ";
		//table -----------------------------------------------------------------------------------------
		$kib = $_REQUEST['kib'];
		switch ($kib){
			case '01':	$tbl = 'view_kib_a'; $fieldId = 'idbi';	break;	
			case '03':	$tbl = 'view_kib_c'; $fieldId = 'idbi';	break;	
			case '04':	$tbl = 'view_kib_d'; $fieldId = 'idbi';	break;	
			case '06':	$tbl = 'view_kib_f'; $fieldId = 'idbi';	break;
			default : 'view_buku_induk2'; $fieldId = 'id'; break;	
		}
		...
		//$arrkond[] = " koordinat_gps<>'' ";		
		if(!empty($fmURUSAN) && $fmURUSAN!='00') $arrkond[] = " c1='$fmURUSAN' ";
		...
		if(!empty($fmSEKSI) && $fmSEKSI!='000') $arrkond[] = " e1='$fmSEKSI' ";
...
*/
		...
		$selSertifikat = $_REQUEST['selSertifikat'];
		if(!empty($selSertifikat)) {
			if($selSertifikat == 3){
				$arrkond[] = " ( bersertifikat='' or bersertifikat is null )  ";
			}else{
				$arrkond[] = " bersertifikat='$selSertifikat'  ";
			}
...
//		$cek=$aqry;
		while($isi=mysql_fetch_array($qry)){
			...
			$rows[] = array(
				'id'=>$isi[$fieldId], 
				'koord'=>preg_replace( '/\s*/m', '', $isi['koordinat_gps'] ), 
				'area'=>preg_replace( '/\s*/m', '', $isi['koord_bidang'] ) ,
				//'info'=>$isi['idbi']
