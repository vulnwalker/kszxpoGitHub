...
class KoreksiObj  extends DaftarObj2{	
	...
	var $fieldSum_lokasi = array( 8,9,12,13,14);
	...
	function Simpan(){
		...
		if($get_ba['tgl_ba']=='0000-00-00' or $get_ba['tgl_ba']=='' or $get_ba['tgl_ba']==NULL){
			$gentgl_perolehan = $bi['thn_perolehan'].'-'.date("m", strtotime($bi['tgl_buku'])).'-'.date("d",strtotime($get_cd['tgl_buku']));			
		}else{
		...
		if($err=='' && $tgl_perolehan<$gentgl_perolehan)$err ='Tanggal Perolehan tidak lebih kecil dari Tanggal BAST/Buku Barang !';
				
		switch ($fmST){
			case 0 : { //baru	
				if($err=='' &&$fmst==1 && $bi['status_barang'] != 1 ) $err= "Hanya Barang Inventaris yang bisa Koreksi!";
				$oldmaxtgl = mysql_fetch_array(mysql_query("select max(tgl_perolehan) as maxtgl from t_koreksi where idbi_awal='".$bi['idawal']."'"));
			...
			case 1 : { //edit
				$old_koreksi = mysql_fetch_array(mysql_query("select * from t_koreksi where id='$idplh'"));
			//$err='tgl_='.$old_koreksi['tgl'];
			//cek tgl buku lama <= tgl closing
			if($err=='' && sudahClosing($tgl,$bi['c'],$bi['d'],$bi['e'],$bi['e1'],$bi['c1'])){
				if($err=='' && $old_koreksi['tgl']!=$tgl)$err = 'Tanggal Koreksi tidak bisa di edit,karena sudah closing !';
				if($err=='' && $old_koreksi['tgl_perolehan']!=$tgl_perolehan)$err = 'Tanggal Perolehan tidak bisa di edit,karena sudah closing !';
				if($err=='' && $old_koreksi['harga_baru']!=$harga_baru)$err = 'Harga Perolehan Baru tidak bisa di edit,karena sudah closing !';
				}			
			//cek tgl buku lama < tgl susut akhir 
			if($err=='' && $old_koreksi['tgl']<$tgl_susutAkhir['tgl']){
				if($err=='' && $old_koreksi['tgl']!=$tgl)$err = 'Tanggal Koreksi tidak bisa di edit,sudah ada penyusutan !';
				if($err=='' && $old_koreksi['tgl_perolehan']!=$tgl_perolehan)$err = 'Tanggal Perolehan tidak bisa di edit,sudah ada penyusutan !';
				if($err=='' && $old_koreksi['harga_baru']!=$harga_baru)$err = 'Harga Perolehan Baru tidak bisa di edit,sudah ada penyusutan !';
				}
			...
			if(($err=='' && $old_koreksi['tgl']<$tgl_korAkhir['tgl']) || ($old_koreksi['tgl']==$tgl_korAkhir['tgl'] && $old_koreksi['tgl_create']<$tgl_korAkhir['tgl_create'])){
				if($err=='' && $old_koreksi['tgl']!=$tgl)$err = 'Tanggal Koreksi tidak bisa di edit,sudah ada koreksi harga !';
				if($err=='' && $old_koreksi['tgl_perolehan']!=$tgl_perolehan)$err = 'Tanggal Perolehan tidak bisa di edit,sudah ada koreksi harga !';
				if($err=='' && $old_koreksi['harga_baru']!=$harga_baru)$err = 'Harga Perolehan Baru tidak bisa di edit,sudah ada koreksi harga !';
				}
			...
			if(($err=='' && $old_koreksi['tgl']<$tgl_nilaiAkhir['tgl_penilaian']) || ($old_koreksi['tgl']==$tgl_nilaiAkhir['tgl_penilaian'] && $old_koreksi['tgl_create']<$tgl_nilaiAkhir['tgl_create'])){
				if($err=='' && $old_koreksi['tgl']!=$tgl)$err = 'Tanggal Koreksi tidak bisa di edit,sudah ada penilaian !';
				if($err=='' && $old_koreksi['tgl_perolehan']!=$tgl_perolehan)$err = 'Tanggal Perolehan tidak bisa di edit,sudah ada penilaian !';
				if($err=='' && $old_koreksi['harga_baru']!=$harga_baru)$err = 'Harga Perolehan Baru tidak bisa di edit,sudah ada penilaian !';
				}
			...
			if(($err=='' && $old_koreksi['tgl']<$tgl_hpsAkhir['tgl_penghapusan']) || ($old_koreksi['tgl']==$tgl_hpsAkhir['tgl_penghapusan'] && $old_koreksi['tgl_create']<$tgl_hpsAkhir['tgl_create'])){
				if($err=='' && $old_koreksi['tgl']!=$tgl)$err = 'Tanggal Koreksi tidak bisa di edit,sudah ada penghapusan sebagian !';
				if($err=='' && $old_koreksi['tgl_perolehan']!=$tgl_perolehan)$err = 'Tanggal Perolehan tidak bisa di edit,sudah ada penghapusan sebagian !';
				if($err=='' && $old_koreksi['harga_baru']!=$harga_baru)$err = 'Harga Perolehan Baru tidak bisa di edit,sudah ada penghapusan sebagian !';
				}	
			//cek (tgl tgl_perolehan >= tgl ba buku_induk 
			if($err=='' && $old_koreksi['tgl_perolehan']<$gentgl_perolehan ){
				if($err=='' && $old_koreksi['tgl_perolehan']!=$tgl_perolehan)$err ='Tanggal Perolehan tidak lebih kecil dari Tanggal BAST/Buku Barang !';
			}	
				if($err ==''){
						//ambil id buku induk
						...
						if ($err =='' && $penatausaha['status_barang']==3 ) $err = "Gagal Edit. Barang sudah dihapuskan!";
						if ($err =='' && $penatausaha['status_barang']==4 ) $err = 'Gagal Edit. Barang sudah dipindah tangankan!';
						if ($err =='' && $penatausaha['status_barang']==5 ) $err = 'Gagal Edit. Barang sudah diganti rugi!';
						//if ($err =='' && $penatausaha['staset']!=$old['staset'] ) $err = 'Gagal Edit. Status Aset Barang sudah berubah!';
						...
						if($err=='' && $oldmax['cnt']>0 && $idplh<$oldmax['maxid']  ) $err = 'Hanya data koreksi terakhir yang bisa di edit!';			
						$oldmax = mysql_fetch_array(mysql_query("select max(tgl) as maxtgl, max(id) as maxid, count(*) as cnt from t_koreksi where idbi_awal='".$penatausaha['idawal']."' and id< $idplh"));
						if($err=='' && $oldmax['cnt']>0 && compareTanggal($tgl, $oldmax['maxtgl'])==0   ) $err = 'Tanggal koreksi tidak lebih kecil dari koreksi sebelumnya!';			
	...
	$tglShow=TRUE, $withBtClear = TRUE,$jns=""){
	//global $$elName, 
	//global $Ref;//= 'entryTgl';
	...
	$entrytgl = $tglShow?
		'<div  style="float:left;padding: 0 4 0 0">' . 
			...
			genCombo_tgl(
				...
				$tgl[2],
				'', 
				" $dis ".'   onchange="'.$this->Prefix.'.TglEntry_createtgl(\'' . $elName . '\',\''.$jns.'\')"').
		'</div>'
		: '';
	$btClear =  $withBtClear?
		'<div style="float:left;padding: 0 4 0 0">
	...
	$entry_thn = 
		'<select id="'. $elName  .'_thn" 
			name="' . $elName . '"_thn"	'.
			$dis. 
			' onchange="'.$this->Prefix.'.TglEntry_createtgl(\'' . $elName . '\',\''.$jns.'\')"
		>'.
			$opsi.
		'</select>';
	...
	function GetHrg_Asal(){
		...
		$tgl= $_REQUEST['tgl'];		
		$tgl_perolehan= $_REQUEST['tgl_perolehan'];	
		$idkoreksi= $_REQUEST['idbukuinduk'];	
		if($jns==0){
			$idbukuinduk= $_REQUEST['idbukuinduk'];
			$query ="select * from buku_induk where id = '$idbukuinduk' ";				
		}else{
			$idkoreksi= $_REQUEST['idbukuinduk'];
			$query ="select idbi from t_koreksi where id='$idkoreksi'";			
			$idbi = table_get_rec($query);	
			$idbukuinduk = $idbi['idbi'];	
		}//$cek.='query='.$query;
	...
	function setFormBaruBI(){
		...
		$dt['staset'] =  $bi['staset'];
		$dt['tgl'] =  $tgl_buku;
		$dt['tgl_perolehan'] = $bi['tgl_buku']!=NULL?$bi['tgl_buku']:$bi['thn_perolehan'].'-00-00';
	...
	function setForm($dt){
		...
		if($err==''){
			...
			$this->form_width = 500;
			$this->form_height = $Main->STASET_OTOMATIS? 200: 180;
			if ($this->form_fmST==0) {		
				$jns = 0;
			}else{
				$jns = 1;
			...
			$vtgl = createEntryTgl(	'tgl', $tgl, 	false, 	'', 	'','adminForm','1');
			$vtgl_perolehan = $this->createEntryTgl3($tgl_perolehan,'tgl_perolehan','','','','',TRUE,TRUE,$jns);
	...
	function getDaftarOpsi($Mode=1){
		...
		$arrOrders[] = $Main->KD_BARANG_P108?" c1,c,d,e,e1,f1,f2,f,g,h,i,j,thn_perolehan,noreg,tgl ":" c1,c,d,e,e1,f,g,h,i,j,thn_perolehan,noreg,tgl ";
	...
	&nbsp&nbsp&nbsp
		...
		if($Main->MENU_VERSI==3){
			/***$menubar = "<table width=\"100%\" class=\"menubar\" cellpadding=\"0\" cellspacing=\"0\" border=\"0\">
					...
					<A href=\"pages.php?Pg=Koreksi\" $styleMenu title='Daftar Koreksi'>KOREKSI</a> |
