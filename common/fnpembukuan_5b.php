...
class PembukuanObj5b extends DaftarObj2{ // utk serang 2013
	var $Prefix = 'Pembukuan5b'; //jsname
	var $SHOW_CEK = FALSE;
	var $withform = TRUE;
	//daftar -------------------
...
function genDaftar($Kondisi='', $Order='', $Limit='', $noAwal = 0, $Mode=1, $vKondisi_old=''){
	...
	| <A href=\"pages.php?Pg=Rekap5b\" title='Rekap BI' $styleMenu >REKAP BI 2</a>
	$menu_rekapneraca_2
...
global $Main,$HTTP_COOKIE_VARS;
	...
	$sqry="select aa.f, aa.g, aa.h, aa.nm_barang,".
		...
		"from v_ref_kib_keu_h1 aa ".
			"left join (select IFNULL(f,'00') as f , IFNULL(g,'00') as g, IFNULL(h,'00') as h, sum(jml_barang) as jmlBrgHPS_awal, sum(jml_harga ) as jmlHrgHPS_awal from v_penghapusan_bi where $Kondisi and tgl_penghapusan < '$tglAwal' group by f,g,h with rollup ) bb on aa.f=bb.f and aa.g=bb.g and aa.h=bb.h ".
			"left join (select IFNULL(f,'00') as f , IFNULL(g,'00') as g, IFNULL(h,'00') as h, count(*) as jmlPLH_awal, sum(biaya_pemeliharaan ) as jmlHrgPLH_awal from v_pemelihara where $Kondisi and tambah_aset=1 and tgl_pemeliharaan < '$tglAwal' group by f,g,h with rollup ) cc on aa.f=cc.f and aa.g=cc.g and aa.h=cc.h ".
			"left join (select IFNULL(f,'00') as f , IFNULL(g,'00') as g, IFNULL(h,'00') as h, count(*) as jmlAman_awal, sum(biaya_pengamanan ) as jmlHrgAman_awal from v_pengaman where $Kondisi and tambah_aset=1 and tgl_pengamanan < '$tglAwal' group by f,g,h with rollup ) dd on aa.f=dd.f and aa.g=dd.g and aa.h=dd.h ".
			"left join (select IFNULL(f,'00') as f , IFNULL(g,'00') as g, IFNULL(h,'00') as h, sum(jml_barang) as jmlBrgBI_awal, sum(jml_harga ) as jmlHrgBI_awal from view_buku_induk where $Kondisi and tgl_buku < '$tglAwal'  group by f,g,h with rollup ) ee on aa.f=ee.f and aa.g=ee.g and aa.h=ee.h ".
			
			
			"left join (select IFNULL(f,'00') as f , IFNULL(g,'00') as g, IFNULL(h,'00') as h, sum(jml_barang) as jmlBrgHPS_akhir, sum(jml_harga ) as jmlHrgHPS_akhir from v_penghapusan_bi where $Kondisi and tgl_penghapusan <= '$tglAkhir' group by f,g,h with rollup ) ff on aa.f=ff.f and aa.g=ff.g and aa.h=ff.h ".
			"left join (select IFNULL(f,'00') as f , IFNULL(g,'00') as g, IFNULL(h,'00') as h, count(*) as jmlPLH_akhir, sum(biaya_pemeliharaan ) as jmlHrgPLH_akhir from v_pemelihara where $Kondisi and tambah_aset=1 and tgl_pemeliharaan <= '$tglAkhir' group by f,g,h with rollup ) gg on aa.f=gg.f and aa.g=gg.g and aa.h=gg.h ".
			"left join (select IFNULL(f,'00') as f , IFNULL(g,'00') as g, IFNULL(h,'00') as h, count(*) as jmlAman_akhir, sum(biaya_pengamanan ) as jmlHrgAman_akhir from v_pengaman where $Kondisi and tambah_aset=1 and tgl_pengamanan <= '$tglAkhir' group by f,g,h with rollup ) hh on aa.f=hh.f and aa.g=hh.g and aa.h=hh.h ".
			"left join (select IFNULL(f,'00') as f , IFNULL(g,'00') as g, IFNULL(h,'00') as h, sum(jml_barang) as jmlBrgBI_akhir, sum(jml_harga ) as jmlHrgBI_akhir from view_buku_induk  where $Kondisi and  tgl_buku <= '$tglAkhir'  group by f,g,h with rollup ) ii on aa.f=ii.f and aa.g=ii.g and aa.h=ii.h ".
			
			
			"left join (select IFNULL(f,'00') as f , IFNULL(g,'00') as g, IFNULL(h,'00') as h, sum(biaya_pemeliharaan ) as jmlHrgHPS_PLH_awal from v2_penghapusan_pelihara where  tgl_penghapusan < '$tglAwal' and tambah_aset=1 $KondisiKIB group by f,g,h with rollup ) jj on aa.f=jj.f  and aa.g=jj.g  and aa.h=jj.h ".
			"left join (select IFNULL(f,'00') as f , IFNULL(g,'00') as g, IFNULL(h,'00') as h, sum(biaya_pengamanan ) as jmlHrgHPS_Aman_awal from v2_penghapusan_pengaman where  tgl_penghapusan < '$tglAwal' and tambah_aset=1 $KondisiKIB group by f,g,h with rollup ) kk on aa.f=kk.f  and aa.g=kk.g and aa.h=kk.h ".			
			"left join (select IFNULL(f,'00') as f , IFNULL(g,'00') as g, IFNULL(h,'00') as h, sum(biaya_pemeliharaan ) as jmlHrgHPS_PLH_akhir from v2_penghapusan_pelihara where  tgl_penghapusan <= '$tglAkhir' and tambah_aset=1 $KondisiKIB group by f,g,h with rollup ) ll on aa.f=ll.f  and aa.g=ll.g and aa.h=ll.h ".
			"left join (select IFNULL(f,'00') as f , IFNULL(g,'00') as g, IFNULL(h,'00') as h, sum(biaya_pengamanan ) as jmlHrgHPS_Aman_akhir from v2_penghapusan_pengaman where  tgl_penghapusan <= '$tglAkhir' and tambah_aset=1 $KondisiKIB group by f,g,h with rollup ) mm on aa.f=mm.f  and aa.g=mm.g and aa.h=mm.h ".
			
			"left join (select IFNULL(f,'00') as f , IFNULL(g,'00') as g, IFNULL(h,'00') as h, count(*) as jmlHSBG_awal, sum(harga_hapus) as jmlHrgHSBG_awal from v_hapus_sebagian where $Kondisi and tgl_penghapusan < '$tglAwal' group by f,g,h with rollup ) nn on aa.f=nn.f and aa.g=nn.g and aa.h=nn.h ".
			"left join (select IFNULL(f,'00') as f , IFNULL(g,'00') as g, IFNULL(h,'00') as h, count(*) as jmlHSBG_akhir, sum(harga_hapus ) as jmlHrgHSBG_akhir from v_hapus_sebagian where $Kondisi  and tgl_penghapusan <= '$tglAkhir' group by f,g,h with rollup ) oo on aa.f=oo.f and aa.g=oo.g and aa.h=oo.h ".
			"left join (select IFNULL(f,'00') as f , IFNULL(g,'00') as g, IFNULL(h,'00') as h, sum(harga_hapus ) as jmlHrgHPS_HSBG_awal from v2_penghapusan_hapussebagian where  tgl_penghapusan < '$tglAwal' $KondisiKIB group by f,g,h with rollup ) pp on aa.f=pp.f  and aa.g=pp.g and aa.h=pp.h	".		
			"left join (select IFNULL(f,'00') as f , IFNULL(g,'00') as g, IFNULL(h,'00') as h, sum(harga_hapus) as jmlHrgHPS_HSBG_akhir from v2_penghapusan_hapussebagian where  tgl_penghapusan <= '$tglAkhir'  $KondisiKIB group by f,g,h with rollup ) qq on aa.f=qq.f  and aa.g=qq.g and aa.h=qq.h ". 
			
			
			"left join (select IFNULL(f,'00') as f , IFNULL(g,'00') as g, IFNULL(h,'00') as h, sum(harga) as jmlHrgSusut_awal from v1_penyusutan where $Kondisi and tahun < '$fmFiltThnBuku'   group by f,g,h with rollup ) rr on aa.f=rr.f  and aa.g=rr.g and aa.h=rr.h ".
			"left join (select IFNULL(f,'00') as f , IFNULL(g,'00') as g, IFNULL(h,'00') as h, sum(harga) as jmlHrgSusut_akhir from v1_penyusutan where $Kondisi and tahun <= '$fmFiltThnBuku'   group by f,g,h with rollup ) ss on aa.f=ss.f  and aa.g=ss.g and aa.h=ss.h ".			
		...
		while($isi=mysql_fetch_array($Qry)){
			...
			if ($no==1)
			{
		        ...
		        $ListData .= "
					...
					<td class=\"$clGaris\" align=left colspan=4 ><b>I.&nbsp;&nbsp;&nbsp;&nbsp;Intrakomptabel</b></td>
					$TampilStyle
		        </tr>
				";
				$no++;
			...
			if ($isi['f'] == "00" && $isi['g'] == "00" )
			{
		        $listRow = "
					...
					<td class=\"$clGaris\" width=\"$kolomwidth[3]\" colspan=3 ><b>A.</b>&nbsp;&nbsp;&nbsp;&nbsp;$nmBarang</td>					
					<td class=\"$clGaris\" align=right width=\"$kolomwidth[4]\">$tampil_jmlBrg_akhir</td>
					<td class=\"$clGaris\" align=right width=\"$kolomwidth[5]\">$tampil_jmlHrg_akhir</td>
					$tampilKet				
		        </tr>
				";		
			} else if ($isi['f'] != "00" && $isi['g'] == "00" )
			{
		        $listRow = "
					<tr class='$clRow'>
					<td class=\"$clGaris\" align=center width=\"$kolomwidth[0]\">$no.</td>
					<td class=\"$clGaris\" align=center width=\"$kolomwidth[1]\"><b>{$isi['f']}</td>					
					<td class=\"$clGaris\" align=center width=\"$kolomwidth[2]\"><b>$kdBidang</td>
					<td class=\"$clGaris\" align=center width=\"$kolomwidth[1]\"></td>
					<td class=\"$clGaris\" width=\"$kolomwidth[3]\"><b>$nmBarang</td>
					<td class=\"$clGaris\" align=right width=\"$kolomwidth[4]\">$tampil_jmlBrg_akhir</td>
					<td class=\"$clGaris\" align=right width=\"$kolomwidth[5]\">$tampil_jmlHrg_akhir</td>
					$tampilKet									
		        </tr>
			...
			else if ($isi['g'] != "00" && $isi['h'] == "00" )
			{
		        $listRow = "
					<tr class='$clRow'>
					<td class=\"$clGaris\" align=center width=\"$kolomwidth[0]\">$no.</td>
					<td class=\"$clGaris\" align=center width=\"$kolomwidth[1]\"><b>{$isi['f']}</td>					
					<td class=\"$clGaris\" align=center width=\"$kolomwidth[2]\"><b>{$isi['g']}</td>
					<td class=\"$clGaris\" align=center width=\"$kolomwidth[1]\"></td>
					<td class=\"$clGaris\" width=\"$kolomwidth[3]\"><b>$nmBarang</td>
		...
		// aset lainnya -----------------------------------------------------------------------------------
	$Kondisi = $KondisiAsal;	 
	$sqry="select aa.f, aa.g,aa.ka,aa.kb,  aa.nm_barang,0 as jml_barang,0 as jml_harga from v_ref_kib_keu_h aa 
       ...
       $kdBidang = $isi['g'] == "00" ? "" : $isi['g'];
		...
		if ($isi['g'] == "22")
		{
			...
			$get= mysql_fetch_array( mysql_query("select sum(jml_barang) as jmlBrgBI_akhir, sum(jml_harga ) as jmlHrgBI_akhir from buku_induk 
			where  $Kondisi and staset=6  and tgl_buku <= '$tglAkhir'  "));  
			$jmlBrg_akhir=$get['jmlBrgBI_akhir'];
			$jmlHrg_akhir=$get['jmlHrgBI_akhir'];
			$jmlHrg_akhir_tot=$jmlHrg_akhir_tot+$jmlHrg_akhir;		
			$jmlBrg_akhir_tot=$jmlBrg_akhir_tot+$jmlBrg_akhir;		
		...
		if ($isi['g'] == "23")
		{
			...
			$get= mysql_fetch_array( mysql_query("select sum(jml_barang) as jmlBrgBI_akhir, sum(jml_harga ) as jmlHrgBI_akhir from buku_induk 
			where  $Kondisi and staset=7  and tgl_buku <= '$tglAkhir'  "));  
			$jmlBrg_akhir=$get['jmlBrgBI_akhir'];
			$jmlHrg_akhir=$get['jmlHrgBI_akhir'];
			$jmlHrg_akhir_tot=$jmlHrg_akhir_tot+$jmlHrg_akhir;		
			$jmlBrg_akhir_tot=$jmlBrg_akhir_tot+$jmlBrg_akhir;		
		...
		if ($isi['g'] == "24")
		{
			...
			$get= mysql_fetch_array( mysql_query("select sum(jml_barang) as jmlBrgBI_akhir, sum(jml_harga ) as jmlHrgBI_akhir from buku_induk 
			where  $Kondisi and f='07' and g='24' and staset=8 and tgl_buku <= '$tglAkhir'  "));  
			$jmlBrg_akhir=$get['jmlBrgBI_akhir'];
			$jmlHrg_akhir=$get['jmlHrgBI_akhir'];
			$jmlHrg_akhir_tot=$jmlHrg_akhir_tot+$jmlHrg_akhir;		
			$jmlBrg_akhir_tot=$jmlBrg_akhir_tot+$jmlBrg_akhir;		
		...
		{
			$get= mysql_fetch_array( mysql_query("select sum(jml_barang) as jmlBrgBI_akhir, sum(jml_harga ) as jmlHrgBI_akhir from buku_induk 
			where $Kondisi and staset=9 and tgl_buku <= '$tglAkhir'  "));  
			$jmlBrg_akhir=$get['jmlBrgBI_akhir'];
			$jmlHrg_akhir=$get['jmlHrgBI_akhir'];			
			$jmlHrg_akhir_tot=$jmlHrg_akhir_tot+$jmlHrg_akhir;		
			$jmlBrg_akhir_tot=$jmlBrg_akhir_tot+$jmlBrg_akhir;		
        ...
        $ListData .= "
			...
			<td class=\"$clGaris\" colspan=3 ><b>B.</b>&nbsp;&nbsp;&nbsp;&nbsp;$nmBarang</td>
			$TampilStyle
        </tr>
		";		
		...
		$ListData .= "
			<tr class='$clRow'>
			<td class=\"$clGaris\" align=center width=\"$kolomwidth[0]\">$no.</td>
			<td class=\"$clGaris\" width=\"$kolomwidth[3]\" colspan=4><b>II.&nbsp;&nbsp;&nbsp;Ekstrakomptabel<b></td>
			$TampilStyle
        </tr>
		";
...
$Pembukuan5b = new PembukuanObj5b();
