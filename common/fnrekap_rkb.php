...
class RekapRkbObj extends DaftarObj2{
	var $Prefix = 'rekaprkb';
	var $SHOW_CEK = TRUE;	
	var $TblName = 'v1_rkb';//view2_sensus';
	...
	var $cetak_xls = FALSE;
	var $fileNameExcel='rekaprkbmd.xls';
	...
	function setTitle(){
		return 'Rekap Rencana Kebutuhan Barang Milik Daerah ';
	...
	function setNavAtas(){
		return
			...
			<td class="menudottedline" width="60%" height="20" style="text-align:right"><b>
				...
				<a href="pages.php?Pg=dkb" title="Daftar Kebutuhan Barang Milik Daerah">DKBMB</a>  |  
...
where aa.g='00' ";*/
		...
		while ( $isi=mysql_fetch_array($qry)){
			...
			$get = mysql_fetch_array(mysql_query(
				"select sum(jml_barang) as jmlbrg, sum(jml_harga) as jmlhrg from rkb ".$Kondisi// and g='".$isi['g']."' "
			));
			...
			while($isig = mysql_fetch_array($qryg)){
				...
				$get = mysql_fetch_array(mysql_query(
					"select sum(jml_barang) as jmlbrg, sum(jml_harga) as jmlhrg from rkb ".$Kondisi2
				));
