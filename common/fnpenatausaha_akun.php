...
*/					
	...
	function genToolbarAtas(){
		...
		$PnlMutasiv2= "<td>" . genPanelIcon1("javascript:MutasiBaru_ins.mutasibaru()", "mutasi.png", "Mutasi V2","Mutasi V2","","","","","style=width:75px") . "</td>";		
		if (empty($ridModul05)) {
			switch ($_GET['SPg']) {
				case 'belumsensus':{
...
*/						
				...
				default:
					...
					$PnlDelete=$Main->PENERIMAAN_P19==1? "<td>" . PanelIcon1("javascript:prosesHapus()", "delete_f2.png", "Batal", "", "", "", "","Batal Mutasi & Reklass") . "</td>" : "<td>" . PanelIcon1("javascript:prosesHapus()", "delete_f2.png", "Delete") . "</td>";
					...
					if($jns=='penyusutan' && $Main->PENYUSUTAN){
						$ToolbarAtas_edit =
							"<td>" . PanelIcon1("javascript:Penyusutan.formInformasi()", "new_f2.png", "Informasi") . "</td>".
		...
		while($isi=mysql_fetch_array($Qry)){ //20 detik
			...
			*/			
			if($Main->TAMPIL_BIDANG){				
				...
				if($fmSKPD==''||$fmSKPD=='00'){
					$get = mysql_fetch_array(mysql_query(
						"select * from v_bidang where $konc1 c='".$isi['c']."' "
					));		
...
$PenatausahaAkun = new PenatausahaAkunObj();
