...
class AppCls{
	...
	function genHTMLHead($OtherCSS='', $OtherScript='', $pathjs=''){
		...
		*/
		return
			"<head>
	<title>$Main->Judul {$HTTP_COOKIE_VARS['coNama']}</title>
...
//inisial =======================================
	function DaftarObj2(){
		global $Main, $Menuv5;
		if($Main->MENU_VERSI == 5){
			include_once('pages/base_main_v5.php');
		}

		if($Main->MENU_VERSI==5){
			$this->footerHeight = 83;
		}
...
//DAFTAR ========================================
	...
	function setMenuView(){
		return 
			//"<td>".genPanelIcon("javascript:".$this->Prefix.".cetakHal()","print_f2.png","SPPT",'Cetak SPPT')."</td>".
			...
			//"<td>".genPanelIcon("javascript:".$this->Prefix.".exportXls()","print_f2.png","Excel",'Export ke Excell')."</td>";
			//"<td>".genPanelIcon("javascript:".$this->Prefix.".cetakHal()","edit_f2.png","Default",'Setting Default')."</td>";		
	}
	function genMenu(){
		global $HTTP_COOKIE_VARS;
...
//		}
	...
	function genTableRow($Koloms, $RowAtr='', $KolomClassStyle=''){
		...
		if (count($Koloms)>0){$baris ="<tr $RowAtr onclick=\"".$this->Prefix.".rowOnClick(this)\"> $baris </tr>"; }
		return $baris;
	}
...
//PAGE ===========================================	
	...
	function genHTMLHead(){		
		...
		*/
		if($Main->MENU_VERSI==5){
			$otherStyle= $Menuv5->style.$this->setPage_OtherStyle();
			$otherScript = $Menuv5->script.$this->setPage_OtherScript();
			return $app->genHTMLHead($otherStyle, $otherScript	);
		}else{
	...
	function pageShow(){
		...
		"<html>".
			...
			"<table id='KerangkaHal' class='menubar' cellspacing='0' cellpadding='0' border='0' width='100%' height='100%' >".
				//header page -------------------		
				"<tr height='$this->headerHeight'><td>".						
					//$this->setPage_Header($IconPage, $TitlePage).
					$this->setPage_Header().
					"<div id='header' ></div>".
				"</td></tr>".	
				...
				//$OtherContentPage.				
				//Footer ------------------------
				"<tr><td height='$this->footerHeight' >".	
					//$app->genPageFoot(FALSE).
					//$Main->CopyRight.	
					$this->setPage_Footer('');						
				"</td></tr>".
