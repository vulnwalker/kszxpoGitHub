...
class JurnalObj  extends DaftarObj2{	
	...
	//var $FieldSum = array('debet','jml_barang_d','kredit','jml_barang_k','saldo_harga','saldo_barang');//array('jml_harga');
	var $FieldSum = array('jml_barang_d','debet','jml_barang_k','kredit','saldo_barang','saldo_harga');//array('jml_harga');
	...
	var $PageIcon = 'images/v3/index_45.gif';
	var $cetak_xls=TRUE ;
	var $fileNameExcel='history.xls';
	...
	function setMenuView(){
		return 
			...
			"<td>".genPanelIcon("javascript:".$this->Prefix.".exportXls()","export_xls.png",'Excel',"Export Excel")."</td>";
	...
	function setDaftar_query($Kondisi='', $Order='', $Limit=''){	
		...
		$aqry = "select (debet - kredit) as saldo_harga, (jml_barang_d-jml_barang_k) as saldo_barang, $this->TblName.* from $this->TblName $Kondisi $Order $Limit ";	
		return $aqry;		
	}
	function setSumHal_query($Kondisi, $fsum){
		...
		return "select count(*) as cnt,sum(debet) as sum_debet,
				sum(jml_barang_d) as sum_jml_barang_d,sum(kredit) as sum_kredit,sum(jml_barang_k) as sum_jml_barang_k,
				sum(debet-kredit) as sum_saldo_harga,sum(jml_barang_d-jml_barang_k) as sum_saldo_barang   from $this->TblName $Kondisi "; //echo $aqry;
		//return " select "
	}
	function genRowSum_setTampilValue($i, $value){
		switch($i){
			case 0: return number_format($value, 0, ',' ,'.'); break;
			case 1: return number_format($value, 2, ',' ,'.'); break;
			case 2: return number_format($value, 0, ',' ,'.'); break;
			case 3: return number_format($value, 2, ',' ,'.'); break;
			case 4: return number_format($value, 0, ',' ,'.'); break;
			case 5: return number_format($value, 2, ',' ,'.'); break;
		}
	...
	function genDaftarInitial(){
		...
		//Jurnal&c1=&c=&d=&e=&e1=000&kint=01&ka=02&kb=06&g=&h=&i=&bold=0&tgl2=2016-12-31&tanpasusut=1
		$idbi = $_REQUEST['idbi'];
		...
		if($c1 != '') 	setcookie('cofmURUSAN',$c1);
		if($c != '') 	setcookie('cofmSKPD',$c);
		if($d != '') 	setcookie('cofmUNIT',$d);
		if($e != '') 	setcookie('cofmSUBUNIT',$e);
		if($e1 != '') 	setcookie('cofmSEKSI',$e1);
		...
		//if($inv==1){ //jika inventaris kd akun tidak perlu
		//	$kdakun =='';			
		//}else{
			$kdakun = $kint;
		...
		/*if($inv==1){
			$kdbrg = $kb!='' && $kb!='00' && $kb != NULL ? $kb : '';
			$kdbrg .= $g!='' && $g!='00' && $g != NULL ? '.'.$g : '';
			$kdbrg .= $h!='' && $h!='00' && $h != NULL ? '.'.$h : '';
			$kdbrg .= $i!='' && $i!='00' && $i != NULL ? '.'.$i : '';
			$kint='';
			$ka='';
			$kb='';
		}else*/
		...
		if($Main->URUSAN==1){
			$urusan = "<input type='hidden' id='JurnalSkpdfmURUSAN' name='JurnalSkpdfmURUSAN' value='$c1'  >";
		}
		...
		$hidden = 
			"<input type='hidden' id='fmIdAwal' name='fmIdAwal' value='$idawal'  >".
			"<input type='hidden' id='fmIdbi' name='fmIdbi' value='$idbi'  >".
			...
			"<input type='hidden' id='fmKdAkun' name='fmKdAkun' value='$kdakun'  >".
			"<input type='hidden' id='fmKodeBarang' name='fmKodeBarang' value='$kdbrg'  >".
			"<input type='hidden' id='fmTgl1' name='fmTgl1' value='$tgl1'  >".
			"<input type='hidden' id='fmTgl2' name='fmTgl2' value='$tgl2'  >".
			"<input type='hidden' id='fmJnsTrans' name='fmJnsTrans' value='$jns_trans'  >".
			"<input type='hidden' id='fmDebet' name='fmDebet' value='$debet'  >".		
			"$urusan
			...
			"<input type='hidden' id='JurnalSkpdfmSEKSI' name='JurnalSkpdfmSEKSI' value='$e1'  >".	
			"<input type='hidden' id='g' name='g' value='$g'  >".		
			"<input type='hidden' id='h' name='h' value='$h'  >".		
			"<input type='hidden' id='i' name='i' value='$i'  >".		
			"<input type='hidden' id='fmTanpaSusut' name='fmTanpaSusut' value='$tanpasusut'  >".		
			//"<input type='hidden' id='fminv' name='fminv' value='$inv'  >".		
			"";
	...
	function setKolomHeader($Mode=1, $Checkbox=''){
		...
		$headerTable =
			...
			<tr>
				...
				<th class='th01' width='' rowspan=2>KD Rekap </th>
				<th class='th01' width='' rowspan=2>Nama Akun </th>
				<!--<th class='th01' width='30' rowspan=2>Rincian Harga </th>-->
				
				
				<th class='th02' colspan=2> Debet</th>
				<th class='th02' colspan=2> Kredit</th>
				<th class='th02' colspan=2>Saldo </th>
				<th class='th01' width='30' rowspan=2>Group </th>	
				<th class='th02' width='' colspan=4>Barang </th>					
			...
			<tr>
				...
				<th class='th01' width='80' rowspan=1>ID Barang/<br> ID Awal</th>
				
				<th class='th01' rowspan=1>KD SKPD/<br> Nama SKPD</th>
				<th class='th01' rowspan=1>KD Barang/<br> Nama Barang </th>
				<th class='th01' width='80' rowspan=1>Tahun Perolehan/<br> No. Reg. </th>
	...
	function setKolomData($no, $isi, $Mode, $TampilCheckBox){		
		...
		$vkdskpd = $c1.$isi['c'].'.'.$isi['d'].'.'.$isi['e'].'.'.$isi['e1'];
		...
		$saldoBarang = $isi['saldo_barang'] ;
		$saldoHarga = $isi['saldo_harga'] ;
		//if( $isi['jns_trans'] ==1 ) $vdebet =$isi['harga_atribusi'];
		
		if($Main->URUSAN==1){
			$kdskpd2 = " concat(c1,'.',c,'.',d) ='". $c1.$isi['c'].'.'.$isi['d']."' ";
		}else{
			$kdskpd2 = " concat(c,'.',d) ='". $isi['c'].'.'.$isi['d']."' ";
		}
		
		$skpd = mysql_fetch_array(mysql_query( " select * from ref_skpd where $kdskpd2  "  )) ;
		$nmSKPD = '';
		if($vkdskpd != '') $nmSKPD = '/<br>'.$skpd['nm_skpd'];
		$brg = mysql_fetch_array(mysql_query(" select * from ref_barang where concat(f,'.',g,'.',h,'.',i,'.',j) = '$vkdbrg'"));
		 $nmBarang='';
		 if($vkdbrg != '')$nmBarang ='/<br>'.$brg['nm_barang'];
		...
		*/
		...
		$Koloms[] = array(" align='right' ", $saldoBarang==0? '': number_format($saldoBarang,0,',','.') );
		$Koloms[] = array(" align='right' ", $saldoHarga==0? '': number_format($saldoHarga,0,',','.') );
		
		
		
		$Koloms[] = array('', $vjnstrans ); //group
		$Koloms[] = array('', $isi['idbi'].'/<br>'.$isi['idawal'] );
				
		$Koloms[] = array('', $vkdskpd.$nmSKPD  );
		$Koloms[] = array('', $vkdbrg.$nmBarang );
		$Koloms[] = array('', $vtahun .'/<br>'.$vnoreg);
	...
	function setPage_HeaderOther(){
		...
		if($Main->MENU_VERSI==3){
			...
			"<tr><td class=\"menudottedline\" width=\"40%\" height=\"20\" style='text-align:right'><B>
			<A href=\"index.php?Pg=09&SPg=01&SSPg=03&mutasi=1\" title='Daftar Mutasi'>PINDAH SKPD</a>  |
			<A href=\"index.php?Pg=09&SPg=01&SSPg=03&mutasi=2\" title='Daftar Reklas Kib'>REKLAS KIB</a> |
			...
			<A href=\"pages.php?Pg=Kondisi\" title='Daftar Kondisi'>KONDISI</a> 
			 | <A href=\"index.php?Pg=09&SPg=01&SSPg=03&mutasi=4\" title='Daftar Penggabungan'>PENGGABUNGAN</a> 
			 | <A href=\"index.php?Pg=09&SPg=01&SSPg=03&mutasi=5\" title='Daftar Double Catat'>DOUBLE CATAT</a> 
			 | <A href=\"pages.php?Pg=Jurnal\" title='Jurnal' style='color:blue'>HISTORI</a> 
	...
	function genDaftarOpsi(){
		...
		$idawal = $_REQUEST['fmIdAwal'];
		$idbi= $_REQUEST['fmIdbi'];
		$kint 	= $_REQUEST['kint'];
		$ka 	= $_REQUEST['ka'];
		$kb 	= $_REQUEST['kb'];
		$kdakun = $_REQUEST['fmKdAkun'];
		$kdbrg = $_REQUEST['fmKodeBarang'];
		$tgl1 	= $_REQUEST['fmTgl1'];
		$tgl2 	= $_REQUEST['fmTgl2'];
		$jns_trans = $_REQUEST['fmJnsTrans'];
		$debet 	= $_REQUEST['fmDebet'];
		$tanpasusut = $_REQUEST['fmTanpaSusut'];
		$g = $_REQUEST['g'];
		$h = $_REQUEST['h'];
		$i = $_REQUEST['i'];
		//$fminv = $_REQUEST['fminv'];
		$jmlperhal = $_REQUEST['jmlperhal'];
		if($jmlperhal=='')$jmlperhal = 50;
		
		//if($fminv==1){//jika inventaris kode akun tidak pengaruh
		//	$kdakun='';
		//}else{
			$kdakun = $kint;
			$kdakun .= $ka!='' && $ka!='00' && $ka != NULL ?  '.'.$ka :'';
		...
		$fmTgl1 =  cekPOST('fmTgl1')==''?$tgl1:cekPOST('fmTgl1');
		$fmTgl2 =  cekPOST('fmTgl2')==''?$tgl2:cekPOST('fmTgl2');	
		$fmIdAwal = cekPOST('fmIdAwal')==''?$idawal:cekPOST('fmIdAwal');
		$fmIdbi = cekPOST('fmIdbi')==''?$idbi:cekPOST('fmIdbi');
		
		$fmKdAkun = cekPOST('fmKdAkun')==''?$kdakun:cekPOST('fmKdAkun');
		
		$fmJnsTrans = cekPOST('fmJnsTrans')==''?$jns_trans:cekPOST('fmJnsTrans');
		$fmJnsTrans2 = cekPOST('fmJnsTrans2');
		$fmDebet = cekPOST('fmDebet')==''?$debet:cekPOST('fmDebet');
		$fmKodeBarang= cekPOST('fmKodeBarang')==''?$kdbrg:cekPOST('fmKodeBarang');
		$fmTanpaSusut = cekPOST('fmTanpaSusut')==''?$tanpasusut:cekPOST('fmTanpaSusut');
		$vtanpasusut = $fmTanpaSusut ?  " checked='true' " : '';
		$vtanpasusut = " <input type='checkbox' name='fmTanpaSusut' id='fmTanpaSusut' value='1' $vtanpasusut  >Tanpa Penyusutan";
		
		//$fminv = cekPOST('fminv')==''?$fminv:cekPOST('fminv');
		//$vinv = $fminv ?  " checked='true' " : '';
		//$vinv = " <input type='checkbox' name='fminv' id='fminv' value='1' $vinv  >Inventaris";
								...
								<table cellspacing='0' cellpadding='0' border='0' style='height:28'>
									<tr valign='middle'>
										<td align='left' style='padding:1 8 0 8;height:30; '>".
											...
											"<div style='float:left;padding: 0 4 0 0'>".createEntryTgl3($fmTgl1, 'fmTgl1', false, '')."</div>".
											"<div style='float:left;padding: 0 4 0 0'>s/d &nbsp;</div>".
											"<div style='float:left;padding: 0 4 0 0'>".createEntryTgl3($fmTgl2, 'fmTgl2', false, '')."</div>".
											$vtanpasusut.
											//$vinv.
											"<div style='float:left;padding: 0 4 0 0'>Baris <input type='text' id='jmlperhal' name='jmlperhal' value='$jmlperhal' style='width: 50;' ></div>".
										"</td>
									</tr>
									<tr>
										<td align='left' style='padding:1 8 0 8;height:30; '>
											ID Awal : <input type='text' name='fmIdAwal' id='fmIdAwal' value='".$fmIdAwal."'> &nbsp;&nbsp;&nbsp;
											ID Barang : <input type='text' name='fmIdbi' id='fmIdbi' value='".$fmIdbi."'> &nbsp;&nbsp;&nbsp;
											KD Rekap : <input type='text' name='fmKdAkun' id='fmKdAkun' value='".$fmKdAkun."'> &nbsp;&nbsp;&nbsp;".
											cmbArray('fmJnsTrans',$fmJnsTrans,$Main->jnsTrans,'--Group--','')."&nbsp&nbsp;&nbsp;".
											cmbArray('fmJnsTrans2',$fmJnsTrans2,$Main->jnsTrans2,'--Transaksi--','')."&nbsp&nbsp;&nbsp;".
											cmbArray('fmDebet',$fmDebet,$arrDebet,'--Semua--','')."&nbsp&nbsp;&nbsp;".
											"<input type='hidden' id='g' name='g' value='$g'  >".	
											"<input type='hidden' id='h' name='h' value='$h'  >".	
											"<input type='hidden' id='i' name='i' value='$i'  >".	
											"Kode Barang : <input type='text' name='fmKodeBarang' id='fmKodeBarang' value='".$fmKodeBarang."'> &nbsp;&nbsp;&nbsp;".
	...
	function getDaftarOpsi($Mode=1){
		...
		//$inv = $_REQUEST['fminv'];
		/*if($inv==1) {
			 $arrKondisi[] = "((kint = '01' and ka='01'  )". // #aset tetap 
			 
			 	"or  (kint = '01' and ka='02' and kb='01' ) ". //pemindahtangan  penjualan , permen 19
				"or (kint = '01' and ka='02' and kb='03' ) ". //#kemitraan
				"or (kint = '01' and ka='02' and kb='04') ". //ATB
				"or (kint = '01' and ka='02' and kb='05') ". //aset lainlain
				"or  (kint = '01' and ka='02' and kb='06' ) ". //pemindahtangan bukan penjualan , permen 19
				"or  (kint = '01' and ka='02' and kb='07' )".  //pemusnahan 				
				
				"or (kint = '02' and ka='00' and kb='00') ". //Ekstra
		...
		if($Main->URUSAN==1){
			if(!($fmURUSAN=='' || $fmURUSAN=='00') ) $arrKondisi[] = "c1='$fmURUSAN'";
		}
		...
		//kode akun
		//if($inv == 1){ //jika inventaris kode akun tidak pengaruh
			$kdakun = $_REQUEST['kint'];
			$kdakun .= $_REQUEST['ka'] != '' && $_REQUEST['ka'] != '00' && $_REQUEST['ka'] != NULL ?  '.'.$_REQUEST['ka'] :'';
			$kdakun .= $_REQUEST['kb'] != '' && $_REQUEST['kb'] != '00' && $_REQUEST['kb'] != NULL ?  '.'.$_REQUEST['kb'] :'';
			$kd_akun = $_REQUEST['fmKdAkun'] == '' ? $kdakun : $_REQUEST['fmKdAkun'];
		...
		$tgl1 = $_REQUEST['fmTgl1'] == '' ? $_REQUEST['tgl1'] : $_REQUEST['fmTgl1'];
		if($tgl1!='') $arrKondisi[] = " tgl_buku >='$tgl1' ";
		$tgl2 = $_REQUEST['fmTgl2'] == '' ? $_REQUEST['tgl2'] : $_REQUEST['fmTgl2'];
		if($tgl2!='') $arrKondisi[] = " tgl_buku <='$tgl2' ";
		$jns_trans = $_REQUEST['fmJnsTrans'] == ''? $_REQUEST['jns_trans'] : $_REQUEST['fmJnsTrans'];
		if($jns_trans!='') $arrKondisi[] = " jns_trans ='$jns_trans' ";
		$jns_trans2 = $_REQUEST['fmJnsTrans2'];
		if($jns_trans2!='') $arrKondisi[] = " jns_trans2 ='$jns_trans2' ";
		$debet = $_REQUEST['fmDebet'] == ''? $_REQUEST['debet'] : $_REQUEST['fmDebet'];
		if($debet!='') {
			switch ($debet){
				case '1': $arrKondisi[] = " debet >0 "; break;
				case '2': $arrKondisi[] = " kredit >0 "; break;				
		...
		//$fmKdBarang = $_REQUEST['fmKodeBarang'] == ''? $_REQUEST['kode_barang'] : $_REQUEST['fmKodeBarang'];
		//$kdbarang = $_REQUEST['fmKodeBarang'] == '' ? explode(".",$kdbrg) : explode(".",$_REQUEST['fmKodeBarang'] ) ;
		$fmKdBarang = $_REQUEST['fmKodeBarang'];
		$kdbarang = explode(".",$fmKdBarang);
		if (!empty($fmKdBarang)){
			if($kdbarang[0]!='') $arrKondisi[] = " f ='{$kdbarang[0]}' ";
			if($kdbarang[1]!='') $arrKondisi[] = " g ='{$kdbarang[1]}' ";
			if($kdbarang[2]!='') $arrKondisi[] = " h ='{$kdbarang[2]}' ";
			if($kdbarang[3]!='') $arrKondisi[] = " i ='{$kdbarang[3]}' ";
			if($kdbarang[4]!='') $arrKondisi[] = " j ='{$kdbarang[4]}' ";
		}
		
		
		$tanpasusut = $_REQUEST['fmTanpaSusut'] == '' ? $_REQUEST['tanpasusut'] : $_REQUEST['fmTanpaSusut'];
		if($tanpasusut==1) $arrKondisi[] = " jns_trans <> 10 ";
		...
		//limit --------------------------------------
		$jmlperhal = $_REQUEST['jmlperhal'];
		if(empty($jmlperhal)) $jmlperhal = 50;
		//$Main->PagePerHal = 50;	
		$HalDefault=cekPOST($this->Prefix.'_hal',1);				
		//$Limit = " limit ".(($HalDefault	*1) - 1) * $Main->PagePerHal.",".$Main->PagePerHal; //$LimitHal = '';
		$Limit = " limit ".(($HalDefault	*1) - 1) * $jmlperhal.",".$jmlperhal; //$LimitHal = '';
		$Limit = $Mode == 3 ? '': $Limit;
		//noawal ------------------------------------
		//$NoAwal= $Main->PagePerHal * (($HalDefault*1) - 1);							
		$NoAwal= $jmlperhal * (($HalDefault*1) - 1);							
