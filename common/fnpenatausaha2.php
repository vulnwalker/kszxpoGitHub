...
class PenatausahaObj {
	...
	function genToolbarAtas(){
		...
		$pnlAsetLainLain =  $Main->MODUL_ASETLAINLAIN ? "<td>" . PanelIcon1("javascript:AsetLainLain.BaruBI()", "edit_f2.png", "Aset Lain") . "</td>": "";
...
*/
	...
	function getDaftarOpsi(){
		...
		switch ($jns){
			...
			case 'penyusutan' : $Kondisi .= " and staset=3 and status_barang=1 and f in('02','03','04','05') "; break;
			...
			default:{//03
				...
				$OptDetail =
					"<table width=100%><tr><td>".
						$vthn_perolehan. $batas.					
						$vkondisi_barang. $batas.
						$vthn_sensus.$batas.$vtahun_susut.
						$TampilFilterSertifikat. 
			...
			$TampilOpt =
				...
				"<div id='daftaropsi_div' style='height:60;overflow-y:hidden;'>".				
					"<table width=100%><tr><td>".
						...
						$vtgl_buku.
						$batas.
						$vthn_buku.$batas.$vasal_usul.$vstaset.
					"</td></tr></table>".
					$baris.				
					$OptDetail.
				"</div>".					
...
//							$ISI7 = "{$isiKIB_E['jenis']}";
					...
					if (($fmCariComboField != 'ket')||($fmCariComboField == 'ket' && stripos( $ISI15, $fmCariComboIsi) !== false  )){						
						...
						if($tipe=='kertaskerja'){ //cetak kertas kerja sensus
							...
							if($jns=='penyusutan' && $Main->PENYUSUTAN){
								$tahun_susut = $_REQUEST['tahun_susut'];
								$aqrsusut = "select count(*) as jmlsusut, ifnull(sum(harga),0) as hrgsusut, ifnull(sum(hrg_rehab),0) as hrgrehab from penyusutan where idbi='".$isi['idawal']."' and tahun<$tahun_susut ";
								//echo $aqrsusut;
								...
								$nilaibuku = $isi['jml_harga'] + $susut['hrgrehab'] - $penyusutantot ;
								...
								$vresidu = number_format($nilairesidu,0,',','.').'%';
								...
								$ListData .= "
									...
									<td class=\"$clGaris\">".utf8_encode($ISI5)."</td>
									<td class=\"$clGaris\">".utf8_encode($ISI6)."</td>
									...
									<td class=\"$clGaris\">".$vmasa_manfaat."/ ".$vresidu."/ ".$thn_akhir."</td>".
									"<td class=\"$clGaris\">".utf8_encode($ISI15)."</td>".
									$isi['vBidang'].
									$tampilDok.
									$tampilCbxKeranjang.
									"</tr>	";
								...
								$ListData .= "
									...
									<td class=\"$clGaris\" align=right>{$isi['jml_barang']} {$isi['satuan']}</td>
									<td class=\"$clGaris\" align=right>".$tampilHarga."</td>
									<td class=\"$clGaris\">$ISI15 $ketgbr</td>".
									$isi['vBidang'].
									$tampilDok.
									$tampilCbxKeranjang.
									"</tr>	";
				...
				case "05" : case "kib_b_cetak" :{		
					//while ($isi = mysql_fetch_array($Qry)){
						...
						if($tipe=='kertaskerja'){
							...
							<td class=\"$clGaris\" align=right>".$tampilHarga."</td>
							<td class=\"$clGaris\">{$ISI12}$ketskpd.$ketgbr </td>".	
							$isi['vBidang'].
							$tampilCbxKeranjang.				
							"</tr>";
						}
...
//						if($Kel != ''){ $Kel='<br>Kel. '.$Kel;}	
	...
	function genSumHal($SPg, $Kondisi){
		...
		//penilaian ---------------------------------------------		
		$aqry = "select sum(nilai_barang - nilai_barang_asal) as tot from penilaian where idbi_awal in(select idawal from buku_induk where $Kondisi  $kondf );"; $cek .= $aqry;
		...
		//koreksi ----------------------------------------------
		$aqry = "select sum(harga_baru - harga) as tot from t_koreksi where idbi_awal in(select idawal from buku_induk where $Kondisi $kondf );"; $cek .= $aqry;
		$get = mysql_fetch_array(mysql_query($aqry));
		$koreksi = $get['tot'];
		...
		if($jns=='penyusutan' && $Main->PENYUSUTAN){
			...
			//tot susut 
			$tahun_susut = $_REQUEST['tahun_susut'];
			$aqry=" select count(*) as jmlsusut, ifnull(sum(harga),0) as hrgsusut, ifnull(sum(hrg_rehab),0) as hrgrehab from penyusutan where  tahun<$tahun_susut ".
