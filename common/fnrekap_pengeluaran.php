...
class RekapPengeluaranObj extends DaftarObj2{
	var $Prefix = 'rekappengeluaran';
	var $SHOW_CEK = TRUE;	
	var $TblName = 'v1_pengeluaran';
	var $TblName_Hapus = 'pengeluaran_vlama';
	var $TblName_Edit = 'pengeluaran_vlama';
	...
	//var $cetak_xls = TRUE;
	var $fileNameExcel='rekappengeluaran.xls';
	var $Cetak_Judul = 'REKAP PENGELUARAN BARANG MILIK DAERAH';	
	var $Cetak_Mode=2;
	...
	function setTitle(){
		return 'Rekap Pengeluaran Barang Milik Daerah ';		
	}
	function setNavAtas(){
	global $Main;
		...
		if ( $Main->VERSI_NAME == 'JABAR') {
			...
			<a href="pages.php?Pg=rekappengeluaran" style="color:blue;" title="Rekap Daftar Pengeluaran Barang Milik Daerah">Rekap Pengeluaran</a>  
			&nbsp;&nbsp;&nbsp;
			</b></td>
			</tr>
			</table>';	
	...
	function setDaftar_hal($jmlData){
		...
		$get = mysql_fetch_array(mysql_query(
			"select count(*) as cnt from ref_barang where h='00' and i='00' and j ='$kdSubsubkel0'"
		));
		$jmlData = $get['cnt'];
	...
	function genRowSum_setTampilValue($i, $value){
		$valuestr = '';
		if ($i==0){
			$valuestr = number_format($value,0, ',', '.');	
		}else{
	...
	function genDaftarOpsi(){
		...
		*/	
		...
		$fmFiltThnAnggaran = $fmFiltThnAnggaran ==''? '2013' : $fmFiltThnAnggaran;
	...
	function getDaftarOpsi($Mode=1){
		...
		*/
		...
		$fmFiltThnAnggaran = cekPOST('fmFiltThnAnggaran','2013');
...
where aa.g='00' ";*/
		...
		while ( $isi=mysql_fetch_array($qry)){
			...
			$get = mysql_fetch_array(mysql_query(
				"select sum(jml_barang) as jmlbrg, sum(jml_harga) as jmlhrg from $this->TblName ".$Kondisi// and g='".$isi['g']."' "
			));
			...
			while($isig = mysql_fetch_array($qryg)){
				...
				$get = mysql_fetch_array(mysql_query(
					"select sum(jml_barang) as jmlbrg, sum(jml_harga) as jmlhrg from $this->TblName ".$Kondisi2
				));
...
$rekappengeluaran = new RekapPengeluaranObj();
