...
class ReclassObj extends DaftarObj2{
	...
	function simpan2(){
		...
		$cekreklas=mysql_fetch_array(mysql_query("select count(*) as cnt from penghapusan where mutasi=2 and idbi_awal = '$idasal'"));
		if($err=='' && $cekreklas['cnt']>0) $err = "Barang sudah di reklas !";
		...
		$thn_perolehan 	= substr($_REQUEST['tgl_ba'],0,4);//$old['thn_perolehan'];		
		...
		if ($err=='' && sudahClosing($tgl_buku, $c, $d, $e, $e1, $c1)) $err = 'Barang Sudah Closing';
		if($err == ''){
			$ceksusut = mysql_fetch_array(mysql_query("select tgl as tgl_penyusutan from penyusutan where idbi='$idasal' and jns_trans2=30 order by Id desc limit 0,1"));
			if($tgl_buku <= $ceksusut['tgl_penyusutan']) $err = 'Gagal Reklas, sudah penyusutan !';
		}
		if ($err ==''){
			...
			**/
			$hps = mysql_fetch_array(mysql_query(  "select max(tgl_buku) as maxtgl from t_transaksi where idbi = '$idasal'" ));
			if ($err=='' && (compareTanggal( $tgl_buku , $hps['maxtgl']  )==0 
				//|| compareTanggal($tgl_buku, $hps['maxtgl'])==1
			) ) $err = 'Tanggal Reklas harus lebih besar dari Tanggal Transaksi lainnya!';
		...
		if($err==''){			
			...
			$nilai_buku = getNilaiBuku($idasal,$tgl_buku,0);
			$nilai_susut = getAkumPenyusutan($idasal,$tgl_buku);		
			$aqry = 
				...
				" values ('".
				$old['a1']."','".$old['a']."','".$old['b']."','".$old['c1']."','".$old['c']."','".$old['d']."','".$old['e']."','".$old['e1']."','".
			...
			if($qry){				
				...
				**/
				...
				$aqry = "insert into penghapusan ".
					"(id_bukuinduk,a1,a,b,c1,c,d,e,e1,".
					"f,g,h,i,j,noreg,thn_perolehan,".
					"tgl_penghapusan,ket,tahun,".
					"mutasi,sudahmutasi,".
					"uid,tgl_update".
					",staset, idbi_awal,kondisi_akhir,nosk,tglsk,nilai_buku,nilai_susut )values('".
					$idasal."','".$old['a1']."','".$old['a']."','".$old['b']."','".$old['c1']."','".$old['c']."','".$old['d']."','".$old['e']."','".$old['e1']."','".
					$old['f']."','".$old['g']."','".$old['h']."','".$old['i']."','".$old['j']."','".$old['noreg']."','".$old['thn_perolehan']."',".
					"'$tgl_buku','reclass','".$old['thn_perolehan']."',".
					"2,1,'".$uid."','now()'".
					",'".$old['staset']."','".$old['idawal']."','".$old['kondisi']."','$no_ba','$tgl_ba','$nilai_buku','$nilai_susut')";
				$cek .= 'hapus='.$aqry;
	...
	function setForm($dt){	
		...
		//if($Main->URUSAN==1){
			//$selc1 = "c1,";
			//$kondkib = $dt['c1'].$dt['c'].$dt['d'].$dt['e'].$dt['e1'].$dt['f'].$dt['g'].$dt['h'].$dt['i'].$dt['j'].$dt['tahun'].$dt['noreg'];
		//}else{
			//$selc1 = "";
			//$kondkib = $dt['c'].$dt['d'].$dt['e'].$dt['e1'].$dt['f'].$dt['g'].$dt['h'].$dt['i'].$dt['j'].$dt['tahun'].$dt['noreg'];
		//}
		//get tabel kib
		...
		$get = mysql_fetch_array(mysql_query("select * from ref_skpd where $konc1 c='".$dt['c']."' and d='00' "));
		$bidang = $get['nm_skpd'];
		$get=mysql_fetch_array(mysql_query("select * from ref_skpd where $konc1 c='".$dt['c']."' and d='".$dt['d']."' and e='00' "));
		$unit = $get['nm_skpd'];
		$get=mysql_fetch_array(mysql_query("select * from ref_skpd where $konc1 c='".$dt['c']."' and d='".$dt['d']."' and e='".$dt['e']."' and e1='$kdSubUnit0' "));
		$subunit = $get['nm_skpd'];				
		$get=mysql_fetch_array(mysql_query("select * from ref_skpd where $konc1 c='".$dt['c']."' and d='".$dt['d']."' and e='".$dt['e']."' and e1='".$dt['e1']."' "));
		$seksi = $get['nm_skpd'];				
		...
		if($Main->URUSAN==1){
			$vUrusan = array(  'label'=>'URUSAN', 'value'=> $urusan, 'labelWidth'=>120, 'row_params'=>" height='24' ", 'type'=>'' );
		}else{
			$vUrusan = array( 'label'=>'', 'value'=>'', 'type'=>'merge', 'row_params'=>"");
		}
		
		$exp_tgl = explode('-',$dt['tgl_buku']); 
		//$tgl_reclass = $thn_login.'-'.$exp_tgl[1].'-'.$exp_tgl[2]; 
		$tgl_reclass = $thn_login.'-00-00'; 
		...
		$this->form_fields = array(		
			...
			'kondisi' => array(  'label'=>'Kondisi', 'value'=> $Main->KondisiBarang[$dt['kondisi']-1][1], 'row_params'=>" height='24' ", 'type'=>'' ),
			...
			'no_ba' => array( 'label'=> 'Nomor BA', 'value'=> 				
				"<input type='text' name='no_ba' id='no_ba' value='".$dt['no_ba']."' >", 
				'type'=>'' ),
			'tgl_ba' => array( 'label' => 'Tgl. BA', 'value' =>	createEntryTgl("tgl_ba",$dt['tgl_ba']),	'type' => '' ),
			'tgl_buku' => array( 'label' => 'Tgl. Buku', 'value' =>	createEntryTgl("tgl_buku",$tgl_reclass,'','','','',2),	'type' => '' ),
