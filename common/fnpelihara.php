...
function Pelihara_ProsesSimpan(){
	...
	if ($MyModulKU[6]=='1'){	
		...
		$fmSURATTANGGAL = $_GET['fmSURATTANGGAL']==NULL?'0000-00-00':$_GET['fmSURATTANGGAL'];
		...
		//$cara_perolehan = $Main->PENERIMAAN_P19 ==1 ?4:$_GET['cara_perolehan'];
		if ($fmst==0){//edit
			if ($idplh!='')
			{
			$isix =  mysql_fetch_array( mysql_query (				
				" select id_bukuinduk,idbi_awal  from pemeliharaan where id ='$idplh'"
			));
			$idbi_awal=$isix['idbi_awal'];
			$idbi=$isix['id_bukuinduk'];
			}
		...
		$tgl_closing=getTglClosing($get_cd['c'],$get_cd['d'],$get_cd['e'],$get_cd['e1'],$get_cd['c1']); 
		
		//get tglakhir susut,koreksi,penilaian,penghapusan_sebagian dgn idbi_awal yg sama
		/*$tgl_susutAkhir = mysql_fetch_array(mysql_query("select tgl from penyusutan where idbi='$idbi' order by tgl desc limit 1"));
		...
		*///$errmsg="select tgl,tgl_create from t_history_aset where idbi_awal='$idbi' order by tgl desc limit 1";
		...
		$tgl_asetAkhir = mysql_fetch_array(mysql_query("select id,tgl,tgl_create,jns from t_history_aset where idbi_awal='$idbi_awal' order by tgl desc limit 1"));
		...
		if($get_ba['tgl_ba']=='0000-00-00' or $get_ba['tgl_ba']=='' or $get_ba['tgl_ba']==NULL){ 
			$gentgl_perolehan = $get_cd['thn_perolehan'].'-'.date("m", strtotime($get_cd['tgl_buku'])).'-'.date("d",strtotime($get_cd['tgl_buku']));			
		}else{
		...
		//get tglakhir susut,koreksi,penilaian,penghapusan_sebagian dgn idbi_awal yg sama		
			...
			//if($fmTAMBAHASET==1 || $fmTAMBAHMasaManfaat==1){
				if($errmsg =='' && compareTanggal($get_cd['tgl_buku'],$fmTANGGALPEMELIHARAAN)==2) $errmsg = 'Tanggal Buku Pemeliharaan tidak lebih kecil dari Tanggal Buku Barang !';				
				if($errmsg =='' && $thn_Perolehan<$get_cd['thn_perolehan']) $errmsg = 'Tahun Perolehan Pemeliharaan tidak lebih kecil dari Tahun Perolehan Buku Barang !';				
		...
		//if($errmsg=='' && $old_pemelihara['tgl_pemeliharaan']<$tgl_susutAkhir['tgl']){			
			...
			if($errmsg=='' && $old_pemelihara['refid_terima']<>0 || $old_pemelihara['refid_terima']<>NULL){
				if($errmsg=='' && $old_pemelihara['tgl_pemeliharaan']!=$fmTANGGALPEMELIHARAAN)$errmsg = 'Hanya keterangan yang bisa diedit !';
				if($errmsg=='' && $old_pemelihara['tgl_perolehan']!=$fmTANGGALPerolehan)$errmsg = 'Hanya keterangan yang bisa diedit !';
				if($errmsg=='' && $old_pemelihara['jenis_pemeliharaan']!=$fmJENISPEMELIHARAAN)$errmsg = 'Hanya keterangan yang bisa diedit !';
				if($errmsg=='' && $old_pemelihara['pemelihara_instansi']!=$fmPEMELIHARAINSTANSI)$errmsg = 'Hanya keterangan yang bisa diedit !';
				if($errmsg=='' && $old_pemelihara['pemelihara_alamat']!=$fmPEMELIHARAALAMAT)$errmsg = 'Hanya keterangan yang bisa diedit !';
				if($errmsg=='' && $old_pemelihara['no_bast']!=$fmNOMORba)$errmsg = 'Hanya keterangan yang bisa diedit !';
				if($errmsg=='' && $old_pemelihara['surat_no']!=$fmSURATNOMOR)$errmsg = 'Hanya keterangan yang bisa diedit !';
				if($errmsg=='' && $old_pemelihara['surat_tgl']!=$fmSURATTANGGAL)$errmsg = 'Hanya keterangan yang bisa diedit !';
				if($errmsg=='' && $old_pemelihara['biaya_pemeliharaan']!=$fmBIAYA)$errmsg = 'Hanya keterangan yang bisa diedit !';
				if($errmsg=='' && $old_pemelihara['cara_perolehan']!=$cara_perolehan)$errmsg = 'Hanya keterangan yang bisa diedit j! lama='.$old_pemelihara['cara_perolehan'].' baru='.$cara_perolehan;
				if($errmsg=='' && $old_pemelihara['tambah_aset']!=$fmTAMBAHASET)$errmsg = 'Hanya keterangan yang bisa diedit !';
				if($errmsg=='' && $old_pemelihara['tambah_masamanfaat']!=$fmTAMBAHMasaManfaat)$errmsg = 'Hanya keterangan yang bisa diedit !';
			}
			if($errmsg=='' && $old_pemelihara['tgl_pemeliharaan']<=$tgl_closing){ //jika data pemeliharaan dicatat sebelum closing
			...
			//cek tgl pemeliharaan lama < tgl susut akhir 
			if($errmsg=='' && $old_pemelihara['tgl_pemeliharaan']<$tgl_susutAkhir['tgl'] ){ //jika data pemeliharaan di catat sebelum penyusutan
			...
			//cek (tgl tgl_perolehan >= tgl ba buku_induk 
			if($errmsg=='' && $old_pemelihara['tgl_perolehan']<$gentgl_perolehan ){
				if($errmsg=='' && $old_pemelihara['tgl_perolehan']!=$fmTANGGALPerolehan)$errmsg ='Tanggal Perolehan tidak lebih kecil dari Tanggal BAST/Buku Barang !';
			}		
			$thn_pelihara = substr($fmTANGGALPEMELIHARAAN,0,4);
			if($errmsg=='' && $old_pemelihara['cara_perolehan']!=$cara_perolehan){
				if($thn_pelihara>=2017)$errmsg = 'Cara Perolehan tidak bisa di edit,karena tahun buku pemeliharaan lebih dari 2017 !';		
			}
		}
		...
		if($errmsg == ''){	
			if ($fmst==1){//simpan baru
				...
				$aqry = "insert into pemeliharaan (id_bukuinduk, 				
					...
					surat_no, surat_tgl, biaya_pemeliharaan,  ket, idbi_awal, tgl_update, uid ,tambah_aset,tambah_masamanfaat, cara_perolehan,tgl_perolehan,no_bast,tgl_bast ) 
					values ('$idbi',					
						...
						'$idbi_awal', now(), '$UID', '$fmTAMBAHASET' , '$fmTAMBAHMasaManfaat' , '$cara_perolehan' ,'$fmTANGGALPerolehan','$fmNOMORba','$fmTANGGALba'			
					) ";//echo "errmsg=$aqry<r>";
...
function Pelihara_GetData($id){
	global $fmNOREG, $fmTANGGALPEMELIHARAAN, $fmJENISPEMELIHARAAN,
			...
			$cara_perolehan,$fmTANGGALPerolehan,$fmNOMORba,$fmTANGGALba,$refid_terima;
	...
	if ($isi = mysql_fetch_array($qry)){
		...
		$fmTANGGALba = $isi['tgl_bast'];
		$refid_terima = $isi['refid_terima'];
	}
...
if($Main->PENERIMAAN_P19==1 && $cara_perolehan == NULL){
	$pilCaper="Penggabungan<input type='hidden' name='cara_perolehan' id='cara_perolehan' value='4' size='1'>";
}elseif($Main->PENERIMAAN_P19==1 && $cara_perolehan != NULL){
	$pilCaper=$arrCaraPerolehan[$cara_perolehan-1][1]."<input type='hidden' name='cara_perolehan' id='cara_perolehan' value='$cara_perolehan' size='1'>";
}else{
	$cara_perolehan =$cara_perolehan==NULL?'2':$cara_perolehan;
	$pilCaper=cmb2D_v2('cara_perolehan', $cara_perolehan, $arrCaraPerolehan );	
}
$space = formEntryBase2('','','',"","",'','valign="middle" height="8"');


if($refid_terima!=0 || $refid_terima!='' || $refid_terima!=NULL){
	$inpnobast = txtField('fmNOMORba',$fmNOMORba,'100','50','text','readonly');
	$inpnosurat = txtField('fmSURATNOMOR',$fmSURATNOMOR,'100','50','text','readonly');
	$inptglbast = txtField('fmTANGGALbastfake',TglInd($fmTANGGALba),'10','10','text','readonly').'&nbsp;'.txtField('fmTANGGALba',$fmTANGGALba,'10','10','hidden','');
	$inptglsurat = txtField('fmSURATTANGGALfake',TglInd($fmSURATTANGGAL),'10','10','text','readonly').'&nbsp;'.txtField('fmSURATTANGGAL',$fmSURATTANGGAL,'10','10','hidden','');
}else{
	$inpnobast = txtField('fmNOMORba',$fmNOMORba,'100','50','text');
	$inpnosurat = txtField('fmSURATNOMOR',$fmNOMORba,'100','50','text');
	$inptglbast = createEntryTgl('fmTANGGALba', $fmTANGGALba, false, '', '','FormPelihara');
	$inptglsurat = createEntryTgl('fmSURATTANGGAL', $fmSURATTANGGAL, false, '', '','FormPelihara');
}
	return "
		<A NAME='FORMENTRY'></A>
	...
	".formEntryBase2(' Berata Acara Serah Terima','','',"style=''","style=''",'','valign="middle" height="21"')."
	".formEntryBase2('&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Nomor Bast',':',
		$inpnobast
		,"style=''","style=''",'','valign="middle" height="21"')."
	".formEntryBase2('&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Tanggal Bast',':',
				$inptglbast,'','','','')."		
...
function Pelihara_List($Tbl, $Fields='*', $Kondisi='', $Limit='', $Order='', $Style=1, $TblStyleClass='koptable', 
	...
	while ($isi = mysql_fetch_array($Qry)){
		...
		$kdKelBarang = $Main->KD_BARANG_P108?$isi['f1'].$isi['f2'].$isi['f'].$isi['g']."00":$isi['f'].$isi['g']."00";
		$concatkd = $Main->KD_BARANG_P108?"f1,f2,f,g,h,i,j":"f,g,h,i,j";
		$nmBarang = mysql_fetch_array(mysql_query("select * from ref_barang where concat($concatkd)='$kdBarang'"));
		...
		$isiF = $Main->KD_BARANG_P108?$isi['f1'].'.'.$isi['f2'].'.'.$isi['f'].'.'.$isi['g'].'.'.$isi['h'].'.'.$isi['i'].'.'.$isi['j']:$isi['f'].'.'.$isi['g'].'.'.$isi['h'].'.'.$isi['i'].'.'.$isi['j'];
		$List_ = $Style==2?
			"<td class=\"$TdStyle\" align=center>$isiF<br>{$isi['id_bukuinduk']}<br>{$isi['idbi_awal']}</td>
		...
		*/
		...
		$retensi = $isi['refid_retensi']==''?'':'Retensi';			
		if ($xls){
		$List_ = 
			"<td class=\"$TdStyle\" align=center>$isiF<br>{$isi['id_bukuinduk']}<br>{$isi['idbi_awal']}</td>
		...
		$ListData .= "	
			...
			"<td class=\"$TdStyle\">{$isi['ket']}<br>{$isi['surat_no']}$tgl_surat<br>{$isi['pemelihara_instansi']}<br>".$retensi."</td>
			$TampilCheckBoxKol2
			</tr>
		";
	...
	if ($Cetak){
		$totalHal = "
			<tr class='row0'>
			<td colspan=9 class=\"$TdStyle\">Total </td>
			<td align=right class=\"$TdStyle\"><b>".number_format(($jmlTotalHargaDisplay), 2, ',', '.')."</td>
			<td colspan=3  class=\"$TdStyle\">&nbsp;</td>
			</tr>";	
	}
	
	
	//menu ----------------------------------------------------------
	$menuhps = "<li><a href='javascript:PeliharaHapus.Hapus()' title='Hapus Pemeliharaan' class='btdel'></a></li>";
	if ($Style==1){
	...
	for($i = 0; $i<count($cidPLH); $i++)	{
		...
		if($errmsg ==''){
			...
			$pelihara = mysql_fetch_array(mysql_query(
				"select idbi_awal, tgl_pemeliharaan, id_bukuinduk, refid_terima from pemeliharaan where id='{$cidPLH[$i]}'"
			));		
			//cek status barangnya
			$penatausaha = mysql_fetch_array(mysql_query(
				"select status_barang,c1,c,d,e,e1 from buku_induk where id='{$pelihara['id_bukuinduk']}'"
			));
			...
			$idterima = $pelihara['refid_terima'];
			$thn_pelihara = substr($fmTANGGALPEMELIHARAAN,0,4);
			...
			if($idterima!=NULL || $idterima!=''){
				$errmsg="Data tidak bisa di hapus,Sudah ada penerimaan !";
			}
			//cek sudah ada Closing untuk data baru
			if(sudahClosing($fmTANGGALPEMELIHARAAN,$penatausaha['c'],$penatausaha['d'],$penatausaha['e'],$penatausaha['e1'],$penatausaha['c1'])){
			...
			return "
				...
				PeliharaSimpan= new AjxSimpanObj('PeliharaSimpan','PeliharaSimpan_cover',
					new Array('fmTANGGALPEMELIHARAAN','fmJENISPEMELIHARAAN','fmPEMELIHARAINSTANSI','cara_perolehan',
						...
						'fmKET','fmTAMBAHASET','fmTAMBAHMasaManfaat','fmTANGGALPerolehan','fmNOMORba','idbi','idbi_awal','idplh','fmst','fmTANGGALba'),
		...
		case 2:{
			...
			return "
				...
				PeliharaSimpan= new AjxSimpanObj('PeliharaSimpan','PeliharaSimpan_cover',
					new Array('fmTANGGALPEMELIHARAAN','fmJENISPEMELIHARAAN','fmPEMELIHARAINSTANSI','cara_perolehan',
						'fmPEMELIHARAALAMAT','fmSURATNOMOR','fmSURATTANGGAL','fmBIAYA',
						'fmKET','fmTAMBAHASET','fmTAMBAHMasaManfaat','fmTANGGALPerolehan','fmNOMORba','idplh','idbi_awal','fmst','fmTANGGALba'),
